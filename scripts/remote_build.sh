...
#
# Defaults:
# - Config file: ~/.config/arterm/remote-build.env (override with ARTERM_REMOTE_CONFIG)
# - Host: ARTERM_REMOTE_HOST from env/config, or --host
# - Remote dir: .cache/remote-builds/arterm/<repo-name> (override with ARTERM_REMOTE_DIR or --remote-dir)
#
# Examples:
#   scripts/remote_build.sh --release
#   scripts/remote_build.sh test
#   scripts/remote_build.sh check --all-targets
#   scripts/remote_build.sh --host mybox --remote-dir ~/src/arterm test -- --nocapture
...
Options:
  ...
  --host HOST          Remote SSH host (default: $ARTERM_REMOTE_HOST from env/config; required if unset)
  --remote-dir DIR     Remote project directory (default: $ARTERM_REMOTE_DIR or .cache/remote-builds/arterm/<repo-name>)
...
Behavior:
  ...
  - For build sync-back, copies target/{debug|release}/<artifact> from remote to local
    (artifact defaults to 'arterm', or '--bin <name>' when provided)
  - Default config file is ~/.config/arterm/remote-build.env
EOF
}
...
REMOTE="${ARTERM_REMOTE_HOST:-}"
REMOTE_DIR="${ARTERM_REMOTE_DIR:-.cache/remote-builds/arterm/${REPO_NAME}}"
SSH_BIN="${ARTERM_REMOTE_SSH_BIN:-ssh}"
RSYNC_BIN="${ARTERM_REMOTE_RSYNC_BIN:-rsync}"
...
if [[ -z "$REMOTE" ]]; then
    echo "error: remote host not configured; set ARTERM_REMOTE_HOST or pass --host HOST" >&2
    exit 2
fi
...
SSH_SERVER_ALIVE_INTERVAL="${ARTERM_REMOTE_SERVER_ALIVE_INTERVAL:-10}"
SSH_SERVER_ALIVE_COUNT_MAX="${ARTERM_REMOTE_SERVER_ALIVE_COUNT_MAX:-1}"
SSH_OPTS=(
    -o BatchMode=yes
...
RSYNC_SSH_COMMAND="${ARTERM_REMOTE_RSYNC_SSH:-$SSH_BIN -o BatchMode=yes -o ConnectTimeout=$SSH_CONNECT_TIMEOUT -o ServerAliveInterval=$SSH_SERVER_ALIVE_INTERVAL -o ServerAliveCountMax=$SSH_SERVER_ALIVE_COUNT_MAX}"
...
if ! preflight_output="$(remote_ssh "printf 'arterm-remote-ok\\n'" 2>&1)"; then
    echo "error: remote host '$REMOTE' is not reachable within ${SSH_CONNECT_TIMEOUT}s" >&2
    echo "$preflight_output" >&2
    echo "hint: set ARTERM_REMOTE_CARGO=0 to force local cargo, or fix ARTERM_REMOTE_HOST/ARTERM_REMOTE_CONNECT_TIMEOUT." >&2
    exit 75
fi
...
if [[ "$SYNC_SOURCE" -eq 1 ]]; then
    ...
    trap 'rm -f "$metadata_file"' EXIT
    {
        ...
        printf 'git_tag=%s\n' "$local_git_tag"
        printf 'git_dirty=%s\n' "$local_git_dirty"
        printf 'changelog_raw<<ARTERM_CHANGELOG_EOF\n%s\nARTERM_CHANGELOG_EOF\n' "$local_changelog_raw"
    } > "$metadata_file"
    "$RSYNC_BIN" -avz -e "$RSYNC_SSH_COMMAND" "$metadata_file" "$REMOTE:$REMOTE_DIR/.arterm-build-meta"
else
    echo ""
...
printf -v REMOTE_INNER_CMD 'cd %q && env ARTERM_BUILD_METADATA_FILE=.arterm-build-meta %s' "$REMOTE_DIR" "$REMOTE_CARGO_CMD"
