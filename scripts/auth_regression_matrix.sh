...
bin=${ARTERM_AUTH_MATRIX_BIN:-}
out_dir=${ARTERM_AUTH_MATRIX_OUT:-"$repo_root/target/auth-test-reports"}
prompt=${ARTERM_AUTH_MATRIX_PROMPT:-"Reply with exactly AUTH_TEST_OK and nothing else. Do not call tools."}
providers=${ARTERM_AUTH_MATRIX_PROVIDERS:-"claude copilot openrouter deepseek zai alibaba-coding-plan openai-compatible"}
mode=${ARTERM_AUTH_MATRIX_MODE:-configured}
keep_going=${ARTERM_AUTH_MATRIX_KEEP_GOING:-1}
per_command_timeout=${ARTERM_AUTH_MATRIX_TIMEOUT:-90}

usage() {
  cat <<'EOF'
Usage: scripts/auth_regression_matrix.sh [options]

Runs arterm auth-test across the auth/provider matrix and writes one JSON report per provider.
...
Options:
  ...
  --bin PATH            arterm binary to run (default: cargo run --bin arterm --)
...
Environment equivalents:
  ARTERM_AUTH_MATRIX_BIN=/path/to/arterm
  ARTERM_AUTH_MATRIX_OUT=target/auth-test-reports
  ARTERM_AUTH_MATRIX_PROVIDERS="claude deepseek zai"
  ARTERM_AUTH_MATRIX_MODE=configured|all
  ARTERM_AUTH_MATRIX_LOGIN=1
  ARTERM_AUTH_MATRIX_NO_SMOKE=1
  ARTERM_AUTH_MATRIX_NO_TOOL_SMOKE=1
  ARTERM_AUTH_MATRIX_KEEP_GOING=0
  ARTERM_AUTH_MATRIX_TIMEOUT=90

Examples:
  scripts/auth_regression_matrix.sh --configured --no-smoke
  scripts/auth_regression_matrix.sh --provider deepseek --provider zai
  ARTERM_AUTH_MATRIX_BIN=target/selfdev/arterm scripts/auth_regression_matrix.sh --all
EOF
}
...
if [[ "${ARTERM_AUTH_MATRIX_LOGIN:-0}" == "1" ]]; then
  extra_args+=(--login)
fi
if [[ "${ARTERM_AUTH_MATRIX_NO_SMOKE:-0}" == "1" ]]; then
  extra_args+=(--no-smoke)
fi
if [[ "${ARTERM_AUTH_MATRIX_NO_TOOL_SMOKE:-0}" == "1" ]]; then
  extra_args+=(--no-tool-smoke)
fi
...
run_arterm() {
  if [[ -n "$bin" ]]; then
    ...
    timeout "$per_command_timeout" cargo run --quiet --bin arterm -- "$@"
  fi
}
...
if [[ "$mode" == "configured" ]]; then
  ...
  if ! run_arterm auth-test --all-configured --no-smoke --no-tool-smoke --json --output "$configured_json" >/tmp/arterm-auth-matrix-discovery.out 2>/tmp/arterm-auth-matrix-discovery.err; then
    if [[ -s "$configured_json" ]]; then
      ...
      cat /tmp/arterm-auth-matrix-discovery.err >&2 || true
