...
def main() -> int:
    ...
    ap.add_argument("--binary",
                    default=str(REPO_ROOT / "target" / "selfdev" / "arterm"))
    ap.add_argument("--watch-s", type=float, default=2.0)
    args = ap.parse_args()

    flick.ROWS, flick.COLS = 48, 160
    binary = str(Path(args.binary).resolve())
    scratch = Path(os.environ.get("ARTERM_SCRATCH_DIR") or tempfile.gettempdir())
    root = Path(tempfile.mkdtemp(prefix="arterm-donut-verify-", dir=str(scratch)))
    ...
    env = os.environ.copy()
    env.update({
        "ARTERM_HOME": str(home), "ARTERM_RUNTIME_DIR": str(run),
        "ARTERM_SOCKET": str(run / "arterm.sock"), "ARTERM_NO_TELEMETRY": "1",
        "ARTERM_DEBUG_CONTROL": "1", "ARTERM_TEMP_SERVER": "1",
        "ARTERM_SERVER_OWNER_PID": str(os.getpid()), "ARTERM_PERF_TIER": "full",
        "ARTERM_THEME": "dark",
        # The donut is opt-in since the config default flipped to false
        # (17e075fb2), and this verifier's temp ARTERM_HOME gets the default
        # config. Without the explicit opt-in the "animation expected" half of
        # this verifier silently verifies nothing.
        "ARTERM_IDLE_ANIMATION": "1",
    })
    ...
    failures: list[str] = []
    try:
        flick.wait_for_socket(Path(env["ARTERM_SOCKET"]))
