...
case "$url" in
  ...
  *arterm.sh/releases/v1.2.3/SHA256SUMS)
    ...
    printf '%s  %s\n' "$checksum" "${TEST_CHECKSUM_ASSET:-arterm-linux-x86_64.tar.gz}"
    ;;
...
artifact="${TEST_ARCHIVE_ARTIFACT:-arterm-linux-x86_64}"
cat > "$dest/$artifact" <<'BIN'
#!/usr/bin/env bash
if [ "${1:-}" = "--version" ]; then printf 'arterm 1.2.3\n'; fi
...
TEST_ARCHIVE_ARTIFACT=arterm-windows-aarch64.exe \
TEST_CHECKSUM_ASSET=arterm-windows-aarch64.tar.gz \
