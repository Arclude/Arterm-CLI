#!/usr/bin/env bash
# Mutation sweep for arterm-desktop2.
#
...
PYEOF
    ...
    local failures=0
    if cargo test --profile selfdev -p arterm-desktop2 --quiet >/dev/null 2>&1; then
        if [ "$gpu" -eq 1 ]; then
            if ! cargo test --profile selfdev -p arterm-desktop2 --quiet -- --ignored \
                >/dev/null 2>&1; then
                failures=1
            fi
        fi
