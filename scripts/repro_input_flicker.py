#!/usr/bin/env python3
"""
Reproduce the "typed character flickers in and out" bug in a live arterm TUI.

The symptom
-----------
Spawn a fresh arterm, press `/`, and the slash appears, vanishes, and reappears.
...
class EmulatedClient:
    """A live arterm under a PTY, with its output rendered into a real screen.
...
def launch_client(binary: str, env: dict, session_id: str,
    ...
    cenv = dict(env)
    cenv["ARTERM_DEBUG_CMD_PATH"] = str(cmd_path)
    cenv["ARTERM_DEBUG_RESPONSE_PATH"] = str(resp_path)
    ...
    cenv.setdefault("ARTERM_THEME", "dark")
    proc = subprocess.Popen(
        [binary, "--no-update", "--no-selfdev",
         "--socket", env["ARTERM_SOCKET"], "--resume", session_id],
...
def main() -> int:
    ...
    default_bin = REPO_ROOT / "target" / "selfdev" / "arterm"
    if not default_bin.exists():
        default_bin = Path.home() / ".arterm" / "builds" / "current" / "arterm"
    ...
    root = Path(tempfile.mkdtemp(prefix="arterm-input-flicker-"))
    ...
    env = os.environ.copy()
    if args.live:
        real_runtime = Path(env.get("ARTERM_RUNTIME_DIR") or f"/run/user/{os.getuid()}")
        env["ARTERM_SOCKET"] = env.get("ARTERM_SOCKET") or str(real_runtime / "arterm.sock")
        env["ARTERM_DEBUG_CONTROL"] = "1"
        debug_sock = real_runtime / "arterm-debug.sock"
    else:
        env["ARTERM_HOME"] = str(home)
        env["ARTERM_RUNTIME_DIR"] = str(run)
        env["ARTERM_SOCKET"] = str(run / "arterm.sock")
        env["ARTERM_NO_TELEMETRY"] = "1"
        env["ARTERM_DEBUG_CONTROL"] = "1"
        env["ARTERM_TEMP_SERVER"] = "1"
        env["ARTERM_SERVER_OWNER_PID"] = str(os.getpid())
        if not env.get("ANTHROPIC_API_KEY"):
            env["ANTHROPIC_API_KEY"] = "sk-ant-repro-input-flicker"
        debug_sock = run / "arterm-debug.sock"
    cmd_path, resp_path = run / "client_cmd", run / "client_resp"

    if not args.json:
        print("== arterm input-flicker repro ==")
    ...
    if not args.live:
        server = subprocess.Popen(
            [binary, "serve", "--socket", env["ARTERM_SOCKET"], "--debug-socket",
    ...
    try:
        wait_for_socket(Path(env["ARTERM_SOCKET"]))
