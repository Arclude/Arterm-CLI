...
def main() -> int:
    ...
    ap.add_argument("--binary", default=str(REPO_ROOT / "target" / "selfdev" / "arterm"))
    ...
    runtime = Path(os.environ.get("ARTERM_RUNTIME_DIR") or f"/run/user/{os.getuid()}")
    env = os.environ.copy()
    ...
    root = Path(tempfile.mkdtemp(prefix="arterm-profile-real-", dir=str(scratch)))
