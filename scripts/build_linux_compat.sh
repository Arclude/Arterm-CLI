...
artifact="${ARTERM_COMPAT_ARTIFACT:-arterm-linux-x86_64}"
profile="${ARTERM_COMPAT_PROFILE:-release}"
image="${ARTERM_COMPAT_IMAGE:-quay.io/pypa/manylinux2014_x86_64}"
cache_root="${ARTERM_COMPAT_CACHE_DIR:-$HOME/.cache/arterm-linux-compat}"
...
# metadata file (read by arterm-build-meta/build.rs through
# ARTERM_BUILD_METADATA_FILE). The repo is bind-mounted into the container and
...
docker run --rm \
  ...
  -e ARTERM_RELEASE_BUILD="${ARTERM_RELEASE_BUILD:-1}" \
  -e ARTERM_BUILD_SEMVER="${ARTERM_BUILD_SEMVER:-}" \
  -e ARTERM_BUILD_METADATA_FILE=/arterm-build-meta \
  -e ARTERM_BUILD_GIT_HASH="$git_hash" \
  -e ARTERM_BUILD_GIT_DATE="$git_date" \
  -e ARTERM_BUILD_GIT_TAG="$git_tag" \
  -e ARTERM_BUILD_GIT_DIRTY="$git_dirty" \
  -e ARTERM_COMPAT_PROFILE="$profile" \
  -e ARTERM_COMPAT_TARGET="$target" \
  -e HOST_UID="$host_uid" \
  -e HOST_GID="$host_gid" \
  -v "$repo_root:/work" \
  -v "$metadata_file:/arterm-build-meta:ro" \
  -v "$out_dir:/out" \
  ...
  bash -lc '
    ...
    if [[ ! -x /root/.cargo/bin/cargo ]]; then
      curl https://sh.rustup.rs -sSf | sh -s -- -y --profile minimal --default-toolchain stable
	    ...
	    # (ARTERM_BUILD_METADATA_FILE=/arterm-build-meta) is the primary source of
	    ...
	    cargo build --profile "$ARTERM_COMPAT_PROFILE" --target "$ARTERM_COMPAT_TARGET" \
	      -p arterm --bin arterm --features linux-compat-vendored-openssl

	    cp "$CARGO_TARGET_DIR/$ARTERM_COMPAT_TARGET/$ARTERM_COMPAT_PROFILE/arterm" "/out/'"$artifact"'.bin"
...
WRAPPER
	    ...
	    # these sibling libraries and sets LD_LIBRARY_PATH for the arterm process.
...
# `arterm-build-meta`'s build script deliberately does not watch .git/HEAD, so a
