...
$testRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("arterm-windows-setup-eval-{0}" -f ([guid]::NewGuid().ToString('N')))
...
$envNames = @(
    ...
    'TMP',
    'PATH',
    'ARTERM_INSTALL_PS1_IMPORT_ONLY',
    'ARTERM_UNINSTALL_PS1_IMPORT_ONLY',
    'ARTERM_SKIP_SERVER_RELOAD',
    'ARTERM_INSTALL_SKIP_BINARY_VALIDATION',
    'ARTERM_DISABLE_ENV_BROADCAST',
    'ARTERM_WINDOWS_SETUP_SKIP_EXTERNALS'
)
$originalEnv = @{}
...
function New-IsolatedWindowsProfile([string]$Name) {
    ...
    $artermHome = Join-Path $root 'Arterm Home'
    $temp = Join-Path $root 'Temp'
    foreach ($dir in @($local, $profile, $appData, $artermHome, $temp)) {
    ...
    return [pscustomobject]@{
        ...
        UserProfile = $profile
        AppData = $appData
        ArtermHome = $artermHome
        Temp = $temp
        InstallDir = Join-Path $local 'arterm\bin'
        LauncherPath = Join-Path $local 'arterm\bin\arterm.exe'
        BuildsDir = Join-Path $local 'arterm\builds'
        SetupHintsPath = Join-Path $artermHome 'setup_hints.json'
        HotkeyDir = Join-Path $artermHome 'hotkey'
        StartupShortcutPath = Join-Path $appData 'Microsoft\Windows\Start Menu\Programs\Startup\arterm-hotkey.lnk'
    }
}

function Set-InstallScriptProfileGlobals($Profile) {
    $script:ArtermHome = $Profile.ArtermHome
...
try {
    ...
    $env:ARTERM_SKIP_SERVER_RELOAD = '1'
    $env:ARTERM_INSTALL_SKIP_BINARY_VALIDATION = '1'
    ...
    Invoke-Case 'path_persistence_and_deduplication' {
        ...
        $pathUpdate = Resolve-ArtermPathUpdate -InstallDir $profile.InstallDir -CurrentPath $currentPath
        ...
        Assert-PathCount $pathUpdate.Path $profile.InstallDir 1 'updated PATH should contain exactly one arterm launcher dir'
        ...
        $first = Set-ArtermUserPath -InstallDir $profile.InstallDir -CurrentPath 'C:\Tools' -SetUserPathAction $setPathAction -BroadcastAction $broadcastAction
        ...
        $second = Set-ArtermUserPath -InstallDir $profile.InstallDir -CurrentPath $script:appliedPath -SetUserPathAction $setPathAction -BroadcastAction $broadcastAction
    ...
    Invoke-Case 'wm_settingchange_broadcast_contract' {
        ...
        $env:ARTERM_DISABLE_ENV_BROADCAST = '1'
        Assert-Equal $false (Send-ArtermEnvironmentChangedBroadcast) 'broadcast helper should honor the deterministic no-mutate opt-out'
        $env:ARTERM_DISABLE_ENV_BROADCAST = $null
    ...
    function Set-ArtermUserPath {
        ...
        $update = Resolve-ArtermPathUpdate -InstallDir $InstallDir -CurrentPath $script:mockUserPath
        if ($update.Changed) {
    ...
    Invoke-Case 'clean_install_isolated_profile_and_opt_out' {
        ...
        $source = Join-Path $profile.Root 'arterm-v1.exe'
        ...
        Assert-PathExists (Join-Path $profile.BuildsDir 'stable\arterm.exe') 'clean install should populate the stable build channel'
        Assert-PathExists (Join-Path $profile.BuildsDir 'versions\0.0.1-eval\arterm.exe') 'clean install should populate the immutable versioned build'
        ...
        Assert-PathExists $profile.SetupHintsPath 'clean install should write setup hints into isolated ARTERM_HOME'
    ...
    Invoke-Case 'upgrade_and_idempotency_do_not_duplicate_path' {
        ...
        Set-InstallScriptProfileGlobals $profile
        $sourceV1 = Join-Path $profile.Root 'arterm-v1.exe'
        $sourceV2 = Join-Path $profile.Root 'arterm-v2.exe'
    ...
    Invoke-Case 'copilot_key_mapping_and_spaces_non_ascii_paths' {
        ...
        Set-InstallScriptProfileGlobals $profile
        $env:ARTERM_WINDOWS_SETUP_SKIP_EXTERNALS = '1'
        $artermExe = Join-Path $profile.Root '路径 With Spaces\arterm.exe'
        New-Item -ItemType Directory -Path (Split-Path -Parent $artermExe) -Force | Out-Null
        Set-Content -Path $artermExe -Value 'fake exe' -NoNewline
        New-Item -ItemType Directory -Path $profile.HotkeyDir -Force | Out-Null
        Set-Content -Path (Join-Path $profile.HotkeyDir 'arterm-hotkey.ps1') -Value 'legacy listener' -Force

        $ok = Install-ArtermHotkey -ArtermExePath $artermExe
        Assert-Equal $true $ok 'hotkey install should succeed using the deterministic external-command skip hook'
        $vbsPath = Join-Path $profile.HotkeyDir 'arterm-hotkey-launcher.vbs'
        Assert-PathMissing $vbsPath 'hotkey install should remove the legacy hidden VBScript trampoline'
        $shortcutScriptPath = Join-Path $profile.HotkeyDir 'arterm-hotkey-shortcut.ps1'
        Assert-PathExists $shortcutScriptPath 'hotkey install should render the deterministic Startup shortcut script under the isolated ARTERM_HOME'
        ...
        Assert-Contains $shortcutScript $artermExe 'Startup shortcut should preserve spaces and non-ASCII characters in the arterm path'
        Assert-PathMissing (Join-Path $profile.HotkeyDir 'arterm-hotkey.ps1') 'hotkey upgrade should remove the legacy PowerShell listener'
        $scriptText = Get-Content -LiteralPath $installScript -Raw
    ...
    Invoke-Case 'missing_windows_terminal_is_not_required' {
        ...
        $artermExe = Join-Path $profile.Root 'No Windows Terminal\arterm.exe'
        New-Item -ItemType Directory -Path (Split-Path -Parent $artermExe) -Force | Out-Null
        Set-Content -Path $artermExe -Value 'fake exe' -NoNewline

        Assert-Equal $true (Install-ArtermHotkey -ArtermExePath $artermExe) 'hotkey setup should not require Windows Terminal to be installed or active'
        $scriptText = Get-Content -LiteralPath $installScript -Raw
    ...
    Invoke-Case 'launcher_rollback_failure_preserves_existing_install' {
        ...
        $threw = $false
        try {
            Install-ArtermLauncher -SourcePath $missingSource -LauncherPath $profile.LauncherPath | Out-Null
        } catch {
            $threw = $true
        }
        ...
        $tempLaunchers = @(Get-ChildItem -LiteralPath $profile.InstallDir -Filter '.arterm-launcher-*.tmp.exe' -Force -ErrorAction SilentlyContinue)
    ...
    $env:ARTERM_UNINSTALL_PS1_IMPORT_ONLY = '1'
    . $uninstallScript
    ...
    function Remove-ArtermUserPath {
        ...
        $update = Resolve-ArtermPathRemoval -InstallDir $InstallDir -CurrentPath $script:uninstallUserPath
        if ($update.Changed) {
    ...
    Invoke-Case 'uninstall_guards_purge_paths_and_process_scope' {
        ...
        Assert-Equal $true (Test-ArtermSafePurgePath $profile.ArtermHome) 'dedicated arterm data directories should be purgeable'
        Assert-Equal $false (Test-ArtermSafePurgePath $profile.UserProfile) 'the user profile must never be accepted as a purge target'
        Assert-Equal $false (Test-ArtermSafePurgePath $profile.Root) 'a parent workspace must never be accepted as a purge target'
        Assert-Equal $true (Test-ArtermManagedExecutablePath -ExecutablePath $profile.LauncherPath -LauncherPath $profile.LauncherPath -BuildsDir $profile.BuildsDir) 'the installed launcher should be recognized as managed'
        Assert-Equal $true (Test-ArtermManagedExecutablePath -ExecutablePath (Join-Path $profile.InstallDir '.arterm-launcher-old-a1b2c3.exe') -LauncherPath $profile.LauncherPath -BuildsDir $profile.BuildsDir) 'a renamed live-upgrade launcher should be recognized as managed'
        Assert-Equal $false (Test-ArtermManagedExecutablePath -ExecutablePath (Join-Path $profile.InstallDir 'other-tool.exe') -LauncherPath $profile.LauncherPath -BuildsDir $profile.BuildsDir) 'unrelated executables beside the launcher must not be terminated'
        Assert-Equal $true (Test-ArtermManagedExecutablePath -ExecutablePath (Join-Path $profile.BuildsDir 'stable\arterm.exe') -LauncherPath $profile.LauncherPath -BuildsDir $profile.BuildsDir) 'installed version binaries should be recognized as managed'
        Assert-Equal $false (Test-ArtermManagedExecutablePath -ExecutablePath (Join-Path $profile.Root 'development\arterm.exe') -LauncherPath $profile.LauncherPath -BuildsDir $profile.BuildsDir) 'unrelated development binaries must not be terminated'
    }
    ...
    Invoke-Case 'uninstall_cleanup_removes_binaries_path_and_keeps_user_data' {
        ...
        New-Item -ItemType Directory -Path $profile.ArtermHome -Force | Out-Null
        New-Item -ItemType Directory -Path $profile.HotkeyDir -Force | Out-Null
        ...
        $oldLauncherPath = Join-Path $profile.InstallDir '.arterm-launcher-old-a1b2c3.exe'
        Set-Content -Path $oldLauncherPath -Value 'previous running launcher' -NoNewline
        Set-Content -Path (Join-Path $profile.BuildsDir 'stable\arterm.exe') -Value 'stable build' -NoNewline
        Set-Content -Path (Join-Path $profile.ArtermHome 'config.toml') -Value 'kept = true' -NoNewline
        Set-Content -Path (Join-Path $profile.HotkeyDir 'arterm-hotkey.ps1') -Value 'legacy listener' -NoNewline
        ...
        $exitCode = Invoke-ArtermUninstall -InstallDir $profile.InstallDir -Yes
        ...
        Assert-PathMissing (Join-Path $profile.HotkeyDir 'arterm-hotkey.ps1') 'uninstall should remove legacy launch-hotkey artifacts'
        Assert-PathExists (Join-Path $profile.ArtermHome 'config.toml') 'uninstall without -Purge should keep user data'
        ...
        Assert-Equal 'C:\Keep' $script:uninstallUserPath 'uninstall should remove all arterm-managed PATH variants and keep unrelated entries'
