...
start_rust_action_log() {
  case "${ARTERM_RUST_ACTION_LOG:-1}" in
    0|false|no|off) return ;;
  esac

  local state_root="${ARTERM_HOME:-${HOME:+$HOME/.arterm}}"
  [[ -n "$state_root" ]] || state_root="$repo_root/target/arterm-state"
  rust_action_log_path="${ARTERM_RUST_ACTION_LOG_PATH:-$state_root/logs/rust-actions.jsonl}"
...
record_rust_action_log() {
  ...
  ARTERM_LOG_STARTED_AT="$rust_action_log_started_at" \
  ARTERM_LOG_DURATION_MS="$duration_ms" \
  ARTERM_LOG_GATE_WAIT_MS="$cargo_gate_wait_ms" \
  ARTERM_LOG_EXIT_CODE="$exit_code" \
  ARTERM_LOG_ACTION="$action" \
  ARTERM_LOG_PROFILE="$profile" \
  ARTERM_LOG_REPO="$repo_root" \
  ARTERM_LOG_EXECUTION="$rust_action_log_execution" \
...
path = sys.argv[1]
record = {
    "started_at": os.environ["ARTERM_LOG_STARTED_AT"],
    "duration_ms": int(os.environ["ARTERM_LOG_DURATION_MS"]),
    "gate_wait_ms": int(os.environ["ARTERM_LOG_GATE_WAIT_MS"]),
    "execution_duration_ms": max(
        0,
        int(os.environ["ARTERM_LOG_DURATION_MS"])
        - int(os.environ["ARTERM_LOG_GATE_WAIT_MS"]),
    ),
    "exit_code": int(os.environ["ARTERM_LOG_EXIT_CODE"]),
    "success": os.environ["ARTERM_LOG_EXIT_CODE"] == "0",
    "action": os.environ["ARTERM_LOG_ACTION"],
    "profile": os.environ["ARTERM_LOG_PROFILE"],
    "repository": os.environ["ARTERM_LOG_REPO"],
    "execution": os.environ["ARTERM_LOG_EXECUTION"],
    "argv": sys.argv[2:],
}
...
configure_build_tmpdir() {
  local candidate="${ARTERM_BUILD_TMPDIR:-}"
  if [[ -n "$candidate" ]]; then
    ...
    return
  elif [[ -n "${ARTERM_SCRATCH_DIR:-}" ]]; then
    candidate="$ARTERM_SCRATCH_DIR/cargo-tmp"
    build_tmpdir_status="arterm-scratch"
  elif [[ -n "${ARTERM_HOME:-}" ]]; then
    candidate="$ARTERM_HOME/scratch/cargo-tmp"
    build_tmpdir_status="arterm-home"
  elif [[ -n "${HOME:-}" ]]; then
    candidate="$HOME/.arterm/scratch/cargo-tmp"
    build_tmpdir_status="home-scratch"
  else
    candidate="$repo_root/target/arterm-scratch/cargo-tmp"
...
maybe_enable_sccache() {
  ...
  # caller explicitly forces it via ARTERM_SCCACHE=1/on/force.
  local force_sccache="${ARTERM_SCCACHE:-auto}"
  case "$force_sccache" in
    1|true|yes|on|force) force_sccache="1" ;;
    0|false|no|off|never)
      sccache_status="disabled-by-arterm-sccache"
      log "sccache disabled by ARTERM_SCCACHE"
      return
      ;;
    *) force_sccache="auto" ;;
  esac
  ...
  if [[ "$force_sccache" != "1" ]] && build_is_incremental "$@"; then
    ...
    log "sccache skipped for incremental build (it cannot cache incremental units; set ARTERM_SCCACHE=on to force, or use a non-incremental profile)"
    return
  fi
...
validate_feature_profile() {
  local profile="${ARTERM_DEV_FEATURE_PROFILE:-default}"
  case "$profile" in
    ""|default|minimal|none|pdf|embeddings|full)
      ;;
    *)
      printf 'error: unsupported ARTERM_DEV_FEATURE_PROFILE=%s (expected default|minimal|pdf|embeddings|full)\n' "$profile" >&2
      exit 1
      ;;
  esac
}
...
selfdev_low_memory_default_needed() {
  ...
  # On small no-swap machines, earlyoom can terminate the root arterm rustc
...
#
# After the monolith was split into the arterm-base/app-core/tui/cli crate DAG,
# the largest single rustc unit is arterm-base, which has grown back to a
...
# coarse sample, and arterm-base keeps growing) so a fresh build under load backs
...
# backs off further. An explicit CARGO_BUILD_JOBS / ARTERM_BUILD_JOBS always
# wins, and non-Linux hosts fall back to the cargo/.cargo default.
select_build_jobs() {
  # Respect an explicit override from either env var.
  local override="${ARTERM_BUILD_JOBS:-${CARGO_BUILD_JOBS:-}}"
  if [[ -n "$override" ]]; then
  ...
  # rustc unit (arterm-base, ~1.6 GiB RSS sampled) so an idle machine uses nearly
  # every core while a memory-pressured one backs off before earlyoom kills a
  # build. Tunable per host via ARTERM_BUILD_MIB_PER_JOB.
  mib_per_job_default=1792
  mib_per_job="${ARTERM_BUILD_MIB_PER_JOB:-$mib_per_job_default}"
  ...
  if (( jobs < cpus )); then
    log "limiting cargo to ${jobs} job(s) under memory pressure (${mem_available_mib}MiB available, ~${mib_per_job}MiB/job); override with ARTERM_BUILD_JOBS"
  fi
}

# Keep `arterm-build-meta`'s embedded git metadata in lockstep with HEAD without
# making it watch `.git` mtimes.
#
# `arterm-build-meta/build.rs` deliberately does NOT declare `.git/HEAD`/`.git/index`
...
# `cargo:rerun-if-env-changed=ARTERM_BUILD_GIT_HASH` (and _DATE), so cargo reruns
...
# export ARTERM_BUILD_GIT_DIRTY here: the dirty flag flips on every edit and would
...
export_git_build_metadata() {
  ...
  if [[ -n "${ARTERM_BUILD_GIT_HASH:-}" ]]; then
    git_meta_status="external:${ARTERM_BUILD_GIT_HASH}"
    return
  fi
  ...
  export ARTERM_BUILD_GIT_HASH="$hash"
  date="$(git -C "$repo_root" log -1 --format=%ci 2>/dev/null || true)"
  if [[ -n "$date" ]]; then
    export ARTERM_BUILD_GIT_DATE="$date"
  fi
...
maybe_configure_low_memory_selfdev() {
  ...
  local mode="${ARTERM_SELFDEV_LOW_MEMORY:-auto}"
  case "$mode" in
    1|true|yes|on|force)
      ;;
    0|false|no|off|never)
    ...
    auto|"")
      ...
      ;;
    *)
      printf 'error: unsupported ARTERM_SELFDEV_LOW_MEMORY=%s (expected auto|on|off)\n' "$mode" >&2
      exit 1
      ;;
  esac
...
# builds. The arterm monoliths (arterm-base/app-core/tui) are ~80% single-threaded
...
#
# Controls:
#   ARTERM_PARALLEL_FRONTEND=auto|0|1   (default auto)
#   ARTERM_FRONTEND_THREADS=<n>         (default 4; diminishing returns past 4)
#   ARTERM_DEV_TOOLCHAIN=<name>         (default: nightly when present)
...
dev_nightly_toolchain() {
  ...
  if [[ -n "${ARTERM_DEV_TOOLCHAIN:-}" ]]; then
    printf '%s\n' "$ARTERM_DEV_TOOLCHAIN"
    return 0
  fi
  local tc
...
configure_parallel_frontend() {
  local requested="${ARTERM_PARALLEL_FRONTEND:-auto}"
  local forced="false"
  case "$requested" in
    0|false|no|off)
  ...
  # (`ARTERM_PARALLEL_FRONTEND=1`) opts dev/test in too, accepting that potential
  # cache contention.
  local profile
  ...
  local threads="${ARTERM_FRONTEND_THREADS:-4}"
...
configure_linux_linker() {
  local requested_mode="${ARTERM_FAST_LINKER:-auto}"
  local mode="$requested_mode"

  case "$mode" in
    auto)
      # Prefer mold over lld: on this repo's large statically-linked binary
      # (~300 MB .text), mold links the arterm bin in ~2.0s vs lld's ~2.9s
    ...
    lld|mold|system)
      ;;
    *)
      printf 'error: unsupported ARTERM_FAST_LINKER=%s (expected auto|lld|mold|system)\n' "$mode" >&2
      exit 1
      ;;
  esac
...
print_setup() {
  if [[ -n "${ARTERM_DEV_FEATURE_PROFILE:-}" && "${ARTERM_DEV_FEATURE_PROFILE}" != "default" ]]; then
    feature_profile_status="${ARTERM_DEV_FEATURE_PROFILE}"
  fi
  local cargo_gate_mode="${ARTERM_CARGO_GATE:-on}"
  local cargo_gate_dir="${ARTERM_CARGO_GATE_DIR:-${XDG_RUNTIME_DIR:-${TMPDIR:-/tmp}}}"
  cat <<EOF
repo_root=$repo_root
os=$(uname -s)
arch=$(uname -m)
...
cargo_gate_path=${ARTERM_CARGO_GATE_PATH:-$cargo_gate_dir/arterm-cargo-build.lock}
build_tmpdir_status=$build_tmpdir_status
tmpdir=${TMPDIR:-<unset>}
feature_profile_status=$feature_profile_status
git_meta_status=$git_meta_status
build_git_hash=${ARTERM_BUILD_GIT_HASH:-<unset>}
...
remote_connect_timeout() {
  local value="${ARTERM_REMOTE_CONNECT_TIMEOUT:-5}"
...
remote_tcp_timeout() {
  ...
  local value="${ARTERM_REMOTE_TCP_TIMEOUT:-1}"
...
remote_recovery_tcp_timeout() {
  ...
  local value="${ARTERM_REMOTE_RECOVERY_TCP_TIMEOUT:-0.3}"
  if [[ ! "$value" =~ ^[0-9]+([.][0-9]+)?$ ]]; then
    value=0.3
  fi
  printf '%s\n' "$value"
...
remote_down_cache_ttl() {
  ...
  local value="${ARTERM_REMOTE_DOWN_TTL:-300}"
  if [[ ! "$value" =~ ^[0-9]+$ ]]; then
    value=300
  fi
  printf '%s\n' "$value"
...
remote_down_cache_path() {
  local remote="${ARTERM_REMOTE_HOST:-}"
  local key
...
remote_cargo_preflight() {
  local remote="${ARTERM_REMOTE_HOST:-}"
  if [[ -z "$remote" ]]; then
    log "remote cargo requested but ARTERM_REMOTE_HOST is not configured"
    return 1
  fi

  local ssh_bin="${ARTERM_REMOTE_SSH_BIN:-ssh}"
  ...
  # disabled via ARTERM_REMOTE_TCP_PROBE=0.
  local tcp_probe="${ARTERM_REMOTE_TCP_PROBE:-1}"
  case "$tcp_probe" in
  ...
  local server_alive_interval="${ARTERM_REMOTE_SERVER_ALIVE_INTERVAL:-10}"
  local server_alive_count="${ARTERM_REMOTE_SERVER_ALIVE_COUNT_MAX:-1}"
  local output
  if ! output=$("$ssh_bin" \
    -o BatchMode=yes \
    ...
    "$remote" "printf 'arterm-remote-ok\\n'" 2>&1); then
...
remote_cargo_fallback_mode() {
  local mode="${ARTERM_REMOTE_CARGO_FALLBACK:-local}"
  case "$mode" in
    local|1|true|yes|on)
      printf 'local\n'
      ;;
    error|fail|0|false|no|off|never)
      ...
      ;;
    *)
      printf 'error: unsupported ARTERM_REMOTE_CARGO_FALLBACK=%s (expected local|error)\n' "$mode" >&2
      exit 2
      ;;
  esac
}
...
run_local_cargo() {
  if cargo_test_has_explicit_filter "${cargo_argv[@]}" && [[ "${ARTERM_DEV_CARGO_ALLOW_ZERO_TESTS:-0}" != "1" ]]; then
    local output_file
    output_file=$(mktemp "${TMPDIR:-/tmp}/arterm-dev-cargo.XXXXXX")
    local status=0
    ...
    if [[ "$status" -eq 0 ]] \
      ...
      printf 'dev_cargo: explicit cargo test filter matched zero tests; check the test path/name or set ARTERM_DEV_CARGO_ALLOW_ZERO_TESTS=1 to allow this intentionally\n' >&2
      rm -f "$output_file"
      return 97
    fi
    rm -f "$output_file"
...
# happen to share those exact directories. Arterm agents also build from scratch
...
#
# Serialize compile-capable local Cargo actions across all arterm worktrees. A
...
# parallelism. Nested wrapper calls inherit ARTERM_CARGO_GATE_HELD and cannot
# deadlock. Set ARTERM_CARGO_GATE=off for an intentional concurrency experiment.
acquire_cargo_gate() {
  cargo_gate_status="not-needed"
  cargo_gate_wait_ms=0
  cargo_action_needs_gate || return 0

  case "${ARTERM_CARGO_GATE:-on}" in
    0|false|no|off|disabled)
      cargo_gate_status="disabled"
      return 0
      ;;
  esac
  if [[ "${ARTERM_CARGO_GATE_HELD:-0}" == "1" ]]; then
  ...
  gate_dir="${ARTERM_CARGO_GATE_DIR:-${XDG_RUNTIME_DIR:-${TMPDIR:-/tmp}}}"
  mkdir -p "$gate_dir"
  gate_path="${ARTERM_CARGO_GATE_PATH:-$gate_dir/arterm-cargo-build.lock}"
  ...
  export ARTERM_CARGO_GATE_HELD=1
...
if [[ "${ARTERM_REMOTE_CARGO:-0}" == "1" ]]; then
  ...
  if [[ "$(remote_cargo_fallback_mode)" == "local" ]]; then
    log "remote cargo unavailable; falling back to local cargo (set ARTERM_REMOTE_CARGO_FALLBACK=error to fail instead)"
  else
...
acquire_cargo_gate
# Size the in-process parallelism only after competing arterm Cargo processes
