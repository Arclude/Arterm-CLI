...
case "$mode" in
  fake)
    # A Node script, kept when the TypeScript tree it came from was removed:
    # it speaks the OpenAI-compatible API this profile targets, and it is the
    # whole of the no-key test path. Porting it to Rust would drop the last
    # Node dependency in the repository; until then it is here on purpose.
