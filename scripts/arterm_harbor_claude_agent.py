...
IN_CONTAINER_HOME = "/tmp/arterm-home"
IN_CONTAINER_RUNTIME = "/tmp/arterm-runtime"
IN_CONTAINER_INPUT = "/tmp/arterm-input"
IN_CONTAINER_OUTPUT = "/tmp/arterm-output"
IN_CONTAINER_BINARY = "/usr/local/bin/arterm"
IN_CONTAINER_LIB_DIR = f"{IN_CONTAINER_RUNTIME}/lib"
IN_CONTAINER_CA_BUNDLE = f"{IN_CONTAINER_HOME}/ca-certificates.crt"
DEFAULT_BINARY_PATH = "/tmp/arterm-compat-dist/arterm-linux-x86_64.bin"
DEFAULT_CLAUDE_AUTH_PATH = "~/.arterm/auth.json"
DEFAULT_OPENROUTER_ENV_PATH = "~/.config/arterm/openrouter.env"
DEFAULT_ANTHROPIC_ENV_PATH = "~/.config/arterm/anthropic.env"
CA_BUNDLE_CANDIDATES = (
    os.environ.get("ARTERM_HARBOR_CA_BUNDLE"),
...
def _load_anthropic_key() -> str | None:
    return _load_key_from_env_file(
        os.environ.get("ARTERM_HARBOR_ANTHROPIC_ENV", DEFAULT_ANTHROPIC_ENV_PATH),
        "ANTHROPIC_API_KEY",
        "ANTHROPIC_API_KEY",
        "ARTERM_HARBOR_ANTHROPIC_KEY",
    )


def _load_openrouter_key() -> str | None:
    # Priority: explicit env, then the arterm openrouter.env file (raw key per line).
    direct = os.environ.get("OPENROUTER_API_KEY") or os.environ.get("ARTERM_HARBOR_OPENROUTER_KEY")
    if direct and direct.strip():
        return direct.strip()
    path = Path(os.environ.get("ARTERM_HARBOR_OPENROUTER_ENV", DEFAULT_OPENROUTER_ENV_PATH)).expanduser()
    if path.exists() and path.is_file():
        for line in path.read_text().splitlines():
            line = line.strip()
            if not line or line.startswith("#"):
...
ARTERM_BINARY = _resolve_existing_file(
    env_name="ARTERM_HARBOR_BINARY",
    default_path=DEFAULT_BINARY_PATH,
)
CA_BUNDLE = _resolve_existing_file(
    env_name="ARTERM_HARBOR_CA_BUNDLE",
...
OPENSSL_RUNTIME_LIBS = tuple(
    ...
    for lib in (
        _resolve_optional_existing_file(
            candidates=(
                os.environ.get("ARTERM_HARBOR_LIBSSL"),
                *_sibling_runtime_lib_candidates(ARTERM_BINARY, "libssl"),
                "/usr/lib/libssl.so.3",
        ...
        _resolve_optional_existing_file(
            candidates=(
                os.environ.get("ARTERM_HARBOR_LIBCRYPTO"),
                *_sibling_runtime_lib_candidates(ARTERM_BINARY, "libcrypto"),
...
def _benchmark_instruction_preamble() -> str:
    return os.environ.get("ARTERM_HARBOR_EXTRA_PREAMBLE", "")
...
class ArtermClaudeHarborAgent(BaseAgent):
    """Harbor adapter that runs arterm with Opus 4.8.
    ...
    OAuth (provider=claude via ~/.arterm/auth.json) are also supported.
    """
    ...
    def __init__(self, logs_dir: Path, model_name: str | None = None, *args, **kwargs):
        ...
        if "/" in self._model_arg:
            self._provider_arg, self._arterm_model = self._model_arg.split("/", 1)
        else:
            self._provider_arg, self._arterm_model = "anthropic-api", self._model_arg
        ...
        if self._provider_arg == "claude":
            self._claude_auth = _resolve_existing_file(
                env_name="ARTERM_HARBOR_CLAUDE_AUTH",
                default_path=DEFAULT_CLAUDE_AUTH_PATH,
            )

    @staticmethod
    def name() -> str:
        return "arterm-harbor-claude"
    ...
    async def setup(self, environment: BaseEnvironment) -> None:
        ...
        # "error setting certificate file") so tests never run. arterm itself
        ...
        await environment.upload_file(ARTERM_BINARY, IN_CONTAINER_BINARY)
        ...
        version_result = await environment.exec(
            ...
            env={
                ...
                "ARTERM_HOME": IN_CONTAINER_HOME,
                "ARTERM_RUNTIME_DIR": IN_CONTAINER_RUNTIME,
    ...
    async def run(self, instruction: str, environment: BaseEnvironment, context: AgentContext) -> None:
        ...
        env = {
            ...
            "ARTERM_RUNTIME_DIR": IN_CONTAINER_RUNTIME,
            "ARTERM_NO_TELEMETRY": "1",
            "ARTERM_PROVIDER": self._provider_arg,
            "ARTERM_MODEL": self._arterm_model,
            "ARTERM_ANTHROPIC_REASONING_EFFORT": os.environ.get("ARTERM_ANTHROPIC_REASONING_EFFORT", "high"),
        ...
        result = await environment.exec(
            command=(
                ...
                '--provider "$ARTERM_PROVIDER" --model "$ARTERM_MODEL" '
        ...
        try:
            await environment.download_dir(IN_CONTAINER_OUTPUT, self.logs_dir / "arterm-output")
        ...
        metadata: dict[str, Any] = {
            ...
            "model": self._arterm_model,
            "arterm_binary": str(ARTERM_BINARY),
        }

        output_dir = self.logs_dir / "arterm-output"
        ...
        if payload is not None:
            ...
            metadata["arterm_result"] = payload
