#!/bin/bash
# Replay a arterm recording as video
#
# This script:
# 1. Starts a fresh arterm instance in a new terminal
...
# Start arterm in a new kitty window
echo "🚀 Starting arterm..."
kitty --title "arterm-replay" -e bash -c "cd $(pwd) && ~/.cargo/bin/arterm; read -p 'Press Enter to close...'" &
KITTY_PID=$!
sleep 2  # Wait for arterm to start

# Focus the new window
sleep 0.5
# Find and focus the arterm-replay window
WINDOW_ID=$(niri msg windows 2>/dev/null | grep -B5 "arterm-replay" | grep -oP 'Window ID \K\d+' | head -1)
if [ -n "$WINDOW_ID" ]; then
