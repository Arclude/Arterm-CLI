#!/bin/bash
# End-to-end test script for arterm
...
echo "=== E2E Testing Script for arterm ==="
echo ""

# Test 1: Check binary exists and runs
echo "Test 1: Check arterm binary..."
if command -v arterm &> /dev/null; then
    echo "✓ arterm binary found"
    arterm --version
else
    echo "✗ arterm binary not found"
    exit 1
fi
...
if [[ "${ARTERM_REAL_PROVIDER:-0}" == "1" ]]; then
    echo ""
    echo "Test 7: Real provider smoke (ARTERM_REAL_PROVIDER=1)..."
    scripts/real_provider_smoke.sh
    echo "✓ Real provider smoke passed"
fi

if [[ "${ARTERM_REAL_AUTH_TEST:-0}" == "1" ]]; then
    echo ""
    echo "Test 8: Auth E2E validation (ARTERM_REAL_AUTH_TEST=1)..."
...
echo "  arterm        # Start TUI mode"
echo "  arterm server # Start server mode"
echo "  arterm client # Connect to server"
