#!/usr/bin/env python3
"""
Test OAuth usage comparison between Claude Code CLI and arterm direct API.

This script:
1. Shells out to Claude Code CLI with a simple prompt
2. Uses arterm's debug socket to send the same prompt via direct OAuth
...
DEBUG_SOCKET = f"/run/user/{os.getuid()}/arterm-debug.sock"
MAIN_SOCKET = f"/run/user/{os.getuid()}/arterm.sock"
...
def run_arterm_oauth(prompt: str) -> dict:
    """Run via arterm debug socket using direct OAuth."""
    print(f"\n{'='*60}")
    print("Testing arterm direct OAuth API...")
    print(f"{'='*60}")
...
def main():
    ...
    # Test arterm OAuth
    arterm_result = run_arterm_oauth(TEST_PROMPT)

    # Check quota after arterm test
    time.sleep(1)  # Wait for API to update
    usage_after_arterm = get_oauth_usage()
    five_hour_after_arterm = usage_after_arterm.get('five_hour', {}).get('utilization', 0)
    arterm_quota_delta = five_hour_after_arterm - five_hour_after_cli
    print(f"\nQuota after arterm: {five_hour_after_arterm:.2f}% (delta: +{arterm_quota_delta:.4f}%)")
    ...
    print("\narterm Direct OAuth:")
    if "error" in arterm_result:
        print(f"  Error: {arterm_result['error']}")
    else:
        print(f"  Time: {arterm_result.get('time', 'N/A'):.2f}s")
        usage = arterm_result.get('usage', {})
        if usage:
            print(f"  Input tokens: {usage.get('input_tokens', 'N/A')}")
            print(f"  Output tokens: {usage.get('output_tokens', 'N/A')}")
            print(f"  Cache read: {usage.get('cache_read_input_tokens', 0) or 0}")
    ...
    arterm_usage = arterm_result.get('usage', {})
    ...
    arterm_total = (arterm_usage.get('input_tokens', 0) or 0) + \
                  (arterm_usage.get('cache_creation_input_tokens', 0) or 0) + \
                  (arterm_usage.get('cache_read_input_tokens', 0) or 0) + \
                  (arterm_usage.get('output_tokens', 0) or 0)

    cli_time = cli_result.get('time', 0)
    arterm_time = arterm_result.get('time', 0)
    speedup = cli_time / arterm_time if arterm_time > 0 else 0
    token_savings = 100 * (1 - arterm_total / cli_total) if cli_total > 0 else 0
...
PERFORMANCE COMPARISON:
                    Claude CLI      arterm
  Response time:    {cli_time:.2f}s           {arterm_time:.2f}s ({speedup:.1f}x faster)
  Total tokens:     {cli_total:,}         {arterm_total:,} ({token_savings:.0f}% fewer)
...
ACTUAL QUOTA CONSUMPTION (from OAuth API):
  ...
  After arterm:      {five_hour_after_arterm:.2f}%  (+{arterm_quota_delta:.4f}%)
