...
# Run from the arterm repo root

set -euo pipefail

ITERATIONS=${1:-5}
RESULTS_FILE="/tmp/arterm_tool_benchmark_$(date +%Y%m%d_%H%M%S).csv"

echo "=== arterm Tool Call Benchmark ==="
...
# Find debug socket
DEBUG_SOCK="${ARTERM_DEBUG_SOCK:-/run/user/$(id -u)/arterm-debug.sock}"

if [ ! -S "$DEBUG_SOCK" ]; then
    echo "ERROR: Debug socket not found at $DEBUG_SOCK"
    echo "Make sure arterm is running with debug control enabled."
    exit 1
fi
