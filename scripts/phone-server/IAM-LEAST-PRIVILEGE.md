...
1. **`ArtermPhoneOperator`** for normal deployments and maintenance of the existing stack.
2. **`ArtermPhoneProvisioner`** for infrequent rebuilds, MFA-gated, time-limited, restricted to `us-east-1`, `arterm-phone-*` resources, and bounded runtime roles.
...
- `breaker-lambda.py` calls `ec2:DescribeInstances`, `ec2:StopInstances`, and `sns:Publish` to `arn:aws:sns:us-east-1:302154194530:arterm-guard-warn`.
  ...
  "Statement": [
    {
      ...
      "Resource": [
        "arn:aws:iam::302154194530:role/arterm-phone/ArtermPhoneOperator",
        "arn:aws:iam::302154194530:role/arterm-phone/ArtermPhoneProvisioner"
      ]
    }
  ]
}
...
Set `ArtermPhoneProvisioner` maximum session duration to one hour. Do not allow either role to change its own trust or permissions.
...
```json
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": "arn:aws:logs:us-east-1:302154194530:log-group:/aws/lambda/arterm-phone-wake:*"
    }
  ]
}
```
...
```json
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Effect": "Allow",
      "Action": "sns:Publish",
      "Resource": "arn:aws:sns:us-east-1:302154194530:arterm-guard-warn"
    },
    {
      "Sid": "WriteOwnLogs",
      "Effect": "Allow",
      ...
      "Resource": "arn:aws:logs:us-east-1:302154194530:log-group:/aws/lambda/arterm-guard-breaker:*"
    }
  ]
}
```
...
```json
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": [
        "arn:aws:lambda:us-east-1:302154194530:function:arterm-phone-wake",
        "arn:aws:lambda:us-east-1:302154194530:function:arterm-phone-wake:*",
        "arn:aws:lambda:us-east-1:302154194530:function:arterm-guard-breaker",
        "arn:aws:lambda:us-east-1:302154194530:function:arterm-guard-breaker:*"
      ]
    },
    {
    ...
    },
    {
      ...
      "Effect": "Allow",
      "Action": "iam:PassRole",
      "Resource": "arn:aws:iam::302154194530:role/arterm-phone/runtime/ArtermPhoneInstance",
      "Condition": {
        "StringEquals": {
    ...
    },
    {
      ...
      "Action": "iam:PassRole",
      "Resource": [
        "arn:aws:iam::302154194530:role/arterm-phone/runtime/ArtermPhoneWakeLambda",
        "arn:aws:iam::302154194530:role/arterm-phone/runtime/ArtermPhoneBreakerLambda"
      ],
      "Condition": {
        "StringEquals": {
    ...
    },
    {
      ...
      "Resource": [
        "arn:aws:logs:us-east-1:302154194530:log-group:/aws/lambda/arterm-phone-wake:*",
        "arn:aws:logs:us-east-1:302154194530:log-group:/aws/lambda/arterm-guard-breaker:*"
      ]
    },
    {
      ...
      "Resource": [
        "arn:aws:sns:us-east-1:302154194530:arterm-guard-stop",
        "arn:aws:sns:us-east-1:302154194530:arterm-guard-warn"
      ]
    },
    {
      "Sid": "ReadNamedBudget",
      ...
      "Resource": "arn:aws:budgets::302154194530:budget/arterm-dev-monthly-cost"
    }
  ]
}
```
...
- Human `ArtermPhoneProvisioner`: CloudFormation stack operations on `arterm-phone-server*`, read-only diagnostics, and `iam:PassRole` only for `ArtermPhoneCloudFormationExecution` with `iam:PassedToService = cloudformation.amazonaws.com`.
- `ArtermPhoneCloudFormationExecution`: service permissions below, usable only by CloudFormation.
- Every created resource is tagged `Project=arterm-phone-server` and `ManagedBy=cloudformation`.
- Every created runtime role is under path `/arterm-phone/runtime/` and must carry the `ArtermPhoneRuntimeBoundary` permissions boundary.
  ...
  "Statement": [
    {
      ...
      "Resource": [
        "arn:aws:cloudformation:us-east-1:302154194530:stack/arterm-phone-server*/*",
        "arn:aws:cloudformation:us-east-1:302154194530:changeSet/arterm-phone-server*/*"
      ]
    },
    {
    ...
    },
    {
      ...
      "Effect": "Allow",
      "Action": "iam:PassRole",
      "Resource": "arn:aws:iam::302154194530:role/arterm-phone/ArtermPhoneCloudFormationExecution",
      "Condition": {
        "StringEquals": {
...
|---|---|---|
| EC2 | Run/terminate the one server; create/tag volume and ENI; create/manage one SG; allocate/associate/release one EIP; modify shutdown behavior; describe AMIs/subnets/VPCs | `us-east-1`; request/resource tag `Project=arterm-phone-server`; approved instance types only; IMDSv2 required; approved VPC/subnet |
| IAM | Create/update/delete the three runtime roles and one instance profile; put/delete inline policies; pass roles | Path `/arterm-phone/runtime/` only; require boundary ARN on `CreateRole`; never allow changing deploy/provisioner/emergency roles |
| Lambda | Create/update/delete the two named functions; versions/aliases; permissions | Function ARN prefix `arterm-phone-*` and `arterm-guard-breaker*`; project tag |
| API Gateway v2 | Create/update/delete one HTTP API, integration, route, and stage | Project tag and stack ownership |
| SNS | Create/manage/delete `arterm-guard-stop` and `arterm-guard-warn`; subscriptions | Exact topic-name ARNs |
...
| Budgets | Create/update/delete `arterm-dev-monthly-cost` | Exact budget ARN |
...
aws lambda get-function --function-name arterm-phone-wake --region us-east-1
aws lambda get-function --function-name arterm-guard-breaker --region us-east-1
aws apigatewayv2 get-api --api-id 8c3wp4cbag --region us-east-1
aws sns list-topics --region us-east-1
aws cloudwatch describe-alarms --alarm-name-prefix arterm- --region us-east-1
aws budgets describe-budget --account-id 302154194530 --budget-name arterm-dev-monthly-cost
...
8. **Canary mutation paths without touching production.** Through the provisioner, deploy and remove a tiny tagged canary stack using the same resource classes where practical. For the operator, update a disposable Lambda alias or canary function rather than invoking `arterm-guard-breaker`, which stops production. Do not use the breaker invocation as an IAM test.
...
- Daily deployment and maintenance succeed through `ArtermPhoneOperator`.
