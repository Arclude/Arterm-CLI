#!/usr/bin/env python3
"""Token-protected HTTP service that generates arterm pairing codes.

GET /pair-code?t=<token> -> {"code": "123456", "host": "100.109.78.41", "port": 7643, "uri": "arterm://pair?..."}
"""
import http.server, json, re, subprocess, os
from urllib.parse import urlparse, parse_qs

TOKEN = open('/etc/arterm-pair-token').read().strip()
HOST = '100.109.78.41'
PORT = 7643
...
class H(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        env['ARTERM_GATEWAY_HOST'] = HOST
        try:
            out = subprocess.run(
                ['sudo', '-u', 'ec2-user', '-i', 'arterm', 'pair'],
