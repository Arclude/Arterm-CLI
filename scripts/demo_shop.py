...
def state_path() -> Path:
    configured = os.environ.get("ARTERM_DEMO_SHOP_STATE")
    if configured:
        return Path(configured).expanduser()
    return Path(f"/tmp/arterm-demo-shop-{os.getuid()}.json")
...
def command_search(args: argparse.Namespace) -> int:
    ...
    print("Add one with: arterm-demo-shop add <sku>")
    return 0
...
def command_add(args: argparse.Namespace) -> int:
    ...
    print("Review it with: arterm-demo-shop cart")
    print("Continue with: arterm-demo-shop checkout")
    print(
