...
User-facing changelog entries for arterm releases. One JSON file per release,
written by the agent during `/cut-release`, reviewed in the release diff, and
consumed by arterm.dev and the GitHub release body.
...
- Write for users of arterm, not contributors. Describe the effect, not the
