...
Arterm supports Windows as a first-class platform. The Windows implementation uses native named pipes, Windows process management, PowerShell installation, and platform-specific launch-hotkey integration.
...
| `arterm update` | Supported with SHA-256 verification |
...
4. Installs immutable, stable, and launcher copies under `%LOCALAPPDATA%\arterm`.
5. Adds `%LOCALAPPDATA%\arterm\bin` to the user `PATH`.
...
```powershell
$script = [scriptblock]::Create((irm https://raw.githubusercontent.com/Arclude/Arterm-CLI/main/scripts/install.ps1))
& $script -ConfigureAlacritty -ConfigureHotkey
```

Arterm can also offer these options interactively after launch.
...
- Launcher: `%LOCALAPPDATA%\arterm\bin\arterm.exe`
- Stable binary: `%LOCALAPPDATA%\arterm\builds\stable\arterm.exe`
- Versioned binary: `%LOCALAPPDATA%\arterm\builds\versions\<version>\arterm.exe`
- User data and configuration: `%USERPROFILE%\.arterm`
...
```powershell
Get-AuthenticodeSignature (Get-Command arterm).Source | Format-List Status,StatusMessage,SignerCertificate
```
...
2. Create a Microsoft Entra application or managed identity with a federated credential for `1jehuang/arterm` GitHub Actions.
...
Unix domain sockets are replaced by Windows named pipes under `crates/arterm-base/src/transport/windows.rs`. Platform-specific filesystem, process, update, and replacement behavior is selected at compile time with `#[cfg(windows)]`, so Windows support does not add runtime branching to Unix builds.

Windows launch-hotkey setup is implemented in `crates/arterm-setup-hints/src/windows_setup.rs` and is only installed after explicit user consent.
...
- Arterm version from `arterm --version`
- Installation method
...
Do not upload private configuration, credentials, session transcripts, or `.arterm` authentication files.
