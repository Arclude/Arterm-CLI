...
(`?via=arterm-discovery`). That works when a human clicks the listing URL in a
browser. Most arterm-driven signups instead happen from an agent following the
...
`cli_flow_attributable` is the **primary check**. Nearly every arterm-driven
...
- every run prints an explicit per-sponsor CLI-attribution verdict
  ...
  `N/M sponsors credit agent-driven CLI signups to arterm` summary line;
...
requests to the discovery service carry `x-arterm-discovery-benchmark: 1` so
