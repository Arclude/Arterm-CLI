...
- Arterm sessions switch the current workspace/client to that session.
- Importable external sessions are first converted to a Arterm session, then resumed in place.
...
- If terminal launch is unavailable, the UI prints manual `arterm --resume <id>` commands.
