...
Arterm to browse the expected sponsored-discovery category and receive a specific
...
The default uses Arterm's normal toolset so Discovery competes with built-in
...
The runner uses a dedicated Arterm server with:
...
```text
x-arterm-discovery-benchmark: 1
```
