...
## Two operating modes (gate: `agents.memory_sidecar_enabled`, env `ARTERM_MEMORY_SIDECAR_ENABLED`, default off)
...
Benchmark (Mode 1, private ~/arterm-memory-bench, Sonnet judge):
...
Benchmark-driven results (Sonnet judge, 28 judged queries, arterm self-dev corpus):
...
- Shared: arterm-base/src/memory_rerank.rs (prompt + parse + rerank_candidates),
...
~/arterm-memory-bench/results/BASELINE_SUMMARY.md):
