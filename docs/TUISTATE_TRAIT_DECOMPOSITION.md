...
This document audits the `TuiState` trait (`crates/arterm-tui/src/tui/mod.rs`) and
...
- `cargo check -p arterm-tui` after each sub-trait extraction (TMPDIR must point
  at real disk, not the RAM-backed tmpfs, or ring/aws-lc-sys build scripts fail
  with "Disk quota exceeded").
- `cargo test -p arterm-tui --lib` once at the end. Note: the lib test suite has
