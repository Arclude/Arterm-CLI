...
Arterm. The root `arterm` crate is still the integration shell, but stable leaf
...
2. Avoid cyclic domain crates. Root `arterm` may depend on leaf crates, but leaf
...
5. Preserve compatibility facades during migration. `crate::storage::*` can
   re-export `arterm-storage::*` while callers move gradually.

## Current first step

`arterm-storage` is now a leaf crate for app paths, permission hardening, atomic
...
- `cargo check -p arterm-storage`: ~0.9s after initial dependencies were built.
- `cargo check -p arterm --lib`: ~14s in the current warm-cache state.

## Recommended next extractions

1. `arterm-provider-anthropic`: move Anthropic request/stream translation out of
   root `src/provider/anthropic.rs` and depend only on `arterm-provider-core`,
   `arterm-message-types`, and serde/reqwest primitives.
2. `arterm-provider-openai`: same for OpenAI request/stream handling. This
   reduces rebuilds when editing server/TUI code and makes provider tests cheap.
3. `arterm-session-core`: move session storage paths, journal metadata, and
   memory-profile pure transforms once dependencies on root prompt/logging are
   cut behind callbacks.
4. `arterm-tui-app-state`: split key/input/navigation state transitions from
   rendering. Keep ratatui rendering in `arterm-tui-render`/root while state tests
   compile without the whole root crate.
5. `arterm-server-protocol-runtime`: split websocket/client event fanout glue from
   agent execution so server tests do not rebuild TUI/provider internals.

## Anti-patterns to avoid

- Extracting crates that depend on root `arterm`. That preserves the compile-time
