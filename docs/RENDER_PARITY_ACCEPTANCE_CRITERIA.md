...
Status: adopted for the `arterm-render-core` -> TUI switchover.
Harness: `crates/arterm-tui-markdown/src/render_core_adapter_tests.rs`
...
|------|---------------------------|------|--------------------------------------------------|
| CI (default) | 5000 (L1, L2), 3000×3 widths = 9000 renders (L3) | must pass on every PR touching `arterm-tui-markdown`, `arterm-render-core` | p < 6.0e-4 per generated document (L1/L2); p < 1.0e-3 per doc per width (L3) |
| Pre-switchover deep run | `ARTERM_MD_FUZZ_ITERS=100000` | must pass once, on the exact commit proposed for switchover | p < 3.0e-5 per generated document |
| Nightly (optional soak) | `ARTERM_MD_FUZZ_ITERS=25000`, rotating `ARTERM_MD_FUZZ_SEED` (e.g. epoch-day) | failures file an issue with repro seed | accumulates coverage across seeds over time |
...
- **Reproducibility:** each failure reports the iteration index `i`; the
  ...
  reproducible with `ARTERM_MD_FUZZ_SEED=<base_seed> ARTERM_MD_FUZZ_ITERS=<i+1>`
  (or by re-deriving the single seed). `base_seed` defaults are fixed
  constants per suite and overridable via `ARTERM_MD_FUZZ_SEED`.
...
cargo test -p arterm-tui-markdown --lib render_core_adapter

# Deep run (switchover gate; ~80s on an XPS 13)
ARTERM_MD_FUZZ_ITERS=100000 \
  cargo test -p arterm-tui-markdown --lib render_core_adapter::tests::fuzz

# Alternate-seed confirmation
ARTERM_MD_FUZZ_ITERS=100000 ARTERM_MD_FUZZ_SEED=20260713 \
  cargo test -p arterm-tui-markdown --lib render_core_adapter::tests::fuzz
```
