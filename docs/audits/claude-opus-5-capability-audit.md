...
This exists because arterm's Claude capability tables are hand-maintained, and
...
| Capability | Probe | Result | arterm encoding |
|---|---|---|---|
...
arterm previously sent a flat `max_tokens = 32768` for every Claude model. Opus 5
...
```bash
set -a; source ~/.config/arterm/anthropic.env; set +a
