...
|---|---|---|---|
| `direct` (default) | Local Google OAuth tokens (`arterm login google`) | No third party in the loop | Unverified-app warning; 7-day refresh-token expiry in Google "Testing" mode |
...
- `ARTERM_GMAIL_BACKEND=direct` (or unset) -> direct Google backend.
- `ARTERM_GMAIL_BACKEND=composio` -> Composio backend (requires `COMPOSIO_API_KEY`).

If `composio` is requested but `COMPOSIO_API_KEY` is missing, arterm warns and
...
1. arterm calls Composio's `POST /connected_accounts/link` (hosted "Connect
...
4. arterm polls `GET /connected_accounts/{id}` until the connection is `ACTIVE`,
   then persists it to `~/.arterm/composio_gmail.json`.
...
3. Export the variables:
   ```bash
   export ARTERM_GMAIL_BACKEND=composio
...
- `direct`: honors the access tier chosen at `arterm login google`
