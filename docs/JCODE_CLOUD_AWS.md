...
`/remote` is the single activation entry point for Arterm Cloud. It opens the Arterm account activation page, where a user signs in, confirms the subscription entitlement, chooses a region, and creates or wakes their host. `/remote status`, `/remote on`, `/remote pair`, and `/remote revoke` remain the explicit self-hosted gateway controls.

Cloud access is bundled into paid Arterm subscriptions rather than sold as a second product. The subscription pays for the control plane and a bounded amount of host runtime/storage. Model-token budgets remain governed by the existing subscription tier.
...
5. Arterm provisions an isolated host, imports only credentials or repository access the user explicitly approves, and displays progress.
...
```mermaid
flowchart LR
  C[Arterm clients] -->|OAuth/device credential| CF[CloudFront]
...
- **Identity:** Cognito user pool federated from the existing Arterm account during migration. Long term, Cognito is the account identity authority. Device authorization is implemented by Lambda/API Gateway with hashed, short-lived codes.
...
1. **Internal alpha:** keep the existing guarded `arterm-phone` EC2 deployment as the reference host. Validate wake, SSM, gateway, Bedrock, idle stop, and breaker paths.
