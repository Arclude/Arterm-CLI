...
This document describes a proposed evolution of arterm's UI architecture from the
...
Today, arterm is effectively organized like this:
...
- a independent arterm window attached to one session
...
- current independent arterm window
...
2. Client spawns a independent arterm client attached to the same session.
...
This can initially be provided through existing arterm control channels such as:
...
- Preserve current `arterm --resume <session>` workflow.
...
- Should workspace mode be a new command (`arterm workspace`) or a runtime mode of
  the normal client?
...
This gives arterm a portable built-in multi-session workspace without sacrificing
