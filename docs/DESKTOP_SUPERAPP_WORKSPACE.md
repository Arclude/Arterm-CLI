...
This document refines the Arterm Desktop product direction from a single chat-like app into a **Niri-like agent workspace superapp**.
...
The goal is not to clone a general-purpose window manager. The goal is to give Arterm users a fast, keyboard-driven, spatial environment for supervising multiple agent sessions and related development tools inside one custom desktop app.
...
Arterm Desktop should be a **local AI development superapp**:
...
Niri's useful idea for Arterm is not the compositor implementation. It is the mental model:
...
Arterm Desktop can bring that workflow to macOS users who do not have a Niri-like environment, while still working well on Linux.
...
| Session surface | A surface attached to a server-owned Arterm session |
...
This is important because Arterm users may run many agents/tasks concurrently.
...
It should be backed by a shared command registry in `arterm-client-core`, not hardcoded separately per UI.

## Data model additions

`arterm-client-core` should include a workspace layout state model:
...
Arterm Desktop should become a **keyboard-driven, Niri-like agent workspace superapp**.
