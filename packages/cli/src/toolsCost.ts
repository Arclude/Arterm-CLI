...
export function runToolsCost(opts: { tier?: string; json?: boolean; cwd?: string }): void {
  ...
  process.stdout.write(
    "\n  not counted: tools a session adds at runtime — todo, plan, task, exec,\n" +
      "  spawn, spawn_parallel, the model-driven fleet (spawn_subagent,\n" +
      "  assign_task, await_tasks, ask_subagent, roll_up, fleet), the memory\n" +
      "  tools, and any MCP or plugin tools.\n",
  );
}
