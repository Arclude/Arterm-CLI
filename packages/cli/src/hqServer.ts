...
/**
 * In-process **single-agent HQ dashboard** — a live web view of THIS agent session.
 *
 * Serves the shared `HqState` (derived from `session.bus`) over loopback HTTP + SSE,
 * and accepts control POSTs that drive `session.autonomy.*`. For monitoring MANY
 * agents from one place, use the aggregator (`arterm hq`) + reporter instead.
 *
