    ...
    let (pending, auth_url, input_kind, user_code, expires_at_ms) = match provider.target {
        ...
        LoginProviderTarget::Google => {
            let creds = auth::google::load_credentials().context(
                "Google/Gmail scriptable auth requires saved OAuth credentials first. Run `arterm login --provider google` once or save google credentials manually.",
            )?;
            let tier = options
                .google_access_tier
                .unwrap_or(auth::google::GmailAccessTier::Full);
...
pub(super) async fn complete_scriptable_claude_login(
    ...
    emit_scriptable_auth_success(
        ...
        ScriptableAuthSuccess {
            ...
            credentials_path: Some(auth::claude::arterm_path()?.display().to_string()),
...
pub(super) async fn complete_scriptable_openai_login(
    ...
    clear_pending_login(&pending_path);
    crate::telemetry::record_auth_success(provider_id, "oauth");
    let credentials_path = crate::storage::arterm_dir()?.join("openai-auth.json");
    emit_scriptable_auth_success(
        options.json,
        ScriptableAuthSuccess {
            status: "authenticated",
...
pub(super) async fn complete_scriptable_google_login(
    ...
    let creds = auth::google::load_credentials().context(
        "Google/Gmail completion requires saved OAuth credentials first. Run `arterm login --provider google` once or save google credentials manually.",
    )?;
...
pub(super) fn pending_login_path(key: &str) -> Result<PathBuf> {
    Ok(crate::storage::arterm_dir()?
        .join("pending-login")
        .join(format!("{key}.json")))
}

pub(super) fn pending_login_dir() -> Result<PathBuf> {
    Ok(crate::storage::arterm_dir()?.join("pending-login"))
}
...
pub(super) fn load_pending_login(path: &PathBuf, provider: &str) -> Result<PendingScriptableLogin> {
    if !path.exists() {
        anyhow::bail!(
            "No pending {} login state found. Run `arterm login --provider {} --print-auth-url` first.",
            provider,
            provider
        );
    }
    ...
    if let Ok(record) = serde_json::from_str::<PendingScriptableLoginRecord>(&data) {
        if record.expires_at_ms <= current_time_ms() {
            ...
            anyhow::bail!(
                "Pending {} login state expired. Run `arterm login --provider {} --print-auth-url` again.",
                provider,
                provider
            );
        }
...
pub(super) fn scriptable_resume_command(provider: &str, input_kind: &str) -> String {
    match input_kind {
        "callback_url" => {
            format!(
                "arterm login --provider {} --callback-url '<url-or-query>'",
                provider
            )
        }
        "auth_code" => format!("arterm login --provider {} --auth-code '<code>'", provider),
        "complete" => format!("arterm login --provider {} --complete", provider),
        _ => format!(
            "arterm login --provider {} --callback-url '<url>'  # or --auth-code '<code>'",
            provider
        ),
    }
}
