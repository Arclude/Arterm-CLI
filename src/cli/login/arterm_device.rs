//! CLI orchestration for the Arterm account device authorization flow.
//!
    ...
    C: Future<Output = std::io::Result<()>>,
{
    ...
    loop {
        ...
        if tokio::time::Instant::now() + delay >= deadline {
            anyhow::bail!(
                "Arterm account login timed out before browser approval. Run `arterm account login` to try again."
            );
        }
        tokio::select! {
        ...
        match subscription_api::poll_device_token_once(client, api_base, device_code).await {
            ...
            Ok(TokenPollOutcome::Expired) => anyhow::bail!(
                "The browser approval expired or was already exchanged. Run `arterm account login` to start a new single-use flow."
            ),
            Ok(TokenPollOutcome::Denied) => {
                anyhow::bail!("Arterm account login was canceled or denied in the browser.")
            }
...
pub(super) async fn login_arterm_device_flow(no_browser: bool) -> Result<LoginCompletion> {
    ...
    let device = subscription_api::request_device_authorization(&client, &api_base, None)
        ...
        .context("Failed to start Arterm account login")?;
    ...
    let completion = match activation {
        ...
        Some(ActivationOutcome::Revoked) => {
            ...
            anyhow::bail!(
                "The newly issued account key was revoked before hosted billing activation. Local credentials were cleared; run `arterm account login` again."
            );
        }
        Some(ActivationOutcome::Denied) => {
            crate::subscription_catalog::clear_account_credentials()?;
            anyhow::bail!(
                "The account server denied hosted billing checks. Local credentials were cleared; run `arterm account login` again."
            );
        }
        None => {
    ...
    crate::telemetry::record_auth_success("arterm-subscription", "device_code_browser");
    Ok(completion)
}

fn print_recovery_actions() {
    eprintln!("  Check:   arterm account status");
    eprintln!("  Manage:  arterm account manage");
    eprintln!("  Log out: arterm account logout");
}
