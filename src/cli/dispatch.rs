...
fn is_file_controlled_debug_client() -> bool {
    std::env::var_os("ARTERM_DEBUG_CMD_PATH").is_some()
}
...
pub(crate) async fn run_main(mut args: Args) -> Result<()> {
        ...
        .filter(|value| !value.is_empty())
    {
        ...
        crate::env::set_var("ARTERM_PROVIDER_PROFILE_NAME", profile_name);
    ...
    if let Some(tool_profile) = args.tool_profile.as_deref() {
        crate::env::set_var("ARTERM_TOOL_PROFILE", tool_profile);
    }
    if let Some(tools) = args.tools.as_deref() {
        crate::env::set_var("ARTERM_TOOLS", tools);
    }
    if let Some(disabled_tools) = args.disabled_tools.as_deref() {
        crate::env::set_var("ARTERM_DISABLED_TOOLS", disabled_tools);
    }
    if args.disable_base_tools {
        crate::env::set_var("ARTERM_DISABLE_BASE_TOOLS", "1");
    }
    ...
    match args.command {
        ...
        Some(Command::ApiBridge { api_socket }) => {
            ...
            .await
            {
                eprintln!("api-bridge: could not start the arterm server: {error:#}");
                eprintln!("api-bridge: continuing; an already-running server will still be used");
            }
            let api_socket = api_socket
                .map(std::path::PathBuf::from)
                .unwrap_or_else(arterm_harness_api_server::api_socket_path);
            // The global `--socket` (and `ARTERM_SOCKET`) already selects the
            // daemon socket; `set_socket_path` exported it during startup.
            let legacy_socket = arterm_harness_api_server::legacy_socket_path();
            arterm_harness_api_server::run_bridge(api_socket, legacy_socket).await?;
        }
        Some(Command::Server { action }) => match action {
            ServerCommand::Start { json } => {
                ...
                if json {
                    ...
                    println!("Arterm server is running.");
                }
            }
        ...
        Some(Command::Memory(subcmd)) => crate::cli::memory_clean::dispatch(subcmd)?,
        ...
        Some(Command::ProviderTestCoverage {
            ...
            let colorize = std::io::stdout().is_terminal()
                ...
                && std::env::var_os("ARTERM_NO_COLOR").is_none();
        ...
        match resolve_resume_id(&resume_id) {
            ...
            Err(e) => {
                match resume_resolution_failure_action(&resume_id, |key| std::env::var_os(key)) {
                    ...
                    // itself with `--resume <id>` and `ARTERM_RESUMING=1`. In the
                    ...
                    // making arterm unusable after an auto-update (issue #328).
                    ...
                    ResumeResolutionFailureAction::Exit => {
                        ...
                        if !output::quiet_enabled() {
                            eprintln!("\nUse `arterm --resume` to list available sessions.");
                        }
                        std::process::exit(1);
                    }
                }
...
where
    F: Fn(&str) -> Option<V>,
{
    if var_os("ARTERM_RESUMING").is_some() {
        ResumeResolutionFailureAction::DeferToServer
    } else {
        ResumeResolutionFailureAction::Exit
    }
...
async fn run_default_command(args: Args) -> Result<()> {
    ...
    let in_arterm_repo = build::is_arterm_repo(&cwd);
    startup_profile::mark("is_arterm_repo");
    ...
    // hotkeys can reopen arterm in the last project directory (Cmd+') and the
    // last arterm repo for self-dev (Cmd+Shift+'). Best-effort; ignored unless a
    ...
    if in_arterm_repo && !already_in_selfdev && !args.no_selfdev {
        output::stderr_info("📍 Detected arterm repository - enabling self-dev mode");
    ...
    if std::env::var_os("ARTERM_RESUMING").is_none() {
    ...
    if !server_running && std::env::var("ARTERM_RESUMING").is_ok() {
    ...
    if !server_running {
        ...
        if server::reap_stale_socket_if_dead(&server::socket_path()).await {
            output::stderr_info("Removed a stale arterm socket from a previous server.");
        }
    ...
    if std::env::var("ARTERM_RESUMING").is_err() && server_running {
...
pub(crate) async fn maybe_prompt_server_bootstrap_login(
    ...
    let cli_bootstrap_requested = std::env::var_os("ARTERM_CLI_BOOTSTRAP_LOGIN").is_some();
    ...
    // `ARTERM_CLI_BOOTSTRAP_LOGIN` env var.
...
pub(crate) async fn spawn_server(
    ...
    if client_requested_selfdev {
        cmd.env("ARTERM_DEBUG_CONTROL", "1");
    }
    ...
    cmd.env("ARTERM_DEFERRED_AUTH_BOOTSTRAP", "1");
