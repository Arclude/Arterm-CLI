...
pub async fn run_debug_command(
    ...
    if !crate::transport::is_socket_path(&debug_socket) {
        ...
        eprintln!("\nMake sure:");
        eprintln!("  1. A arterm server is running (arterm or arterm serve)");
        eprintln!("  2. debug_socket is enabled in ~/.arterm/config.toml");
        eprintln!("     [display]");
        eprintln!("     debug_socket = true");
        eprintln!("\nOr use 'arterm debug start' to start a server.");
        eprintln!("Use 'arterm debug list' to see running servers.");
...
async fn debug_list_servers() -> Result<()> {
    ...
    if servers.is_empty() {
        println!("No running arterm servers found.");
        println!("\nStart one with: arterm debug start");
        return Ok(());
    }

    println!("Running arterm servers:\n");
    ...
    println!("  arterm debug -s /path/to/socket.sock sessions");
...
async fn debug_start_server(arg: &str, socket_path: Option<String>) -> Result<()> {
            ...
            .is_ok()
    {
        ...
        eprintln!("Use 'arterm debug list' to see all servers.");
        return Ok(());
    }
    ...
    eprintln!("Starting arterm server...");
    ...
    if crate::transport::is_socket_path(&debug_socket) {
        ...
        eprintln!("⚠ Debug socket not enabled. Add to ~/.arterm/config.toml:");
        eprintln!("  [display]");
        eprintln!("  debug_socket = true");
    }
