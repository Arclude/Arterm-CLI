...
fn set_socket_test_env(socket_path: &Path, runtime_dir: &Path) -> EnvVarGuard {
    let guard = EnvVarGuard::capture(&["ARTERM_SOCKET", "ARTERM_RUNTIME_DIR"]);
...
impl TestEnvGuard {
    fn new() -> anyhow::Result<Self> {
        ...
        let temp_home = tempfile::Builder::new()
            .prefix("arterm-selfdev-test-home-")
            .tempdir()?;
        let env = EnvVarGuard::capture(&["ARTERM_HOME", "ARTERM_TEST_SESSION"]);

        crate::env::set_var("ARTERM_HOME", temp_home.path());
        crate::env::set_var("ARTERM_TEST_SESSION", "1");
...
async fn test_wait_for_reloading_server_returns_false_when_reload_failed() {
    ...
    let socket_path = temp.path().join("arterm.sock");
...
fn isolated_launcher_env() -> (
    ...
    let lock = lock_env();
    let temp = tempfile::tempdir().expect("tempdir");
    let env = EnvVarGuard::capture(&["ARTERM_INSTALL_DIR", "ARTERM_HOME", "HOME", "USERPROFILE"]);
    crate::env::set_var("HOME", temp.path());
    crate::env::set_var("USERPROFILE", temp.path());
    crate::env::remove_var("ARTERM_INSTALL_DIR");
...
#[test]
fn test_launcher_dir_uses_trimmed_install_dir_before_arterm_home() {
    ...
    set_var("ARTERM_HOME", &arterm_home);
...
fn default_launcher_dir(home: &Path) -> PathBuf {
    if cfg!(windows) {
        home.join("AppData").join("Local").join("arterm").join("bin")
    } else {
...
fn test_selfdev_build_command_prefers_repo_wrapper_when_present() {
    ...
    assert!(command.contains("dev_cargo.sh' build --profile selfdev -p arterm --bin arterm"));
    assert!(!command.contains("arterm-desktop"));
    assert!(build.display.contains("-p arterm --bin arterm"));
    assert!(!build.display.contains("arterm-desktop"));
}
...
fn test_selfdev_build_command_falls_back_to_cargo_when_wrapper_missing() {
    ...
    assert_eq!(build.args.first().map(String::as_str), Some("-lc"));
    let command = build.args.get(1).expect("shell command");
    assert!(command.contains("cargo build --profile selfdev -p arterm --bin arterm"));
    assert!(!command.contains("arterm-desktop"));
}
...
fn test_selfdev_build_command_can_target_all() {
    ...
    assert!(
        build
            .display
            .contains("-p arterm-desktop2 --bin arterm-desktop2")
    );
}
