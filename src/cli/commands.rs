...
fn cloud_sessions_config_path() -> Result<PathBuf> {
    Ok(crate::storage::arterm_dir()?.join("cloud_sessions.json"))
}
...
fn cloud_sessions_sync_state_path() -> Result<PathBuf> {
    Ok(crate::storage::arterm_dir()?.join("cloud_sessions_sync.json"))
}
...
fn resolve_sync_sessions_dir(override_path: Option<&str>) -> Result<PathBuf> {
    ...
    Ok(crate::storage::arterm_dir()?.join("sessions"))
}
...
fn resolve_jade_sessions_helper(override_path: Option<&str>) -> Result<PathBuf> {
    ...
    if let Some(path) = std::env::var_os("ARTERM_JADE_SESSIONS_HELPER")
    ...
    anyhow::bail!(
        "Could not find Jade session helper. Set --helper PATH or ARTERM_JADE_SESSIONS_HELPER. Expected a private helper like ~/jade/scripts/jade_sessions.py"
    );
}
...
async fn run_ambient_visible() -> Result<()> {
    ...
    let _ = crossterm::execute!(
        std::io::stdout(),
        crossterm::terminal::SetTitle(terminal_title("🤖 arterm ambient cycle"))
    );
...
pub fn run_pair_command(list: bool, revoke: Option<String>) -> Result<()> {
    ...
    if !gw_config.enabled {
        eprintln!("\x1b[33m⚠\x1b[0m  Gateway is disabled. Enable it in ~/.arterm/config.toml:\n");
        eprintln!("    \x1b[2m[gateway]\x1b[0m");
        eprintln!("    \x1b[2menabled = true\x1b[0m");
        eprintln!("    \x1b[2mport = {}\x1b[0m\n", gw_config.port);
        eprintln!("  Then restart the arterm server.\n");
    }

    let code = registry.generate_pairing_code();
    let connect_host = resolve_connect_host(&gw_config.bind_addr);
    let pair_uri = format!(
        "arterm://pair?host={}&port={}&code={}",
        connect_host, gw_config.port, code
    );

    eprintln!();
    eprintln!("  \x1b[1mScan with the arterm iOS app:\x1b[0m\n");
    ...
    if connect_host == gateway::UNKNOWN_CONNECT_HOST {
        eprintln!(
            "\n  \x1b[33mTip:\x1b[0m set ARTERM_GATEWAY_HOST to your reachable Tailscale hostname."
        );
    }
    ...
    match action {
        ...
        "status" => {
            ...
            if status.ready {
                ...
                println!(
                    "\nThe browser bridge is connected, but the installed Firefox extension is out of date for this arterm build. Run `arterm browser setup` to repair or update it."
                );
            } else {
                println!("\nRun `arterm browser setup` to install or repair it.");
            }
        }
        other => {
...
pub async fn run_server_reload_command(force: bool, emit_json: bool) -> Result<()> {
    ...
    // `arterm server reload` unconditionally after swapping the binary.
    if !crate::server::has_live_listener(&socket).await {
        ...
        let detail = if reaped {
            "No running arterm server found; cleared a stale socket.".to_string()
        } else {
            "No running arterm server found; nothing to reload.".to_string()
        };
    ...
    if skipped && !reloading {
        return emit(ServerReloadReport {
            ...
            handoff_ready: true,
            detail: "arterm server is already running the newest binary; no reload needed."
                .to_string(),
        });
    }
    ...
    let detail = if handoff_ready {
        "arterm server reloaded onto the newest binary.".to_string()
    } else {
        "arterm server reload requested; the new server is still coming up.".to_string()
    };
...
pub async fn run_server_stop_command(force: bool, emit_json: bool) -> Result<()> {
    ...
    if !force {
        let msg = "`arterm server stop` terminates the daemon and drops any live headless/swarm sessions. \
Prefer `arterm server reload` to pick up an upgrade gracefully. \
Re-run with `--force` if you really want to stop the server.";
    ...
    if let Some(info) = server_info.as_ref() {
        ...
        if crate::platform::is_process_running(pid) {
            #[cfg(unix)]
            {
                ...
                match crate::platform::signal_detached_process_group(pid, libc::SIGTERM) {
                    Ok(()) => {
                        ...
                        detail = format!("Sent SIGTERM to arterm server (pid {pid}).");
                    }
                    Err(e) => {
                        detail = format!("Failed to signal arterm server (pid {pid}): {e}");
                    }
                }
            }
            #[cfg(not(unix))]
            {
                match crate::platform::signal_detached_process_group(pid, 0) {
                    Ok(()) => {
                        signaled_pid = Some(pid);
                        detail = format!("Terminated arterm server (pid {pid}).");
                    }
                    Err(e) => {
                        detail = format!("Failed to terminate arterm server (pid {pid}): {e}");
                    }
                }
            }
        } else {
            detail = format!("Registered arterm server (pid {pid}) is not running.");
        }
        ...
        detail = "No running arterm server found.".to_string();
    }
    ...
    if emit_json {
        ...
        if stopped && signaled_pid.is_some() {
            println!("arterm server stopped.");
        } else if stopped && !had_listener && signaled_pid.is_none() {
            // Nothing was running; this is still a success for an installer.
        } else if !stopped {
            println!(
                "arterm server did not exit cleanly; it may still be shutting down. Re-run if needed."
            );
        }
        if reaped {
            println!("Cleared a stale arterm socket.");
        }
    }
...
pub async fn run_single_message_command(
    ...
    // Load MCP servers from ~/.arterm/mcp.json so headless `arterm run` has the
    ...
    if run_command_mcp_enabled() {
        ...
        // for it, and a single-turn `arterm run` locks its tool snapshot before
        // the background connection finishes, so the model would never see those
        // tools. Long-lived sessions recover on a later turn, but `arterm run`
...
fn run_command_auto_poke_enabled() -> bool {
    std::env::var("ARTERM_RUN_AUTO_POKE")
        .ok()
        .map(|value| {
...
/// Whether headless `arterm run` should load MCP servers from `~/.arterm/mcp.json`.
/// Enabled by default; set `ARTERM_RUN_MCP=0` (or `false`/`off`/`no`) to skip MCP
/// registration for latency-sensitive scripting. (#390)
fn run_command_mcp_enabled() -> bool {
    std::env::var("ARTERM_RUN_MCP")
        .ok()
        .map(|value| {
            let value = value.trim().to_ascii_lowercase();
            !matches!(value.as_str(), "0" | "false" | "off" | "no")
...
/// Max time `arterm run` waits for cold-cache MCP servers to register their
/// tools before running the single turn. Override with `ARTERM_RUN_MCP_WAIT_MS`
/// (0 disables the wait).
fn run_command_mcp_cold_wait() -> std::time::Duration {
    let ms = std::env::var("ARTERM_RUN_MCP_WAIT_MS")
        .ok()
...
/// Bridge the cold-cache gap for `arterm run`: if any configured MCP server has
...
async fn wait_for_cold_cache_mcp_tools(registry: &crate::tool::Registry) {
    ...
    crate::logging::info(&format!(
        "arterm run: waiting up to {}ms for cold-cache MCP server(s) to register tools: {}",
        budget.as_millis(),
        cold_servers.join(", ")
    ));
    ...
    loop {
        ...
        if covered {
            crate::logging::info(
                "arterm run: cold-cache MCP server(s) registered tools; proceeding",
            );
            return;
        }
        if std::time::Instant::now() >= deadline {
            crate::logging::warn(
                "arterm run: timed out waiting for cold-cache MCP server(s); \
...
fn run_command_auto_poke_max_turns() -> Option<usize> {
    std::env::var("ARTERM_RUN_AUTO_POKE_MAX_TURNS")
        .ok()
    ...
    loop {
        ...
        match build_run_auto_poke_follow_up_from_todos(
            ...
            Some(RunAutoPokeFollowUp::GateDigest { message }) => {
                ...
                eprintln!(
                    "We asked the agent to double-check this turn's weak points. Set ARTERM_RUN_AUTO_POKE=0 to disable."
                );
                continue;
            }
            Some(RunAutoPokeFollowUp::ConfidenceSummary {
                ...
                next_message = message;
                eprintln!(
                    "Todos are done. Asking the agent for a final confidence check. Set ARTERM_RUN_AUTO_POKE=0 to disable."
                );
                continue;
            }
            Some(RunAutoPokeFollowUp::Incomplete { count, message }) => {
                ...
                next_message = message;
                eprintln!(
                    "{} incomplete todo(s). We poked the agent for you. Set ARTERM_RUN_AUTO_POKE=0 to disable.",
                    count
                );
            }
            None => break,
