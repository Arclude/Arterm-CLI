...
fn server_start_and_internal_keepalive_parse() {
    let args = Args::try_parse_from(["arterm", "server", "start", "--json"])
    ...
    let keepalive = Args::try_parse_from(["arterm", "server", "keepalive"])
...
fn test_provider_choice_aliases_parse() {
    let args = Args::try_parse_from(["arterm", "--provider", "z.ai", "run", "smoke"]).unwrap();
    assert_eq!(args.provider, ProviderChoice::Zai);

    let args =
        Args::try_parse_from(["arterm", "--provider", "kimi-for-coding", "run", "smoke"]).unwrap();
    assert_eq!(args.provider, ProviderChoice::Kimi);

    let args =
        Args::try_parse_from(["arterm", "--provider", "cerebrascode", "run", "smoke"]).unwrap();
    assert_eq!(args.provider, ProviderChoice::Cerebras);

    let args = Args::try_parse_from(["arterm", "--provider", "compat", "run", "smoke"]).unwrap();
    assert_eq!(args.provider, ProviderChoice::OpenaiCompatible);

    let args = Args::try_parse_from(["arterm", "--provider", "bailian", "run", "smoke"]).unwrap();
    assert_eq!(args.provider, ProviderChoice::AlibabaCodingPlan);

    let args = Args::try_parse_from(["arterm", "--provider", "together", "run", "smoke"]).unwrap();
    assert_eq!(args.provider, ProviderChoice::TogetherAi);

    let args = Args::try_parse_from(["arterm", "--provider", "grok", "run", "smoke"]).unwrap();
    assert_eq!(args.provider, ProviderChoice::Xai);

    let args = Args::try_parse_from(["arterm", "--provider", "cgc", "run", "smoke"]).unwrap();
...
fn serve_server_name_option_parses() {
    let args =
        Args::try_parse_from(["arterm", "serve", "--server-name", "mount-cloud/fabian"]).unwrap();
    match args.command {
...
fn model_list_subcommand_parses() {
    let args = Args::try_parse_from(["arterm", "model", "list", "--json", "--verbose"]).unwrap();
    match args.command {
...
fn session_rename_subcommand_parses() {
    ...
    let args = Args::try_parse_from(["arterm", "session", "rename", "fox", "--clear"]).unwrap();
    match args.command {
        Some(Command::Session(SessionCommand::Rename {
            session,
            name,
...
fn cloud_sessions_subcommands_parse() {
    ...
    let args = Args::try_parse_from(["arterm", "cloud", "sessions", "status", "--json"]).unwrap();
    match args.command {
        Some(Command::Cloud(CloudCommand::Sessions {
...
fn login_no_browser_flag_parses() {
    let args = Args::try_parse_from(["arterm", "login", "--no-browser"]).unwrap();
    match args.command {
        Some(Command::Login {
            provider,
            account,
    ...
    let args = Args::try_parse_from(["arterm", "login", "--headless"]).unwrap();
    match args.command {
...
fn login_accepts_provider_positional() {
    let args = Args::try_parse_from(["arterm", "login", "google"]).unwrap();
    match args.command {
...
fn login_scriptable_flags_parse() {
    let args = Args::try_parse_from(["arterm", "login", "--print-auth-url", "--json"]).unwrap();
    match args.command {
        Some(Command::Login {
            print_auth_url,
            json,
    ...
    let args = Args::try_parse_from(["arterm", "login", "--auth-code", "abc123"]).unwrap();
    match args.command {
...
fn account_subcommands_parse() {
    ...
    let status =
        Args::try_parse_from(["arterm", "account", "status", "--json"]).expect("account status");
    assert!(matches!(
        status.command,
        Some(Command::Account {
    ...
    let manage = Args::try_parse_from(["arterm", "account", "manage"]).expect("account manage");
    assert!(matches!(
        manage.command,
        Some(Command::Account {
    ...
    let logout = Args::try_parse_from(["arterm", "account", "logout"]).expect("account logout");
    assert!(matches!(
        logout.command,
        Some(Command::Account {
...
fn quiet_global_flag_parses() {
    let args = Args::try_parse_from(["arterm", "--quiet", "model", "list"]).unwrap();
    assert!(args.quiet);
}

#[test]
fn acp_subcommand_parses() {
    let args = Args::try_parse_from(["arterm", "acp"]).unwrap();
    match args.command {
        Some(Command::Acp) => {}
        other => panic!("unexpected command: {:?}", other),
    }
...
fn run_json_subcommand_parses() {
    let args = Args::try_parse_from(["arterm", "run", "--json", "hello"]).unwrap();
    match args.command {
        Some(Command::Run {
            json,
            ndjson,
...
fn run_ndjson_subcommand_parses() {
    let args = Args::try_parse_from(["arterm", "run", "--ndjson", "hello"]).unwrap();
    match args.command {
        Some(Command::Run {
            json,
            ndjson,
...
fn version_subcommand_parses() {
    let args = Args::try_parse_from(["arterm", "version", "--json"]).unwrap();
    match args.command {
...
fn usage_subcommand_parses() {
    let args = Args::try_parse_from(["arterm", "usage", "--json"]).unwrap();
    match args.command {
...
fn auth_status_subcommand_parses() {
    let args = Args::try_parse_from(["arterm", "auth", "status", "--json"]).unwrap();
    match args.command {
...
fn auth_doctor_subcommand_parses() {
    let args = Args::try_parse_from(["arterm", "auth", "doctor", "openai", "--validate", "--json"])
        .unwrap();
    match args.command {
...
fn provider_list_subcommand_parses() {
    let args = Args::try_parse_from(["arterm", "provider", "list", "--json"]).unwrap();
    match args.command {
...
fn provider_current_subcommand_parses() {
    let args = Args::try_parse_from(["arterm", "provider", "current", "--json"]).unwrap();
    match args.command {
...
fn restart_save_subcommand_parses() {
    let args = Args::try_parse_from(["arterm", "restart", "save"]).unwrap();
    match args.command {
...
fn restart_save_auto_restore_flag_parses() {
    let args = Args::try_parse_from(["arterm", "restart", "save", "--auto-restore"]).unwrap();
    match args.command {
        Some(Command::Restart {
...
/// `arterm-tui::tui::app::onboarding_repair::build_repair_brief`). The brief
...
fn onboarding_repair_brief_commands_are_valid_cli() {
    // Diagnose.
    Args::try_parse_from(["arterm", "auth-test", "--provider", "openai", "--json"])
        .expect("auth-test --provider --json must parse");
    Args::try_parse_from(["arterm", "auth-test", "--all-configured", "--json"])
        .expect("auth-test --all-configured --json must parse");
    Args::try_parse_from(["arterm", "auth", "doctor"]).expect("auth doctor must parse");

    // Fix: OAuth and API-key logins.
    Args::try_parse_from(["arterm", "login", "--provider", "openai"])
        .expect("login --provider must parse");
    Args::try_parse_from(["arterm", "login", "--provider", "openai", "--api-key", "k"])
...
fn api_bridge_socket_flags_do_not_collide() {
    ...
    let ambiguous = Args::try_parse_from(["arterm", "api-bridge", "--socket", "/tmp/x.sock"]).ok();
    assert!(
        matches!(
