...
fn only_file_controlled_debug_clients_need_parent_lifetime_binding() {
    ...
    let previous = std::env::var_os("ARTERM_DEBUG_CMD_PATH");
    crate::env::remove_var("ARTERM_DEBUG_CMD_PATH");
    assert!(!is_file_controlled_debug_client());

    crate::env::set_var("ARTERM_DEBUG_CMD_PATH", "/tmp/arterm-test-debug-command");
    assert!(is_file_controlled_debug_client());

    if let Some(previous) = previous {
        crate::env::set_var("ARTERM_DEBUG_CMD_PATH", previous);
    } else {
        crate::env::remove_var("ARTERM_DEBUG_CMD_PATH");
    }
}
...
fn init_and_systemd_are_recognized_as_orphan_adopters() {
    ...
    assert!(!is_orphan_adopter_name("arterm\n"));
}
...
fn auth_doctor_positional_provider_wins_over_global_provider() {
    assert_eq!(
        ...
        Some("openai"),
        "`arterm --provider cerebras auth doctor openai` should diagnose the explicit positional provider"
    );
}
...
impl ReloadTestEnv {
    fn new() -> Self {
        ...
        let prev_runtime = std::env::var_os("ARTERM_RUNTIME_DIR");
        crate::server::set_socket_path(socket_path.to_str().expect("utf8 socket path"));
        crate::env::set_var("ARTERM_RUNTIME_DIR", temp.path());
...
impl Drop for ReloadTestEnv {
    fn drop(&mut self) {
        ...
        if let Some(prev_socket) = &self.prev_socket {
            crate::env::set_var("ARTERM_SOCKET", prev_socket);
        } else {
            crate::env::remove_var("ARTERM_SOCKET");
        }
        if let Some(prev_runtime) = &self.prev_runtime {
            crate::env::set_var("ARTERM_RUNTIME_DIR", prev_runtime);
        } else {
...
fn resume_failure_defers_to_server_during_reload_handoff() {
    ...
    // update/restart handoff is in progress (ARTERM_RESUMING set), we must defer
    // to the server instead of exiting with "No session found matching ...".
    let with_resuming = |key: &str| {
        if key == "ARTERM_RESUMING" {
