...
pub async fn run() -> Result<()> {
    ...
    std::thread::Builder::new()
        .name("arterm-memlog-cleanup".to_string())
    ...
    // provider registry. These crates sit downstream of arterm-base (so
...
/// Register provider runtimes that live downstream of `arterm-base` with the
...
pub fn register_external_provider_runtimes() {
    ...
    crate::provider::external::register_external_provider(
        ...
        || std::sync::Arc::new(arterm_provider_antigravity_runtime::AntigravityProvider::new()),
    );
    crate::provider::external::register_external_provider(
        crate::provider::external::CLAUDE_CLI_RUNTIME,
        || std::sync::Arc::new(arterm_provider_claude_cli_runtime::ClaudeProvider::new()),
    );
    crate::provider::external::register_external_provider(
        crate::provider::external::ANTHROPIC_RUNTIME,
        || std::sync::Arc::new(arterm_provider_anthropic_runtime::AnthropicProvider::new()),
    );
    ...
    crate::provider::external::register_external_provider_fallible(
        ...
        || {
            let provider = match crate::auth::codex::load_credentials() {
                Ok(credentials) => arterm_provider_openai_runtime::OpenAIProvider::new(credentials),
                Err(_) => arterm_provider_openai_runtime::OpenAIProvider::new_browser_only(),
            };
    ...
    crate::provider::external::register_external_provider_fallible(
        ...
        || {
            let provider = std::sync::Arc::new(
                arterm_provider_copilot_runtime::CopilotApiProvider::new().ok()?,
            );
            let eager_tier_detection = std::env::var("ARTERM_NON_INTERACTIVE").is_err();
                ...
                && update_available
            {
                ...
                if hot_exec::local_commits_ahead_of_upstream() == Some(true) {
                    ...
                    if auto_update {
                        ...
                        logging::info(
                            "Update available! Run `arterm update` or `/reload` to update.",
                        );
                    }
                }
            } else {
...
fn report_main_error(error: &anyhow::Error) {
    ...
    if let Some(session_id) = terminal::get_current_session() {
        ...
        output::stderr_info(format!("  arterm --resume {}", session_id));
        output::stderr_blank_line();
    }
}
...
mod tests {
    ...
    fn auto_install_allowed_without_live_terminal() {
        let args = parse_args(&["arterm", "login"]);
    ...
    fn auto_install_respects_explicit_disable_even_without_terminal() {
        let mut args = parse_args(&["arterm", "login"]);
    ...
    fn update_command_still_skips_background_check_before_auto_install_logic() {
        let args = parse_args(&["arterm", "update"]);
    ...
    fn hidden_spawn_hotkey_argument_is_global_and_preserves_canonical_text() {
        let args = parse_args(&["arterm", "--spawn-hotkey", "shift+cmd+'", "self-dev"]);
