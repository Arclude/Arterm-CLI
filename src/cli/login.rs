    ...
    match choice {
        ProviderChoice::Auto => {
            if options.uses_scriptable_flow()? {
                anyhow::bail!(
                    "Scriptable login flags require an explicit provider. Use `arterm login --provider <provider> ...`."
                );
            }
            ...
            if !io::stdin().is_terminal() {
                anyhow::bail!(
                    "`arterm login --provider auto` requires an interactive terminal. Use `arterm login --provider <provider>` in non-interactive mode."
                );
            }
...
pub async fn run_login_provider(
    ...
    let login_result = if explicit_scriptable_flow {
        ...
        match provider.target {
            ...
            LoginProviderTarget::Arterm => login_arterm_flow(options.no_browser)
                .await
...
/// Best-effort: tell a running arterm server that on-disk auth has changed so it
...
async fn login_arterm_flow(no_browser: bool) -> Result<()> {
    eprintln!("Starting arterm subscription sign-in...");
    let _ = arterm_device::login_arterm_device_flow(no_browser).await?;
    Ok(())
}

pub(crate) async fn run_arterm_account_login(no_browser: bool) -> Result<()> {
    login_arterm_flow(no_browser).await
}
...
async fn login_claude_flow(requested_label: Option<&str>, no_browser: bool) -> Result<()> {
    ...
    eprintln!(
        "Account '{}' stored at {}",
        label,
        auth::claude::arterm_path()?.display()
    );
...
fn login_azure_flow() -> Result<()> {
    ...
    eprintln!(
        "Reference: OpenCode supports Azure OpenAI with Entra credentials. arterm uses Azure OpenAI's newer `/openai/v1` API with either Microsoft Entra ID or an API key.\n"
    );
    ...
    if use_entra {
        ...
        eprintln!(
            "That means arterm can authenticate via `az login`, managed identity, or Azure environment credentials."
        );
    } else {
...
fn login_cursor_flow() -> Result<()> {
    ...
    eprintln!("arterm will use the native Cursor HTTPS transport.");
...
async fn login_antigravity_flow(no_browser: bool) -> Result<()> {
    ...
    eprintln!(
        "arterm will authenticate directly with Google Antigravity; the Antigravity desktop app is not required."
    );
    eprintln!(
        "If browser launch fails, or you pass `--no-browser`, arterm will prompt for the callback URL instead."
    );
    eprintln!(
...
async fn login_gemini_flow(no_browser: bool) -> Result<()> {
    ...
    eprintln!(
        "If browser launch fails, or you pass `--no-browser`, arterm will prompt for the manual authorization code."
    );
    eprintln!(
        ...
        Err(_) => {
            ...
            match input.trim() {
                ...
                "3" => {
                    ...
                    eprintln!("   - Fill in app name (e.g. 'arterm') and your email");
