...
//! coordinated build queue), the freshly built `target/selfdev/arterm` has no
...
//!
//! ```sh
//! cargo run -p arterm-build-support --example write_dev_sidecar
//! ```
fn main() -> anyhow::Result<()> {
    let repo = std::env::current_dir()?;
    let state = arterm_build_support::current_source_state(&repo)?;
    let path = arterm_build_support::write_current_dev_binary_source_metadata(&repo, &state)?;
