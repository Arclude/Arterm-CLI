...
pub fn builds_dir() -> Result<PathBuf> {
    let dir = resolve_builds_dir(
        std::env::var_os("ARTERM_HOME").map(PathBuf::from),
...
fn resolve_builds_dir(
    arterm_home: Option<PathBuf>,
    local_app_data: Option<PathBuf>,
    default_arterm_dir: PathBuf,
    is_windows: bool,
) -> PathBuf {
    if let Some(arterm_home) = arterm_home {
        return arterm_home.join("builds");
    }

    if is_windows && let Some(local_app_data) = local_app_data {
        // Keep runtime channel discovery aligned with scripts/install.ps1 and
        // the supported Windows layout under %LOCALAPPDATA%\arterm\builds.
        // Durable user state and logs still live under ~/.arterm.
        return local_app_data.join("arterm").join("builds");
    }

    default_arterm_dir.join("builds")
}
...
mod tests {
    ...
    fn windows_builds_use_local_app_data() {
        let resolved = resolve_builds_dir(
            ...
            PathBuf::from("/home/test/.arterm"),
            true,
        );

        assert_eq!(resolved, PathBuf::from("/local-app-data/arterm/builds"));
    }

    #[test]
    fn arterm_home_override_wins_on_windows() {
        let resolved = resolve_builds_dir(
            Some(PathBuf::from("/isolated-arterm")),
            Some(PathBuf::from("/local-app-data")),
            PathBuf::from("/home/test/.arterm"),
            true,
        );

        assert_eq!(resolved, PathBuf::from("/isolated-arterm/builds"));
    }

    #[test]
    fn non_windows_builds_stay_under_arterm_home() {
        let resolved = resolve_builds_dir(
            None,
        ...
        assert_eq!(resolved, PathBuf::from("/home/test/.arterm/builds"));
    }
}
