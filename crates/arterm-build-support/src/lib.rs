...
pub use paths::{
    ...
    find_repo_in_ancestors, get_repo_dir, is_arterm_repo, launcher_binary_path, launcher_dir,
...
pub fn smoke_test_server_binary(binary: &Path) -> Result<()> {
    ...
    let socket_path = temp.path().join("arterm-smoke.sock");
    ...
    let mut child = Command::new(binary)
        ...
        .env("ARTERM_TEMP_SERVER_IDLE_SECS", "300")
        .stdin(Stdio::null())
        .stdout(Stdio::null())
