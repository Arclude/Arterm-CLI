...
/// Get the arterm repository directory
pub fn get_repo_dir() -> Option<PathBuf> {
    if let Ok(path) = std::env::var("ARTERM_REPO_DIR") {
        let path = PathBuf::from(path);
        if is_arterm_repo(&path) {
            return Some(path);
        }
    }
...
///
/// Release archives install a tiny `arterm` wrapper script alongside the real
/// `arterm-<platform>.bin` payload (the wrapper sets `LD_LIBRARY_PATH` and execs
...
pub fn selfdev_build_command_for_target(
    ...
    let specs = match target {
        SelfDevBuildTarget::Tui => vec![("arterm", "arterm")],
        ...
        SelfDevBuildTarget::Desktop2 => vec![
            ("arterm-desktop2", "arterm-desktop2"),
            ("arterm-harness-api-server", "arterm-harness-api-bridge"),
        ],
...
fn build_target_for_paths<'a>(paths: impl Iterator<Item = &'a str>) -> SelfDevBuildTarget {
    ...
    for path in paths {
        ...
        if path == "Cargo.toml" || path == "Cargo.lock" || path.starts_with(".cargo/") {
            ...
            desktop2 = true;
            other = true;
        } else if path.starts_with("crates/arterm-desktop2/") {
            desktop2 = true;
        } else {
            other = true;
        }
...
///
/// Defaults to `~/.local/bin` on Unix, `%LOCALAPPDATA%\arterm\bin` on Windows.
/// Overridable with `ARTERM_INSTALL_DIR`.
pub fn launcher_dir() -> Result<PathBuf> {
    if let Some(custom) = non_empty_env_path("ARTERM_INSTALL_DIR") {
        return Ok(custom);
    }

    if let Some(sandbox_home) = non_empty_env_path("ARTERM_HOME") {
    ...
    #[cfg(windows)]
    {
        if let Ok(local) = std::env::var("LOCALAPPDATA") {
            return Ok(PathBuf::from(local).join("arterm").join("bin"));
        }
        Ok(home_dir()?
            .join("AppData")
            .join("Local")
...
/// Path to the launcher binary (`~/.local/bin/arterm` by default).
...
mod tests {
    ...
    fn repo_fixture(git_file: bool) -> tempfile::TempDir {
        ...
        if git_file {
            std::fs::write(temp.path().join(".git"), "gitdir: /tmp/arterm-test-git\n")
                .expect("git file");
        } else {
    ...
    fn find_repo_in_ancestors_finds_workspace_from_crate_dir() {
        ...
        let crate_dir = repo.path().join("crates").join("arterm-build-support");
    ...
    fn every_build_target_builds_its_own_package() {
        let repo = repo_fixture(false);
        let cases = [
            (SelfDevBuildTarget::Tui, vec!["-p arterm "]),
            (
                SelfDevBuildTarget::Desktop2,
                vec!["-p arterm-desktop2 ", "--bin arterm-harness-api-bridge"],
            ),
            (
    ...
    fn single_targets_do_not_build_other_binaries() {
        ...
        assert!(!tui.display.contains("arterm-desktop"));
        let desktop2 = selfdev_build_command_for_target(repo.path(), SelfDevBuildTarget::Desktop2);
        assert!(!desktop2.display.contains("-p arterm "));
    }
    ...
    fn auto_routes_changed_paths_to_the_right_binary() {
        let cases: Vec<(Vec<&str>, SelfDevBuildTarget)> = vec![
            ...
            (
                vec!["crates/arterm-desktop2/src/editor.rs", "src/main.rs"],
                SelfDevBuildTarget::All,
            ),
    ...
    fn porcelain_lines_are_parsed_including_renames() {
        ...
        assert_eq!(
            porcelain_path("?? crates/arterm-desktop2/src/new.rs"),
            "crates/arterm-desktop2/src/new.rs"
        );
        assert_eq!(
            porcelain_path("R  old/path.rs -> crates/arterm-desktop2/src/moved.rs"),
            "crates/arterm-desktop2/src/moved.rs"
        );
    }
    ...
    fn build_targets_parse_from_their_names() {
        for (name, expected) in [
            ...
            ("arterm-desktop2", SelfDevBuildTarget::Desktop2),
    ...
    fn release_install_fixture() -> (tempfile::TempDir, PathBuf, PathBuf) {
        ...
        let wrapper = temp.path().join("arterm");
        let payload = temp.path().join("arterm-linux-x86_64.bin");
        std::fs::write(
            &wrapper,
    ...
    fn resolve_binary_payload_follows_channel_symlink_to_wrapper() {
        // The real layout: builds/<channel>/arterm -> versions/<v>/arterm (wrapper).
        ...
        let link = channel_dir.join("arterm");
        #[cfg(unix)]
    ...
    fn resolve_binary_payload_keeps_real_binary() {
        ...
        let temp = tempfile::TempDir::new().expect("temp install");
        let binary = temp.path().join("arterm");
        std::fs::write(&binary, vec![0x7fu8; 8192]).expect("binary");
        std::fs::write(temp.path().join("arterm-linux-x86_64.bin"), [0u8; 8]).expect("bin");
        assert_eq!(
    ...
    fn resolve_binary_payload_keeps_small_script_without_payload() {
        let temp = tempfile::TempDir::new().expect("temp install");
        let script = temp.path().join("arterm");
    ...
    fn resolve_binary_payload_refuses_ambiguous_payloads() {
        ...
        std::fs::write(temp.path().join("arterm-macos-aarch64.bin"), [0u8; 8]).expect("second bin");
        assert_eq!(
            resolve_binary_payload(&wrapper),
