...
fn with_temp_arterm_home<T>(f: impl FnOnce() -> T) -> T {
    ...
    arterm_core::env::set_var("ARTERM_HOME", temp_home.path());
    let result = f();
...
fn test_binary_version_hash_mismatch_rejects_publish_candidate() {
    ...
    let error = validate_binary_version_matches_source_report(&report, Path::new("arterm"), &source)
...
fn test_find_repo_in_ancestors_walks_upward() {
    let temp = tempfile::tempdir().expect("tempdir");
    let repo = temp.path().join("arterm-repo");
...
#[test]
fn launcher_dir_uses_sandbox_bin_when_arterm_home_is_set() {
    with_temp_arterm_home(|| {
        let launcher_dir = launcher_dir().expect("launcher dir");
        let expected = storage::arterm_dir().expect("arterm dir").join("bin");
...
fn update_launcher_symlink_stays_inside_sandbox_home() {
    with_temp_arterm_home(|| {
        ...
        let expected_launcher = storage::arterm_dir()
            .expect("arterm dir")
            .join("bin")
            .join(binary_name());
...
fn daemon_reload_target_version() -> Option<String> {
    ...
    // versions/<version>/arterm -> <version>
    canonical
        .parent()
...
/// Write a distinct, real binary into `versions/<version>/arterm` with an
