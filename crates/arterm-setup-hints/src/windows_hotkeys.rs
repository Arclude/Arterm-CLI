...
//! low-level keyboard hook in the first-party Rust listener (`arterm setup-hotkey
...
/// Returns `None` for keys with no stable VK code. arterm `cmd` maps to Alt
...
/// Translate a canonical arterm key token into a Win32 virtual-key code.
...
/// User-facing chord rendering for Windows notices: arterm's `cmd` modifier
...
fn ps_dir_expr(dir: &str, last_dir_file: &str, last_repo_file: &str) -> String {
    match dir {
        "$HOME" => "(Resolve-ArtermDir $null)".to_string(),
        "$LAST_DIR" => format!("(Resolve-ArtermDir {})", ps_quote(last_dir_file)),
        "$LAST_REPO" => format!("(Resolve-ArtermDir {})", ps_quote(last_repo_file)),
        path => format!("(Resolve-ArtermFixedDir {})", ps_quote(path)),
    }
}

/// Render the full hotkey-listener PowerShell script.
///
/// `launch_exe`/`launch_args` describe the terminal command that opens arterm
/// (e.g. `wt.exe` + profile args, or `alacritty -e arterm`); `{DIR}` never
...
pub(crate) fn render_windows_listener_ps1(
    ...
    for (index, entry) in entries.iter().enumerate() {
        ...
        registrations.push_str(&format!(
            "Register-ArtermHotkey -Id 0x{id:X} -Mods 0x{mods:X} -Vk 0x{vk:X} -Label {label}\n",
    ...
    Some(format!(
        r#"# arterm global launch hotkey listener
# Auto-generated by arterm setup-hotkey from [launch_hotkeys] config. Runs at
# login via a startup shortcut. Do not edit; re-run `arterm setup-hotkey`.
...
function Register-ArtermHotkey {{
    ...
    if ([HotKeyHelper]::RegisterHotKey([IntPtr]::Zero, $Id, $Mods -bor $MOD_NOREPEAT, $Vk)) {{
        ...
        Write-Warning "arterm: failed to register hotkey $Label (already claimed?)"
    }}
}}

# Resolve a dynamic launch dir from a tracking file, falling back to $HOME.
function Resolve-ArtermDir {{
    param([string]$File)
    if ($File) {{
        try {{
...
function Resolve-ArtermFixedDir {{
    param([string]$Dir)
    if ($Dir -and (Test-Path -LiteralPath $Dir -PathType Container)) {{ return $Dir }}
    return $env:USERPROFILE
}}
...
if ($script:RegisteredIds.Count -eq 0) {{
    Write-Error "arterm: no launch hotkeys could be registered"
    exit 1
}}
...
mod tests {
    ...
    fn args_for(entry: &WindowsHotkey) -> String {
        ...
        if entry.self_dev {
            format!(r#"-e "C:\arterm.exe" --spawn-hotkey "{hotkey}" self-dev"#)
        } else {
            format!(r#"-e "C:\arterm.exe" --spawn-hotkey "{hotkey}""#)
        }
    }
    ...
    fn listener_script_registers_each_entry_and_dispatches_dirs() {
        let entries = vec![
            hk("win+shift+f23", "C:\\Users\\u\\arterm", "arterm", false),
        ...
        let script = render_windows_listener_ps1(
            ...
            "wt.exe",
            args_for,
            "C:\\Users\\u\\.arterm\\hotkey\\last_dir",
            "C:\\Users\\u\\.arterm\\hotkey\\last_repo",
        )
        .unwrap();

        // Three registrations with distinct ids.
        assert_eq!(script.matches("Register-ArtermHotkey").count(), 3 + 1); // 3 calls + fn def
        ...
        assert!(script.contains("Resolve-ArtermFixedDir 'C:\\Users\\u\\arterm'"));
        assert!(script.contains("Resolve-ArtermDir $null"));
        assert!(script.contains("Resolve-ArtermDir 'C:\\Users\\u\\.arterm\\hotkey\\last_repo'"));
