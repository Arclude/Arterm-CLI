use super::{
    ...
    escape_shell_single_quotes, launch_command_for_macos_terminal, paused_arterm_shell_command,
    save_preferred_macos_terminal,
};
use anyhow::{Context, Result};
use std::path::{Path, PathBuf};

const MACOS_APP_ICON_FILE_NAME: &str = "Arterm.icns";
const MACOS_APP_ICON_BYTES: &[u8] = include_bytes!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../../assets/app-icons/Arterm.icns"
));
const MACOS_NOTIFICATION_APP_NAME: &str = "Arterm Notifications.app";
const MACOS_NOTIFICATION_EXECUTABLE: &str = "arterm-notification-broker";
const MACOS_NOTIFICATION_VERSION_MARKER: &str = "arterm-broker-version";
...
pub(super) fn install_macos_app_launcher() -> Result<(PathBuf, MacTerminalKind)> {
    ...
    let launcher_path = macos_dir.join("arterm-launcher");
...
<dict>
    ...
    <string>Arterm</string>
    <key>CFBundleDisplayName</key>
    <string>Arterm</string>
    <key>CFBundleIdentifier</key>
    <string>com.arterm.launcher</string>
    <key>CFBundleVersion</key>
    <string>{version}</string>
    <key>CFBundleShortVersionString</key>
    <string>{version}</string>
    <key>CFBundleExecutable</key>
    <string>arterm-launcher</string>
...
fn macos_app_launcher_dir() -> Result<PathBuf> {
    ...
    Ok(home.join("Applications").join("Arterm.app"))
}
...
fn macos_notification_broker_is_valid(app_dir: &Path) -> bool {
    app_dir.is_dir()
        ...
        && std::fs::read_to_string(macos_notification_broker_marker_path(app_dir))
            .is_ok_and(|version| version.trim() == arterm_build_meta::version())
}

/// Install a faceless Notification Center owner using the already-built arterm
...
fn install_macos_notification_broker(arterm_executable: &Path) -> Result<PathBuf> {
    ...
    let staging = parent.join(format!(
        ".Arterm Notifications.app.installing-{}",
        std::process::id()
    ));
    ...
    if std::fs::hard_link(arterm_executable, &broker_executable).is_err() {
        std::fs::copy(arterm_executable, &broker_executable).with_context(|| {
            format!(
                "failed to install notification broker executable from {}",
                arterm_executable.display()
            )
        })?;
    }
    #[cfg(unix)]
...
<dict>
    <key>CFBundleName</key>
    <string>Arterm Notifications</string>
    <key>CFBundleDisplayName</key>
    <string>Arterm Notifications</string>
    <key>CFBundleIdentifier</key>
    <string>com.arterm.notifications</string>
    <key>CFBundleVersion</key>
    <string>{version}</string>
    <key>CFBundleShortVersionString</key>
    <string>{version}</string>
...
/// `Path::exists()` on `arterm.app` also matches `Arterm.app`. The legacy-bundle
...
fn macos_launcher_script(terminal: MacTerminalKind, exe_path: &str, app_dir: &Path) -> String {
    ...
    let shell_command = paused_arterm_shell_command(exe_path);
    ...
    let missing_message = escape_applescript_text(&format!(
        "Arterm could not launch because the executable was not found.\n\nExpected path:\n{}\n\nTry reinstalling arterm or rerun:\narterm setup-launcher",
        exe_path
    ));
    let terminal_failure_message = escape_applescript_text(&format!(
        "Arterm could not open {}.\n\nTry rerunning:\narterm setup-launcher\n\nLauncher log:\n~/.arterm/launcher/macos-launcher.log",
        terminal.label()
    ));
...
LOG_DIR="$HOME/.arterm/launcher"
LOG_FILE="$LOG_DIR/macos-launcher.log"
mkdir -p "$LOG_DIR" >/dev/null 2>&1 || true

show_missing_executable() {{
  /usr/bin/osascript <<'APPLESCRIPT' >/dev/null 2>&1 || true
display alert "Arterm launch failed" message "{missing_message}" as critical
APPLESCRIPT
}}

show_terminal_launch_failure() {{
  /usr/bin/osascript <<'APPLESCRIPT' >/dev/null 2>&1 || true
display alert "Arterm launch failed" message "{terminal_failure_message}" as critical
APPLESCRIPT
}}
