//! Resolve the global "launch a new arterm" hotkeys from config into a concrete
...
//!   [`ResolvedLaunchHotkey`]s. An empty config reproduces arterm's historical
...
/// Build a arterm launch snippet that pauses on non-zero exit so the user can
...
#[cfg(any(test, target_os = "macos"))]
fn paused_arterm_shell_command_with_args(exe_path: &str, args: &[String]) -> String {
...
pub(crate) struct ResolvedLaunchHotkey {
    /// arterm-style chord string, e.g. `cmd+;`.
    pub chord: String,
    ...
    /// fallback) before launching arterm.
    ...
    pub dir: String,
    /// Extra CLI args passed to arterm (e.g. `self-dev`).
    pub args: Vec<String>,
...
/// `launch_arterm_cmd_bracketleft.sh`. Collisions are avoided by appending the
...
fn script_name_for(chord: &str, index: usize) -> String {
    ...
    format!("launch_arterm_{index}_{slug}.sh")
}

/// The built-in default entries, used when config has none. Mirrors arterm's
...
pub(crate) fn shell_command_for(entry: &ResolvedLaunchHotkey, exe_path: &str) -> String {
    format!(
        "{}{}",
        entry.cd_prefix,
        paused_arterm_shell_command_with_args(exe_path, &entry.args)
    )
}

/// Map a arterm key token onto a `global_hotkey` `Code`. Returns `None` for
...
mod tests {
    ...
    fn shell_command_includes_cd_and_exe() {
        ...
        let cmd = shell_command_for(&resolved[0], "/bin/arterm");
        assert!(cmd.contains("/Users/jeremy/proj"));
        assert!(cmd.contains("/bin/arterm"));
    }
}
