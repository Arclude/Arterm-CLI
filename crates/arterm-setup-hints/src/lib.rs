...
//!   notice that it renders arterm poorly and suggest a modern terminal (Ghostty).
//! - Linux: create a .desktop launcher file.
//!
//! Each nudge can be dismissed permanently with "Don't ask again".
//! State is persisted in `~/.arterm/setup_hints.json`.
...
use macos_terminal::{
    ...
    launch_command_for_macos_terminal, paused_arterm_shell_command, save_preferred_macos_terminal,
};
#[cfg(windows)]
use windows_setup::{
...
///   connection.
/// - 3: register three launch hotkeys instead of one. `Cmd+;` opens arterm in
///   `$HOME`, `Cmd+'` opens it in the last project directory, and `Cmd+Shift+'`
///   opens a self-dev session in the last arterm repo. Existing users are
...
///   `arterm-terminal-launch`, avoiding the generated shell-script hop on hotkey
///   press. Scripts/plan are still written for compatibility and diagnostics.
/// - 6: direct launches pass `--spawn-hotkey` into the new Arterm process so
...
impl SetupHintsState {
    fn path() -> Result<PathBuf> {
        Ok(storage::arterm_dir()?.join("setup_hints.json"))
    }
...
fn mac_hotkey_support_dir() -> Result<PathBuf> {
    Ok(storage::arterm_dir()?.join("hotkey"))
}

/// File holding the last project directory arterm was launched from. The `Cmd+'`
/// global hotkey reads this at fire time to reopen arterm there.
#[cfg(any(test, target_os = "macos", target_os = "linux", windows))]
fn mac_hotkey_last_dir_file() -> Result<PathBuf> {
    Ok(mac_hotkey_support_dir()?.join("last_dir"))
}

/// File holding the last arterm *repository* directory the user worked in. The
...
/// Load the `[launch_hotkeys]` table from `~/.arterm/config.toml`.
///
...
fn load_launch_hotkeys_config() -> arterm_config_types::LaunchHotkeysConfig {
    #[derive(serde::Deserialize, Default)]
    struct Wrapper {
        #[serde(default)]
        launch_hotkeys: arterm_config_types::LaunchHotkeysConfig,
    }
    let Ok(dir) = storage::arterm_dir() else {
...
/// its own `Cmd+;` hotkey). When `dir` is inside a arterm repo, the repo root is
...
pub fn record_launch_dirs(dir: &std::path::Path, repo_dir: Option<&std::path::Path>) {
    #[cfg(any(target_os = "macos", target_os = "linux", windows))]
    {
        if let Err(err) = record_launch_dirs_inner(dir, repo_dir) {
            arterm_logging::warn(&format!("failed to record launch dirs for hotkeys: {err}"));
        }
    }
...
<dict>
    <key>Label</key>
    <string>com.arterm.hotkey</string>
    ...
    <dict>
        <key>ARTERM_PREFERRED_TERMINAL</key>
...
/// Launch a new arterm window in the user's preferred macOS terminal, passing
/// `extra_args` (e.g. `["--resume", "<session-id>"]`) to the arterm invocation.
///
...
#[cfg(target_os = "macos")]
pub fn launch_arterm_in_macos_terminal(extra_args: &[String]) -> Result<()> {
    ...
    let command = match macos_terminal::no_automation_launch(terminal, &shell_command) {
        ...
        macos_terminal::NoAutomationLaunch::CommandFile { app } => {
            let dir = storage::arterm_dir()?.join("launcher");
    ...
    let status = std::process::Command::new("sh")
        ...
        .arg(&command)
        .status()
        .context("failed to launch terminal for arterm")?;
    if !status.success() {
        anyhow::bail!(
...
fn install_macos_hotkey_listener(
    ...
    let status = std::process::Command::new("launchctl")
        ...
        .status()
        .context("failed to load arterm LaunchAgent")?;
    if !status.success() {
...
fn startup_hints_for_launch(_state: &SetupHintsState) -> Option<StartupHints> {
    ...
    let spawn_notice = if !_state.hotkey_configured || _state.startup_spawn_hint_dismissed {
        ...
        Some(format!(
            "Cmd+; launches a new arterm in your home directory from anywhere, system-wide (opens in {}). Cmd+' reopens your last project; Cmd+Shift+' opens a self-dev session.",
...
fn macos_terminal_notice(
    ...
    let message = "The built-in macOS Terminal.app renders arterm poorly (slow, limited colors, no inline images). Consider a modern terminal such as Ghostty, iTerm2, or Alacritty for a much better experience.".to_string();

    Some(StartupHints::with_status_and_display(
        "Tip: Terminal.app renders arterm poorly. Try Ghostty, iTerm2, or Alacritty.".to_string(),
        "Terminal",
        message,
    ))
}
...
/// Manual `arterm setup-hotkey` command.
///
...
pub fn run_setup_hotkey(
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let terminal = effective_macos_terminal();
        eprintln!("\x1b[1marterm setup-hotkey\x1b[0m");
        eprintln!();
        eprintln!("  Preferred terminal: {}", terminal.label());
        eprintln!("  Installing a LaunchAgent with three system-wide arterm launch hotkeys.");
        eprintln!();
        ...
        match install_macos_hotkey_listener(Some(terminal)) {
            Ok(installed_terminal) => {
                ...
                let _ = state.save();
                eprintln!(
                    "  \x1b[32m✓\x1b[0m Created launch hotkeys → {} + arterm",
        ...
        if let Some(comp) = detect_linux_compositor() {
            ...
            match install_linux_launch_hotkeys(comp) {
                Ok(changed) => {
                    if changed {
                        eprintln!(
                            "  \x1b[32m✓\x1b[0m Installed arterm launch hotkeys into your {} config.",
                            comp.name()
                        );
                    } else {
                        eprintln!(
                            "  \x1b[32m✓\x1b[0m arterm launch hotkeys already up to date in your {} config.",
                            comp.name()
                        );
                    }
                    state.hotkey_configured = true;
...
pub(crate) fn install_cli_launch_hints_notice() {
    match cli_launch_hints::install_available() {
        ...
        Ok(_) => {}
        Err(err) => arterm_logging::warn(&format!(
...
pub(crate) fn active_primary_launch_hotkey() -> Option<(String, String)> {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let exe_path = std::env::current_exe()
            ...
            .unwrap_or_else(|_| "arterm".to_string());
...
fn run_macos_hotkey_listener() -> Result<()> {
    ...
    // launches arterm on key-down, then hand the thread to the event loop so the
    ...
    // standard arterm log plus the plist's StandardOut/ErrorPath.
    arterm_logging::init();
    macos_hotkey_log("starting macOS arterm launch hotkey listener");
    ...
    // to a cwd + arterm argv. Older versions dispatched through generated shell
    ...
    if launch_for_id.is_empty() {
        anyhow::bail!("failed to register any arterm launch hotkey");
    }
    ...
    GlobalHotKeyEvent::set_event_handler(Some(move |event: GlobalHotKeyEvent| {
        ...
        if let Some(launch) = launch_for_id.get(&event.id) {
            ...
            let command = TerminalCommand::new(&exe_path, args)
                .fresh_spawn()
                .kind("hotkey")
                .spawn_env("ARTERM_SPAWN_LABEL", launch.label.clone());
            match spawn_command_in_new_terminal_with(&command, &cwd, |cmd| cmd.spawn().map(|_| ()))
            {
                Ok(true) => {}
                Ok(false) => {
                    macos_hotkey_log("failed to launch arterm: no terminal candidate worked")
                }
                Err(err) => macos_hotkey_log(&format!("failed to launch arterm: {err}")),
            }
        }
    }));

    macos_hotkey_log("macOS arterm launch hotkeys registered; entering event loop");
    ...
    macos_hotkey_log("macOS arterm launch hotkey event loop exited");
    Ok(())
}
...
/// Log a hotkey-listener diagnostic to both the arterm log and stderr.
///
...
fn macos_hotkey_log(message: &str) {
    arterm_logging::info(message);
    eprintln!("[arterm hotkey] {message}");
}
...
pub fn maybe_show_setup_hints() -> Option<StartupHints> {
            ...
            && state.launch_hotkey_tracking_version < LAUNCH_HOTKEY_TRACKING_VERSION
        {
            match windows_setup::refresh_windows_launch_hotkeys() {
                Ok(()) => {
                    state.launch_hotkey_tracking_version = LAUNCH_HOTKEY_TRACKING_VERSION;
                    let _ = state.save();
                    arterm_logging::info("Migrated Windows launch hotkeys to usage tracking");
                }
                Err(err) => arterm_logging::warn(&format!(
    ...
    // `arterm setup-launcher` explicitly.
...
fn macos_launch_hotkeys_notice(state: &SetupHintsState) -> Option<StartupHints> {
    ...
    Some(StartupHints::with_status_and_display(
        ...
        "Launch hotkeys",
        format!("Configured Arterm launch hotkeys:\n{}", lines.join("\n")),
    ))
}
...
/// Config file arterm manages for a flat (`#`-commented) compositor config.
...
/// Directory for arterm's hidden KDE launcher desktop files.
#[cfg(target_os = "linux")]
...
/// The config file arterm would manage for the *current* session's compositor.
#[cfg(target_os = "linux")]
...
/// The sentinel that marks arterm's managed region in `path` for `comp`.
#[cfg(target_os = "linux")]
...
/// Whether arterm's launch hotkeys are already installed for `comp`.
#[cfg(target_os = "linux")]
fn linux_hotkeys_installed(comp: linux_env::LinuxCompositor) -> bool {
    use linux_env::LinuxCompositor;
    match comp {
        LinuxCompositor::Gnome => gnome_keybinding_list().contains("/arterm-launch-"),
        LinuxCompositor::Cinnamon => {
            dconf_read("/org/cinnamon/desktop/keybindings/custom-list").contains("arterm-launch-")
        }
        LinuxCompositor::Mate => {
            dconf_list("/org/mate/desktop/keybindings/").contains("arterm-launch-")
        }
        LinuxCompositor::Xfce => xfce_shortcut_commands_text().contains("/launch_arterm_"),
        LinuxCompositor::Kde => kde_globalshortcutsrc_path()
            .and_then(|p| std::fs::read_to_string(p).ok())
            .map(|text| text.contains("[services][arterm-launch-"))
            .unwrap_or(false),
...
/// Pick a terminal emulator to launch arterm in on Linux. Honors `$TERMINAL`,
...
/// scripts written under `~/.arterm/hotkey/`, so the config never embeds shell
...
/// xfconf-query. Stale arterm entries bound to accelerators we no longer use
...
fn install_xfce_launch_hotkeys() -> Result<bool> {
    ...
    // Remove stale arterm entries bound to accelerators we no longer use.
    for line in existing.lines() {
        let Some((prop, value)) = line.split_once(char::is_whitespace) else {
            continue;
        };
        if value.trim().contains("/launch_arterm_") && !wanted.iter().any(|(p, _)| p == prop) {
...
/// `~/.arterm/hotkey/` and return the chord -> script binds. The scripts `cd`
/// into the target (with `$HOME` fallback for stale/dynamic dirs) and exec the
/// user's terminal running arterm, so compositor bind lines stay trivial.
#[cfg(target_os = "linux")]
fn write_linux_launch_scripts() -> Result<Vec<linux_env::ScriptBind>> {
    ...
    for entry in resolved {
        ...
        let script_body = format!(
            "#!/bin/sh\n# Auto-generated by arterm setup-hotkey; re-run it to refresh.\n{cd}exec {exec}\n",
            cd = entry.cd_prefix,
        );
        let script_path = hotkey_dir.join(&entry.script_file_name);
...
/// Timestamped backup matching the `.bak-arterm-*` convention, taken before
...
fn backup_compositor_config(config_path: &std::path::Path) {
    ...
    let backup = config_path.with_file_name(format!("{file_name}.bak-arterm-hotkeys-{ts}"));
...
fn reload_compositor_config(comp: linux_env::LinuxCompositor) {
        ...
        .status()
    {
        ...
        Ok(status) => arterm_logging::warn(&format!(
            "{} exited with {status} while reloading hotkey binds",
            cmd[0]
        )),
        Err(err) => arterm_logging::warn(&format!("failed to run {} reload: {err}", cmd[0])),
    }
}
...
fn linux_launch_hotkeys_notice(state: &SetupHintsState) -> Option<StartupHints> {
    ...
    Some(StartupHints::with_status_and_display(
        ...
        "Launch hotkeys",
        format!(
            "Configured Arterm launch hotkeys ({}): {} {}",
            comp.name(),
            lines.join("; "),
            footer
        ),
...
/// - Once the user has learned at least one binding and has launched arterm at
...
/// Check whether arterm's keybindings conflict with shortcuts owned by the
...
pub fn maybe_show_keymap_conflict_hint(
    keybindings: &arterm_config_types::KeybindingsConfig,
) -> Option<StartupHints> {
    if !io::stdin().is_terminal() || !io::stderr().is_terminal() {
        return None;
    }
...
/// Whether the current terminal triggers arterm's glyph-safe color quantization
/// (macOS VS Code integrated terminal / Apple Terminal). Mirrors the detection
/// in `arterm-tui-style`'s color module and `arterm-app-core::perf` so the
/// disclosure fires exactly when the behavior is active. Overridable with
/// `ARTERM_GLYPH_SAFE_MODE=on|off`.
fn glyph_safe_mode_active() -> bool {
    if let Ok(raw) = std::env::var("ARTERM_GLYPH_SAFE_MODE") {
...
/// shown the first time arterm launches in a fragile-glyph terminal. Discloses
...
its glyph cache under arterm's full-color animations, rendering letters as boxes. \
arterm automatically quantizes colors to the 256-palette here to keep text readable; \
the only tradeoff is slightly reduced color fidelity. Animations still run. \
For full color, use Ghostty, iTerm2, kitty, or WezTerm, or set ARTERM_GLYPH_SAFE_MODE=off."
        .to_string();
    (
        Some(StartupHints::with_status_and_display(
...
/// Manual `arterm setup-launcher` command.
pub fn run_setup_launcher() -> Result<()> {
    #[cfg(target_os = "macos")]
    {
        let mut state = SetupHintsState::load();
        eprintln!("\x1b[1marterm setup-launcher\x1b[0m");
        eprintln!();
        ...
        match install_macos_app_launcher() {
            Ok((app_dir, terminal)) => {
                ...
                eprintln!(
                    "  \x1b[32m✓\x1b[0m Spotlight/Launchpad/Dock will launch arterm in {}",
                    terminal.label()
                );
                eprintln!();
                eprintln!("  Tip: pin Arterm.app to your Dock or launch it with Cmd+Space.");
                return Ok(());
            }
            Err(e) => {
                eprintln!("  \x1b[31m✗\x1b[0m Failed: {}", e);
    ...
    #[cfg(windows)]
    {
        ...
        match create_windows_desktop_shortcut(&mut state) {
            Ok(()) => {
                eprintln!("  \x1b[32m✓\x1b[0m Created desktop shortcut: arterm.lnk");
                return Ok(());
            }
            Err(e) => {
                eprintln!("  \x1b[31m✗\x1b[0m Failed: {}", e);
...
/// Create a desktop shortcut/launcher for arterm.
///
/// - macOS: creates a arterm.app bundle in ~/Applications/
/// - Windows uses [`windows_setup::create_windows_desktop_shortcut`] via
///   `arterm setup-launcher` instead (PowerShell/COM is too slow for the
///   startup path).
...
fn create_desktop_shortcut(state: &mut SetupHintsState) -> Result<()> {
    #[cfg(any(test, target_os = "macos"))]
    {
        ...
        arterm_logging::info(&format!("Created macOS app bundle: {}", app_dir.display()));
    }
...
pub fn reinstall_launch_hotkeys_after_config_change() {
    #[cfg(target_os = "macos")]
    {
        ...
        match install_macos_hotkey_listener(preferred) {
            Ok(terminal) => arterm_logging::info(&format!(
                "Reinstalled launch hotkeys after config change for {}",
                terminal.label()
            )),
            Err(err) => arterm_logging::warn(&format!("failed to reinstall launch hotkeys: {err}")),
        }
    }
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        match install_linux_launch_hotkeys(comp) {
            Ok(true) => arterm_logging::info(&format!(
