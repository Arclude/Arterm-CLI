...
//!   config grammars), each bind simply executes a launch script arterm writes
//!   to `~/.arterm/hotkey/`.
...
//!
//! ```text
//! # >>> arterm launch hotkeys (managed) >>>
//! bind = SUPER, semicolon, exec, '/home/u/.arterm/hotkey/launch_arterm_0_cmd_semicolon.sh'
//! # <<< arterm launch hotkeys (managed) <<<
//! ```
...
pub(crate) const HASH_BLOCK_BEGIN: &str = "# >>> arterm launch hotkeys (managed) >>>";
/// Closing sentinel for the managed region in `#`-commented configs.
pub(crate) const HASH_BLOCK_END: &str = "# <<< arterm launch hotkeys (managed) <<<";

/// A Linux desktop environment / compositor arterm can install launch hotkeys
/// into.
...
/// Translate a canonical arterm key token into an XKB keysym name (the
...
/// expressed. arterm's `cmd` modifier maps to `SUPER`.
///
...
/// Hyprland modifier list (space-separated, e.g. `SUPER SHIFT`). arterm `cmd`
/// maps to `SUPER`.
...
/// expressed. arterm's `cmd` maps to `Mod4` (super) and `alt` to `Mod1`.
///
...
pub(crate) fn render_flat_block(
    ...
    for bind in binds {
        if let Some(line) = render_line(bind) {
            lines.push(format!("# arterm: {label}", label = bind.label));
            lines.push(line);
        }
    }
    if lines.is_empty() {
...
/// line and the command indented beneath. arterm's `cmd` maps to `super`.
...
/// `<Super>semicolon` or `<Super><Shift>apostrophe`. arterm's `cmd` maps to
...
/// The dconf path for arterm's Nth GNOME custom keybinding. Slot-stable so
/// re-installs overwrite rather than accumulate.
pub(crate) fn gnome_keybinding_path(index: usize) -> String {
    format!(
        "/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/arterm-launch-{index}/"
    )
}
...
/// Merge arterm's entries into an existing gsettings/dconf string-array value
/// (e.g. `['/a/', '/b/']` or `@as []`), preserving foreign entries and
/// replacing any previous arterm slots (matched on the `arterm-launch-` marker).
...
pub(crate) fn merge_gnome_keybinding_list(current: &str, ours: &[String]) -> String {
    ...
    while let Some(start) = rest.find('\'') {
        ...
        let entry = &after[..end];
        if !entry.contains("arterm-launch-") {
...
/// Stable dconf slot name for arterm's Nth custom keybinding (Cinnamon/MATE).
pub(crate) fn dconf_slot_name(index: usize) -> String {
    format!("arterm-launch-{index}")
}
...
/// arterm's `cmd` maps to `Meta` (the KDE name for Super).
...
/// Stable desktop-file name for arterm's Nth KDE launch shortcut.
pub(crate) fn kde_desktop_file_name(index: usize) -> String {
    format!("arterm-launch-{index}.desktop")
}
...
/// Upsert `_launch=` entries for arterm's services into `kglobalshortcutsrc`
/// INI text: existing `[services][arterm-launch-*.desktop]` sections are
...
pub(crate) fn upsert_kde_shortcut_sections(current: &str, shortcuts: &[KdeShortcut]) -> String {
    ...
    for line in current.lines() {
        if line.starts_with('[') {
            skipping = line.starts_with("[services][arterm-launch-");
        }
        if !skipping {
...
/// Build the shell command a launch script uses to open arterm in the user's
/// terminal, as a `argv`-quoted string (e.g. `'kitty' '/bin/arterm' 'self-dev'`).
...
mod tests {
    ...
    fn gnome_bindings_use_angle_bracket_modifiers_and_stable_slots() {
        ...
        assert_eq!(kbs[0].name, "arterm: home");
    ...
    fn gnome_list_merge_preserves_foreign_entries_and_replaces_ours() {
        ...
        // Foreign entries stay; stale arterm slots are dropped before re-adding.
        let current = format!(
        ...
        assert!(!merged.contains("arterm-launch-7"));
        assert_eq!(merged.matches("arterm-launch-0").count(), 1);
        assert_eq!(merged.matches("arterm-launch-1").count(), 1);
        // Idempotent.
    ...
    fn dconf_keybindings_share_gnome_accelerators_with_slot_names() {
        ...
        assert_eq!(kbs.len(), 2);
        assert_eq!(kbs[0].slot, "arterm-launch-0");
        assert_eq!(kbs[1].slot, "arterm-launch-1");
        assert_eq!(kbs[0].binding, "<Super>semicolon");
        assert_eq!(kbs[1].binding, "<Super><Shift>apostrophe");
        assert_eq!(kbs[1].name, "arterm: self-dev");
    }
    ...
    fn gnome_list_merge_also_handles_cinnamon_slot_names() {
        ...
        let current = "['custom0', 'arterm-launch-0', 'arterm-launch-5']";
        let merged = merge_gnome_keybinding_list(current, &ours);
        assert!(merged.contains("'custom0'"));
        assert!(!merged.contains("arterm-launch-5"));
        assert_eq!(merged.matches("arterm-launch-0").count(), 1);
        assert!(merged.contains("arterm-launch-1"));
        assert_eq!(merge_gnome_keybinding_list(&merged, &ours), merged);
    }
    ...
    fn kde_shortcuts_use_meta_and_literal_keys() {
        ...
        assert_eq!(scs.len(), 2);
        assert_eq!(scs[0].desktop_file_name, "arterm-launch-0.desktop");
    ...
    fn kde_ini_upsert_replaces_our_sections_and_keeps_others() {
        ...
        let current = "[services][firefox.desktop]\n_launch=Meta+F\n\n[services][arterm-launch-0.desktop]\n_launch=Meta+X\n\n[services][arterm-launch-9.desktop]\n_launch=Meta+Y\n";
        ...
        assert_eq!(
            updated
                .matches("[services][arterm-launch-0.desktop]")
                .count(),
            1
        );
        assert!(updated.contains("_launch=Meta+;"));
        assert!(!updated.contains("Meta+X"));
        assert!(!updated.contains("arterm-launch-9"));
        // Idempotent.
    ...
    fn hyprland_bind_line_maps_cmd_to_super() {
        let line = render_hyprland_bind_line(&bind(
            "cmd+;",
            "/home/u/.arterm/hotkey/launch_arterm_0_cmd_semicolon.sh",
            "arterm",
            false,
        ))
        .unwrap();
    ...
    fn sway_bind_line_maps_cmd_to_mod4_and_alt_to_mod1() {
        let line = render_sway_bind_line(&bind("cmd+;", "/s.sh", "arterm", false)).unwrap();
        assert_eq!(
            line,
    ...
    fn blocks_wrap_sentinels_and_carry_labels() {
        let block = render_hyprland_block(&[
            bind("cmd+;", "/a.sh", "arterm", false),
            bind("cmd+'", "/b.sh", "home", false),
        ])
        .unwrap();
        assert!(block.starts_with(HASH_BLOCK_BEGIN));
        assert!(block.ends_with(HASH_BLOCK_END));
        assert!(block.contains("# arterm: arterm"));
        assert!(block.contains("# arterm: home"));

        let sway = render_sway_block(&[bind("cmd+;", "/a.sh", "arterm", false)]).unwrap();
    ...
    fn terminal_exec_command_varies_by_terminal() {
        assert_eq!(
            terminal_exec_command("kitty", "/bin/arterm", "cmd+;", false),
            "'kitty' '/bin/arterm' '--spawn-hotkey' 'cmd+;'"
        );
        assert_eq!(
            terminal_exec_command("alacritty", "/bin/arterm", "cmd+shift+'", true),
            r#"'alacritty' '-e' '/bin/arterm' '--spawn-hotkey' 'cmd+shift+'\''' 'self-dev'"#
        );
        assert_eq!(
            terminal_exec_command("wezterm", "/bin/arterm", "cmd+;", false),
            "'wezterm' 'start' '--' '/bin/arterm' '--spawn-hotkey' 'cmd+;'"
        );
        assert_eq!(
            terminal_exec_command("foot", "/bin/arterm", "cmd+;", false),
            "'foot' '/bin/arterm' '--spawn-hotkey' 'cmd+;'"
        );
        // Full paths keep the path but dispatch on the basename.
        assert_eq!(
            terminal_exec_command("/usr/bin/ghostty", "/bin/arterm", "cmd+;", false),
            "'/usr/bin/ghostty' '-e' '/bin/arterm' '--spawn-hotkey' 'cmd+;'"
        );
    }
}
