...
use arterm_config_types::{LaunchHotkeyEntry, LaunchHotkeysConfig};
use arterm_core::{terminal_eprint as eprint, terminal_eprintln as eprintln};
...
fn startup_shortcut_path() -> PathBuf {
    startup_dir().join("arterm-hotkey.lnk")
}

fn hotkey_vbs_path() -> Result<PathBuf> {
    Ok(storage::arterm_dir()?
        .join("hotkey")
        .join("arterm-hotkey-launcher.vbs"))
}

fn legacy_hotkey_ps1_path() -> Result<PathBuf> {
    Ok(storage::arterm_dir()?
        .join("hotkey")
        .join("arterm-hotkey.ps1"))
}
...
Get-CimInstance Win32_Process -ErrorAction SilentlyContinue |
  Where-Object {{
    ($_.ProcessId -ne $current) -and ($_.ProcessId -ne $PID) -and (
      (($_.Name -eq 'powershell.exe' -or $_.Name -eq 'pwsh.exe') -and $_.CommandLine -like '*arterm-hotkey*') -or
      (($_.Name -eq 'arterm.exe') -and $_.CommandLine -like '*--listen-windows-hotkey*')
    )
  }} |
...
pub(super) fn uninstall_windows_hotkey_listener() -> Result<()> {
    ...
    state.save()?;
    eprintln!("  \x1b[32m✓\x1b[0m Removed arterm Windows launch-hotkey listener");
    Ok(())
}
...
fn create_hotkey_shortcut(_use_alacritty: bool) -> Result<()> {
    ...
    let hotkey_dir = storage::arterm_dir()?.join("hotkey");
...
fn launch_windows_hotkey(entry: &WindowsHotkey) -> Result<()> {
    ...
    let command = arterm_terminal_launch::TerminalCommand::new(exe, args)
        .title("arterm")
        .fresh_spawn()
        .kind("launch-hotkey");

    let launched =
        arterm_terminal_launch::spawn_command_in_new_terminal_with(&command, &cwd, |cmd| {
            cmd.spawn().map(|_| ())
        })?;
    if !launched {
        anyhow::bail!("no terminal found to launch arterm");
    }
    Ok(())
}
...
fn windows_native_hotkey_loop(entries: Vec<WindowsHotkey>) -> Result<()> {
    ...
    const MUTEX_NAME: &str = "Local\\ArtermLaunchHotkeyListener";
    ...
    let mutex = loop {
        ...
        if retry_count >= 40 {
            arterm_logging::warn("previous Windows launch-hotkey listener is still exiting");
            return Ok(());
        }
        retry_count += 1;
            ...
            if let Some(entry) = entry.cloned() {
                std::thread::spawn(move || {
                    if let Err(err) = launch_windows_hotkey(&entry) {
                        arterm_logging::warn(&format!(
                            "failed to launch arterm from Windows hotkey: {err}"
                        ));
                    }
                });
            }
...
/// interactive `arterm setup-hotkey` flow to install it.
...
pub(super) fn reinstall_windows_launch_hotkeys() {
    ...
    match refresh_windows_launch_hotkeys() {
        Ok(()) => arterm_logging::info("Reinstalled Windows launch hotkeys after config change"),
...
fn nudge_hotkey(state: &mut SetupHintsState) -> bool {
    ...
    eprintln!(
        "\x1b[36m│\x1b[0m \x1b[1m💡 Set up global keys to launch arterm?\x1b[0m                  \x1b[36m│\x1b[0m"
    );
    eprintln!(
    ...
    eprintln!(
        "\x1b[36m│\x1b[0m    Opens arterm in {:<39}    \x1b[36m│\x1b[0m",
        ...
        "y" | "yes" => {
            ...
            match create_hotkey_shortcut(using_alacritty) {
                Ok(()) => {
                    ...
                    let _ = state.save();
                    eprintln!(
                        "  \x1b[32m✓\x1b[0m Created hotkeys (\x1b[1mAlt+;\x1b[0m and \x1b[1mCopilot\x1b[0m) → {} + arterm",
                        terminal_name
                    );
                    eprintln!();
                    true
                ...
                Err(e) => {
                    ...
                    eprintln!(
                        "    You can set it up manually later with: \x1b[1marterm setup-hotkey\x1b[0m"
                    );
                    eprintln!();
                    false
                }
...
fn nudge_alacritty(state: &mut SetupHintsState) -> bool {
    ...
    eprintln!("\x1b[36m┌─────────────────────────────────────────────────────────────┐\x1b[0m");
    eprintln!(
        "\x1b[36m│\x1b[0m \x1b[1m💡 Alacritty: the fastest terminal for arterm\x1b[0m               \x1b[36m│\x1b[0m"
    );
    eprintln!(
        "\x1b[36m│\x1b[0m                                                             \x1b[36m│\x1b[0m"
    );
                ...
                Ok(()) => {
                    ...
                    if state.hotkey_configured {
                        ...
                        match create_hotkey_shortcut(true) {
                            Ok(()) => {
                                eprintln!(
                                    "  \x1b[32m✓\x1b[0m Hotkeys updated: \x1b[1mAlt+;\x1b[0m and \x1b[1mCopilot\x1b[0m → Alacritty + arterm"
                                );
                            }
                            Err(e) => {
...
fn prompt_try_it_out(installed_alacritty: bool) {
    ...
    eprintln!(
        "\x1b[32m│\x1b[0m    Press \x1b[1mAlt+;\x1b[0m or the \x1b[1mCopilot key\x1b[0m to launch arterm.       \x1b[32m│\x1b[0m"
    );
    eprintln!(
        "\x1b[32m│\x1b[0m    The listener is native Arterm, no AutoHotkey required.          \x1b[32m│\x1b[0m"
    );
    ...
    eprintln!(
        "\x1b[32m│\x1b[0m    \x1b[90m(Starting arterm normally in 3 seconds...)\x1b[0m                 \x1b[32m│\x1b[0m"
    );
    eprintln!("\x1b[32m└─────────────────────────────────────────────────────────────┘\x1b[0m");
    eprintln!();
...
mod tests {
    ...
    fn powershell_single_quote_escapes_embedded_quotes() {
        assert_eq!(ps_single_quote(r"C:\O'Hara\arterm"), r"'C:\O''Hara\arterm'");
    }

    #[test]
    fn listener_stop_sweep_excludes_arterm_and_powershell_processes_running_it() {
    ...
    fn startup_shortcut_uses_native_listener_without_vbscript_or_bypass() {
        let script = render_startup_shortcut_script(
            Path::new(r"C:\Users\O'Hara\Startup\arterm-hotkey.lnk"),
            Path::new(r"C:\Program Files\Arterm O'Hara\arterm.exe"),
        );
...
pub(super) fn run_setup_hotkey_windows() -> Result<()> {
    ...
    eprintln!(
        "  Setting up global launch hotkeys → {} + arterm...",
        terminal_name
    );
...
pub(super) fn create_windows_desktop_shortcut(state: &mut SetupHintsState) -> Result<()> {
    ...
    let shortcut_path = format!("{}\\Desktop\\arterm.lnk", desktop_dir);
...
$shortcut.Description = "arterm - AI coding agent"
$shortcut.Save()
Write-Output "OK"
"#,
    ...
    if output.status.success() {
        ...
        if stdout.contains("OK") {
            ...
            let _ = state.save();
            arterm_logging::info(&format!("Created desktop shortcut: {}", shortcut_path));
        }
    }
