//! Detect conflicts between arterm's own key bindings and the bindings
//! discovered on the machine (terminal emulator + macOS system shortcuts).
//!
//! The flow is:
//!   1. enumerate arterm's configured bindings as `(field, label, KeyChord)`
//!      from [`arterm_config_types::KeybindingsConfig`] ([`arterm_bindings`]),
...
/// One configured arterm binding, tied back to its config field.
#[derive(Debug, Clone)]
pub struct ArtermBinding {
...
/// A detected conflict between a arterm binding and something on the machine.
#[derive(Debug, Clone)]
pub struct Conflict {
    /// The arterm binding that may not reach the app.
    pub arterm: ArtermBinding,
...
/// Enumerate arterm's configured bindings as comparable chords. Bindings that are
...
/// chord.
pub fn arterm_bindings(cfg: &KeybindingsConfig) -> Vec<ArtermBinding> {
...
/// Find conflicts between arterm's configured bindings and the discovered
/// machine bindings in `snapshot`.
///
/// Conflicts are deduplicated per `(arterm field, interceptor chord, source)` so
...
pub fn detect_conflicts(cfg: &KeybindingsConfig, snapshot: &KeymapSnapshot) -> Vec<Conflict> {
    ...
    for arterm in arterm_bindings(cfg) {
        let Some(interceptors) = by_chord.get(&arterm.chord) else {
            continue;
        };
...
mod tests {
    ...
    fn detects_ghostty_ctrl_tab_conflict() {
        ...
        let fields: Vec<&str> = conflicts.iter().map(|c| c.arterm.field.as_str()).collect();
    ...
    fn detects_window_manager_prompt_jump_conflict() {
        ...
        // focus, shadowing arterm's built-in prompt navigation fallback.
        let cfg = KeybindingsConfig::default();
        let snapshot = snapshot_with(vec![
        ...
        assert!(
            conflicts
                .iter()
                .all(|c| c.arterm.field.contains("built-in fallback")),
    ...
    fn no_conflict_when_chords_differ() {
        ...
        // arterm has no cmd+t binding by default.
    ...
    fn disabled_binding_is_not_reported() {
        ...
        assert!(
            !conflicts
                .iter()
                .any(|c| c.arterm.field == "keybindings.model_switch_next")
        );
    }
