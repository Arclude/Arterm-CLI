...
//! *before* the terminal (and therefore arterm) ever sees them.
//!
//! macOS lets window managers and automation tools register system-wide hotkeys.
//! When one of those overlaps a key arterm wants (the classic case is a tiling WM
//! binding `Cmd+J`/`Cmd+K` to window focus, which shadows arterm's prompt
//! navigation), the keystroke never reaches the terminal and the arterm binding
...
fn apply_modifier(token: &str, mods: &mut Mods, hyper_mods: Mods) -> bool {
    match token.trim().to_ascii_lowercase().as_str() {
        ...
        // "fn" is not representable as a arterm modifier; ignore it so the rest of
