...
impl ToolDefinition {
    ...
    ///
    /// This uses arterm's standard chars/4 heuristic, matching other token
...
pub enum ContentBlock {
    ...
    /// compaction state across turns/saves when arterm explicitly triggers it.
    OpenAICompaction {
...
pub enum StreamEvent {
    ...
    /// duplicating. Safe for arterm HTTP providers because tools only execute
    ...
    /// Native tool call from a provider bridge that needs execution by arterm
    NativeToolCall {
        request_id: String,
        tool_name: String,
        input: serde_json::Value,
