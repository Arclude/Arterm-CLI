use anyhow::Result;
use arterm_message_types::{ContentBlock, Message, Role, ToolCall, ToolDefinition};
...
/// `arterm-schema-dialect` so that every provider shares one implementation and
...
pub fn gemini_compatible_schema(schema: &Value) -> Value {
    arterm_schema_dialect::normalize(schema, &arterm_schema_dialect::registry::GEMINI)
}
