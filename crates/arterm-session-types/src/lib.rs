...
/// Identifies a session to resume, across the agent backends arterm can import
/// from. This is pure data (only ids/paths) with no UI dependency; it lives in
/// `arterm-session-types` so the foundation/import layer can match on it without
/// depending on any `arterm-tui-*` crate. The session-picker UI re-exports it.
...
impl ResumeTarget {
    pub fn stable_id(&self) -> &str {
        match self {
            Self::ArtermSession { session_id } => session_id,
...
pub struct EnvSnapshot {
    ...
    pub arterm_git_hash: Option<String>,
    pub arterm_git_dirty: Option<bool>,
    pub os: String,
    pub arch: String,
    pub pid: u32,
    pub is_selfdev: bool,
...
pub struct SessionSearchReport {
    ...
    pub scanned_arterm_sessions: usize,
    pub candidate_arterm_sessions: usize,
...
pub fn format_session_search_results(
    ...
    output.push_str(&format!(
        "_Scanned: {} Arterm sessions ({} candidates), {} external sessions{}{}._\n\n",
