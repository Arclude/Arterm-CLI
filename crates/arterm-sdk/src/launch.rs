//! Runtime startup and private arterm instances.
//!
...
/// Options for starting a private arterm instance.
pub struct LaunchOptions {
    ...
    pub arterm_home: Option<PathBuf>,
    ...
    /// arterm executable. Defaults to `arterm` on `$PATH`.
    ...
    /// Client identity used by [`crate::ArtermClient::launch`].
    pub client_name: String,
    /// Request timeout used by [`crate::ArtermClient::launch`].
...
impl Default for LaunchOptions {
    fn default() -> Self {
        Self {
            ...
            client_name: concat!("arterm-sdk-rs/", env!("CARGO_PKG_VERSION")).to_string(),
...
impl LaunchedInstance {
    fn shutdown(&self) {
        ...
        stop_instance_daemon(&self.arterm_home, &self.runtime_dir);
        ...
        if self.ephemeral {
            remove_ephemeral_home(&self.arterm_home, self.cleanup_timeout);
        }
    }
}
...
pub fn launch_instance(options: &LaunchOptions) -> Result<LaunchedInstance> {
    let ephemeral = options.arterm_home.is_none();
    let arterm_home = match &options.arterm_home {
        Some(path) => {
            fs::create_dir_all(path).map_err(launch_io)?;
            path.clone()
        }
        None => tempfile::Builder::new()
            .prefix("arterm-sdk-instance-")
            .tempdir()
            .map_err(launch_io)?
            .keep(),
    };
    set_owner_only_dir(&arterm_home)?;

    let runtime_dir = arterm_home.join("run");
    fs::create_dir_all(&runtime_dir).map_err(launch_io)?;
    set_owner_only_dir(&runtime_dir)?;
    let socket_path = runtime_dir.join("arterm-api.sock");
    for stale in [
        "arterm-api.sock",
        "arterm.sock",
        "arterm-debug.sock",
    ...
    let cleanup_on_error = || {
        if ephemeral {
            remove_ephemeral_home(&arterm_home, Duration::ZERO);
        }
    };
    if options.inherit_logins {
        if let Err(error) = inherit_credentials(&user_arterm_home(), &arterm_home) {
            cleanup_on_error();
            return Err(error);
        }
    }
    ...
    let binary = options
        .binary
        .clone()
        .unwrap_or_else(|| PathBuf::from("arterm"));
    ...
    command
        ...
        .env("ARTERM_HOME", &arterm_home)
        .env("ARTERM_RUNTIME_DIR", &runtime_dir)
        .env("ARTERM_API_SOCKET", &socket_path)
        .env("ARTERM_SOCKET", runtime_dir.join("arterm.sock"))
    ...
    let mut child = match command.spawn() {
        ...
        Err(cause) => {
            ...
            return Err(Error::new(ErrorKind::ArtermNotFound, message));
        }
    };
        ...
        match child.try_wait() {
            Ok(Some(status)) => {
                ...
                return Err(Error::new(
                    ...
                    format!(
                        "arterm exited during startup ({status}){}",
    ...
    Err(Error::new(
        ...
        format!(
            "arterm did not create its API socket at {} within {:?}{}",
            socket_path.display(),
...
/// Resolve the user's normal arterm home before a private instance overrides it.
pub fn user_arterm_home() -> PathBuf {
    std::env::var_os("ARTERM_HOME")
        .map(PathBuf::from)
        .unwrap_or_else(|| home_dir().join(".arterm"))
}
...
pub fn inherit_credentials(from_home: &Path, to_home: &Path) -> Result<Vec<PathBuf>> {
    ...
    if let Ok(entries) = fs::read_dir(user_app_config_dir()) {
        for entry in entries.flatten() {
            ...
            let relative = PathBuf::from("config").join("arterm").join(&name);
...
pub fn ensure_runtime(options: &LaunchOptions, progress: &Progress<'_>) -> Result<()> {
    let api = options
        .api_socket
        .clone()
        .unwrap_or_else(arterm_harness_api::api_socket_path);
    if socket_accepts(&api) {
        return Ok(());
    }
    let legacy = arterm_harness_api::legacy_socket_path();
    if !socket_accepts(&legacy) {
        progress("starting arterm runtime...");
        spawn_detached(&sibling_exe("arterm"), &["serve"]).map_err(|error| {
            Error::new(
                ErrorKind::LaunchFailed,
                format!("could not start the arterm runtime: {error}"),
            )
        })?;
        wait_for_socket(&legacy, "arterm runtime", options.start_timeout)?;
    }
    progress("starting harness API bridge...");
    spawn_detached(&sibling_exe("arterm-harness-api-bridge"), &[]).map_err(|error| {
        Error::new(
            ErrorKind::LaunchFailed,
            format!("could not start arterm-harness-api-bridge: {error}"),
        )
    })?;
...
fn read_daemon_pid(home: &Path, runtime_dir: &Path) -> Option<i32> {
    ...
    let socket = runtime_dir.join("arterm.sock");
...
fn remove_ephemeral_home(home: &Path, timeout: Duration) {
    ...
    if home.parent() != Some(std::env::temp_dir().as_path())
        || !name.starts_with("arterm-sdk-instance-")
    {
        return;
    }
    let deadline = Instant::now() + timeout;
...
/// Resolve the user's platform arterm config directory.
pub fn user_app_config_dir() -> PathBuf {
    #[cfg(target_os = "macos")]
    {
        return home_dir().join("Library/Application Support/arterm");
    }
...
mod tests {
    ...
    fn dropping_an_ephemeral_instance_stops_its_bridge_and_removes_its_home() {
        ...
        let instance = LaunchedInstance {
            socket_path: runtime_dir.join("arterm-api.sock"),
            arterm_home: home.clone(),
            runtime_dir,
            child: Mutex::new(Some(child)),
            ephemeral: true,
