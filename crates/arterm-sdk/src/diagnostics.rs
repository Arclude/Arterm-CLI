...
impl Stage {
    ...
    pub fn doing(self) -> &'static str {
        match self {
            Self::Starting => "starting the arterm runtime",
...
pub fn describe_disconnect(
    ...
    let cause = if daemon_closed {
        format!(
            "the arterm runtime connection closed while {} (the bridge is still available and will reconnect)",
            stage.doing()
        )
...
mod tests {
    ...
    fn a_live_socket_after_a_drop_points_at_a_replacement_bridge() {
        let text = describe_disconnect(
            ...
            Path::new("/run/user/1000/arterm-api.sock"),
