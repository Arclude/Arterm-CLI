...
impl UnixHarness {
    ...
    fn connect(&self) -> ArtermClient {
        ArtermClient::connect(ConnectOptions {
...
fn missing_launch_binary_is_typed_and_cleans_up_its_temporary_home() {
    let before = std::fs::read_dir(std::env::temp_dir())
        ...
        .filter(|entry| {
            entry
                .file_name()
                .to_string_lossy()
                .starts_with("arterm-sdk-instance-")
        })
        .count();
    let error = match ArtermClient::launch(LaunchOptions {
        binary: Some(PathBuf::from("arterm-definitely-not-installed-sdk-test")),
    ...
    assert_eq!(error.code(), "arterm_not_found");
