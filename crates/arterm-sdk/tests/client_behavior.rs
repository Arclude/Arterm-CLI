...
impl Transport for PairTransport {
    fn split(
        self: Box<Self>,
    ) -> arterm_sdk::Result<(Box<dyn BufRead + Send>, Box<dyn Write + Send>)> {
...
fn fake_harness(handle: impl Fn(&ClientFrame, &mut dyn Write) + Send + 'static) -> ArtermClient {
...
fn ga_runtime_and_file_methods_map_requests_and_typed_replies() {
    ...
    client.clear_api_key("arterm").expect("clear key");
...
fn an_error_frame_becomes_a_typed_failure() {
    let client = fake_harness(|frame, writer| {
        reply(
            frame,
            ApiEvent::Error {
                code: arterm_harness_api::ErrorCode::UnknownSession,
...
fn an_error_mid_turn_fails_the_run_instead_of_hanging() {
    let client = fake_harness(|frame, writer| {
        if let ApiRequest::SendMessage { session_id, .. } = &frame.request {
            ...
            push(
                ApiEvent::Error {
                    code: arterm_harness_api::ErrorCode::Internal,
...
fn a_lost_connection_fails_requests_in_flight() {
    ...
    let client = ArtermClient::connect_with(
        Box::new(PairTransport(ours)),
        ConnectOptions {
            request_timeout: Some(Duration::from_secs(5)),
            ensure_runtime: false,
