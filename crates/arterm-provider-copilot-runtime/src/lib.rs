...
//! tier detection, premium request modes), moved out of `arterm-base` so
...
//! root registers [`CopilotApiProvider`] with `arterm_base::provider::external`
//! at startup.
...
#[cfg(test)]
use arterm_provider_copilot::max_token_parameter_for_model as copilot_max_token_parameter_for_model;
use arterm_provider_copilot::{
...
impl CopilotApiProvider {
    ...
    fn persisted_catalog_path() -> Result<std::path::PathBuf> {
        Ok(arterm_base::storage::app_config_dir()?.join("copilot_models_cache.json"))
    }

    fn load_persisted_catalog() -> Option<PersistedCatalog> {
        let path = Self::persisted_catalog_path().ok()?;
        arterm_base::storage::read_json(&path)
            .ok()
    ...
    fn persist_catalog(models: &[String]) {
        ...
        if let Err(error) = arterm_base::storage::write_json(&path, &payload) {
    ...
    pub fn new() -> Result<Self> {
        ...
        let model =
            std::env::var("ARTERM_COPILOT_MODEL").unwrap_or_else(|_| DEFAULT_MODEL.to_string());
    ...
    fn env_premium_mode() -> u8 {
        match std::env::var("ARTERM_COPILOT_PREMIUM").ok().as_deref() {
    ...
    fn startup_prefetch_grace_ms() -> u64 {
        std::env::var("ARTERM_COPILOT_PREFETCH_STARTUP_GRACE_MS")
            .ok()
    ...
    pub fn set_premium_mode(&self, mode: PremiumMode) {
        ...
        if mode != PremiumMode::Normal {
            arterm_base::logging::info(&format!("Copilot premium mode set to {:?}", mode));
        }
    }
    ...
    /// If ARTERM_COPILOT_MODEL is set, this is a no-op (user override).
    pub async fn detect_tier_and_set_default(&self) {
        let detect_start = std::time::Instant::now();
        if std::env::var("ARTERM_COPILOT_MODEL").is_ok() {
            arterm_base::logging::info(
                "Copilot model overridden via ARTERM_COPILOT_MODEL, skipping tier detection",
            );
            self.mark_init_done();
            return;
        }
    ...
    async fn stream_request(
        ...
        for attempt in 0..MAX_RETRIES {
            if attempt > 0 {
                let delay = arterm_provider_core::attempt_tracker::retry_backoff_delay(
                    attempt,
                    RETRY_BASE_DELAY_MS,
                );
                arterm_base::logging::info(&format!(
            ...
            if Self::is_auth_error(status) && !attempted_auth_refresh {
                ...
                *self.bearer_token.write().await = None;
                arterm_base::logging::info("Copilot bearer token expired, refreshing...");
                last_error = Some(anyhow::anyhow!("Copilot auth error (HTTP {})", status));
                continue;
            }

            if !status.is_success() {
                let body_text = arterm_base::util::http_error_body(resp, "HTTP error").await;
                let error_str =
    ...
    async fn process_sse_stream(
        ...
        let sse_chunk_timeout = arterm_base::provider::stream_idle_timeout();
        ...
        loop {
            ...
            while let Some(line_end) = buffer.find('\n') {
                ...
                if let Some(data) = arterm_base::util::sse_data_line(&line) {
                    if data.trim() == "[DONE]" {
                        ...
                        arterm_base::copilot_usage::record_request(
                            input_tokens,
                            output_tokens,
                            true,
                        );
...
impl Provider for CopilotApiProvider {
    ...
    fn set_model(&self, model: &str) -> Result<()> {
        ...
        let trimmed = arterm_provider_core::strip_own_model_prefix(model, "copilot:");
        if trimmed.is_empty() {
