...
use super::*;
use arterm_base::provider::{ModelCatalogRefreshSummary, summarize_model_catalog_refresh};
...
impl Provider for OpenRouterProvider {
    ...
    async fn complete(
        ...
        let api_messages = arterm_provider_openrouter::request::build_chat_messages(
            &effective_messages,
            system,
            allow_reasoning,
            ...
            .map(|t| {
                serde_json::json!({
                    ...
                    "function": {
                        ...
                        "parameters": arterm_provider_openrouter::request::sanitize_tool_parameters_schema(&t.input_schema),
                    }
                })
            })
            .collect();
        ...
        if let Some(effort) = reasoning_effort.as_deref() {
            if self.supports_deepseek_reasoning_effort() {
                ...
                let effort = if arterm_base::prompt::is_swarm_effort(effort) {
                    "max"
                } else {
                    effort
                };
        ...
        // arterm-generated field with the same key (issue #341).
        ...
        arterm_base::logging::info("OpenRouter transport: HTTPS (SSE)");
    ...
    fn set_model(&self, model: &str) -> Result<()> {
        ...
        let (model_id, provider) = if self.supports_provider_features {
            ...
            let model_id = if provider.is_some() {
                arterm_base::provider::openrouter_catalog_model_id(&model_id).unwrap_or(model_id)
            } else {
                model_id
            };
            (model_id, provider)
        ...
        if let Some(stored_effort) = stored_effort
            && !arterm_base::prompt::is_swarm_effort(&stored_effort)
    ...
    fn available_efforts(&self) -> Vec<&'static str> {
        if self.supports_deepseek_reasoning_effort() {
            ...
            arterm_provider_core::OPENROUTER_SELECTABLE_EFFORTS.to_vec()
        } else {
            vec![]
        }
    }
    ...
    fn model_routes(&self) -> Vec<arterm_provider_core::ModelRoute> {
        ...
        self.available_models_display()
            .into_iter()
            .filter(|model| arterm_base::provider::is_listable_model_name(model))
            .map(|model| {
                ...
                arterm_provider_core::ModelRoute {
                    model,
    ...
    fn context_window(&self) -> usize {
        ...
        if let Some(profile_id) = self.profile_id.as_deref()
            && let Some(limit) =
                arterm_base::provider_catalog::openai_compatible_profile_context_limit(
                    profile_id, &model_id,
                )
        {
            return limit;
        }
        arterm_provider_core::context_limit_for_model_with_provider(&model_id, Some(self.name()))
...
impl OpenRouterProvider {
    ...
    /// `ARTERM_OPENROUTER_CACHE_NAMESPACE` env var, so with several named
    ...
    pub(crate) fn load_usable_model_disk_cache_entry(
        &self,
    ) -> Option<arterm_provider_openrouter::DiskCache> {
    ...
    pub(crate) fn load_disk_cache_entry_for_this_profile(
        ...
        match self.foreground_cache_namespace() {
            ...
            None => arterm_provider_openrouter::load_disk_cache_entry(),
        }
    }
    ...
    pub(crate) fn is_user_named_profile(&self) -> bool {
        ...
        match arterm_base::provider_catalog::openai_compatible_profile_by_id(id) {
