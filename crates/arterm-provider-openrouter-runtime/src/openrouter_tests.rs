...
impl SharedEnvLock {
    ...
    pub(crate) fn lock(&self) -> std::sync::MutexGuard<'static, ()> {
        arterm_base::storage::lock_test_env()
    }
}
...
impl EnvVarGuard {
    pub(crate) fn set(key: &'static str, value: impl AsRef<std::ffi::OsStr>) -> Self {
        ...
        arterm_base::env::set_var(key, value);
        Self { key, previous }
    }

    pub(crate) fn remove(key: &'static str) -> Self {
        let previous = std::env::var_os(key);
        arterm_base::env::remove_var(key);
        Self { key, previous }
    }
}
...
impl Drop for EnvVarGuard {
    fn drop(&mut self) {
        if let Some(previous) = &self.previous {
            arterm_base::env::set_var(self.key, previous);
        } else {
            arterm_base::env::remove_var(self.key);
        }
    }
}
...
fn write_test_api_key(temp: &TempDir, env_file: &str, env_key: &str, value: &str) {
    let config_dir = test_config_dir(temp).join("arterm");
...
fn isolate_openrouter_autodetect_env() -> Vec<EnvVarGuard> {
    let mut guards = vec![
        EnvVarGuard::remove("ARTERM_OPENROUTER_API_BASE"),
        EnvVarGuard::remove("ARTERM_OPENROUTER_API_KEY_NAME"),
        EnvVarGuard::remove("ARTERM_OPENROUTER_ENV_FILE"),
        EnvVarGuard::remove("ARTERM_OPENROUTER_DYNAMIC_BEARER_PROVIDER"),
        EnvVarGuard::remove("ARTERM_OPENROUTER_MODEL"),
        EnvVarGuard::remove("ARTERM_OPENROUTER_CACHE_NAMESPACE"),
        EnvVarGuard::remove("ARTERM_OPENROUTER_ALLOW_NO_AUTH"),
        EnvVarGuard::remove("ARTERM_OPENROUTER_TRANSPORT_STATE"),
        EnvVarGuard::remove("ARTERM_OPENROUTER_PROVIDER_FEATURES"),
        EnvVarGuard::remove("ARTERM_OPENROUTER_MODEL_CATALOG"),
        EnvVarGuard::remove("ARTERM_OPENROUTER_AUTH_HEADER"),
        EnvVarGuard::remove("ARTERM_OPENROUTER_AUTH_HEADER_NAME"),
        EnvVarGuard::remove("ARTERM_OPENROUTER_STATIC_MODELS"),
        EnvVarGuard::remove("ARTERM_ACTIVE_PROVIDER"),
        EnvVarGuard::remove("ARTERM_RUNTIME_PROVIDER"),
        EnvVarGuard::remove("ARTERM_NAMED_PROVIDER_PROFILE"),
        EnvVarGuard::remove("ARTERM_PROVIDER_PROFILE_NAME"),
        EnvVarGuard::remove("ARTERM_PROVIDER_PROFILE_ACTIVE"),
        EnvVarGuard::remove("ARTERM_OPENAI_COMPAT_API_BASE"),
        EnvVarGuard::remove("ARTERM_OPENAI_COMPAT_API_KEY_NAME"),
        EnvVarGuard::remove("ARTERM_OPENAI_COMPAT_ENV_FILE"),
        EnvVarGuard::remove("ARTERM_OPENAI_COMPAT_SETUP_URL"),
        EnvVarGuard::remove("ARTERM_OPENAI_COMPAT_DEFAULT_MODEL"),
        EnvVarGuard::remove("ARTERM_OPENAI_COMPAT_LOCAL_ENABLED"),
    ];
    guards.extend(
        arterm_base::provider_catalog::openai_compatible_profiles()
            .iter()
...
fn named_openai_compatible_provider_sets_catalog_cache_namespace() {
    ...
    assert_eq!(
        std::env::var("ARTERM_OPENROUTER_CACHE_NAMESPACE").as_deref(),
        Ok("example-compat")
    );
}
...
fn named_openai_compatible_provider_exposes_static_models_as_routes() {
    ...
    let profile = arterm_base::config::NamedProviderConfig {
        ...
        models: vec![arterm_base::config::NamedProviderModelConfig {
...
fn named_openai_compatible_provider_uses_per_model_image_input_support() {
    ...
    let profile = arterm_base::config::NamedProviderConfig {
        ...
        models: vec![
            arterm_base::config::NamedProviderModelConfig {
...
fn kimi_for_coding_tool_call_message_includes_reasoning_content() {
    let _lock = ENV_LOCK.lock();
    let _thinking = EnvVarGuard::remove("ARTERM_OPENROUTER_THINKING");
    let (api_base, request_rx) = spawn_single_response_chat_server();
    let provider = OpenRouterProvider {
        api_base,
...
fn minimax_profile_exposes_static_models_before_catalog_refresh() {
    let models = arterm_base::provider_catalog::openai_compatible_profile_static_models(
        arterm_provider_metadata::MINIMAX_PROFILE,
    );
...
fn cerebras_profile_exposes_live_chat_models_before_catalog_refresh() {
    assert_eq!(
        arterm_provider_metadata::CEREBRAS_PROFILE.default_model,
        Some("gpt-oss-120b")
    );

    let models = arterm_base::provider_catalog::openai_compatible_profile_static_models(
        arterm_provider_metadata::CEREBRAS_PROFILE,
    );
...
fn openai_compatible_profiles_with_unverified_live_catalogs_have_static_fallbacks() {
    let cases = [
        (arterm_provider_metadata::OPENCODE_PROFILE, "minimax-m2.7"),
        (arterm_provider_metadata::OPENCODE_GO_PROFILE, "kimi-k2.5"),
        (arterm_provider_metadata::ZAI_PROFILE, "glm-4.7"),
        (
            arterm_provider_metadata::AI302_PROFILE,
            "qwen3-235b-a22b-instruct-2507",
        ),
        (arterm_provider_metadata::BASETEN_PROFILE, "zai-org/GLM-4.7"),
        (arterm_provider_metadata::CORTECS_PROFILE, "kimi-k2.5"),
        (arterm_provider_metadata::KIMI_PROFILE, "kimi-for-coding"),
        (arterm_provider_metadata::FIRMWARE_PROFILE, "kimi-k2.5"),
        (
            arterm_provider_metadata::HUGGING_FACE_PROFILE,
            "Qwen/Qwen3-Coder-480B-A35B-Instruct",
        ),
        (arterm_provider_metadata::MOONSHOT_PROFILE, "kimi-k2.5"),
        (
            arterm_provider_metadata::NEBIUS_PROFILE,
            "openai/gpt-oss-120b",
        ),
        (
            arterm_provider_metadata::SCALEWAY_PROFILE,
            "qwen3-coder-30b-a3b-instruct",
        ),
        (
            arterm_provider_metadata::STACKIT_PROFILE,
            "openai/gpt-oss-120b",
        ),
        (arterm_provider_metadata::PERPLEXITY_PROFILE, "sonar"),
        (
            arterm_provider_metadata::DEEPINFRA_PROFILE,
            "moonshotai/Kimi-K2-Instruct",
        ),
        (
            arterm_provider_metadata::FIREWORKS_PROFILE,
...
fn profiles_use_endpoint_default_max_tokens() {
    let _lock = ENV_LOCK.lock();
    let _override = EnvVarGuard::remove("ARTERM_OPENROUTER_MAX_TOKENS");
...
fn max_tokens_env_overrides_profile_default() {
    let _lock = ENV_LOCK.lock();
    let _override = EnvVarGuard::set("ARTERM_OPENROUTER_MAX_TOKENS", "4096");
...
fn test_configured_api_base_accepts_https() {
    let _lock = ENV_LOCK.lock();
    let prev = std::env::var("ARTERM_OPENROUTER_API_BASE").ok();
    arterm_base::env::set_var(
        "ARTERM_OPENROUTER_API_BASE",
        "https://api.groq.com/openai/v1/",
    );
    assert_eq!(configured_api_base(), "https://api.groq.com/openai/v1");
    if let Some(value) = prev {
        arterm_base::env::set_var("ARTERM_OPENROUTER_API_BASE", value);
    } else {
        arterm_base::env::remove_var("ARTERM_OPENROUTER_API_BASE");
    }
}
...
fn test_configured_api_base_rejects_insecure_http_remote() {
    let _lock = ENV_LOCK.lock();
    let prev = std::env::var("ARTERM_OPENROUTER_API_BASE").ok();
    arterm_base::env::set_var("ARTERM_OPENROUTER_API_BASE", "http://example.com/v1");
...
fn autodetects_single_saved_openai_compatible_profile() {
    ...
    let opencode = arterm_base::provider_catalog::resolve_openai_compatible_profile(
        arterm_base::provider_catalog::OPENCODE_PROFILE,
    );
    write_test_api_key(
        &temp,
        &opencode.env_file,
...
fn autodetects_single_saved_local_openai_compatible_profile() {
    ...
    let lmstudio = arterm_base::provider_catalog::resolve_openai_compatible_profile(
        arterm_base::provider_catalog::LMSTUDIO_PROFILE,
    );
    let config_dir = test_config_dir(&temp).join("arterm");
    std::fs::create_dir_all(&config_dir).expect("create test config dir");
    std::fs::write(
        ...
        format!(
            "{}=1\n",
            arterm_base::provider_catalog::OPENAI_COMPAT_LOCAL_ENABLED_ENV
        ),
    )
...
fn openrouter_transport_state_distinguishes_runtime_identities() {
    ...
    arterm_base::env::set_var("ARTERM_OPENROUTER_TRANSPORT_STATE", "direct-api-key");
    assert_eq!(
        OpenRouterTransportState::from_current_env(None),
        OpenRouterTransportState::DirectApiKey
    );
    arterm_base::env::remove_var("ARTERM_OPENROUTER_TRANSPORT_STATE");

    arterm_base::env::set_var("ARTERM_RUNTIME_PROVIDER", "openrouter");
    assert_eq!(
    ...
    arterm_base::env::remove_var("ARTERM_RUNTIME_PROVIDER");

    arterm_base::env::set_var("ARTERM_RUNTIME_PROVIDER", "arterm");
    assert_eq!(
    ...
    arterm_base::env::set_var("ARTERM_OPENROUTER_ALLOW_NO_AUTH", "1");
    assert_eq!(
        OpenRouterTransportState::from_current_env(Some("openai-compatible")),
    ...
    arterm_base::env::remove_var("ARTERM_OPENROUTER_ALLOW_NO_AUTH");
    arterm_base::env::remove_var("ARTERM_RUNTIME_PROVIDER");
    arterm_base::env::set_var("ARTERM_NAMED_PROVIDER_PROFILE", "my-gateway");
    assert_eq!(
        OpenRouterTransportState::from_current_env(None),
        OpenRouterTransportState::DirectApiKey
    );
...
fn does_not_guess_when_multiple_saved_openai_compatible_profiles_exist() {
    ...
    let chutes = arterm_base::provider_catalog::resolve_openai_compatible_profile(
        arterm_base::provider_catalog::CHUTES_PROFILE,
    );
    write_test_api_key(
        &temp,
        &opencode.env_file,
...
fn autodetected_profile_seeds_default_model_and_cache_namespace() {
    ...
    let zai = arterm_base::provider_catalog::resolve_openai_compatible_profile(
        arterm_base::provider_catalog::ZAI_PROFILE,
    );
    ...
    assert_eq!(
        std::env::var("ARTERM_OPENROUTER_CACHE_NAMESPACE")
            .ok()
            .as_deref(),
        Some("zai")
    );
...
fn live_openrouter_models() -> Vec<String> {
    std::env::var("ARTERM_LIVE_OPENROUTER_MODELS")
        .or_else(|_| std::env::var("ARTERM_OPENROUTER_MODEL"))
...
async fn live_openrouter_unified_reasoning_smoke() -> Result<()> {
    ...
    let effort = std::env::var("ARTERM_LIVE_OPENROUTER_REASONING_EFFORT")
        .unwrap_or_else(|_| "low".to_string());
    let max_tokens = std::env::var("ARTERM_LIVE_OPENROUTER_MAX_TOKENS")
        .ok()
...
fn openai_compatible_model_catalog_refresh_calls_models_endpoint_and_updates_display() {
    ...
    assert!(
        request.to_ascii_lowercase().contains("user-agent: arterm/"),
...
fn direct_deepseek_profile_uses_static_1m_context_when_catalog_is_absent() {
    let _lock = ENV_LOCK.lock();
    let _base = EnvVarGuard::set("ARTERM_OPENROUTER_API_BASE", "https://api.deepseek.com");
    let _key_name = EnvVarGuard::set("ARTERM_OPENROUTER_API_KEY_NAME", "DEEPSEEK_API_KEY");
    let _api_key = EnvVarGuard::set("DEEPSEEK_API_KEY", "test");
    let _namespace = EnvVarGuard::set("ARTERM_OPENROUTER_CACHE_NAMESPACE", "deepseek");
    let _model = EnvVarGuard::set("ARTERM_OPENROUTER_MODEL", "deepseek-v4-flash");
    let _catalog = EnvVarGuard::set("ARTERM_OPENROUTER_MODEL_CATALOG", "0");
...
fn named_openai_compatible_model_context_window_overrides_default() {
    let _lock = ENV_LOCK.lock();
    let _namespace = EnvVarGuard::remove("ARTERM_OPENROUTER_CACHE_NAMESPACE");
    let mut config = arterm_base::config::NamedProviderConfig {
...
fn named_openai_compatible_loads_api_key_from_env_file() {
    ...
    let config = arterm_base::config::NamedProviderConfig {
        base_url: "https://compat.example.test/v1".to_string(),
...
fn runtime_display_name_for_profile_runtime_instance() {
    ...
    let _arterm_home = EnvVarGuard::set("ARTERM_HOME", &arterm_home);
    let _home = EnvVarGuard::set("HOME", temp.path());
    let _appdata = EnvVarGuard::set("APPDATA", temp.path().join("AppData").join("Roaming"));
    let _env = isolate_openrouter_autodetect_env();
    ...
    let nim = OpenRouterProvider::new_openai_compatible_profile_runtime(
        arterm_base::provider_catalog::NVIDIA_NIM_PROFILE,
    )
...
#[test]
fn arterm_subscription_runtime_has_explicit_display_and_route_identity() {
    let _lock = ENV_LOCK.lock();
    let temp = TempDir::new().expect("create temp home");
    let arterm_home = temp.path().join("arterm-home");
    let _arterm_home = EnvVarGuard::set("ARTERM_HOME", &arterm_home);
    ...
    let _base = EnvVarGuard::set(
        "ARTERM_OPENROUTER_API_BASE",
        arterm_base::subscription_catalog::DEFAULT_ARTERM_API_BASE,
    );
    let _key_name = EnvVarGuard::set(
        "ARTERM_OPENROUTER_API_KEY_NAME",
        arterm_base::subscription_catalog::ARTERM_API_KEY_ENV,
    );
    let _env_file = EnvVarGuard::set(
        "ARTERM_OPENROUTER_ENV_FILE",
        arterm_base::subscription_catalog::ARTERM_ENV_FILE,
    );
    let _provider_features = EnvVarGuard::set("ARTERM_OPENROUTER_PROVIDER_FEATURES", "0");
    let _transport = EnvVarGuard::set("ARTERM_OPENROUTER_TRANSPORT_STATE", "arterm-subscription");
    let _key = EnvVarGuard::set(
        arterm_base::subscription_catalog::ARTERM_API_KEY_ENV,
        "arterm_test_subscription_key",
    );

    let provider = OpenRouterProvider::new().expect("build arterm subscription runtime");
    assert_eq!(provider.runtime_display_name(), "Arterm Subscription");
    assert_eq!(Provider::display_name(&provider), "Arterm Subscription");
    assert_eq!(Provider::name(&provider), "openrouter");
    assert_eq!(
        provider.direct_openai_compatible_route_parts(),
        Some((
            "Arterm Subscription".to_string(),
            "arterm-subscription".to_string(),
            arterm_base::subscription_catalog::DEFAULT_ARTERM_API_BASE.to_string(),
        ))
    );
}
...
fn non_subscription_runtimes_keep_existing_display_and_route_identity() {
    ...
    let _base = EnvVarGuard::set("ARTERM_OPENROUTER_API_BASE", "https://example.com/v1");
    let _key_name = EnvVarGuard::set("ARTERM_OPENROUTER_API_KEY_NAME", "GENERIC_API_KEY");
    let _provider_features = EnvVarGuard::set("ARTERM_OPENROUTER_PROVIDER_FEATURES", "0");
    let _transport = EnvVarGuard::set("ARTERM_OPENROUTER_TRANSPORT_STATE", "direct-compatible");
...
#[test]
fn custom_endpoint_using_arterm_key_name_is_not_a_subscription_runtime() {
    let _lock = ENV_LOCK.lock();
    let temp = TempDir::new().expect("create temp home");
    let arterm_home = temp.path().join("arterm-home");
    let _arterm_home = EnvVarGuard::set("ARTERM_HOME", &arterm_home);
...
fn resolve_extra_body_returns_none_when_unset() {
    let _lock = ENV_LOCK.lock();
    let _guard = EnvVarGuard::remove("ARTERM_OPENAI_EXTRA_BODY");
...
fn resolve_extra_body_ignores_invalid_env_json() {
    let _lock = ENV_LOCK.lock();
    let _guard = EnvVarGuard::set("ARTERM_OPENAI_EXTRA_BODY", "not-json");
    assert!(OpenRouterProvider::resolve_extra_body(None, "nonexistent.env").is_none());
}
...
fn resolve_extra_body_ignores_non_object_env_json() {
    let _lock = ENV_LOCK.lock();
    let _guard = EnvVarGuard::set("ARTERM_OPENAI_EXTRA_BODY", "[1,2,3]");
    assert!(OpenRouterProvider::resolve_extra_body(None, "nonexistent.env").is_none());
}
...
fn named_profile_extra_body_threads_into_provider() {
    ...
    let _appdata = EnvVarGuard::set("APPDATA", temp.path().join("AppData").join("Roaming"));
    let _env = isolate_openrouter_autodetect_env();
    let _extra_guard = EnvVarGuard::remove("ARTERM_OPENAI_EXTRA_BODY");

    let mut profile = arterm_base::config::NamedProviderConfig {
...
fn named_profile_construction_reads_openai_reasoning_effort_config() {
    ...
    let configured = arterm_base::config::config()
        .provider
...
/// `arterm run` (which binds profile_id in-process) works fine.
#[test]
fn user_named_profile_prefix_is_stripped_even_without_profile_id() {
    ...
    std::fs::create_dir_all(&arterm_home).expect("create test config dir");
    std::fs::write(
        arterm_home.join("config.toml"),
        r#"
[provider]
...
"#,
    ...
    arterm_base::config::invalidate_config_cache();
