...
//! NVIDIA NIM), moved out of `arterm-base` so provider edits compile only this
...
//! with `arterm_base::provider::external::register_openrouter_factory`.
...
use arterm_provider_openrouter::{
...
const OPENROUTER_TRANSPORT_STATE_ENV: &str = "ARTERM_OPENROUTER_TRANSPORT_STATE";
...
-> Option<arterm_base::provider_catalog::ResolvedOpenAiCompatibleProfile> {
    ...
    let mut matches = openai_compatible_profiles()
        ...
        .filter_map(|profile| {
            ...
            if arterm_base::provider_catalog::openai_compatible_profile_is_configured(*profile) {
                Some(resolved)
            } else {
                None
            }
...
fn configured_api_base() -> String {
    let raw = std::env::var("ARTERM_OPENROUTER_API_BASE")
        .ok()
    ...
    normalize_api_base(&raw).unwrap_or_else(|| {
        arterm_base::logging::warn(&format!(
            "Ignoring invalid ARTERM_OPENROUTER_API_BASE '{}'; using {}",
...
fn configured_api_key_name() -> String {
    let raw = std::env::var("ARTERM_OPENROUTER_API_KEY_NAME")
        .ok()
        .map(|v| v.trim().to_string())
        .filter(|v| !v.is_empty())
    ...
    if is_safe_env_key_name(&raw) {
        ...
        arterm_base::logging::warn(&format!(
            "Ignoring invalid ARTERM_OPENROUTER_API_KEY_NAME '{}'; using {}",
...
fn configured_env_file_name() -> String {
    let raw = std::env::var("ARTERM_OPENROUTER_ENV_FILE")
        .ok()
        .map(|v| v.trim().to_string())
        .filter(|v| !v.is_empty())
    ...
    if is_safe_env_file_name(&raw) {
        ...
        arterm_base::logging::warn(&format!(
            "Ignoring invalid ARTERM_OPENROUTER_ENV_FILE '{}'; using {}",
...
fn load_named_profile_api_key(
    env_key: &str,
    profile: &arterm_base::config::NamedProviderConfig,
) -> Option<String> {
...
fn provider_features_enabled(api_base: &str) -> bool {
    if let Ok(raw) = std::env::var("ARTERM_OPENROUTER_PROVIDER_FEATURES") {
        if let Some(value) = parse_env_bool(&raw) {
            return value;
        }
        arterm_base::logging::warn(&format!(
            "Ignoring invalid ARTERM_OPENROUTER_PROVIDER_FEATURES '{}'; expected true/false",
            raw
        ));
    }
...
fn model_catalog_enabled() -> bool {
    if let Ok(raw) = std::env::var("ARTERM_OPENROUTER_MODEL_CATALOG") {
        if let Some(value) = parse_env_bool(&raw) {
            return value;
        }
        arterm_base::logging::warn(&format!(
            "Ignoring invalid ARTERM_OPENROUTER_MODEL_CATALOG '{}'; expected true/false",
            raw
        ));
    }
    true
...
fn configured_auth_header_mode() -> AuthHeaderMode {
    let Some(raw) = std::env::var("ARTERM_OPENROUTER_AUTH_HEADER")
        .ok()
    ...
    match raw.as_str() {
        ...
        other => {
            arterm_base::logging::warn(&format!(
                "Ignoring invalid ARTERM_OPENROUTER_AUTH_HEADER '{}'; expected authorization-bearer or api-key",
                other
            ));
...
fn configured_auth_header_name() -> HeaderName {
    let raw = std::env::var("ARTERM_OPENROUTER_AUTH_HEADER_NAME")
        .ok()
        .map(|v| v.trim().to_string())
        .filter(|v| !v.is_empty())
    ...
    HeaderName::from_bytes(raw.as_bytes()).unwrap_or_else(|_| {
        arterm_base::logging::warn(&format!(
            "Ignoring invalid ARTERM_OPENROUTER_AUTH_HEADER_NAME '{}'; using api-key",
            raw
        ));
...
fn configured_dynamic_bearer_provider() -> Option<String> {
    std::env::var("ARTERM_OPENROUTER_DYNAMIC_BEARER_PROVIDER")
        .ok()
        .map(|v| v.trim().to_ascii_lowercase())
        .filter(|v| !v.is_empty())
}
...
fn configured_allow_no_auth() -> bool {
    std::env::var("ARTERM_OPENROUTER_ALLOW_NO_AUTH")
        .ok()
...
pub enum OpenRouterTransportState {
    ...
    OpenRouterApiKey,
    /// Arterm subscription access currently reuses the OpenRouter HTTP slot, but is
...
impl OpenRouterTransportState {
    pub fn from_current_env(runtime_provider: Option<&str>) -> Self {
        ...
        if matches!(runtime_provider.as_deref(), Some("arterm")) {
            return Self::ArtermSubscription;
        }
    ...
    fn from_env_marker() -> Option<Self> {
        ...
        match value.as_str() {
            ...
            "arterm" | "arterm-subscription" | "subscription" => Some(Self::ArtermSubscription),
            ...
            other => {
                arterm_base::logging::warn(&format!(
                    "Ignoring invalid {} '{}'; expected openrouter-api-key, arterm-subscription, direct-api-key, or direct-no-auth",
    ...
    fn runtime_provider_is_direct_compatible(runtime_provider: Option<&str>) -> bool {
        matches!(runtime_provider, Some("openai-compatible" | "azure-openai"))
            || runtime_provider
                .and_then(arterm_base::provider_catalog::openai_compatible_profile_by_id)
                .is_some()
    }
...
async fn fetch_models_from_api(
    ...
    if !response.status().is_success() {
        ...
        let body = arterm_base::util::http_error_body(response, "HTTP error").await;
        anyhow::bail!(
    ...
    let mut models = parse_openai_compatible_models_response(&raw_body).with_context(|| {
            format!(
                ...
                url,
                auth.label(),
                arterm_base::util::truncate_str(&raw_body.trim().replace('\n', "\\n"), 1200)
            )
        })?;
...
pub fn maybe_schedule_openai_compatible_profile_catalog_refresh(
    profile: arterm_base::provider_catalog::OpenAiCompatibleProfile,
    context: &'static str,
) -> bool {
    let resolved = arterm_base::provider_catalog::resolve_openai_compatible_profile(profile);
    ...
    let previous_fingerprint =
        arterm_provider_openrouter::load_disk_cache_entry_for_namespace(&profile_id)
    ...
    handle.spawn(async move {
        ...
        let result = fetch_models_from_api(
            arterm_provider_core::shared_http_client(),
            api_base,
            auth,
            models_cache,
        ...
        match result {
            Ok(models) => {
                ...
                if updated {
                    ...
                    arterm_base::bus::Bus::global().publish_models_updated();
                } else {
                    arterm_base::logging::info(&format!(
            ...
            Err(error) => arterm_base::logging::info(&format!(
...
pub fn maybe_schedule_standard_openrouter_catalog_refresh(context: &'static str) -> bool {
    ...
    // when ARTERM_OPENROUTER_* env vars are set by an active named profile
    // (e.g. NVIDIA NIM via `[providers.mynvidia]`, which sets
    // ARTERM_OPENROUTER_API_BASE to the NVIDIA endpoint): that profile owns the
    ...
    let cache_is_fresh = current_unix_secs()
        .zip(arterm_provider_openrouter::load_disk_cache_entry_for_namespace(namespace))
        .map(|(now, cache)| {
    ...
    let previous_fingerprint =
        arterm_provider_openrouter::load_disk_cache_entry_for_namespace(namespace)
            .map(|cache| models_fingerprint(&cache.models))
            .unwrap_or_default();
    handle.spawn(async move {
        let models_cache = Arc::new(RwLock::new(ModelsCache::default()));
...
pub struct OpenRouterProvider {
    ...
    /// `ARTERM_OPENAI_EXTRA_BODY` env/env-file value.
...
impl OpenRouterProvider {
    ...
    fn configured_max_tokens(_profile_id: Option<&str>) -> Option<u32> {
        if let Ok(raw) = std::env::var("ARTERM_OPENROUTER_MAX_TOKENS") {
            ...
            match trimmed.parse::<u32>() {
                ...
                Err(_) => arterm_base::logging::warn(&format!(
                    "Ignoring invalid ARTERM_OPENROUTER_MAX_TOKENS '{}'; expected a positive integer or auto",
                    raw
                )),
            }
        }
    ...
    /// 2. The `ARTERM_OPENAI_EXTRA_BODY` env var (or the same key inside the
    ...
    fn resolve_extra_body(
        ...
        if let Some(value) = config {
            match value.as_object() {
                ...
                None => arterm_base::logging::warn(
                    "Ignoring provider `extra_body`: expected a table/object of top-level request fields",
                ),
            }
        }

        if let Some(raw) = load_env_value_from_env_or_config("ARTERM_OPENAI_EXTRA_BODY", env_file) {
            match serde_json::from_str::<Value>(&raw) {
                ...
                Ok(_) => arterm_base::logging::warn(
                    "Ignoring ARTERM_OPENAI_EXTRA_BODY: expected a JSON object string, e.g. {\"chat_template_kwargs\":{\"thinking\":true}}",
                ),
                Err(err) => arterm_base::logging::warn(&format!(
                    "Ignoring invalid ARTERM_OPENAI_EXTRA_BODY JSON: {err}"
                )),
            }
        }
    ...
    ///
    /// Unlike the env-var based [`arterm_base::provider_catalog::runtime_provider_display_name`],
    ...
    pub fn runtime_display_name(&self) -> String {
        if self.is_arterm_subscription_runtime() {
            return arterm_base::subscription_catalog::ARTERM_PROVIDER_DISPLAY_NAME.to_string();
        }
        ...
        if !self.supports_provider_features {
            if let Some(profile_id) =
                arterm_base::provider_catalog::openai_compatible_profile_id_for_api_base(
                    &self.api_base,
                )
            ...
            if std::env::var("ARTERM_RUNTIME_PROVIDER")
                .ok()
    ...
    pub fn direct_openai_compatible_route_parts(&self) -> Option<(String, String, String)> {
        ...
        if self.is_arterm_subscription_runtime() {
            return Some((
                arterm_base::subscription_catalog::ARTERM_PROVIDER_DISPLAY_NAME.to_string(),
                arterm_base::subscription_catalog::ARTERM_ROUTE_API_METHOD.to_string(),
                self.api_base.clone(),
            ));
        }
    ...
    /// scoped `ARTERM_API_KEY`, then routes through the OpenAI-compatible
    ...
    fn is_arterm_subscription_runtime(&self) -> bool {
        !self.supports_provider_features
            && self.api_base.trim_end_matches('/')
                == arterm_base::subscription_catalog::DEFAULT_ARTERM_API_BASE.trim_end_matches('/')
            && self
                .auth
                .label()
                .eq_ignore_ascii_case(arterm_base::subscription_catalog::ARTERM_API_KEY_ENV)
    }
    ...
    pub fn new_named_openai_compatible(
        ...
        arterm_base::env::set_var("ARTERM_OPENROUTER_CACHE_NAMESPACE", profile_name);
        ...
        let auth = match profile.auth {
            arterm_base::config::NamedProviderAuth::None => ProviderAuth::None {
                label: "local endpoint (no auth)".to_string(),
            },
            arterm_base::config::NamedProviderAuth::Bearer => ProviderAuth::AuthorizationBearer {
                token: key
                    .ok_or_else(|| anyhow::anyhow!("{} not found in environment", key_label))?,
                label: key_label,
            },
            arterm_base::config::NamedProviderAuth::Header => ProviderAuth::HeaderValue {
        ...
        Ok(Self {
            ...
            supports_provider_features: matches!(
                ...
                arterm_base::config::NamedProviderType::OpenRouter
    ...
    fn is_kimi_model(model: &str) -> bool {
        arterm_provider_openrouter::is_kimi_model(model)
    }
    ...
    fn strip_session_profile_prefix<'a>(&self, model: &'a str) -> &'a str {
        ...
        if arterm_provider_core::explicit_model_provider_prefix(model).is_some() {
            return model;
        }
        let rest = rest.trim();
        if rest.is_empty() {
    ...
    fn thinking_override() -> Option<bool> {
        let raw = std::env::var("ARTERM_OPENROUTER_THINKING").ok()?;
        ...
        match value.as_str() {
            ...
            other => {
                arterm_base::logging::info(&format!(
                    "Warning: Unsupported ARTERM_OPENROUTER_THINKING '{}'; expected enabled/disabled/auto",
                    other
                ));
                None
            }
    ...
    pub fn new() -> Result<Self> {
        ...
        let profile_id = std::env::var("ARTERM_OPENROUTER_CACHE_NAMESPACE")
            .ok()
            .map(|value| value.trim().to_ascii_lowercase())
            .filter(|value| !value.is_empty())
        ...
        let static_context_limits = profile_id
            ...
            .unwrap_or_default();
        let static_models = std::env::var("ARTERM_OPENROUTER_STATIC_MODELS")
            .ok()
            .map(|raw| {
                raw.lines()
                    .map(str::trim)
        ...
        if std::env::var_os("ARTERM_OPENROUTER_CACHE_NAMESPACE").is_none()
            && let Some(profile) = autodetected_profile.as_ref()
        {
            arterm_base::env::set_var("ARTERM_OPENROUTER_CACHE_NAMESPACE", &profile.id);
        }

        let model = std::env::var("ARTERM_OPENROUTER_MODEL")
            .ok()
    ...
    pub fn new_openai_compatible_profile_runtime(
        ...
        let auth = match profile_auth(&resolved) {
            ...
            None => {
                ...
                anyhow::bail!(
                    "{} credentials not available. {} not found in environment or {}. Run `arterm login --provider {}` first.",
    ...
    pub(crate) fn filter_profile_chat_supported_models(&self, models: Vec<String>) -> Vec<String> {
        ...
        models
            .into_iter()
            .filter(|model| {
                arterm_base::provider_catalog::openai_compatible_profile_model_supports_chat(
                    profile_id, model,
                )
            })
            .collect()
    ...
    fn model_disk_cache_source_matches(
        &self,
        cache_entry: &arterm_provider_openrouter::DiskCache,
    ) -> bool {
    ...
    fn maybe_schedule_model_catalog_refresh(&self, cache_age_secs: u64, context: &'static str) {
        ...
        handle.spawn(async move {
            match fetch_models_from_api(client, api_base, auth, models_cache, ns).await {
                ...
                Err(e) => arterm_base::logging::info(&format!(
    ...
    fn parse_provider_routing() -> ProviderRouting {
        arterm_provider_openrouter::parse_provider_routing_from_env()
    }
    ...
    fn rank_providers_from_endpoints(endpoints: &[EndpointInfo]) -> Vec<String> {
        arterm_provider_openrouter::rank_providers_from_endpoints(endpoints)
    }
    ...
    pub fn has_credentials() -> bool {
        if matches!(
            ...
            return arterm_base::auth::azure::has_configuration();
        }
        if configured_allow_no_auth() {
            return true;
        }
    ...
    fn resolve_auth() -> Result<ProviderAuth> {
        if let Some(provider) = configured_dynamic_bearer_provider() {
            return match provider.as_str() {
                "azure" => {
                    if arterm_base::auth::azure::has_configuration() {
                        Ok(ProviderAuth::AzureEntra {
                            label: "Azure OpenAI Entra ID".to_string(),
                        })
                    } else {
                        anyhow::bail!(
                            "Azure OpenAI is configured for Entra ID, but Azure settings are incomplete. Run `arterm login --provider azure`."
                        )
                    }
                }
                other => anyhow::bail!(
                    "Unsupported ARTERM_OPENROUTER_DYNAMIC_BEARER_PROVIDER '{}'.",
                    other
                ),
            };
        }
