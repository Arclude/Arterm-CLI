...
//! persist a backoff window every arterm process on the machine can see, and
//! label the error so UIs can stay quiet about it.

use super::update_metadata::UpdateMetadata;
use arterm_update_core::format_duration_estimate;
...
/// Persist the backoff window so every arterm process on this machine stops
