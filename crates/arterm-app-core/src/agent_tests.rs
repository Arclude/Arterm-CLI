...
async fn gmail_is_exposed_by_default_and_can_be_explicitly_disabled() {
    ...
    let prev_tools = std::env::var_os("ARTERM_TOOLS");
    let prev_disabled_tools = std::env::var_os("ARTERM_DISABLED_TOOLS");
    let prev_tool_profile = std::env::var_os("ARTERM_TOOL_PROFILE");
    let prev_disable_base_tools = std::env::var_os("ARTERM_DISABLE_BASE_TOOLS");
    let temp_home = tempfile::TempDir::new().expect("temp home");

    crate::env::set_var("ARTERM_HOME", temp_home.path());
    crate::env::remove_var("ARTERM_TOOLS");
    crate::env::remove_var("ARTERM_DISABLED_TOOLS");
    crate::env::remove_var("ARTERM_TOOL_PROFILE");
    crate::env::remove_var("ARTERM_DISABLE_BASE_TOOLS");
    ...
    assert!(
        tool_names.iter().any(|name| name == "arterm_docs"),
        "arterm_docs must be model-visible in regular sessions"
    );
    assert!(
    ...
    crate::env::set_var("ARTERM_DISABLED_TOOLS", tool_name);
    crate::config::Config::invalidate_cache();
    ...
    if let Some(previous) = prev_home {
        crate::env::set_var("ARTERM_HOME", previous);
    } else {
        crate::env::remove_var("ARTERM_HOME");
    }
    if let Some(previous) = prev_tools {
        crate::env::set_var("ARTERM_TOOLS", previous);
    } else {
        crate::env::remove_var("ARTERM_TOOLS");
    }
    if let Some(previous) = prev_disabled_tools {
        crate::env::set_var("ARTERM_DISABLED_TOOLS", previous);
    } else {
        crate::env::remove_var("ARTERM_DISABLED_TOOLS");
    }
    if let Some(previous) = prev_tool_profile {
        crate::env::set_var("ARTERM_TOOL_PROFILE", previous);
    } else {
        crate::env::remove_var("ARTERM_TOOL_PROFILE");
    }
    if let Some(previous) = prev_disable_base_tools {
        crate::env::set_var("ARTERM_DISABLE_BASE_TOOLS", previous);
    } else {
        crate::env::remove_var("ARTERM_DISABLE_BASE_TOOLS");
    }
    crate::config::Config::invalidate_cache();
...
async fn memory_injection_message_defaults_to_ephemeral_history() {
    let _guard = crate::storage::lock_test_env();
    let previous = std::env::var_os("ARTERM_PERSIST_MEMORY_INJECTIONS");
    crate::env::set_var("ARTERM_PERSIST_MEMORY_INJECTIONS", "false");
    ...
    match previous {
        Some(value) => crate::env::set_var("ARTERM_PERSIST_MEMORY_INJECTIONS", value),
        None => crate::env::remove_var("ARTERM_PERSIST_MEMORY_INJECTIONS"),
    }
    crate::config::invalidate_config_cache();
}
...
async fn memory_injection_message_can_persist_to_history() {
    let _guard = crate::storage::lock_test_env();
    let previous = std::env::var_os("ARTERM_PERSIST_MEMORY_INJECTIONS");
    crate::env::set_var("ARTERM_PERSIST_MEMORY_INJECTIONS", "true");
    crate::config::invalidate_config_cache();
...
async fn env_snapshot_detail_is_minimal_for_empty_sessions_and_full_after_history() {
    ...
    assert!(minimal.arterm_git_hash.is_none());
    assert!(minimal.arterm_git_dirty.is_none());
