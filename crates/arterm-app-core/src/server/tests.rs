...
async fn removing_server_session_clears_active_pid_marker() {
    ...
    let home = tempfile::tempdir().expect("create temporary ARTERM_HOME");
    let _home_guard = ScopedEnvVar::set("ARTERM_HOME", home.path());
...
fn server_identity_uses_configured_name() {
    let _guard = crate::storage::lock_test_env();
    let _server_name_guard = ScopedEnvVar::set("ARTERM_SERVER_NAME", "env-name");
    let _server_display_name_guard = ScopedEnvVar::set("ARTERM_SERVER_DISPLAY_NAME", "display-name");
...
fn server_identity_reads_name_from_env() {
    let _guard = crate::storage::lock_test_env();
    let _server_name_guard = ScopedEnvVar::set("ARTERM_SERVER_NAME", "mount-cloud/john");
    let _server_display_name_guard = ScopedEnvVar::set("ARTERM_SERVER_DISPLAY_NAME", "ignored");
...
impl Drop for EnvGuard {
    fn drop(&mut self) {
        if let Some(value) = &self.prev_home {
            crate::env::set_var("ARTERM_HOME", value);
        } else {
            crate::env::remove_var("ARTERM_HOME");
        }
        if let Some(value) = &self.prev_runtime_dir {
            crate::env::set_var("ARTERM_RUNTIME_DIR", value);
        } else {
            crate::env::remove_var("ARTERM_RUNTIME_DIR");
        }
        if let Some(value) = &self.prev_socket {
            crate::env::set_var("ARTERM_SOCKET", value);
        } else {
...
fn configure_test_env(root: &tempfile::TempDir) -> EnvGuard {
    ...
    crate::env::set_var("ARTERM_HOME", &home_dir);
    crate::env::set_var("ARTERM_RUNTIME_DIR", &runtime_dir);
    crate::env::remove_var("ARTERM_SOCKET");
    EnvGuard {
        prev_home,
        prev_runtime_dir,
...
#[allow(
    ...
    reason = "test intentionally serializes process-wide ARTERM_HOME/env state across async recovery assertions"
)]
...
#[allow(
    clippy::await_holding_lock,
    reason = "test intentionally serializes process-wide ARTERM_HOME/env state across async startup assertions"
)]
async fn startup_ready_signal_is_not_blocked_by_headless_recovery_delay() -> Result<()> {
    ...
    let temp = tempfile::TempDir::new()?;
    let _env = configure_test_env(&temp);
    let _delay_guard = ScopedEnvVar::set("ARTERM_TEST_HEADLESS_STARTUP_RECOVERY_DELAY_MS", "500");
    ...
    let _ready_fd_guard = ScopedEnvVar::set("ARTERM_READY_FD", write_fd.to_string());
