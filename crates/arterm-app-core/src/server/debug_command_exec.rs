...
use anyhow::Result;
use arterm_agent_runtime::{InterruptSignal, SoftInterruptSource};
...
pub(super) fn debug_message_timeout_secs() -> Option<u64> {
    let raw = std::env::var("ARTERM_DEBUG_MESSAGE_TIMEOUT_SECS").ok()?;
...
pub(super) async fn execute_debug_command(
    ...
    if trimmed == "mcp" || trimmed == "mcp:servers" {
        ...
        let config_path = if let Ok(arterm_dir) = crate::storage::arterm_dir() {
            let path = arterm_dir.join("mcp.json");
            if path.exists() {
        ...
        let default_model = match provider.as_str() {
            "claude" | "anthropic" => {
                if claude_usage_exhausted {
                    ...
                    arterm_provider_core::DEFAULT_CLAUDE_MODEL
                }
            }
            "openai" | "codex" => arterm_provider_core::DEFAULT_OPENAI_MODEL,
    ...
    if trimmed == "reload" {
        let repo_dir = crate::build::get_repo_dir()
            .ok_or_else(|| anyhow::anyhow!("Could not find arterm repository directory"))?;

        let target_binary = crate::build::find_dev_binary(&repo_dir)
            .unwrap_or_else(|| build::release_binary_path(&repo_dir));
        if !target_binary.exists() {
            return Err(anyhow::anyhow!(format!(
                "No binary found at {}. Run 'arterm self-dev --build' first, or build with 'scripts/dev_cargo.sh build --profile selfdev -p arterm --bin arterm' and publish current.",
        ...
        let arterm_dir = crate::storage::arterm_dir()?;
        let info_path = arterm_dir.join("reload-info");
...
mod tests {
    ...
    /// which set `ARTERM_HOME` under `storage::lock_test_env`). Two mutexes
    ...
    async fn debug_tool_selfdev_reload_returns_promptly_for_direct_execution() {
        ...
        let _test_session = EnvGuard::set("ARTERM_TEST_SESSION", "1");
        let _debug_control = EnvGuard::set("ARTERM_DEBUG_CONTROL", "1");
