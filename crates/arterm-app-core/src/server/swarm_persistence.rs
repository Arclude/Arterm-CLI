...
use crate::storage;
use arterm_swarm_core::{SwarmLifecycleStatus, SwarmMemberRecord};
...
/// Directory name under the durable state dir (`~/.arterm/state`).
const SWARM_STATE_DIR: &str = "swarm";
/// Pre-0.36 location under the runtime dir (tmpfs on Linux, wiped on reboot).
const LEGACY_SWARM_STATE_DIR: &str = "arterm-swarm-state";
...
fn dormant_plan_retention() -> Duration {
    Duration::from_secs(
        std::env::var("ARTERM_SWARM_DORMANT_PLAN_RETENTION_SECS")
            .ok()
...
fn persisted_plan_is_dormant(plan: &PersistedVersionedPlan) -> bool {
    plan.items.is_empty()
        || plan
            .items
            .iter()
            .all(|item| item.assigned_to.is_none() && !arterm_plan::is_active_status(&item.status))
}
...
/// `ARTERM_RUNTIME_DIR`. Never let those tests fall through to the real
/// `~/.arterm/state/swarm`: that leaked synthetic `swarm-1` plans/members into
/// live user state during ordinary `cargo test` runs. Tests that need an
/// isolated explicit location still set `ARTERM_RUNTIME_DIR` and use the normal
/// resolver; otherwise use a process-local temp directory.
#[cfg(test)]
fn state_dir() -> PathBuf {
    if std::env::var_os("ARTERM_RUNTIME_DIR").is_some() {
        storage::durable_state_dir().join(SWARM_STATE_DIR)
    } else {
        std::env::temp_dir()
            .join(format!("arterm-test-state-{}", std::process::id()))
