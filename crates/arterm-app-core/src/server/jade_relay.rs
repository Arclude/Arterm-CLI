...
impl RelayListenerConfig {
    fn from_safety(safety: &SafetyConfig) -> Option<Self> {
        ...
        Some(Self {
            ...
            process_existing_prompts: env_flag("ARTERM_JADE_RELAY_PROCESS_EXISTING_PROMPTS"),
        })
    }
}
...
fn default_device_id() -> String {
    if let Ok(value) = std::env::var("ARTERM_JADE_RELAY_DEVICE_ID")
        && !value.trim().is_empty()
    {
...
impl RelayLauncherClient {
    ...
    async fn handle_launch(
        ...
        if !launched {
            anyhow::bail!("no supported terminal found for headed Arterm launch")
        }
        ...
        let _ = self
            .post_device_event(
                "launch_status",
                &format!("Launched headed Arterm session {session_id}"),
                event.seq,
                Some(launched_data),
            )
            .await;
...
async fn deliver_to_session(
    ...
    let Some(agent) = agent else {
        anyhow::bail!("session '{session_id}' is not live in this Arterm server")
    };
