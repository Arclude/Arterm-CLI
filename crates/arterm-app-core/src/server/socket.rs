...
pub fn socket_path() -> PathBuf {
    if let Ok(custom) = std::env::var("ARTERM_SOCKET") {
    ...
    match Stream::connect(path).await {
        ...
        Err(err) if err.kind() == std::io::ErrorKind::ConnectionRefused && path.exists() => {
            Err(anyhow::Error::new(err).context(format!(
                "Socket exists but refused the connection at {}. Retry, or remove it after confirming no arterm server is running.",
                path.display()
            )))
        }
...
///
/// Safety: a live daemon holds an exclusive `flock` on `arterm-daemon.lock` for
...
pub async fn reap_stale_socket_if_dead(path: &std::path::Path) -> bool {
    ...
    crate::logging::warn(&format!(
        "Reaping stale arterm socket with no live listener at {}",
        path.display()
    ));
...
pub(super) fn daemon_lock_path() -> PathBuf {
    crate::storage::runtime_dir().join("arterm-daemon.lock")
}
...
pub(super) fn acquire_daemon_lock() -> Result<DaemonLockGuard> {
    ...
    try_acquire_daemon_lock(&path)?.ok_or_else(|| {
        anyhow::anyhow!(
            "Another arterm server process is already running for runtime dir {}",
...
/// `ARTERM_READY_FD`, and awaits a single byte on the read end. The server
...
pub async fn spawn_server_notify(cmd: &mut std::process::Command) -> Result<std::process::Child> {
    ...
    cmd.env("ARTERM_READY_FD", write_fd.to_string());
...
pub(super) fn server_start_matches_existing_server(stderr_output: &str) -> bool {
    stderr_output.contains("Another arterm server process is already running")
...
pub(super) fn format_server_start_error(
    ...
    if stderr_output.trim().is_empty() {
        format!(
            "Server exited before signalling ready ({}). Check logs at ~/.arterm/logs/",
            status
        )
    } else {
        format!(
...
/// Write a single byte to the fd in `ARTERM_READY_FD` and close it.
...
pub(super) fn signal_ready_fd() {
    #[cfg(unix)]
    {
        ...
        if let Ok(fd_str) = std::env::var("ARTERM_READY_FD") {
            crate::env::remove_var("ARTERM_READY_FD");
