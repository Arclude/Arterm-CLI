...
fn prepare_server_exec(cmd: &mut std::process::Command, socket_path: &std::path::Path) {
    ...
    cmd.env_remove("ARTERM_READY_FD");
...
pub(super) async fn await_reload_signal(
    ...
    loop {
        ...
        if std::env::var("ARTERM_TEST_SESSION")
            .map(|value| {
        ...
        {
            crate::logging::info(
                "Server: ARTERM_TEST_SESSION set, skipping process exec for reload test",
            );
            continue;
        }
        ...
        if let Some((binary, label)) = super::reload_exec_target(prefers_selfdev) {
            if binary.exists() {
                ...
                {
                    cmd.env("ARTERM_RELOAD_AUTH_STATUS", auth_status);
                }
