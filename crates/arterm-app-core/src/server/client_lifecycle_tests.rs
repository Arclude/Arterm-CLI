...
impl IsolatedReloadRecoveryEnv {
    fn new() -> Self {
        let home = tempfile::TempDir::new().expect("arterm home");
        ...
        crate::env::set_var("ARTERM_RUNTIME_DIR", runtime.path());
        crate::server::clear_reload_marker();
        Self {
            prev_home,
            prev_runtime,
...
fn initial_subscribe_requires_an_absolute_client_working_dir() {
    ...
    let absolute = std::env::temp_dir().join("arterm-client-project");
    assert_eq!(
...
async fn new_client_agent_stamps_client_cwd_into_initial_context() {
    ...
    let registry = Registry::new(Arc::clone(&provider)).await;
    let client_cwd = std::env::temp_dir().join("arterm-authoritative-client-project");
