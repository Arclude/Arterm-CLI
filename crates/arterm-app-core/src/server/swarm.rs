...
/// `arterm_swarm_core` so the server, tools, and prompts all agree on the one
...
/// cap still apply.
pub(super) use arterm_swarm_core::MAX_SWARM_MEMBERS;
...
fn swarm_status_debounce_member_threshold() -> usize {
    ...
    CACHED
        .get_or_init(|| {
            let configured = std::env::var("ARTERM_SWARM_STATUS_DEBOUNCE_MEMBER_THRESHOLD")
                .ok()
...
fn swarm_status_debounce_ms() -> u64 {
    ...
    CACHED
        .get_or_init(|| {
            let configured = std::env::var("ARTERM_SWARM_STATUS_DEBOUNCE_MS")
                .ok()
...
pub(super) fn swarm_task_heartbeat_interval() -> Duration {
    Duration::from_secs(configured_positive_u64(
        "ARTERM_SWARM_TASK_HEARTBEAT_SECS",
...
pub(super) fn swarm_task_stale_after() -> Duration {
    Duration::from_secs(configured_positive_u64(
        "ARTERM_SWARM_TASK_STALE_AFTER_SECS",
...
pub(super) fn swarm_task_sweep_interval() -> Duration {
    Duration::from_secs(configured_positive_u64(
        "ARTERM_SWARM_TASK_SWEEP_INTERVAL_SECS",
...
pub(super) fn swarm_terminal_member_retention() -> Duration {
    Duration::from_secs(configured_positive_u64(
        "ARTERM_SWARM_TERMINAL_MEMBER_RETENTION_SECS",
...
pub(super) fn swarm_terminal_member_gc_interval() -> Duration {
    Duration::from_secs(configured_positive_u64(
        "ARTERM_SWARM_TERMINAL_MEMBER_GC_INTERVAL_SECS",
...
pub(super) fn swarm_status_broadcast_terminal_retention() -> Duration {
    Duration::from_secs(configured_positive_u64(
        "ARTERM_SWARM_STATUS_BROADCAST_TERMINAL_SECS",
...
pub(super) fn swarm_idle_worker_reap_after() -> Option<Duration> {
    let secs = std::env::var("ARTERM_SWARM_IDLE_WORKER_REAP_SECS")
        .ok()
        .and_then(|value| value.trim().parse::<u64>().ok())
...
pub(super) async fn set_member_task_label(
    ...
    let Some(label) = arterm_swarm_core::derive_swarm_task_label(task_text) else {
        return;
    };
    let mut members = swarm_members.write().await;
    if let Some(member) = members.get_mut(session_id) {
...
mod tests {
    ...
    fn idle_worker_reap_window_env_zero_disables() {
        ...
        // it.
        unsafe {
            std::env::set_var("ARTERM_SWARM_IDLE_WORKER_REAP_SECS", "0");
        }
        assert_eq!(super::swarm_idle_worker_reap_after(), None);
        unsafe {
            std::env::set_var("ARTERM_SWARM_IDLE_WORKER_REAP_SECS", "90");
        }
        assert_eq!(
        ...
        unsafe {
            std::env::remove_var("ARTERM_SWARM_IDLE_WORKER_REAP_SECS");
        }
    ...
    /// sending. Swarms below `ARTERM_SWARM_STATUS_DEBOUNCE_MEMBER_THRESHOLD`
