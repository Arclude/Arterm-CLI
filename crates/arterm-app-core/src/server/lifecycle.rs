...
const OWNER_PID_ENV: &str = "ARTERM_SERVER_OWNER_PID";
const TEMP_IDLE_SECS_ENV: &str = "ARTERM_TEMP_SERVER_IDLE_SECS";
...
mod tests {
    ...
    fn temporary_metadata_path_is_socket_scoped() {
        assert_eq!(
            metadata_path(Path::new("/tmp/example/arterm.sock")),
            PathBuf::from("/tmp/example/arterm.sock.server.json")
        );
    }
