...
static ARTERM_REPO_SOURCE_STATE: LazyLock<(Option<String>, Option<bool>)> = LazyLock::new(|| {
...
impl Agent {
    ...
    fn record_client_cache_request(&mut self, messages: &[Message]) {
        ...
        let fast_snapshot =
            if !self.provider.uses_arterm_compaction() && self.session.compaction.is_none() {
