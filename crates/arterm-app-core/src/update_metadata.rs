//! On-disk state shared by every arterm process that checks for updates.
//!
//! Update checks are cheap but rate limited, and several arterm processes can run
...
pub struct UpdateMetadata {
    ...
    /// the metadata file so every arterm process on the machine backs off, not
...
pub(super) fn metadata_path() -> Result<PathBuf> {
    Ok(storage::arterm_dir()?.join("update_metadata.json"))
}
