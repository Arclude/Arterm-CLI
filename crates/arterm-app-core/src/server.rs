...
const SERVER_NAME_ENV: &str = "ARTERM_SERVER_NAME";
const SERVER_DISPLAY_NAME_ENV: &str = "ARTERM_SERVER_DISPLAY_NAME";
...
pub use self::socket::{
    ...
    reap_stale_socket_if_dead, set_socket_path, socket_is_remote_peer, socket_path,
...
impl Server {
    ...
    fn spawn_registry_metadata_publisher(&self, registry_info: crate::registry::ServerInfo) {
        ...
        tokio::spawn(async move {
            ...
            let _ = std::fs::write(&hash_path, arterm_build_meta::git_hash());
    ...
    fn spawn_power_inhibitor(swarm_members: Arc<RwLock<HashMap<String, SwarmMember>>>) {
        ...
        if !inhibitor.is_available() {
            ...
            crate::logging::info(
                "power_inhibit: unavailable (unsupported platform or ARTERM_DISABLE_POWER_INHIBIT set); not monitoring",
            );
            return;
        }
    ...
    pub async fn run(&self) -> Result<()> {
        // Ensure socket directory exists (for named sockets like /run/user/1000/arterm/)
