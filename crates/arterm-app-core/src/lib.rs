...
//! Application core for arterm (upper layer).
//!
...
//! message, memory, telemetry, ...) lives in the `arterm-base` crate and is
//! re-exported here via `pub use arterm_base::*`, so every existing
//! `crate::<module>` path (e.g. `crate::config`, `crate::provider`) keeps
//! resolving unchanged across this crate and the root `arterm` crate, which in
//! turn re-exports this crate via `pub use arterm_app_core::*`.

// Foundational layer: re-export every `arterm-base` module so `crate::<module>`
