...
const DISCOVERY_REQUEST_ID_HEADER: &str = "x-arterm-discovery-request-id";
const DISCOVERY_CORRELATION_ID_HEADER: &str = "x-arterm-session-correlation-id";
const DISCOVERY_BENCHMARK_HEADER: &str = "x-arterm-discovery-benchmark";
const DISCOVERY_SESSION_ID_HEADER: &str = "x-arterm-discovery-session-id";
const DISCOVERY_SESSION_METADATA_HEADER: &str = "x-arterm-discovery-session-metadata";
const DISCOVERY_SELF_DEV_HEADER: &str = "x-arterm-discovery-self-dev";
const DISCOVERY_DEBUG_HEADER: &str = "x-arterm-discovery-debug";
const DISCOVERY_CANARY_HEADER: &str = "x-arterm-discovery-canary";
const DISCOVERY_EXECUTION_MODE_HEADER: &str = "x-arterm-discovery-execution-mode";
const DISCOVERY_BUILD_CHANNEL_HEADER: &str = "x-arterm-discovery-build-channel";
const DISCOVERY_GIT_CHECKOUT_HEADER: &str = "x-arterm-discovery-git-checkout";
const DISCOVERY_CI_HEADER: &str = "x-arterm-discovery-ci";
const DISCOVERY_RAN_FROM_CARGO_HEADER: &str = "x-arterm-discovery-ran-from-cargo";
const DISCOVERY_BENCHMARK_ENV: &str = "ARTERM_DISCOVERY_BENCHMARK";
...
impl Tool for DiscoverToolsTool {
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        // metered coarsely; see arterm_base::sponsors::provenance).
    ...
    let mut request = context.provenance.apply(
        context
            ...
            .header(
                ...
                format!("arterm/{}", env!("CARGO_PKG_VERSION")),
            )
...
fn render_listing(category: &str, listing: &Value, request_id: &str) -> Result<String> {
    ...
    let mut out = format!(
        "Available integrations in '{category}' (Arterm integration directory; recommendations must be based \
         only on fit; details: {}):\n",
    ...
    out.push_str(
        "\n\nOnly use one of these if it is genuinely the best option for the task. \
         ...
         actions (signups, spending) must note that setup is provided through a Arterm integration in the confirmation \
         shown to the user.",
    );
    out.push_str(&format!("\n\nSearch request ID: `{request_id}`"));
    Ok(out)
...
fn render_suggestion(
    ...
    out.push_str(
        "\n\nStatus: received for Arterm maintainer review. Suggestions are not sent to integration providers. This does not mean the tool has integrated with Arterm or that it is approved or available.",
    );
    Ok(out)
}
...
fn render_selection(category: &str, tool_name: &str, listing: &Value) -> Result<String> {
    ...
    if !listed {
        ...
        return Ok(format!(
            "Selected off-catalog product '{selected_tool}' for '{category}'.\n\n\
             Selection recorded as demand data. Arterm does not list an integration for this \
    ...
    let blurb = tool.get("blurb").and_then(|v| v.as_str()).unwrap_or("");
    let mut out = format!(
        "Selected '{name}' from '{category}' (Arterm integration directory; the choice must be based only \
         on fit; details: {}):\n\n{name}: {blurb}",
        crate::sponsors::DISCOVERY_PARTNERS_URL
    );
    if let Some(url) = tool.get("url").and_then(|v| v.as_str()) {
    ...
    out.push_str(&format!("\n\nSetup: {setup}"));
    out.push_str(
        "\n\nConsequential actions (signups, spending) must note that setup is provided through a Arterm integration in \
...
mod tests {
    ...
    fn render_listing_includes_disclosure_and_tools() {
        ...
        assert!(out.contains("Arterm integration directory"));
    ...
    fn suggestion_validation_distinguishes_product_and_capability_gap() {
        ...
        known.product_url = Some("https://example.com/tool?via=arterm#setup".to_string());
    ...
    fn render_suggestion_is_clear_about_review_status_and_recipient() {
        ...
        assert!(out.contains("does not mean the tool has integrated with Arterm"));
        assert!(!out.to_ascii_lowercase().contains("partner"));
    }
    ...
    async fn fetch_listing_round_trips_and_sends_only_expected_params() {
        ...
        assert!(
            request
                .to_ascii_lowercase()
                .contains("x-arterm-discovery-request-id: request-test-1"),
            "{request}"
        );
        assert!(
            request
                .to_ascii_lowercase()
                .contains("x-arterm-discovery-benchmark: 1"),
            "{request}"
        );
        for expected in [
            "x-arterm-discovery-session-id: session-test-1",
            "x-arterm-session-correlation-id: aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee",
            "x-arterm-discovery-session-metadata: 1",
            "x-arterm-discovery-self-dev: 1",
            "x-arterm-discovery-debug: 0",
            "x-arterm-discovery-canary: 1",
            "x-arterm-discovery-execution-mode: agent_turn",
            "x-arterm-discovery-build-channel: selfdev",
            "x-arterm-discovery-git-checkout: 1",
            "x-arterm-discovery-ci: 0",
            "x-arterm-discovery-ran-from-cargo: 1",
        ] {
    ...
    async fn submit_suggestion_posts_structured_maintainer_only_payload() {
        ...
        assert!(
            lower.contains("x-arterm-discovery-request-id: aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee"),
            "{request}"
        );
        assert!(
            lower.contains("x-arterm-discovery-benchmark: 1"),
            "{request}"
        );
    ...
    async fn execute_end_to_end_with_enabled_config_and_local_server() {
        ...
        assert!(output.output.contains("Arterm integration directory"));
        assert!(
            output
                .output
