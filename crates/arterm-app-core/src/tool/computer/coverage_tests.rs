...
//! run explicitly:
//!   cargo test -p arterm-app-core tool::computer::coverage -- --ignored --nocapture --test-threads=1
//!
...
use super::*;
use arterm_tool_core::{ToolContext, ToolExecutionMode};
...
async fn coverage_clipboard_scripting_system() {
    ...
    ok(json!({"action":"notify","text":"arterm coverage test","title":"arterm"})).await;
