...
fn repository_commands_export_a_logged_cargo_function() {
    let repo =
        crate::build::find_repo_in_ancestors(std::path::Path::new(env!("CARGO_MANIFEST_DIR")))
            .expect("test runs inside the arterm repository");
    let wrapped = wrap_repo_cargo_commands("cargo test -p demo && echo done", Some(&repo))
        .expect("arterm repository has dev_cargo.sh");

    assert!(wrapped.contains("export ARTERM_DEV_CARGO_SCRIPT="));
    assert!(wrapped.contains("ARTERM_IN_DEV_CARGO=1 \"$ARTERM_DEV_CARGO_SCRIPT\" \"$@\""));
    assert!(wrapped.contains("export -f cargo"));
    assert!(wrapped.ends_with("cargo test -p demo && echo done"));
}

#[test]
fn cargo_routing_is_limited_to_the_arterm_repository() {
...
fn make_agent_ctx(signal: arterm_agent_runtime::InterruptSignal) -> ToolContext {
    ToolContext {
        session_id: "test-session".to_string(),
        message_id: "test-msg".to_string(),
...
async fn test_reload_persistable_bash_continues_in_background() {
    let tool = BashTool::new();
    let signal = arterm_agent_runtime::InterruptSignal::new();
...
fn test_parse_progress_marker_handles_percent_payloads() {
    let progress = parse_progress_marker(
        r#"ARTERM_PROGRESS {"percent":25,"message":"Downloading dependencies"}"#,
    )
...
fn test_parse_heuristic_progress_handles_phase_output() {
    let progress = parse_heuristic_progress("Compiling arterm v0.10.2")
        .expect("heuristic parser should not fail")
...
async fn test_background_command_progress_marker_updates_status_and_stays_out_of_output() {
    ...
    let result = tool
            .execute(
                json!({
                    "command": "printf '%s\n' 'ARTERM_PROGRESS {\"current\":3,\"total\":10,\"unit\":\"steps\",\"message\":\"Building\"}'; sleep 0.1; echo done",
    ...
    assert!(
        !output.contains("ARTERM_PROGRESS"),
...
fn test_bash_tool_schema_advertises_background_progress_guidance() {
    ...
    assert!(
        command_description.contains("ARTERM_SCRATCH_DIR"),
        "command description should keep the scratch-dir guidance"
    );
    assert!(
        background_description.contains("ARTERM_PROGRESS"),
...
//
// The unit-level policy is covered in arterm-command-risk. These tests pin the
