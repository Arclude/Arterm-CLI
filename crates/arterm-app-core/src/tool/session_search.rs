...
struct SearchInput {
    ...
    /// Restrict Arterm sessions by saved/bookmarked flag.
    #[serde(default)]
    saved: Option<bool>,
    /// Restrict Arterm sessions by debug flag.
    #[serde(default)]
    debug: Option<bool>,
    /// Restrict Arterm sessions by canary flag.
    #[serde(default)]
    canary: Option<bool>,
    /// Restrict source: arterm, claude, codex, pi, opencode, cursor, or all.
    #[serde(default)]
    ...
    /// Scan every available Arterm session instead of the recent indexed subset.
    #[serde(default)]
...
/// Covers the arterm store plus the external stores (claude/codex/pi/cursor).
pub fn spawn_recent_index_warmup() {
    tokio::task::spawn_blocking(|| {
        let start = std::time::Instant::now();
        remove_legacy_index();
        let empty_query = QueryProfile::new("__arterm_index_warmup__");

        let arterm_count = (|| -> Result<usize> {
            let sessions_dir = storage::arterm_dir()?.join("sessions");
            let collection = collect_session_files(&sessions_dir, DEFAULT_MAX_SCAN_SESSIONS)?;
            if collection.files.is_empty() {
                return Ok(0);
            }
            let _ = arterm_index_candidates(&collection.files, &empty_query)?;
            Ok(collection.files.len())
        })()
        .unwrap_or_else(|err| {
            crate::logging::info(&format!("arterm session index warmup skipped: {err}"));
            0
        });
        ...
        crate::logging::info(&format!(
            "Session search index warmup completed for {arterm_count} arterm + {external_count} external session(s) in {}ms",
    ...
    fn parameters_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                ...
                "saved": {
                    "type": "boolean",
                    "description": "Restrict Arterm sessions by saved/bookmarked flag."
                },
                "debug": {
                    "type": "boolean",
                    "description": "Restrict Arterm sessions by debug/test flag."
                },
                "canary": {
                    "type": "boolean",
                    "description": "Restrict Arterm sessions by canary flag."
                },
                "source": {
                    "type": "string",
                    "enum": ["all", "arterm", "claude", "codex", "pi", "opencode", "cursor"],
...
fn normalize_source_filter(raw: Option<&str>) -> std::result::Result<Option<String>, String> {
    ...
    match normalized.as_str() {
        "all" => Ok(None),
        "arterm" | "claude" | "claude-code" | "codex" | "pi" | "opencode" | "cursor" => {
            Ok(Some(normalized.replace("claude-code", "claude")))
        }
        _ => Err(format!(
            "source must be one of all, arterm, claude, codex, pi, opencode, or cursor; received {source}."
        )),
    }
}
...
fn search_sessions_blocking(
    ...
    if source_matches_filter("arterm", options) {
        ...
        if !files.is_empty() {
            ...
            report.scanned_arterm_sessions = files.len();
            ...
            if !files.is_empty() {
                ...
                let mut candidates = if options.exhaustive {
                    ...
                    match arterm_index_candidates(&files, query) {
                ...
                report.candidate_arterm_sessions = candidates.len();
                if using_index {
...
fn index_dir() -> Result<PathBuf> {
    Ok(storage::arterm_dir()?.join("cache"))
}
...
/// Build/update the incremental arterm session index and return the candidate
/// subset of `files` that plausibly match `query`.
fn arterm_index_candidates(
    files: &[SessionFileCandidate],
    query: &QueryProfile,
) -> Result<Vec<SessionFileCandidate>> {
    let index_path = index_dir()?.join("session_search_arterm_index_v2.bin");
...
fn collect_external_jsonl_source(
    ...
    // Like the arterm path, cap how many candidate files get fully parsed.
...
fn claude_index_candidates(
    sessions: &[arterm_import_core::ClaudeCodeSessionInfo],
    query: &QueryProfile,
) -> Vec<arterm_import_core::ClaudeCodeSessionInfo> {
    let index = index_dir()
                ...
                for session in chunk {
                    ...
                    records.push(ExternalSessionRecord {
                        ...
                        short_name: Some(format!(
                            "claude {}",
                            arterm_core::util::truncate_str(&session.session_id, 8)
                        )),
                        title: Some(title),
...
fn external_text_matches_query(text: &str, query: &QueryProfile) -> bool {
    arterm_session_types::normalized_session_search_text_matches(&text.to_lowercase(), query)
}
...
fn append_session_results(
    ...
    if !arterm_session_matches_filters(session, options) {
        return;
    }
    ...
    for (message_index, msg) in session.messages.iter().enumerate() {
        ...
        results.push(SearchResult {
            ...
            matched_terms: match_score.matched_terms,
            exact_match: match_score.exact_match,
            context: build_arterm_context(&session.messages, message_index, options),
        });
    }
}
...
fn arterm_session_matches_filters(session: &Session, options: &SearchOptions) -> bool {
    if !source_matches_filter("arterm", options) {
        return false;
    }
    if !provider_matches(session.provider_key.as_deref(), "arterm", options) {
        return false;
    }
