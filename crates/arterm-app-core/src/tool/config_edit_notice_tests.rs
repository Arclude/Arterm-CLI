...
/// Point the process at a temp arterm home and return it with a restore guard.
fn temp_arterm_home() -> (tempfile::TempDir, Option<std::ffi::OsString>) {
...
fn restore_arterm_home(prev: Option<std::ffi::OsString>) {
...
fn writing_the_active_config_reports_what_changed() {
    ...
    let (_dir, prev) = temp_arterm_home();
    ...
    restore_arterm_home(prev);
}

#[test]
fn writing_an_unrelated_file_reports_nothing() {
    let _guard = crate::storage::lock_test_env();
    let (dir, prev) = temp_arterm_home();
