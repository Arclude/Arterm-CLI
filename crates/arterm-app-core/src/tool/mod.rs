...
pub(crate) fn accept_large_output_schema_property_for_test() -> Value {
    arterm_tool_core::accept_large_output_schema_property()
}
...
fn accepts_large_output(input: &Value) -> bool {
    ...
    match input.get(arterm_tool_core::ACCEPT_LARGE_OUTPUT_KEY) {
...
impl Registry {
    ...
    fn base_tools(skills: &Arc<RwLock<SkillRegistry>>) -> HashMap<String, Arc<dyn Tool>> {
        ...
        let base = BASE.get_or_init(|| {
            ...
            Self::insert_tool_timed(
                ...
                &mut timings,
                "arterm_docs",
                arterm_docs::ArtermDocsTool::new,
            );
    ...
    /// correctly.
    ///
    /// The canonical mapping lives in `arterm-tool-types::resolve_tool_name` so
    ...
    pub(crate) fn resolve_tool_name(name: &str) -> &str {
        arterm_tool_types::resolve_tool_name(name)
    }
    ...
    /// (`.mcp.json`, `.arterm/mcp.json`, `.claude/mcp.json`) against
