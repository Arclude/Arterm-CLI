...
async fn registry_execute_pre_tool_hook_blocks_and_allows() {
    ...
    crate::env::set_var("ARTERM_HOOK_PRE_TOOL", policy.to_string_lossy().to_string());
    // arterm-base is compiled without cfg(test) here, so the config cache only
    ...
    match prev {
        Some(value) => crate::env::set_var("ARTERM_HOOK_PRE_TOOL", value),
...
/// the gate continuation messages in arterm-base::todo).
#[tokio::test]
...
async fn gemini_build_tools_from_registry_definitions_omits_const_keywords() {
    // Moved from arterm-base/src/provider/gemini_tests.rs: this is the one test
...
async fn test_every_tool_advertises_the_large_output_escape_hatch() {
    ...
    for def in &defs {
        let flag = &def.input_schema["properties"][arterm_tool_core::ACCEPT_LARGE_OUTPUT_KEY];
        ...
        if let Some(required) = def.input_schema["required"].as_array() {
            assert!(
                !required
                    .iter()
                    .any(|v| v.as_str() == Some(arterm_tool_core::ACCEPT_LARGE_OUTPUT_KEY)),
...
async fn tool_schemas_are_sendable_to_every_provider_dialect() {
    ...
    for def in &defs {
        for error in arterm_schema_dialect::untyped_properties(&def.input_schema) {
            failures.push(format!("tool `{}` {error}", def.name));
        }
    }
    for spec in arterm_schema_dialect::registry::ALL {
        for def in &defs {
            let normalized = arterm_schema_dialect::dialect::apply(&def.input_schema, spec);
            for error in
                arterm_schema_dialect::must_not_contain_unsupported_constructs(&normalized, spec)
            {
...
fn the_dialect_sweep_catches_the_issue_754_schema() {
    ...
    let unnormalized = arterm_schema_dialect::must_not_contain_unsupported_constructs(
        &hostile,
        &arterm_schema_dialect::registry::GEMINI,
    );
    assert!(
        unnormalized
            .iter()
    ...
    let normalized =
        arterm_schema_dialect::dialect::apply(&hostile, &arterm_schema_dialect::registry::GEMINI);
    assert!(
        arterm_schema_dialect::must_not_contain_unsupported_constructs(
            &normalized,
            &arterm_schema_dialect::registry::GEMINI,
        )
        .is_empty(),
...
/// Failing strict eligibility closed for #711/#713 must not quietly cost arterm's
...
async fn only_the_known_open_world_tools_are_ineligible_for_openai_strict_mode() {
    ...
    for def in &defs {
        let compatible =
            arterm_provider_core::openai_schema::openai_compatible_schema(&def.input_schema);
        if !arterm_provider_core::openai_schema::schema_supports_strict(&compatible) {
