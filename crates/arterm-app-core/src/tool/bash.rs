...
const PROGRESS_MARKER_PREFIX: &str = "ARTERM_PROGRESS ";
const CHECKPOINT_MARKER_PREFIX: &str = "ARTERM_CHECKPOINT ";
const BACKGROUND_PROGRESS_GUIDANCE: &str = "For long-running background commands, prefer scripts or commands that periodically print progress updates. Best format: print lines starting with `ARTERM_PROGRESS ` followed by JSON like {\"percent\":42,\"message\":\"Running\"} or {\"current\":120,\"total\":1000,\"unit\":\"batches\",\"message\":\"Epoch 2/5\",\"eta_seconds\":30}. Supported JSON fields are `percent`, `message`, `current`, `total`, `unit`, `eta_seconds`, and optional `kind`=`indeterminate` or `kind`=`checkpoint`. For milestone-style wakeups, print `ARTERM_CHECKPOINT {\"message\":\"Unit tests passed\"}`. Generic fallback output that can be parsed includes `42%`, `3/10 tests`, `3 of 10 steps`, `1.5/3.0 GiB`, or phase lines like `Compiling ...`, `Downloading ...`, `Running ...`, and `Building ...`. If you are writing the script yourself, add these progress/checkpoint lines explicitly. Put large temporary files, worktrees, and virtual environments under `$ARTERM_SCRATCH_DIR`, not `/tmp`, because `/tmp` may be RAM-backed.";
...
fn wrap_repo_cargo_commands(command: &str, working_dir: Option<&Path>) -> Option<String> {
    ...
    Some(format!(
        r#"export ARTERM_DEV_CARGO_SCRIPT={wrapper}
cargo() {{
  if [[ "${{ARTERM_IN_DEV_CARGO:-0}}" == "1" ]]; then
    command cargo "$@"
  else
    ARTERM_IN_DEV_CARGO=1 "$ARTERM_DEV_CARGO_SCRIPT" "$@"
  fi
}}
export -f cargo
{command}"#,
...
async fn handle_background_output_line(
    ...
    match parse_heuristic_progress(raw_line) {
        ...
        Ok(None) => {}
        Err(err) => {
            let warning = format!("[arterm warning] failed to parse background progress: {err}\n");
...
fn tool_scratch_dir() -> Option<std::path::PathBuf> {
    let dir = std::env::var_os("ARTERM_SCRATCH_DIR")
...
fn configure_tool_scratch(command: &mut TokioCommand) {
    if let Some(dir) = tool_scratch_dir() {
        command.env("TMPDIR", &dir).env("ARTERM_SCRATCH_DIR", dir);
    }
}
...
fn build_detached_shell_wrapper(command: &str) -> StdCommand {
    ...
    cmd.arg("-lc")
        ...
        .env("ARTERM_RELOAD_DETACH_COMMAND", command);
    if let Some(dir) = tool_scratch_dir() {
...
mod utf8_truncation_tests {
    ...
    async fn build_shell_command_uses_disk_backed_scratch_directory() {
        let expected = super::tool_scratch_dir().expect("arterm scratch directory");
        let output = build_shell_command("printf '%s\\n%s\\n' \"$TMPDIR\" \"$ARTERM_SCRATCH_DIR\"")
            .output()
            .await
...
impl Tool for BashTool {
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        if crate::browser::is_browser_command(&params.command) {
            ...
            // Start/attach a browser session for this arterm session.
