...
impl SelfDevTool {
    ...
    /// building arterm from source: the rust toolchain, git, and a local repo
    ...
    pub(super) async fn do_setup(&self, ctx: &ToolContext) -> Result<ToolOutput> {
        ...
        // Rust toolchain: cargo + rustc are required to build arterm.
        ...
        if repo_dir.is_none() {
            ...
            if SelfDevTool::is_test_session() {
                clone_note = Some("Test mode: skipped cloning the arterm source.".to_string());
            } else if git_available {
                match Self::clone_selfdev_source() {
                    Ok(path) => {
                        clone_note = Some(format!("Cloned arterm source into {}.", path.display()));
                        repo_dir = Some(path);
                    }
                    Err(err) => {
                        clone_note =
                            Some(format!("Could not clone arterm source automatically: {err}",));
                    }
                }
            }
        }
        ...
        match &repo_dir {
            ...
            None => {
                let target = Self::selfdev_clone_dir()
                    ...
                    .unwrap_or_else(|_| "~/.arterm/source/arterm".to_string());
                checks.push(SetupCheck::missing(
                    "repository",
                    "no local arterm checkout found",
                    format!(
                        "Clone the source manually: `git clone {} {}`.",
                        super::ARTERM_REPO_URL,
                        target
                    ),
                ));
            }
        ...
        if let Some(repo) = repo_dir.as_deref() {
            match build::find_dev_binary(repo) {
                ...
                None => checks.push(SetupCheck::missing(
                    ...
                    "Build it once with `arterm self-dev --build`, or inside a \
        ...
        if !failing.is_empty() {
            ...
            output.push_str(
                "\nAll prerequisites satisfied. Use `selfdev enter` to start working on arterm.\n",
            );
        }
    ...
    /// `selfdev find-config`: report the key arterm paths (config file, home,
    ...
    pub(super) async fn do_find_config(&self, ctx: &ToolContext) -> Result<ToolOutput> {
        let arterm_home = storage::arterm_dir().ok();
        let config_path = arterm_home.as_ref().map(|home| home.join("config.toml"));
        ...
        let mut output = String::from("## arterm config & paths\n\n");
        output.push_str(&format!(
            "**Config file:** {}\n",
        ...
        output.push_str(&format!(
            "**arterm home:** {}\n",
            format_path(arterm_home.as_deref())
        ));
        output.push_str(&format!(
            "**Logs dir:** {}\n",
        ...
        let metadata = json!({
            ...
            "arterm_home": arterm_home.as_ref().map(|p| p.display().to_string()),
    ...
    pub(super) async fn do_reload_to_newer_build(&self, _ctx: &ToolContext) -> Result<ToolOutput> {
        ...
        if !server::server_has_newer_binary() {
            return Ok(ToolOutput::new(
                "Already running the newest installed arterm build; no reload needed.",
            ));
        }

        let hash = arterm_build_meta::git_hash().to_string();
    ...
    fn selfdev_clone_dir() -> Result<std::path::PathBuf> {
        Ok(storage::arterm_dir()?.join("source").join("arterm"))
    }

    /// Clone the arterm source into the default self-dev source directory.
    fn clone_selfdev_source() -> Result<std::path::PathBuf> {
        let repo_dir = Self::selfdev_clone_dir()?;
        if repo_dir.exists() {
            if build::is_arterm_repo(&repo_dir) {
                return Ok(repo_dir);
            }
            anyhow::bail!(
                "{} exists but is not a arterm repository; move it aside and retry",
                repo_dir.display()
            );
        }
        ...
        if !build::is_arterm_repo(&repo_dir) {
            anyhow::bail!(
                "cloned source at {} is not a valid arterm repository",
                repo_dir.display()
            );
        }
        Ok(repo_dir)
