...
pub fn enter_selfdev_session(
    ...
    let repo_dir = SelfDevTool::resolve_repo_dir(working_dir).ok_or_else(|| {
        anyhow::anyhow!("Could not find the arterm repository to enter self-dev mode")
    })?;
    ...
    pub(super) async fn do_enter(
        ...
        if !launch.launched {
            let command_preview = launch
                .command_preview()
                .unwrap_or_else(|| format!("arterm --resume {} self-dev", launch.session_id));
            return Ok(ToolOutput::new(format!(
                "Created self-dev session {} but could not find a supported terminal to spawn automatically.\n\nRun manually:\n`{} --resume {} self-dev`",
                launch.session_id,
                launch.exe.as_ref().map(|exe| exe.display().to_string()).unwrap_or_else(|| "arterm".to_string()),
                launch.session_id
            ))
            .with_metadata(json!({
                "session_id": launch.session_id,
        ...
        let mut output = format!(
            ...
            launch
                ...
                .unwrap_or_else(|| "arterm".to_string()),
            launch.session_id
        );
