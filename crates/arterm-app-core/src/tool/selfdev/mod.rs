...
//! Self-development tool - manage canary builds when working on arterm itself
...
impl BuildRequest {
    ...
    fn requests_dir() -> Result<PathBuf> {
        let dir = storage::arterm_dir()?.join("selfdev-build-requests");
    ...
    fn terminal_history_limit() -> usize {
        std::env::var("ARTERM_SELFDEV_REQUEST_HISTORY_LIMIT")
            .ok()
            ...
            json!({
                ...
                "properties": {
                    ...
                    "target": {
                        ...
                        "description": "Build target for action=build. auto chooses from changed paths; tui builds arterm; desktop2 builds arterm-desktop2; all builds every binary."
                    },
                    "command": {
                        "type": "string",
            ...
            json!({
                ...
                "properties": {
                    ...
                    "action": {
                        ...
                        "description": "Action. `enter` starts a self-dev session; `setup` installs prerequisites; `reload` restarts arterm."
                    },
                    "prompt": {
                        "type": "string",
...
impl Tool for SelfDevTool {
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        let result = match action.as_str() {
            ...
            // access only make sense once you are working on arterm itself.
            "build" => {
                self.do_build(
                    params.reason,
                    params.target,
...
impl SelfDevTool {
    fn is_test_session() -> bool {
        std::env::var("ARTERM_TEST_SESSION")
            .map(|value| {
    ...
    fn reload_timeout_secs() -> u64 {
        std::env::var("ARTERM_SELFDEV_RELOAD_TIMEOUT_SECS")
            .ok()
    ...
    fn build_reload_wait_secs() -> u64 {
        std::env::var("ARTERM_SELFDEV_BUILD_WAIT_SECS")
            .ok()
            .and_then(|raw| raw.trim().parse::<u64>().ok())
            .filter(|secs| *secs > 0)
            .unwrap_or(1800)
    ...
    fn resolve_repo_dir(working_dir: Option<&std::path::Path>) -> Option<std::path::PathBuf> {
        if let Some(dir) = working_dir {
            for ancestor in dir.ancestors() {
                if build::is_arterm_repo(ancestor) {
    ...
    fn launch_binary() -> Result<std::path::PathBuf> {
        build::client_update_candidate(true)
            ...
            .ok_or_else(|| anyhow::anyhow!("Could not resolve arterm executable to launch"))
    }
    ...
    fn build_lock_path(worktree_scope: &str) -> Result<PathBuf> {
        let dir = storage::arterm_dir()?.join("selfdev-build-locks");
        storage::ensure_dir(&dir)?;
