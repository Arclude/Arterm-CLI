...
impl RunPlanReporter {
    ...
    /// Record an explicit checkpoint (a ARTERM_CHECKPOINT-style milestone) on
...
/// The `arterm login` invocation most likely to fix a credential wave for
...
fn credential_login_fix_hint(provider: Option<&str>) -> String {
    ...
    format!("`arterm login --provider {target}`")
}
...
fn plan_status_budget_line(summary: &PlanGraphStatus, deep_cap: usize) -> Option<String> {
    ...
    let budget_label = if budget == usize::MAX {
        format!("{} (member cap)", arterm_swarm_core::MAX_SWARM_MEMBERS)
    } else {
        budget.to_string()
    };
    ...
    let effective_budget = if budget == usize::MAX {
        arterm_swarm_core::MAX_SWARM_MEMBERS
    } else {
        budget
    };
...
fn format_swarm_model_list(
    ...
    model_routes: &[arterm_provider_core::ModelRoute],
) -> String {
...
impl CommunicateTool {
    pub fn new() -> Self {
        ...
        let description = if swarm_prompt.is_empty() {
            ...
            format!(
                "{BASE_DESCRIPTION}\n\nSwarm prompt (user-tunable via ~/.arterm/swarm-prompt.md):\n{swarm_prompt}"
            )
        };
        Self { description }
    }
