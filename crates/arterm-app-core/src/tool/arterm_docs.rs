...
include!(concat!(env!("OUT_DIR"), "/arterm_docs.rs"));

const DEFAULT_LIMIT: usize = 5;
const MAX_LIMIT: usize = 10;
const MAX_SECTION_CHARS: usize = 4_000;

pub struct ArtermDocsTool;
...
impl Tool for ArtermDocsTool {
    ...
    async fn execute(&self, input: Value, _ctx: ToolContext) -> Result<ToolOutput> {
        let params: ArtermDocsInput = serde_json::from_value(input)?;
        ...
        Ok(ToolOutput::new(output).with_title(format!("arterm docs {}", params.action)))
    }
}

fn list_docs() -> String {
    let mut output = format!(
        "Bundled Arterm documentation ({} files):\n",
        ARTERM_DOCS.len()
    );
    for (path, body) in ARTERM_DOCS {
        let title = body
            .lines()
...
fn read_doc(path: &str) -> Result<String> {
    ...
    Ok(format!(
        "Source: `{path}` (bundled with this Arterm build)\n\n{body}"
    ))
}
...
fn terms(query: &str) -> Vec<String> {
    let stop: HashSet<&str> = [
        "a", "an", "and", "about", "does", "for", "how", "i", "in", "is", "arterm", "of", "on",
        "the", "to", "what", "with",
    ]
    .into_iter()
    .collect();
...
fn sections() -> Vec<Section<'static>> {
    ...
    for (path, document) in ARTERM_DOCS {
...
mod tests {
    ...
    fn search_finds_relevant_version_matched_documentation() {
        ...
        assert!(output.contains("bundled with this Arterm build"));
    }

    #[test]
    fn exact_document_can_be_read() {
        let output = read_doc("docs/README.md").unwrap();
        assert!(output.contains("# arterm Docs"));
    }
}
