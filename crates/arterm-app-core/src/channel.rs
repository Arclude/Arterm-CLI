...
impl MessageChannel for JadeRelayChannel {
    ...
    fn is_reply_enabled(&self) -> bool {
        ...
        // Arterm session directly. Keep this channel for outbound notifications
    ...
    async fn reply_loop(&self, runner: AmbientRunnerHandle) {
        ...
        let device_id = format!("arterm-{}", host);
        logging::info(&format!(
...
mod tests {
    ...
    ///   cargo test -p arterm-app-core relay_live -- --ignored --nocapture
    #[tokio::test]
    ...
    async fn test_relay_live_roundtrip() {
        ...
        ch.heartbeat("arterm-test-device").await;
