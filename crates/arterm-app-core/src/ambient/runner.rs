...
impl AmbientRunnerHandle {
    ...
    pub async fn log_json(&self) -> String {
        let transcripts_dir = match crate::storage::arterm_dir() {
    ...
    async fn run_cycle_visible(
        ...
        let arterm_bin =
            std::env::current_exe().unwrap_or_else(|_| std::path::PathBuf::from("arterm"));

        // Spawn kitty with `arterm ambient run-visible`
        logging::info("Ambient visible: spawning kitty with arterm TUI");
        let child = std::process::Command::new("kitty")
            .args([
                "--title",
                "🤖 arterm ambient cycle",
                "-e",
                &arterm_bin.to_string_lossy(),
                "ambient",
                "run-visible",
            ])
