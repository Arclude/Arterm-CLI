...
use arterm_notify_email::{
    ReplyAction, SendEmailRequest, build_permission_email_html, poll_imap_once, send_email,
};
pub use arterm_notify_email::{extract_permission_id, parse_permission_reply};
...
impl NotificationDispatcher {
    ...
    pub fn dispatch_permission_request(&self, action: &str, description: &str, request_id: &str) {
        let title = format!("arterm: permission needed ({})", action);
        let safe_body = "An ambient action needs your approval. Open arterm to review.".to_string();
        let detailed_body = format!(
            "Action: {}\n{}\n\nRequest ID: {}\nReview in arterm to approve or deny.",
...
#[cfg(target_os = "macos")]
const MACOS_NOTIFICATION_BROKER_APP_NAME: &str = "Arterm Notifications.app";
#[cfg(target_os = "macos")]
const MACOS_NOTIFICATION_BROKER_EXECUTABLE: &str = "arterm-notification-broker";
...
fn macos_notification_broker_app_path() -> Option<std::path::PathBuf> {
    if let Some(path) = std::env::var_os("ARTERM_MACOS_NOTIFICATION_BROKER_APP") {
...
pub fn macos_notification_inbox_dir() -> Option<std::path::PathBuf> {
    if let Some(path) = std::env::var_os("ARTERM_MACOS_NOTIFICATION_INBOX") {
        return Some(path.into());
    }
    dirs::home_dir().map(|home| {
        home.join(".arterm")
...
fn reap_notification_child(mut child: std::process::Child) {
    let _ = std::thread::Builder::new()
        .name("arterm-notification-child".to_string())
        .spawn(move || {
            let _ = child.wait();
        });
}
...
fn format_cycle_body_safe(transcript: &AmbientTranscript) -> String {
    ...
    lines.push("Check arterm for full details.".to_string());
    lines.join("\n")
}
...
fn format_cycle_body_detailed(transcript: &AmbientTranscript) -> String {
    ...
    if transcript.pending_permissions > 0 {
        lines.push(String::new());
        lines.push(format!(
            "**⚠ {} permission request(s) pending** — review in arterm",
            transcript.pending_permissions
        ));
    }
...
mod tests {
    ...
    fn test_format_cycle_body_safe() {
        ...
        assert!(body.contains("Check arterm for full details"));
    ...
    fn test_format_cycle_body_with_pending_permissions() {
        ...
        assert!(safe.contains("Check arterm for full details"));
    ...
    fn macos_envelope_roundtrip_preserves_origin_metadata() {
        let envelope = MacosNotificationEnvelope {
            schema_version: MACOS_NOTIFICATION_SCHEMA_VERSION,
            notification_id: "arterm-turn-test".to_string(),
            title: "arterm · done".to_string(),
