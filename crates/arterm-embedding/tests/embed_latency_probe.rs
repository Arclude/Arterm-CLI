...
//!
//! Run with:
//!   cargo test -p arterm-embedding --test embed_latency_probe -- --ignored --nocapture
//!
//! Requires the MiniLM model to be installed (~/.arterm/models/all-MiniLM-L6-v2).
...
fn model_dir() -> Option<PathBuf> {
    ...
    let dir = home
        .join(".arterm")
        .join("models")
        .join(arterm_embedding::MODEL_NAME);
...
fn embed_latency_probe() {
    ...
    let embedder = arterm_embedding::Embedder::load_from_dir(&dir).expect("load model");
