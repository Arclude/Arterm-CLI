...
//! key), moved out of `arterm-base` so provider edits compile only this crate
...
pub use arterm_provider_gemini::{
...
impl GeminiProvider {
    fn persisted_catalog_path() -> Result<std::path::PathBuf> {
        Ok(arterm_base::storage::app_config_dir()?.join("gemini_models_cache.json"))
    }
    ...
    pub fn new() -> Self {
        let model = std::env::var("ARTERM_GEMINI_MODEL").unwrap_or_else(|_| DEFAULT_MODEL.into());
        let provider = Self {
    ...
    /// cloudcode-pa tier. Set `ARTERM_GEMINI_FORCE_OAUTH=1` to pin OAuth even when
    /// a key is present.
    fn auth_mode() -> GeminiAuthMode {
        let force_oauth = std::env::var("ARTERM_GEMINI_FORCE_OAUTH")
            .map(|value| {
    ...
    ///
    /// arterm advertises every tool on every request, so one construct the
    ...
    /// `arterm-schema-dialect` remembers it so later requests never send it.
    ///
    ...
    async fn retry_after_schema_rejection(
        ...
        let dialect = &arterm_schema_dialect::registry::GEMINI;
        match arterm_schema_dialect::recover_from_error(error, dialect) {
            ...
            arterm_schema_dialect::RecoveryAction::Unrecoverable { hint } => {
                arterm_base::logging::warn(&format!("Gemini tool-schema rejection: {hint}"));
                None
            }
            arterm_schema_dialect::RecoveryAction::RetryWithoutConstruct { description } => {
                arterm_base::logging::warn(&format!("Gemini {description}"));
                Some(
...
impl Provider for GeminiProvider {
    ...
    fn set_model(&self, model: &str) -> Result<()> {
        ...
        let trimmed = arterm_provider_core::strip_own_model_prefix(model, "gemini:");
        if trimmed.is_empty() {
...
fn gemini_http_client() -> reqwest::Client {
    reqwest::Client::builder()
        ...
        .build()
        .unwrap_or_else(|_| arterm_provider_core::shared_http_client())
}
...
fn is_transient_gemini_transport_error(err: &reqwest::Error) -> bool {
    ...
    err.is_connect()
        || err.is_timeout()
        || arterm_provider_core::is_transient_transport_error(&err.to_string())
}
