//! Tracking of active session process IDs under `~/.arterm/active_pids`.
//!
...
//! lives in the storage crate because it only needs [`arterm_dir`] and is a
...
use crate::arterm_dir;
use std::path::PathBuf;

/// Directory holding one file per active session ID (`~/.arterm/active_pids`).
pub fn active_pids_dir() -> Option<PathBuf> {
    arterm_dir().ok().map(|d| d.join("active_pids"))
}
...
pub fn streaming_pids_dir() -> Option<std::path::PathBuf> {
    arterm_dir().ok().map(|d| d.join("streaming_pids"))
}
...
pub fn internal_pids_dir() -> Option<std::path::PathBuf> {
    arterm_dir().ok().map(|d| d.join("internal_pids"))
}
...
/// List active session IDs currently tracked in `~/.arterm/active_pids`.
...
/// Live snapshot of how many arterm sessions are running, and how many of those
/// are actively streaming a model response right now. Used by the menu bar
/// indicator (`arterm menubar`) and any other presence UI.
...
mod tests {
    ...
    /// Serialize tests that mutate `ARTERM_HOME`.
    ...
    fn session_counts_counts_live_and_streaming_only() {
        ...
        arterm_core::env::remove_var("ARTERM_HOME");
    }
