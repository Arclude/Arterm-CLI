use arterm_provider_gemini::CodeAssistGenerateResponse;
...
pub fn antigravity_dialect(model: &str) -> &'static arterm_schema_dialect::DialectSpec {
    if model_is_gemini(model) {
        &arterm_schema_dialect::registry::GEMINI
    } else if model_is_claude(model) {
        &arterm_schema_dialect::registry::ANTIGRAVITY_CLAUDE
    } else {
        &arterm_schema_dialect::registry::ANTIGRAVITY_BRIDGE
    }
}
...
/// and `arterm-schema-dialect` for why the subsets are allow-lists.
pub fn antigravity_compatible_schema(schema: &Value, model: &str) -> Value {
    arterm_schema_dialect::normalize(schema, antigravity_dialect(model))
}
