...
fn available_models_display_seeds_from_persisted_catalog() {
    ...
    arterm_base::env::set_var("ARTERM_HOME", temp.path());

    let path = arterm_base::provider::antigravity::persisted_catalog_path().expect("catalog path");
    ...
    if let Some(previous) = previous {
        arterm_base::env::set_var("ARTERM_HOME", previous);
    } else {
        arterm_base::env::remove_var("ARTERM_HOME");
    }
}
...
fn resolve_model_for_request_default_prefers_gemini_catalog_model() {
    ...
    *provider.fetched_catalog.write().expect("catalog lock") = vec![
    ...
    // Cloud Code backend. Claude on this backend rejects arterm's tool schemas.
    assert_eq!(
        provider.resolve_model_for_request("default"),
        "gemini-3-flash"
    );
...
fn antigravity_gemini_uses_the_same_dialect_as_the_native_gemini_provider() {
    ...
    assert_eq!(
        out,
        arterm_provider_gemini::gemini_compatible_schema(&schema),
...
fn pseudo_tool_call_turn_only_matches_the_recovery_marker_as_a_call() {
    ...
    assert!(arterm_provider_antigravity::is_pseudo_tool_call_turn(
