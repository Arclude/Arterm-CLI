...
//! Gemini/Claude/gpt-oss upstreams), moved out of `arterm-base` so provider
...
impl AntigravityProvider {
    fn load_persisted_catalog() -> Option<PersistedCatalog> {
        arterm_base::provider::antigravity::load_persisted_catalog()
    }

    fn persist_catalog(snapshot: &CatalogSnapshot) {
        arterm_base::provider::antigravity::persist_catalog(snapshot);
    }
    ...
    pub fn new() -> Self {
        let model =
            std::env::var("ARTERM_ANTIGRAVITY_MODEL").unwrap_or_else(|_| DEFAULT_MODEL.into());
    ...
    /// by `arterm login --provider antigravity`; there is no API-key path. This
    ...
    fn resolve_model_for_request(&self, model: &str) -> String {
        ...
        // Claude models on the Cloud Code backend currently reject arterm's tool
    ...
    async fn fetch_available_models(&self) -> Result<CatalogSnapshot> {
        arterm_base::provider::antigravity::fetch_catalog_snapshot(&self.client).await
    }

    /// Recover from a tool-schema rejection by learning what the backend
    /// refused and re-sending the turn without it.
    ///
    /// Because arterm advertises every tool on every request, one construct the
    ...
    /// #446). `arterm-schema-dialect` instead parses the construct out of the
    ...
    async fn retry_after_schema_rejection(
        ...
        signature_policy: arterm_provider_gemini::SignaturePolicy,
    ) -> Option<Result<CodeAssistGenerateResponse>> {
        let resolved = self.resolve_model_for_request(model);
        let dialect = arterm_provider_antigravity::antigravity_dialect(&resolved);
        match arterm_schema_dialect::recover_from_error(error, dialect) {
            arterm_schema_dialect::RecoveryAction::NotSchemaRelated => None,
            arterm_schema_dialect::RecoveryAction::Unrecoverable { hint } => {
                arterm_base::logging::warn(&format!("Antigravity tool-schema rejection: {hint}"));
                None
            }
            arterm_schema_dialect::RecoveryAction::RetryWithoutConstruct { description } => {
                arterm_base::logging::warn(&format!("Antigravity {description}"));
                Some(
    ...
    async fn generate_content(
        ...
        let mut tools = arterm_provider_gemini::build_tools(tools);
        ...
        // construct. Gemini-native accepts everything arterm emits, so Gemini
        ...
        let request = CodeAssistGenerateRequest {
            ...
            request: VertexGenerateContentRequest {
                contents: arterm_provider_gemini::build_contents_with_signature_policy(
                    messages,
                    signature_policy,
                ),
...
impl Provider for AntigravityProvider {
    async fn complete(
        ...
        let (tx, rx) = mpsc::channel::<Result<arterm_message_types::StreamEvent>>(100);
        ...
        tokio::spawn(async move {
                ...
                .await
            {
                ...
                Err(err) => {
                        ...
                        .await
                    {
                    ...
                    } else if !arterm_provider_gemini::is_missing_thought_signature_error(
                        &err.to_string(),
                    ) {
                        ...
                        signature_policy =
                            arterm_provider_gemini::SignaturePolicy::DowngradeToolCallsToText;
                        match provider
                            .generate_content(
                                &model,
                                &messages,
            ...
            if arterm_provider_antigravity::is_pseudo_tool_call_turn(&response) {
            ...
            if let Some(content) = candidate.content {
                ...
                // calls through the standard ToolUseStart/End path so arterm
                ...
                for part in content.parts {
                    ...
                    if let Some(function_call) = part.function_call {
                        ...
                        let call_id = arterm_message_types::sanitize_tool_id(&raw_call_id);
                        let _ = tx
    ...
    fn set_model(&self, model: &str) -> Result<()> {
        ...
        // resumed turn 404.
        let trimmed = arterm_provider_core::strip_own_model_prefix(model, "antigravity:");
        if trimmed.is_empty() {
    ...
    fn model_routes(&self) -> Vec<arterm_provider_core::ModelRoute> {
        let catalog = self.fetched_catalog();
        if !catalog.is_empty() {
            return catalog
                .into_iter()
                .map(|model| arterm_provider_core::ModelRoute {
