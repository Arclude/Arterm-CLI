...
///
/// Disable entirely with `ARTERM_WATCHDOG=0`. Tune with
/// `ARTERM_WATCHDOG_STALL_SECS` and `ARTERM_WATCHDOG_HEARTBEAT_SECS`.
pub fn start() {
    if std::env::var("ARTERM_WATCHDOG").is_ok_and(|value| value == "0" || value == "off") {
        return;
    }
    ...
    let stall = Duration::from_secs(env_secs("ARTERM_WATCHDOG_STALL_SECS", DEFAULT_STALL_SECS));
    let heartbeat = Duration::from_secs(env_secs(
        "ARTERM_WATCHDOG_HEARTBEAT_SECS",
        DEFAULT_HEARTBEAT_SECS,
    ));

    std::thread::Builder::new()
        .name("arterm-watchdog".to_string())
