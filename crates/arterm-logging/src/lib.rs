//! Logging infrastructure for arterm
//!
//! Logs to ~/.arterm/logs/ with automatic rotation
//!
...
impl LogLevel {
    ...
    fn is_enabled(self) -> bool {
        !matches!(self, Self::Debug) || std::env::var("ARTERM_TRACE").is_ok()
    }
}
...
fn log_dir() -> Option<PathBuf> {
    arterm_storage::logs_dir().ok()
}

impl Logger {
    fn new() -> Option<Self> {
        let log_dir = log_dir()?;
        arterm_storage::ensure_dir(&log_dir).ok()?;

        // Use date-based log file
        let date = Local::now().format("%Y-%m-%d");
        let path = log_dir.join(format!("arterm-{}.log", date));
    ...
    fn write(&mut self, level: &str, message: &str) {
        ...
        if let Err(err) = self.file.write_all(line.as_bytes()) {
            eprintln!("arterm logger write failed: {err}");
            return;
        }
        if let Err(err) = self.file.flush() {
            eprintln!("arterm logger flush failed: {err}");
        }
    }
}
...
pub fn init() {
    ...
    // `arterm-*.log` files, so it is safe to run unconditionally.
    ...
    if !CLEANUP_STARTED.swap(true, Ordering::SeqCst) {
        std::thread::Builder::new()
            .name("arterm-log-cleanup".to_string())
...
/// Log a debug message (only if ARTERM_TRACE is set)
#[expect(
    clippy::collapsible_if,
    reason = "Debug logging keeps env gating and logger access explicit"
)]
pub fn debug(message: &str) {
    if std::env::var("ARTERM_TRACE").is_ok() {
        if let Ok(mut guard) = LOGGER.lock() {
            if let Some(logger) = guard.as_mut() {
...
fn structured_json_enabled() -> bool {
    matches!(
        std::env::var("ARTERM_LOG_JSON").as_deref(),
...
pub fn log_path() -> Option<PathBuf> {
    let log_dir = log_dir()?;
    let date = Local::now().format("%Y-%m-%d");
    Some(log_dir.join(format!("arterm-{}.log", date)))
}

/// Remove daily `arterm-*.log` / `arterm-desktop-*.log` files older than 7 days.
///
...
fn cleanup_old_logs_in(log_dir: &std::path::Path, now: chrono::DateTime<Local>) {
    ...
    for entry in entries.flatten() {
        ...
        let is_arterm_log = (name.starts_with("arterm-") || name.starts_with("arterm-desktop-"))
            && name.ends_with(".log");
        if !is_arterm_log {
            continue;
        }
            ...
            && let Ok(modified) = metadata.modified()
        {
            ...
            {
                eprintln!("arterm logger cleanup failed: {err}");
            }
        }
    }
}
...
fn truncate(s: &str, max_len: usize) -> String {
    if s.len() > max_len {
        format!("{}...", arterm_core::util::truncate_str(s, max_len))
    } else {
        s.to_string()
    }
}
...
mod tests {
    ...
    #[test]
    fn cleanup_removes_only_old_arterm_logs() {
        use std::time::{Duration, SystemTime};

        let dir = std::env::temp_dir().join(format!(
            "arterm-log-cleanup-test-{}-{}",
            std::process::id(),
            SystemTime::now()
        ...
        let old_log = write("arterm-2000-01-01.log", true);
        let old_desktop = write("arterm-desktop-2000-01-01.log", true);
        // Recent log file that SHOULD survive.
        let new_log = write("arterm-2099-01-01.log", false);
        ...
        assert!(!old_log.exists(), "old arterm log should be deleted");
        assert!(!old_desktop.exists(), "old desktop log should be deleted");
        assert!(new_log.exists(), "recent arterm log must survive");
