...
fn test_openai_supports_codex_models() {
    ...
    arterm_base::auth::codex::set_active_account_override(Some(
        "openai-supports-codex-models".to_string(),
    ));
    arterm_base::provider::populate_account_models(vec![
    ...
    arterm_base::auth::codex::set_active_account_override(None);
}
...
fn test_openai_switching_models_include_dynamic_catalog_entries() {
    ...
    arterm_base::auth::codex::set_active_account_override(Some("switching-test".to_string()));
    arterm_base::provider::populate_account_models(vec![
        "gpt-5.4".to_string(),
...
fn test_chatgpt_web_model_bypasses_live_api_catalog() {
    let _guard = arterm_base::storage::lock_test_env();
    arterm_base::auth::codex::set_active_account_override(Some("web-model-test".to_string()));
    arterm_base::provider::populate_account_models(vec!["gpt-5.6-sol".to_string()]);
    let _model = EnvVarGuard::set("ARTERM_OPENAI_MODEL", CHATGPT_WEB_MODEL);
...
fn test_chatgpt_browser_only_runtime_rejects_api_models_and_uses_local_compaction() {
    ...
    assert!(provider.uses_arterm_compaction());
...
fn test_chatgpt_web_model_environment_override_is_trimmed() {
    let _guard = arterm_base::storage::lock_test_env();
    let _model = EnvVarGuard::set("ARTERM_OPENAI_MODEL", "  gpt-5.6-pro[web]  ");
    let provider = OpenAIProvider::new(CodexCredentials {
        access_token: "test".to_string(),
        refresh_token: String::new(),
        id_token: None,
...
async fn test_set_model_clears_persistent_ws_state() {
    ...
    arterm_base::provider::populate_account_models(vec!["gpt-5.3-codex".to_string()]);
...
async fn test_switching_to_https_clears_persistent_ws_state() {
    // Serialize with the tests that set ARTERM_OPENAI_MODEL via EnvVarGuard:
...
fn responses_url_honors_api_base_override_in_api_key_mode() {
    let _guard = arterm_base::storage::lock_test_env();
    let _b = EnvVarGuard::remove("ARTERM_OPENAI_API_BASE");
    ...
    let _override = EnvVarGuard::set("ARTERM_OPENAI_API_BASE", "http://127.0.0.1:8317/v1/");
    assert_eq!(
        OpenAIProvider::responses_url(&api_key_creds),
...
fn resolve_api_base_precedence_and_validation() {
    let _guard = arterm_base::storage::lock_test_env();
    let _a = EnvVarGuard::remove("ARTERM_OPENAI_API_BASE");
