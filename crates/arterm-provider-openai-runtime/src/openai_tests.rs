...
impl LiveOpenAITestEnv {
    fn new() -> Result<Option<Self>> {
        let lock = arterm_base::storage::lock_test_env();
        let Some(source_auth) = real_codex_auth_path() else {
            return Ok(None);
        };

        let temp = tempfile::Builder::new()
            .prefix("arterm-openai-live-")
            .tempdir()?;
        ...
        let arterm_home = EnvVarGuard::set_path("ARTERM_HOME", temp.path());
        let transport = EnvVarGuard::set("ARTERM_OPENAI_TRANSPORT", "https");

        Ok(Some(Self {
            _lock: lock,
            _arterm_home: arterm_home,
...
async fn live_openai_catalog() -> Result<Option<arterm_base::provider::OpenAIModelCatalog>> {
    let Some(_env) = LiveOpenAITestEnv::new()? else {
        return Ok(None);
    };
    let creds = arterm_base::auth::codex::load_credentials()?;
    ...
    Ok(Some(
        arterm_base::provider::fetch_openai_model_catalog(&token).await?,
    ))
}
...
fn openai_credential_mode_runtime_provider_identity_round_trips() {
    ...
    arterm_base::env::set_var("ARTERM_RUNTIME_PROVIDER", "openai");
    assert_eq!(
    ...
    arterm_base::env::set_var("ARTERM_RUNTIME_PROVIDER", "openai-api");
    assert_eq!(
        OpenAICredentialMode::from_runtime_env(arterm_provider_core::DualAuthProvider::OpenAI),
