...
//! persistent WebSocket), moved out of `arterm-base` so provider edits compile
...
//! [`OpenAIProvider`] with `arterm_base::provider::external` at startup.
//!
//! Model-catalog/account-availability state stays in `arterm_base::provider`
...
const DEFAULT_MODEL: &str = arterm_provider_core::DEFAULT_OPENAI_MODEL;
const ORIGINATOR: &str = "codex_cli_rs";

pub(crate) const CHATGPT_WEB_MODEL: &str = arterm_provider_core::CHATGPT_WEB_MODEL;
...
/// `ARTERM_OPENAI_WS_IDLE_RECONNECT_SECS` (0 disables the idle reconnect entirely,
...
static WEBSOCKET_PERSISTENT_IDLE_RECONNECT_SECS: LazyLock<Option<u64>> = LazyLock::new(|| {
    match std::env::var("ARTERM_OPENAI_WS_IDLE_RECONNECT_SECS") {
        Ok(raw) => match raw.trim().parse::<u64>() {
            Ok(0) => {
                arterm_base::logging::info(
                    "OpenAI persistent WS idle reconnect disabled (ARTERM_OPENAI_WS_IDLE_RECONNECT_SECS=0); relying on healthcheck + max-age",
                );
                None
            }
            Ok(secs) => {
                arterm_base::logging::info(&format!(
                    "OpenAI persistent WS idle reconnect threshold set to {}s (ARTERM_OPENAI_WS_IDLE_RECONNECT_SECS)",
                    secs
                ));
                Some(secs)
            }
            Err(_) => {
                arterm_base::logging::info(&format!(
                    "Warning: invalid ARTERM_OPENAI_WS_IDLE_RECONNECT_SECS '{}'; using default {}s",
        ...
        match raw.trim().to_ascii_lowercase().as_str() {
            ...
            other => {
                arterm_base::logging::warn(&format!(
                    "Unknown ARTERM_OPENAI_TRANSPORT '{}'; using auto. Use: auto, websocket, or https.",
                    other
                ));
                Self::Auto
            }
...
/// Shared dual-auth credential pin (see `arterm_provider_core::CredentialMode`).
/// The OpenAI-specific alias is kept so existing call sites read naturally.
pub(crate) use arterm_provider_core::CredentialMode as OpenAICredentialMode;
...
pub(crate) fn load_credentials_for_mode(mode: OpenAICredentialMode) -> Result<CodexCredentials> {
    match mode {
        OpenAICredentialMode::Auto => arterm_base::auth::codex::load_credentials(),
        OpenAICredentialMode::OAuth => arterm_base::auth::codex::load_oauth_credentials(),
        OpenAICredentialMode::ApiKey => arterm_base::auth::codex::load_api_key_credentials(),
    }
}
...
fn log_openai_stream_lifecycle(
    level: arterm_base::logging::LogLevel,
    phase: &str,
    ...
    arterm_base::logging::event(level, "PROVIDER_STREAM_LIFECYCLE", owned);
}
...
async fn emit_connection_phase(
    ...
    phase: arterm_message_types::ConnectionPhase,
) {
        ...
        loop {
                ...
                >= Duration::from_secs(WEBSOCKET_PERSISTENT_MAX_AGE_SECS)
            {
                log_openai_stream_lifecycle(
                    arterm_base::logging::LogLevel::Info,
            ...
            match ensure_persistent_ws_is_healthy(state, false).await {
                ...
                Err(error) => {
                    ...
                    log_openai_stream_lifecycle(
                        arterm_base::logging::LogLevel::Warn,
                        "persistent_state_reset",
                        vec![
                            ("model", model.clone()),
...
impl OpenAIProvider {
    pub(crate) fn supports_extended_prompt_cache_retention(model_id: &str) -> bool {
        arterm_base::provider::openai::supports_extended_prompt_cache_retention(model_id)
    }
    ...
    fn new_inner(credentials: CodexCredentials, browser_only: bool) -> Self {
        let credential_mode = if browser_only {
            ...
            OpenAICredentialMode::from_runtime_env(arterm_provider_core::DualAuthProvider::OpenAI)
        };
        ...
        let mut model = if browser_only {
            ...
            std::env::var("ARTERM_OPENAI_MODEL")
                .unwrap_or_else(|_| DEFAULT_MODEL.to_string())
                .trim()
                .to_string()
        };
        if !is_chatgpt_web_model(&model)
            && !arterm_base::provider::known_openai_model_ids()
                .iter()
        ...
        let prompt_cache_key = std::env::var("ARTERM_OPENAI_PROMPT_CACHE_KEY")
            .ok()
            .map(|v| v.trim().to_string())
            .filter(|v| !v.is_empty());
        let prompt_cache_retention = std::env::var("ARTERM_OPENAI_PROMPT_CACHE_RETENTION")
            .ok()
            .map(|v| v.trim().to_string())
            .filter(|v| !v.is_empty());
        let prompt_cache_retention = match prompt_cache_retention.as_deref() {
            ...
            Some(other) => {
                arterm_base::logging::info(&format!(
                    "Warning: Unsupported ARTERM_OPENAI_PROMPT_CACHE_RETENTION '{}'; expected 'in_memory' or '24h'",
                    other
                ));
                None
            }
        ...
        let native_compaction_threshold_tokens = arterm_base::config::config()
            .provider
            .openai_native_compaction_threshold_tokens
            .max(1000);
        let model_reasoning_efforts =
            arterm_base::provider::cached_openai_reasoning_efforts().unwrap_or_default();
    ...
    pub(crate) fn set_credential_mode(&self, mode: OpenAICredentialMode) -> Result<()> {
        ...
        if let Some(route) = mode.auth_route(arterm_provider_core::DualAuthProvider::OpenAI) {
    ...
    fn reload_cached_reasoning_efforts(&self) {
        let cached = arterm_base::provider::cached_openai_reasoning_efforts().unwrap_or_default();
    ...
    async fn clear_persistent_ws(&self, reason: &str) {
        ...
        if persistent_ws.is_some() {
            arterm_base::logging::info(&format!("Clearing persistent OpenAI WS state: {}", reason));
        }
        *persistent_ws = None;
    }
    ...
    fn default_reasoning_effort_for_model(model: &str) -> Option<String> {
        let key = arterm_provider_core::model_id::canonical(model);
    ...
    fn revalidate_reasoning_effort(&self) {
        ...
        if arterm_base::prompt::is_swarm_effort(&current)
    ...
    fn api_reasoning_effort(&self, effort: Option<&str>) -> Option<String> {
        match effort {
            Some(e) if arterm_base::prompt::is_swarm_effort(e) => {
                let available = self.available_efforts();
                arterm_provider_core::OPENAI_SELECTABLE_EFFORTS
                    .iter()
                    .rev()
                    .find(|candidate| {
                        !arterm_base::prompt::is_swarm_effort(candidate)
    ...
    fn parse_max_output_tokens(raw: Option<&str>) -> Option<u32> {
        ...
        match raw.parse::<u32>() {
            ...
            Err(_) => {
                arterm_base::logging::warn(&format!(
                    "Invalid ARTERM_OPENAI_MAX_OUTPUT_TOKENS='{}'; using default {}",
    ...
    fn load_max_output_tokens() -> Option<u32> {
        let raw = std::env::var("ARTERM_OPENAI_MAX_OUTPUT_TOKENS").ok();
        let parsed = Self::parse_max_output_tokens(raw.as_deref());
        if raw.is_some() {
            match parsed {
                Some(value) => arterm_base::logging::info(&format!(
                    "OpenAI max_output_tokens configured to {}",
                    value
                )),
                None => arterm_base::logging::info(
                    "OpenAI max_output_tokens disabled (ARTERM_OPENAI_MAX_OUTPUT_TOKENS=0)",
                ),
            }
        }
        parsed
    ...
    /// `ARTERM_OPENAI_API_BASE`, `OPENAI_BASE_URL`, `OPENAI_API_BASE`.
    ///
    ...
    pub(crate) fn resolve_api_base() -> String {
        arterm_base::provider::openai::resolve_api_base()
    }
    ...
    async fn model_id(&self) -> String {
        ...
        let availability = arterm_base::provider::model_availability_for_account(&current);

        match availability.state {
            arterm_base::provider::AccountModelAvailabilityState::Unavailable => {
                ...
                if let Some(fallback) = arterm_base::provider::get_best_available_openai_model()
                    && fallback != current
                {
                    arterm_base::logging::info(&format!(
            ...
            arterm_base::provider::AccountModelAvailabilityState::Unknown => {
                if arterm_base::provider::should_refresh_openai_model_catalog()
                    && arterm_base::provider::begin_openai_model_catalog_refresh()
                {
                    let is_chatgpt_mode = {
                    ...
                    arterm_base::provider::refresh_openai_model_catalog_in_background(
                        token,
                        is_chatgpt_mode,
            ...
            arterm_base::provider::AccountModelAvailabilityState::Available => {}
        }
