...
fn catalog_error_is_auth_rejection(err: &anyhow::Error) -> bool {
    err.downcast_ref::<arterm_base::provider::ModelCatalogHttpStatus>()
...
impl Provider for OpenAIProvider {
    ...
    async fn complete(
        ...
        let reasoning_effort = self
            ...
            // No explicit user effort: fall back to the model's arterm-side
        ...
        let prompt_cache_key_hash = request
            ...
            .map(arterm_provider_core::fingerprint::stable_hash_json);
        ...
        let usage_snapshot = arterm_base::usage::get_openai_usage_sync();
        ...
        tokio::spawn(async move {
                ...
                for attempt in 0..MAX_RETRIES {
                    if attempt > 0 {
                        emit_connection_phase(
                            &tx,
                            arterm_message_types::ConnectionPhase::Retrying {
                                attempt: attempt + 1,
                                max: MAX_RETRIES,
                            },
                        )
                    ...
                    match result {
                        ...
                        Err(OpenAIStreamFailure::Other(error)) => {
                            ...
                            if is_retryable_error(&error_str) && attempt + 1 < MAX_RETRIES {
                                ...
                                next_retry_delay =
                                    arterm_provider_core::retry_after::retry_after_from_error(
                                        &error,
                                    );
                                last_error = Some(error);
                                continue;
                            ...
                            log_openai_stream_lifecycle(
                                arterm_base::logging::LogLevel::Error,
                                "attempt_failed",
                                vec![
                                    ("model", model_for_transport.clone()),
                                    ("attempt", (attempt + 1).to_string()),
                            ...
                            let error = match arterm_schema_dialect::learn_from_error(
                                &error.to_string(),
                                &arterm_schema_dialect::registry::OPENAI,
                            ) {
                                Some(explanation) => {
                                    arterm_base::logging::warn(&format!(
            ...
            if let Err(panic_payload) = result {
                ...
                arterm_base::logging::error(&format!(
    ...
    fn set_model(&self, model: &str) -> Result<()> {
        ...
        if self.is_browser_only() && !is_chatgpt_web_model(model) {
            anyhow::bail!(
                "OpenAI API credentials are not available for '{}'. The browser-only runtime can use '{}'; run `arterm login --provider openai` before selecting API models.",
                model,
                CHATGPT_WEB_MODEL,
            );
        }
        ...
        let availability = arterm_base::provider::model_availability_for_account(model);
        if !is_chatgpt_web_model(model)
            && availability.state
                == arterm_base::provider::AccountModelAvailabilityState::Unavailable
        {
            let detail =
                arterm_base::provider::format_account_model_availability_detail(&availability)
        ...
        if arterm_provider_core::is_openai_api_only_pro_model(model) {
        ...
        if let Ok(mut current) = self.model.try_write() {
            ...
            *current = model.to_string();
            arterm_base::provider::clear_model_unavailable_for_account(model);
            drop(current);
            if changed {
    ...
    fn available_models(&self) -> Vec<&'static str> {
        ...
        arterm_provider_core::ALL_OPENAI_MODELS.to_vec()
    }
    ...
    fn available_models_for_switching(&self) -> Vec<String> {
        ...
        let mut models =
            arterm_base::provider::cached_openai_model_ids().unwrap_or_else(|| vec![self.model()]);
        ...
        if arterm_base::provider::openai_platform_api_key_configured() {
            for pro in arterm_provider_core::OPENAI_API_ONLY_PRO_MODELS {
    ...
    async fn prefetch_models(&self) -> Result<()> {
        ...
        let account_label = arterm_base::auth::codex::active_account_label();
        ...
        let catalog = if is_chatgpt_mode {
            ...
            match arterm_base::provider::fetch_openai_model_catalog(&access_token).await {
                Ok(catalog) => catalog,
                ...
                Err(err) if catalog_error_is_auth_rejection(&err) => {
                    ...
                    arterm_base::provider::fetch_openai_model_catalog(&refreshed).await?
                }
                Err(err) => return Err(err),
            }
        } else {
            arterm_base::provider::fetch_openai_api_key_model_catalog(&access_token).await?
        };
        ...
        if current_credential_identity != credential_identity
            || arterm_base::auth::codex::active_account_label() != account_label
        {
            arterm_base::logging::info(
        ...
        self.revalidate_reasoning_effort();
        arterm_base::provider::persist_openai_model_catalog(&catalog);
        ...
        if !catalog.available_models.is_empty() {
            arterm_base::provider::populate_account_models(catalog.available_models);
        }
        Ok(())
    }
    ...
    fn set_reasoning_effort(&self, effort: &str) -> Result<()> {
        ...
        if !requested.is_empty()
            && arterm_provider_core::canonical_reasoning_effort(&requested).is_none()
            && !arterm_base::prompt::is_swarm_effort(&requested)
        {
            anyhow::bail!(
            ...
            && !arterm_base::prompt::is_swarm_effort(requested)
        {
            ...
            if !available.contains(&requested) {
                anyhow::bail!(
                    ...
                    available
                        .into_iter()
                        .filter(|effort| !arterm_base::prompt::is_swarm_effort(effort))
                        .collect::<Vec<_>>()
                        .join(", ")
                );
            }
    ...
    fn available_efforts(&self) -> Vec<&'static str> {
        let model = arterm_provider_core::model_id::canonical(&self.model());
        ...
        if let Some(advertised) = advertised {
            let mut efforts: Vec<&'static str> = advertised
                .iter()
                .filter_map(|effort| arterm_provider_core::canonical_reasoning_effort(effort))
                .collect();
    ...
    async fn native_compact(
    ...
    ) -> Result<arterm_provider_core::NativeCompactionResult> {
        ...
        if let Some(encrypted_content) = existing_openai_encrypted_content {
            if !arterm_base::provider::openai_request::openai_encrypted_content_is_sendable(
                encrypted_content,
            ) {
                anyhow::bail!(
                    "OpenAI native compaction payload is too large to replay ({} chars > safe limit {} chars)",
                    encrypted_content.len(),
                    arterm_base::provider::openai_request::OPENAI_ENCRYPTED_CONTENT_SAFE_MAX_CHARS,
                );
            }
        ...
        Ok(arterm_provider_core::NativeCompactionResult {
            summary_text: None,
    ...
    fn context_window(&self) -> usize {
        ...
        arterm_provider_core::context_limit_for_model_with_provider(&model, Some(self.name()))
            .unwrap_or(arterm_provider_core::DEFAULT_CONTEXT_LIMIT)
    }
