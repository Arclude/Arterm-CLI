...
//! recorded across their agent sessions (arterm + imported Claude/Codex/etc.).
//!
...
//! launch hotkeys (e.g. `Cmd+[`, `Cmd+]`) to "open arterm here". The ranking is a
...
pub struct RankOptions {
    ...
    /// the user's home directory, which is noise from launching arterm at `$HOME`.
...
/// arterm in. Produced by [`build_launch_hotkey_plan`] from a ranking, then
/// persisted to config so the mapping is baked once and does not move around.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PlannedHotkey {
    /// arterm-style chord string, e.g. `cmd+;` or `cmd+[`.
    pub chord: String,
    /// Absolute directory the hotkey opens arterm in.
    pub dir: String,
...
/// Scan a arterm sessions directory and extract one [`SessionLocation`] per
...
pub fn collect_arterm_session_locations(sessions_dir: &Path) -> Vec<SessionLocation> {
    use std::fs;
...
/// Compute the baked launch-hotkey plan from a arterm sessions directory.
///
...
pub fn plan_launch_hotkeys_from_sessions(
    ...
    let locations = collect_arterm_session_locations(sessions_dir);
...
mod tests {
    ...
    fn plan_assigns_top_repo_home_then_next_repos() {
        ...
        assert_eq!(plan[0].dir, "/u/jeremy/arterm");
    ...
    fn plan_skips_home_if_it_appears_in_ranking() {
        ...
        // Top repo slot is arterm (home filtered out), then home gets cmd+'.
        assert_eq!(plan[0].dir, "/u/jeremy/arterm");
        assert_eq!(plan[1].dir, "/u/jeremy");
    ...
    fn collect_reads_working_dirs_and_skips_junk() {
        ...
        write(
            "session_a.json",
            r#"{"working_dir":"/Users/jeremy/arterm-github","id":"a"}"#,
        );
        write(
            "session_b.json",
    ...
    fn collect_missing_dir_is_empty() {
        let locs = collect_arterm_session_locations(Path::new("/nonexistent/arterm/sessions"));
