...
///
/// e.g. `/run/user/1000/arterm.sock` -> `\\.\pipe\arterm`
/// e.g. `/run/user/1000/arterm/myserver.sock` -> `\\.\pipe\arterm-myserver`
fn path_to_pipe_name(path: &Path) -> String {
    let stem: String = path
        ...
        .take(32)
        .collect();
    let stem = if stem.is_empty() { "arterm" } else { &stem };
    let normalized = path
        .to_string_lossy()
        .replace('\\', "/")
        .to_ascii_lowercase();
...
impl Stream {
    ...
    pub fn pair() -> io::Result<(Self, Self)> {
        ...
        let pipe_name = format!(r"\\.\pipe\arterm-pair-{}-{}", std::process::id(), counter);
...
mod tests {
    ...
    fn pipe_name_is_stable_and_normalizes_case_and_separators() {
        let a = path_to_pipe_name(Path::new(r"C:\Temp\Arterm\server.sock"));
        let b = path_to_pipe_name(Path::new("c:/temp/arterm/server.sock"));
    ...
    fn pipe_name_falls_back_when_stem_is_empty() {
        ...
        assert!(
            name.starts_with(r"\\.\pipe\arterm-"),
            "unexpected pipe name: {}",
            name
        );
    }
