...
//! knowledge dies with the process. Writing it to `~/.arterm/schema-quirks.json`
//! makes the second request the fast path and means the fleet self-heals in
//! front of a provider change instead of waiting for a arterm release.
//!
...
fn store_path() -> Option<PathBuf> {
    ...
    // developer's real `~/.arterm/schema-quirks.json`, silently stripping that
    ...
    {
        if let Ok(explicit) = std::env::var("ARTERM_SCHEMA_QUIRKS_PATH") {
            return Some(PathBuf::from(explicit));
        }
        let home = if let Ok(arterm_home) = std::env::var("ARTERM_HOME") {
            PathBuf::from(arterm_home)
        } else {
            dirs::home_dir()?.join(".arterm")
        };
...
/// developer's real `~/.arterm/schema-quirks.json`.
#[cfg(any(test, feature = "test-support"))]
...
/// alternate `ARTERM_HOME`) never read each other's state.
