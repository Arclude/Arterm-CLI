...
/// Build OpenAI-compatible messages array from arterm's message format.
///
...
pub fn build_tools(tools: &[ToolDefinition]) -> Vec<Value> {
    tools
        ...
        .map(|t| {
            ...
            let parameters = arterm_schema_dialect::normalize(
                &t.input_schema,
                &arterm_schema_dialect::registry::OPENROUTER,
            );
            json!({
                "type": "function",
                "function": {
