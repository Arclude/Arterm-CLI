...
use arterm_base::auth::test_sandbox::AuthTestSandbox;
use arterm_base::protocol::{
...
impl AuthLifecycleSpec {
    pub(crate) fn cerebras_fixture(auth_path: AuthLifecycleAuthPath) -> Self {
        let mut spec = Self::openai_compatible_fixture(
            arterm_base::provider_catalog::CEREBRAS_PROFILE,
            auth_path,
        );
...
impl AuthLifecycleDriver {
    ...
    pub(crate) fn run_openai_compatible_fixture(
        ...
        let resolved =
            arterm_base::provider_catalog::resolve_openai_compatible_profile(spec.profile);
        ensure!(
        ...
        let auth = AuthChanged {
            provider: arterm_base::protocol::AuthProviderId::new(spec.provider_id),
        ...
        // (`handle_notify_auth_changed`): once the live catalog lands, arterm
    ...
    fn apply_credentials(
        ...
        match spec.auth_path {
            ...
            AuthLifecycleAuthPath::ProcessEnvPreseeded => {
                arterm_base::env::set_var(&resolved.api_key_env, &spec.api_key);
                arterm_base::auth::AuthStatus::invalidate_cache();
    ...
    fn user_visible_transcript(
        ...
        if spec.auth_path.shows_paste_prompt() {
            ...
            transcript.push(format!(
                "**{} API key saved.**\n\nStored at `{}`.\nFetching models now. Arterm will switch to an accessible model returned by the live catalog and show the catalog diff when discovery finishes.",
                spec.provider_label,
...
mod tests {
    ...
    fn is_native_routed_compat_profile(
        ...
        arterm_base::auth::lifecycle::normalized_auth_provider_id(Some(profile.id))
    ...
    struct LiveTestApiKey {
        secret: String,
        auth: arterm_base::live_tests::LiveVerificationAuth,
    }

    fn live_cerebras_api_key() -> Option<LiveTestApiKey> {
        std::env::var("ARTERM_AUTH_LIFECYCLE_CEREBRAS_API_KEY")
            .ok()
            .map(|value| value.trim().to_string())
            .filter(|value| !value.is_empty())
            .map(|secret| LiveTestApiKey {
                auth: arterm_base::live_tests::LiveVerificationAuth::from_secret(
                    "env:ARTERM_AUTH_LIFECYCLE_CEREBRAS_API_KEY",
                    Some("ARTERM_AUTH_LIFECYCLE_CEREBRAS_API_KEY"),
                    &secret,
                ),
                secret,
            })
    ...
    fn live_opencode_zen_api_key() -> Option<LiveTestApiKey> {
        if let Some(secret) = std::env::var("ARTERM_AUTH_LIFECYCLE_OPENCODE_API_KEY")
            .ok()
            .map(|value| value.trim().to_string())
            .filter(|value| !value.is_empty())
        {
            return Some(LiveTestApiKey {
                auth: arterm_base::live_tests::LiveVerificationAuth::from_secret(
                    "env:ARTERM_AUTH_LIFECYCLE_OPENCODE_API_KEY",
                    Some("ARTERM_AUTH_LIFECYCLE_OPENCODE_API_KEY"),
                    &secret,
                ),
                secret,
            });
        ...
        let resolved = arterm_base::provider_catalog::resolve_openai_compatible_profile(
            arterm_base::provider_catalog::OPENCODE_PROFILE,
        );
        arterm_base::provider_catalog::load_api_key_from_env_or_config(
            &resolved.api_key_env,
            &resolved.env_file,
        )
        .map(|secret| LiveTestApiKey {
    ...
    fn live_event<I, S>(
        ...
        model: Option<&str>,
        capabilities: I,
        result: arterm_base::live_tests::LiveVerificationResult,
    ) -> arterm_base::live_tests::LiveVerificationEvent
    where
        I: IntoIterator<Item = S>,
        S: Into<String>,
    {
        let resolved = arterm_base::provider_catalog::resolve_openai_compatible_profile(profile);
        let mut event = arterm_base::live_tests::LiveVerificationEvent::new(
            test_name,
            resolved.id,
    ...
    fn append_live_event(event: &arterm_base::live_tests::LiveVerificationEvent) {
        let event = event.clone().with_not_run_for_missing_expected_checkpoints(
            "checkpoint not exercised by this live test invocation",
        );
        let paths = arterm_base::live_tests::append_event(&event)
    ...
    fn cost_quota_safety_stage(
        spend_enabled: bool,
    ) -> arterm_base::live_tests::LiveVerificationStage {
        arterm_base::live_tests::LiveVerificationStage::passed(
            arterm_base::live_tests::checkpoints::COST_QUOTA_SAFETY,
        )
    ...
    fn covered_stage_names(
        stages: &[arterm_base::live_tests::LiveVerificationStage],
    ) -> Vec<String> {
        stages
            .iter()
            .filter(|stage| {
                stage.status != arterm_base::live_tests::LiveVerificationStageStatus::NotRun
            })
    ...
    fn openai_compatible_provider_matrix_preserves_identity_catalog_and_picker() {
        ...
        for profile in arterm_base::provider_catalog::openai_compatible_profiles()
            .iter()
            .copied()
    ...
    fn provider_switch_reauth_matrix_recovers_from_stale_previous_provider_state() {
        ...
        let profiles: Vec<_> = arterm_base::provider_catalog::openai_compatible_profiles()
            .iter()
            .copied()
            .filter(|profile| !is_native_routed_compat_profile(profile))
            .collect();
    ...
    async fn cerebras_live_opt_in_catalog_lifecycle_uses_isolated_sandbox() {
        if !env_truthy("ARTERM_AUTH_LIFECYCLE_LIVE") {
            eprintln!(
                "skipping live Cerebras auth lifecycle test; set ARTERM_AUTH_LIFECYCLE_LIVE=1 and ARTERM_AUTH_LIFECYCLE_CEREBRAS_API_KEY"
            );
            return;
        }
        let api_key = live_cerebras_api_key()
            .expect("ARTERM_AUTH_LIFECYCLE_LIVE=1 requires ARTERM_AUTH_LIFECYCLE_CEREBRAS_API_KEY");

        let spend_smoke = env_truthy("ARTERM_AUTH_LIFECYCLE_SMOKE");
        let stream_smoke = env_truthy("ARTERM_AUTH_LIFECYCLE_STREAM_SMOKE");
        let mut stages = vec![
            arterm_base::live_tests::LiveVerificationStage::passed(
                arterm_base::live_tests::checkpoints::AUTH_CREDENTIAL_LOADED,
            )
            .with_evidence(
                "auth_source",
        ...
        let models = match models_result {
            Ok(models) => models,
            Err(error) => {
                stages.push(arterm_base::live_tests::LiveVerificationStage::failed(
                    arterm_base::live_tests::checkpoints::MODEL_CATALOG_LIVE_ENDPOINT,
                    error.to_string(),
                ));
                ...
                let event = live_event(
                    ...
                    None,
                    capabilities,
                    arterm_base::live_tests::LiveVerificationResult::Failed,
                )
                .with_stages(stages);
        ...
        stages.push(
            ...
            .with_evidence(
                "models",
                arterm_base::live_tests::concise_model_sample(&models, 12),
            ),
        );
        let default_model = arterm_base::provider_catalog::CEREBRAS_PROFILE.default_model;
        ...
        stages.push(
            arterm_base::live_tests::LiveVerificationStage::passed(
                arterm_base::live_tests::checkpoints::AUTH_UX_KEY_ENTRY,
            )
        ...
        stages.push(
            arterm_base::live_tests::LiveVerificationStage::passed(
                arterm_base::live_tests::checkpoints::CREDENTIAL_PERSISTENCE,
            )
            .with_evidence(
                "credential_location",
        ...
        stages.push(
            arterm_base::live_tests::LiveVerificationStage::passed(
                arterm_base::live_tests::checkpoints::CATALOG_HOT_RELOAD_CURRENT_SESSION,
            )
        ...
        stages.push(
            arterm_base::live_tests::LiveVerificationStage::passed(
                arterm_base::live_tests::checkpoints::PICKER_LIVE_MODELS,
            )
        ...
        stages.push(
            arterm_base::live_tests::LiveVerificationStage::passed(
                arterm_base::live_tests::checkpoints::PICKER_FALLBACK_LABELING,
            )
        ...
        stages.push(
            arterm_base::live_tests::LiveVerificationStage::passed(
                arterm_base::live_tests::checkpoints::MODEL_SWITCH_ROUTE,
            )
            .with_evidence(
                "switch_request",
        ...
        if spend_smoke {
            ...
            .await
            {
                ...
                Err(error) => {
                    stages.push(arterm_base::live_tests::LiveVerificationStage::failed(
                        arterm_base::live_tests::checkpoints::NON_STREAMING_CHAT_COMPLETION,
                        error.to_string(),
                    ));
                    let capabilities = covered_stage_names(&stages);
                    let event = live_event(
        ...
        if stream_smoke {
            ...
            .await
            {
                ...
                Err(error) => {
                    stages.push(arterm_base::live_tests::LiveVerificationStage::failed(
                        arterm_base::live_tests::checkpoints::STREAMING_CHAT_COMPLETION,
                        error.to_string(),
                    ));
                    let capabilities = covered_stage_names(&stages);
                    let event = live_event(
        ...
        let event = live_event(
            ...
            Some(&selected),
            capabilities,
            arterm_base::live_tests::LiveVerificationResult::Passed,
        )
        .with_stages(stages);
        append_live_event(&event);
    }
    ...
    async fn opencode_zen_live_opt_in_tool_call_smoke() {
        if !env_truthy("ARTERM_OPENCODE_ZEN_LIVE_TOOL_TEST") {
            eprintln!(
                "skipping live OpenCode Zen tool-call smoke; set ARTERM_OPENCODE_ZEN_LIVE_TOOL_TEST=1 and provide OPENCODE_API_KEY"
            );
            return;
        }
        let api_key = live_opencode_zen_api_key().expect(
            "ARTERM_OPENCODE_ZEN_LIVE_TOOL_TEST=1 requires OPENCODE_API_KEY or ARTERM_AUTH_LIFECYCLE_OPENCODE_API_KEY",
        );
        let model = std::env::var("ARTERM_OPENCODE_ZEN_LIVE_TOOL_MODEL")
            .ok()
            .map(|value| value.trim().to_string())
            .filter(|value| !value.is_empty())
            .unwrap_or_else(|| "kimi-k2.6".to_string());

        let stream_smoke = env_truthy("ARTERM_OPENCODE_ZEN_LIVE_STREAM_TEST")
            || env_truthy("ARTERM_AUTH_LIFECYCLE_STREAM_SMOKE");
        let mut stages = vec![
            arterm_base::live_tests::LiveVerificationStage::passed(
                arterm_base::live_tests::checkpoints::AUTH_CREDENTIAL_LOADED,
            )
        ...
        stages.push(
            ...
            .with_evidence(
                "models",
                arterm_base::live_tests::concise_model_sample(&models, 16),
            )
            .with_evidence(
        ...
        .await
        {
            ...
            Err(error) => {
                stages.push(arterm_base::live_tests::LiveVerificationStage::failed(
                    arterm_base::live_tests::checkpoints::TOOL_CALL_PARSE,
                    error.to_string(),
                ));
                let capabilities = covered_stage_names(&stages);
                let event = live_event(
    ...
    async fn issue_driven_openai_compatible_live_target_smoke() {
        let Some(provider_id) = std::env::var("ARTERM_ISSUE_DRIVEN_LIVE_PROVIDER")
            .ok()
            .map(|value| value.trim().to_string())
            .filter(|value| !value.is_empty())
        else {
            eprintln!(
                "skipping issue-driven live provider test; set ARTERM_ISSUE_DRIVEN_LIVE_PROVIDER to an OpenAI-compatible profile id"
            );
            return;
        };
        let profile = arterm_base::provider_catalog::openai_compatible_profile_by_id(&provider_id)
        ...
        let api_key = live_openai_compatible_api_key(profile).unwrap_or_else(|| {
            panic!(
                "ARTERM_ISSUE_DRIVEN_LIVE_PROVIDER={} requires {} or {}",
                provider_id, resolved.api_key_env, resolved.env_file
            )
        });
        let requested_model = std::env::var("ARTERM_ISSUE_DRIVEN_LIVE_MODEL")
            .ok()
            .map(|value| value.trim().to_string())
            .filter(|value| !value.is_empty())
            .or_else(|| resolved.default_model.clone());
        let spend_smoke = env_truthy("ARTERM_ISSUE_DRIVEN_LIVE_COMPLETION_SMOKE");
        let stream_smoke = env_truthy("ARTERM_ISSUE_DRIVEN_LIVE_STREAM_SMOKE");
    ...
    fn fresh_start_sandbox_is_unconfigured_then_tui_key_lifecycle_configures_provider() {
        ...
        let provider = arterm_base::provider_catalog::resolve_login_provider(spec.provider_id)
        ...
        assert!(
            !arterm_base::provider_catalog::openai_compatible_profile_is_configured(spec.profile),
            "fresh sandbox should report the provider as unconfigured before setup"
        );
        arterm_base::auth::AuthStatus::invalidate_cache();
        assert_eq!(
            arterm_base::auth::AuthStatus::check_fast().state_for_provider(provider),
            arterm_base::auth::AuthState::NotConfigured
        );
        ...
        assert_eq!(
            arterm_base::auth::AuthStatus::check_fast().state_for_provider(provider),
            arterm_base::auth::AuthState::Available
        );
    }
}
