...
mod tests {
    ...
    fn summarize_update_error_is_always_one_short_line() {
        let inputs = [
            "Update failed: Update check failed: error sending request for url (https://api.github.com/repos/1jehuang/arterm/releases/latest)\n\nCaused by:\n    dns error: failed to lookup address information",
            "cargo build failed: error[E0308]: mismatched types\n  --> src/lib.rs:1:1",
            "Checksum mismatch for arterm-linux-x86_64.tar.gz: expected aaa, got bbb",
            "Failed to install /home/u/.arterm/builds/versions/0.1.0/arterm: Permission denied (os error 13)",
    ...
    fn summarize_update_error_maps_known_causes() {
        ...
        assert_eq!(
            summarize_update_error("Checksum mismatch for arterm: expected a, got b"),
            "download failed checksum verification"
        );
        assert_eq!(
            summarize_update_error("No asset found for platform: arterm-linux-x86_64"),
    ...
    fn asset_name_is_supported() {
        assert_ne!(get_asset_name(), "arterm-unknown");
    }
    ...
    fn sha256sums_accepts_standard_and_binary_lines() {
        let checksums = parse_sha256sums(
            "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa  arterm-linux-x86_64\n\
             bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb *arterm-macos-aarch64\n",
        )
        .unwrap();
        assert_eq!(
            checksums.get("arterm-linux-x86_64").map(String::as_str),
            Some("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
        );
        assert_eq!(
            checksums.get("arterm-macos-aarch64").map(String::as_str),
    ...
    fn checksum_verification_accepts_matching_digest() {
        ...
        let sums = format!("{}  arterm-linux-x86_64\n", digest);
        verify_asset_checksum_text(&sums, "arterm-linux-x86_64", bytes).unwrap();
    }

    #[test]
    fn checksum_verification_rejects_mismatch() {
        let sums = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa  arterm-linux-x86_64\n";
        let err = verify_asset_checksum_text(sums, "arterm-linux-x86_64", b"hello")
            .unwrap_err()
            .to_string();
    ...
    fn sha256sums_rejects_invalid_digest() {
        let err = parse_sha256sums("not-a-digest  arterm\n")
            .unwrap_err()
            .to_string();
