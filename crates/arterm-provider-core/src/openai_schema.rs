...
///
/// The subset is now an allow-list in `arterm-schema-dialect`, shared with every
...
pub fn openai_compatible_schema(schema: &Value) -> Value {
    arterm_schema_dialect::normalize(schema, &arterm_schema_dialect::registry::OPENAI)
}
...
pub fn schema_supports_strict(schema: &Value) -> bool {
    fn check_map(map: &serde_json::Map<String, Value>) -> bool {
        ...
        // same rule the registry-wide sweep already enforces on arterm's own
        // tools.
...
mod tests {
    ...
    /// constructs from a real MCP catalog that arterm marked `strict: true` and
