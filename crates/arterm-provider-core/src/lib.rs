...
pub use reasoning::{
    DEEPSEEK_SELECTABLE_EFFORTS, GROK_SELECTABLE_EFFORTS, OPENAI_SELECTABLE_EFFORTS,
...
pub trait Provider: Send + Sync {
    ...
    /// Returns true if arterm should use its own compaction for this provider.
    fn supports_compaction(&self) -> bool {
        false
    }

    /// Returns true if arterm should proactively run its own summary-based compaction.
...
impl CredentialMode {
    /// Resolve the runtime-env pin (ARTERM_*_AUTH / route aliases) for a
...
/// Native tool result to send back to provider bridges that delegate tool execution to arterm.
...
/// Canonical User-Agent for generic outbound Arterm HTTP requests.
pub const ARTERM_USER_AGENT: &str = concat!("arterm/", env!("CARGO_PKG_VERSION"));
    ...
    CLIENT
        .get_or_init(|| {
            reqwest::Client::builder()
                ...
                .build()
                .unwrap_or_else(|err| {
                    eprintln!("arterm: failed to build shared provider HTTP client: {err}");
                        ...
                        .build()
                    {
                        ...
                        Err(fallback_err) => {
                            eprintln!(
                                "arterm: failed to build fallback provider HTTP client: {fallback_err}"
                            );
                            reqwest::Client::new()
                        }
                    }
...
impl RuntimeKey {
    pub fn from_api_method(api_method: &ModelRouteApiMethod, _provider_label: &str) -> Self {
        match api_method {
            ModelRouteApiMethod::ArtermSubscription => Self::ArtermSubscription,
    ...
    pub fn stable_id(&self) -> String {
        match self {
            Self::ArtermSubscription => "arterm-subscription".to_string(),
...
impl RouteSelection {
    ...
    pub fn routed_model_spec(&self) -> String {
        ...
        match &self.runtime_key {
            RuntimeKey::ArtermSubscription => model.to_string(),
...
/// through. Mirrors `arterm_base::provider::openrouter_catalog_model_id` but
...
impl ModelRouteApiMethod {
    ...
    pub fn parse(value: &str) -> Self {
        ...
        match lower.as_str() {
            "arterm-subscription" => Self::ArtermSubscription,
    ...
    pub fn display_label(&self) -> String {
        match self {
            Self::ArtermSubscription => "subscription".to_string(),
...
mod tests {
    ...
    #[test]
    fn canonical_user_agent_identifies_arterm() {
        assert!(ARTERM_USER_AGENT.starts_with("arterm/"));
    }
