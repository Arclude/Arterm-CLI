...
//! of `arterm-base` so provider edits compile only this crate plus a binary
...
/// Native tools that arterm handles locally (not Claude Code built-ins)
...
impl ClaudeCliConfig {
    fn from_env() -> Self {
        let cli_path = std::env::var("ARTERM_CLAUDE_CLI_PATH")
            .ok()
            .filter(|value| !value.trim().is_empty())
            .unwrap_or_else(|| "claude".to_string());

        let mut model = std::env::var("ARTERM_CLAUDE_CLI_MODEL")
            .ok()
            .filter(|value| !value.trim().is_empty())
        ...
        let permission_mode = std::env::var("ARTERM_CLAUDE_CLI_PERMISSION_MODE")
            .ok()
            .filter(|value| !value.trim().is_empty())
            .or_else(|| {
                std::env::var("ARTERM_CLAUDE_SDK_PERMISSION_MODE")
                    .ok()
                    .filter(|value| !value.trim().is_empty())
            })
            .or_else(|| Some(DEFAULT_PERMISSION_MODE.to_string()));

        let include_partial_messages = std::env::var("ARTERM_CLAUDE_CLI_PARTIAL")
            .ok()
            .or_else(|| std::env::var("ARTERM_CLAUDE_SDK_PARTIAL").ok())
            .map(|value| {
...
impl Provider for ClaudeProvider {
        ...
        tokio::spawn(async move {
            ...
            for attempt in 0..MAX_RETRIES {
                if attempt > 0 {
                    ...
                    let base_delay = arterm_provider_core::attempt_tracker::retry_backoff_delay(
                        attempt,
                        RETRY_BASE_DELAY_MS,
                    );
    ...
    async fn prefetch_models(&self) -> Result<()> {
        ...
        match arterm_base::provider::fetch_anthropic_model_catalog_oauth(&access_token).await {
            Ok(catalog) => {
...
async fn run_claude_cli(
    ...
    tokio::spawn(async move {
        ...
        while let Ok(Some(line)) = reader.next_line().await {
            arterm_base::logging::debug(&format!("[claude-cli] {}", line));
        }
        drop(tx_stderr);
    });
