...
//! `arterm-tui`'s `swarm_plan_graph` emits) through the real mermaid pipeline
//! and report success/error. Useful when iterating on the renderer:
//!   cargo run -p arterm-tui-mermaid --features renderer --example plan_graph_probe
fn probe(name: &str, src: &str) -> bool {
    match arterm_tui_mermaid::render_mermaid_untracked(src, Some(100)) {
        arterm_tui_mermaid::RenderResult::Image { width, height, .. } => {
