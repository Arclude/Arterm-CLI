...
//! gallery the way it looks live in the arterm TUI, without touching any running
...
//!
//! Run with:
//!   cargo run --profile selfdev -p arterm-tui-render --example swarm_gallery_live
//!
//! Controls:
//!   q / Esc      quit
//!   + / -        more / fewer agents
...
use arterm_tui_render::swarm_gallery::{GalleryMember, humanize_age, render_gallery};
    ...
    let pool: Vec<(&str, Option<&'static str>, Vec<&str>)> = vec![
        (
            ...
            vec![
                ...
                "Reading crates/arterm-app-core/src/auth.rs",
        ...
        (
            ...
            None,
            vec![
                "Editing crates/arterm-base/src/config.rs",
...
fn draw(f: &mut Frame, workers: &[MockWorker], max_pct: usize, paused: bool) {
    ...
    let help = vec![
        ...
        Line::from(""),
        Line::from(vec![Span::styled(
            "  The band above is the same renderer the arterm TUI shows above your chat",
