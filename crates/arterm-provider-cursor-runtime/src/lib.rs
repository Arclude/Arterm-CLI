...
//! `arterm_base::provider::cursor` because base's model-routing logic needs it
//! without a runtime.
...
fn runtime_cursor_api_key() -> Option<String> {
    arterm_base::auth::cursor::load_api_key().ok()
}
...
impl CursorCliProvider {
    fn persisted_catalog_path() -> Result<std::path::PathBuf> {
        Ok(arterm_base::storage::app_config_dir()?.join("cursor_models_cache.json"))
    }
    ...
    pub fn new() -> Self {
        let model = std::env::var("ARTERM_CURSOR_MODEL").unwrap_or_else(|_| DEFAULT_MODEL.into());
        let provider = Self {
...
impl Provider for CursorCliProvider {
    ...
    fn set_model(&self, model: &str) -> Result<()> {
        ...
        let trimmed = arterm_provider_core::strip_own_model_prefix(model, "cursor:");
        if trimmed.is_empty() {
