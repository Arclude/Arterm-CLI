...
/// `ARTERM_CURSOR_CLI_VERSION` if Cursor moves the floor.
const CLI_CLIENT_VERSION_DEFAULT: &str = "cli-2026.07.08-0c04a8a";

fn cli_client_version() -> String {
    std::env::var("ARTERM_CURSOR_CLI_VERSION")
        .ok()
...
fn agent_host_from_cursor_cli_config() -> Option<String> {
    let path = match arterm_base::storage::user_home_path(".cursor/cli-config.json") {
        Ok(path) => path,
        Err(err) => {
...
/// 1. `ARTERM_CURSOR_AGENT_HOST` / `CURSOR_AGENT_HOST` (explicit override)
...
fn agent_host() -> String {
    for var in ["ARTERM_CURSOR_AGENT_HOST", "CURSOR_AGENT_HOST"] {
...
mod tests {
    ...
    fn agent_host_normalizes_env_overrides() {
        ...
        // shared lock in arterm-base.
        let _guard = arterm_base::storage::lock_test_env();
        let prev = std::env::var_os("ARTERM_CURSOR_AGENT_HOST");

        arterm_base::env::set_var(
            "ARTERM_CURSOR_AGENT_HOST",
            "https://agentn.us.api5.cursor.sh/agent.v1.AgentService/Run",
        );
        assert_eq!(agent_host(), "agentn.us.api5.cursor.sh");

        arterm_base::env::set_var("ARTERM_CURSOR_AGENT_HOST", "agentn.eu.api5.cursor.sh:443");
        assert_eq!(agent_host(), "agentn.eu.api5.cursor.sh");

        // A blank override must not win; it falls through to the next source.
        arterm_base::env::set_var("ARTERM_CURSOR_AGENT_HOST", "   ");
        assert_ne!(agent_host(), "   ");

        match prev {
            Some(value) => arterm_base::env::set_var("ARTERM_CURSOR_AGENT_HOST", value),
            None => arterm_base::env::remove_var("ARTERM_CURSOR_AGENT_HOST"),
        }
    }
