...
fn available_models_display_seeds_from_persisted_catalog() {
    ...
    if let Some(prev_home) = prev_home {
        arterm_base::env::set_var("ARTERM_HOME", prev_home);
    } else {
...
fn runtime_cursor_api_key_reads_env() {
    ...
    arterm_base::env::set_var("CURSOR_API_KEY", "cursor-env-test");

    assert_eq!(runtime_cursor_api_key().as_deref(), Some("cursor-env-test"));

    if let Some(previous) = previous {
        arterm_base::env::set_var("CURSOR_API_KEY", previous);
    } else {
        arterm_base::env::remove_var("CURSOR_API_KEY");
    }
}
