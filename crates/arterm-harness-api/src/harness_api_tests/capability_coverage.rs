...
fn reference_client_requests() -> BTreeSet<String> {
    ...
    for dir in ["../arterm-tui/src", "../arterm-desktop2/src"] {
...
///
/// Run with `cargo test -p arterm-harness-api -- --nocapture capability_report`.
#[test]
...
/// The SDK mirrors arterm's external credential locations by hand, so a new one
/// added in Rust must also be inherited by launched instances.
///
/// `ARTERM_HOME` redirects every `user_home_path` lookup into
/// `$ARTERM_HOME/external/`, which is what makes an instance private. The cost
...
fn the_sdk_inherits_every_external_credential_location() {
    ...
    for path in external_credential_paths() {
        ...
        assert!(
            covered,
            "arterm reads credentials from `~/{path}`, but sdk/typescript/src/launch.ts \
...
/// Home-relative paths arterm reads other tools' credentials from.
///
/// Read out of the source so the list cannot drift: `user_home_path` is the
/// single funnel for "a file in the user's home that ARTERM_HOME sandboxes".
...
fn the_ledger_only_names_real_daemon_requests() {
    ...
    assert!(
        ...
        "the capability ledger names requests that do not exist in \
         arterm-protocol's Request enum: {unknown:?}. These are usually local \
...
fn protocol_request_variants() -> Option<BTreeSet<String>> {
    let path =
        std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../arterm-protocol/src/wire.rs");
