...
//! bridge and in `arterm-desktop2`, and the two copies disagreed: the bridge
//! resolved `$XDG_RUNTIME_DIR` while the desktop always looked in
//! `~/.arterm`. The result was a desktop app that could never connect even
//! with a healthy bridge running. One definition, used by both sides, makes
//! that class of bug impossible.
//!
//! The rules match `arterm-storage::runtime_dir` so the API socket always lands
...
/// Path of the versioned harness API socket. `ARTERM_API_SOCKET` overrides it.
pub fn api_socket_path() -> PathBuf {
    if let Ok(custom) = std::env::var("ARTERM_API_SOCKET") {
        return PathBuf::from(custom);
    }
    runtime_dir().join("arterm-api.sock")
}

/// Path of the internal daemon socket the bridge translates onto.
/// `ARTERM_SOCKET` overrides it.
pub fn legacy_socket_path() -> PathBuf {
    if let Ok(custom) = std::env::var("ARTERM_SOCKET") {
        return PathBuf::from(custom);
    }
    runtime_dir().join("arterm.sock")
}
...
mod tests {
    ...
    fn the_api_socket_sits_beside_the_daemon_socket() {
        ...
        let api = runtime_dir().join("arterm-api.sock");
        let legacy = runtime_dir().join("arterm.sock");
    ...
    fn socket_names_are_stable() {
        assert_eq!(
            runtime_dir().join("arterm-api.sock").file_name().unwrap(),
            "arterm-api.sock"
        );
    }
