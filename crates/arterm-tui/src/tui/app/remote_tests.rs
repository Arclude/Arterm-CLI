...
fn create_test_app() -> crate::tui::app::App {
    ...
    // from its own ARTERM_HOME (scheduled reminders read as a notification).
...
/// Point ARTERM_HOME at a per-process temp dir when the environment does not
/// already pin one, so tests never read the developer's real `~/.arterm`
...
fn reload_handoff_inactive_without_flag_or_marker() {
    ...
    // `arterm.reload` left by a live self-dev reload on this machine cannot
...
fn remote_skill_invocation_with_prompt_sends_remote_turn() {
    ...
    let skill_dir = temp.path().join(".arterm/skills/remote-skill");
