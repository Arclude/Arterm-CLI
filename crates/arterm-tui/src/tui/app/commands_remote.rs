...
**`/remote`** - use arterm from any device

`/remote` opens your Arterm account, the entry point for Arterm Cloud access.
Cloud is being bundled with eligible Arterm subscriptions. The managed host
control plane is currently in early access.

- `/remote` or `/remote cloud` - activate or manage Arterm Cloud
...
fn activate_cloud(app: &mut App) {
    ...
    let account_line = if signed_in {
        match tier {
            ...
            None => "Signed in to your Arterm account.".to_string(),
        }
    } else {
    ...
    let open_line = if opened {
        "Opened your secure Arterm account page in the browser."
    } else {
        "Open your secure Arterm account page:"
    };
    app.push_display_message(DisplayMessage::system(format!(
        "**Arterm Cloud early access**\n\n{account_line}\n\n{open_line}\n\n{CLOUD_ACCOUNT_URL}\n\n\
    ...
    app.set_status_notice(if opened {
        "Arterm Cloud account opened"
    } else {
        "Arterm Cloud account link ready"
    });
}
