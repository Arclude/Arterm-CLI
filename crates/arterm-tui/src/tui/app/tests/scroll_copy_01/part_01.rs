...
fn create_error_copy_test_app() -> (App, ratatui::Terminal<ratatui::backend::TestBackend>) {
    ...
    app.display_messages = vec![
        ...
        DisplayMessage::error("permission denied while opening ~/.arterm/config.toml"),
    ];
    app.bump_display_messages_version();
    app.scroll_offset = 0;
    app.auto_scroll_paused = false;
...
/// much as arterm. On a loaded developer machine or a shared CI runner they fail
...
/// ones with `ARTERM_TEST_PERF_ASSERTIONS=1` on an idle machine (refs #592).
fn perf_assertions_enabled() -> bool {
    std::env::var("ARTERM_TEST_PERF_ASSERTIONS")
...
fn assert_perf_budget(within_budget: bool, message: impl FnOnce() -> String) {
    if perf_assertions_enabled() {
        ...
        eprintln!(
            "note: perf budget exceeded ({}); set ARTERM_TEST_PERF_ASSERTIONS=1 \
...
fn test_queued_file_activity_repaint_does_not_leave_trailing_digit_artifact() {
    ...
    app.pending_soft_interrupts = vec![
        "⚠️ File activity: /home/jeremy/arterm/src/lib.rs - amber previously read this file: read lines 1-9999"
            .to_string(),
    ];
    ...
    app.pending_soft_interrupts = vec![
        "⚠️ File activity: /home/jeremy/arterm/src/lib.rs - amber previously read this file: read lines 1-9"
            .to_string(),
    ];
...
fn test_notification_file_activity_repaint_does_not_leave_trailing_digit_artifact() {
    ...
    app.status_notice = Some((
        "File activity · /home/jeremy/arterm/src/lib.rs · read lines 1-9999".to_string(),
    ...
    app.status_notice = Some((
        "File activity · /home/jeremy/arterm/src/lib.rs · read lines 1-9".to_string(),
        std::time::Instant::now(),
    ));
    let second = render_and_snap(&app, &mut terminal);
