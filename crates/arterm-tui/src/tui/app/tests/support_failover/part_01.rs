...
fn cleanup_background_task_files(task_id: &str) {
    let task_dir = std::env::temp_dir().join("arterm-bg-tasks");
...
/// Point `ARTERM_HOME` at a per-process scratch directory if nothing set one.
///
...
/// that scope their own `ARTERM_HOME` restore it by *removing* the variable, so
/// without serialization this function would observe the gap and repoint
/// `ARTERM_HOME` at the shared scratch home while that test was still running.
/// That is the race behind arterm-tui's intermittent failures in unrelated
/// ambient, header, and model-picker tests, which all read files under
/// `ARTERM_HOME` mid-assertion.
///
...
/// `with_temp_arterm_home` hold this same non-reentrant mutex for their whole
...
fn ensure_test_arterm_home_if_unset() {
    ...
    // own ARTERM_HOME under `lock_test_env`. The mutex is not reentrant and
    // several tests hold it while calling `create_test_app` (e.g. the
    // pinned-todo-band test), so a blocking `lock_test_env()` here would
    // self-deadlock whenever a preceding test removed ARTERM_HOME on drop.
...
fn clear_persisted_test_ui_state() {
    if let Ok(home) = crate::storage::arterm_dir() {
...
fn with_temp_arterm_home<T>(f: impl FnOnce() -> T) -> T {
...
/// Run `f` in a hermetic `ARTERM_HOME` with reasoning display pinned to
/// `current`.
///
...
fn create_arterm_repo_fixture() -> tempfile::TempDir {
