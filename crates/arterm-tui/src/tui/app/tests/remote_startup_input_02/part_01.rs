...
fn test_remote_hydrated_catalog_restores_missing_direct_bedrock_route() {
    with_temp_arterm_home(|| {
        let previous_enable = std::env::var_os("ARTERM_BEDROCK_ENABLE");
        crate::env::set_var("ARTERM_BEDROCK_ENABLE", "1");
        crate::auth::AuthStatus::invalidate_cache();
        ...
        match previous_enable {
            Some(value) => crate::env::set_var("ARTERM_BEDROCK_ENABLE", value),
            None => crate::env::remove_var("ARTERM_BEDROCK_ENABLE"),
        }
        crate::auth::AuthStatus::invalidate_cache();
...
#[test]
fn test_remote_arterm_subscription_catalog_is_not_augmented_with_local_auth_routes() {
    with_temp_arterm_home(|| {
        ...
        app.remote_provider_name =
            Some(crate::subscription_catalog::ARTERM_PROVIDER_DISPLAY_NAME.to_string());
        ...
        let expected = crate::subscription_catalog::curated_models()
            .iter()
            .filter(|model| {
                crate::subscription_catalog::ArtermTier::Plus.allows(model.min_tier)
            })
            .map(|model| model.id)
            .collect::<std::collections::BTreeSet<_>>();
        assert_eq!(app.remote_model_options.len(), expected.len());
        assert!(app.remote_model_options.iter().all(|route| {
            route.provider == crate::subscription_catalog::ARTERM_PROVIDER_DISPLAY_NAME
                && route.api_method == crate::subscription_catalog::ARTERM_ROUTE_API_METHOD
                && route.available
        }));
        assert_eq!(
            app.remote_model_options
...
#[test]
fn test_remote_mixed_catalog_keeps_arterm_subscription_separate_from_other_providers() {
    ...
    // `with_temp_arterm_home` takes the shared env lock and points auth at an
...
#[test]
fn test_remote_hydrated_catalog_adds_entitled_arterm_subscription_routes() {
    with_temp_arterm_home(|| {
        let previous_key = std::env::var_os(crate::subscription_catalog::ARTERM_API_KEY_ENV);
        let previous_tier = std::env::var_os(crate::subscription_catalog::ARTERM_TIER_ENV);
        crate::env::set_var(
            crate::subscription_catalog::ARTERM_API_KEY_ENV,
            "arterm_test_subscription_key",
        );
        crate::env::set_var(crate::subscription_catalog::ARTERM_TIER_ENV, "plus");
        ...
        match previous_key {
            Some(value) => {
                crate::env::set_var(crate::subscription_catalog::ARTERM_API_KEY_ENV, value)
            }
            None => crate::env::remove_var(crate::subscription_catalog::ARTERM_API_KEY_ENV),
        }
        match previous_tier {
            Some(value) => crate::env::set_var(crate::subscription_catalog::ARTERM_TIER_ENV, value),
            None => crate::env::remove_var(crate::subscription_catalog::ARTERM_TIER_ENV),
        }
        ...
        let expected = crate::subscription_catalog::curated_models()
            ...
            .map(|model| model.id)
            .collect::<std::collections::BTreeSet<_>>();
        assert_eq!(arterm_routes.len(), expected.len());
        assert_eq!(
            arterm_routes
                .iter()
                .map(|route| route.model.as_str())
...
#[test]
fn test_remote_non_arterm_catalog_repairs_poisoned_all_arterm_routes() {
    with_temp_arterm_home(|| {
        let previous_tier = std::env::var_os(crate::subscription_catalog::ARTERM_TIER_ENV);
        crate::env::set_var(crate::subscription_catalog::ARTERM_TIER_ENV, "plus");
