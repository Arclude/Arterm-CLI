...
fn test_account_command_uses_fast_auth_snapshot_without_running_cursor_status() {
    ...
    with_temp_arterm_home(|| {
        let prev_cursor_cli_path = std::env::var_os("ARTERM_CURSOR_CLI_PATH");
        ...
        crate::env::set_var("ARTERM_CURSOR_CLI_PATH", &script);
        ...
        match prev_cursor_cli_path {
            Some(value) => crate::env::set_var("ARTERM_CURSOR_CLI_PATH", value),
            None => crate::env::remove_var("ARTERM_CURSOR_CLI_PATH"),
        }
        crate::auth::AuthStatus::invalidate_cache();
    });
}
