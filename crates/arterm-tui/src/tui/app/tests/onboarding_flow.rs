    ...
    fn model_routes(&self) -> Vec<crate::provider::ModelRoute> {
        vec![
            crate::provider::ModelRoute {
                model: arterm_provider_core::DEFAULT_CLAUDE_MODEL.to_string(),
            ...
            crate::provider::ModelRoute {
                model: arterm_provider_core::DEFAULT_OPENAI_MODEL.to_string(),
                provider: "OpenAI".to_string(),
                api_method: "openai-api-key".to_string(),
                available: true,
                detail: String::new(),
...
fn onboarding_strongest_model_only_runs_without_explicit_defaults() {
    with_temp_arterm_home(|| {
        let previous_explicit = std::env::var_os("ARTERM_INITIAL_PROVIDER_EXPLICIT");
        ...
        if let Some(value) = previous_explicit {
            crate::env::set_var("ARTERM_INITIAL_PROVIDER_EXPLICIT", value);
        } else {
            crate::env::remove_var("ARTERM_INITIAL_PROVIDER_EXPLICIT");
        }
    });
...
fn import_review_collects_checked_logins() {
    ...
    .unwrap();
    // The default is the summary screen with Arterm subscription preselected.
...
fn subscription_choice_exposes_the_canonical_pricing_page() {
    with_temp_arterm_home(|| {
        ...
        assert_eq!(
            ...
            Some(format!(
                "Open Arterm pricing: {}",
                crate::subscription_catalog::ARTERM_PRICING_URL
            ))
        );
...
fn openrouter_key_typed_through_full_key_path_does_not_reopen_picker() {
    ...
    with_temp_arterm_home(|| {
        ...
        // It is written to $ARTERM_HOME/config/arterm/openrouter.env and exported
...
fn startup_check_skips_selfdev_canary_session() {
    with_temp_arterm_home(|| {
        ...
        app.onboarding_flow = None;
        app.onboarding_startup_checked = false;
        // Self-dev / canary sessions (e.g. the niri `arterm self-dev` hotkey) take
...
fn startup_check_skips_user_with_established_session_history() {
    with_temp_arterm_home(|| {
        ...
        // user when their arterm home has a substantial native session history
        // (e.g. setup_hints.json was reset or lost). Seed >=10 native session
        // files in the temp home.
        let sessions_dir = crate::storage::arterm_dir()
            .expect("arterm dir")
            .join("sessions");
...
fn import_failure_h_key_prepares_agent_repair_brief() {
    ...
    with_temp_arterm_home(|| {
        ...
        assert!(brief.contains("arterm auth-test --provider openai --json"), "{brief}");
        ...
        assert!(on_disk.contains("arterm auth-test --provider openai --json"), "{on_disk}");
...
fn import_continue_reaches_ready_quality_first_openai_model() {
    ...
    with_temp_arterm_home(|| {
        ...
        runtime.block_on(async {
            ...
            assert_eq!(model, arterm_provider_core::DEFAULT_OPENAI_MODEL);
