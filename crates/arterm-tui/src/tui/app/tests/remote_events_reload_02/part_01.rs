...
fn test_swarm_completion_notification_inserts_agent_snapshot_without_report_prose() {
    ...
    app.handle_server_event(
        crate::protocol::ServerEvent::Notification {
            ...
            message: "Demo completed; README first heading is arterm.".to_string(),
        },
        &mut remote,
    );
...
fn test_background_task_markdown_renders_card_even_if_role_was_lost() {
    ...
    app.push_display_message(DisplayMessage::user(
        "**Background task** `594967sj63` · `Run arterm library tests afte` (`bash`) · ✗ failed · 1.0s · exit 124\n\n```text\n\n--- Command timed out after 1000ms ---\n```\n\n_Full output:_ `bg action=\"output\" task_id=\"594967sj63\"`",
    ));
    ...
    assert!(
        text.contains("✗ bg Run arterm library tests afte failed · 594967sj63"),
