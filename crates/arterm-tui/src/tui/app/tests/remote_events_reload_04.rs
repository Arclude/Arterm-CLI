...
/// Run `f` with the shared env lock held and `ARTERM_MODEL`/`ARTERM_PROVIDER`
...
/// config `default_model` (shared test home) or exported `ARTERM_MODEL` would
...
fn test_info_widget_remote_openai_uses_remote_provider_for_usage_and_context() {
    ...
    app.remote_resolved_credential = Some(arterm_provider_core::ResolvedCredential::Oauth);
...
fn test_info_widget_remote_anthropic_api_key_shows_cost_based_usage() {
    ...
    app.remote_resolved_credential = Some(arterm_provider_core::ResolvedCredential::ApiKey);
    app.token_accounting.total_input_tokens = 12_000;
    app.token_accounting.total_output_tokens = 3_400;
...
fn test_info_widget_local_direct_api_runtime_shows_cost_based_usage() {
    ...
    for (runtime_provider, provider_name, model, expected_auth) in cases {
        crate::env::set_var("ARTERM_RUNTIME_PROVIDER", runtime_provider);
    ...
    crate::env::set_var("ARTERM_RUNTIME_PROVIDER", "arterm");
    crate::env::remove_var("ARTERM_OPENROUTER_ALLOW_NO_AUTH");
    ...
    crate::env::set_var("ARTERM_RUNTIME_PROVIDER", "openai-compatible");
...
fn test_anthropic_api_cost_accounts_for_split_cache_tokens() {
    ...
    let _guard = crate::storage::lock_test_env();
    let saved_runtime = std::env::var_os("ARTERM_RUNTIME_PROVIDER");
...
fn test_remote_anthropic_api_key_accrues_cost_from_token_usage() {
    ...
    oauth_app.remote_resolved_credential = Some(arterm_provider_core::ResolvedCredential::Oauth);
