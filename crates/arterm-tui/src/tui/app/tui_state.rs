...
impl App {
    ...
    ///   [`AuthStatus`] snapshot (cached up to 60s) or a `ARTERM_RUNTIME_PROVIDER`
    ...
    fn dual_credential_active(
        ...
        if route.is_remote {
            ...
            return self
                ...
                .route_api_method
                .as_deref()
                .and_then(arterm_provider_core::AuthRoute::parse)
    ...
    fn widget_auth_method(&self, route: WidgetRouteInfo) -> crate::tui::info_widget::AuthMethod {
        ...
        match route.provider {
            WidgetProviderKind::Anthropic => {
                match self
                    .dual_credential_active(route, arterm_provider_core::ActiveProvider::Claude)
                {
            ...
            WidgetProviderKind::OpenAI => {
                match self
                    .dual_credential_active(route, arterm_provider_core::ActiveProvider::OpenAI)
                {
...
impl crate::tui::TuiState for App {
    ...
    fn session_compaction_count(&self) -> usize {
        if self.is_remote || !self.provider.uses_arterm_compaction() {
            return 0;
        }
        self.registry
            .compaction()
    ...
    fn status_notice(&self) -> Option<String> {
        if !self.is_remote
            && self.provider.uses_arterm_compaction()
    ...
    fn context_snapshot(&self) -> crate::tui::ContextSnapshot {
        ...
        let (compaction_count, compaction_summary_chars, is_compacting, compaction_fresh) =
            if self.is_remote {
                (0, 0, false, true)
            } else if self.provider.uses_arterm_compaction() {
        ...
        if self.is_remote {
            ...
            let skip = if self.provider.uses_arterm_compaction() {
    ...
    fn info_widget_data(&self) -> crate::tui::info_widget::InfoWidgetData {
        ...
        let compaction_info = if !self.is_remote && self.provider.uses_arterm_compaction() {
            let compaction = self.registry.compaction();
        ...
        crate::tui::info_widget::InfoWidgetData {
            ...
            cache_hit_info,
            compaction_info,
            is_compacting: if !self.is_remote && self.provider.uses_arterm_compaction() {
                let compaction = self.registry.compaction();
                compaction
                    .try_read()
    ...
    fn memory_clean_confirm(&self) -> Option<crate::tui::MemoryCleanConfirmView<'_>> {
        App::memory_clean_confirm(self)
    }
...
impl App {
    ...
    pub(crate) fn pop_out_selected_swarm_agent(&mut self) {
        ...
        let exe = std::env::current_exe().unwrap_or_else(|_| std::path::PathBuf::from("arterm"));
        let cwd = std::env::current_dir().unwrap_or_else(|_| std::path::PathBuf::from("."));
        match arterm_app_core::session_launch::spawn_resume_in_new_terminal(&exe, &session_id, &cwd)
        {
