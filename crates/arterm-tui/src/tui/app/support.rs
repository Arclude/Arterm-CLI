...
pub(super) const SUPPORT_EMAIL: &str = "support@arterm.dev";
pub(super) const SUPPORT_SUBJECT: &str = "arterm support";
...
fn build_channel() -> String {
    ...
    if arterm_build_meta::is_release_build() {
...
fn read_telemetry_id() -> Option<String> {
    let path = crate::storage::arterm_dir().ok()?.join("telemetry_id");
...
fn gather_diagnostics(app: &App) -> SupportDiagnostics {
    ...
    // Account identity is only present when a arterm subscription is
    ...
    let (account_id, account_email, tier) = if has_subscription {
        (
            load_env_value_from_env_or_config(cat::ARTERM_ACCOUNT_ID_ENV, cat::ARTERM_ENV_FILE),
            load_env_value_from_env_or_config(cat::ARTERM_ACCOUNT_EMAIL_ENV, cat::ARTERM_ENV_FILE),
...
mod tests {
    ...
    fn support_mailto_url_is_correctly_encoded() {
        ...
        assert!(url.starts_with("mailto:support@arterm.dev?subject=arterm%20support&body="));
