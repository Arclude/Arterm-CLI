...
/// NO_BROWSER/ARTERM_NO_BROWSER and refuses to open anything from test binaries
...
pub(crate) fn open_path_or_url_detached(
    ...
    if crate::auth::browser_suppressed(false) {
        return Err(std::io::Error::other(
            "opening files/URLs is suppressed (NO_BROWSER/ARTERM_NO_BROWSER or test harness)",
        ));
    }
...
pub(super) fn launch_client_executable() -> PathBuf {
    crate::build::client_update_candidate(arterm_selfdev_types::client_selfdev_requested())
...
pub(super) fn debug_cmd_path() -> PathBuf {
    if let Ok(path) = std::env::var("ARTERM_DEBUG_CMD_PATH") {
        return PathBuf::from(path);
    }
    std::env::temp_dir().join("arterm_debug_cmd")
}

/// Debug response file path
pub(super) fn debug_response_path() -> PathBuf {
    if let Ok(path) = std::env::var("ARTERM_DEBUG_RESPONSE_PATH") {
        return PathBuf::from(path);
    }
    std::env::temp_dir().join("arterm_debug_response")
}
...
pub(super) fn inferred_reasoning_efforts(
    ...
    arterm_provider_core::inferred_reasoning_efforts(provider_name, model_name)
}
...
/// Spawn a new terminal window that resumes a arterm session.
...
/// Build the terminal command used to spawn a brand-new arterm session.
...
fn build_fresh_session_command(socket: Option<&str>) -> crate::terminal_launch::TerminalCommand {
    ...
    crate::terminal_launch::TerminalCommand::new(&exe, args)
        .title("arterm · new session".to_string())
        .kind("new-terminal")
        .fresh_spawn()
}

/// Spawn a brand-new arterm session in a new terminal window, staying on the
...
pub(super) fn clipboard_image() -> Option<(String, String)> {
            ...
            && !html_output.stdout.is_empty()
        {
            ...
            if let Some(url) = extract_image_url(&html) {
                crate::logging::info(&format!(
                    ...
                    arterm_core::util::truncate_str(&url, 80)
                ));
    ...
    #[cfg(target_os = "macos")]
    {
        let temp_path = std::env::temp_dir().join("arterm_clipboard.png");
        let script = format!(
