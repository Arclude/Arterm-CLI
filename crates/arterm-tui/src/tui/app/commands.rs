...
pub(super) fn is_auto_poke_connectivity_error(error: &str) -> bool {
    // Delegate to the shared connectivity classifier (arterm-app-core's
        ...
        Ok(result) => {
            ...
            match result {
                Ok(prepared) => {
                    ...
                    let socket = std::env::var("ARTERM_SOCKET").ok();
...
fn handle_subagent_model_command(app: &mut App, trimmed: &str) -> bool {
    ...
    if app.is_remote {
        app.push_display_message(DisplayMessage::error(
            "/subagent-model requires a live arterm server connection in remote mode.".to_string(),
        ));
        return true;
    }
...
fn handle_subagent_command(app: &mut App, trimmed: &str) -> bool {
    ...
    if app.is_remote {
        app.push_display_message(DisplayMessage::error(
            "/subagent requires a live arterm server connection in remote mode.".to_string(),
        ));
        return true;
    }
...
/// and macOS shortcuts, and any conflicts with arterm's own keybindings.
...
pub(super) fn handle_log_command(app: &mut App, trimmed: &str) -> bool {
    ...
    crate::logging::info(&format!(
        "ARTERM_LOG_MARK id={} session={} provider={} model={} cwd={} note={}",
        marker_id,
        app.session.id,
        app.provider_name(),
        app.provider_model(),
    ...
    let mut message = format!(
        "Log mark written: {}\n\nAgents can search ~/.arterm/logs/ for ARTERM_LOG_MARK or this marker id.",
        marker_id
    );
    if !note.is_empty() {
...
Step 1/4: Tell Arterm where to connect.
...
Security model
  - Arterm stores this host/user target so you can run /ssh {} later.
  - Arterm does not ask for or store your SSH password.
  - If a password is needed, it will be typed into your system ssh prompt, not into Arterm.
...
Arterm will ask for the SSH target, then use your system SSH client for authentication. Arterm never stores SSH passwords."
                    .to_string(),
            ));
        }
        Ok(config) => {
            ...
            lines.push("".to_string());
            lines.push("Security: Arterm stores targets only, never SSH passwords.".to_string());
...
Arterm verified that {} is reachable through your system SSH client.

What this means:
  - Authentication is handled by OpenSSH / your SSH agent.
  - Arterm did not see or store your password.
  - The SSH connection setup is ready for remote Arterm tools.

Next implementation step: start the remote Arterm server over this verified SSH connection.",
...
Arterm could not connect without an interactive login, so it opened a separate terminal running your system ssh command.

What to expect in that terminal
  1. OpenSSH may ask for your password or two-factor prompt.
  2. You type credentials into OpenSSH, not into Arterm.
  3. After login, SSH creates a temporary background control socket.
  4. The terminal verifies that socket before closing.

Security model
  - Arterm cannot read what you type in the SSH terminal.
  - Arterm stores only the target {}.
...
Arterm could not open a terminal automatically. Run this command yourself:

  ssh -f -M -S {} -N {}

Type your password into that SSH prompt if asked. Arterm will not see or store it.",
            profile.name,
            crate::ssh_remote::control_socket_path(&profile.name)
                .map(|p| p.display().to_string())
                .unwrap_or_else(|_| "~/.arterm/ssh-control/remote.sock".to_string()),
            profile.ssh_target
        ))),
        Err(error) => app.push_display_message(DisplayMessage::error(format!(
...
fn git_command_repo_dir(app: &App) -> Result<PathBuf, String> {
    if let Some(path) = active_working_dir(app) {
        ...
        return Err(format!(
            "Unable to run /git: session working directory {} is not accessible from this arterm client.",
            path.display()
        ));
    }
...
pub(super) fn build_triage_prompt(focus: &str) -> String {
    let mut prompt = String::from(
        ...
        Hard rules: every public comment, issue reply, or PR description you post MUST end with a clear agent attribution line like '--- *— Arterm agent (automated triage), on behalf of @<repo-owner>*' so it can never be mistaken for the human. \
...
fn build_test_verification_prompt(claim: &str) -> String {
    ...
    format!(
        "Run Arterm's /test verification orchestrator for: {target}\n\n\
...
pub(super) fn handle_dictation_command(app: &mut App, trimmed: &str) -> bool {
    ...
    if trimmed.starts_with("/dictate ") || trimmed.starts_with("/dictation ") {
        app.push_display_message(DisplayMessage::error(
            "Usage: /dictate\nConfigure [dictation] in ~/.arterm/config.toml to customize command, mode, hotkey, and timeout."
                .to_string(),
        ));
        return true;
    }
...
fn ensure_swarm_prompt_edit_path(
    ...
    arterm_dir: &std::path::Path,
) -> std::io::Result<PathBuf> {
    let project_dir = match working_dir {
        Some(path) => PathBuf::from(path),
        None => std::env::current_dir()?,
    };
    let project_path = project_dir.join(".arterm").join("swarm-prompt.md");
    if file_has_nonblank_content(&project_path) {
        return Ok(project_path);
    }

    let global_path = arterm_dir.join("swarm-prompt.md");
    if file_has_nonblank_content(&global_path) {
        return Ok(global_path);
    }

    std::fs::create_dir_all(arterm_dir)?;
...
pub(super) fn handle_swarm_prompt_command(app: &mut App, trimmed: &str) -> bool {
    ...
    let arterm_dir = match crate::storage::arterm_dir() {
        Ok(path) => path,
        Err(error) => {
            app.push_display_message(DisplayMessage::error(format!(
                "Failed to locate the Arterm config directory: {}",
                error
            )));
            return true;
        }
    ...
    match run_interactive_editor(&mut command) {
        Ok(status) if status.success() => {
            app.push_display_message(DisplayMessage::system(format!(
                "Edited the active swarm routing prompt in {}:\n{}\n\nChanges apply after restarting or reloading Arterm because running agent tool registries cache the prompt.",
                editor,
                path.display()
            )));
...
/// Run a terminal editor without letting it fight Arterm's raw-mode event loop.
///
...
fn suspend_terminal_for_editor() {
    ...
    arterm_tui_style::restore_terminal_quietly();
}
...
fn handle_alignment_command(app: &mut App, trimmed: &str) -> bool {
    ...
    if rest.is_empty() || matches!(rest, "show" | "status") {
        ...
        app.push_display_message(DisplayMessage::system(format!(
            ...
            alignment_label(saved), arterm_tui_core::keybind::alt_chord("C")
        )));
        return true;
    }
...
pub(super) fn handle_config_command(app: &mut App, trimmed: &str) -> bool {
    ...
    if trimmed == "/subscription" || trimmed == "/subscription status" {
        app.show_arterm_subscription_status();
        return true;
    }
            ...
            match parts.next() {
                Some(bin) => {
                    ...
                    match run_interactive_editor(&mut command) {
                        Ok(status) if status.success() => {
                            app.push_display_message(DisplayMessage::system(format!(
                                "Edited config in {}:\n{}\n\n*Restart arterm after editing for changes to take effect.*",
                                editor,
                                path.display()
                            )));
