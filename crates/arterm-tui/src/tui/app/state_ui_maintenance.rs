...
impl App {
            ...
            crate::bus::ClientMaintenanceAction::Rebuild => {
                ...
                self.set_client_maintenance_message(
                    ...
                    Self::client_maintenance_card_message(
                        ...
                        "Running in the background. arterm will reload automatically after the rebuild succeeds.",
                    ),
                );
    ...
    pub(super) fn maybe_finish_background_client_reload(&mut self) -> bool {
        ...
        self.set_client_maintenance_message(
            ...
            Self::client_maintenance_card_message(
                action,
                "reloading client",
                "The new binary is ready, so arterm is switching over now.",
            ),
        );
        ...
        if let Ok(now) = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH) {
            ...
            crate::env::set_var("ARTERM_RELOAD_GAP_STARTED_MS", now.as_millis().to_string());
        }
            ...
            SessionUpdateStatus::Status {
                ...
                self.set_client_maintenance_message(
                    ...
                    Self::client_maintenance_card_message(
                        action,
                        message,
                        "Still running in the background. arterm will reload automatically when ready.",
                    ),
                );
            }
    ...
    /// user can hand the reconciliation to a fresh arterm agent with one key.
    ///
    ...
    pub(super) fn offer_update_merge(
        ...
        let content = format!(
            "Update diverged. Press {} to let a arterm agent merge local and upstream (or run `git pull` / `git rebase` yourself).",
            key_label
        );
    ...
    /// Accept the armed merge offer: spawn a fresh arterm session pre-loaded with
    ...
    pub(super) fn accept_update_merge_offer(&mut self) -> bool {
        ...
        let repo_label = offer
            ...
            .unwrap_or_else(|| "the arterm repository".to_string());
        let prompt = format!(
            "A arterm self-update could not fast-forward because the local checkout and upstream have diverged.\n\n\
Repository: {repo}\n\
Update error: {detail}\n\n\
...
Do not force-push or discard local commits without confirming they are already upstream.",
        ...
        match self.launch_update_merge_agent(prompt, offer.repo_dir.as_deref()) {
            Ok(true) => {
                self.push_display_message(DisplayMessage::system(
                    "↗ Spawned a arterm agent to merge the diverged update.",
                ));
                self.set_status_notice("Merge agent launched");
            }
            Ok(false) => {
                self.push_display_message(DisplayMessage::system(
                    "Could not open a new terminal for the merge agent. Run `git pull` / `git rebase` manually, or start `arterm` in the repo and ask it to merge.",
                ));
    ...
    /// Spawn a fresh arterm session, in the repo directory when known, with a
