...
/// (see `arterm_tui_markdown::reasoning_line_markup`). Trailing blank lines left
/// behind are trimmed so the remaining answer renders cleanly.
pub(super) fn strip_reasoning_lines(content: &str) -> String {
    let sentinel = arterm_tui_markdown::REASONING_SENTINEL;
...
/// the reporting modes arterm enables, so `array[0]` and `[TODO]` are left alone
...
impl App {
    ...
    /// Spawn a brand-new arterm session in a new terminal window.
    pub(crate) fn handle_new_terminal_hotkey(&mut self) {
        ...
        match super::spawn_fresh_session_in_new_terminal(&cwd) {
            ...
            Ok(false) => {
                self.set_status_notice("No supported terminal found; run `arterm` manually")
            }
...
pub(super) fn handle_modal_key(
    ...
    if super::commands::commands_device::handle_pairing_key(app, code) {
        return Ok(true);
    }
    ...
    if app.copy_selection_mode {
            ...
            && matches!(code, KeyCode::Char('c') | KeyCode::Char('d'))
        {
            ...
            // closed arterm and lost the error (issue #497). Only fall
...
impl App {
    ...
    fn handle_key_core(
        ...
        // Accept an armed "merge the diverged update" offer: spawn a arterm agent
    ...
    pub(super) fn submit_input(&mut self) {
        ...
        if let Some(command) = extract_input_shell_command(&input) {
            ...
            if self.is_remote {
                self.push_display_message(DisplayMessage::system(
                    "Input-line ! shell commands are only available in a local arterm TUI session.",
                ));
        ...
        if let Some(invocation) = skill_invocation {
            ...
            // as .arterm/skills/optimization work immediately after reload/build.
            if skill.is_none() {
            ...
            if let Some(skill) = skill {
                ...
                let endorsed_hint = crate::skill::endorsed_skills()
                    ...
                    .map(|endorsed| match endorsed.install {
                        ...
                        None => format!(
                            "Skill /{} is endorsed but not installed (source: {}). Install it into ~/.arterm/skills/{}/SKILL.md.",
