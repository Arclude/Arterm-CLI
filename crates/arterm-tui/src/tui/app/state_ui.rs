...
impl App {
    ...
    pub(super) fn save_input_for_reload(&self, session_id: &str) {
            ...
            && !self.todo_confidence_spike_challenged
        {
            ...
            if let Ok(arterm_dir) = crate::storage::arterm_dir() {
                let path = arterm_dir.join(format!("client-input-{}", session_id));
    ...
    pub(super) fn restore_input_for_reload(session_id: &str) -> Option<RestoredReloadInput> {
        let arterm_dir = crate::storage::arterm_dir().ok()?;
        let path = arterm_dir.join(format!("client-input-{}", session_id));
        if !path.exists() {
            return None;
        }
    ...
    pub fn debug_socket_path() -> std::path::PathBuf {
        crate::storage::runtime_dir().join("arterm-debug.sock")
    }
}
...
/// plus the curated list of arterm-endorsed skills (marking which are installed).
fn build_skills_report(app: &App) -> String {
    ...
    if app.is_remote && !app.remote_skills.is_empty() {
        ...
        if skills.is_empty() {
            out.push_str(
                "- none loaded\n  Add skills under ~/.arterm/skills/<name>/SKILL.md or ./.arterm/skills/<name>/SKILL.md\n",
            );
        } else {
            for skill in skills {
    ...
    out.push_str("\nEndorsed skills (recommended by arterm)\n");
...
pub(super) fn handle_info_command(app: &mut App, trimmed: &str) -> bool {
    ...
    if trimmed == "/version" {
        let version = arterm_build_meta::version();
        let is_canary = if app.session.is_canary {
            " (canary/self-dev)"
        } else {
            ""
        };
        let mut content = format!("arterm client: {}{}", version, is_canary);
        if app.is_remote {
    ...
    if trimmed == "/info" {
        ...
        if let Some(ref provider_id) = app.provider_session_id {
            info.push_str(&format!(
                "Provider Session: {}...\n",
                arterm_core::util::truncate_str(provider_id, 16)
            ));
        }
        ...
        let compaction_summary = if app.provider.supports_compaction() {
            ...
            if let Ok(manager) = manager.try_read() {
                ...
                format!(
                    "- supported: yes\n- mode: {}\n- arterm-managed: {}\n- active summary: {} ({})\n- compacted messages: {}\n- active messages: {}\n- summary chars: {}\n- estimated tokens: {}\n- effective tokens: {}\n- observed tokens: {}\n- usage: {:.1}%\n- compacting now: {}\n- budget: {}",
                    mode,
                    if app.provider.uses_arterm_compaction() {
                        "yes"
                    } else {
                        "no"
                    },
