...
#[test]
fn tui_openai_compatible_api_base_keeps_arterm_docs_and_remote_endpoint() -> anyhow::Result<()> {
    with_temp_arterm_home(|| {
        ...
        assert!(resolved.requires_api_key);
        assert!(resolved.setup_url.contains("github.com/Arclude/Arterm-CLI"));
...
fn tui_arterm_subscription_logout_clears_credentials_and_preserves_api_base() -> anyhow::Result<()>
{
    with_temp_arterm_home(|| {
        ...
        assert!(std::env::var_os(crate::subscription_catalog::ARTERM_API_KEY_ENV).is_none());
        assert_eq!(
            std::env::var(crate::subscription_catalog::ARTERM_API_BASE_ENV).as_deref(),
            Ok("https://subscription.example/v1")
        );
        assert!(std::env::var_os(crate::subscription_catalog::ARTERM_ACCOUNT_ID_ENV).is_none());
        assert!(std::env::var_os(crate::subscription_catalog::ARTERM_ACCOUNT_EMAIL_ENV).is_none());
