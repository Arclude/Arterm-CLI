...
impl App {
    pub(super) fn command_help(&self, topic: &str) -> Option<String> {
        ...
        let help = match topic.as_str() {
            ...
            | "model-status"
            | "model status" => {
                "/provider-test-coverage\nShow arterm live verification evidence for the current provider/model.\n\n/provider-test-coverage <provider> <model>\nLook up a specific provider/model pair in the live-test coverage ledger.\n\nThe report shows last-tested time, arterm build, passed/missing checkpoints, readiness gaps, and a caveat that missing evidence is not a provider failure."
            }
            "refresh-model-list" => {
            ...
            "swarm-prompt" => {
                "/swarm-prompt\nOpen the active swarm routing prompt in $VISUAL or $EDITOR.\n\nArterm uses a nonblank project override at ./.arterm/swarm-prompt.md when present, then ~/.arterm/swarm-prompt.md, then the built-in default. If no editable override exists, this command creates the global file from the built-in default. Restart or reload Arterm after editing because running agent tool registries cache the prompt."
            }
            "subagent" => {
            ...
            "triage" => {
                "/triage [focus]\nTriage open GitHub issues for the current repo, then autonomously fix the safe ones.\n\nThe agent lists untriaged issues with gh, classifies each (auto-fix, needs-info, needs-human, duplicate, question), applies existing labels, fixes and verifies the clear-cut bugs, and reports back with a summary table. Every public comment is clearly signed as the Arterm agent, and issues are never closed as wontfix/invalid without your confirmation.\n\nOptional focus text narrows the triage, for example /triage only crash reports."
            }
            "catchup" => {
            ...
            "restart" => {
                "/restart\nRestart arterm with the current binary. Session is preserved.\nUseful after config changes, MCP server updates, or env var changes."
            }
            "rebuild" => {
                "/rebuild\nRun git pull --ff-only, cargo build --release, and release tests in the background. arterm stays usable and reloads automatically when the build is ready."
            }
            "selfdev" => {
            ...
            "fork" | "split" => {
                "/fork\nFork the current session into a new terminal pane or window. Clones the full conversation history so both sessions continue from the same point. Inside tmux, arterm automatically opens a right-side pane.\n\n/fork <prompt>\nFork the session and start the new pane/window by answering the prompt. The original session keeps working uninterrupted.\n\n/split\nAlias for /fork."
            }
            "resume" | "sessions" => {
            ...
            "subscribe" | "hosted" => {
                "/hosted\nUse Arterm hosted models without a subscription: choose a monthly spending limit, receive milestone warnings without throttling, and pay in progressively larger tranches. Sign in once with /login arterm.\n\n/hosted status\nShow usage and your current spending limit.\n\n/subscribe\nCompatibility alias for /hosted."
            }
            "version" => "/version\nShow arterm version/build details.",
            ...
            "auth" | "login" => {
                "/auth\nShow authentication status for all providers.\n\n/login\nInteractive provider selection - pick a provider to log into.\n\n/login <provider>\nStart login flow directly for any provider shown by /login or the /login completions.\n\nUse /login arterm for pay-as-you-go hosted models through the Arterm router. Set a monthly spending limit in the browser; no API key is pasted into the terminal."
            }
            "account" | "accounts" => {
        ...
        let help = help.replace(
            "Alt+",
            &format!("{}+", arterm_tui_core::keybind::alt_label()),
        );
        Some(help)
    }
}
