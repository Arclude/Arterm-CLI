...
//!      login (`arterm auth-test --json`, `arterm provider add --api-key-stdin`,
//!      `arterm login`). The agent can add the key/login for the user itself.
//!
...
/// An external coding agent arterm can hand a repair task to. These map to the
...
/// `ARTERM_HOME`/external isolation, matching the onboarding import detectors.
...
pub(crate) fn build_repair_brief(
    ...
    let log_line = arterm_logging::log_path()
        .map(|p| format!("Logs:    {}", p.display()))
        .unwrap_or_else(|| "Logs:    ~/.arterm/logs/arterm-<date>.log".to_string());

    let mut brief = String::new();
    brief.push_str(
        "arterm (a terminal coding agent) could not import or validate a login \
...
\u{20}     --base-url https://api.example.com/v1 --model <model> --api-key-stdin\n\n",
    ...
    brief.push_str(
        "When auth-test reports success, tell the user to restart arterm (or press Enter on \
...
/// to paste anything. Lives under the arterm home so it honors `ARTERM_HOME`.
...
mod tests {
    ...
    fn brief_lists_the_non_interactive_commands_and_log_path() {
        ...
        assert!(
            brief.contains("arterm auth-test --provider openai --json"),
            "{brief}"
        );
        assert!(brief.contains("arterm login --provider openai"), "{brief}");
        assert!(brief.contains("--api-key-stdin"), "{brief}");
        assert!(brief.contains("arterm provider add"), "{brief}");
        // Points at the logs.
    ...
    fn brief_without_provider_or_agent_is_still_actionable() {
        ...
        assert!(
            brief.contains("arterm auth-test --all-configured --json"),
            "{brief}"
        );
        assert!(brief.contains("arterm auth doctor"), "{brief}");
        // No agent label, but still tells the user what to do.
        assert!(brief.contains("restart arterm"), "{brief}");
    }
