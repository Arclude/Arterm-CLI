...
struct RemoteModelCatalogCache {
    ...
    origin: String,
    #[serde(flatten)]
    snapshot: arterm_provider_core::ModelCatalogSnapshot,
...
fn model_picker_route_is_current(
    ...
    arterm_provider_core::model_route_provider_labels_match(&route.provider, current_provider)
}
...
fn model_picker_route_is_recommended(model_name: &str, route: &PickerOption) -> bool {
    RECOMMENDED_MODELS.contains(&model_name)
        && arterm_provider_core::model_route_metadata_is_recommended(
            model_name,
            &route.provider,
            &route.api_method,
            route.available,
...
fn model_picker_route_provider_matches_key(
    ...
    arterm_provider_core::model_route_provider_matches_key(
        route_provider_key,
...
fn model_picker_route_is_default(
    ...
    if let Some((bare_model, provider_label)) = default_model.rsplit_once('@') {
        return bare_model == model_name
            && arterm_provider_core::model_route_provider_labels_match(
                &route.provider,
                provider_label,
            );
    }
...
impl App {
    pub(super) fn remote_model_catalog_snapshot(
        &self,
    ) -> arterm_provider_core::ModelCatalogSnapshot {
        arterm_provider_core::ModelCatalogSnapshot::new(
    ...
    fn append_arterm_subscription_routes_static(
        ...
        let existing = routes
            ...
            .filter(|route| {
                route
                    .api_method
                    .eq_ignore_ascii_case(crate::subscription_catalog::ARTERM_ROUTE_API_METHOD)
            })
            ...
            })
        {
            routes.push(crate::provider::ModelRoute {
                ...
                provider: crate::subscription_catalog::ARTERM_PROVIDER_DISPLAY_NAME.to_string(),
                api_method: crate::subscription_catalog::ARTERM_ROUTE_API_METHOD.to_string(),
                available: true,
    ...
    fn extend_remote_routes_for_uncovered_models_static(
        ...
        // Arterm subscription routes are a complete, server-managed catalog.
        ...
        let provider_is_arterm_subscription = remote_provider_name.is_some_and(|name| {
            name.eq_ignore_ascii_case(crate::subscription_catalog::ARTERM_PROVIDER_DISPLAY_NAME)
        });
        if provider_is_arterm_subscription {
            routes.clear();
            Self::append_arterm_subscription_routes_static(
        ...
        let poisoned_by_arterm_subscription = !routes.is_empty()
            && routes.iter().all(|route| {
                route
                    .api_method
                    .eq_ignore_ascii_case(crate::subscription_catalog::ARTERM_ROUTE_API_METHOD)
            });
        if poisoned_by_arterm_subscription {
            // Version 1 could turn a mixed provider catalog into all-Arterm rows
        ...
        // signed-in Arterm subscriber still needs the managed route for each
    ...
    fn open_cached_model_picker_if_fresh(
        ...
        if std::env::var("ARTERM_LOG_MODEL_PICKER_TIMING").is_ok() {
    ...
    fn open_model_picker_with_routes(
        ...
        if total_ms >= 250 || std::env::var("ARTERM_LOG_MODEL_PICKER_TIMING").is_ok() {
            crate::logging::info(&format!(
    ...
    pub(super) fn handle_session_picker_selection(&mut self, targets: &[ResumeTarget]) {
        ...
        if self.session_picker_mode == SessionPickerMode::CatchUp {
            ...
            for target in targets {
                let ResumeTarget::ArtermSession { session_id } = target else {
                    continue;
                };
        ...
        for target in targets {
            ...
            let name = match target {
                ...
                ResumeTarget::CodexSession { session_id, .. } => {
                    format!("Codex {}", arterm_core::util::truncate_str(session_id, 8))
                }
            ...
            match spawn_resume_target_in_new_terminal(&resolved_target, &cwd, socket.as_deref()) {
                ...
                Ok(false) | Err(_) => {
                    // No terminal emulator could be spawned. For a single arterm
                    // session, fall back to resuming in the current terminal
                    // instead of dead-ending with a manual command (issue #203).
                    if targets.len() == 1
                        && spawned == 0
                        && matches!(resolved_target, ResumeTarget::ArtermSession { .. })
                    {
    ...
    pub(super) fn handle_session_picker_current_terminal_selection(
        ...
        let ResumeTarget::ArtermSession { session_id } = resolved_target else {
            self.push_display_message(DisplayMessage::error(format!(
    ...
    fn handle_live_claude_takeover(&mut self, target: &ResumeTarget) -> bool {
        ...
        let display_id = arterm_core::util::truncate_str(session_id, 8);
        ...
        let ResumeTarget::ArtermSession { session_id } = resolved else {
            self.push_display_message(DisplayMessage::error(
                "Claude takeover did not produce a Arterm session.",
            ));
            return false;
        };
    ...
    pub(super) fn handle_batch_crash_restore(&mut self, session_ids: &[String]) {
        ...
        if spawned > 0 && failed.is_empty() {
            ...
            let manual: Vec<String> = failed
                .iter()
                .map(|id| format!("  arterm --resume {}", id))
                .collect();
            self.push_display_message(DisplayMessage::system(format!(
    ...
    pub(super) fn picker_fuzzy_score(pattern: &str, text: &str) -> Option<i32> {
        arterm_fuzzy::fuzzy_score_tokens(pattern, text)
    }
    ...
    pub(super) fn apply_inline_interactive_filter(picker: &mut InlineInteractiveState) {
        if picker.filter.is_empty() {
            ...
            let prepared = arterm_fuzzy::PreparedTokenQuery::new(&picker.filter);
...
mod tests {
    ...
    fn model_picker_current_route_allows_provider_aliases() {
        assert!(arterm_provider_core::model_route_provider_labels_match(
            "Anthropic",
            "Claude"
        ));
        assert!(arterm_provider_core::model_route_provider_labels_match(
    ...
    fn model_picker_recommended_route_is_provider_aware() {
        ...
        // Current policy (see arterm-provider-core): claude-opus-4-8 is the
    ...
    fn remote_model_catalog_cache_rejects_stale_and_future_timestamps() {
        let snapshot = arterm_provider_core::ModelCatalogSnapshot::new(
    ...
    fn remote_model_catalog_cache_rejects_forged_or_oversized_routes() {
        let safe_snapshot = arterm_provider_core::ModelCatalogSnapshot::new(
