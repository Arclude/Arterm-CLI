...
fn tui_mouse_scroll_trace_enabled() -> bool {
    std::env::var_os("ARTERM_TUI_SCROLL_TRACE").is_some()
}
...
impl App {
    ...
    pub(super) fn toggle_swarm_message_expand(&mut self, msg_idx: usize) -> bool {
        ...
        let Some(toggled) = arterm_tui_messages::toggle_collapsible_swarm_content(&message.content)
        else {
            return false;
        };
        let expanded = arterm_tui_messages::parse_collapsible_swarm_content(&toggled)
    ...
    pub(super) fn toggle_inline_images(&mut self) {
        ...
        self.set_status_notice(if self.inline_images_visible {
            ...
            let alt = arterm_tui_core::keybind::alt_chord("Shift+I");
