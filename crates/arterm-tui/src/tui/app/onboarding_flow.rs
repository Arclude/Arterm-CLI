...
pub(crate) enum SummaryPill {
    ...
    Continue,
    /// Skip importing and sign in with a Arterm subscription instead (default).
    Subscription,
...
impl ImportReview {
    ...
    /// starting on the summary screen with the Arterm subscription preselected.
...
/// `ARTERM_HOME`/external isolation, matching the import detectors.
...
mod tests {
    ...
    fn external_oauth_present_requires_nonempty_file() {
        let dir = std::env::temp_dir().join(format!("arterm-onb-test-{}", std::process::id()));
