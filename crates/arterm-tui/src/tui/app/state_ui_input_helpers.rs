...
const REGISTERED_COMMANDS: &[RegisteredCommand] = &[
    ...
    RegisteredCommand::public(
        "/init",
        "Generate or update AGENTS.md so later sessions understand this repo",
    ),
    ...
    RegisteredCommand::public(
        "/skills",
        "Show loaded skills and arterm-endorsed recommendations",
    ),
    ...
    RegisteredCommand::public("/feedback", "Send feedback about arterm"),
    RegisteredCommand::public("/telemetry", "Show or change what arterm sends"),
    ...
    RegisteredCommand::public("/selfdev", "Open a new self-dev arterm session"),
    RegisteredCommand::public("/update", "Background update and auto reload"),
    RegisteredCommand::public("/resume", "Open session picker"),
    RegisteredCommand::hidden("/sessions", "Alias for /resume"),
    RegisteredCommand::hidden("/session", "Alias for /resume"),
    ...
    RegisteredCommand::public("/quit", "Exit arterm"),
...
impl App {
    ...
    pub fn suggestion_prompts(&self) -> Vec<(String, String)> {
        ...
        // A suggested prompt that told the agent to pipe a stranger's install
        // script into bash used to sit here, offered to every new macOS user.
        // Whatever the script does, shipping `curl … | bash` against a third
        // party as a one-click suggestion is not something this project can
        // stand behind, and it was upstream's project rather than ours.
...
mod external_cli_suggestion_tests {
    ...
    /// Run with:
    ///   cargo test -p arterm-tui --lib onboarding_suggestion_scan_cost -- --ignored --nocapture
    #[test]
    #[ignore]
...
"#,
    ...
    fn parses_codex_input_text_blocks() {
        ...
        std::fs::write(
            &path,
            r#"{"type":"session_meta","payload":{"id":"sid","cwd":"/home/jeremy/arterm"}}
{"type":"response_item","payload":{"type":"message","role":"user","content":[{"type":"input_text","text":"check in on arterm"}]}}
"#,
        )
        .expect("write fixture");
        ...
        assert_eq!(
            ...
            Some("/home/jeremy/arterm")
        );
        assert_eq!(candidate.context.as_deref(), Some("check in on arterm"));
    }
