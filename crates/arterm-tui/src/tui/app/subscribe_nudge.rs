...
//!     arterm account credentials, never in replay/test runtimes.
//!
...
pub(super) const RATE_LIMIT_NUDGE_LINE: &str =
    "Avoid provider rate limits with Arterm hosted models: /hosted";
/// Status-line tail shared by trigger B.
const SUPPORT_NUDGE_NOTICE: &str = "Try Arterm hosted models: /hosted";
...
/// Persisted nudge bookkeeping (one small JSON file under the arterm dir).
...
pub(super) fn subscribe_pitch_markdown() -> String {
    let mut message = String::from("Arterm hosted models\n\n");
    ...
    message.push_str("  - Sign in once: Arterm saves the hosted API key and routes the rest\n");
...
impl App {
    ...
    pub(super) fn claim_subscribe_nudge(&mut self, trigger: SubscribeNudgeTrigger) -> bool {
        ...
        // Never pitch existing arterm account holders.
    ...
    pub(super) fn show_subscribe_pitch(&mut self) {
        ...
        self.set_status_notice("Hosted models: /login arterm to start");
    }
}
...
mod tests {
    ...
    fn long_task_message_reads_support_wording_with_elapsed() {
        ...
        assert_eq!(
            message,
            "✦ arterm just worked 1h 23m for you. Try Arterm hosted models: /hosted"
        );
    ...
    fn pitch_explains_limits_warnings_tranches_and_next_step() {
        ...
        assert!(pitch.contains("/login arterm"));
