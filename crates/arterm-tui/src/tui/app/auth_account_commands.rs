...
fn parse_account_command(trimmed: &str) -> Option<Result<AccountCommand, String>> {
    ...
    if let Some(provider) = resolve_account_provider_descriptor(first) {
        ...
        let parsed = match subcommand {
            ...
            "status" if provider.id == "arterm" => AccountCommand::ArtermStatus,
            "manage" if provider.id == "arterm" => AccountCommand::ArtermManage,
            "logout" if provider.id == "arterm" => AccountCommand::ArtermLogout,
...
pub(crate) fn execute_account_command_local(app: &mut App, command: AccountCommand) {
    match command {
        ...
        AccountCommand::ArtermStatus => app.show_arterm_subscription_status(),
        AccountCommand::ArtermManage => app.open_arterm_account_management(),
        AccountCommand::ArtermLogout => app.start_arterm_account_logout(),
...
fn save_default_provider_setting(app: &mut App, provider: Option<&str>) {
    ...
    let provider = match normalized.as_deref() {
        ...
        Some(other) if arterm_provider_core::AuthRoute::parse(other).is_some() => normalized,
        Some(other) => {
            app.push_display_message(DisplayMessage::error(format!(
...
fn save_openai_compat_setting(app: &mut App, setting: OpenAiCompatSetting, value: Option<&str>) {
    ...
    let (env_key, normalized_value) = match setting {
        OpenAiCompatSetting::ApiBase => {
            ...
            ("ARTERM_OPENAI_COMPAT_API_BASE", normalized)
        }
...
mod tests {
    ...
    #[test]
    fn parse_native_arterm_account_actions() {
        assert!(matches!(
            parse_account_command("/account arterm login"),
            Some(Ok(AccountCommand::Login { provider_id })) if provider_id == "arterm"
        ));
        assert!(matches!(
            parse_account_command("/account arterm status"),
            Some(Ok(AccountCommand::ArtermStatus))
        ));
        assert!(matches!(
            parse_account_command("/account arterm manage"),
            Some(Ok(AccountCommand::ArtermManage))
        ));
        assert!(matches!(
            parse_account_command("/account arterm logout"),
            Some(Ok(AccountCommand::ArtermLogout))
        ));
    }
    ...
    fn render_auth_doctor_markdown_includes_recovery_steps() {
        ...
        assert!(markdown.contains("arterm login --provider openai"));
        assert!(markdown.contains("Review current state: arterm auth status --json"));
    }
}
