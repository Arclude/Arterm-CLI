...
fn shell_command_quotes_single_quotes_for_handterm_exec() {
    let command = shell_command(&[
        "/tmp/arterm binary".to_string(),
        "--resume".to_string(),
        "session'quote".to_string(),
    ]);
    assert_eq!(
        command,
        "'/tmp/arterm binary' '--resume' 'session'\"'\"'quote'"
    );
}
...
fn resume_invocation_args_includes_socket_when_present() {
    let args = resume_invocation_args("ses_123", Some("/tmp/arterm-test.sock"));
    assert_eq!(
        args,
        vec![
            ...
            "ses_123".to_string(),
            "--socket".to_string(),
            "/tmp/arterm-test.sock".to_string()
        ]
    );
}
...
/// Pin ARTERM_HOME to a tempdir containing a `builds/current/arterm` binary so
...
/// other tests mutating ARTERM_HOME in parallel. Returns the guards that keep
...
fn pinned_resume_test_home() -> (
    ...
    std::fs::write(current.join("arterm"), b"#!/bin/sh\n").expect("write fake arterm binary");
    let home = EnvVarGuard::set_path("ARTERM_HOME", temp.path());
    (env_lock, temp, home)
}

#[test]
fn build_resume_command_uses_imported_arterm_session_for_claude_code() {
...
#[test]
fn build_resume_command_uses_imported_arterm_session_for_codex() {
    let _pinned = pinned_resume_test_home();
    let (program, args, title) = build_resume_command(
...
fn gather_ambient_info_filters_to_session_reminders_when_ambient_disabled() {
    ...
    // Read synchronously while the temporary ARTERM_HOME and its files are pinned:
    // an unrelated in-flight cache refresh can otherwise overwrite the cleared
    // process-global cache with data loaded under another test's ARTERM_HOME.
...
fn fresh_session_command_includes_fresh_spawn_and_socket() {
    ...
    assert_eq!(command.title.as_deref(), Some("arterm · new session"));
    assert_eq!(
        command.args,
        vec![
            "--fresh-spawn".to_string(),
