...
impl ActiveCredentialOverrides {
    fn from_app(app: &dyn TuiState) -> Self {
        Self {
            anthropic: app.active_dual_credential(arterm_provider_core::ActiveProvider::Claude),
            openai: app.active_dual_credential(arterm_provider_core::ActiveProvider::OpenAI),
        }
    }

    fn get(
        &self,
        provider: arterm_provider_core::ActiveProvider,
    ) -> Option<crate::auth::ActiveCredential> {
        match provider {
            arterm_provider_core::ActiveProvider::Claude => self.anthropic,
            arterm_provider_core::ActiveProvider::OpenAI => self.openai,
            _ => None,
        }
    }
}
...
fn auth_full_specs(
    ...
    fn dual_method_label(
        ...
        let runtime_provider = std::env::var("ARTERM_RUNTIME_PROVIDER").ok();
    ...
    let anthropic_label = provider_label(
        ...
        dual_method_label(arterm_provider_core::ActiveProvider::Claude, auth, active),
    );

    let openai_label = provider_label(
        "openai",
        auth.openai,
        dual_method_label(arterm_provider_core::ActiveProvider::OpenAI, auth, active),
    );
...
fn header_provider_auth_tag(
    ...
    if let Some(provider) = arterm_provider_core::parse_provider_hint(name) {
        use crate::auth::{ActiveCredential, resolve_dual_credential_auth};
        match resolve_dual_credential_auth(provider, auth, runtime_provider.as_deref()) {
            ...
            None if matches!(
                provider,
                arterm_provider_core::ActiveProvider::Claude
                    | arterm_provider_core::ActiveProvider::OpenAI
            ) =>
            {
                return "";
            }
...
fn version_display_candidates() -> Vec<String> {
    let full = format!("arterm {}", semver());
    let core = format!("arterm {}", semver_core());
    let minor = format!("arterm {}", semver_minor());
...
fn build_persistent_header_with_auth(
    ...
    let client_version_full = server_name
        .as_ref()
        .map(|_| arterm_build_meta::version().to_string());
    ...
    // still surface the running version on the arterm line's own row.
    if client_version_label.is_none() {
        let version_text = if is_running_stable_release() {
            let tag = arterm_build_meta::git_tag();
...
mod tests {
    ...
    fn ensure_test_arterm_home_if_unset() {
        static TEST_HOME: OnceLock<std::path::PathBuf> = OnceLock::new();

        if std::env::var_os("ARTERM_HOME").is_some() {
            return;
        }
        ...
        crate::env::set_var("ARTERM_HOME", path);
    }

    fn create_test_app() -> crate::tui::app::App {
        ensure_test_arterm_home_if_unset();
    ...
    fn persistent_header_labels_server_and_client_versions() {
        ...
        let client_version = compact_version_label(arterm_build_meta::version());
        assert!(
    ...
    fn persistent_header_keeps_git_hash_when_semvers_match_but_builds_differ() {
        let mut app = create_test_app();
        let client_semver = compact_version_label(arterm_build_meta::version());
        ...
        assert!(
            client_line.contains(&format!("· {}", arterm_build_meta::version())),
    ...
    fn header_model_display_name_sweeps_real_model_catalog() {
        ...
        // prettify_model_id, over the model ids arterm actually routes.
        let cases = [
            // Anthropic
    ...
    fn header_provider_auth_tag_reports_active_credential_for_openai() {
        ...
        let prev = std::env::var_os("ARTERM_RUNTIME_PROVIDER");
        ...
        if let Some(value) = prev {
            crate::env::set_var("ARTERM_RUNTIME_PROVIDER", value);
        }
    }
    ...
    fn header_provider_auth_tag_prefers_app_resolved_credential_over_env() {
        let _guard = crate::storage::lock_test_env();
        let prev = std::env::var_os("ARTERM_RUNTIME_PROVIDER");
        // The TUI client usually does not inherit ARTERM_RUNTIME_PROVIDER, so the
    ...
    fn header_provider_auth_tag_honors_runtime_selection_and_oauth_first() {
        ...
        crate::env::set_var("ARTERM_RUNTIME_PROVIDER", "claude-api");
        assert_eq!(
        ...
        crate::env::set_var("ARTERM_RUNTIME_PROVIDER", "claude");
        assert_eq!(
            header_provider_auth_tag("anthropic", &both, ActiveCredentialOverrides::default()),
            "oauth"
        );
    ...
    fn build_persistent_header_prefers_configured_model_during_remote_connect() {
        let _guard = crate::storage::lock_test_env();
        let prev_model = std::env::var_os("ARTERM_MODEL");
        let prev_provider = std::env::var_os("ARTERM_PROVIDER");
        crate::env::set_var("ARTERM_MODEL", "gpt-5.4");
        crate::env::set_var("ARTERM_PROVIDER", "openai");
        ...
        if let Some(prev_model) = prev_model {
            crate::env::set_var("ARTERM_MODEL", prev_model);
        } else {
            crate::env::remove_var("ARTERM_MODEL");
        }
        if let Some(prev_provider) = prev_provider {
            crate::env::set_var("ARTERM_PROVIDER", prev_provider);
        } else {
            crate::env::remove_var("ARTERM_PROVIDER");
        }
    }
    ...
    fn build_header_lines_omits_placeholder_provider_label_when_unknown() {
        ...
        // sibling test that sets ARTERM_MODEL=gpt-5.4 mid-flight leaks into this
    ...
    fn build_header_lines_hides_secondary_placeholder_during_brief_connecting_phase() {
        // Same env sensitivity as the placeholder test above: ARTERM_MODEL /
        // ARTERM_PROVIDER mutations from sibling tests change what renders.
        let _guard = crate::storage::lock_test_env();
        let app = crate::tui::app::App::new_for_remote(None);
    ...
    fn auth_status_line_marks_active_credential_when_both_configured() {
        ...
        let rendered_with = |runtime: Option<&str>| {
            match runtime {
                Some(value) => crate::env::set_var("ARTERM_RUNTIME_PROVIDER", value),
                None => crate::env::remove_var("ARTERM_RUNTIME_PROVIDER"),
            }
