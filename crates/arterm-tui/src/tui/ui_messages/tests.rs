...
fn render_compact_launch_and_divergence_notices_as_one_line() {
    ...
    let notices = [
        DisplayMessage::system(
            "Configured Arterm launch hotkeys (niri):\nSuper+; → arterm (/home/user/project)\n\nBound system-wide.",
        )
        .with_title("Launch hotkeys"),
        DisplayMessage::system(
            "Update diverged. Press Ctrl+Y to let a arterm agent merge local and upstream (or run `git pull` / `git rebase` yourself).",
        )
        .with_title("Update"),
    ];
...
fn render_assistant_message_plan_card_keeps_nested_fences_inside() {
    ...
    assert!(plain.contains("cargo test -p arterm-tui"), "plain: {plain}");
    ...
    let bash_line = lines
        .iter()
        .map(extract_line_text)
        .find(|line| line.contains("cargo test -p arterm-tui"))
...
fn render_system_message_uses_connection_card_for_reconnect_status() {
    let msg = DisplayMessage::system(
        "⚡ Connection lost - retrying (attempt 2, 7s) - connection reset by server · resume: arterm --resume koala",
    )
    .with_title("Connection");
    ...
    assert!(plain.contains("arterm --resume koala"));
}
...
fn render_swarm_message_centered_mode_caps_wrap_width_for_long_notifications() {
    ...
    let msg = DisplayMessage::swarm(
        "File activity",
        "/home/jeremy/arterm/src/tui/ui_messages.rs - moss just edited this file while you were working nearby, so the notification should still read as centered in wide layouts.",
    );
...
fn render_swarm_message_collapsed_shows_tldr_and_expand_badge_only() {
    let saved = crate::tui::markdown::center_code_blocks();
    crate::tui::markdown::set_center_code_blocks(false);
    let content = arterm_tui_messages::encode_collapsible_swarm_content(
...
fn render_swarm_message_expanded_shows_body_and_collapse_badge() {
    let saved = crate::tui::markdown::center_code_blocks();
    crate::tui::markdown::set_center_code_blocks(false);
    let collapsed = arterm_tui_messages::encode_collapsible_swarm_content(
        "fixed the flaky test",
        "The flaky test was caused by a race in the setup helper.",
    );
    let expanded =
        arterm_tui_messages::toggle_collapsible_swarm_content(&collapsed).expect("toggle");
...
fn render_tool_message_shows_intent_and_technical_preview_on_one_line() {
    ...
    let msg = DisplayMessage {
        ...
        tool_data: Some(crate::message::ToolCall {
            ...
            name: "bash".to_string(),
            input: serde_json::json!({
                "command": "cargo test -p arterm render_background_task --lib",
...
fn render_tool_message_marks_off_catalog_selection_without_fake_details() {
    let msg = discovery_message(
        "Selected off-catalog product 'firecrawl' for 'web-data'.\n\nSelection recorded as demand data. Arterm does not list or partner with this product, so no provider information, recommendation, or setup instructions are provided.",
        serde_json::json!({
            "action": "select",
            "category": "web-data",
            "tool": "firecrawl",
...
fn render_tool_message_shows_catalog_suggestion_receipt_and_trust_line() {
    ...
    assert!(plain.contains("Arterm maintainers only"), "{plain}");
...
fn discovery_cards_wrap_within_narrow_transcript_width() {
    let msg = discovery_message(
        "Catalog suggestion submitted.\n\nStatus: received for Arterm maintainer review.",
        serde_json::json!({
            "action": "suggest",
