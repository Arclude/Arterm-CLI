...
fn from_markdown_diagram_mode(
    mode: arterm_tui_markdown::DiagramDisplayMode,
...
) -> arterm_tui_markdown::MarkdownSpacingMode {
    match mode {
        crate::config::MarkdownSpacingMode::Compact => {
            arterm_tui_markdown::MarkdownSpacingMode::Compact
        }
        crate::config::MarkdownSpacingMode::Document => {
            arterm_tui_markdown::MarkdownSpacingMode::Document
        }
    }
}
...
) -> arterm_tui_markdown::LatexRenderingMode {
    match mode {
        crate::config::LatexRenderingMode::None => arterm_tui_markdown::LatexRenderingMode::None,
        crate::config::LatexRenderingMode::Unicode => {
            arterm_tui_markdown::LatexRenderingMode::Unicode
        }
        crate::config::LatexRenderingMode::Image => arterm_tui_markdown::LatexRenderingMode::Image,
    }
}

pub fn install_arterm_markdown_hooks() {
    arterm_tui_markdown::set_latex_log_hook(|error| {
        crate::logging::warn(&format!(
            "LaTeX image rendering fell back to Unicode: {error}"
        ));
    });
    arterm_tui_markdown::set_config_snapshot_hook(|| {
        let cfg = crate::config::config();
        arterm_tui_markdown::MarkdownConfigSnapshot {
    ...
    arterm_tui_markdown::set_memory_snapshot_hook(|| {
        let snapshot = crate::process_memory::snapshot_with_source("client:markdown:memory");
        arterm_tui_markdown::ProcessMemorySnapshot {
...
pub fn set_diagram_mode_override(mode: Option<crate::config::DiagramDisplayMode>) {
    arterm_tui_markdown::set_diagram_mode_override(mode.map(to_markdown_diagram_mode));
}

pub fn get_diagram_mode_override() -> Option<crate::config::DiagramDisplayMode> {
    arterm_tui_markdown::get_diagram_mode_override().map(from_markdown_diagram_mode)
}
...
pub fn with_diagram_mode_scope<T>(
    ...
    arterm_tui_markdown::with_diagram_mode_scope(to_markdown_diagram_mode(mode), f)
}

pub fn with_deferred_mermaid_render_context<T>(f: impl FnOnce() -> T) -> T {
    arterm_tui_markdown::with_deferred_mermaid_render_context(f)
}
