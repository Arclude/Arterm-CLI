...
fn test_tool_summary_bash_truncation_keeps_start_and_end() {
    let tool = ToolCall {
        ...
        name: "bash".to_string(),
        input: serde_json::json!({
            "command": "cargo test --package arterm --lib tui::ui::tests::render_tool_message_batch_flat_subcall_params_include_read_details -- --nocapture"
        }),
        intent: None,
        thought_signature: None,
    };
...
fn test_tool_summary_bash_keeps_full_command_when_width_fits() {
    let tool = ToolCall {
        ...
        name: "bash".to_string(),
        input: serde_json::json!({
            "command": "cargo test --package arterm --lib tui::ui::tests::render_tool_message_batch_rows_do_not_soft_wrap_on_narrow_width -- --nocapture"
        }),
        intent: None,
        thought_signature: None,
    };
    ...
    assert_eq!(
        summary,
        "$ cargo test --package arterm --lib tui::ui::tests::render_tool_message_batch_rows_do_not_soft_wrap_on_narrow_width -- --nocapture"
    );
    assert!(!summary.contains('…'), "summary={summary:?}");
}
...
fn test_render_batch_subcall_line_keeps_full_bash_summary_when_row_fits() {
    ...
    assert!(
        rendered.contains("bash $ cargo test --package arterm"),
        "rendered={rendered:?}"
    );
...
fn test_render_tool_message_keeps_token_badge_when_intent_is_truncated() {
    let msg = DisplayMessage {
        ...
        tool_data: Some(ToolCall {
            ...
            name: "bash".to_string(),
            input: serde_json::json!({
                "command": "cargo test --package arterm --lib tui::ui::tests::very_long_test_name -- --nocapture"
            }),
            intent: Some(
...
/// Regression for https://github.com/Arclude/Arterm-CLI/issues/284:
