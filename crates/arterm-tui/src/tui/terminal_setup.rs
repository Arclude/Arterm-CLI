...
//! arterm already requests it at startup, and crossterm decodes it, so on any
...
/// A terminal whose Shift+Enter behavior arterm can configure.
...
impl SetupTarget {
    ...
    pub fn activation_note(self) -> &'static str {
        match self {
            ...
            Self::Tmux => "Run `tmux source ~/.tmux.conf`, then restart arterm.",
...
///
/// `extended-keys on` only forwards when the app asks (arterm does), which is
...
# arterm: let Shift+Enter reach the application instead of collapsing to Enter.
...
pub const MANAGED_MARKER: &str = "# arterm: let Shift+Enter reach the application";

/// Whether `config` already contains arterm's managed tmux block.
...
/// Configure tmux so a capable outer terminal's Shift+Enter reaches arterm.
        ...
        std::fs::write(
            ...
            format!(
                "local wezterm = require 'wezterm'\n\
                 ...
                 -- arterm: report Shift+Enter distinctly from Enter.\n\
                 {WEZTERM_CONFIG_LINE}\n\
                 return config\n"
            ),
        )?;
