...
fn parse_changelog() -> Vec<ChangelogEntry<'static>> {
    let changelog: &'static str = arterm_build_meta::CHANGELOG;
    parse_changelog_from_impl(changelog)
}
...
/// Reads the last-seen commit hash from ~/.arterm/last_seen_changelog,
...
pub(super) fn get_unseen_changelog_entries() -> &'static Vec<String> {
    ...
    ENTRIES.get_or_init(|| {
        ...
        let state_file = dirs::home_dir()
            .map(|h| h.join(".arterm").join("last_seen_changelog"))
            .unwrap_or_else(|| std::path::PathBuf::from(".arterm/last_seen_changelog"));
