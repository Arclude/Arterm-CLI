...
pub(super) fn draw_help_overlay(frame: &mut Frame, area: Rect, scroll: usize, app: &dyn TuiState) {
    ...
    let alt = arterm_tui_core::keybind::alt_chord;
    ...
    lines.push(help_entry(
        "/subscription",
        "Inspect arterm subscription scaffold",
    ));
    lines.push(help_entry(
        "/subscribe",
        "Why and how to subscribe to arterm",
    ));
    ...
    lines.push(help_entry("/quit", "Exit arterm"));
    ...
    if let Some(label) = crate::tui::keybind::load_new_terminal_key().label {
        lines.push(key_entry(
            &label,
            "Spawn new arterm session in a new terminal",
        ));
    }
...
pub(super) fn draw_model_status_overlay(
    ...
    lines.push(Line::from(Span::styled(
        "  Live verification evidence for provider/model behavior in arterm",
        dim_style,
    )));
    lines.push(Line::from(""));
...
fn model_status_line_style(raw: &str, default: Style) -> Style {
    ...
    // and `arterm provider-test-coverage` stay color-consistent.
