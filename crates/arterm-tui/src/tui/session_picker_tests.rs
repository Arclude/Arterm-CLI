...
fn onboarding_external_filter_picks_latest_visible_transcript() {
    ...
    let arterm = make_session("arterm_one", "arterm", false, SessionStatus::Closed);

    let mut picker = SessionPicker::new(vec![older, arterm, newer]);
...
fn onboarding_external_filter_with_no_matches_has_no_target() {
    let arterm = make_session("arterm_only", "arterm", false, SessionStatus::Closed);
    let mut picker = SessionPicker::new(vec![arterm]);
...
fn onboarding_banner_renders_prompt_and_both_action_rows() {
    ...
    let welcome_y = lines
        .iter()
        .position(|line| line.contains("Welcome to arterm"))
        .expect("welcome row");
    let review_y = lines
        .iter()
...
//
//   cargo test -p arterm-tui --lib --release -- --ignored --nocapture benchmark_resume_op
//
