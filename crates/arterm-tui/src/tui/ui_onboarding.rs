...
//!   2. "Welcome to arterm onboarding" title.
...
/// everywhere. Override with `ARTERM_ROUNDED_PILLS=on|off`.
fn rounded_pill_caps_supported() -> bool {
    ...
    *SUPPORTED.get_or_init(|| {
        ...
        if let Ok(raw) = std::env::var("ARTERM_ROUNDED_PILLS") {
...
/// use a Arterm subscription, with secondary controls for a selective import and
/// telemetry settings.
        ...
        OnboardingWelcomeKind::Login {
            ...
            match import {
                ...
                None if error.is_some() => {
                    ...
                    // to hand the fix to it (it can run arterm's auth-test and add
