...
// `arterm-terminal-image` crate (shared with the `read` tool). Re-exported here
...
pub mod mermaid;
pub mod permissions {
    pub use arterm_tui_permissions::*;
}
mod redraw_schedule;
...
pub trait TuiState {
    ...
    /// from the `ARTERM_RUNTIME_PROVIDER` env var. The header must prefer this
    /// over its own env-based heuristic: the TUI client process often does not
    /// inherit `ARTERM_RUNTIME_PROVIDER` (it is set inside the agent/server
    ...
    fn active_dual_credential(
        &self,
        _provider: arterm_provider_core::ActiveProvider,
    ...
    /// Number of arterm compactions already applied to this session, when known.
...
fn supports_reliable_zero_cache_read_warning(
    ...
    // OpenRouter/Arterm-subscription routes can only be treated as reliable for zero-read
...
pub struct LoginImportPrompt {
    ...
    /// Whether the environment (ARTERM_NO_TELEMETRY / DO_NOT_TRACK) already
...
pub enum ImportSummaryPill {
    ...
    Continue,
    /// Sign in with a Arterm subscription instead of importing.
    Subscription,
...
pub(crate) fn subscribe_metadata(
    ...
    resolve_subscribe_metadata(
        ...
        remote_working_dir,
        arterm_selfdev_types::client_selfdev_requested(),
    )
}
...
pub(crate) fn resolve_subscribe_metadata(
    ...
    if !selfdev && let Some(dir) = client_working_dir {
        ...
        while let Some(path) = current {
            if crate::build::is_arterm_repo(path) {
                selfdev = true;
                break;
            }
            current = path.parent();
...
pub fn display_messages_from_session(session: &crate::session::Session) -> Vec<DisplayMessage> {
    let mut messages = arterm_tui_messages::display_messages_from_rendered_messages(
