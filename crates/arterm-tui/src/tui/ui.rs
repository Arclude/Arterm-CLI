...
pub(crate) use arterm_tui_markdown::{CopyTargetKind, RawCopyTarget};
pub(crate) use arterm_tui_messages::{
...
/// arterm-tui's intermittent layout failures.
///
...
impl CopyViewportSnapshot {
    ...
    fn inline_image_id_for_label_line(&self, abs_line: usize) -> Option<u64> {
        ...
        prepared
            ...
            .iter()
            .find(|region| {
                region.render == arterm_tui_messages::ImageRegionRender::Fit
...
pub fn draw(frame: &mut Frame, app: &dyn TuiState) {
    ...
    arterm_tui_style::adapt_buffer_for_theme(frame.buffer_mut());
    arterm_tui_style::palette::adapt_buffer_for_palette(frame.buffer_mut());
...
fn draw_inner(frame: &mut Frame, app: &dyn TuiState) {
    ...
    if let Some(overlay) = app.settings_overlay() {
    ...
    if let Some(overlay) = app.mcp_picker_overlay() {
        overlay.render(frame);
        finalize_frame_metrics(
            app,
            total_start,
