...
use super::{
    ...
    ServerGroup, SessionInfo,
};

use super::{ResumeTarget, SessionSource};

// Search-index construction and query matching live in `search.rs`; the
// helpers are re-exported here so existing `loading::` call sites (including
// the picker's own module tree and tests) keep resolving unchanged.
#[cfg(test)]
pub(super) use super::search::session_matches_picker_query;
#[cfg(test)]
pub(super) use super::search::session_matches_query;
pub(super) use super::search::{
    MESSAGE_SEARCH_EXCERPT_BYTES, build_search_index, build_search_index_from_summary,
    push_raw_search_excerpt, raw_value_display_text, raw_value_search_excerpt,
};

fn session_scan_limit() -> usize {
    std::env::var("ARTERM_SESSION_PICKER_MAX_SESSIONS")
        .ok()
...
/// for users who only ever want arterm's own sessions, so `[display]
/// external_sessions = false` (or `ARTERM_EXTERNAL_SESSIONS=0`) opts out
...
fn include_old_saved_sessions_on_initial_load() -> bool {
    std::env::var("ARTERM_SESSION_PICKER_INCLUDE_OLD_SAVED")
        .ok()
...
fn session_list_disk_cache_path() -> Result<PathBuf> {
    Ok(storage::arterm_dir()?.join("cache/session-picker-list-v1.json"))
}
...
pub fn load_cached_sessions_grouped() -> Option<(Vec<ServerGroup>, Vec<SessionInfo>)> {
    let sessions_dir = storage::arterm_dir().ok()?.join("sessions");
...
/// Parse a single arterm session snapshot (+ journal) into a [`SessionInfo`],
...
pub fn load_sessions() -> Result<Vec<SessionInfo>> {
    ...
    let (mut sessions, external_sessions) = std::thread::scope(|scope| {
        ...
        // default list appear to jump from a handful of Arterm rows straight to old
    ...
    sessions
        ...
        .map(|session| {
            ...
            let short_name = working_dir
                ...
                .unwrap_or_else(|| {
                    format!("claude {}", arterm_core::util::truncate_str(&session_id, 8))
                });
...
fn load_codex_session_stub(path: &Path) -> Result<Option<SessionInfo>> {
    ...
    let working_dir = meta
        ...
        .and_then(|v| v.as_str())
        .map(|s| s.to_string());
    let short_name = format!("codex {}", arterm_core::util::truncate_str(&session_id, 8));
    let title = format!(
        "Codex session {}",
...
fn load_pi_session_stub(path: &Path) -> Result<Option<SessionInfo>> {
    ...
    let working_dir = header
        ...
        .and_then(|v| v.as_str())
        .map(|s| s.to_string());
    let short_name = format!("pi {}", arterm_core::util::truncate_str(&session_id, 8));
    let title = format!(
        "Pi session {}",
        arterm_core::util::truncate_str(&session_id, 8)
    );
...
fn load_cursor_session_stub(path: &Path) -> Result<Option<SessionInfo>> {
    ...
    let short_name = working_dir
        ...
        .and_then(|name| name.to_str())
        .map(|name| name.to_string())
        .unwrap_or_else(|| format!("cursor {}", arterm_core::util::truncate_str(&session_id, 8)));
