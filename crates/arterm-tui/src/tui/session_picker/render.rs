...
impl SessionPicker {
    fn running_spinner_frame() -> usize {
        std::time::SystemTime::now()
            ...
            .saturating_div(u128::from(
                arterm_tui_render::swarm_gallery::STRIP_SPINNER_FRAME_MS,
            )) as usize
    }
    ...
    pub(super) fn render_session_item_lines_at_frame(
        ...
        let live_badge = if self.session_is_live(session) {
            if session.source == SessionSource::ClaudeCode {
                ...
                Some((
                    arterm_tui_render::swarm_gallery::STRIP_SPINNER_FRAMES[spinner_frame
                        % arterm_tui_render::swarm_gallery::STRIP_SPINNER_FRAMES.len()],
                    rgb(255, 193, 7),
                    label,
                ))
            } else {
    ...
    pub(super) fn render_session_list(&mut self, frame: &mut Frame, area: Rect) {
        ...
        let spinner = arterm_tui_render::swarm_gallery::STRIP_SPINNER_FRAMES
            [spinner_frame % arterm_tui_render::swarm_gallery::STRIP_SPINNER_FRAMES.len()];
        ...
        let items: Vec<ListItem> = if let Some(message) = self.loading_message.as_deref() {
            ...
            && self.filter_mode == arterm_tui_session_picker::SessionFilterMode::Active
        {
            vec![
                ListItem::new(Line::from(vec![Span::styled(
        ...
        } else if self.filter_mode == arterm_tui_session_picker::SessionFilterMode::Active {
