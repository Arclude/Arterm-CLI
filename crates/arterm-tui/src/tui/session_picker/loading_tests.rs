...
/// appear in the picker, while arterm's own sessions still do.
#[test]
fn load_sessions_hides_external_sessions_when_opted_out() {
    let _env_lock = crate::storage::lock_test_env();
    let temp = tempfile::tempdir().expect("temp dir");
    let _home = EnvVarGuard::set_path("ARTERM_HOME", temp.path());
    let _opt_out = EnvVarGuard::set_str("ARTERM_EXTERNAL_SESSIONS", "0");
    ...
    assert!(
        !sessions
            .iter()
            .any(|session| session.source != SessionSource::Arterm),
...
fn load_sessions_includes_saved_sessions_beyond_scan_limit() {
    ...
    let temp = tempfile::tempdir().expect("temp dir");
    let _home = EnvVarGuard::set_path("ARTERM_HOME", temp.path());
    let _scan_limit = EnvVarGuard::set_str("ARTERM_SESSION_PICKER_MAX_SESSIONS", "50");
...
#[test]
fn session_matches_query_searches_arterm_transcript_contents() {
    let _env_lock = crate::storage::lock_test_env();
    let temp = tempfile::tempdir().expect("temp dir");
    let _home = EnvVarGuard::set_path("ARTERM_HOME", temp.path());
...
fn benchmark_real_resume_loading_phases() {
    ...
    let sessions_dir = storage::arterm_dir().expect("arterm dir").join("sessions");
    ...
    let arterm_summary_elapsed = phase_start.elapsed();
    ...
    eprintln!(
        concat!(
            ...
            "arterm_summary={}ms arterm_loaded={} skipped_empty={} skipped_imported={} summary_errors={} ",
        ...
        arterm_summary_elapsed.as_millis(),
        sessions.len(),
        skipped_empty,
        skipped_imported,
        summary_errors,
...
#[test]
#[ignore = "developer benchmark: scans the real ARTERM_HOME session directory"]
...
fn onboarding_scoped_loader_returns_only_codex_sessions() {
    ...
    // A arterm session that must NOT appear in the scoped Codex view (the whole
    // point of the scoped loader is to skip parsing these on onboarding).
    let mut arterm_session = Session::create_with_id(
        "session_onboarding_arterm_1780000000000".to_string(),
        Some("/tmp/arterm-onboard".to_string()),
        Some("Arterm Onboarding".to_string()),
    );
    arterm_session.append_stored_message(crate::session::StoredMessage {
        id: "msg-1".to_string(),
        role: crate::message::Role::User,
        content: vec![crate::message::ContentBlock::Text {
    ...
    arterm_session.save().expect("save arterm session");
    ...
    assert!(
        ...
        "scoped Codex load must not include arterm/other-CLI sessions"
    );
}
...
fn hidden_debug_sessions_do_not_consume_default_resume_budget() {
    ...
    // loader stopped after the first 50, leaving no ordinary Arterm sessions for
...
/// arterm's own sessions, and toggling that must not be masked by either cache.
mod external_session_opt_out {
    ...
    fn disk_cache_written_with_externals_is_rejected_after_opting_out() {
        let dir = PathBuf::from("/tmp/arterm-test-sessions");
    ...
    fn legacy_disk_cache_without_the_field_defaults_to_externals_included() {
        let json = serde_json::json!({
            "version": super::super::SESSION_LIST_DISK_CACHE_VERSION,
            "generated_at": chrono::Utc::now(),
            "sessions_dir": "/tmp/arterm-test-sessions",
            "scan_limit": 50,
    ...
    fn external_sessions_default_is_on() {
        assert!(arterm_config_types::DisplayConfig::default().external_sessions);
    }
}
