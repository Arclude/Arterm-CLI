...
impl BridgeState {
    ...
    pub fn api_request_to_legacy(&mut self, request: &Value) -> Vec<Outbound> {
        ...
        match req {
            ...
            "create_session" | "attach_session" => {
                ...
                // Sessions rooted inside a arterm checkout are self-dev
                ...
                if working_dir
                    .as_deref()
                    .is_some_and(Self::path_is_inside_arterm_repo)
                {
            ...
            "set_api_key" | "clear_api_key" => {
                ...
                let Some((provider, env_keys, file_name)) = Self::credential_binding(provider)
                    ...
                    return Self::error_reply(
                        api_id,
                        ErrorCode::InvalidRequest,
                        "unsupported API-key provider; supported: claude-api, openai-api, openrouter, cursor, gemini, arterm",
                    );
                };
    ...
    /// True when `path`, or any ancestor, looks like a arterm source checkout.
    ///
    ...
    fn path_is_inside_arterm_repo(path: &str) -> bool {
        let mut current = Some(std::path::Path::new(path));
        while let Some(dir) = current {
            if dir.join("Cargo.toml").is_file() && dir.join("crates/arterm-base").is_dir() {
                return true;
            }
            current = dir.parent();
        }
    ...
    /// plain session id.
    ///
    /// One funnel for three reasons. It honours `ARTERM_HOME`, without which a
    /// launched instance reads the *user's* sessions: `peek_session` served
    /// the real transcripts of the arterm the user runs interactively, which
    ...
    fn session_record_path(session_id: &str) -> Option<std::path::PathBuf> {
        ...
        let home = match std::env::var_os("ARTERM_HOME") {
            Some(home) => std::path::PathBuf::from(home),
            None => std::path::Path::new(&std::env::var_os("HOME")?).join(".arterm"),
        };
    ...
    fn arterm_home() -> Option<std::path::PathBuf> {
        std::env::var_os("ARTERM_HOME")
            .map(std::path::PathBuf::from)
            .or_else(|| {
                std::env::var_os("HOME").map(|home| std::path::Path::new(&home).join(".arterm"))
            })
    }

    fn stored_session_ids() -> Vec<String> {
        let Some(home) = Self::arterm_home() else {
            return Vec::new();
        };
    ...
    fn archive_state_path() -> Option<std::path::PathBuf> {
        Some(Self::arterm_home()?.join("sdk-archive.json"))
    }
    ...
    fn app_config_dir() -> Option<std::path::PathBuf> {
        if let Some(home) = std::env::var_os("ARTERM_HOME") {
            return Some(std::path::Path::new(&home).join("config/arterm"));
        }
        #[cfg(target_os = "macos")]
        return Some(
            std::path::Path::new(&std::env::var_os("HOME")?)
                .join("Library/Application Support/arterm"),
        );
        #[cfg(target_os = "windows")]
        return Some(std::path::Path::new(&std::env::var_os("APPDATA")?).join("arterm"));
    ...
    fn credential_binding(
        ...
        match provider.trim().to_ascii_lowercase().as_str() {
            ...
            "arterm" | "subscription" | "arterm-subscription" => {
                Some(("arterm", &["ARTERM_API_KEY"], "arterm-subscription.env"))
            }
            _ => None,
        }
    }
    ...
    fn write_credential(
        ...
        let dir = Self::app_config_dir()
            .ok_or_else(|| "could not resolve arterm config directory".to_string())?;
