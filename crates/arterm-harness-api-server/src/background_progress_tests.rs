...
fn the_daemons_own_formatter_round_trips() {
    ...
    use arterm_base::bus::BackgroundTaskProgressEvent;
    ...
    let event = BackgroundTaskProgressEvent {
        ...
        progress: BackgroundTaskProgress {
            ...
            message: Some("Running arterm-desktop2 tests".into()),
            eta_seconds: Some(90),
    ...
    let rendered = arterm_base::message::format_background_task_progress_markdown(&event);
    let parsed = parse(&rendered);
    assert_eq!(parsed.task_id, "t42");
    assert_eq!(parsed.label, "workspace tests");
    assert_eq!(parsed.percent, Some(37.0));
    assert!(
        parsed.summary.contains("Running arterm-desktop2 tests"),
        "the status line was lost: {rendered}"
    );
    assert!(!parsed.done);

    // And the completion shape, from the same source of truth.
    let done = arterm_base::message::format_background_task_notification_markdown(
        &arterm_base::bus::BackgroundTaskCompleted {
            session_id: "s1".into(),
            task_id: "t42".into(),
            tool_name: "bash".into(),
            display_name: Some("workspace tests".into()),
            status: arterm_background_types::BackgroundTaskStatus::Completed,
            exit_code: Some(0),
            duration_secs: 12.5,
