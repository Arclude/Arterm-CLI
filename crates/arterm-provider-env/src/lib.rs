...
use arterm_provider_metadata::{is_safe_env_file_name, is_safe_env_key_name};
...
pub fn load_api_key_from_env_or_config(env_key: &str, file_name: &str) -> Option<String> {
    ...
    let config_path = arterm_storage::app_config_dir().ok()?.join(file_name);
    arterm_storage::harden_secret_file_permissions(&config_path);
...
/// Load a value only from the saved env file under the arterm config dir,
...
pub fn save_env_value_to_env_file(
    ...
    let config_dir = arterm_storage::app_config_dir()?;
    let file_path = config_dir.join(file_name);
    arterm_storage::upsert_env_file_value(&file_path, env_key, value)?;

    if let Some(value) = value {
        arterm_core::env::set_var(env_key, value);
    } else {
        arterm_core::env::remove_var(env_key);
    }
...
mod tests {
    ...
    fn loads_api_key_from_env_before_config_file() {
        ...
        let _guard = EnvGuard::new(&["ARTERM_HOME", "ARTERM_PROVIDER_ENV_TEST_KEY"]);
    ...
    fn loads_and_removes_values_from_sandboxed_config_file() {
        let temp = tempfile::tempdir().expect("tempdir");
        let _guard = EnvGuard::new(&["ARTERM_HOME", "ARTERM_PROVIDER_ENV_TEST_VALUE"]);
        arterm_core::env::set_var("ARTERM_HOME", temp.path());

        save_env_value_to_env_file(
            "ARTERM_PROVIDER_ENV_TEST_VALUE",
            "provider-env-test.env",
            Some("file-value"),
        )
        .expect("save file value");

        arterm_core::env::remove_var("ARTERM_PROVIDER_ENV_TEST_VALUE");
        assert_eq!(
    ...
    fn accepts_legacy_zai_key_for_zhipu() {
        let temp = tempfile::tempdir().expect("tempdir");
        let _guard = EnvGuard::new(&["ARTERM_HOME", "ZHIPU_API_KEY", "ZAI_API_KEY"]);
        arterm_core::env::set_var("ARTERM_HOME", temp.path());

        save_env_value_to_env_file("ZAI_API_KEY", "zai.env", Some("legacy-zai-key"))
            .expect("save legacy key");
        arterm_core::env::remove_var("ZAI_API_KEY");
    ...
    fn loads_api_key_with_zero_width_space_from_config_file() {
        let temp = tempfile::tempdir().expect("tempdir");
        let _guard = EnvGuard::new(&["ARTERM_HOME", "ARTERM_PROVIDER_FOO_API_KEY"]);
        arterm_core::env::set_var("ARTERM_HOME", temp.path());

        // Write an env file with a U+200B zero-width space prefixed onto the key,
        // mirroring issue #376's reproduction.
        let config_dir = arterm_storage::app_config_dir().expect("config dir");
        std::fs::create_dir_all(&config_dir).expect("create config dir");
        std::fs::write(
            config_dir.join("provider-foo.env"),
            "ARTERM_PROVIDER_FOO_API_KEY=\u{200B}sk-mykey123\n",
        )
        .expect("write env file");

        assert_eq!(
            load_api_key_from_env_or_config("ARTERM_PROVIDER_FOO_API_KEY", "provider-foo.env")
                .as_deref(),
            Some("sk-mykey123")
        );
    }
    ...
    fn loads_api_key_with_invisible_chars_from_env_var() {
        let temp = tempfile::tempdir().expect("tempdir");
        let _guard = EnvGuard::new(&["ARTERM_HOME", "ARTERM_PROVIDER_BAR_API_KEY"]);
        arterm_core::env::set_var("ARTERM_HOME", temp.path());
        // NBSP + BOM padding around the env-provided key.
        arterm_core::env::set_var("ARTERM_PROVIDER_BAR_API_KEY", "\u{00A0}sk-env-key\u{FEFF}");

        assert_eq!(
            load_api_key_from_env_or_config("ARTERM_PROVIDER_BAR_API_KEY", "provider-bar.env")
                .as_deref(),
            Some("sk-env-key")
        );
    }
