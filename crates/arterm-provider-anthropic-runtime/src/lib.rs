...
//! `arterm-base` so provider edits compile only this crate plus a binary
...
//! loading, cache-TTL toggles) stay in `arterm_base::provider::anthropic`;
...
use arterm_base::auth;
use arterm_base::auth::oauth;
use arterm_provider_core::{EventStream, NativeToolResultSender, Provider};
fn oauth_beta_headers(model: &str) -> &'static str {
    arterm_provider_core::anthropic_oauth_beta_headers(model)
}
...
#[cfg(test)]
use arterm_base::provider::anthropic::{OAUTH_BETA_HEADERS, effectively_1m};
#[cfg(test)]
use arterm_message_types::{ContentBlock, Role};
use arterm_message_types::{Message, StreamEvent, ToolDefinition};
#[cfg(test)]
use arterm_provider_anthropic::{ApiContentBlock, ToolResultContent, ToolResultContentBlock};
use arterm_provider_anthropic::{
...
fn load_official_claude_client_metadata() -> OAuthClientMetadata {
    let path = match arterm_base::storage::user_home_path(".claude.json") {
        Ok(path) => path,
...
async fn oauth_preflight_get(
    ...
    if !resp.status().is_success() {
        ...
        let body = arterm_base::util::http_error_body(resp, "HTTP error").await;
...
fn record_oauth_preflight_result(label: &str, result: Result<()>) -> bool {
    match result {
        Ok(()) => true,
        Err(err) => {
            arterm_base::logging::warn(&format!(
...
async fn ensure_oauth_preflight(
    ...
    let eval = OAuthEvalRequest {
        attributes: OAuthEvalAttributes {
            ...
            subscription_type: arterm_base::auth::claude::get_subscription_type()
    ...
    if all_ok {
        arterm_base::logging::info("Claude OAuth preflight completed successfully");
    }
    Ok(())
}

/// Quality-first default shared with model routing and first-run selection.
const DEFAULT_MODEL: &str = arterm_provider_core::DEFAULT_CLAUDE_MODEL;
...
pub struct AnthropicProvider {
    ...
    /// Explicit `ARTERM_ANTHROPIC_MAX_TOKENS` override. When unset, the output
...
impl AnthropicProvider {
    fn is_usage_exhausted() -> bool {
        let usage = arterm_base::usage::get_sync();
        usage.five_hour >= 0.99 && usage.seven_day >= 0.99
    }

    fn best_available_opus_model(exclude: &str) -> Option<String> {
        let mut models = arterm_base::provider::cached_anthropic_model_ids()
            .unwrap_or_else(arterm_base::provider::known_anthropic_model_ids);
        models.retain(|model| {
    ...
    fn fallback_for_model_scoped_usage(
        selected_model: &str,
        usage: &arterm_base::usage::UsageData,
    ) -> Option<String> {
    ...
    async fn model_after_oauth_quota_check(
        ...
        let Ok(usage) = arterm_base::usage::fetch_usage_for_access_token(token).await else {
            return selected_model;
        };
    ...
    /// `ARTERM_RUNTIME_PROVIDER` happens to be in the current process (e.g. a
    /// self-dev session may have it set to `claude-api`). This also updates
    /// `ARTERM_RUNTIME_PROVIDER` so any provider instances the probes build
    ...
    pub async fn fetch_live_model_ids_for_doctor(&self) -> Result<Vec<String>> {
        ...
        let catalog = if is_oauth {
            arterm_base::provider::fetch_anthropic_model_catalog_oauth(&token).await?
        } else {
            arterm_base::provider::fetch_anthropic_model_catalog(&token).await?
        };
        // Persist so the rest of the process benefits from the warm catalog,
        // exactly like the runtime's own prefetch.
        arterm_base::provider::persist_anthropic_model_catalog(&catalog);
        if !catalog.context_limits.is_empty() {
            arterm_base::provider::populate_context_limits(catalog.context_limits.clone());
        }
        if !catalog.available_models.is_empty() {
            arterm_base::provider::populate_anthropic_models(catalog.available_models.clone());
        }
        Ok(catalog.available_models)
    }

    pub fn new() -> Self {
        let model = std::env::var("ARTERM_ANTHROPIC_MODEL").unwrap_or_else(|_| {
        ...
        let _ = tokio::runtime::Handle::try_current().map(|_| {
            tokio::spawn(async {
                let _ = arterm_base::usage::get().await;
            })
        });

        let max_tokens_override = std::env::var("ARTERM_ANTHROPIC_MAX_TOKENS")
            .ok()
            .and_then(|v| v.trim().parse::<u32>().ok());
        let reasoning_effort = arterm_base::config::config()
            .provider
    ...
    fn model_supports_output_effort(model: &str) -> bool {
        ...
        // generations); see `arterm_provider_core::anthropic_reasoning_caps`.
        // Fable 5 verified live 2026-07-01; Sonnet 5 verified live 2026-07-07.
        arterm_provider_core::anthropic_reasoning_caps(model).output_effort
    }

    fn model_supports_adaptive_thinking(model: &str) -> bool {
        arterm_provider_core::anthropic_reasoning_caps(model).adaptive_thinking
    }

    fn model_supports_manual_thinking(model: &str) -> bool {
        arterm_provider_core::anthropic_reasoning_caps(model).manual_thinking
    }

    fn model_supports_xhigh_effort(model: &str) -> bool {
        arterm_provider_core::anthropic_reasoning_caps(model).xhigh_effort
    }
    ...
    fn model_supports_max_effort(model: &str) -> bool {
        arterm_provider_core::anthropic_reasoning_caps(model).max_effort
    }

    fn model_supports_reasoning_effort(model: &str) -> bool {
        arterm_provider_core::anthropic_reasoning_caps(model).supports_reasoning_effort()
    }
    ...
    fn actual_effort_for_model(model: &str, effort: &str) -> String {
        if arterm_base::prompt::is_swarm_effort(effort) {
    ...
    fn store_effort_for_model(model: &str, effort: &str) -> String {
        if arterm_base::prompt::is_deep_swarm_effort(effort) {
            arterm_base::prompt::SWARM_DEEP_EFFORT.to_string()
        } else if arterm_base::prompt::is_swarm_effort(effort) {
            arterm_base::prompt::SWARM_EFFORT.to_string()
        } else {
    ...
    fn max_tokens_for(&self, model: &str) -> u32 {
        self.max_tokens_override
            .unwrap_or_else(|| arterm_provider_core::anthropic::anthropic_max_output_tokens(model))
    }
    ...
    fn manual_thinking_budget(effort: &str, max_tokens: u32) -> Option<u32> {
        let desired = match effort {
            ...
            "high" => 8_192,
            "xhigh" | "max" => 16_384,
            e if arterm_base::prompt::is_swarm_effort(e) => 16_384,
            _ => return None,
        };
    ...
    fn build_reasoning_request_parts(
        ...
        let show_thinking = arterm_base::config::config().display.show_thinking;
    ...
    async fn get_access_token(&self) -> Result<(String, bool)> {
        ...
        // arterm OAuth account may still deserialize successfully even after its
    ...
    async fn get_oauth_access_token(&self) -> Result<(String, bool)> {
        ...
        {
            anyhow::bail!(
                "Claude OAuth credentials are missing the required user:inference scope (scopes: {}). Run `arterm login --provider claude` to mint a fresh Claude.ai OAuth token, or import/use a fresh Claude Code login.",
        ...
        if fresh_creds.expires_at < now + 300_000 && !fresh_creds.refresh_token.is_empty() {
            ...
            if auth::refresh_state::refresh_token_is_known_rejected(
                ...
                anyhow::bail!(
                    "Claude OAuth refresh token was previously rejected by Anthropic and cannot be refreshed. Run `arterm login --provider claude` to mint a fresh token."
                );
            }

            arterm_base::logging::info(
            ...
            {
                Ok(refreshed) => {
                    arterm_base::logging::info("OAuth token refreshed successfully");
                ...
                Err(e) => {
                    arterm_base::logging::error(&format!("OAuth token refresh failed: {}", e));
        ...
        if fresh_creds.expires_at <= now {
            anyhow::bail!(
                "Claude OAuth token is expired and no usable refresh token is available. Run `arterm login --provider claude` to refresh OAuth credentials"
            );
        }
    ...
    pub(crate) fn set_credential_mode(&self, mode: AnthropicCredentialMode) -> Result<()> {
        ...
        *mode_guard = mode;
        ...
        if let Some(route) = mode.auth_route(arterm_provider_core::DualAuthProvider::Anthropic) {
            arterm_base::env::set_var("ARTERM_RUNTIME_PROVIDER", route.runtime_provider_key());
        }
        ...
        arterm_base::auth::AuthStatus::invalidate_cached_status();
        Ok(())
    }
    ...
    fn format_messages(&self, messages: &[Message], is_oauth: bool) -> Vec<ApiMessage> {
        arterm_provider_anthropic::format_messages(messages, is_oauth)
    }
    ...
    fn format_content_blocks(
        ...
        arterm_provider_anthropic::format_content_blocks(blocks, is_oauth)
    }
    ...
    fn format_tools(&self, tools: &[ToolDefinition], is_oauth: bool) -> Vec<ApiTool> {
        arterm_provider_anthropic::format_tools(tools, is_oauth, is_cache_ttl_1h())
    }
}
...
fn log_anthropic_canonical_input(
    ...
    arterm_provider_core::fingerprint::log_provider_canonical_input(
        "anthropic",
        model,
        format,
        &payload,
...
impl Provider for AnthropicProvider {
    ...
    fn credential_mode(&self) -> arterm_provider_core::CredentialMode {
        self.credential_mode_snapshot()
    }

    fn set_credential_mode(&self, mode: arterm_provider_core::CredentialMode) -> Result<()> {
    ...
    fn set_model(&self, model: &str) -> Result<()> {
        ...
        let model: &str = if is_1m_model(model)
            && matches!(
                arterm_provider_core::anthropic_context_mode(model),
                arterm_provider_core::AnthropicContextMode::Native1M
            ) {
            strip_1m_suffix(model)
        } else {
            model
        };
        if !arterm_base::provider::known_anthropic_model_ids()
            .iter()
    ...
    fn available_models_for_switching(&self) -> Vec<String> {
        arterm_base::provider::cached_anthropic_model_ids()
            .unwrap_or_else(arterm_base::provider::known_anthropic_model_ids)
    }
    ...
    async fn prefetch_models(&self) -> Result<()> {
        ...
        let catalog = if is_oauth {
            match arterm_base::provider::fetch_anthropic_model_catalog_oauth(&token).await {
                ...
                Err(err) if is_oauth_catalog_auth_error(&err.to_string()) => {
                    ...
                    arterm_base::provider::fetch_anthropic_model_catalog_oauth(&refreshed_token)
                        .await
                }
                Err(err) => Err(err),
            }
        ...
        arterm_base::provider::persist_anthropic_model_catalog(&catalog);
        if !catalog.context_limits.is_empty() {
            arterm_base::provider::populate_context_limits(catalog.context_limits);
        }
        if !catalog.available_models.is_empty() {
            arterm_base::provider::populate_anthropic_models(catalog.available_models);
        }
        Ok(())
    }
...
async fn run_stream_with_retries(
    ...
    for attempt in 0..MAX_RETRIES {
        if attempt > 0 {
            ...
            let delay = arterm_provider_core::retry_after::retry_delay(
                attempt,
                RETRY_BASE_DELAY_MS,
            ...
            let _ = tx
                .send(Ok(StreamEvent::ConnectionPhase {
                    phase: arterm_message_types::ConnectionPhase::Retrying {
                        attempt: attempt + 1,
                        max: MAX_RETRIES,
                    },
                }))
        ...
        let (attempt_tx, attempt_guard) =
            arterm_provider_core::attempt_tracker::track_attempt_output(tx.clone());
        ...
        let attempt_client = if attempt == 0 {
            ...
            arterm_provider_core::fresh_transport_client()
        };
        ...
        .await
        {
            ...
            Err(e) => {
                ...
                if is_oauth && is_oauth_auth_error(&error_str) && !attempted_forced_refresh {
                    ...
                    let _ = tx
                        .send(Ok(StreamEvent::ConnectionPhase {
                            phase: arterm_message_types::ConnectionPhase::Authenticating,
                        }))
                        .await;
                        ...
                        Err(refresh_err) => {
                            let _ = tx
                                .send(Err(anyhow::anyhow!(
                                    "{}\n\nAutomatic Claude OAuth refresh failed: {}\nRun `arterm login --provider claude` (preferred) or `claude`, then retry.",
                                    e,
                                    refresh_err
                                )))
                                .await;
                ...
                if is_oauth && is_oauth_auth_error(&error_str) {
                    let _ = tx
                        .send(Err(anyhow::anyhow!(
                            "{}\n\nClaude OAuth authentication failed. Run `arterm login --provider claude` (preferred) or `claude`, then retry.",
                            e
                        )))
                        .await;
                } else {
...
async fn stream_response(
    ...
    use arterm_message_types::ConnectionPhase;
    let requested_model_base = strip_1m_suffix(&request.model).to_ascii_lowercase();
    if std::env::var("ARTERM_ANTHROPIC_DEBUG")
        .map(|v| v == "1")
        .unwrap_or(false)
        && let Ok(json) = serde_json::to_string_pretty(&request)
    {
        arterm_base::logging::info(&format!("Anthropic request payload:\n{}", json));
    }
    ...
    let stream_idle_timeout = arterm_base::provider::stream_idle_timeout();
    ...
    let response = arterm_provider_core::transport::send_with_initial_response_timeout(
        req.json(&request),
        stream_idle_timeout,
    )
    .await
    ...
    if !response.status().is_success() {
        ...
        let retry_after = arterm_provider_core::retry_after::retry_after(response.headers());
        let error_text = arterm_base::util::http_error_body(response, "HTTP error").await;
        return Err(arterm_provider_core::retry_after::error_with_retry_after(
    ...
    // `[provider] stream_idle_timeout_secs` / `ARTERM_STREAM_IDLE_TIMEOUT_SECS`
...
fn is_retryable_error(error_str: &str) -> bool {
    arterm_provider_core::is_transient_transport_error(error_str)
        // Server errors (5xx)
...
fn anthropic_model_quality_rank(model: &str) -> usize {
    ...
    let normalized = arterm_provider_core::model_id::strip_date_suffix(
        &arterm_provider_core::model_id::canonical(model),
    )
    .to_string();
    arterm_provider_core::ALL_CLAUDE_MODELS
        .iter()
        .position(|candidate| {
            arterm_provider_core::model_id::strip_date_suffix(
                &arterm_provider_core::model_id::canonical(candidate),
            ) == normalized
        })
        ...
        // available.
        .unwrap_or(arterm_provider_core::ALL_CLAUDE_MODELS.len())
}
...
fn anthropic_recommended_model_from_error(error_str: &str) -> Option<String> {
    ...
    arterm_base::provider::known_anthropic_model_ids()
        .into_iter()
...
fn parse_sse_event(buffer: &mut String) -> Option<SseEvent> {
    ...
    for line in event_str.lines() {
        ...
        } else if let Some(rest) = arterm_base::util::sse_data_line(line) {
            data = rest.to_string();
        }
    }
    ...
    match event.event_type.as_str() {
        "message_start" => {
            ...
            if let Ok(parsed) = serde_json::from_str::<MessageStartEvent>(&event.data) {
                ...
                // substitution (and surface it under ARTERM_LOG_SERVED_MODEL).
                if let Some(served) = parsed.message.model.as_deref() {
                    arterm_base::logging::info(&format!("Anthropic served model={}", served));
                    if std::env::var("ARTERM_LOG_SERVED_MODEL").is_ok() {
                ...
                if let Some(usage) = parsed.message.usage {
                    ...
                    if let Some(tier) = usage.service_tier.as_deref() {
                        ...
                        if std::env::var("ARTERM_LOG_SERVICE_TIER").is_ok() {
        ...
        "ping" => {
            ...
            events.push(StreamEvent::ConnectionPhase {
                phase: arterm_message_types::ConnectionPhase::Streaming,
            });
        }
        "error" => {
            arterm_base::logging::error(&format!("Anthropic stream error: {}", event.data));
...
fn build_system_param(system: &str, is_oauth: bool) -> Option<ApiSystem> {
    arterm_provider_anthropic::build_system_param(system, is_oauth, is_cache_ttl_1h())
}
...
fn build_system_param_split(
    ...
    arterm_provider_anthropic::build_system_param_split(
        static_part,
        dynamic_part,
        is_oauth,
        is_cache_ttl_1h(),
...
fn format_messages_with_identity(messages: Vec<ApiMessage>, is_oauth: bool) -> Vec<ApiMessage> {
    arterm_provider_anthropic::format_messages_with_identity(messages, is_oauth, is_cache_ttl_1h())
}

#[cfg(test)]
fn add_message_cache_breakpoint(messages: &mut [ApiMessage]) {
    arterm_provider_anthropic::add_message_cache_breakpoint(messages, is_cache_ttl_1h())
}
