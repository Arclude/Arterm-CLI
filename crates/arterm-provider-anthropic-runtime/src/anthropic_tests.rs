...
#[tokio::test]
#[ignore = "live smoke: requires ANTHROPIC_API_KEY, or set ARTERM_LIVE_ANTHROPIC_ALLOW_OAUTH=1 to use Claude OAuth credentials"]
async fn live_anthropic_reasoning_smoke() -> Result<()> {
    let _env_lock = arterm_base::storage::lock_test_env();
    let using_api_key = std::env::var_os("ANTHROPIC_API_KEY").is_some();
    let allow_oauth = std::env::var_os("ARTERM_LIVE_ANTHROPIC_ALLOW_OAUTH").is_some();
    if !using_api_key && !allow_oauth {
        eprintln!(
            "skipping live Anthropic smoke: set ANTHROPIC_API_KEY or ARTERM_LIVE_ANTHROPIC_ALLOW_OAUTH=1"
        );
        return Ok(());
    }

    let _max_tokens = EnvVarGuard::set_if_missing("ARTERM_ANTHROPIC_MAX_TOKENS", "2048");
    let model = std::env::var("ARTERM_LIVE_ANTHROPIC_MODEL")
        .or_else(|_| std::env::var("ARTERM_ANTHROPIC_MODEL"))
        .unwrap_or_else(|_| "claude-sonnet-4-6".to_string());
    let effort = std::env::var("ARTERM_LIVE_ANTHROPIC_REASONING_EFFORT")
        .unwrap_or_else(|_| "low".to_string());
    let prompt = std::env::var("ARTERM_LIVE_ANTHROPIC_PROMPT")
        .unwrap_or_else(|_| "Live smoke test: answer exactly OK.".to_string());
    let system = std::env::var("ARTERM_LIVE_ANTHROPIC_SYSTEM").unwrap_or_else(|_| {
        "You are a live provider smoke test. Keep the answer tiny.".to_string()
    });
    let require_thinking = std::env::var_os("ARTERM_LIVE_ANTHROPIC_REQUIRE_THINKING").is_some();
    ...
    if require_thinking {
        assert!(
            thinking_bytes > 0,
            "live Anthropic response did not include thinking deltas despite ARTERM_LIVE_ANTHROPIC_REQUIRE_THINKING"
        );
    }
    Ok(())
}
...
fn credential_mode_runtime_provider_identity_round_trips() {
    ...
    let previous = std::env::var_os("ARTERM_RUNTIME_PROVIDER");
    ...
    match previous {
        Some(value) => arterm_base::env::set_var("ARTERM_RUNTIME_PROVIDER", value),
        None => arterm_base::env::remove_var("ARTERM_RUNTIME_PROVIDER"),
    }
}
...
async fn auto_mode_falls_back_to_api_key_when_oauth_is_expired() {
    ...
    let _home = EnvVarGuard::set("ARTERM_HOME", temp.path());
    let _api_key = EnvVarGuard::set("ANTHROPIC_API_KEY", "test-anthropic-api-key");
    let _runtime = EnvVarGuard::set("ARTERM_RUNTIME_PROVIDER", "auto");

    arterm_base::auth::claude::upsert_account(arterm_base::auth::claude::AnthropicAccount {
...
async fn explicit_oauth_mode_does_not_silently_fall_back_to_api_key() {
    ...
    let _home = EnvVarGuard::set("ARTERM_HOME", temp.path());
    let _api_key = EnvVarGuard::set("ANTHROPIC_API_KEY", "test-anthropic-api-key");
    let _runtime = EnvVarGuard::set("ARTERM_RUNTIME_PROVIDER", "claude");
...
fn anthropic_fallback_prefers_best_available_and_skips_tried_and_retired() {
    ...
    let _guard = arterm_base::storage::lock_test_env();
    arterm_base::provider::models::reset_model_catalog_services_for_tests();
    let known = arterm_base::provider::known_anthropic_model_ids();
    assert!(
        !known.is_empty(),
...
fn fable_quota_fallback_selects_the_best_available_opus() {
    ...
    let candidates = arterm_base::provider::cached_anthropic_model_ids()
...
fn model_scoped_usage_routes_only_exhausted_fable_to_opus() {
    let usage = arterm_base::usage::UsageData {
        model_scoped: vec![arterm_base::usage::ModelScopedUsageWindow {
    ...
    let available = arterm_base::usage::UsageData {
        model_scoped: vec![arterm_base::usage::ModelScopedUsageWindow {
            model_name: "Fable".to_string(),
            utilization: 0.98,
            resets_at: None,
...
fn test_anthropic_opus_5_low_effort_reaches_the_wire() {
    ...
    // *defaults* to `low` (arterm's default model/effort pairing), and an
