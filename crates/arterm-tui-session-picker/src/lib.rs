...
// `ResumeTarget` is pure data and now lives in `arterm-session-types` so the
// foundation/import layer can use it without depending on this UI crate. It is
// re-exported here so existing `arterm_tui_session_picker::ResumeTarget` paths
// keep working.
pub use arterm_session_types::ResumeTarget;
...
mod tests {
    ...
    fn source_predicates_cover_provider_and_model_fallbacks() {
        ...
        assert!(session_is_pi(SessionSource::Arterm, Some("pi-main"), None));
