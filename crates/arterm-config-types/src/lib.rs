...
pub struct NamedProviderConfig {
    ...
    /// Must be a JSON object; keys here override arterm-generated body fields.
    ...
    /// `reasoning_effort` request field (`/effort` support). When unset, arterm
...
pub struct AuthConfig {
    /// External auth source ids that the user has approved arterm to read/use.
...
pub struct AgentsConfig {
    ...
    /// local. Env override: `ARTERM_MEMORY_EMBEDDING_BACKEND`.
    ...
    /// Unset = `text-embedding-3-small`. Env: `ARTERM_MEMORY_EMBEDDING_MODEL`.
    #[serde(default)]
    ...
    /// Env: `ARTERM_MEMORY_EMBEDDING_BASE_URL`.
    #[serde(default)]
...
/// pane by the built-in launcher. `ARTERM_TERMINAL` explicitly selects a terminal
/// emulator instead.
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
#[serde(default)]
pub struct TerminalConfig {
    ...
    /// opening a terminal emulator itself, with `ARTERM_SPAWN_*` metadata env
    ...
    /// tmux window in the current server. If the hook fails to launch, arterm
    ...
    ///
    /// When set, arterm runs the hook (instead of wmctrl/xdotool) whenever it
    /// wants to bring a session's window to the foreground, with
    /// `ARTERM_FOCUS_SESSION_ID` and `ARTERM_FOCUS_TITLE` env vars. Pair this
    ...
    /// which terminal arterm launches into and is preferred over the legacy
    /// `~/.arterm/preferred_terminal.json` file. Re-run `arterm setup-hotkey`
...
/// Lifecycle hooks: external commands arterm runs at well-defined points.
///
/// Hook commands are parsed shell-style (quotes work) but executed directly,
/// with `ARTERM_HOOK_*` env vars describing the event (`ARTERM_HOOK_EVENT`,
/// `ARTERM_HOOK_SESSION_ID`, `ARTERM_HOOK_CWD`, event-specific fields, and a
/// `ARTERM_HOOK_PAYLOAD` JSON mirror). Hook processes get
/// `ARTERM_HOOKS_DISABLED=1` so nested arterm invocations don't recurse.
///
...
pub struct HooksConfig {
    ...
    /// ("chat"/"resume"/"ambient"). Env override: ARTERM_HOOK_TURN_START.
...
pub struct KeybindingsConfig {
    ...
    /// Spawn a fresh arterm session in a new terminal window (default: unbound).
    /// Example: "alt+enter".
...
pub enum WebSearchEngine {
    ...
    /// `ARTERM_SEARXNG_URL` env var) to point at a SearXNG instance. Useful on
...
impl Default for WebSearchConfig {
    fn default() -> Self {
        Self {
            ...
            bing_api_key: None,
            bing_api_key_env: "ARTERM_BING_API_KEY".to_string(),
            bing_market: "en-US".to_string(),
            searxng_url: None,
            searxng_url_env: "ARTERM_SEARXNG_URL".to_string(),
        }
    }
}
...
pub struct ProviderConfig {
    ...
    /// Whether arterm should automatically try another account on the same provider
    ...
    /// automatically (see `arterm_base::provider::stream_idle_timeout_for_effort`).
    /// Default: 180. Overridable via `ARTERM_STREAM_IDLE_TIMEOUT_SECS`.
...
pub struct SafetyConfig {
    ...
    /// SMTP password (prefer ARTERM_SMTP_PASSWORD env var)
    ...
    /// Jade relay bearer token (prefer ARTERM_JADE_RELAY_TOKEN env var)
...
pub struct PowerConfig {
    /// Prevent automatic system sleep while any arterm session is actively
    ...
    ///
    /// Honored by the shared `arterm serve` daemon. The `ARTERM_DISABLE_POWER_INHIBIT`
...
/// A single global launch hotkey: a chord plus the directory it opens arterm in.
///
...
/// - `$LAST_DIR` -> the most recent non-home project directory arterm ran in.
/// - `$LAST_REPO` -> the most recent arterm repo (for self-dev).
///
...
pub struct LaunchHotkeyEntry {
    /// arterm-style chord string, e.g. `cmd+;`, `cmd+[`, `cmd+shift+'`.
    pub chord: String,
...
/// Configuration for the global "launch a new arterm" hotkeys (macOS).
///
/// When `entries` is empty, arterm uses its built-in defaults (`Cmd+;` -> home,
