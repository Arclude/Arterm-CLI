...
// flip `ARTERM_NO_TELEMETRY` / `DO_NOT_TRACK`, while the session tests drive the
...
fn telemetry_endpoint_uses_production_custom_domain() {
    assert_eq!(TELEMETRY_ENDPOINT, "https://telemetry.arterm.dev/v1/event");
}
...
fn test_is_ci_detects_ci_env() {
    ...
    arterm_core::env::set_var("CI", "true");
    assert!(
        is_ci(),
        "CI env var should mark the run as CI (gates install skip)"
    );
    arterm_core::env::remove_var("CI");
    assert!(!is_ci());
    ...
    arterm_core::env::set_var("NEXTEST", "1");
    assert!(is_ci(), "nextest runs should be tagged as automation");
    arterm_core::env::remove_var("NEXTEST");
    assert!(!is_ci());
}
...
fn test_onboarding_step_milestone_key_includes_provider_and_method() {
    assert_eq!(
        onboarding_step_milestone_key("auth_success", Some("arterm"), Some("API key")),
        "auth_success_arterm_api_key"
    );
    assert_eq!(
...
fn test_install_marker_tracks_current_telemetry_id() {
    ...
    if let Some(prev_home) = prev_home {
        arterm_core::env::set_var("ARTERM_HOME", prev_home);
    } else {
...
fn test_begin_session_closes_superseded_session() {
    let _guard = lock_telemetry_test_state();
    arterm_core::env::set_var("ARTERM_TELEMETRY_DISABLED", "1");
    ...
    arterm_core::env::remove_var("ARTERM_TELEMETRY_DISABLED");
}
