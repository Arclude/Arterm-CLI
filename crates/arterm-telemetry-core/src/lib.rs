...
const TELEMETRY_ENDPOINT: &str = "https://telemetry.arterm.dev/v1/event";
...
const DEFAULT_DISCOVERY_ENDPOINT: &str = "https://api.arterm.dev/v1/discovery";
...
/// Its presence is equivalent to setting `ARTERM_NO_TELEMETRY=1`, but persists
...
fn opt_out_marker_path() -> Option<std::path::PathBuf> {
    storage::arterm_dir().ok().map(|d| d.join("no_telemetry"))
}
...
pub fn opt_out_forced_by_env() -> bool {
    std::env::var("ARTERM_NO_TELEMETRY").is_ok() || std::env::var("DO_NOT_TRACK").is_ok()
}
...
fn detect_project_profile() -> ProjectProfile {
    ...
    profile.repo_present = root.join(".git").exists() || is_arterm_repo_dir(root);
...
fn post_payload(payload: serde_json::Value, timeout: Duration) -> bool {
    ...
    let client = TELEMETRY_HTTP_CLIENT.get_or_init(|| {
        reqwest::blocking::Client::builder()
            .user_agent(arterm_provider_core::ARTERM_USER_AGENT)
            .build()
    ...
    F: FnMut(Value) + Send + 'static,
{
    ...
    std::thread::Builder::new()
        .name("arterm-telemetry".to_string())
        .spawn(move || {
...
pub fn record_install_if_first_run() {
    ...
    // fresh ~/.arterm (so a new telemetry_id) on every job, which would otherwise
...
pub fn record_auth_started(provider: &str, method: &str) {
    arterm_logging::auth_event("auth_started", provider, &[("method", method)]);
...
pub fn record_auth_cancelled(provider: &str, method: &str) {
    arterm_logging::auth_event("auth_cancelled", provider, &[("method", method)]);
...
pub fn record_auth_surface_blocked(provider: &str, method: &str) {
    arterm_logging::auth_event("auth_surface_blocked", provider, &[("method", method)]);
...
pub fn record_auth_success(provider: &str, method: &str) {
    arterm_logging::auth_event("auth_success", provider, &[("method", method)]);
    if !is_enabled() {
        return;
    }
    let Some(id) = get_or_create_id() else {
...
fn show_first_run_notice() {
    ...
    let (dim, reset) = if arterm_core::console::stderr_supports_ansi() {
        ("\x1b[90m", "\x1b[0m")
    } else {
        ("", "")
    };
