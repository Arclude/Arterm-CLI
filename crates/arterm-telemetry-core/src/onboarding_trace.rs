...
//! `arterm-tui::tui::app::onboarding_graph`. The interesting signal is the
...
use arterm_usage_types::{OnboardingTraceEvent, OnboardingTraceStep};
...
impl TraceRecorder {
    ...
    /// Exactly the bytes that would be sent, for `arterm telemetry show-last-trace`.
    ///
