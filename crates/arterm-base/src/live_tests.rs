...
const LEDGER_ENV: &str = "ARTERM_LIVE_TEST_LEDGER";
const COVERAGE_ENV: &str = "ARTERM_LIVE_TEST_COVERAGE";
...
pub mod checkpoints {
    ...
    pub const REAL_ARTERM_TOOL_SMOKE: &str = "real_arterm_tool_smoke";
...
const END_TO_END_CHECKPOINTS: &[LiveVerificationCheckpointDefinition] = &[
    ...
    LiveVerificationCheckpointDefinition {
        ...
        required_for_user_ready: true,
        spends_balance: false,
        description: "Arterm can load the credential from the expected env/config source and records only a fingerprint.",
    },
    LiveVerificationCheckpointDefinition {
    ...
    LiveVerificationCheckpointDefinition {
        ...
        required_for_user_ready: true,
        spends_balance: true,
        description: "A full Arterm turn executes a harmless local tool requested by the model.",
    },
    LiveVerificationCheckpointDefinition {
    ...
    LiveVerificationCheckpointDefinition {
        id: checkpoints::REAL_ARTERM_TOOL_SMOKE,
        label: "Real Arterm tool smoke",
        category: "tools",
        required_for_user_ready: true,
        spends_balance: true,
        description: "A normal Arterm agent turn uses the real streamed parser, advertised tool schema, registry execution, tool-result followup, and transcript validation without malformed tool calls.",
    },
    LiveVerificationCheckpointDefinition {
...
pub struct LiveVerificationBuild {
    pub arterm_version: String,
    pub arterm_git_hash: String,
    pub arterm_git_date: String,
    pub arterm_git_dirty: bool,
    pub arterm_semver: String,
    pub os: String,
    pub arch: String,
    pub pid: u32,
}
...
impl LiveVerificationBuild {
    pub fn current() -> Self {
        let version = arterm_build_meta::version().to_string();
        Self {
            arterm_git_dirty: version.contains("dirty"),
            arterm_version: version,
            arterm_git_hash: arterm_build_meta::git_hash().to_string(),
            arterm_git_date: arterm_build_meta::git_date().to_string(),
            arterm_semver: arterm_build_meta::semver().to_string(),
...
pub struct LiveProviderModelCoveragePair {
    ...
    /// arterm version string that produced the most recent entry for this pair.
    #[serde(default)]
    pub latest_arterm_version: String,
...
pub struct LiveProviderModelCoverageSummary {
    ...
    /// Full monitoring roster: every provider arterm knows about (OpenAI-compatible
...
pub struct ProviderMonitorEntry {
    ...
    /// True when `arterm provider-doctor <id>` can drive this provider today
...
fn update_coverage(event: &LiveVerificationEvent, path: &Path) -> Result<()> {
    ...
    coverage.latest.insert(
        ...
        LiveVerificationCoverageEntry {
            ...
            arterm_version: event.build.arterm_version.clone(),
            arterm_git_hash: event.build.arterm_git_hash.clone(),
            arterm_git_dirty: event.build.arterm_git_dirty,
...
pub fn format_provider_test_coverage_report(
    ...
    out.push_str("Developer/live verification evidence recorded by arterm. This is evidence, not a guarantee of future provider availability.\n\n");
    ...
    let (coverage, path) = match load_coverage(coverage_path) {
        ...
        Err(err) => {
            ...
            out.push_str("Once arterm ships a curated developer coverage snapshot, this command should prefer that snapshot and separately show local evidence.\n\n");
            out.push_str(&format!("Ledger error: {}\n\n", err));
            out.push_str("You can generate local evidence with:\n\n");
            out.push_str(&format!(
                "  arterm auth-test --provider {} --model {}",
    ...
    let Some(entry) = matches.last() else {
        out.push_str("Status: Not yet covered by this arterm verification ledger\n\n");
        out.push_str(&format!("Ledger: {}\n\n", path.display()));
        out.push_str("This does not mean the provider/model is broken. It only means arterm has no recorded live verification evidence for this exact provider/model pair.\n");
        return out;
    };
    ...
    out.push_str(&format!(
        ...
        coverage_actor_label(entry.arterm_git_dirty, &entry.arterm_version),
    ));
    ...
    out.push_str("\n## What this means\n\n");
    out.push_str("These checks exercise real arterm runtime paths, including basic chat and tool-use smoke tests when present. Missing evidence should be read as 'not yet recorded', not as a failure.\n");
    out
}
...
fn provider_test_coverage_checkpoint_label(checkpoint: &str) -> String {
    match checkpoint {
        ...
        checkpoints::REAL_ARTERM_TOOL_SMOKE => "Real arterm tool smoke".to_string(),
...
struct ProviderModelCoverageBuilder {
    ...
    latest_arterm_version: String,
    latest_arterm_dirty: bool,
    entries: usize,
...
impl ProviderModelCoverageBuilder {
    fn record(&mut self, entry: &LiveVerificationCoverageEntry) {
            ...
            .unwrap_or(true)
        {
            ...
            self.latest_arterm_version = entry.arterm_version.clone();
            self.latest_arterm_dirty = entry.arterm_git_dirty;
        }
    ...
    fn finish(self) -> LiveProviderModelCoveragePair {
        ...
        LiveProviderModelCoveragePair {
            ...
            latest_arterm_version: self.latest_arterm_version,
            latest_arterm_dirty: self.latest_arterm_dirty,
...
pub fn strict_live_provider_model_coverage_summary(
        ...
        .map(ProviderModelCoverageBuilder::finish)
    {
        ...
        match pair.checkpoint_status(checkpoints::REAL_ARTERM_TOOL_SMOKE) {
...
const STRICT_PIPELINE_STAGES: &[(&str, &str)] = &[
    ...
    (checkpoints::REAL_ARTERM_TOOL_SMOKE, "real tool smoke"),
];
...
fn doctor_tier_for_stage(stage_id: &str) -> &'static str {
    match stage_id {
        ...
        | checkpoints::TOOL_EXECUTION_LOOP
        | checkpoints::TOOL_RESULT_FOLLOWUP
        | checkpoints::REAL_ARTERM_TOOL_SMOKE => "full",
...
/// the report. Lets `provider-test-coverage` enumerate *every* provider arterm
...
pub fn format_strict_live_provider_model_coverage_summary(
    ...
    out.push_str("A pair is READY only after every stage below passes in a real Arterm runtime.\n");
    ...
    if all_pairs.is_empty() {
        ...
        out.push_str("`arterm provider-doctor <provider> --tier full` to record one.\n\n");
    } else {
        ...
        for pair in &shown_pairs {
            ...
            let tested = format!(
                ...
                coverage_actor_label(pair.latest_arterm_dirty, &pair.latest_arterm_version),
            );
    ...
    // -- Full provider monitoring roster: EVERY provider arterm knows about. ----
    ...
    out.push_str("    arterm provider-doctor <provider> --tier full   # spends balance\n");
    out.push_str(
        "    arterm provider-doctor <provider> --tier offline # wiring only, no key/spend\n",
    );
...
fn pair_fix_hint(provider_id: &str, model: &str, stage_id: &str) -> String {
    if doctor_supports_provider(provider_id) {
        ...
        format!("run `arterm provider-doctor {provider_id} --model {model} --tier {tier}`")
    } else {
...
mod tests {
    ...
    fn coverage_entry(
        ...
        LiveVerificationCoverageEntry {
            ...
            arterm_version: "test".to_string(),
            arterm_git_hash: "test".to_string(),
            arterm_git_dirty: false,
            user_ready: false,
