...
pub const ARTERM_API_KEY_ENV: &str = "ARTERM_API_KEY";
pub const ARTERM_API_BASE_ENV: &str = "ARTERM_API_BASE";
pub const ARTERM_ACCOUNT_ID_ENV: &str = "ARTERM_ACCOUNT_ID";
pub const ARTERM_ACCOUNT_EMAIL_ENV: &str = "ARTERM_ACCOUNT_EMAIL";
pub const ARTERM_TIER_ENV: &str = "ARTERM_TIER";
...
pub const ARTERM_ROUTE_API_METHOD: &str = "arterm-subscription";
...
impl ArtermTier {
    pub const ALL: &'static [ArtermTier] = &[
        ArtermTier::Plus,
        ArtermTier::Pro,
        ArtermTier::Max,
        ArtermTier::Ultra,
    ...
    pub fn allows(self, required: ArtermTier) -> bool {
        self >= required
    }
}
...
pub enum UpstreamRoutingPolicy {
    /// Routing is decided server-side by the arterm router (model -> provider +
...
pub const CURATED_MODELS: &[CuratedModel] = &[
    CuratedModel {
        ...
        min_tier: ArtermTier::Plus,
        note: "Frontier model; routed server-side to Anthropic by the arterm router.",
    },
    CuratedModel {
        id: "claude-opus-5",
    ...
    CuratedModel {
        ...
        routing_policy: UpstreamRoutingPolicy::ServerManaged,
        min_tier: ArtermTier::Plus,
        note: "Frontier model; routed server-side to Amazon Bedrock by the arterm router.",
    },
    CuratedModel {
        id: "gpt-5.5",
        ...
        routing_policy: UpstreamRoutingPolicy::ServerManaged,
        min_tier: ArtermTier::Plus,
        note: "Frontier model; routed server-side to OpenAI by the arterm router.",
    },
    CuratedModel {
        id: "claude-fable-5",
    ...
    CuratedModel {
        ...
        routing_policy: UpstreamRoutingPolicy::ServerManaged,
        min_tier: ArtermTier::Plus,
        note: "Open-weight coding model; routed server-side to Amazon Bedrock by the arterm router.",
    },
    CuratedModel {
        id: "devstral-2-123b",
    ...
    CuratedModel {
        ...
        routing_policy: UpstreamRoutingPolicy::ServerManaged,
        min_tier: ArtermTier::Plus,
        note: "Open-weight reasoning and coding model; routed server-side to Amazon Bedrock by the arterm router.",
    },
    CuratedModel {
        id: "nova-2-lite",
        ...
        routing_policy: UpstreamRoutingPolicy::ServerManaged,
        min_tier: ArtermTier::Plus,
        note: "Recent efficient multimodal model; routed server-side to Amazon Bedrock by the arterm router.",
    },
    CuratedModel {
        id: "minimax-m2.5",
        ...
        routing_policy: UpstreamRoutingPolicy::ServerManaged,
        min_tier: ArtermTier::Plus,
        note: "Recent reasoning and coding model; routed server-side to Amazon Bedrock by the arterm router.",
    },
    CuratedModel {
        id: "mistral-large-3",
        ...
        routing_policy: UpstreamRoutingPolicy::ServerManaged,
        min_tier: ArtermTier::Plus,
        note: "Open-weight general and coding model; routed server-side to Amazon Bedrock by the arterm router.",
    },
    CuratedModel {
        id: "kimi-k2.5",
        ...
        routing_policy: UpstreamRoutingPolicy::ServerManaged,
        min_tier: ArtermTier::Plus,
        note: "Recent reasoning and agentic model; routed server-side to Amazon Bedrock by the arterm router.",
    },
    CuratedModel {
        id: "kimi-k2-thinking",
        ...
        routing_policy: UpstreamRoutingPolicy::ServerManaged,
        min_tier: ArtermTier::Plus,
        note: "Reasoning-focused model; routed server-side to Amazon Bedrock by the arterm router.",
    },
    CuratedModel {
        ...
        routing_policy: UpstreamRoutingPolicy::ServerManaged,
        min_tier: ArtermTier::Plus,
        note: "Efficient open-weight reasoning model; routed server-side to Amazon Bedrock by the arterm router.",
    },
    CuratedModel {
        id: "gpt-oss-120b",
        ...
        routing_policy: UpstreamRoutingPolicy::ServerManaged,
        min_tier: ArtermTier::Plus,
        note: "Large open-weight reasoning model; routed server-side to Amazon Bedrock by the arterm router.",
    },
    CuratedModel {
        id: "gpt-oss-20b",
    ...
    CuratedModel {
        ...
        routing_policy: UpstreamRoutingPolicy::ServerManaged,
        min_tier: ArtermTier::Plus,
        note: "Recent general and coding model; routed server-side to Amazon Bedrock by the arterm router.",
    },
    CuratedModel {
        id: "glm-4.7-flash",
        ...
        routing_policy: UpstreamRoutingPolicy::ServerManaged,
        min_tier: ArtermTier::Plus,
        note: "Efficient recent general model; routed server-side to Amazon Bedrock by the arterm router.",
    },
];
...
fn normalize_model_key(model: &str) -> String {
    ...
    arterm_provider_core::model_id::canonical(base)
}
...
pub fn effective_tier() -> ArtermTier {
    cached_tier().unwrap_or(ArtermTier::Plus)
}

/// The last tier reported by the backend, if any was persisted.
pub fn cached_tier() -> Option<ArtermTier> {
    provider_catalog::load_env_value_from_env_or_config(ARTERM_TIER_ENV, ARTERM_ENV_FILE)
        .as_deref()
        .and_then(ArtermTier::parse)
}

/// Persist the last-known tier reported by the backend (`None` clears it).
pub fn store_cached_tier(tier: Option<ArtermTier>) -> anyhow::Result<()> {
    provider_catalog::save_env_value_to_env_file(
        ARTERM_TIER_ENV,
        ARTERM_ENV_FILE,
        tier.map(ArtermTier::as_str),
    )
}
...
pub fn routing_policy_detail(model: &CuratedModel) -> String {
    match model.routing_policy {
        UpstreamRoutingPolicy::ServerManaged => {
            "Arterm hosted routing · managed server-side".to_string()
        }
    }
}

pub fn configured_api_key() -> Option<String> {
    provider_catalog::load_env_value_from_env_or_config(ARTERM_API_KEY_ENV, ARTERM_ENV_FILE)
}

pub fn configured_api_base() -> Option<String> {
    provider_catalog::load_env_value_from_env_or_config(ARTERM_API_BASE_ENV, ARTERM_ENV_FILE)
}
...
/// Persist an account API key and its non-secret account metadata in arterm's
...
pub fn persist_account_credentials(
    ...
    if api_key.is_empty() {
        anyhow::bail!("refusing to persist an empty arterm account API key");
    }

    for (key, value) in [
        (ARTERM_API_KEY_ENV, Some(api_key)),
        (ARTERM_ACCOUNT_ID_ENV, nonempty(account_id)),
        (ARTERM_ACCOUNT_EMAIL_ENV, nonempty(email)),
        (ARTERM_TIER_ENV, nonempty(tier)),
    ] {
        provider_catalog::save_env_value_to_env_file(key, ARTERM_ENV_FILE, value)?;
    }
...
pub fn clear_account_credentials() -> anyhow::Result<()> {
    for key in [
        ARTERM_API_KEY_ENV,
        ARTERM_ACCOUNT_ID_ENV,
        ARTERM_ACCOUNT_EMAIL_ENV,
        ARTERM_TIER_ENV,
    ] {
        provider_catalog::save_env_value_to_env_file(key, ARTERM_ENV_FILE, None)?;
    }
    clear_runtime_env();
    ensure_account_credential_permissions()
}
...
pub fn account_credential_path() -> anyhow::Result<std::path::PathBuf> {
    Ok(crate::storage::app_config_dir()?.join(ARTERM_ENV_FILE))
}
...
pub fn ensure_account_credential_permissions() -> anyhow::Result<()> {
    ...
    #[cfg(unix)]
    {
        ...
        if mode & 0o077 != 0 {
            anyhow::bail!(
                "arterm account credential file has unsafe permissions {:03o}; expected owner-only access",
                mode
            );
        }
    }
...
pub fn is_runtime_mode_enabled() -> bool {
    std::env::var(ARTERM_SUBSCRIPTION_ACTIVE_ENV)
        .ok()
        .map(|value| {
            matches!(
...
pub fn apply_runtime_env() {
    crate::env::set_var(ARTERM_SUBSCRIPTION_ACTIVE_ENV, "1");
    crate::env::set_var(
        "ARTERM_OPENROUTER_API_BASE",
        configured_api_base().unwrap_or_else(|| DEFAULT_ARTERM_API_BASE.to_string()),
    );
    crate::env::set_var("ARTERM_OPENROUTER_API_KEY_NAME", ARTERM_API_KEY_ENV);
    crate::env::set_var("ARTERM_OPENROUTER_ENV_FILE", ARTERM_ENV_FILE);
    crate::env::set_var("ARTERM_OPENROUTER_CACHE_NAMESPACE", ARTERM_CACHE_NAMESPACE);
    crate::env::set_var("ARTERM_OPENROUTER_PROVIDER_FEATURES", "0");
    crate::env::set_var("ARTERM_OPENROUTER_TRANSPORT_STATE", "arterm-subscription");
    crate::env::remove_var("ARTERM_OPENROUTER_ALLOW_NO_AUTH");
    crate::env::remove_var("ARTERM_OPENROUTER_PROVIDER");
    crate::env::remove_var("ARTERM_OPENROUTER_NO_FALLBACK");
}

pub fn clear_runtime_env() {
    crate::env::remove_var(ARTERM_SUBSCRIPTION_ACTIVE_ENV);
    crate::env::remove_var("ARTERM_OPENROUTER_API_BASE");
    crate::env::remove_var("ARTERM_OPENROUTER_API_KEY_NAME");
    crate::env::remove_var("ARTERM_OPENROUTER_ENV_FILE");
    crate::env::remove_var("ARTERM_OPENROUTER_CACHE_NAMESPACE");
    crate::env::remove_var("ARTERM_OPENROUTER_PROVIDER_FEATURES");
    crate::env::remove_var("ARTERM_OPENROUTER_TRANSPORT_STATE");
    crate::env::remove_var("ARTERM_OPENROUTER_ALLOW_NO_AUTH");
    crate::env::remove_var("ARTERM_OPENROUTER_PROVIDER");
    crate::env::remove_var("ARTERM_OPENROUTER_NO_FALLBACK");
}
...
mod tests {
    ...
    fn curated_catalog_has_exact_hosted_set_without_legacy_model_gates() {
        assert_eq!(
            CURATED_MODELS
                .iter()
                .filter(|model| model.min_tier == ArtermTier::Plus)
                .map(|model| model.id)
                .collect::<Vec<_>>(),
        ...
        assert_eq!(
            CURATED_MODELS
                .iter()
                .filter(|model| model.min_tier == ArtermTier::Ultra)
                .map(|model| model.id)
                .collect::<Vec<_>>(),
            Vec::<&str>::new()
        );
        assert!(
            CURATED_MODELS
                .iter()
                .all(|model| model.min_tier != ArtermTier::Flagship)
        );
    ...
    fn tier_pricing_matches_launched_plans() {
        let expected = [
            (ArtermTier::Plus, "plus", "Plus", 10, 20.00),
            (ArtermTier::Pro, "pro", "Pro", 20, 40.00),
            (ArtermTier::Max, "max", "Max", 100, 200.00),
            (ArtermTier::Ultra, "ultra", "Ultra", 200, 400.00),
            (ArtermTier::Flagship, "flagship", "Solo", 1000, 2000.00),
        ];

        assert_eq!(ArtermTier::ALL, expected.map(|(tier, ..)| tier));
    ...
    fn tier_parse_round_trips() {
        for tier in ArtermTier::ALL {
            assert_eq!(ArtermTier::parse(tier.as_str()), Some(*tier));
        }
        assert_eq!(ArtermTier::parse("PLUS"), Some(ArtermTier::Plus));
        assert_eq!(ArtermTier::parse(" Pro "), Some(ArtermTier::Pro));
        assert_eq!(ArtermTier::parse("MAX"), Some(ArtermTier::Max));
        assert_eq!(ArtermTier::parse(" ultra "), Some(ArtermTier::Ultra));
        assert_eq!(ArtermTier::parse(" Flagship "), Some(ArtermTier::Flagship));
        assert_eq!(ArtermTier::parse(" Solo "), Some(ArtermTier::Flagship));
        assert_eq!(ArtermTier::parse("starter"), None);
    }

    #[test]
    fn tier_gating_follows_catalog_order() {
        for (account_index, account_tier) in ArtermTier::ALL.iter().copied().enumerate() {
            for (required_index, required_tier) in ArtermTier::ALL.iter().copied().enumerate() {
                assert_eq!(
    ...
    fn every_hosted_model_is_available_from_the_base_subscription() {
        for model in CURATED_MODELS {
            assert_eq!(model.min_tier, ArtermTier::Plus, "{}", model.id);
        }
    ...
    fn hosted_catalog_access_does_not_depend_on_legacy_cached_tier() {
        ...
        crate::env::remove_var(ARTERM_TIER_ENV);
        let temp = tempfile::tempdir().expect("temp home");
        crate::env::set_var("ARTERM_HOME", temp.path().to_string_lossy().to_string());

        assert_eq!(cached_tier(), None);
        assert_eq!(effective_tier(), ArtermTier::Plus);
        for model in EXPECTED_PLUS_MODELS {
            assert!(is_model_allowed_for_current_tier(model));
        }
        assert!(is_model_allowed_for_current_tier("claude-fable-5"));

        crate::env::set_var(ARTERM_TIER_ENV, "mystery");
        assert_eq!(cached_tier(), None);
        assert_eq!(effective_tier(), ArtermTier::Plus);

        for tier in [ArtermTier::Pro, ArtermTier::Max] {
            crate::env::set_var(ARTERM_TIER_ENV, tier.as_str());
        ...
        crate::env::set_var(ARTERM_TIER_ENV, ArtermTier::Ultra.as_str());
        assert_eq!(effective_tier(), ArtermTier::Ultra);
        assert!(!is_model_allowed_for_current_tier("not-a-hosted-model"));
        store_cached_tier(Some(ArtermTier::Flagship)).expect("persist tier");
        assert_eq!(cached_tier(), Some(ArtermTier::Flagship));
        assert!(is_model_allowed_for_current_tier("claude-fable-5"));
