...
//! all arterm sessions. Instead of each session spawning its own set of
...
///
/// Only one pool exists per arterm daemon. It owns the child processes
    ...
    async fn connect_all_skips_non_shared_servers() {
        ...
        config.servers.insert(
            ...
            crate::mcp::protocol::McpServerConfig {
                command: "/nonexistent/arterm-test-mcp-557".to_string(),
                args: vec![],
                env: Default::default(),
                shared: false,
                transport: None,
