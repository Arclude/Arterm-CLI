...
fn load_for_dir_expands_the_winning_merged_definition() {
    let _guard = crate::storage::lock_test_env();
    let previous_home = std::env::var_os("ARTERM_HOME");
    let previous_value = std::env::var_os("ARTERM_MCP_EXPANSION_TEST_VALUE");
    let home = tempfile::tempdir().expect("home tempdir");
    let project = tempfile::tempdir().expect("project tempdir");
    crate::env::set_var("ARTERM_HOME", home.path());
    crate::env::set_var("ARTERM_MCP_EXPANSION_TEST_VALUE", "expanded-value");
    ...
    std::fs::write(
        project.path().join(".mcp.json"),
        r#"{"mcpServers":{"same-name":{"command":"project-bin","args":["${ARTERM_MCP_EXPANSION_TEST_VALUE}"]}}}"#,
    )
    .unwrap();
    ...
    match previous_value {
        Some(value) => crate::env::set_var("ARTERM_MCP_EXPANSION_TEST_VALUE", value),
        None => crate::env::remove_var("ARTERM_MCP_EXPANSION_TEST_VALUE"),
    }
...
fn test_load_project_locals_merge_order() {
    // `.arterm/mcp.json` loads first, then `.mcp.json` overrides same-named
    // servers, then `.claude/mcp.json`.
    let temp = tempfile::tempdir().expect("tempdir");
    let project = temp.path();
    std::fs::create_dir_all(project.join(".arterm")).unwrap();
    std::fs::create_dir_all(project.join(".claude")).unwrap();
    std::fs::write(
        project.join(".arterm/mcp.json"),
        r#"{"servers":{"shared-name":{"command":"arterm-bin"},"arterm-only":{"command":"a"}}}"#,
    )
    .unwrap();
    std::fs::write(
        project.join(".mcp.json"),
    ...
    assert_eq!(
        ...
        "claude-bin",
        ".mcp.json must override .arterm/mcp.json for same-named servers"
    );
    assert!(config.servers.contains_key("arterm-only"));
...
#[test]
fn claude_json_http_entry_does_not_displace_arterm_stdio_server() {
    // The exact configuration from issue #653: `github` is stdio in
    // ~/.arterm/mcp.json and http in ~/.claude.json. The http entry used to win
    ...
    .expect("write arterm mcp config");
    // `user_home_path()` maps external configs under ARTERM_HOME to an
...
#[test]
fn claude_only_config_never_creates_a_arterm_snapshot() {
    let _guard = crate::storage::lock_test_env();
    let previous_home = std::env::var_os("ARTERM_HOME");
    let home = tempfile::tempdir().expect("home tempdir");
    crate::env::set_var("ARTERM_HOME", home.path());
    ...
    let result = std::panic::catch_unwind(|| {
        ...
        assert!(
            ...
            "a live Claude source must not be persisted into arterm config"
        );
    });
