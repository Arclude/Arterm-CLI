//! Embedding facade for arterm.
//!
//! The heavy ONNX/tokenizer implementation lives in the `arterm-embedding`
//! workspace crate so unchanged embedding code can stay cached across self-dev
//! builds. This module keeps arterm's process-wide cache, stats, and local path /
//! logging integration stable.

use anyhow::Result;
use arterm_embedding as backend;
use serde::Serialize;
...
pub fn models_dir() -> Result<PathBuf> {
    let dir = arterm_dir()?.join("models").join(backend::MODEL_NAME);
