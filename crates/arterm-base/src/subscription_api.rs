//! Typed client for the Arterm account and hosted-model billing API.
//!
...
use crate::subscription_catalog::{self, ArtermTier};
...
impl SubscriptionMe {
    pub fn parsed_tier(&self) -> Option<ArtermTier> {
        ArtermTier::parse(&self.tier)
    }
...
impl fmt::Display for AccountApiError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Unauthorized => write!(f, "the Arterm account key is revoked or expired"),
            ...
            Self::Http { status, code } => match code {
                ...
                None => write!(f, "Arterm account API returned HTTP {status}"),
            },
            Self::InvalidResponse(detail) => {
                write!(
                    f,
                    "Arterm account API returned an invalid response: {detail}"
                )
            }
        }
    }
...
pub fn configured_api_base() -> String {
    subscription_catalog::configured_api_base()
        .unwrap_or_else(|| subscription_catalog::DEFAULT_ARTERM_API_BASE.to_string())
        .trim_end_matches('/')
        .to_string()
}
...
pub async fn fetch_subscription_me() -> Result<SubscriptionMe> {
    let api_key = subscription_catalog::configured_api_key()
        .context("no Arterm account credential configured (run `arterm account login`)")?;
...
mod tests {
    ...
    async fn metered_device_request_does_not_select_a_subscription_tier() {
        ...
        assert!(request.contains(r#"{"client_name":"arterm-cli"}"#));
    ...
    async fn legacy_device_response_is_explained_without_echoing_body() {
        ...
        let error = request_device_authorization(&client(), &base, Some(ArtermTier::Pro))
            .await
