...
fn harden_secret_file_permissions_sets_owner_only_modes() {
    ...
    let secret_dir = dir.path().join("arterm");
...
#[test]
fn user_home_path_uses_external_dir_under_arterm_home() {
...
#[test]
fn app_config_dir_uses_arterm_home_when_set() {
    let _guard = lock_test_env();
    let prev_home = std::env::var_os("ARTERM_HOME");
    let temp = tempfile::TempDir::new().expect("create temp dir");
    crate::env::set_var("ARTERM_HOME", temp.path());
    ...
    assert_eq!(resolved, temp.path().join("config").join("arterm"));
