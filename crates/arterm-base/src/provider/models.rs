...
pub(crate) fn ensure_model_allowed_for_subscription(model: &str) -> Result<()> {
    ...
    match crate::subscription_catalog::find_curated_model(model) {
        None => {
            anyhow::bail!(
                "Model '{}' is not included in the current arterm subscription catalog",
                model
            );
        }
        Some(curated) => {
            ...
            if !tier.allows(curated.min_tier) {
                anyhow::bail!(
                    "Model '{}' requires the {} tier (current tier: {}). Upgrade your arterm subscription to use it.",
...
fn model_exposes_1m_alias(normalized_model: &str) -> bool {
    if normalized_model.starts_with("claude-") {
        let mode = arterm_provider_core::anthropic_context_mode(normalized_model);
        ...
        if mode != arterm_provider_core::AnthropicContextMode::Standard {
...
pub(crate) fn config_context_limit_cache_keys(profile_id: &str, model_id: &str) -> Vec<String> {
    ...
    let slash_base = arterm_provider_core::model_id::slash_base(&id).to_string();
...
pub fn provider_for_model_with_hint(
    ...
    if model.contains('@') {
        Some("openrouter")
    } else if arterm_provider_core::model_id::matches_known_model(model, ALL_CLAUDE_MODELS) {
        Some("claude")
    } else if arterm_provider_core::model_id::matches_known_model(model, ALL_OPENAI_MODELS) {
        Some("openai")
