use super::*;
use arterm_provider_core::{FailoverDecision, ProviderFailoverPrompt};
...
impl MultiProvider {
    ...
    pub(super) fn fallback_sequence(active: ActiveProvider) -> Vec<ActiveProvider> {
        arterm_provider_core::fallback_sequence(active)
    }
    ...
    pub(super) fn classify_failover_error(err: &anyhow::Error) -> FailoverDecision {
        arterm_provider_core::classify_failover_error_message(&err.to_string())
    }
