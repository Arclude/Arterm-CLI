...
pub fn remote_model_routes_fallback(
    ...
    if remote_provider_name.is_some_and(|name| {
        ...
        return remote_available_entries
            ...
            .map(|model| ModelRoute {
                ...
                available: true,
                detail: "arterm subscription routing · managed server-side".to_string(),
                cheapness: None,
            })
            .collect();
    }
    ...
    for model in remote_available_entries {
        ...
        if arterm_provider_core::model_id::matches_known_model(model, ALL_OPENAI_MODELS) {
            let availability = model_availability_for_account(model);
...
pub fn remote_model_routes_lightweight_fallback(
    ...
    let is_arterm_subscription = remote_provider_name.is_some_and(|name| {
        name.eq_ignore_ascii_case(crate::subscription_catalog::ARTERM_PROVIDER_DISPLAY_NAME)
    });
    ...
    for model in remote_available_entries {
        ...
        routes.push(ModelRoute {
            ...
            api_method: if is_arterm_subscription {
                crate::subscription_catalog::ARTERM_ROUTE_API_METHOD.to_string()
            } else {
                "remote-catalog".to_string()
            },
            available: true,
            detail: if is_arterm_subscription {
                "arterm subscription routing · managed server-side".to_string()
            } else {
...
fn remote_openai_compatible_profile_models(
    ...
    if let Some(cache) =
        arterm_provider_openrouter::load_disk_cache_entry_for_namespace(&resolved.id)
    {
...
mod tests {
    ...
    impl EnvGuard {
        fn new() -> Self {
            ...
            let vars = vec![
                ("ARTERM_HOME", std::env::var_os("ARTERM_HOME")),
        ...
        fn save_opencode_cache(&self, source_api_base: &str, model_ids: &[&str]) {
            let arterm_home = std::env::var_os("ARTERM_HOME").expect("ARTERM_HOME set");
            let cache_dir = std::path::PathBuf::from(arterm_home).join("cache");
            std::fs::create_dir_all(&cache_dir).expect("create cache dir");
            let cache = arterm_provider_openrouter::DiskCache {
                cached_at: arterm_provider_openrouter::current_unix_secs()
    ...
    fn named_provider_profile_model_routes_to_its_own_profile() {
        ...
        assert!(matches!(
            ...
            arterm_provider_core::ModelRouteApiMethod::OpenAiCompatible { .. }
        ));
    }
