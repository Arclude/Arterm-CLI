...
/// paths for OpenAI (codex-spark) and Claude (haiku) OAuth, but arterm also runs
...
pub(crate) fn cached_live_models_for_openai_compatible_profile(
    ...
    let cache = arterm_provider_openrouter::load_disk_cache_entry_for_namespace(&resolved.id)?;
    let cache_is_stale = arterm_provider_openrouter::current_unix_secs()
...
fn configured_standard_openrouter_profile_routes() -> Vec<ModelRoute> {
    let Some(cache) = arterm_provider_openrouter::load_disk_cache_entry_for_namespace("openrouter")
    else {
        return Vec::new();
    };
...
pub struct MultiProvider {
    ...
    /// `arterm-provider-copilot-runtime` and is instantiated through
    ...
    /// `arterm-provider-antigravity-runtime` and is instantiated through
    /// `external::instantiate_external_provider`.
    antigravity: RwLock<Option<Arc<dyn Provider>>>,
    /// Gemini provider (hot-swappable after login). Held as `dyn Provider`:
    /// the concrete runtime lives downstream in `arterm-provider-gemini-runtime`
    ...
    /// `dyn Provider`: the concrete runtime lives downstream in
    /// `arterm-provider-cursor-runtime` and is instantiated through
    /// `external::instantiate_external_provider`.
...
impl MultiProvider {
    ...
    fn routes_memo_key(&self) -> String {
        ...
        format!(
            "{}|{}|{}|{:?}|{}|{}|{}|{}",
            // Scope by home so sandboxes (tests, ARTERM_HOME switches) never
            // share catalogs that were built from different credential files.
            std::env::var("ARTERM_HOME").unwrap_or_default(),
    ...
    fn activity_source_key(&self, provider: ActiveProvider) -> String {
        match provider {
            ...
            ActiveProvider::OpenRouter => {
                ...
                // arterm subscription, and direct OpenAI-compatible profiles.
                let label = self
                    .active_openrouter_execution_provider()
                    .map(|execution| execution.runtime_display_name())
                    .unwrap_or_else(|| "OpenRouter".to_string());
                let runtime = std::env::var("ARTERM_RUNTIME_PROVIDER").ok();
    ...
    /// Bind the shared OpenAI-compatible slot to the managed arterm endpoint.
    ///
    ...
    fn set_model_on_arterm_subscription(&self, model: &str) -> Result<()> {
        let model = model.trim();
        if model.is_empty() {
            anyhow::bail!("Model cannot be empty");
        }
        ...
        crate::provider::activation::ProviderActivation::arterm_subscription(model).apply_env()?;
        ...
        if !identity.as_ref().is_some_and(|(_, api_method, _)| {
            ModelRouteApiMethod::parse(api_method) == ModelRouteApiMethod::ArtermSubscription
        }) {
            anyhow::bail!(
                "Refusing to select arterm subscription: managed runtime identity was not established"
            );
        }
        ...
        match provider {
            ActiveProvider::Claude => {
                ...
                if let Some(anthropic) = self.anthropic_provider() {
                    ...
                    anyhow::bail!(
                        "Claude credentials not available. Run `arterm login --provider claude` first."
                    );
                }
            ...
            ActiveProvider::OpenAI => {
                let Some(openai) = self.openai_provider() else {
                    ...
                    anyhow::bail!(
                        "OpenAI credentials not available. Run `arterm login --provider openai` first."
                    );
                };
            ...
            ActiveProvider::Copilot => {
                let Some(copilot) = self.copilot_provider() else {
                    anyhow::bail!(
                        "GitHub Copilot credentials not available. Run `arterm login --provider copilot` first."
                    );
                };
            ...
            ActiveProvider::Antigravity => {
                let Some(antigravity) = self.antigravity_provider() else {
                    anyhow::bail!(
                        "Antigravity credentials not available. Run `arterm login --provider antigravity` first."
                    );
                };
            ...
            ActiveProvider::Gemini => {
                let Some(gemini) = self.gemini_provider() else {
                    anyhow::bail!(
                        "Gemini credentials not available. Run `arterm login --provider gemini` first."
                    );
                };
            ...
            ActiveProvider::Cursor => {
                let Some(cursor) = self.cursor_provider() else {
                    anyhow::bail!(
                        "Cursor credentials not available. Run `arterm login --provider cursor` first."
                    );
                };
            ...
            ActiveProvider::OpenRouter => {
                        ...
                        provider.direct_openai_compatible_route_parts().is_some_and(
                            |(_, api_method, _)| {
                                ModelRouteApiMethod::parse(&api_method)
                                    == ModelRouteApiMethod::ArtermSubscription
                            },
                ...
                let (openrouter, install_openrouter) = if needs_rebind {
                    ...
                    let Some(openrouter) = self.openrouter_provider() else {
                        anyhow::bail!(
                            "OpenRouter/OpenAI-compatible credentials not available. Set the configured API key or run `arterm login --provider openrouter` first."
                        );
                    };
                    (openrouter, false)
                };
    ...
    fn set_model_on_openai_compatible_profile(
        ...
        if !crate::provider_catalog::openai_compatible_profile_is_configured(profile) {
            anyhow::bail!(
                "{} credentials not available. Run `arterm login --provider {}` first.",
    ...
    pub(super) fn set_config_default_model(
            ...
            Self::resolve_config_provider_selection(pref, crate::config::config())
        {
            ...
            let pinned = arterm_provider_core::AuthRoute::parse_explicit_credential_prefix(pref);
            let anthropic_credential_mode = pinned.and_then(|route| {
                ...
                .then(|| match route.mode {
                    arterm_provider_core::AuthMode::ApiKey => {
                        anthropic::AnthropicCredentialMode::ApiKey
                    }
                    arterm_provider_core::AuthMode::Oauth => {
            ...
            let openai_credential_mode = pinned.and_then(|route| {
                matches!(
                    route.provider,
                    arterm_provider_core::DualAuthProvider::OpenAI
                )
                .then(|| match route.mode {
                    arterm_provider_core::AuthMode::ApiKey => openai::OpenAICredentialMode::ApiKey,
                    arterm_provider_core::AuthMode::Oauth => openai::OpenAICredentialMode::OAuth,
                })
            });
        ...
        let prefix = match active {
            ActiveProvider::Claude => {
                if let Some(anthropic) = self.anthropic_provider() {
                    ...
                    anthropic
                        .credential_mode()
                        .auth_route(arterm_provider_core::DualAuthProvider::Anthropic)
            ...
            ActiveProvider::OpenAI => {
                if let Some(openai) = self.openai_provider() {
                    openai
                        .credential_mode()
                        .auth_route(arterm_provider_core::DualAuthProvider::OpenAI)
                        .map(|route| route.model_prefix())
                        .unwrap_or("openai")
                } else {
                    "openai"
...
impl Provider for MultiProvider {
        ...
        match self.active_provider() {
            ActiveProvider::Claude => {
                ...
                if let Some(anthropic) = self.anthropic_provider() {
                    ...
                    arterm_provider_core::DEFAULT_CLAUDE_MODEL.to_string()
                }
            }
            ActiveProvider::OpenAI => self
                .openai_provider()
                .map(|o| o.model())
                .unwrap_or_else(|| arterm_provider_core::DEFAULT_OPENAI_MODEL.to_string()),
    ...
    fn active_resolved_credential(&self) -> Option<arterm_provider_core::ResolvedCredential> {
        use arterm_provider_core::ResolvedCredential;
        match self.active_provider() {
            ActiveProvider::Claude => {
    ...
    fn active_explicit_credential(&self) -> Option<arterm_provider_core::ResolvedCredential> {
        use arterm_provider_core::ResolvedCredential;
    ...
    fn set_model(&self, model: &str) -> Result<()> {
            ...
            explicit_model_provider_prefix(requested_model)
        {
            ...
            let pinned = arterm_provider_core::AuthRoute::parse_explicit_credential_prefix(prefix);
            let openai_credential_mode = pinned.and_then(|route| {
                matches!(
                    route.provider,
                    arterm_provider_core::DualAuthProvider::OpenAI
    ...
    fn set_route_selection(&self, selection: &RouteSelection) -> Result<()> {
        ...
        if selection.runtime_key == RuntimeKey::ArtermSubscription {
            return self.set_model_on_arterm_subscription(&selection.model);
        }
    ...
    fn handles_tools_internally(&self) -> bool {
        match self.active_provider() {
            ActiveProvider::Claude => {
                // Direct API does NOT handle tools internally - arterm executes them
            ...
            ActiveProvider::Cursor => self
                ...
                .map(|o| o.handles_tools_internally())
                .unwrap_or(false),
            ActiveProvider::Bedrock => false, // arterm executes Bedrock tool calls
            ActiveProvider::OpenRouter => false, // arterm executes tools
        }
    }
    ...
    fn supports_compaction(&self) -> bool {
        match self.active_provider() {
            ...
            ActiveProvider::Bedrock => self
                .bedrock_provider()
                .map(|o| o.uses_arterm_compaction())
                .unwrap_or(false),
            ActiveProvider::OpenRouter => self
                .active_openrouter_execution_provider()
                .map(|o| o.supports_compaction())
    ...
    fn uses_arterm_compaction(&self) -> bool {
        match self.active_provider() {
            ActiveProvider::Claude => {
                if self.anthropic_provider().is_some() {
                    true
                    ...
                    self.claude_provider()
                        .map(|c| c.uses_arterm_compaction())
                        .unwrap_or(false)
                }
            }
            ActiveProvider::OpenAI => self
