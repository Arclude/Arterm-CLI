...
use arterm_provider_core::pricing as core_pricing;
use arterm_provider_core::{RouteCheapnessEstimate, RouteCostConfidence, RouteCostSource};
use std::sync::Mutex;
...
/// Memoization is skipped in test builds: test sandboxes swap `ARTERM_HOME`
...
pub(crate) fn copilot_pricing(model: &str) -> RouteCheapnessEstimate {
    let zero_premium_mode = matches!(
        std::env::var("ARTERM_COPILOT_PREMIUM").ok().as_deref(),
        Some("0")
    );
...
pub(crate) fn cheapness_for_route(
    ...
    use arterm_provider_core::{AuthMode, AuthRoute, DualAuthProvider};
...
mod tests {
    use super::*;
    use crate::env;
    use arterm_provider_core::{RouteBillingKind, RouteCostConfidence, RouteCostSource};
    ...
    fn with_clean_provider_test_env<T>(f: impl FnOnce() -> T) -> T {
        ...
        let prev_copilot_premium = std::env::var_os("ARTERM_COPILOT_PREMIUM");
        crate::auth::claude::set_active_account_override(None);
        crate::auth::codex::set_active_account_override(None);
        env::set_var("ARTERM_HOME", temp.path());
        env::remove_var("OPENAI_API_KEY");
        env::remove_var("ARTERM_COPILOT_PREMIUM");
        ...
        if let Some(prev_home) = prev_home {
            env::set_var("ARTERM_HOME", prev_home);
        } else {
            env::remove_var("ARTERM_HOME");
        }
        ...
        if let Some(prev_copilot_premium) = prev_copilot_premium {
            env::set_var("ARTERM_COPILOT_PREMIUM", prev_copilot_premium);
        } else {
            env::remove_var("ARTERM_COPILOT_PREMIUM");
        }
        result
    ...
    fn copilot_zero_mode_marks_estimate_high_confidence_and_zero_reference_cost() {
        with_clean_provider_test_env(|| {
            env::set_var("ARTERM_COPILOT_PREMIUM", "0");
