use anyhow::Result;
use arterm_provider_core::{ActiveProvider, provider_key};
...
impl RuntimeProviderId {
    ...
    pub const fn label(self) -> &'static str {
        match self {
            Self::Arterm => "Arterm Subscription",
...
impl ProviderActivation {
    ...
    pub fn azure_openai(model: Option<String>) -> Self {
        ...
        if let Some(model) = model.filter(|value| !value.trim().is_empty()) {
            activation.with_model_hint("ARTERM_OPENROUTER_MODEL", model)
        } else {
            activation
        }
    }
    ...
    pub fn arterm_subscription(model: impl Into<String>) -> Self {
        Self::initial(RuntimeProviderId::Arterm, ActiveProvider::OpenRouter)
            .with_model_hint("ARTERM_OPENROUTER_MODEL", model)
    }

    pub fn apply_env(&self) -> Result<()> {
        crate::env::set_var("ARTERM_RUNTIME_PROVIDER", self.runtime_id.key());
        match self.runtime_id {
            RuntimeProviderId::Arterm => {
            ...
            RuntimeProviderId::OpenAiCompatible => {
                if std::env::var_os("ARTERM_OPENROUTER_TRANSPORT_STATE").is_none() {
                    crate::env::set_var("ARTERM_OPENROUTER_TRANSPORT_STATE", "direct-api-key");
                }
            }
            _ => {
        ...
        match self.selection {
            RuntimeSelection::Initial(active_provider) => {
                ...
                crate::env::set_var("ARTERM_ACTIVE_PROVIDER", active_key_for_log);
                crate::env::set_var("ARTERM_INITIAL_PROVIDER_EXPLICIT", "1");
            }
...
pub fn select_initial_runtime_provider_key(provider_key_raw: &str) {
    crate::env::set_var("ARTERM_ACTIVE_PROVIDER", provider_key_raw);
    crate::env::set_var("ARTERM_INITIAL_PROVIDER_EXPLICIT", "1");
    crate::logging::auth_event(
...
mod tests {
    ...
    fn azure_activation_preserves_identity_while_using_openrouter_slot() {
        ...
        // (e.g. anthropic_tests sets ARTERM_RUNTIME_PROVIDER=claude); without
        ...
        assert_eq!(
            std::env::var("ARTERM_OPENROUTER_MODEL").as_deref(),
            Ok("gpt-4.1-mini")
        );
    }
}
