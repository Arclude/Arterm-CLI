...
fn with_clean_provider_test_env<T>(f: impl FnOnce() -> T) -> T {
    ...
    // Concrete provider runtimes live downstream (arterm-provider-*-runtime),
    ...
    let prev_subscription =
        std::env::var_os(crate::subscription_catalog::ARTERM_SUBSCRIPTION_ACTIVE_ENV);
    ...
    if let Some(prev_subscription) = prev_subscription {
        crate::env::set_var(
            crate::subscription_catalog::ARTERM_SUBSCRIPTION_ACTIVE_ENV,
            prev_subscription,
        );
    } else {
...
fn save_test_openrouter_model_cache(namespace: &str, source_api_base: &str, model_ids: &[&str]) {
    ...
    let cache_dir = std::path::PathBuf::from(arterm_home).join("cache");
    std::fs::create_dir_all(&cache_dir).expect("create model cache dir");
    let cache = arterm_provider_openrouter::DiskCache {
        cached_at: arterm_provider_openrouter::current_unix_secs().expect("current unix time"),
        source_api_base: Some(source_api_base.to_string()),
        models: model_ids
            .iter()
            .map(|id| arterm_provider_openrouter::ModelInfo {
                id: (*id).to_string(),
                name: String::new(),
                context_length: None,
                pricing: arterm_provider_openrouter::ModelPricing::default(),
                created: None,
            })
            .collect(),
    };
...
fn standard_openrouter_catalog_refresh_fires_when_named_profile_owns_slot() {
    with_clean_provider_test_env(|| {
        ...
        runtime.block_on(async {
            ...
            crate::env::set_var("ARTERM_OPENROUTER_CACHE_NAMESPACE", "mynvidia");
            ...
            arterm_provider_openrouter_runtime::reset_profile_catalog_refresh_tracker_for_tests();

            assert!(
                openrouter::maybe_schedule_standard_openrouter_catalog_refresh(
                    "unit test named profile owns slot"
                ),
                "standard OpenRouter refresh must fire even when a named profile sets ARTERM_OPENROUTER_* env"
            );
        });
    });
}
...
/// (arterm-provider-{gemini,cursor,antigravity}-runtime) and therefore cannot
...
struct StubExternalRuntime {
    ...
    credential_mode: std::sync::RwLock<arterm_provider_core::CredentialMode>,
}
...
impl StubExternalRuntime {
    fn new(
        ...
        Self {
            ...
            credential_mode: std::sync::RwLock::new(arterm_provider_core::CredentialMode::Auto),
        }
    }
...
fn register_test_external_runtimes() {
    ...
    external::register_profile_catalog_refresh(
        arterm_provider_openrouter_runtime::maybe_schedule_openai_compatible_profile_catalog_refresh,
    );
    external::register_standard_openrouter_catalog_refresh(
        arterm_provider_openrouter_runtime::maybe_schedule_standard_openrouter_catalog_refresh,
    );
}
    ...
    with_clean_provider_test_env(|| {
        ...
        runtime.block_on(async {
            ...
            assert_eq!(
                ...
                Some(arterm_provider_core::ResolvedCredential::ApiKey)
            );
