...
impl ArtermProvider {
    fn runtime_model_spec(model: &str) -> String {
        ...
        // as `arterm-subscription`, for every curated model.
    ...
    fn apply_runtime_profile() {
        let _ = crate::provider::activation::ProviderActivation::arterm_subscription(
    ...
    fn entitled_models_for(
        _tier: crate::subscription_catalog::ArtermTier,
    ...
    fn model_routes_for(tier: crate::subscription_catalog::ArtermTier) -> Vec<ModelRoute> {
...
impl Default for ArtermProvider {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl Provider for ArtermProvider {
    async fn complete(
        &self,
        messages: &[Message],
    ...
    fn uses_arterm_compaction(&self) -> bool {
        self.inner.uses_arterm_compaction()
    }
...
mod tests {
    ...
    #[test]
    fn arterm_provider_enables_subscription_runtime_mode() {
        let _guard = crate::storage::lock_test_env();
        crate::subscription_catalog::clear_runtime_env();
        let runtime = tokio::runtime::Runtime::new().expect("tokio runtime");

        runtime.block_on(async {
            let provider = ArtermProvider::new();
    ...
    #[test]
    fn arterm_provider_name_and_default_model_are_curated() {
        let _guard = crate::storage::lock_test_env();
        crate::subscription_catalog::clear_runtime_env();
        let runtime = tokio::runtime::Runtime::new().expect("tokio runtime");
        ...
        runtime.block_on(async {
            let provider = ArtermProvider::new();
            assert_eq!(provider.name(), "Arterm Subscription");
    ...
    #[test]
    fn arterm_models_are_pinned_to_the_managed_transport() {
        assert_eq!(
            ArtermProvider::runtime_model_spec("claude-opus-4-8"),
            "openrouter:claude-opus-4-8"
        );
        assert_eq!(
            ArtermProvider::runtime_model_spec("gpt-5.5"),
            "openrouter:gpt-5.5"
        );
        assert_eq!(
            ArtermProvider::runtime_model_spec("minimax-m2.5"),
            "openrouter:minimax-m2.5"
        );
    }

    #[test]
    fn arterm_provider_exposes_only_explicit_subscription_routes() {
        use crate::subscription_catalog::ArtermTier;

        let plus_routes = ArtermProvider::model_routes_for(ArtermTier::Plus);
        let gpt_route = plus_routes
            .iter()
            .find(|route| route.model == "gpt-5.5")
            .expect("Plus tier includes GPT-5.5");
        let route_selection = arterm_provider_core::RouteSelection::from_model_route(gpt_route);
        let flagship_routes = ArtermProvider::model_routes_for(ArtermTier::Flagship);
        ...
        assert!(plus_routes.iter().all(|route| {
            route.provider == "Arterm Subscription"
                && route.api_method == "arterm-subscription"
                && route.available
        }));
        assert_eq!(
            ArtermProvider::entitled_models_for(ArtermTier::Plus)
                .map(|model| model.id.to_string())
                .collect::<Vec<_>>(),
            expected_models
                .iter()
        ...
        assert_eq!(
            ...
            arterm_provider_core::RuntimeKey::ArtermSubscription
        );
        assert_eq!(route_selection.api_method, "arterm-subscription");
        assert_eq!(route_selection.provider_label, "Arterm Subscription");
