...
fn test_direct_chutes_ignores_legacy_openrouter_catalog_cache() {
    with_clean_provider_test_env(|| {
        ...
        with_env_var("HOME", &home, || {
            let cache_dir = temp_home.path().join(".arterm").join("cache");
...
fn test_state_space_openrouter_default_survives_switch_to_nvidia_nim() {
    with_clean_provider_test_env(|| {
        ...
        assert!(
            std::env::var_os("ARTERM_OPENROUTER_API_BASE").is_none(),
...
fn test_active_compatible_route_treats_claude_like_bare_model_as_provider_local() {
    with_clean_provider_test_env(|| {
        with_env_var("OPENROUTER_API_KEY", "test-openrouter-key", || {
            with_env_var("ARTERM_OPENROUTER_PROVIDER_FEATURES", "0", || {
                with_env_var(
...
fn test_config_default_provider_openai_compatible_keeps_gpt_model_provider_local() {
    with_clean_provider_test_env(|| {
        with_env_var(
            ...
            "https://compat.example.test/v1",
            || {
                with_env_var("ARTERM_OPENAI_COMPAT_API_KEY_NAME", "OPENAI_API_KEY", || {
...
fn test_named_config_provider_models_appear_in_picker_and_are_selectable() {
    ...
    with_clean_provider_test_env(|| {
        let arterm_home = std::env::var_os("ARTERM_HOME").expect("test ARTERM_HOME should be set");
        std::fs::write(
            std::path::PathBuf::from(arterm_home).join("config.toml"),
            r#"
[provider]
...
fn test_openai_auth_mode_prefixed_model_switch_changes_credentials() {
    with_clean_provider_test_env(|| {
        let prev_runtime = std::env::var_os("ARTERM_RUNTIME_PROVIDER");
        ...
        // covered by arterm-provider-openai-runtime's tests.
        assert_eq!(
            openai.credential_mode(),
            arterm_provider_core::CredentialMode::Auto,
        ...
        assert_eq!(
            openai.credential_mode(),
            arterm_provider_core::CredentialMode::ApiKey
        );
        ...
        assert_eq!(
            openai.credential_mode(),
            arterm_provider_core::CredentialMode::OAuth
        );

        if let Some(prev_runtime) = prev_runtime {
            crate::env::set_var("ARTERM_RUNTIME_PROVIDER", prev_runtime);
        } else {
            crate::env::remove_var("ARTERM_RUNTIME_PROVIDER");
        }
    });
...
fn test_initial_openai_provider_can_switch_to_anthropic_auth_routes() {
    with_clean_provider_test_env(|| {
        ...
        // for each pin is covered by arterm-provider-anthropic-runtime's
...
fn test_config_default_provider_anthropic_api_pins_api_credential() {
    use arterm_provider_core::{Provider, ResolvedCredential};
...
fn test_config_default_model_with_credential_prefix_applies_model_and_pin() {
    ...
    for (spec, expect_oauth) in [
        ...
        with_clean_provider_test_env(|| {
            ...
            // covered by arterm-provider-anthropic-runtime's tests.
            if expect_oauth {
                assert_ne!(
                    anthropic.credential_mode(),
                    arterm_provider_core::CredentialMode::ApiKey,
...
fn isolate_openrouter_autodetect_env_or() -> Vec<OrEnvVarGuard> {
    let mut guards = vec![
        OrEnvVarGuard::remove("ARTERM_OPENROUTER_API_BASE"),
        OrEnvVarGuard::remove("ARTERM_OPENROUTER_API_KEY_NAME"),
        OrEnvVarGuard::remove("ARTERM_OPENROUTER_ENV_FILE"),
        OrEnvVarGuard::remove("ARTERM_OPENROUTER_DYNAMIC_BEARER_PROVIDER"),
        OrEnvVarGuard::remove("ARTERM_OPENROUTER_MODEL"),
        OrEnvVarGuard::remove("ARTERM_OPENROUTER_CACHE_NAMESPACE"),
        OrEnvVarGuard::remove("ARTERM_OPENROUTER_ALLOW_NO_AUTH"),
        OrEnvVarGuard::remove("ARTERM_OPENROUTER_TRANSPORT_STATE"),
        OrEnvVarGuard::remove("ARTERM_OPENROUTER_PROVIDER_FEATURES"),
        OrEnvVarGuard::remove("ARTERM_OPENROUTER_MODEL_CATALOG"),
        OrEnvVarGuard::remove("ARTERM_OPENROUTER_AUTH_HEADER"),
        OrEnvVarGuard::remove("ARTERM_OPENROUTER_AUTH_HEADER_NAME"),
        OrEnvVarGuard::remove("ARTERM_OPENROUTER_STATIC_MODELS"),
        OrEnvVarGuard::remove("ARTERM_ACTIVE_PROVIDER"),
        OrEnvVarGuard::remove("ARTERM_RUNTIME_PROVIDER"),
        OrEnvVarGuard::remove("ARTERM_NAMED_PROVIDER_PROFILE"),
        OrEnvVarGuard::remove("ARTERM_PROVIDER_PROFILE_NAME"),
        OrEnvVarGuard::remove("ARTERM_PROVIDER_PROFILE_ACTIVE"),
        OrEnvVarGuard::remove("ARTERM_OPENAI_COMPAT_API_BASE"),
        OrEnvVarGuard::remove("ARTERM_OPENAI_COMPAT_API_KEY_NAME"),
        OrEnvVarGuard::remove("ARTERM_OPENAI_COMPAT_ENV_FILE"),
        OrEnvVarGuard::remove("ARTERM_OPENAI_COMPAT_SETUP_URL"),
        OrEnvVarGuard::remove("ARTERM_OPENAI_COMPAT_DEFAULT_MODEL"),
        OrEnvVarGuard::remove("ARTERM_OPENAI_COMPAT_LOCAL_ENABLED"),
...
fn default_named_openai_compatible_provider_uses_direct_compatible_request_path() {
    ...
    let _arterm_home = OrEnvVarGuard::set("ARTERM_HOME", &arterm_home);
