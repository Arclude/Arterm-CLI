...
const CLAUDE_CODE_ARTERM_NOTICE: &str = "You are arterm, powered by Claude Code. You are a third-party CLI, not the official Claude Code CLI.";
...
impl Sidecar {
    ...
    /// 3. The live agent provider (works for EVERY provider arterm supports:
    ...
    ///
    /// This is the universal path: it works for every provider arterm supports,
...
fn build_claude_system_param(system: &str) -> Option<ClaudeApiSystem<'_>> {
    ...
    blocks.push(ClaudeApiSystemBlock {
        block_type: "text",
        text: CLAUDE_CODE_ARTERM_NOTICE,
    });
    if !system.is_empty() {
        blocks.push(ClaudeApiSystemBlock {
            block_type: "text",
...
///
/// The "You are Claude Code" identity spoof and arterm notice are only valid
...
fn anthropic_sidecar_prefers_api_key() -> bool {
    match arterm_provider_core::runtime_env_pinned_mode(
        arterm_provider_core::DualAuthProvider::Anthropic,
    ) {
        Some(arterm_provider_core::AuthMode::ApiKey) => true,
        Some(arterm_provider_core::AuthMode::Oauth) => false,
...
mod tests {
    ...
    fn test_backend_selection_prefers_openai() {
        ...
        let temp = tempfile::TempDir::new().expect("create temp arterm home");
    ...
    impl crate::provider::Provider for StubProvider {
        async fn complete(
            ...
            let stream = futures::stream::once(async move {
                Ok(arterm_message_types::StreamEvent::TextDelta(reply))
            });
            Ok(Box::pin(stream))
        }
    ...
    /// Every provider arterm supports should drive the sidecar end-to-end via the
    ...
    fn test_anthropic_sidecar_prefers_api_key_respects_pinned_mode() {
        ...
        let _g =
            EnvVarGuard::set_path("ARTERM_RUNTIME_PROVIDER", std::path::Path::new("claude-api"));
        assert!(
        ...
        let _g2 = EnvVarGuard::set_path("ARTERM_RUNTIME_PROVIDER", std::path::Path::new("claude"));
        assert!(
            !anthropic_sidecar_prefers_api_key(),
