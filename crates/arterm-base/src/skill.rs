...
impl SkillRegistry {
    ...
    ///
    /// Holds GLOBAL skills only (plugins, `~/.arterm/skills/`,
    ...
    /// Only runs if ~/.arterm/skills/ doesn't exist yet.
    fn import_from_external() {
        let arterm_skills = match crate::storage::arterm_dir() {
            Ok(dir) => dir.join("skills"),
            Err(_) => return,
        };

        if arterm_skills.exists() {
        ...
        {
            let count = Self::copy_skills_dir(&claude_skills, &arterm_skills);
            if count > 0 {
                sources.push(format!("{} from Claude Code", count));
                copied.extend(Self::list_skill_names(&arterm_skills));
            }
        }
        ...
        if let Ok(codex_skills) = crate::storage::user_home_path(".codex/skills")
            && codex_skills.is_dir()
        {
            let count = Self::copy_skills_dir(&codex_skills, &arterm_skills);
            if count > 0 {
    ...
    /// `~/.arterm/skills/`, and `~/.agents/skills/`.
    ///
    ...
    pub fn load_global() -> Result<Self> {
        ...
        // first, so explicit arterm/agents skills with the same name win below.
        if let Some(plugins_root) = Self::claude_plugins_root() {
            registry.load_plugin_skills_from_root(&plugins_root);
        }

        // Load from ~/.arterm/skills/ (arterm's own global skills)
        if let Ok(arterm_dir) = crate::storage::arterm_dir() {
            let arterm_skills = arterm_dir.join("skills");
            if arterm_skills.exists() {
                registry.load_from_dir(&arterm_skills)?;
            }
        }
    ...
    /// `./.arterm/skills/`, `./.agents/skills/`, and `./.claude/skills/`.
    ///
    ...
    fn load_project_local_dirs(&mut self, working_dir: Option<&Path>) -> Result<()> {
        // Load from ./.arterm/skills/ (project-local arterm skills)
        let local_arterm = Self::project_local_dir(working_dir, ".arterm");
        if local_arterm.exists() {
            self.load_from_dir(&local_arterm)?;
        }
    ...
    /// plugin never prevents arterm's own skills from loading.
    ...
    /// Reload the shared global skill sources (plugins, `~/.arterm/skills/`,
    ...
    pub fn reload_global(&mut self) -> Result<usize> {
        ...
        if let Ok(arterm_dir) = crate::storage::arterm_dir() {
            let arterm_skills = arterm_dir.join("skills");
            if arterm_skills.exists() {
                count += self.load_from_dir_count(&arterm_skills)?;
            }
        }
...
/// A skill recommended/curated by arterm that the user may want to install.
...
pub struct EndorsedSkill {
    ...
    /// Grouping label used to organize the endorsed list (e.g. "arterm",
    /// "NVIDIA CUDA-X").
...
/// Curated list of skills endorsed by arterm. Used by the `/skills` command to
...
pub const ENDORSED_SKILLS: &[EndorsedSkill] = &[
    EndorsedSkill {
        ...
        category: "arterm",
        source: "bundled in arterm repo (.arterm/skills/optimization)",
        install: None,
    },
    EndorsedSkill {
        ...
        category: "arterm",
        source: "bundled with arterm / Claude Code skills",
        install: None,
    },
    EndorsedSkill {
...
/// Return the curated list of skills endorsed by arterm.
...
mod tests {
    ...
    #[test]
    fn load_for_working_dir_reads_project_local_arterm_skills() {
        let temp = tempfile::tempdir().expect("tempdir");
        write_test_skill(temp.path(), ".arterm", "wd-only");
    ...
    fn project_overlay_is_session_scoped_and_composes_over_globals() {
        let temp = tempfile::tempdir().expect("tempdir");
        write_test_skill(temp.path(), ".arterm", "session-skill");
    ...
    fn registry_contains_reports_loaded_skills() {
        let temp = tempfile::tempdir().expect("tempdir");
        write_test_skill(temp.path(), ".arterm", "present-skill");
    ...
    #[test]
    fn explicit_arterm_skill_wins_over_plugin_skill_with_same_name() {
        let temp = tempfile::tempdir().expect("tempdir");
        ...
        // Explicit arterm skill with the same name.
        write_test_skill(temp.path(), ".arterm", "shared-name");
        ...
        registry.load_plugin_skills_from_root(&plugins_root);
        registry
            .load_from_dir(&temp.path().join(".arterm/skills"))
        ...
        assert_eq!(
            ...
            "explicit arterm skill must override the plugin-provided one"
        );
    }
