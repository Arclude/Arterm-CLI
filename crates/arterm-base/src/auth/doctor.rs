...
/// True when `arterm provider-doctor` has a native-runtime driver for
...
/// Cursor, Copilot, Bedrock, arterm, Azure OpenAI).
///
/// The drivers themselves live downstream in the `arterm-provider-doctor`
...
/// crate. A sync test in `arterm-provider-doctor` asserts this list matches its
...
pub fn diagnostics(
    ...
    match assessment.state {
        AuthState::NotConfigured => diagnostics.push(format!(
            "{} is not configured for arterm yet.",
...
pub fn recommended_actions(
    ...
    match assessment.state {
        AuthState::NotConfigured => actions.push(format!(
            "Connect it: arterm login --provider {}",
            provider.id
        )),
        AuthState::Expired
            if matches!(
        ...
        {
            actions.push(format!(
                "Re-run login; this provider cannot auto-refresh: arterm login --provider {}",
                provider.id
            ));
        }
        AuthState::Expired => actions.push(format!(
            "Refresh or replace the current login: arterm login --provider {}",
            provider.id
        )),
        AuthState::Available => {}
    }
        ...
        .and_then(|record| record.last_error.as_deref())
    {
        ...
        if lower.contains("invalid_grant") || lower.contains("refresh token") {
            actions.push(format!(
                "Replace the stale OAuth account/token: arterm login --provider {}",
                provider.id
            ));
            ...
            || lower.contains("too many requests")
        {
            ...
            actions.push(format!(
                "Retry credential refresh by re-running validation: arterm auth doctor {} --validate",
                provider.id
            ));
        }
    }
    ...
    if assessment.state != AuthState::NotConfigured {
        match assessment.last_validation.as_ref() {
            None => actions.push(format!(
                "Run runtime verification: arterm auth-test --provider {}",
                provider.id
            )),
            Some(record) if !record.success => actions.push(format!(
                "Inspect runtime readiness: arterm auth-test --provider {}",
                provider.id
            )),
            Some(record) if validation_is_stale(record.checked_at_ms) => actions.push(format!(
                "Refresh stale runtime verification: arterm auth-test --provider {}",
                provider.id
            )),
            Some(_) => {}
        }
    ...
    if validation_result.is_some_and(|value| value != "validation passed") {
        actions.push(format!(
            "Re-run detailed auth diagnostics: arterm auth-test --provider {}",
            provider.id
        ));
    }
    ...
    {
        actions.push(format!(
            "For browser/callback issues, use the manual-safe flow: arterm login --provider {} --print-auth-url",
            provider.id
        ));
    }

    actions.push("Review current state: arterm auth status --json".to_string());
    actions.dedup();
    actions
}
...
mod tests {
    ...
    fn base_assessment() -> ProviderAuthAssessment {
        ProviderAuthAssessment {
            ...
            credential_source: AuthCredentialSource::ArtermManagedFile,
            credential_source_detail: "~/.arterm/auth.json".to_string(),
