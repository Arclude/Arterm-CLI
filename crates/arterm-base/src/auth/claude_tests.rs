...
#[test]
fn arterm_auth_file_default_is_empty() {
    let auth = ArtermAuthFile::default();
    assert!(auth.anthropic_accounts.is_empty());
    assert!(auth.active_anthropic_account.is_none());
}

#[test]
fn arterm_auth_file_roundtrip() {
    let auth = ArtermAuthFile {
    ...
    let parsed: ArtermAuthFile = serde_json::from_str(&json).unwrap();
...
#[test]
fn arterm_path_respects_arterm_home() {
    let _lock = crate::storage::lock_test_env();
    let temp = tempfile::TempDir::new().unwrap();
    let _home = EnvVarGuard::set("ARTERM_HOME", temp.path());

    assert_eq!(arterm_path().unwrap(), temp.path().join("auth.json"));
    assert_eq!(
...
#[test]
fn arterm_auth_file_multi_account() {
    let auth = ArtermAuthFile {
...
#[test]
fn arterm_auth_file_legacy_migration_format() {
    let legacy_json = r#"{
        "anthropic": {
            "access": "legacy_acc",
            "refresh": "legacy_ref",
    ...
    let parsed: ArtermAuthFile = serde_json::from_str(legacy_json).unwrap();
...
fn update_account_profile_sets_email() {
    let mut auth = ArtermAuthFile::default();
...
///   cargo test -p arterm-base --lib auth::claude::tests::live_keychain -- --ignored --nocapture
#[cfg(target_os = "macos")]
#[test]
