...
/// Cached auth status plus the `ARTERM_HOME` it was computed under.
///
/// Auth probes read credential files relative to `ARTERM_HOME`. Tests swap
/// `ARTERM_HOME` to per-test temp dirs, and a status computed under one home
...
/// CLI tools don't get installed/uninstalled while arterm is running, so caching
...
///
/// Without this, arterm would open a browser that does not exist (or that opens
...
/// location no installed or self-dev arterm binary ever runs from.
///
...
/// multiple browser tabs. Set `ARTERM_ALLOW_BROWSER_IN_TESTS=1` to opt out
...
pub fn running_in_test_harness() -> bool {
    ...
    *IN_TEST_HARNESS.get_or_init(|| {
        if env_truthy("ARTERM_ALLOW_BROWSER_IN_TESTS") {
            return false;
        }
        std::env::current_exe()
            .ok()
...
fn log_auth_status_snapshot(event: &str, status: &AuthStatus) {
    crate::logging::auth_event(
        ...
        "all",
        &[
            ("arterm", auth_state_label(status.arterm)),
...
impl AuthStatus {
    ...
    /// per-test `ARTERM_HOME` swaps would poison the shared cache.
    ...
    /// arterm believes credentials are available/expired/missing without leaking
    ...
    pub fn log_snapshot(&self, surface: &str) {
        crate::logging::event_info(
            ...
            vec![
                ...
                ("arterm", self.arterm.label().to_string()),
    ...
    pub fn state_for_key(&self, key: LoginProviderAuthStateKey) -> AuthState {
        match key {
            ...
            LoginProviderAuthStateKey::Arterm => self.arterm,
    ...
    pub fn state_for_provider(&self, provider: LoginProviderDescriptor) -> AuthState {
        // Answered from its own credential file, never from the target it
        // shares with the API-key entry -- the ClaudeApiKey comment below
        // describes the borrowed-availability bug that rule prevents.
        if provider.id == crate::provider_catalog::XAI_OAUTH_LOGIN_PROVIDER.id {
            return crate::auth::xai::auth_state();
        }
        ...
        let (
            ...
            crate::provider_catalog::LoginProviderTarget::Arterm => {
                let (source, detail) = summarize_sources(vec![
                    env_source(crate::subscription_catalog::ARTERM_API_KEY_ENV),
                    config_source(
                        crate::subscription_catalog::ARTERM_API_KEY_ENV,
                        crate::subscription_catalog::ARTERM_ENV_FILE,
                        "~/.config/arterm/arterm-subscription.env",
                    ),
                ]);
                (
                    source,
            ...
            crate::provider_catalog::LoginProviderTarget::ClaudeApiKey => {
                ...
                // config file (`~/.config/arterm/anthropic.env`), *not* an env
                // var and *not* `~/.arterm/auth.json` (which holds the separate
            ...
            crate::provider_catalog::LoginProviderTarget::Azure => {
                let (source, detail) = summarize_sources(vec![
                    ...
                    config_source(
                        ...
                        "~/.config/arterm/azure-openai.env",
                    ),
                ]);
                (
                    source,
            ...
            crate::provider_catalog::LoginProviderTarget::Bedrock => {
                let (source, detail) = summarize_sources(vec![
                    ...
                    config_source(
                        ...
                        "~/.config/arterm/bedrock.env",
                    ),
                    env_source("AWS_PROFILE"),
                    env_source("ARTERM_BEDROCK_PROFILE"),
                    ...
                    crate::provider_catalog::active_named_provider_profile_credential_source()
                {
                    ...
                    summarize_sources(vec![
                        ...
                        config_source(
                            &resolved.api_key_env,
                            &resolved.env_file,
                            format!("~/.config/arterm/{}", resolved.env_file),
                        ),
...
fn build_auth_status_uncached(mode: AuthProbeMode) -> (AuthStatus, Vec<(&'static str, u128)>) {
    ...
    record_auth_probe_step(&mut timings, "arterm", || probe_arterm_status(&mut status));
...
/// dead in `/login`, the header, onboarding, and `arterm auth status`.
///
...
fn probe_arterm_status(status: &mut AuthStatus) {
    if crate::subscription_catalog::has_credentials() {
        status.arterm = AuthState::Available;
    }
}
...
fn assessment_for_key(
    ...
    match key {
        ...
        LoginProviderAuthStateKey::Arterm
...
fn anthropic_oauth_source(status: &AuthStatus) -> Option<(AuthCredentialSource, String)> {
    ...
    {
        return Some((
            AuthCredentialSource::ArtermManagedFile,
            "~/.arterm/auth.json".to_string(),
        ));
    }
...
fn openai_oauth_source(status: &AuthStatus) -> Option<(AuthCredentialSource, String)> {
        ...
        .is_empty()
    {
        return Some((
            AuthCredentialSource::ArtermManagedFile,
            "~/.arterm/openai-auth.json".to_string(),
        ));
    }
