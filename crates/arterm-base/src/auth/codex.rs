...
const ALLOW_LEGACY_AUTH_ENV: &str = "ARTERM_ALLOW_CODEX_LEGACY_AUTH";
...
pub struct ArtermOpenAiAuthFile {
...
fn relabel_accounts(auth: &mut ArtermOpenAiAuthFile) -> bool {
...
fn arterm_auth_path() -> Result<PathBuf> {
    Ok(crate::storage::arterm_dir()?.join("openai-auth.json"))
}
...
pub fn save_auth_file(auth: &ArtermOpenAiAuthFile) -> Result<()> {
    let auth_path = arterm_auth_path()?;
    let clean = ArtermOpenAiAuthFile {
...
fn load_arterm_credentials() -> Result<CodexCredentials> {
    let auth = load_auth_file()?;
    if auth.openai_accounts.is_empty() {
        anyhow::bail!("No OpenAI accounts configured in arterm auth file")
    }
    ...
    let account = auth
        ...
        .context("No OpenAI accounts in arterm auth file")?;
...
/// the raw tokens plus a `last_refresh` timestamp. Without an expiry, arterm
...
pub(crate) fn expires_at_from_access_token(access_token: &str) -> Option<i64> {
    ...
    // `exp` is in seconds since epoch; the rest of arterm tracks expiry in millis.
