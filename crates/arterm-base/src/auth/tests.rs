...
fn auth_status_check_fast_includes_bedrock_probe() {
    ...
    let prev_bedrock_enable = std::env::var_os("ARTERM_BEDROCK_ENABLE");
    ...
    restore_env_var("ARTERM_BEDROCK_ENABLE", prev_bedrock_enable);
    AuthStatus::invalidate_cache();
}
...
fn full_and_fast_auth_status_match_for_shared_probe_fields() {
    ...
    assert_eq!(full.arterm, AuthState::Available);
...
fn full_and_fast_auth_status_document_cursor_cli_exception() {
    ...
    crate::env::remove_var("CURSOR_ACCESS_TOKEN");
    crate::env::remove_var("CURSOR_REFRESH_TOKEN");
    crate::env::set_var("ARTERM_CURSOR_CLI_PATH", &mock_cli);
    AuthStatus::invalidate_cache();
...
fn assert_auth_status_shared_fields_match(full: &AuthStatus, fast: &AuthStatus) {
    assert_eq!(full.arterm, fast.arterm, "arterm");
    assert_eq!(
...
fn openrouter_status_excludes_shared_compatible_transport() {
    ...
    crate::env::set_var("ARTERM_OPENROUTER_API_BASE", "https://example.test/v1");
    crate::env::set_var("ARTERM_OPENROUTER_API_KEY_NAME", "OPENAI_API_KEY");
...
fn cursor_status_is_available_when_api_key_exists_without_cli() {
    ...
    let prev_cli_path = std::env::var_os("ARTERM_CURSOR_CLI_PATH");
    let temp = tempfile::TempDir::new().expect("create temp dir");
    ...
    restore_env_var("ARTERM_CURSOR_CLI_PATH", prev_cli_path);
    AuthStatus::invalidate_cache();
}
...
fn configured_api_key_source_uses_valid_overrides() {
    let _lock = crate::storage::lock_test_env();
    let key_var = "ARTERM_OPENAI_COMPAT_API_KEY_NAME";
    let file_var = "ARTERM_OPENAI_COMPAT_ENV_FILE";
...
/// it even without --no-browser or NO_BROWSER/ARTERM_NO_BROWSER.
#[test]
...
/// in `/login`, the header, onboarding, and `arterm auth status`, which is what
