...
//! [`AuthStatus`] plus the `ARTERM_RUNTIME_PROVIDER` override on its own, keyed on
...
impl From<arterm_provider_core::ResolvedCredential> for ActiveCredential {
    fn from(value: arterm_provider_core::ResolvedCredential) -> Self {
        match value {
            arterm_provider_core::ResolvedCredential::Oauth => Self::OAuth,
            arterm_provider_core::ResolvedCredential::ApiKey => Self::ApiKey,
        }
    }
}
...
pub struct ResolvedProviderAuth {
    ...
    /// True when the active credential was pinned via `ARTERM_RUNTIME_PROVIDER`
...
///
/// `runtime_provider` is the raw `ARTERM_RUNTIME_PROVIDER` value (if any); it
...
pub fn resolve_dual_credential_auth(
    ...
    let dual = arterm_provider_core::DualAuthProvider::from_active_provider(provider)?;
    ...
    let forced =
        arterm_provider_core::pinned_mode_for(dual, runtime_provider).map(|mode| match mode {
            arterm_provider_core::AuthMode::Oauth => ActiveCredential::OAuth,
            arterm_provider_core::AuthMode::ApiKey => ActiveCredential::ApiKey,
        });
