...
const CLIENT_ID_ENV: &str = "ARTERM_ANTIGRAVITY_CLIENT_ID";
const CLIENT_SECRET_ENV: &str = "ARTERM_ANTIGRAVITY_CLIENT_SECRET";
const VERSION_ENV: &str = "ARTERM_ANTIGRAVITY_VERSION";
...
pub fn tokens_path() -> Result<std::path::PathBuf> {
    Ok(crate::storage::arterm_dir()?.join("antigravity_oauth.json"))
}
...
pub fn load_tokens() -> Result<AntigravityTokens> {
    ...
    if path.exists() {
        ...
        return crate::storage::read_json(&path).map_err(|_| {
            anyhow::anyhow!(
                "No Antigravity tokens found. Run `arterm login --provider antigravity`."
            )
        });
    }
    ...
    anyhow::bail!("No Antigravity tokens found. Run `arterm login --provider antigravity`.");
}
...
pub async fn login(no_browser: bool) -> Result<AntigravityTokens> {
        ...
        && let Ok(listener) = crate::auth::oauth::bind_callback_listener(DEFAULT_PORT)
    {
        ...
        if browser_opened {
            ...
            eprintln!(
                "If the browser lands on a loopback error page instead of returning to arterm, copy the full URL from the address bar and re-run with `--no-browser` to paste it manually."
            );
...
async fn manual_login(
    ...
    eprintln!(
        "After approving access, paste the full callback URL (or query string) here so arterm can verify the login state.\n"
    );
    eprintln!(
