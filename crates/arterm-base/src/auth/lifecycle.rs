...
use arterm_provider_core::ActiveProvider;
...
fn globally_preferred_model_rank(model: &str) -> (u8, usize) {
    ...
    let openai_default = normalize_model_for_preference(arterm_provider_core::DEFAULT_OPENAI_MODEL);
    let claude_default = normalize_model_for_preference(arterm_provider_core::DEFAULT_CLAUDE_MODEL);

    if normalized == openai_default {
        return (0, 0);
    }
    // Some catalogs expose the clean release id instead of arterm's Sol route.
...
/// order. The Claude/OpenAI subscription default bias mirrors arterm's global
...
fn normalize_model_for_preference(model: &str) -> String {
    let mut id = arterm_provider_core::model_id::canonical(model);
    ...
    id = arterm_provider_core::model_id::strip_date_suffix(&id).to_string();
...
fn route_matches_activation(route: &ModelRoute, activation: &AuthActivationResult) -> bool {
    ...
    match provider_id {
        ...
        "arterm" => {
            // Arterm subscription routes deliberately keep their managed public
            // identity even though the runtime reuses OpenRouter transport code.
            return matches!(
                api_method,
                crate::provider::ModelRouteApiMethod::ArtermSubscription
            );
        }
        "azure-openai" => {
...
/// `arterm_provider_core::auto_default_provider`: Claude wins when both families
...
fn normalized_login_provider_id(provider_id: &str) -> Option<&'static str> {
    match provider_id.trim().to_ascii_lowercase().as_str() {
        ...
        "arterm" | "subscription" | "arterm-subscription" => Some("arterm"),
...
fn api_key_env_bindings_for_provider(provider_id: &str) -> Vec<(String, String)> {
    match provider_id {
        ...
        "arterm" => vec![
            (
                crate::subscription_catalog::ARTERM_API_KEY_ENV.to_string(),
                crate::subscription_catalog::ARTERM_ENV_FILE.to_string(),
            ),
            (
                crate::subscription_catalog::ARTERM_API_BASE_ENV.to_string(),
                crate::subscription_catalog::ARTERM_ENV_FILE.to_string(),
            ),
        ],
        "bedrock" => vec![
...
/// process (issue #453).
///
/// `/login` persists API keys to the per-provider env file under the arterm
...
fn direct_provider_activation(provider_id: &str) -> Option<ProviderActivation> {
    let (runtime_id, active) = match normalized_login_provider_id(provider_id)? {
        ...
        "arterm" => (RuntimeProviderId::Arterm, ActiveProvider::OpenRouter),
...
mod tests {
    ...
    fn direct_login_provider_ids_are_normalized_with_display_labels() {
        for (hint, normalized, label) in [
            ...
            ("subscription", "arterm", "Arterm Subscription"),
    ...
    fn direct_login_provider_activation_sets_runtime_identity_and_active_provider() {
        // Sandbox ARTERM_HOME so activation's env-file credential sync (#453)
        // cannot read the developer's real ~/.config/arterm/*.env files and
        ...
        for (provider, runtime, active) in [
            ...
            crate::env::remove_var("ARTERM_INITIAL_PROVIDER_EXPLICIT");
            ...
            assert_eq!(
                std::env::var("ARTERM_RUNTIME_PROVIDER").as_deref(),
                Ok(runtime)
            );
            assert_eq!(
                std::env::var("ARTERM_ACTIVE_PROVIDER").as_deref(),
                Ok(active)
            );
            assert_eq!(
                std::env::var("ARTERM_INITIAL_PROVIDER_EXPLICIT").as_deref(),
                Ok("1")
            );
        }
    }
    ...
    fn direct_login_provider_descriptor_matrix_has_full_lifecycle_parity() {
        // Sandbox ARTERM_HOME for the same reason as the activation matrix
        ...
        for provider in crate::provider_catalog::login_providers() {
            let Some((normalized, runtime, active, switch_prefix)) = (match provider.target {
                crate::provider_catalog::LoginProviderTarget::Arterm => {
                    Some(("arterm", "arterm", "openrouter", ""))
                }
    ...
    #[test]
    fn arterm_auth_lifecycle_matches_only_managed_subscription_routes() {
        let activation = AuthActivationResult {
            provider_id: Some("arterm".to_string()),
            provider_label: Some("Arterm Subscription".to_string()),
            activated_model: Some("gpt-5.5".to_string()),
            expected_runtime: Some("arterm-subscription".to_string()),
            expected_catalog_namespace: Some("arterm-subscription".to_string()),
        };
        let routes = vec![
            ...
            route(
                "gpt-5.5",
                "Arterm Subscription",
                crate::subscription_catalog::ARTERM_ROUTE_API_METHOD,
                true,
            ),
        ];
        ...
        assert!(
            report.ok(),
            "canonical Arterm route should match: {report:?}"
        );
        assert_eq!(report.selectable_provider_routes, 1);
        assert_eq!(
            report.route_sample,
        ...
        assert_eq!(
            activation.model_switch_request("Arterm Subscription", "gpt-5.5"),
            "gpt-5.5"
        );
    }
    ...
    fn post_auth_model_selection_keeps_catalog_order_for_unranked_providers() {
        ...
        // shared test-env lock (and a private ARTERM_HOME) or a sibling test's
        // cached catalog decides this assertion depending on ordering.
        let _env = EnvGuard::new(&["ARTERM_HOME"]);
    ...
    /// Claude flagship over the OpenAI flagship to mirror arterm's default model.
    #[test]
