...
impl AuthTestSandbox {
    pub fn new() -> anyhow::Result<Self> {
        ...
        std::fs::create_dir_all(temp.path().join("config").join("arterm"))?;
    ...
    pub fn config_dir(&self) -> PathBuf {
        self.root().join("config").join("arterm")
    }
...
mod tests {
    ...
    #[test]
    fn sandbox_isolates_arterm_home_and_config_dir() {
        let sandbox = AuthTestSandbox::new().expect("sandbox");

        assert_eq!(
            std::env::var("ARTERM_HOME").ok().as_deref(),
