...
impl MemoryManager {
    ...
    pub fn project_memory_path(&self) -> Result<Option<PathBuf>> {
        ...
        let memory_dir = storage::arterm_dir()?.join("memory").join("projects");
        Ok(Some(memory_dir.join(format!("{}.json", project_hash))))
    }

    fn legacy_notes_path(&self) -> Result<Option<PathBuf>> {
        if self.test_mode {
            let test_dir = storage::arterm_dir()?.join("notes").join("test");
            std::fs::create_dir_all(&test_dir)?;
    ...
    fn global_memory_path(&self) -> Result<PathBuf> {
        if self.test_mode {
            ...
            Ok(storage::arterm_dir()?.join("memory").join("global.json"))
        }
    }
    ...
    fn should_generate_embedding_for_entry(&self, entry: &MemoryEntry) -> bool {
        ...
        #[cfg(test)]
        if std::env::var_os("ARTERM_TEST_ALLOW_MEMORY_EMBEDDINGS").is_none() {
            return false;
        }
