...
fn test_skill_prompt_integration() {
    ...
    assert!(prompt.contains("Your name is Arterm."));
...
#[test]
fn load_agents_md_falls_back_to_claude_md_when_agents_md_is_missing() {
    let project_dir = tempfile::TempDir::new().unwrap();
    std::fs::write(
    ...
    assert!(content.contains("claude briefing for this repo"));
}

#[test]
fn load_agents_md_prefers_agents_md_over_claude_md() {
    let project_dir = tempfile::TempDir::new().unwrap();
    std::fs::write(project_dir.path().join("AGENTS.md"), "arterm briefing").unwrap();
    ...
    assert!(content.contains("arterm briefing"));
...
fn test_session_context_includes_time_timezone_and_system_info() {
    ...
    assert!(context.contains("Arterm version: "));
...
#[test]
fn test_prompt_overlay_files_are_loaded_from_project_and_global_arterm_dirs() {
    let _guard = crate::storage::lock_test_env();
    let prev_home = std::env::var_os("ARTERM_HOME");
    let temp = tempfile::TempDir::new().unwrap();
    crate::env::set_var("ARTERM_HOME", temp.path());
    ...
    let project_dir = tempfile::TempDir::new().unwrap();
    std::fs::create_dir_all(project_dir.path().join(".arterm")).unwrap();
    std::fs::write(
        project_dir.path().join(".arterm/prompt-overlay.md"),
...
#[test]
fn test_preferred_tools_files_are_loaded_from_project_and_global_arterm_dirs() {
    let _guard = crate::storage::lock_test_env();
    let prev_home = std::env::var_os("ARTERM_HOME");
    let temp = tempfile::TempDir::new().unwrap();
    crate::env::set_var("ARTERM_HOME", temp.path());
    ...
    std::fs::create_dir_all(project_dir.path().join(".arterm")).unwrap();
    std::fs::write(
        project_dir.path().join(".arterm/preferred-tools.md"),
    ...
    let direct_content = direct.0.unwrap();
    assert!(
        direct_content.contains("Project Preferred Tools (.arterm/preferred-tools.md)"),
    ...
    assert!(
        direct_content.contains("Global Preferred Tools (~/.arterm/preferred-tools.md)"),
...
fn test_swarm_prompt_prefers_project_then_global_then_default() {
    ...
    std::fs::create_dir_all(project_dir.path().join(".arterm")).unwrap();
    std::fs::write(
        project_dir.path().join(".arterm/swarm-prompt.md"),
    ...
    std::fs::write(project_dir.path().join(".arterm/swarm-prompt.md"), "   \n").unwrap();
    let prompt = load_swarm_prompt(Some(project_dir.path()));
    assert_eq!(prompt, "global swarm routing");
...
fn test_non_selfdev_prompt_leaves_selfdev_guidance_to_the_tool_schema() {
    ...
    assert!(!prompt.contains("You are working on the arterm codebase itself."));
}
...
fn test_selfdev_prompt_uses_full_selfdev_instructions() {
    ...
    assert!(prompt.contains("You are working on the arterm codebase itself."));
    assert!(prompt.contains("launched from the TUI/root arterm context"));
...
fn test_selfdev_prompt_uses_desktop_focus_for_desktop_working_dir() {
    let desktop_dir = std::path::Path::new("/tmp/arterm/crates/arterm-desktop2/src");
    let (prompt, _info) = build_system_prompt_full(None, &[], true, None, Some(desktop_dir));
    assert!(prompt.contains("launched from the arterm-desktop2"));
...
fn test_split_selfdev_prompt_defaults_to_tui_focus_for_repo_root() {
    let repo_dir = std::path::Path::new("/tmp/arterm");
    let (split, _info) = build_system_prompt_split(None, &[], true, None, Some(repo_dir));
    assert!(
        split
            .static_part
            .contains("launched from the TUI/root arterm context")
    );
...
fn test_selfdev_prompt_prefers_publish_flow_for_active_builds() {
    ...
    assert!(prompt.contains("scripts/dev_cargo.sh build --profile selfdev -p arterm --bin arterm"));
...
fn test_selfdev_prompt_uses_desktop2_focus_for_desktop2_working_dir() {
    let desktop2_dir = std::path::Path::new("/tmp/arterm/crates/arterm-desktop2/src");
...
fn project_system_prompt_file_replaces_default_base_prompt() {
    ...
    let dir = std::env::temp_dir().join(format!("arterm-sysprompt-{}", std::process::id()));
    let arterm_dir = dir.join(".arterm");
    std::fs::create_dir_all(&arterm_dir).unwrap();
    std::fs::write(
        arterm_dir.join("system-prompt.md"),
    ...
    assert!(!prompt.contains("Arterm is open source"));

    // Empty override falls back to the built-in default.
    std::fs::write(arterm_dir.join("system-prompt.md"), "   \n").unwrap();
