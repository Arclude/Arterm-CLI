...
const CLIENT_TITLE_PREFIXES: &[&str] = &["arterm:d:", "arterm:c:"];
...
pub async fn run_configured() -> Result<DictationRun> {
    ...
    if command.is_empty() {
        anyhow::bail!(
            "Dictation is not configured. Set `[dictation].command` in `~/.arterm/config.toml`."
        );
    }
...
pub fn remember_last_focused_session(session_id: &str) -> Result<()> {
    ...
    std::fs::write(&path, session_id).context("failed to persist last focused arterm session")?;
...
pub fn last_focused_session() -> Result<Option<String>> {
    ...
    let session_id = match std::fs::read_to_string(path) {
        ...
        Err(err) => return Err(err).context("failed to read last focused arterm session"),
    };
    if session_id.is_empty() {
        return Ok(None);
    }
...
pub fn focused_arterm_session() -> Result<Option<String>> {
...
fn extract_session_short_name_from_window_title(title: &str) -> Option<String> {
    let (_, rest) = title
        .split_once("arterm/")
        .or_else(|| title.split_once("arterm "))?;
...
fn last_focused_session_path() -> Result<std::path::PathBuf> {
    Ok(crate::storage::arterm_dir()?.join("last_focused_client_session"))
}
