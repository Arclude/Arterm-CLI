...
fn resolved_named_profile_suggests_newest_cached_live_release() {
    ...
    let _guard = EnvGuard::save(&["ARTERM_HOME"]);
    let temp = tempfile::tempdir().expect("tempdir");
    crate::env::set_var("ARTERM_HOME", temp.path());
    arterm_provider_openrouter::save_disk_cache_with_source_for_namespace(
        "cerebras",
        &[
            arterm_provider_openrouter::ModelInfo {
...
fn auth_issue_runtime_display_name_tracks_direct_compatible_profiles() {
    ...
    let _guard = EnvGuard::save(&[
        ...
        "ARTERM_OPENROUTER_TRANSPORT_STATE",
        "ARTERM_OPENROUTER_ALLOW_NO_AUTH",
        "ARTERM_RUNTIME_PROVIDER",
        "ARTERM_NAMED_PROVIDER_PROFILE",
        "ARTERM_PROVIDER_PROFILE_ACTIVE",
    ]);

    crate::env::set_var("ARTERM_RUNTIME_PROVIDER", "azure-openai");
    assert_eq!(runtime_provider_display_name("openrouter"), "Azure OpenAI");
    crate::env::remove_var("ARTERM_RUNTIME_PROVIDER");
...
fn auth_profile_env_application_flushes_stale_openrouter_catalog_state() {
    ...
    let _guard = EnvGuard::save(&[
        ...
        "ARTERM_NAMED_PROVIDER_PROFILE",
        "ARTERM_PROVIDER_PROFILE_ACTIVE",
        "ARTERM_PROVIDER_PROFILE_NAME",
    ]);

    crate::env::set_var("ARTERM_OPENROUTER_API_BASE", "https://openrouter.ai/api/v1");
    crate::env::set_var("ARTERM_OPENROUTER_API_KEY_NAME", "OPENROUTER_API_KEY");
    crate::env::set_var("ARTERM_OPENROUTER_ENV_FILE", "openrouter.env");
    crate::env::set_var("ARTERM_OPENROUTER_CACHE_NAMESPACE", "openrouter");
    crate::env::set_var("ARTERM_OPENROUTER_PROVIDER_FEATURES", "1");
    crate::env::set_var("ARTERM_OPENROUTER_TRANSPORT_STATE", "stale");
    crate::env::set_var("ARTERM_OPENROUTER_ALLOW_NO_AUTH", "1");
    crate::env::set_var(
        "ARTERM_OPENROUTER_MODEL_CATALOG",
        "stale-openrouter-catalog.json",
    );
    crate::env::set_var("ARTERM_OPENROUTER_MODEL", "gpt-5.5");
    crate::env::set_var(
        "ARTERM_OPENROUTER_STATIC_MODELS",
        "stale-openrouter-only-model",
    );
    crate::env::set_var("ARTERM_OPENROUTER_AUTH_HEADER", "Bearer stale");
    crate::env::set_var("ARTERM_OPENROUTER_AUTH_HEADER_NAME", "Authorization");
    crate::env::set_var("ARTERM_OPENROUTER_DYNAMIC_BEARER_PROVIDER", "openrouter");
    crate::env::set_var("ARTERM_OPENROUTER_PROVIDER", "openrouter");
    crate::env::set_var("ARTERM_OPENROUTER_NO_FALLBACK", "1");
    crate::env::set_var("ARTERM_NAMED_PROVIDER_PROFILE", "openrouter");
    crate::env::set_var("ARTERM_PROVIDER_PROFILE_ACTIVE", "1");
    crate::env::set_var("ARTERM_PROVIDER_PROFILE_NAME", "openrouter");

    force_apply_openai_compatible_profile_env(Some(CEREBRAS_PROFILE));

    assert_eq!(
        std::env::var("ARTERM_OPENROUTER_API_BASE").as_deref(),
        Ok("https://api.cerebras.ai/v1")
    );
    assert_eq!(
        std::env::var("ARTERM_OPENROUTER_API_KEY_NAME").as_deref(),
        Ok("CEREBRAS_API_KEY")
    );
    assert_eq!(
        std::env::var("ARTERM_OPENROUTER_ENV_FILE").as_deref(),
        Ok("cerebras.env")
    );
    assert_eq!(
    ...
    assert_eq!(
        std::env::var("ARTERM_OPENROUTER_PROVIDER_FEATURES").as_deref(),
        Ok("0")
    );
    assert_eq!(
        std::env::var("ARTERM_OPENROUTER_TRANSPORT_STATE").as_deref(),
        Ok("direct-api-key")
    );
    assert!(std::env::var_os("ARTERM_OPENROUTER_ALLOW_NO_AUTH").is_none());
    assert!(std::env::var_os("ARTERM_OPENROUTER_MODEL_CATALOG").is_none());
    assert!(std::env::var_os("ARTERM_OPENROUTER_MODEL").is_none());
    assert!(std::env::var_os("ARTERM_OPENROUTER_AUTH_HEADER").is_none());
    assert!(std::env::var_os("ARTERM_OPENROUTER_AUTH_HEADER_NAME").is_none());
    assert!(std::env::var_os("ARTERM_OPENROUTER_DYNAMIC_BEARER_PROVIDER").is_none());
    assert!(std::env::var_os("ARTERM_OPENROUTER_PROVIDER").is_none());
    assert!(std::env::var_os("ARTERM_OPENROUTER_NO_FALLBACK").is_none());
    assert!(std::env::var_os("ARTERM_NAMED_PROVIDER_PROFILE").is_none());
    assert!(std::env::var_os("ARTERM_PROVIDER_PROFILE_ACTIVE").is_none());
    assert!(std::env::var_os("ARTERM_PROVIDER_PROFILE_NAME").is_none());
    assert_ne!(
        std::env::var("ARTERM_OPENROUTER_STATIC_MODELS")
            .ok()
            .as_deref(),
...
fn matrix_openrouter_like_sources_include_all_static_profiles() {
    ...
    let guard = EnvGuard::save(&[
        "ARTERM_OPENROUTER_API_KEY_NAME",
        "ARTERM_OPENROUTER_ENV_FILE",
        "ARTERM_OPENAI_COMPAT_API_KEY_NAME",
        "ARTERM_OPENAI_COMPAT_ENV_FILE",
    ]);
    crate::env::remove_var("ARTERM_OPENROUTER_API_KEY_NAME");
    crate::env::remove_var("ARTERM_OPENROUTER_ENV_FILE");
    crate::env::remove_var("ARTERM_OPENAI_COMPAT_API_KEY_NAME");
    crate::env::remove_var("ARTERM_OPENAI_COMPAT_ENV_FILE");
...
fn matrix_openrouter_like_sources_accept_valid_overrides() {
    ...
    crate::env::set_var("ARTERM_OPENROUTER_API_KEY_NAME", "ALT_OPENROUTER_KEY");
    crate::env::set_var("ARTERM_OPENROUTER_ENV_FILE", "alt-openrouter.env");
    crate::env::set_var("ARTERM_OPENAI_COMPAT_API_KEY_NAME", "ALT_COMPAT_KEY");
    crate::env::set_var("ARTERM_OPENAI_COMPAT_ENV_FILE", "alt-compat.env");
...
fn named_provider_profile_reports_malformed_config_instead_of_unknown_profile() {
    let _lock = crate::storage::lock_test_env();
    let previous_home = std::env::var_os("ARTERM_HOME");
    ...
    if let Some(previous_home) = previous_home {
        crate::env::set_var("ARTERM_HOME", previous_home);
    } else {
...
fn named_provider_profile_maps_to_openai_compatible_runtime_env() {
    ...
    assert_eq!(
        std::env::var("ARTERM_OPENROUTER_API_BASE").ok().as_deref(),
        Some("https://llm.example.com/v1")
    );
    assert_eq!(
        std::env::var("ARTERM_OPENROUTER_API_KEY_NAME")
            .ok()
            .as_deref(),
        Some("MY_GATEWAY_API_KEY")
    );
    assert_eq!(
        std::env::var("ARTERM_OPENROUTER_PROVIDER_FEATURES")
            .ok()
            .as_deref(),
        Some("0")
    );
    assert_eq!(
        std::env::var("ARTERM_OPENROUTER_TRANSPORT_STATE")
            .ok()
            .as_deref(),
        Some("direct-api-key")
    );
    assert_eq!(
        std::env::var("ARTERM_OPENROUTER_MODEL_CATALOG")
            .ok()
            .as_deref(),
        Some("0")
    );
    ...
    assert_eq!(
        std::env::var("ARTERM_OPENROUTER_AUTH_HEADER")
            .ok()
            .as_deref(),
        Some("api-key")
    );
    assert_eq!(
        std::env::var("ARTERM_OPENROUTER_AUTH_HEADER_NAME")
            .ok()
            .as_deref(),
        Some("x-api-key")
    );
    assert_eq!(
        std::env::var("ARTERM_NAMED_PROVIDER_PROFILE")
            .ok()
            .as_deref(),
        Some("my-gateway")
    );
...
fn named_provider_inline_api_key_is_private_runtime_fallback() {
    ...
    let _guard = EnvGuard::save(&[
        ...
        "ARTERM_OPENROUTER_MODEL_CATALOG",
        "ARTERM_NAMED_PROVIDER_PROFILE",
        "ARTERM_PROVIDER_MY_GATEWAY_API_KEY",
    ]);
    ...
    assert_eq!(
        std::env::var("ARTERM_OPENROUTER_API_KEY_NAME")
            .ok()
            .as_deref(),
        Some("ARTERM_PROVIDER_MY_GATEWAY_API_KEY")
    );
    assert_eq!(
        std::env::var("ARTERM_PROVIDER_MY_GATEWAY_API_KEY")
            .ok()
            .as_deref(),
        Some("inline-secret")
    );
...
fn matrix_openrouter_like_sources_reject_invalid_overrides() {
    ...
    crate::env::set_var("ARTERM_OPENROUTER_API_KEY_NAME", "bad-key-name");
    crate::env::set_var("ARTERM_OPENROUTER_ENV_FILE", "../bad.env");
    crate::env::set_var("ARTERM_OPENAI_COMPAT_API_KEY_NAME", "bad key");
    crate::env::set_var("ARTERM_OPENAI_COMPAT_ENV_FILE", "../bad-compat.env");
...
fn matrix_openai_compatible_profile_overrides_apply_when_valid() {
    ...
    let _guard = EnvGuard::save(&[
        ...
        "ARTERM_OPENAI_COMPAT_API_KEY_NAME",
        "ARTERM_OPENAI_COMPAT_ENV_FILE",
        "ARTERM_OPENAI_COMPAT_DEFAULT_MODEL",
    ]);
    ...
    crate::env::set_var("ARTERM_OPENAI_COMPAT_API_KEY_NAME", "GROQ_API_KEY");
    crate::env::set_var("ARTERM_OPENAI_COMPAT_ENV_FILE", "groq.env");
    crate::env::set_var("ARTERM_OPENAI_COMPAT_DEFAULT_MODEL", "openai/gpt-oss-120b");
...
fn matrix_openai_compatible_profile_overrides_reject_invalid_values() {
    ...
    crate::env::set_var("ARTERM_OPENAI_COMPAT_API_BASE", "http://example.com/v1");
    crate::env::set_var("ARTERM_OPENAI_COMPAT_API_KEY_NAME", "bad-key-name");
    crate::env::set_var("ARTERM_OPENAI_COMPAT_ENV_FILE", "../bad.env");
...
fn matrix_openai_compatible_profile_overrides_read_from_env_file() {
    let _lock = crate::storage::lock_test_env();
    let temp = tempfile::tempdir().expect("tempdir");
    let config_root = temp.path().join("config").join("arterm");
    ...
    crate::env::set_var("ARTERM_HOME", temp.path());
    crate::env::remove_var("ARTERM_OPENAI_COMPAT_API_BASE");
    crate::env::remove_var("ARTERM_OPENAI_COMPAT_API_KEY_NAME");
    crate::env::remove_var("ARTERM_OPENAI_COMPAT_ENV_FILE");
    crate::env::remove_var("ARTERM_OPENAI_COMPAT_DEFAULT_MODEL");
    std::fs::write(
        config_root.join(OPENAI_COMPAT_PROFILE.env_file),
        concat!(
            "ARTERM_OPENAI_COMPAT_API_BASE=https://api.example.com/v1\n",
            "ARTERM_OPENAI_COMPAT_API_KEY_NAME=EXAMPLE_API_KEY\n",
            "ARTERM_OPENAI_COMPAT_ENV_FILE=example.env\n",
            "ARTERM_OPENAI_COMPAT_DEFAULT_MODEL=example/model\n",
        ),
    )
    .expect("env file");
...
fn matrix_openai_compatible_localhost_override_allows_no_auth() {
    ...
    let _guard = EnvGuard::save(&[
        ...
        "ARTERM_OPENAI_COMPAT_API_KEY_NAME",
        "ARTERM_OPENAI_COMPAT_ENV_FILE",
        "ARTERM_OPENAI_COMPAT_LOCAL_ENABLED",
    ]);

    crate::env::set_var("ARTERM_OPENAI_COMPAT_API_BASE", "http://localhost:11434/v1");
    crate::env::remove_var("ARTERM_OPENAI_COMPAT_API_KEY_NAME");
    crate::env::remove_var("ARTERM_OPENAI_COMPAT_ENV_FILE");
    crate::env::remove_var("ARTERM_OPENAI_COMPAT_LOCAL_ENABLED");
...
fn matrix_load_api_key_from_env_or_config_prefers_env() {
    ...
    let _guard = EnvGuard::save(&["ARTERM_HOME", "OPENCODE_API_KEY"]);
    crate::env::set_var("ARTERM_HOME", temp.path());
...
fn load_api_key_accepts_legacy_zai_key_name() {
    ...
    let _guard = EnvGuard::save(&["ARTERM_HOME", "ZHIPU_API_KEY", "ZAI_API_KEY"]);
    crate::env::set_var("ARTERM_HOME", temp.path());
...
fn newest_release_picker_prefers_strongest_tier_over_newest_cheap() {
    use arterm_provider_openrouter::ModelInfo;
    let _lock = crate::storage::lock_test_env();
    let _env = EnvGuard::save(&["ARTERM_HOME"]);
    let temp = tempfile::tempdir().expect("tempdir");
    crate::env::set_var("ARTERM_HOME", temp.path());
...
fn every_static_profile_model_has_a_known_context_limit() {
    use arterm_provider_core::models::context_limit_for_model_with_provider;
    ...
    for profile in arterm_provider_metadata::openai_compatible_profiles()
        .iter()
        .copied()
    {
...
fn open_weight_family_context_limits_match_published_windows() {
    use arterm_provider_core::models::open_weight_family_context_limit as f;
...
fn minimax_default_provider_applies_openai_api_key_env_not_openrouter() {
    ...
    assert_eq!(
        std::env::var("ARTERM_OPENROUTER_ENV_FILE").ok().as_deref(),
        Some("minimax.env"),
