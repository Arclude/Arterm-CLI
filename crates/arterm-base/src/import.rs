//! Import Claude Code sessions into arterm
//!
//! This module handles discovering, parsing, and converting Claude Code sessions
//! so they can be resumed within arterm.
...
/// Convert Claude Code content blocks to arterm ContentBlocks
...
fn truncate_import_text(text: String, max_bytes: usize) -> String {
    ...
    format!(
        "{}{}",
        arterm_core::util::truncate_str(&text, prefix_budget),
        suffix
    )
}
...
fn reuse_existing_imported_session(session_id: &str) -> bool {
    // An imported snapshot becomes a normal arterm continuation as soon as the
    ...
    // doing so can discard arterm-only turns and journal state. New imports are
...
pub fn imported_session_id_for_target(
    ...
    match target {
        ...
        arterm_session_types::ResumeTarget::CodexSession { session_id, .. } => {
            Some(imported_codex_session_id(session_id))
        }
        arterm_session_types::ResumeTarget::PiSession { session_path } => {
            Some(imported_pi_session_id(session_path))
        }
        arterm_session_types::ResumeTarget::OpenCodeSession { session_id, .. } => {
            Some(imported_opencode_session_id(session_id))
        }
        arterm_session_types::ResumeTarget::CursorSession { session_id, .. } => {
            Some(imported_cursor_session_id(session_id))
        }
    }
}

pub fn resolve_resume_target_to_arterm(
    target: &arterm_session_types::ResumeTarget,
) -> Result<arterm_session_types::ResumeTarget> {
    ...
    let session_id = match target {
        ResumeTarget::ArtermSession { session_id } => {
            return Ok(ResumeTarget::ArtermSession {
                session_id: session_id.clone(),
            });
        }
    ...
    Ok(ResumeTarget::ArtermSession { session_id })
}
...
fn import_session_from_file_with_target(
    ...
    let mut session = Session::create_with_id(arterm_session_id, None, title);
...
/// Explicitly hand a currently-running Claude Code session over to Arterm.
///
/// This is deliberately separate from normal resume. It first imports the
/// current transcript into a fresh durable Arterm session, then gracefully stops
...
/// A stop failure rolls back the staged Arterm session and leaves ordinary
...
fn take_over_live_claude_session_with_timeout(
    ...
    // Use a normal memorable Arterm session ID so the handed-off conversation
    ...
    if stop_outcome == crate::claude_live::StopLiveClaudeOutcome::ExitUnconfirmed {
        crate::session_list_cache::invalidate();
        anyhow::bail!(
            "Claude Code process {} was asked to exit, but its exit was not confirmed; prepared Arterm session {} was preserved and can be resumed",
            live.pid,
            takeover_id
        );
    }
    ...
    if let Some(err) = final_refresh_error {
        ...
        return Err(err).with_context(|| {
            format!(
                "Claude Code exited, but its final transcript could not be refreshed; prepared Arterm session {takeover_id} was preserved"
            )
        });
    }

    crate::logging::info(&format!(
        "Claude takeover complete: source_session={session_id} source_pid={} arterm_session={takeover_id}",
        live.pid
    ));
    crate::session_list_cache::invalidate();
    Ok(ResumeTarget::ArtermSession {
...
fn finalize_imported_session(
    ...
    // Never overwrite a arterm-side continuation with a shorter external
...
pub fn import_cursor_session_from_path(
    ...
    let created_at =
        arterm_import_core::file_modified_datetime(session_path).unwrap_or_else(Utc::now);
