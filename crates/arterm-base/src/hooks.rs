...
//!
//! Hooks are external commands that arterm runs at well-defined lifecycle
//! points so other programs can observe or gate agent behavior without
//! forking arterm. They are configured in `[hooks]` in config.toml (or
//! `ARTERM_HOOK_*` env vars) and follow the same command-line conventions as
//! `[terminal] spawn_hook`: the command is parsed shell-style but executed
//! directly (no shell), with `ARTERM_HOOK_*` metadata env vars describing the
//! event.
//!
//! Two dispatch styles:
//!
...
/// Maximum bytes of JSON payload exported via `ARTERM_HOOK_PAYLOAD`.
...
pub struct HookEvent {
    ...
    /// `ARTERM_HOOK_STATUS=ok` and `"status": "ok"` in the JSON payload.
...
fn hooks_suppressed() -> bool {
    std::env::var_os("ARTERM_HOOKS_DISABLED").is_some()
}
...
/// JSON payload mirroring the env fields, exported as `ARTERM_HOOK_PAYLOAD`.
...
fn apply_event_env(cmd: &mut std::process::Command, event: &HookEvent) {
    cmd.env("ARTERM_HOOKS_DISABLED", "1");
    cmd.env("ARTERM_HOOK_EVENT", event.event);
    if let Some(session_id) = &event.session_id {
        cmd.env("ARTERM_HOOK_SESSION_ID", session_id);
    }
    if let Some(cwd) = &event.cwd {
        cmd.env("ARTERM_HOOK_CWD", cwd);
    }
    for (key, value) in &event.fields {
        cmd.env(format!("ARTERM_HOOK_{key}"), value);
    }
    cmd.env("ARTERM_HOOK_PAYLOAD", payload_json(event));
}
...
mod tests {
    ...
    fn gate_test_config(hook: &str, timeout_ms: u64) -> impl Drop + use<> {
        ...
        let reset = EnvReset(vec![
            (
                ...
                std::env::var_os("ARTERM_HOOK_PRE_TOOL"),
            ),
            (
                "ARTERM_HOOK_PRE_TOOL_TIMEOUT_MS",
                std::env::var_os("ARTERM_HOOK_PRE_TOOL_TIMEOUT_MS"),
            ),
        ]);
        crate::env::set_var("ARTERM_HOOK_PRE_TOOL", hook);
        crate::env::set_var("ARTERM_HOOK_PRE_TOOL_TIMEOUT_MS", timeout_ms.to_string());
        reset
    }
    ...
    async fn pre_tool_gate_allows_on_exit_zero_and_blocks_on_exit_two() {
        ...
        let block = write_executable_script(
            temp.path(),
            "block.sh",
            "#!/bin/sh\ncat > /dev/null\necho \"dangerous tool: $ARTERM_HOOK_TOOL_NAME\" >&2\nexit 2\n",
        );
        {
    ...
    fn observer_dispatch_runs_hook_with_event_env() {
        ...
        let script = write_executable_script(
            ...
            "observe.sh",
            &format!(
                "#!/bin/sh\nprintf '%s|%s|%s|%s' \"$ARTERM_HOOK_EVENT\" \"$ARTERM_HOOK_SESSION_ID\" \"$ARTERM_HOOK_STATUS\" \"$ARTERM_HOOKS_DISABLED\" > {}\n",
                crate::terminal_launch::sh_escape(&record.to_string_lossy())
            ),
        );

        let prev = std::env::var_os("ARTERM_HOOK_TURN_END");
        crate::env::set_var("ARTERM_HOOK_TURN_END", script.to_string_lossy().to_string());
        ...
        match prev {
            Some(value) => crate::env::set_var("ARTERM_HOOK_TURN_END", value),
            None => crate::env::remove_var("ARTERM_HOOK_TURN_END"),
        }
    ...
    fn observer_dispatch_runs_each_configured_command() {
        ...
        let previous = std::env::var_os("ARTERM_HOOK_SESSION_START");
        crate::env::set_var(
        ...
        match previous {
            Some(value) => crate::env::set_var("ARTERM_HOOK_SESSION_START", value),
            None => crate::env::remove_var("ARTERM_HOOK_SESSION_START"),
        }
    ...
    async fn hook_process_replaces_daemon_terminal_env_with_client_snapshot() {
        ...
        let script = write_executable_script(
            temp.path(),
            "env.sh",
            "#!/bin/sh\nprintf '%s|%s|%s|%s' \"$TMUX_PANE\" \"$HERDR_PANE_ID\" \"$ARTERM_CLIENT_TMUX_PANE\" \"$ARTERM_CLIENT_HERDR_PANE_ID\"\n",
        );
