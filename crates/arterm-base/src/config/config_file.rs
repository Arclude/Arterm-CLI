...
impl Config {
    ...
    pub fn path() -> Option<PathBuf> {
        arterm_dir().ok().map(|d| d.join("config.toml"))
    }
    ...
    pub fn set_launch_hotkeys(
        entries: Vec<arterm_config_types::LaunchHotkeyEntry>,
        enabled: bool,
    ) -> anyhow::Result<()> {
        let mut cfg = Self::load();
    ...
    ///
    /// Scans `~/.arterm/sessions` for the directories the user works in most,
    ...
    pub fn bake_launch_hotkeys_once() -> bool {
        use arterm_import_core::repo_ranking;

        let cfg = Self::load();
        if cfg.launch_hotkeys.imported {
            return false;
        }
        let Ok(arterm_dir) = arterm_dir() else {
            return false;
        };
        let sessions_dir = arterm_dir.join("sessions");
        ...
        let entries: Vec<arterm_config_types::LaunchHotkeyEntry> = plan
            .into_iter()
            .map(|p| arterm_config_types::LaunchHotkeyEntry {
                chord: p.chord,
    ...
    pub fn external_auth_source_allowed_for_path_cached(
        ...
        // a new path-bound trust decision, or before tests switch ARTERM_HOME. Fall
...
#[cfg(test)]
mod sandbox_default_tests {
    use crate::config::{Config, default_sandbox_mode};

    /// A config file written before the default changed says `sandbox_mode = ""`.
    /// Empty is "unset", so it must resolve to the sandbox, not to no sandbox --
    /// otherwise turning the sandbox on by default silently skips every existing
    /// installation, which is the population that matters.
    #[test]
    fn an_empty_mode_resolves_to_the_default() {
        let mut config = Config::default();
        config.sandbox_mode = String::new();
        config.apply_defaults();
        assert_eq!(config.sandbox_mode, default_sandbox_mode());
    }

    #[test]
    fn whitespace_is_still_unset() {
        let mut config = Config::default();
    ...
    /// The escape hatch has to survive, or the setting is not a setting.
    #[test]
    fn an_explicit_full_access_is_left_alone() {
        let mut config = Config::default();
        config.sandbox_mode = "full-access".to_string();
        config.apply_defaults();
        assert_eq!(config.sandbox_mode, "full-access");
    }

    #[test]
    fn an_explicit_read_only_is_left_alone() {
        let mut config = Config::default();
        config.sandbox_mode = "read-only".to_string();
        config.apply_defaults();
        assert_eq!(config.sandbox_mode, "read-only");
    }

    /// The load path is what actually has to resolve this. `apply_defaults`
    /// being right is not the same as it being called: this reads a real
    /// `config.toml` off disk, shaped like the ones already installed.
    #[test]
    fn a_config_file_on_disk_with_an_empty_mode_loads_sandboxed() {
        let _guard = crate::storage::lock_test_env();
        let temp = tempfile::tempdir().expect("tempdir");
        std::fs::write(temp.path().join("config.toml"), "sandbox_mode = \"\"\n")
            .expect("write config");

        let prev = std::env::var_os("ARTERM_HOME");
        arterm_core::env::set_var("ARTERM_HOME", temp.path());
        let loaded = Config::load().sandbox_mode.clone();
        match prev {
            Some(prev) => arterm_core::env::set_var("ARTERM_HOME", prev),
            None => arterm_core::env::remove_var("ARTERM_HOME"),
        }

        assert_eq!(loaded, default_sandbox_mode());
    }

    /// A missing key deserializes through serde's default, not through
    /// `apply_defaults`, so it needs its own case.
    #[test]
    fn a_config_that_never_mentions_the_key_is_sandboxed() {
