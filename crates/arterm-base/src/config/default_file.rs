...
impl Config {
    ...
    pub fn default_config_file_contents() -> String {
        let default_content = r##"# arterm configuration file
# Location: ~/.arterm/config.toml
#
# Environment variables override these settings.
# Run `/config` in arterm to see current settings.
...
# Spawn a fresh arterm session in a new terminal window, reusing the current
# session's working directory. Companion to the system-wide launch hotkeys.
# `arterm setup-hotkey` installs these three global launch hotkeys on macOS,
...
#   Cmd+;        new arterm in your home directory
#   Cmd+'        new arterm in your last project directory
#   Cmd+Shift+'  new arterm self-dev session (last arterm repo)
...
# How to apply the transcript inside arterm: insert|append|replace|send
mode = "send"
...
# Set false here or set ARTERM_NO_EMOJI=1 for ASCII fallbacks.
emoji = true
...
# (default: true). Set false to list only arterm's own sessions.
...
# bing_api_key_env = "ARTERM_BING_API_KEY"
# bing_api_key = ""
...
# this. Configure here or via the ARTERM_SEARXNG_URL environment variable, then
...
# extended/full additionally emit ignorable _arterm/* extension notifications.
profile = "standard"
# Tool profile requested when `arterm acp` starts the daemon itself.
...
#   claude        = Claude via OAuth/subscription (token in ~/.arterm/auth.json)
#   anthropic-api = Claude via direct Anthropic API key (ANTHROPIC_API_KEY env
#                   or ~/.config/arterm/anthropic.env). API-key mode does NOT fall
...
# to match this value. Also overridable per-launch via ARTERM_STREAM_IDLE_TIMEOUT_SECS.
...
# Env override: ARTERM_SWARM_MODEL
# swarm_model = "inherit"
#
...
# Env override: ARTERM_SWARM_SPAWN_MODE
...
# Env override: ARTERM_SWARM_MAX_CONCURRENT_AGENTS
...
# Env override: ARTERM_SWARM_STRIP_LAYOUT
...
# Env override: ARTERM_MEMORY_MODEL
...
# dormant instead of degrading to the no-LLM path. Env: ARTERM_MEMORY_SIDECAR_ENABLED
...
# Env override: ARTERM_MEMORY_EMBEDDING_BACKEND
...
# Set ARTERM_TERMINAL to force a supported terminal emulator instead.
...
#
# When set, arterm runs `<spawn_hook> <arterm-binary> <args...>` instead of
# opening a terminal emulator itself. The hook receives ARTERM_SPAWN_* env vars
# describing the spawn so multiplexers/wrappers can decide where it appears:
#   ARTERM_SPAWN_KIND        - "swarm-agent", "resume", "selfdev", "restart", ...
#   ARTERM_SPAWN_SESSION_ID  - session the window will run
#   ARTERM_SPAWN_TITLE       - suggested window/tab title
#   ARTERM_SPAWN_CWD         - session working directory (also the hook's cwd)
#   ARTERM_SPAWN_PROGRAM     - arterm binary path
#   ARTERM_SPAWN_COMMAND     - full shell-escaped command line
#   ARTERM_SPAWN_SWARM_ID / ARTERM_SPAWN_COORDINATOR_SESSION_ID (swarm spawns)
# If the hook fails to start, arterm falls back to built-in terminal detection.
# Env override: ARTERM_SPAWN_HOOK (set empty to disable a config hook).
#
# Examples:
#   spawn_hook = "tmux new-window"                # tmux window per agent
#   spawn_hook = "kitty @ launch --type=tab --"   # kitty tab per agent
#   spawn_hook = "~/bin/arterm-spawn-router"       # custom placement script
# spawn_hook = ""
#
# External command used to focus/raise an existing session window, replacing
# the built-in wmctrl/xdotool title search. Receives ARTERM_FOCUS_SESSION_ID
# and ARTERM_FOCUS_TITLE env vars. Pair with spawn_hook so the program that
# placed the window also brings it to the front.
# Env override: ARTERM_FOCUS_HOOK (set empty to disable a config hook).
#
# Example:
#   focus_hook = "~/bin/arterm-focus-router"
# focus_hook = ""
#
# macOS only: terminal that the Cmd+; launch hotkey and in-app session spawns
# open arterm into. One of: ghostty, iterm2, wezterm, warp, alacritty, vscode,
# terminal (Apple Terminal). Preferred over the legacy
# ~/.arterm/preferred_terminal.json file. After changing this, re-run
# `arterm setup-hotkey` so the generated launcher script (Cmd+;) picks it up.
# preferred = "ghostty"
...
[hooks]
# Lifecycle hooks: external commands arterm runs at well-defined points so other
# programs can observe or gate agent behavior. Commands are parsed shell-style
# (quotes work) but executed directly, with ARTERM_HOOK_* env vars describing
# the event:
#   ARTERM_HOOK_EVENT       - "turn_start", "turn_end", "session_start",
#                            "session_end", "pre_tool", "post_tool"
#   ARTERM_HOOK_SESSION_ID  - the session the event belongs to
#   ARTERM_HOOK_CWD         - session working directory (also the hook's cwd)
#   ARTERM_HOOK_PAYLOAD     - JSON mirror of all fields
...
# think/stream window before any tool call. Extra fields: ARTERM_HOOK_MODEL,
# ARTERM_HOOK_SOURCE ("chat"/"resume"/"ambient").
# turn_start = "~/bin/arterm-turn-start"
#
# Runs when an agent turn completes. Extra fields: ARTERM_HOOK_STATUS
# ("ok"/"error"), ARTERM_HOOK_DURATION_MS, ARTERM_HOOK_MODEL,
# ARTERM_HOOK_LAST_ASSISTANT_TEXT (first 4000 chars), ARTERM_HOOK_ERROR.
# turn_end = "~/bin/arterm-turn-notify"
#
# Runs when a session becomes active. Extra: ARTERM_HOOK_SOURCE
...
#
# Runs after each tool call. Extra fields: ARTERM_HOOK_TOOL_NAME,
# ARTERM_HOOK_STATUS, ARTERM_HOOK_DURATION_MS, ARTERM_HOOK_OUTPUT_BYTES,
# ARTERM_HOOK_ERROR.
# post_tool = ""
...
[power]
# Prevent automatic system sleep while any arterm session is actively working.
...
# ntfy_topic = "arterm-ambient-your-secret-topic"
...
# email_from = "arterm@example.com"
# email_smtp_host = "smtp.gmail.com"
# email_smtp_port = 587
# Password via env: ARTERM_SMTP_PASSWORD (preferred) or config below
# email_password = ""
...
# telegram_bot_token = ""  # From @BotFather (prefer ARTERM_TELEGRAM_BOT_TOKEN env var)
...
# ARTERM_JADE_RELAY_USER_ID, ARTERM_JADE_RELAY_SESSION_ID.
...
# developer tools from Arterm's hosted integration directory. These providers
...
# endpoint = "https://api.arterm.dev/v1/discovery"
	"##;
        ...
        let p = arterm_config_types::KeybindingPlatform::current();
        let effort_increase =
            arterm_config_types::default_binding("effort_increase", p).unwrap_or("alt+right");
        let effort_decrease =
            arterm_config_types::default_binding("effort_decrease", p).unwrap_or("alt+left");
        default_content
...
mod tests {
    ...
    /// a config file that arterm itself cannot read. Parse it here.
    #[test]
    ...
    fn documented_color_example_is_valid_when_uncommented() {
        ...
        for (role, value) in &parsed.display.colors {
            assert!(
                arterm_config_types::DisplayConfig::default()
                    .colors
                    .is_empty(),
