//! Shared control logic behind the `/remote` command and `arterm pair`.
//!
...
pub enum RemoteCommand {
    /// Activate or manage the subscription-backed Arterm Cloud host.
    Cloud,
        ...
        if self.enabled && self.host_unknown {
            out.push_str(
                "\nCould not determine a reachable address for this machine. \
                 Set `ARTERM_GATEWAY_HOST` to a hostname or IP the other machine can reach.\n",
            );
        }
...
mod tests {
    use super::*;
    use arterm_gateway_types::PairedDevice;
    ...
    /// These tests mutate the process-global `ARTERM_HOME`, so they must
    ...
    /// `ARTERM_HOME` out from under a provider test mid-assertion, failing it
