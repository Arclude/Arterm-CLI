...
/// Default system prompt for arterm (embedded at compile time)
...
/// [`DEFAULT_SYSTEM_PROMPT`]. Precedence: project `./.arterm/system-prompt.md`,
/// then global `~/.arterm/system-prompt.md`, then the built-in default.
///
/// This is a *replacement* hook. To merely add guidance on top of the default,
/// use `.arterm/prompt-overlay.md` instead.
pub fn load_base_system_prompt(working_dir: Option<&Path>) -> String {
    let project_dir = working_dir.unwrap_or(Path::new("."));
    let candidates = [
        Some(project_dir.join(".arterm").join("system-prompt.md")),
...
/// creating `~/.arterm/swarm-prompt.md` (global) or `./.arterm/swarm-prompt.md`
...
/// project `./.arterm/swarm-prompt.md`, then global `~/.arterm/swarm-prompt.md`,
/// then the built-in [`DEFAULT_SWARM_PROMPT`].
pub fn load_swarm_prompt(working_dir: Option<&Path>) -> String {
    let project_dir = working_dir.unwrap_or(Path::new("."));
    let candidates = [
        Some(project_dir.join(".arterm").join("swarm-prompt.md")),
        crate::storage::arterm_dir()
            .ok()
...
/// `mission` module in the upper `arterm-app-core` layer; the asset lives here
...
pub fn build_system_prompt_full_with_capabilities(
    ...
    // Add optional prompt overlays from ~/.arterm/ and ./.arterm/
    ...
    // Add optional preferred-tool guidance from ~/.arterm/ and ./.arterm/
...
impl SelfDevProductContext {
    fn from_working_dir(working_dir: Option<&Path>) -> Self {
        ...
        if path.contains("/crates/arterm-desktop") || path.ends_with("crates/arterm-desktop2") {
            Self::Desktop2
        } else {
            Self::Tui
        }
...
/// Load optional prompt overlay markdown from ~/.arterm/ and ./.arterm/
fn load_prompt_overlay_files_from_dir(working_dir: Option<&Path>) -> (Option<String>, usize) {
    ...
    let project_dir = working_dir.unwrap_or(Path::new("."));
    if let Some((content, size)) = load_file(
        &project_dir.join(".arterm").join("prompt-overlay.md"),
        "Project Prompt Overlay (.arterm/prompt-overlay.md)",
    ) {
        total_chars += size;
        contents.push(content);
    }
...
/// Load optional preferred-tool guidance from ~/.arterm/ and ./.arterm/
fn load_preferred_tools_files_from_dir(working_dir: Option<&Path>) -> (Option<String>, usize) {
    ...
    let project_dir = working_dir.unwrap_or(Path::new("."));
    if let Some((content, size)) = load_file(
        &project_dir.join(".arterm").join("preferred-tools.md"),
        "Project Preferred Tools (.arterm/preferred-tools.md)",
    ) {
        total_chars += size;
        contents.push(content);
    }
    ...
    if let Ok(global_preferred_tools) =
        crate::storage::arterm_dir().map(|dir| dir.join("preferred-tools.md"))
        && let Some((content, size)) = load_file(
            &global_preferred_tools,
            "Global Preferred Tools (~/.arterm/preferred-tools.md)",
        )
    {
        total_chars += size;
        contents.push(content);
