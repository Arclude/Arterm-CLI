...
#[cfg(target_os = "linux")]
const KILLALL_PROCESS_NAME: &str = "arterm";
...
/// identifies the session/connection, so do not repeat `arterm` or the memorable
...
pub fn set_server_title(server_name: &str) {
    set_title(compact_process_title("arterm:s:", Some(server_name)));
}
...
pub fn set_client_display_title(session_name: &str, is_selfdev: bool) {
    let prefix = if is_selfdev { "arterm:d:" } else { "arterm:c:" };
...
pub fn set_client_remote_display_title(server_name: &str, session_name: &str, is_selfdev: bool) {
    if server_name.is_empty() || server_name.eq_ignore_ascii_case("arterm") {
...
mod tests {
    ...
    fn terminal_window_title_omits_product_and_animal_names() {
        ...
        assert_eq!(
            terminal_window_title("🐙", None, Some("arterm Octopus"), false),
            "🐙 arterm Octopus"
        );
        assert_eq!(
            terminal_window_title(
