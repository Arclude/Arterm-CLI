...
impl Meta {
    ...
    pub fn alert(&self) -> Option<String> {
        if self.account.is_none() {
            return Some("not signed in: run `arterm` and use /login".into());
        }
...
/// written by the installer, so comparing it against `~/.arterm/builds` is
...
/// Pure so the display rules are tested without touching `~/.arterm/auth.json`.
...
fn detect_update_state() -> UpdateState {
    ...
    let builds = home.join(".arterm/builds");
    ...
    let channels: Vec<Option<SystemTime>> = ["current", "stable"]
        .iter()
        .map(|channel| mtime(&builds.join(channel).join("arterm-desktop2")))
        .collect();
    update_state(running, &channels)
}

/// Read the active account out of `~/.arterm/auth.json`. Only the account
/// identity is read; tokens are never touched.
fn detect_account() -> Option<String> {
    let path = home()?.join(".arterm/auth.json");
...
mod tests {
    ...
    /// payload in `~/.arterm/builds/current`, so every CLI install left the
    /// desktop permanently claiming "update ready: restart to apply".
    #[test]
    fn a_dev_build_outside_the_install_tree_is_not_managed() {
        let builds = Path::new("/home/u/.arterm/builds");
        assert!(!is_managed_install(
            Path::new("/home/u/arterm/target/selfdev/arterm-desktop2"),
            builds
        ));
        assert!(is_managed_install(
            Path::new("/home/u/.arterm/builds/versions/abc123/arterm-desktop2"),
            builds
        ));
    }
