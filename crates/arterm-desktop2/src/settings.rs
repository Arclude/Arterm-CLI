...
pub enum Row {
    ...
    Motion,
    CopyOnSelect,
    /// Not a setting of its own: opens `~/.arterm/config.toml`, where the rest
    /// of arterm's configuration lives. The panel is for the handful of choices
...
pub struct Settings {
    ...
    /// settings file and `ARTERM_DESKTOP2_COPY_ON_SELECT` rather than spending
...
impl Settings {
    ...
    /// `ARTERM_DESKTOP2_THEME` and friends sees the panel agree with the window
    ...
    pub fn from_env() -> Self {
        Self {
            ...
            copy_on_select: std::env::var("ARTERM_DESKTOP2_COPY_ON_SELECT")
...
mod tests {
    ...
    #[test]
    fn the_saved_path_lives_under_the_arterm_directory() {
        if let Some(path) = Settings::path() {
            assert!(path.to_string_lossy().contains("/.arterm/"));
        }
    }
