...
//! explaining a lost connection) lives in `arterm-sdk`, so this file is about
...
pub use arterm_sdk::{SocketState, Stage, describe_disconnect, explain};
...
pub(crate) fn default_working_dir() -> Option<String> {
    if let Some(raw) = std::env::var_os("ARTERM_DESKTOP2_WORKING_DIR") {
...
fn run(
    ...
    arterm_sdk::ensure_runtime(&LaunchOptions::default(), &|status| {
        ui.send(HarnessUpdate::Status(status.to_string()))
    })?;

    set_stage(Stage::Connecting);
    let client = ArtermClient::connect(ConnectOptions {
        client_name: concat!("arterm-desktop2/", env!("CARGO_PKG_VERSION")).to_string(),
                ...
                let outcome = match command {
                    ...
                    Command::Peek(target) => {
                        ...
                        std::thread::spawn(move || {
                            ...
                            let preview = ArtermClient::connect(ConnectOptions {
                                client_name: concat!(
                                    "arterm-desktop2-preview/",
    ...
    std::thread::spawn({
        ...
        move || {
            let Ok(client) = ArtermClient::connect(ConnectOptions {
                client_name: concat!("arterm-desktop2-sessions/", env!("CARGO_PKG_VERSION"))
                    .to_string(),
                ensure_runtime: false,
                ..Default::default()
            }) else {
...
fn to_entry(session: arterm_sdk::SessionInfo) -> crate::strip::Panel {
    crate::strip::Panel {
        session_id: session.session_id,
        title: session.title,
        working_dir: session.working_dir,
...
fn to_transcript(messages: Vec<arterm_sdk::HistoryMessage>) -> crate::transcript::Transcript {
