// arterm-desktop2: greenfield desktop app.
//
...
// connection (via arterm-harness-api-bridge) with a minimal chat loop.
...
struct App {
    ...
    /// Live per-frame timing, off unless `ARTERM_DESKTOP2_FRAME_METER` is set.
...
/// Escape hatch: `ARTERM_DESKTOP2_DONUT=0` turns the animation off for users who
/// do not want motion, and for benchmarking the rest of the frame.
pub(crate) fn donut_disabled() -> bool {
    matches!(
        std::env::var("ARTERM_DESKTOP2_DONUT").as_deref(),
...
impl App {
    ...
    fn on_pointer_pressed(&mut self) {
        ...
        if std::env::var_os("ARTERM_DESKTOP2_LOG_INPUT").is_some() {
            eprintln!(
    ...
    /// window list distinguishes two arterm windows on different checkouts.
    fn retitle(&self) {
        if let Some(state) = self.state.as_ref() {
...
impl hot_worker::ApplicationWorker for App {
    ...
    fn worker_window_event(
        ...
        match event {
            ...
            // System mode follows: an explicit ARTERM_DESKTOP2_THEME choice is
