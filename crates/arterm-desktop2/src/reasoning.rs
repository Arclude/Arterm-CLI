...
//! `ARTERM_REASONING_DISPLAY`, then `display.reasoning_display` in
//! `~/.arterm/config.toml`, then the legacy `show_thinking` boolean, then the
//! desktop default.
...
impl ReasoningMode {
    ...
    pub fn from_env() -> Self {
        Self::resolve(
            std::env::var("ARTERM_REASONING_DISPLAY").ok().as_deref(),
...
fn read_config_text() -> Option<String> {
    std::fs::read_to_string(arterm_home_dir()?.join("config.toml")).ok()
}
...
fn arterm_home_dir() -> Option<std::path::PathBuf> {
    match std::env::var_os("ARTERM_HOME") {
        Some(path) => Some(std::path::PathBuf::from(path)),
        None => std::env::var_os("HOME")
            .map(std::path::PathBuf::from)
            .map(|home| home.join(".arterm")),
    }
}
