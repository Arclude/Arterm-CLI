...
//!
//! - [`scan`] reads `~/.arterm/sessions` and is the only part that touches the
...
/// `~/.arterm/sessions`, or `$ARTERM_HOME/sessions` when the home is overridden.
///
...
pub fn sessions_dir() -> Option<PathBuf> {
    let home = match std::env::var_os("ARTERM_HOME") {
        ...
        None => PathBuf::from(std::env::var_os("HOME")?).join(".arterm"),
    };
...
mod tests {
    ...
    fn picker() -> Picker {
        ...
        picker.set_records(vec![
            record("session_fox_1_a", Some("/home/j/arterm"), 400, 30),
            record("session_owl_2_b", Some("/home/j/site"), 200, 20),
            record("session_bat_3_c", Some("/home/j/arterm"), 100, 10),
        ]);
        picker
    }
    ...
    fn rows_group_sessions_under_their_project() {
        ...
        assert!(
            matches!(&rows[0], Row::Group { label, count, .. } if label == "arterm" && *count == 2)
        );
    ...
    fn a_rescan_keeps_the_selected_session() {
        ...
        picker.set_records(vec![
            record("session_new_9_z", Some("/home/j/arterm"), 10, 90),
            record("session_fox_1_a", Some("/home/j/arterm"), 400, 30),
            record("session_bat_3_c", Some("/home/j/arterm"), 100, 10),
        ]);
    ...
    fn scanning_reads_identity_and_directory_from_a_record() {
        let dir = std::env::temp_dir().join(format!("arterm-resume-scan-{}", std::process::id()));
        ...
        std::fs::write(
            ...
            format!(
                r#"{{"id":"session_fox_1_a","title":"a name","messages":["{filler}"],"working_dir":"/home/j/arterm"}}"#
            ),
        )
        .unwrap();
        ...
        assert_eq!(records[0].working_dir.as_deref(), Some("/home/j/arterm"));
    ...
    fn scanning_a_missing_directory_is_empty() {
        assert!(scan(Path::new("/nonexistent/arterm/sessions"), SCAN_LIMIT).is_empty());
    }
