...
use arterm_tui_mermaid as mermaid;
...
///
/// These pure-string helpers were moved to `arterm-render-core` so the
/// foundation/streaming layer can format reasoning without depending on any
/// `arterm-tui-*` crate. Re-exported here so existing
/// `arterm_tui_markdown::{reasoning_line_markup, reasoning_partial_markup,
/// REASONING_SENTINEL}` paths keep working.
pub use arterm_render_core::{REASONING_SENTINEL, reasoning_line_markup, reasoning_partial_markup};

/// One-line collapsed reasoning summary markup (e.g. `▸ thought (3 lines)`).
///
/// Moved to `arterm-render-core` (pure/backend-neutral) so the foundation/
/// streaming layer can format it without depending on any `arterm-tui-*` crate.
/// Re-exported here so the existing
/// `arterm_tui_markdown::reasoning_summary_line_markup` path keeps working.
pub use arterm_render_core::reasoning_summary_line_markup;
...
use arterm_tui_workspace::color_support::rgb;
...
fn math_inline_span(math: &str) -> Span<'static> {
    Span::styled(
        arterm_render_core::render_inline_latex(math),
...
fn math_display_lines(math: &str) -> Vec<Line<'static>> {
    ...
    for line in arterm_render_core::render_display_latex(math) {
        out.push(
            Line::from(vec![
                Span::styled("│ ", dim),
