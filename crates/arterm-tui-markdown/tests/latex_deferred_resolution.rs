...
//! renderer at a hanging toolchain via process-global `ARTERM_*_COMMAND`
...
fn a_deferred_formula_resolves_into_an_image_once_the_worker_finishes() {
    ...
    unsafe {
        std::env::remove_var("ARTERM_LATEX_COMMAND");
        std::env::remove_var("ARTERM_DVIPNG_COMMAND");
        std::env::remove_var("ARTERM_PDFLATEX_COMMAND");
        std::env::remove_var("ARTERM_PDFTOCAIRO_COMMAND");
    }
    ...
    let render = || {
        arterm_tui_mermaid::with_image_protocol_override(Some(true), || {
            arterm_tui_markdown::render_markdown_with_width(&text, Some(90))
        })
    };
    ...
    let pending = first
        .iter()
        .any(arterm_tui_markdown::line_is_mermaid_pending_placeholder);
    if !pending {
...
fn rendered_a_formula_image(lines: &[ratatui::text::Line<'static>]) -> bool {
    let has_placeholder = lines.iter().any(|line| {
        arterm_tui_mermaid::parse_image_placeholder(line).is_some()
            || arterm_tui_mermaid::parse_inline_image_placeholder(line).is_some()
    });
