...
impl AuthTestTarget {
    ...
    fn credential_paths(self) -> Result<Vec<String>> {
        match self {
            Self::Claude => Ok(vec![
                crate::auth::claude::arterm_path()?.display().to_string(),
...
impl AuthTestSmokeKind {
    ...
    fn success_detail(self) -> &'static str {
        match self {
            ...
            Self::Tool => {
                "Tool-enabled provider request returned AUTH_TEST_OK after one validated real Arterm bash tool call, successful registry execution, and tool-result followup."
            }
        }
    }
