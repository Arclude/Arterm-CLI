//! `arterm menubar` - a lightweight live indicator of how many arterm sessions
...
/// Ensure a single background `arterm menubar` helper is running on macOS so the
/// session-count indicator shows up automatically for every macOS user without
/// them needing to run `arterm menubar` by hand.
///
/// This is a best-effort, fire-and-forget singleton: it records the helper's
/// PID in the *global* `~/.arterm/menubar.pid` (see [`global_menubar_dir`]) and
...
/// hard against sandboxed arterm processes (tests, self-dev, onboarding) ever
/// spawning a helper: each such process runs with a throwaway `$ARTERM_HOME`,
...
pub fn ensure_menubar_helper_running() {
    ...
    if std::env::var_os("ARTERM_NO_MENUBAR").is_some() {
        return;
    }

    // Sandboxed arterm (tests / self-dev / onboarding, anything with a throwaway
    // `$ARTERM_HOME`) must never manage the real user's global menu bar.
...
/// arterm process for this user, so this state must live at a fixed location
/// that does **not** depend on `$ARTERM_HOME`. Sandboxes (tests, self-dev,
/// onboarding) override `$ARTERM_HOME` with throwaway temp dirs; anchoring to
/// the real home (`$HOME/.arterm`) gives every process the same lock inode so
/// the singleton actually holds across them. For a normal (non-sandboxed)
/// launch this is exactly `crate::storage::arterm_dir()`, so behavior for the
/// real user is unchanged.
#[cfg(target_os = "macos")]
fn global_menubar_dir() -> Option<std::path::PathBuf> {
    let home = dirs::home_dir()?;
    let dir = home.join(".arterm");
    let _ = std::fs::create_dir_all(&dir);
    Some(dir)
}

/// True when this process is a sandboxed arterm that must not own the real
/// user's global menu bar. A throwaway `$ARTERM_HOME` (anything other than the
/// real `~/.arterm`) or an explicit test/temp marker means "sandbox".
#[cfg(target_os = "macos")]
fn running_in_menubar_sandbox() -> bool {
    is_menubar_sandbox(
        env_truthy("ARTERM_TEST_SESSION"),
        env_truthy("ARTERM_TEMP_SERVER"),
        std::env::var_os("ARTERM_HOME").as_deref(),
        dirs::home_dir().map(|home| home.join(".arterm")).as_deref(),
    )
}
...
/// - A `$ARTERM_HOME` that differs from the real `~/.arterm` is a sandbox home.
...
fn is_menubar_sandbox(
    ...
    real_arterm_home: Option<&std::path::Path>,
) -> bool {
    if test_session || temp_server {
        return true;
    }

    // No explicit override: the real user's default `~/.arterm`.
    let Some(custom_home) = custom_home else {
        return false;
    };
    let custom = std::path::Path::new(custom_home);
    let Some(real) = real_arterm_home else {
...
mod macos {
    ...
    /// `~/.arterm/menubar.lock` (see [`super::global_menubar_dir`]) so the lock
    /// is shared across every arterm process for this OS user, including ones
    /// running with a sandboxed `$ARTERM_HOME`. The menu bar itself is a single
    ...
    const STATUS_ITEM_AUTOSAVE: &str = "arterm-menubar";
    ...
    define_class!(
           ...
           #[name = "ArtermMenubarHandler"]
           struct MenuHandler;

           impl MenuHandler {
               /// Open the clicked session (stored in the item's representedObject)
               /// in a new terminal window via `arterm --resume <id>`.
               ...
               fn open_session(&self, sender: &NSMenuItem)
    {
                   ...
                   launch_arterm_window(vec!["--resume".to_string(), session_id.to_string()]);
               }

               /// Launch a brand-new arterm session in a new terminal window.
               #[unsafe(method(newWindow:))]
               fn new_window(&self, _sender: &NSMenuItem) {
                   launch_arterm_window(Vec::new());
               }
           }
       );
    ...
    /// Launch a arterm window off the main thread so slow terminal startup
    /// (osascript / `open`) never blocks the menu bar UI.
    fn launch_arterm_window(args: Vec<String>) {
        std::thread::spawn(move || {
            if let Err(err) = crate::setup_hints::launch_arterm_in_macos_terminal(&args) {
                crate::logging::warn(&format!(
                    "menubar: failed to launch arterm window ({args:?}): {err}"
                ));
            }
        });
    }
    ...
    pub(super) fn run_status_item_app() {
        let mtm = MainThreadMarker::new()
            .expect("arterm menubar must run on the main thread (the process entry point)");
        ...
        // stray `arterm menubar` invoked directly inside a test harness should
        // still never realize a status item.
        if super::env_truthy("ARTERM_TEST_SESSION") || super::env_truthy("ARTERM_TEMP_SERVER") {
            return;
        }
        ...
        // bypassed entirely (e.g. a self-dev `target/.../arterm` and the
        // installed `~/.local/bin/arterm` both spawn helpers, or a reload
        ...
        // Follow the system's Light/Dark setting explicitly. `arterm` runs as a
        ...
        if let Some(button) = status_item.button(mtm) {
            let icon = NSImage::imageWithSystemSymbolName_accessibilityDescription(
                ...
                Some(ns_string!("arterm sessions")),
            );
        ...
        let quit_item = unsafe {
            NSMenuItem::initWithTitle_action_keyEquivalent(
                NSMenuItem::alloc(mtm),
                ns_string!("Quit arterm menu bar"),
    ...
    ///
    /// `arterm` runs as a bare executable without an `Info.plist` app bundle, so
...
mod tests {
    ...
    fn session_menu_item_title_loads_persisted_todo_title() {
        ...
        let temp = tempfile::tempdir().expect("create temporary ARTERM_HOME");
    ...
    fn menubar_sandbox_detection() {
        ...
        let real = Path::new("/Users/me/.arterm");
        ...
        assert!(!is_menubar_sandbox(
            false,
            false,
            Some(OsStr::new("/Users/me/.arterm")),
            Some(real),
        ));
        ...
        assert!(is_menubar_sandbox(
            false,
            false,
            Some(OsStr::new("/private/tmp/arterm-e2e-home-xyz")),
            Some(real),
        ));
