...
pub(crate) async fn run_login(no_browser: bool) -> Result<()> {
    super::login::run_arterm_account_login(no_browser).await
}

pub(crate) async fn run_status(json: bool) -> Result<()> {
    let Some(api_key) = crate::subscription_catalog::configured_api_key() else {
        anyhow::bail!(
            "No Arterm account credential is configured. Run `arterm account login` to sign in."
        );
    };
    ...
    match subscription_api::fetch_subscription_me_with(&client, &api_base, &api_key).await {
        ...
        Err(AccountApiError::Unauthorized) => {
            ...
            anyhow::bail!(
                "The Arterm account key was revoked or expired. Local credentials were cleared. Run `arterm account login` to sign in again."
            )
        }
...
pub(crate) fn run_manage() -> Result<()> {
    ...
    println!("Opening Arterm account management: {url}");
...
pub(crate) async fn run_logout() -> Result<()> {
    ...
    crate::subscription_catalog::clear_account_credentials()
        .context("Failed to securely clear local Arterm account credentials")?;
    crate::auth::AuthStatus::invalidate_cache();

    match (api_key.is_some(), remote) {
        (false, _) => {
            println!("No local Arterm account credential was present. Local account cache is clear.")
        }
        (true, Ok(())) => println!(
            "Arterm account key revoked. Local credentials and account cache were securely cleared."
        ),
        (true, Err(AccountApiError::Unauthorized)) => println!(
            "The Arterm account key was already revoked. Local credentials and account cache were securely cleared."
        ),
...
fn public_manage_url(candidate: Option<&str>) -> &str {
    candidate
        ...
        .unwrap_or(crate::subscription_catalog::ARTERM_ACCOUNT_URL)
}
