...
fn resolve_or_clone_repo_dir() -> Result<PathBuf> {
    ...
    if repo_dir.exists() {
        ...
        anyhow::bail!(
            "Self-dev source directory exists but is not a arterm repository: {}\n\
    ...
    output::stderr_info(format!(
        "No local arterm checkout found; cloning self-dev source into {}...",
        repo_dir.display()
    ));
    ...
    if !build::is_arterm_repo(&repo_dir) {
        anyhow::bail!(
            "Cloned self-dev source is not a valid arterm repository: {}",
            repo_dir.display()
        );
    }
...
pub async fn run_self_dev(should_build: bool, resume_session: Option<String>) -> Result<()> {
    ...
    crate::env::set_var("ARTERM_REPO_DIR", &repo_dir);
    ...
    if !target_binary.exists() {
        anyhow::bail!(
            "No binary found at {:?}\n\
             Run 'arterm self-dev --build' first, or build with '{}' and then publish current.",
            target_binary,
