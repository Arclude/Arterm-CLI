...
fn test_provider_choice_arg_values() {
    assert_eq!(ProviderChoice::Arterm.as_arg_value(), "arterm");
...
async fn explicit_anthropic_api_choice_pins_api_key_over_available_oauth() {
    ...
    let keys = [
        ...
        "ARTERM_ACTIVE_PROVIDER",
        "ARTERM_INITIAL_PROVIDER_EXPLICIT",
    ];
    ...
    assert_eq!(
        std::env::var("ARTERM_RUNTIME_PROVIDER").ok().as_deref(),
        Some("claude-api")
    );
...
#[test]
fn test_init_provider_arterm_delegates_runtime_profile_to_wrapper() {
    let _guard = lock_env();
    let _env_guard = crate::storage::lock_test_env();
    // Sandbox ARTERM_HOME: with the real home, persisted auth/credential state
    // (e.g. a pinned anthropic api-key route) re-pins ARTERM_RUNTIME_PROVIDER
    // during MultiProvider construction and breaks the assertions below.
    let dir = TempDir::new().expect("temp dir");
    let saved_home = std::env::var("ARTERM_HOME").ok();
    crate::env::set_var("ARTERM_HOME", dir.path());
    crate::subscription_catalog::clear_runtime_env();
    crate::env::remove_var("ARTERM_OPENROUTER_MODEL");
    ...
    assert_eq!(
        std::env::var("ARTERM_ACTIVE_PROVIDER").ok().as_deref(),
        Some("openrouter")
    );
    assert_eq!(
        std::env::var("ARTERM_RUNTIME_PROVIDER").ok().as_deref(),
    ...
    assert_eq!(
        std::env::var("ARTERM_INITIAL_PROVIDER_EXPLICIT")
            .ok()
            .as_deref(),
        Some("1")
    );
    ...
    crate::env::remove_var("ARTERM_INITIAL_PROVIDER_EXPLICIT");
    match saved_home {
        Some(home) => crate::env::set_var("ARTERM_HOME", home),
...
fn choice_for_login_provider_round_trips_core_targets() {
    assert_eq!(
        choice_for_login_provider(provider_catalog::ARTERM_LOGIN_PROVIDER),
        Some(ProviderChoice::Arterm)
    );
    assert_eq!(
...
fn resolved_profile_default_model_uses_openai_compatible_override() {
    ...
    crate::env::set_var("ARTERM_OPENAI_COMPAT_DEFAULT_MODEL", "llama3.2");
...
fn apply_login_provider_profile_env_keeps_an_explicit_named_profile() {
    ...
    // ARTERM_NAMED_PROVIDER_PROFILE before probing, so the probe evaluated the
    ...
    crate::env::set_var("ARTERM_NAMED_PROVIDER_PROFILE", "company-gateway");
    crate::env::set_var("ARTERM_OPENROUTER_API_BASE", "https://gw.example.com/v1");
    crate::env::set_var("ARTERM_OPENROUTER_API_KEY_NAME", "COMPANY_GATEWAY_KEY");
...
fn apply_login_provider_profile_env_preserves_compatible_profile_for_auto_spawn() {
    ...
    assert_eq!(
        std::env::var("ARTERM_PROVIDER_PROFILE_ACTIVE")
            .ok()
            .as_deref(),
        Some("1")
    );
...
async fn auto_provider_uses_config_default_named_no_auth_provider() {
    ...
    let saved: Vec<(String, Option<String>)> = [
        ...
        "ARTERM_OPENROUTER_ALLOW_NO_AUTH",
        "ARTERM_OPENROUTER_ENV_FILE",
        "ARTERM_OPENROUTER_DEFAULT_MODEL",
        "ARTERM_OPENROUTER_CACHE_NAMESPACE",
        "ARTERM_PROVIDER_PROFILE_ACTIVE",
        "ARTERM_PROVIDER_PROFILE_NAME",
        "ARTERM_NAMED_PROVIDER_PROFILE",
    ...
    crate::env::set_var("ARTERM_HOME", dir.path());
    crate::env::set_var("ARTERM_NON_INTERACTIVE", "1");
    for key in [
        "ANTHROPIC_API_KEY",
        "OPENAI_API_KEY",
        "OPENROUTER_API_KEY",
...
async fn auto_provider_noninteractive_skips_untrusted_external_auth_instead_of_blocking() {
    ...
    let saved: Vec<(String, Option<String>)> = [
        "ARTERM_HOME",
        "ARTERM_NON_INTERACTIVE",
        "ARTERM_DEFERRED_AUTH_BOOTSTRAP",
        "ANTHROPIC_API_KEY",
        "OPENAI_API_KEY",
        "OPENROUTER_API_KEY",
        "GITHUB_TOKEN",
