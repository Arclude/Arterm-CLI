...
pub async fn run_tui_client(
    ...
    if let Some(ref session_id) = resume_session {
        ...
        let _ = crossterm::execute!(std::io::stdout(), crossterm::terminal::SetTitle("arterm"));
    }
        ...
        if let Some(output) = video_output {
            let output_path = if output == "auto" {
                ...
                let safe_name = session_id_or_path
                    ...
                    .collect::<String>();
                std::path::PathBuf::from(format!("arterm_swarm_replay_{}_{}.mp4", safe_name, date))
            } else {
    ...
    if let Some(output) = video_output {
        let output_path = if output == "auto" {
            ...
            let safe_name = session_name
                ...
                .collect::<String>();
            std::path::PathBuf::from(format!("arterm_replay_{}_{}.mp4", safe_name, date))
        } else {
            std::path::PathBuf::from(output)
        };
        eprintln!(
...
pub fn list_sessions() -> Result<()> {
    fn build_resume_target_command(
        exe: &std::path::Path,
        target: &arterm_tui_session_picker::ResumeTarget,
    ) -> (std::path::PathBuf, Vec<String>) {
        match target {
            arterm_tui_session_picker::ResumeTarget::ArtermSession { session_id } => (
                exe.to_path_buf(),
                vec!["--resume".to_string(), session_id.clone()],
            ),
            arterm_tui_session_picker::ResumeTarget::ClaudeCodeSession { session_id, .. } => (
                exe.to_path_buf(),
                vec![
                    "--resume".to_string(),
            ...
            arterm_tui_session_picker::ResumeTarget::CodexSession { session_id, .. } => (
                exe.to_path_buf(),
                vec![
                    "--resume".to_string(),
            ...
            arterm_tui_session_picker::ResumeTarget::PiSession { session_path } => (
                exe.to_path_buf(),
                vec![
                    "--resume".to_string(),
            ...
            arterm_tui_session_picker::ResumeTarget::OpenCodeSession { session_id, .. } => (
                exe.to_path_buf(),
                vec![
                    "--resume".to_string(),
            ...
            arterm_tui_session_picker::ResumeTarget::CursorSession { session_id, .. } => (
                exe.to_path_buf(),
                vec![
                    "--resume".to_string(),
    ...
    match tui::session_picker::pick_session()? {
        Some(tui::session_picker::PickerResult::TakeOverClaude(target)) => {
            ...
            let arterm_tui_session_picker::ResumeTarget::ArtermSession { session_id } =
                &resolved_target
            else {
                anyhow::bail!("Claude takeover did not produce a Arterm session");
            };
        ...
        Some(
            ...
            if targets.len() == 1 {
                ...
                let resolved_target = crate::import::resolve_resume_target_to_arterm(target)?;
                let mut session_cwd = cwd.clone();
                if let arterm_tui_session_picker::ResumeTarget::ArtermSession { session_id } =
                    &resolved_target
                ...
                for target in targets {
                    let resolved_target =
                        match crate::import::resolve_resume_target_to_arterm(&target) {
                            Ok(target) => target,
                            Err(e) => {
            ...
            for session_id in recovered {
                ...
                match spawn_resume_in_new_terminal(&exe, &session_id, &session_cwd) {
                    ...
                    Ok(false) => {
                        ...
                        eprintln!("  arterm --resume {}", session_id);
                    }
                    Err(e) => {
