...
$originalEnvironment = @{
    ...
    ARTERM_HOME = $env:ARTERM_HOME
    ARTERM_WINDOWS_SETUP_SKIP_PROCESS_LIFECYCLE = $env:ARTERM_WINDOWS_SETUP_SKIP_PROCESS_LIFECYCLE
}
...
if (-not $Version) {
    ...
    if ($artifactVersionText -notmatch '(?i)\barterm\s+v?(\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?)\b') {
        throw "Could not parse arterm version from local artifact output: $artifactVersionText"
    }
    $Version = 'v' + $Matches[1]
} else {
...
$tempRoot = Join-Path $tempBase ("arterm-windows-install-verify-" + [guid]::NewGuid().ToString('N'))
$localAppData = Join-Path $tempRoot 'localappdata'
$appData = Join-Path $tempRoot 'appdata'
$userProfile = Join-Path $tempRoot 'userprofile'
$artermHome = Join-Path $tempRoot '.arterm'
$installDir = Join-Path $localAppData 'arterm\bin'

try {
New-Item -ItemType Directory -Force -Path $localAppData, $appData, $userProfile, $artermHome | Out-Null

$env:LOCALAPPDATA = $localAppData
$env:APPDATA = $appData
$env:USERPROFILE = $userProfile
$env:ARTERM_HOME = $artermHome
$env:ARTERM_WINDOWS_SETUP_SKIP_PROCESS_LIFECYCLE = '1'
...
$versionDir = Join-Path $localAppData ('arterm\builds\versions\' + $Version.TrimStart('v') + '\arterm.exe')
$stablePath = Join-Path $localAppData 'arterm\builds\stable\arterm.exe'
...
$startupShortcut = Join-Path $appData 'Microsoft\Windows\Start Menu\Programs\Startup\arterm-hotkey.lnk'
...
if ($versionOutput -notmatch 'arterm') {
...
$legacyVbs = Join-Path $hotkeyDir 'arterm-hotkey-launcher.vbs'
