...
arterm requests the protocol at startup (`enable_keyboard_enhancement`) and
...
Three situations still break, and arterm handles each explicitly:
...
- **Trailing backslash then Enter** inserts a newline, matching shell line
  continuation. The first time you use it, arterm points you at
  `/terminal-setup`.
