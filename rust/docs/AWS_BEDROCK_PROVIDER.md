...
Arterm supports a native AWS Bedrock provider that talks directly to Bedrock Runtime with the AWS Rust SDK and `ConverseStream`.

## Configure credentials

Arterm supports two Bedrock auth styles:

- **Bedrock API key / bearer token**: easiest for local onboarding. Arterm stores the token in its config env file and sends it through the AWS SDK as `AWS_BEARER_TOKEN_BEDROCK`.
...
```bash
arterm login --provider bedrock
```

This saves `AWS_BEARER_TOKEN_BEDROCK` and `ARTERM_BEDROCK_REGION` to `~/.config/arterm/bedrock.env`.
...
export AWS_REGION=us-east-1
# Optional Arterm-specific overrides:
export ARTERM_BEDROCK_PROFILE=my-profile
export ARTERM_BEDROCK_REGION=us-east-1
```

If you rely on instance/container metadata credentials and have no local profile env vars, opt in explicitly:

```bash
export ARTERM_BEDROCK_ENABLE=1
export AWS_REGION=us-east-1
```
...
For AWS CLI console-login profiles, Arterm can also use credentials exported by:

```bash
aws configure export-credentials --profile my-profile --format env-no-export
```

Arterm does not store these exported session credentials; it asks the AWS CLI profile provider when the Bedrock provider initializes.
...
If you enable STS validation with `ARTERM_BEDROCK_VALIDATE_STS=1`, allow `sts:GetCallerIdentity`.

## Run Arterm with Bedrock

```bash
arterm --provider bedrock --model anthropic.claude-3-5-sonnet-20241022-v2:0
```

or:

```bash
arterm --model bedrock:anthropic.claude-3-5-sonnet-20241022-v2:0
```

Inference profile IDs/ARNs are accepted as model IDs, for example:

```bash
arterm --model bedrock:us.anthropic.claude-3-5-sonnet-20241022-v2:0
```
...
```bash
export ARTERM_BEDROCK_MAX_TOKENS=4096
export ARTERM_BEDROCK_TEMPERATURE=0.2
export ARTERM_BEDROCK_TOP_P=0.9
export ARTERM_BEDROCK_STOP_SEQUENCES='</done>,STOP'
```

## Model discovery

Arterm will use a static Bedrock model list immediately. When model prefetch/catalog refresh runs, it calls `ListFoundationModels` and `ListInferenceProfiles`, then caches results in Arterm's config directory. Cached Bedrock catalogs are region-scoped; if you switch `ARTERM_BEDROCK_REGION`/`AWS_REGION`, Arterm ignores the old-region cache and refreshes for the new region.
...
```bash
ARTERM_BEDROCK_LIVE_TEST=1 \
AWS_PROFILE=my-profile \
AWS_REGION=us-east-1 \
cargo test -p arterm --lib provider::bedrock::tests::bedrock_live_smoke_test -- --ignored
```
...
- Missing region: set `AWS_REGION` or `ARTERM_BEDROCK_REGION`.
