# Arterm Desktop Architecture Direction
...
This document captures the initial direction for a desktop application for Arterm under these constraints:
...
The goal is to make the desktop client a first-class Arterm surface without forking the Arterm runtime or turning the app into a heavyweight IDE clone.
...
Build Arterm Desktop as a small Rust desktop client with a custom GPU-rendered UI. The app should connect to a local Arterm server/daemon that owns sessions, tools, agent execution, persistence, and permissions.
...
| Backend/runtime | Existing Rust Arterm server/session runtime |
| Process model | Desktop frontend + local Arterm daemon/server |
...
Arterm Desktop should not start as a full IDE and should not look like a conventional chatbot.
...
1. Existing Arterm server channel, if compatible with desktop needs.
...
A debug HUD should exist in the prototype before real Arterm integration is considered complete.
...
- connects to local Arterm server/daemon
- lists sessions
...
```text
crates/
  arterm-desktop-protocol/   # shared protocol/event types if not already covered by server types
  arterm-desktop-ui/         # UI tree, layout, text/cache abstractions, renderer-agnostic pieces
  arterm-desktop-renderer/   # wgpu renderer and GPU resources
  arterm-desktop/            # app shell, platform window, protocol client, product UI
```
...
- small serialization/protocol crates already consistent with Arterm
...
4. Reuse the existing Arterm server protocol or introduce a desktop-specific event protocol crate?
...
Only after that should the real Arterm event stream be connected.
