...
- `arterm-tui-mermaid/src/lib.rs` is still a state hub despite the crate split.
...
The renderer now has an `mmdr-size-api` path guarded by the `mmdr-size-api` feature plus `ARTERM_MMDR_SIZE_API_AVAILABLE=1`. That should become the primary path for the redesign:
