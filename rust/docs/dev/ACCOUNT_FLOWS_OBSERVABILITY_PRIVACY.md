...
- Client telemetry pipeline: `crates/arterm-telemetry-core/src/lib.rs`
  (`record_auth_success` @1409, `sanitize_telemetry_label` applied to all
  auth labels @398-400; `AuthEvent` schema in
  `crates/arterm-usage-types/src/lib.rs:270-284` has NO account_id/email slot,
  by design — see the TODO at `src/cli/login/arterm_device.rs:370-374`).
- Onboarding auth funnel: `auth_provider`, `auth_method`,
  `auth_failure_reason` on onboarding step events
  (`crates/arterm-telemetry-core/src/lib.rs:369-400`).
- Failure classification labels: `crates/arterm-base/src/auth/login_diagnostics.rs`
  (`AuthFailureReason::label`) — the only failure detail that may be reported.
- Local logs: `~/.arterm/logs/arterm-YYYY-MM-DD.log`.
...
- `api_key` (ARTERM_API_KEY) — full or truncated beyond `jc_...last4`.
...
- Env-file contents (`arterm-subscription` env file) and `Authorization`
...
`auth_method` ("arterm-subscription", "device_code_magic_link"), failure
...
- Do NOT add `account_id` to `AuthEvent` (the existing TODO at
  `src/cli/login/arterm_device.rs:370` should be resolved as "backend audit
...
- `me_requests_total{status}`; alert on 401 spike (mass revocation bug) and
  ...
  `crates/arterm-base/src/subscription_api.rs:14`).
...
| Local client logs | `~/.arterm/logs/` | user-owned; must satisfy never-log list |
| Support bundles | created on demand | delete after case close (<= 90 days) |

## Support diagnostics

`arterm doctor`-style account diagnostics (extend existing provider doctor,
`crates/arterm-provider-doctor/`): prints auth_base, masked key
...
- Add `record_auth_failure` for the arterm device flow (labels only).
- Resolve the `account_linked` TODO in `arterm_device.rs` as "won't add to
