...
This demo gives Arterm a deterministic local storefront. The agent can search,
...
> Use `./bin/arterm-demo-shop` to see whether this shop has a USB-C laptop charger
...
1. Arterm inspects the CLI, searches the shop, and chooses the qualifying 65W
   charger.
2. The shop verifies the simulated `$43.19` total and reports that no payment
   method is available.
3. Arterm independently browses `payments`, receives AgentCard, selects it, and
