...
impl AnthropicContextMode {
    ...
    /// Whether arterm should surface a distinct `[1m]` picker alias for this model.
...
/// small truncates mid-tool-call and silently ends an agent turn. arterm used a
...
pub fn anthropic_max_output_tokens(model: &str) -> u32 {
    ...
    // Older/unknown generations keep the conservative 32K arterm has always used.
    32_768
}
...
mod tests {
    ...
    fn max_output_tokens_never_undercut_the_legacy_default() {
        ...
        // flat 32K default arterm shipped before, or turns that used to fit would
        // start truncating.
