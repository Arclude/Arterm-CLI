...
fn arterm_home_test_lock() -> MutexGuard<'static, ()> {
...
struct ScopedArtermHome {
    path: PathBuf,
    previous: Option<OsString>,
    _guard: MutexGuard<'static, ()>,
}

impl ScopedArtermHome {
    fn new(label: &str) -> Self {
        let guard = arterm_home_test_lock();
        let previous = std::env::var_os("ARTERM_HOME");
        let unique = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap_or_default()
            .as_nanos();
        let path = std::env::temp_dir().join(format!(
            "arterm-harness-api-{label}-{}-{unique}",
            std::process::id()
        ));
        std::fs::create_dir_all(&path).expect("create isolated ARTERM_HOME");
        // SAFETY: all tests in this module that mutate ARTERM_HOME share `LOCK`,
        // and this guard restores the prior value before it is released.
        unsafe { std::env::set_var("ARTERM_HOME", &path) };
        Self {
            path,
            previous,
            _guard: guard,
...
impl Drop for ScopedArtermHome {
    fn drop(&mut self) {
        match self.previous.take() {
            Some(value) => unsafe { std::env::set_var("ARTERM_HOME", value) },
...
#[test]
fn create_session_in_a_arterm_checkout_requests_selfdev() {
...
///
/// `launch()` gives an embedded instance its own `ARTERM_HOME` precisely so it
/// cannot see the user's work. Reading the user's home directly made
/// `peek_session` return the real transcripts of the arterm the user runs
/// interactively, from a client that was supposed to be sandboxed.
#[test]
fn session_records_are_read_from_the_instance_home() {
    let home = ScopedArtermHome::new("instance-home");
    ...
    assert!(
        ...
        "ARTERM_HOME must scope session records, got {}",
        path.display()
    );
}
...
fn unattached_list_sessions_discovers_all_persisted_records() {
    let home = ScopedArtermHome::new("persisted-discovery");
...
fn archive_restore_and_retention_are_reversible_and_owner_only() {
    let home = ScopedArtermHome::new("archive");
...
#[test]
fn credential_provisioning_normalizes_gemini_and_supports_arterm() {
    let home = ScopedArtermHome::new("credentials");
    let config = home.path.join("config/arterm");
    ...
    let outbound = state.api_request_to_legacy(&json!({
        ...
        "api_key": "arterm-secret"
    }));
    let [Outbound::Legacy(notify)] = outbound.as_slice() else {
        panic!("arterm credential should notify the daemon: {outbound:?}");
    };
    assert_eq!(notify["provider"], "arterm");
    assert_eq!(
        std::fs::read_to_string(config.join("arterm-subscription.env")).unwrap(),
        "ARTERM_API_KEY=arterm-secret\n"
    );
...
fn owner_only_writes_refuse_symlink_targets_and_directories() {
    ...
    let home = ScopedArtermHome::new("credential-symlinks");
    ...
    assert!(!outside_dir.join("arterm-subscription.env").exists());
}
...
fn rooted_file_operations_reject_traversal_and_symlink_escapes_and_bound_results() {
    ...
    let home = ScopedArtermHome::new("rooted-files");
    let root = home.path.join("project");
