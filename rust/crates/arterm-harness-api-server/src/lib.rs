...
//! Unix socket and translates to the internal (legacy) arterm protocol.
//!
//! Architecture (milestone 2 of docs/HARNESS_API_AND_DESKTOP_REWRITE.md):
//! - Listens on `~/.arterm/arterm-api.sock` (or `ARTERM_API_SOCKET`).
//! - For each API client, dials the legacy daemon socket (`ARTERM_SOCKET` or
//!   `~/.arterm/arterm.sock`) and speaks `subscribe`/`message`/... on its
//!   behalf.
...
use arterm_harness_api::{API_VERSION_MAJOR, ApiEvent, ErrorCode, ServerFrame};
...
// all.
use arterm_transport::{Listener, Stream};

// Socket paths live in `arterm-harness-api` so clients and the bridge can never
// resolve different directories (they once did, and the desktop app could not
// connect as a result).
pub use arterm_harness_api::{api_socket_path, legacy_socket_path};
...
async fn handle_api_client(stream: Stream, legacy_socket: PathBuf) -> Result<()> {
    ...
    let hello_ok = ServerFrame::reply(
        ...
        ApiEvent::HelloOk {
            ...
            server: format!("arterm-harness-api-bridge/{}", env!("CARGO_PKG_VERSION")),
            capabilities: [
                "sessions",
                "streaming",
...
mod single_instance_tests {
    ...
    fn a_second_bridge_cannot_take_the_socket() {
        let dir = std::env::temp_dir().join(format!("arterm-bridge-lock-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let socket = dir.join("arterm-api.sock");
...
mod socket_permission_tests {
    ...
    async fn the_api_socket_is_owner_only() {
        ...
        let dir = std::env::temp_dir().join(format!("arterm-api-perm-{}", std::process::id()));
