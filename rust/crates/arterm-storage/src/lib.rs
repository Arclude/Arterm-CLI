...
///
/// Can be overridden with `$ARTERM_RUNTIME_DIR`.
pub fn runtime_dir() -> PathBuf {
    if let Ok(dir) = std::env::var("ARTERM_RUNTIME_DIR") {
...
fn fallback_runtime_dir() -> PathBuf {
    std::env::temp_dir().join(format!("arterm-{}", runtime_user_discriminator()))
}
...
fn ensure_private_runtime_dir(path: &Path) {
    ...
    #[cfg(unix)]
    {
        let _ = arterm_core::fs::set_directory_permissions_owner_only(path);
    }
}

pub fn arterm_dir() -> Result<PathBuf> {
    if let Ok(path) = std::env::var("ARTERM_HOME") {
...
/// to `~/.arterm/state` (respecting `ARTERM_HOME`).
///
/// When `ARTERM_RUNTIME_DIR` is set (tests and sandboxed temp servers), it
/// takes precedence so isolated runs never touch the real arterm home.
...
/// Resolve arterm's app-owned config directory.
///
/// Default location is the platform config dir + `arterm` (for example
/// `~/.config/arterm` on Linux). When `ARTERM_HOME` is set, sandbox this under
/// `$ARTERM_HOME/config/arterm` so self-dev/tests do not leak into the user's
/// real config directory.
pub fn app_config_dir() -> Result<PathBuf> {
    if let Ok(path) = std::env::var("ARTERM_HOME") {
        return Ok(PathBuf::from(path).join("config").join("arterm"));
    }

    let config_dir =
        dirs::config_dir().ok_or_else(|| anyhow::anyhow!("No config directory found"))?;
    Ok(config_dir.join("arterm"))
}

/// Resolve a path under the user's home directory, but sandbox it under
/// `$ARTERM_HOME/external/` when `ARTERM_HOME` is set.
///
...
pub fn harden_user_config_permissions() {
    #[cfg(windows)]
    {
        if let Some(config_dir) = dirs::config_dir() {
            let arterm_config_dir = config_dir.join("arterm");
            if arterm_config_dir.exists() {
                schedule_windows_path_hardening(&arterm_config_dir, true);
            }
        }

        if let Ok(arterm_home) = arterm_dir()
            && arterm_home.exists()
        {
            schedule_windows_path_hardening(&arterm_home, true);
        }
        return;
    }
    ...
    #[cfg(not(windows))]
    {
        if let Some(config_dir) = dirs::config_dir() {
            let arterm_config_dir = config_dir.join("arterm");
            if arterm_config_dir.exists() {
                let _ = arterm_core::fs::set_directory_permissions_owner_only(&arterm_config_dir);
            }
        }
        ...
        if let Ok(arterm_home) = arterm_dir()
            && arterm_home.exists()
        {
            let _ = arterm_core::fs::set_directory_permissions_owner_only(&arterm_home);
        }
    }
}
...
pub fn harden_secret_file_permissions(path: &Path) {
    ...
    #[cfg(not(windows))]
    {
        if let Some(parent) = path.parent() {
            let _ = arterm_core::fs::set_directory_permissions_owner_only(parent);
        }
        if path.exists() {
            let _ = arterm_core::fs::set_permissions_owner_only(path);
        }
    }
}
...
fn harden_secret_file_permissions_windows(path: &Path) {
    ...
    // Read-time hardening is opportunistic, while Arterm's own secret writes
...
fn schedule_windows_path_hardening(path: &Path, directory: bool) {
    ...
    if std::thread::Builder::new()
        .name("arterm-windows-acl-harden".to_string())
        .spawn(|| {
...
fn run_windows_hardening_worker() {
    loop {
        ...
        for path in &directories {
            let succeeded = arterm_core::fs::set_directory_permissions_owner_only(path).is_ok();
            directory_results.push((path.clone(), succeeded));
        }
        let mut file_results = Vec::with_capacity(files.len());
        for path in &files {
            let succeeded =
                !path.exists() || arterm_core::fs::set_permissions_owner_only(path).is_ok();
...
///
/// arterm intentionally avoids mutating these files. We also reject obvious risky
...
pub fn ensure_dir(path: &Path) -> Result<()> {
    if !path.exists() {
        ...
        arterm_core::fs::set_directory_permissions_owner_only(path)?;
    }
    Ok(())
}
...
fn write_bytes_inner(path: &Path, bytes: &[u8], durable: bool, secret: bool) -> Result<()> {
    if let Some(parent) = path.parent() {
        ...
        if secret {
            ...
            arterm_core::fs::set_directory_permissions_owner_only(parent)?;
        }
    }
    ...
    let result = (|| -> Result<()> {
        ...
        if secret {
            arterm_core::fs::set_permissions_owner_only(&tmp_path)?;
        }
        ...
        if path.exists() {
            ...
            if secret {
                arterm_core::fs::set_permissions_owner_only(path)?;
            }
            ...
            if secret && bak_path.exists() {
                arterm_core::fs::set_permissions_owner_only(&bak_path)?;
            }
        }
...
mod windows_hardening_tests {
    ...
    fn first_path_starts_one_worker_and_repeated_paths_are_coalesced() {
        ...
        let now = Instant::now();
        let directory = Path::new(r"C:\Users\test\.arterm");
    ...
    fn recently_attempted_paths_are_not_requeued() {
        ...
        let file = PathBuf::from(r"C:\Users\test\.arterm\auth.json");
        state
            .files
