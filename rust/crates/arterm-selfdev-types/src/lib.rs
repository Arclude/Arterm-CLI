...
/// the `cli` subsystem.
pub const CLIENT_SELFDEV_ENV: &str = "ARTERM_CLIENT_SELFDEV_MODE";
...
pub enum SelfDevBuildTarget {
    Auto,
    Tui,
    /// The desktop app (`arterm-desktop2`).
    Desktop2,
    All,
}
...
impl SelfDevBuildTarget {
    pub fn parse(value: Option<&str>) -> Result<Self> {
        match value.unwrap_or("auto").trim().to_ascii_lowercase().as_str() {
            "" | "auto" => Ok(Self::Auto),
            "tui" | "arterm" => Ok(Self::Tui),
            "desktop" | "desktop2" | "arterm-desktop2" => Ok(Self::Desktop2),
