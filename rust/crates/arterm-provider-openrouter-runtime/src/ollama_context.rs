...
//!
//! Ollama's `/v1/models` response carries no `context_length`, so arterm used to
...
//! the conversation" (issue: Ollama conversations appear stateless in arterm).
//!
...
/// user knows to raise `OLLAMA_CONTEXT_LENGTH` instead of assuming arterm lost
/// the conversation.
