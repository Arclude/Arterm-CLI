...
fn named_profile_static_models_survive_live_catalog_refresh() {
    ...
    // include the user's config-declared model.
    {
        ...
        cache.models = vec![arterm_provider_openrouter::ModelInfo {
...
async fn config_toml_models_survive_a_real_catalog_fetch() {
    ...
    // Exactly the shape a user writes in ~/.arterm/config.toml.
    let toml_src = format!(
        r#"
base_url = "{api_base}"
auth = "none"
...
/// `ARTERM_OPENROUTER_CACHE_NAMESPACE` env var, so with several named profiles
...
fn named_profiles_keep_distinct_disk_cache_namespaces() {
    ...
    let _home = EnvVarGuard::set("ARTERM_HOME", temp.path().to_str().expect("utf8 temp path"));
    let _namespace = EnvVarGuard::remove("ARTERM_OPENROUTER_CACHE_NAMESPACE");
    let _key = EnvVarGuard::set("TEST_NS_KEY", "test-key");

    let make_profile = |base_url: &str| arterm_base::config::NamedProviderConfig {
