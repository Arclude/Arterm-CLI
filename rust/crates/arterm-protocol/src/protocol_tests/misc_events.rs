...
fn test_input_shell_result_event_roundtrip() -> Result<()> {
    let event = ServerEvent::InputShellResult {
        result: arterm_message_types::InputShellResult {
...
fn test_structured_set_route_decodes_as_set_route_not_set_model() -> Result<()> {
    ...
    let request = Request::SetRoute {
        id: 7,
        selection: arterm_provider_core::RouteSelection {
            model: "gpt-5.5".to_string(),
            runtime_key: arterm_provider_core::RuntimeKey::OpenAIApiKey,
...
#[test]
fn test_arterm_subscription_set_route_is_wire_safe() -> Result<()> {
    let request = Request::SetRoute {
        id: 8,
        selection: arterm_provider_core::RouteSelection {
            model: "gpt-5.5".to_string(),
            runtime_key: arterm_provider_core::RuntimeKey::ArtermSubscription,
            api_method: "arterm-subscription".to_string(),
            provider_label: "Arterm Subscription".to_string(),
    ...
    let line = serde_json::to_string(&request)?;
    assert!(line.contains("\"type\":\"set_route\""));
    assert!(line.contains("\"kind\":\"arterm-subscription\""));

    let decoded = decode_request(&line)?;
    let Request::SetRoute { id, selection } = decoded else {
        return Err(anyhow!("expected Arterm SetRoute, got {decoded:?}"));
    };
    assert_eq!(id, 8);
    assert_eq!(selection.model, "gpt-5.5");
    assert_eq!(
