...
//! session holds, so "the wide one in the arterm row" is a thing you can point
//! at and remember.
//!
...
mod tests {
    ...
    fn field() -> Field {
        layout(
            &[
                entry("a1", "/home/j/arterm", 400.0),
                entry("a2", "/home/j/arterm", 40.0),
                entry("a3", "/home/j/arterm", 4.0),
    ...
    fn rows_are_by_working_dir_and_stable() {
        ...
        assert_eq!(field.rows[0].label, "arterm");
    ...
    fn left_and_right_stop_at_the_rows_ends() {
        ...
        for id in ["a1", "a2"] {
            ...
            assert_eq!(right.label, "arterm", "{id} wrapped into another row");
        }
    }
    ...
    fn up_and_down_change_rows() {
        ...
        assert_eq!(field.neighbor("b1", Dir::Up).unwrap().label, "arterm");
    ...
    fn dead_axes_are_reported_but_edges_are_not() {
        let field = field();
        // Two rows and three cards in "arterm": nothing is dead here, even at
        // the edges.
