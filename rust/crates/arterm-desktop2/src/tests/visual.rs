...
//! are `#[ignore]`d; run with `cargo test -p arterm-desktop2 -- --ignored`.
...
/// Not an assertion: writes frames to `ARTERM_DESKTOP2_DUMP` for eyeballing.
/// Ignored, so it only runs when a human asks for pictures.
#[test]
#[ignore = "writes files for review"]
fn dump_frames_for_review() {
    let Some(dir) = std::env::var_os("ARTERM_DESKTOP2_DUMP") else {
        return;
    };
