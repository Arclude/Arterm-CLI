...
pub fn dispatch(args: &[String]) -> Option<Result<()>> {
    if args.iter().any(|arg| arg == "--version" || arg == "-V") {
        println!("arterm-desktop2 {}", arterm_build_meta::version());
        return Some(Ok(()));
    }
...
fn profile_transitions() -> Result<()> {
    ...
    arterm_sdk::ensure_runtime(&arterm_sdk::LaunchOptions::default(), &|_| {})?;
    let runtime = start.elapsed();

    let start = Instant::now();
    let client = arterm_sdk::ArtermClient::connect(arterm_sdk::ConnectOptions {
        client_name: concat!("arterm-desktop2-profile/", env!("CARGO_PKG_VERSION")).to_string(),
...
fn check_connect() -> Result<()> {
    arterm_sdk::ensure_runtime(&arterm_sdk::LaunchOptions::default(), &|status| {
        println!("[connect] {status}")
    })?;
    let client = arterm_sdk::ArtermClient::connect(arterm_sdk::ConnectOptions {
        client_name: concat!("arterm-desktop2-check/", env!("CARGO_PKG_VERSION")).to_string(),
        ensure_runtime: false,
        ..Default::default()
    })?;
    println!(
...
fn check_primary_selection() -> Result<()> {
    ...
    let sentinel = format!("arterm-primary-check-{}", std::process::id());
    ...
    if std::env::var_os("ARTERM_DESKTOP2_HOLD_SELECTION").is_some() {
...
///
///   arterm-desktop2 --script 'type:alpha beta' ctrl+a shift+right shift+right
///
...
/// its bench flag:
///
///   arterm-desktop2 --script 'sessions:a=arterm,b=arterm,c=site' super+l super+j
...
fn print_keys() {
    println!("keybindings (ported from the arterm TUI)\n");
    ...
    while std::time::Instant::now() < deadline {
        ...
        match update {
            ...
            harness::HarnessUpdate::TurnDone if sent => {
                println!("\n[e2e] turn done");
                let out = std::env::temp_dir().join("arterm-desktop2-e2e.png");
...
fn check_reconnect() -> Result<()> {
    ...
    let runtime = std::env::temp_dir().join(format!("arterm-reconnect-{}", std::process::id()));
    std::fs::create_dir_all(&runtime)?;
    // SAFETY: single-threaded, before any connection thread is spawned.
    unsafe {
        std::env::set_var("ARTERM_API_SOCKET", runtime.join("api.sock"));
    }
    let bridge = |kill: bool| -> Option<u32> {
        let listening = std::process::Command::new("pgrep")
            .args(["-f", "arterm-harness-api-bridge"])
            .output()
            .ok()?;
