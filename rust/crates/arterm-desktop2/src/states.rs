...
fn fixed_meta() -> crate::meta::Meta {
    crate::meta::Meta {
        ...
        account: Some("demo@arterm.dev (anthropic)".into()),
    }
}
...
fn connecting() -> Model {
    Model {
        ...
        meta: fixed_meta(),
        status: "connecting to ~/.arterm/arterm-api.sock...".into(),
        session_id: None,
...
fn attached_empty() -> Model {
    Model {
        ...
        working_dir: Some("/home/j/arterm".into()),
        file_tree: crate::file_tree::FileTree::default(),
        // Pinned off: a live RAM figure would make every capture depend on
        // the machine and moment it ran on.
        mem: None,
...
/// The donut turned off (`ARTERM_DESKTOP2_DONUT=0`): the empty screen must still
...
fn background_progress() -> Model {
    ...
    transcript.set_progress(
        "224715dw29",
        "bash",
        "62% · Running arterm-desktop2 tests",
        Some(62.0),
    );
    Model {
        transcript,
...
fn edit_card() -> Model {
    ...
    transcript.push_edit(&EditCard {
        ...
        files: vec!["crates/arterm-desktop2/src/scroll.rs".into()],
...
fn edit_cards_many() -> Model {
    ...
    for (file, line) in [
        ("crates/arterm-desktop2/src/scroll.rs", 118usize),
        ("crates/arterm-desktop2/src/scene.rs", 402),
        ("crates/arterm-desktop2/src/layout.rs", 77),
    ] {
        transcript.push_edit(&EditCard {
            intent: Some(format!(
                "rename the field in {}",
...
fn streaming() -> Model {
    Model {
        ...
        activity: crate::activity::Activity::pinned(
            ...
            Some("reading crates/arterm-desktop2/src/scene.rs"),
        ),
        ..attached_empty()
    }
}
...
fn markdown_typography() -> Model {
    Model {
        transcript: conversation(vec![(
            ...
            concat!(
                "# Renderer\n\n",
                "Markdown comes from `arterm-render-core`, so the desktop and the TUI ",
