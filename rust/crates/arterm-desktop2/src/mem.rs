...
//!
//! Shows how much RAM this window (the client) and the arterm daemon (the
...
/// Whether a `/proc/<pid>/cmdline` argv is the arterm daemon. The daemon is
/// `arterm serve ...` wherever its binary lives (selfdev target, shared-server
...
pub fn is_daemon_cmdline(args: &[&str]) -> bool {
    ...
    exe == "arterm" && args.get(1).copied() == Some("serve")
}
...
mod tests {
    ...
    fn the_daemon_is_matched_by_name_and_subcommand_only() {
        assert!(is_daemon_cmdline(&[
            "/home/j/.arterm/builds/shared-server/arterm",
            "serve",
            "--socket",
            "/run/user/1000/arterm.sock",
        ]));
        assert!(is_daemon_cmdline(&["arterm", "serve"]));
        // The desktop client, the bridge, and an interactive `arterm` must not
        // count as the server.
        assert!(!is_daemon_cmdline(&["/x/arterm-desktop2"]));
        assert!(!is_daemon_cmdline(&["/x/arterm-harness-api-bridge"]));
        assert!(!is_daemon_cmdline(&["/x/arterm"]));
        assert!(!is_daemon_cmdline(&["/x/arterm", "self-dev"]));
        assert!(!is_daemon_cmdline(&[]));
    }
