...
/// which checkout this window is for is what distinguishes two arterm windows
/// from each other.
pub fn window_title(working_dir: Option<&str>) -> String {
    match working_dir {
        Some(dir) => format!("{} - arterm", display_path(dir, home().as_deref())),
        None => "arterm desktop2".to_string(),
    }
}
...
mod tests {
    ...
    fn home_collapses_to_tilde() {
        assert_eq!(
            display_path("/home/j/arterm/crates/x", Some("/home/j")),
            "~/arterm/crates/x"
        );
    ...
    fn title_names_the_directory_first() {
        ...
        assert!(title.ends_with("arterm"), "{title}");
        assert_eq!(window_title(None), "arterm desktop2");
    }
}
