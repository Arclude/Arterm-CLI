//! Build and runtime version metadata for arterm.
//!
...
//! `ARTERM_RUNTIME_RELEASE_SEMVER`; the accessor functions below then present the
...
pub fn runtime_release_semver() -> Option<&'static str> {
    RUNTIME_RELEASE_SEMVER
        .get_or_init(|| {
            std::env::var("ARTERM_RUNTIME_RELEASE_SEMVER")
                .ok()
...
pub fn git_hash() -> &'static str {
    RUNTIME_GIT_HASH
        .get_or_init(|| runtime_identity_value("ARTERM_RUNTIME_RELEASE_GIT_HASH"))
        .as_deref()
        .unwrap_or(GIT_HASH)
}
...
pub fn git_date() -> &'static str {
    RUNTIME_GIT_DATE
        .get_or_init(|| runtime_identity_value("ARTERM_RUNTIME_RELEASE_GIT_DATE"))
        .as_deref()
        .unwrap_or(GIT_DATE)
}
...
pub fn git_tag() -> &'static str {
    RUNTIME_GIT_TAG
        .get_or_init(|| runtime_identity_value("ARTERM_RUNTIME_RELEASE_GIT_TAG"))
        .as_deref()
        .unwrap_or(GIT_TAG)
}
...
pub fn is_release_build() -> bool {
    option_env!("ARTERM_RELEASE_BUILD").is_some() || runtime_release_semver().is_some()
}
