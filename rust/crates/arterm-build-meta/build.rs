...
// Build metadata generator for the arterm workspace.
//
// This is the single source of truth for the ARTERM_* compile-time values that
// previously lived in the root `arterm` crate's build.rs. It is hosted in the
// leaf `arterm-build-meta` crate so every workspace crate can read identical
// values (via `arterm_build_meta::*`) without duplicating this script.
//
// NOTE: because this crate's own package version is unrelated to arterm's, we
...
fn main() {
    ...
    let dirty = match std::env::var("ARTERM_BUILD_GIT_DIRTY") {
        Ok(value) => matches!(
    ...
    let raw_log = std::env::var("ARTERM_BUILD_CHANGELOG_RAW")
        .ok()
    ...
    let is_release = std::env::var("ARTERM_RELEASE_BUILD").is_ok();
    ...
    println!("cargo:rustc-env=ARTERM_GIT_HASH={}", git_hash);
    println!("cargo:rustc-env=ARTERM_GIT_DATE={}", git_date);
    println!("cargo:rustc-env=ARTERM_VERSION={}", version);
    println!("cargo:rustc-env=ARTERM_SEMVER={}", build_semver);
    println!("cargo:rustc-env=ARTERM_BASE_SEMVER={}", base_semver);
    println!("cargo:rustc-env=ARTERM_UPDATE_SEMVER={}", update_semver);
    println!("cargo:rustc-env=ARTERM_GIT_TAG={}", git_tag);
    println!("cargo:rustc-env=ARTERM_CHANGELOG={}", changelog);
    println!("cargo:rustc-env=ARTERM_PKG_VERSION={}", pkg_version);

    // Forward ARTERM_RELEASE_BUILD env var if set (CI sets this for release binaries)
    if std::env::var("ARTERM_RELEASE_BUILD").is_ok() {
        println!("cargo:rustc-env=ARTERM_RELEASE_BUILD=1");
    }
    ...
    // Since `arterm-build-meta` sits at the bottom of the crate graph
    ...
    //   * Release/dist builds set ARTERM_RELEASE_BUILD=1 and ARTERM_BUILD_SEMVER,
    ...
    println!("cargo:rerun-if-env-changed=ARTERM_RELEASE_BUILD");
    println!("cargo:rerun-if-env-changed=ARTERM_BUILD_SEMVER");
    // Allow callers to force a metadata refresh (e.g. install scripts) without a
    // full clean, by bumping this env var.
    println!("cargo:rerun-if-env-changed=ARTERM_BUILD_GIT_HASH");
    println!("cargo:rerun-if-env-changed=ARTERM_BUILD_GIT_DATE");
    println!("cargo:rerun-if-env-changed=ARTERM_BUILD_GIT_DIRTY");
    println!("cargo:rerun-if-env-changed=ARTERM_BUILD_GIT_TAG");
}

/// Workspace root, derived from this crate's manifest dir (`crates/arterm-build-meta`).
fn repo_root() -> PathBuf {
    let manifest_dir = std::env::var("CARGO_MANIFEST_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from("."));
    // crates/arterm-build-meta -> crates -> <repo root>
    manifest_dir
        .parent()
        .and_then(|p| p.parent())
...
fn explicit_build_semver_override() -> Option<String> {
    std::env::var("ARTERM_BUILD_SEMVER")
        .ok()
...
fn resolve_build_semver(base_version: (u32, u32, u32)) -> Result<String, String> {
    ...
    // function of HEAD, so the emitted ARTERM_SEMVER/ARTERM_VERSION only change when
    ...
    // turn invalidated `arterm-build-meta` and force-recompiled the entire crate
...
fn metadata_value(key: &str) -> Option<String> {
    let path = std::env::var("ARTERM_BUILD_METADATA_FILE").ok()?;
