//! Stress probe: build the mermaid source that `arterm_plan::mermaid::
...
//!
//!   scripts/dev_cargo.sh run -p arterm-tui-mermaid --features renderer \
//!       --example swarm_plan_stress
//!
//! Cases:
//!   (a) real deep-mode plan fixture (examples/swarm_plan_fixture.json,
//!       snapshot of ~/.arterm/state/swarm/_home_jeremy_arterm__git.json)
...
use arterm_plan::PlanItem;
use arterm_plan::mermaid::swarm_plan_mermaid;
...
fn probe(name: &str, src: &str) -> bool {
    match arterm_tui_mermaid::render_mermaid_untracked(src, Some(100)) {
        ...
        arterm_tui_mermaid::RenderResult::Error(err) => {
