...
/// Terminal multiplexers / agent-multiplexers that sit between arterm and the
...
/// Detect whether arterm is running inside a known multiplexer, using the same
...
///
/// * `probe_override` is the parsed value of `ARTERM_MERMAID_PICKER_PROBE`
...
/// Parse only the explicit `ARTERM_MERMAID_PICKER_PROBE` override into a mode,
...
pub(super) fn iterm2_images_opt_in() -> bool {
    std::env::var("ARTERM_ITERM2_IMAGES")
        .ok()
        .as_deref()
...
pub(crate) fn prewarm_svg_font_db_async() {
    SVG_FONT_DB_PREWARM_STARTED.get_or_init(|| {
        let _ = std::thread::Builder::new()
            .name("arterm-mermaid-fontdb-prewarm".to_string())
            .spawn(|| {
                let _ = &*SVG_FONT_DB;
            });
    });
...
/// By default arterm uses environment-based detection and never blocks startup
/// on terminal capability responses. Set ARTERM_MERMAID_PICKER_PROBE=1 to run an
...
pub fn init_picker() {
    PICKER.get_or_init(|| {
        ...
        let probe_override = std::env::var("ARTERM_MERMAID_PICKER_PROBE")
            .ok()
            .as_deref()
            .and_then(parse_env_bool);
...
mod tests {
    ...
    fn rediscovery_fails_when_the_external_image_file_is_gone() {
        let path = std::env::temp_dir().join(format!(
            "arterm-missing-external-image-{}-{:?}.png",
            std::process::id(),
            std::thread::current().id()
        ));
