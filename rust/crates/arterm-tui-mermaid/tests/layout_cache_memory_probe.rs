...
//! `cargo test -p arterm-tui-mermaid --test layout_cache_memory_probe -- --ignored --nocapture`
...
fn layout_cache_stays_bounded_under_50_distinct_renders() {
    ...
    arterm_tui_mermaid::reset_debug_stats();
    let stats_before = arterm_tui_mermaid::debug_stats();
    ...
    for idx in 0..RENDERS {
        ...
        match arterm_tui_mermaid::render_mermaid_untracked(&content, Some(90)) {
            arterm_tui_mermaid::RenderResult::Image { .. } => {}
            arterm_tui_mermaid::RenderResult::Error(error) => {
                render_errors += 1;
    ...
    let stats = arterm_tui_mermaid::debug_stats();
    let memory = arterm_tui_mermaid::debug_memory_profile();
