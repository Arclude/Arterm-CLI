...
fn the_issue_604_command_is_blocked_outright() {
    // The reported incident: "arterm just deleted everything in my ~".
    for command in [
        "rm -rf ~",
        "rm -rf $HOME",
        "rm -rf \"$HOME\"",
...
fn read_only_find_with_stderr_redirect_is_safe() {
    let command = "find ~/.arterm -maxdepth 3 -type f \\
...
fn harmless_command_operands_are_not_redirect_targets() {
    for command in [
        "grep needle /home/u/.arterm/config 2>/dev/null",
