...
//!
//! This powers `arterm provider-doctor`: it walks the same strict provider/model
...
//! - [`DoctorTier::Offline`]: no API key, no network, no spend. Validates arterm's
...
use arterm_base::protocol::{AuthChanged, CatalogNamespace, RuntimeProviderKey};
use arterm_base::provider::ModelRoute;
use arterm_base::provider_catalog::OpenAiCompatibleProfile;

/// How much of the strict pipeline to exercise.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum DoctorTier {
    /// No key, no network, no spend. Validates arterm-side wiring only.
    Offline,
...
fn tool_stage_detail(stage: &arterm_base::live_tests::LiveVerificationStage) -> String {
...
/// doctor report.
fn reasoning_stage_detail(stage: &arterm_base::live_tests::LiveVerificationStage) -> String {
    match stage
        .evidence
...
///
/// The predicate itself lives in `arterm_base::auth::doctor` so base-internal
...
pub use arterm_base::auth::doctor::native_doctor_supports_provider;
...
pub async fn run_claude_native_e2e(
    ...
    use arterm_base::provider::Provider;
    use arterm_provider_anthropic_runtime::AnthropicProvider;

    let normalized = arterm_base::auth::lifecycle::normalized_auth_provider_id(Some(provider_id))
        .unwrap_or("claude");
    let provider_label = arterm_base::auth::lifecycle::provider_display_label(Some(normalized))
    ...
    // `ARTERM_RUNTIME_PROVIDER=claude-api` would silently test the API-key path
        ...
        checks.push(DoctorCheck::failed(
            ...
            format!(
                "could not select the Claude OAuth credential path: {error}. \
                 Run `arterm login --provider claude` to mint a fresh OAuth token."
            ),
        ));
        return Ok(finish_report(
            provider_id,
    ...
    let catalog_models: Vec<String> = if tier.requires_api_key() {
        match provider_runtime.fetch_live_model_ids_for_doctor().await {
            ...
            Ok(_) => {
                ...
                let fallback = arterm_base::provider::known_anthropic_model_ids();
                checks.push(DoctorCheck::passed(
                    checkpoints::MODEL_CATALOG_LIVE_ENDPOINT,
                    label_for(checkpoints::MODEL_CATALOG_LIVE_ENDPOINT),
                    format!(
...
/// Google OAuth tokens minted by `arterm login --provider antigravity`; the
...
/// accepts every schema construct arterm emits), then any Gemini model, then any
...
pub async fn run_antigravity_native_e2e(
    ...
    use arterm_base::provider::Provider;
    use arterm_provider_antigravity_runtime::AntigravityProvider;
    ...
    let _ = arterm_base::auth::lifecycle::normalized_auth_provider_id(Some(provider_id));
    let provider_label = arterm_base::auth::lifecycle::provider_display_label(Some("antigravity"))
                ...
                checks.push(DoctorCheck::failed(
                    ...
                    format!(
                        "could not resolve an Antigravity credential: {error}. \
                         Run `arterm login --provider antigravity` to sign in."
                    ),
                ));
                return Ok(finish_report(
                    provider_id,
...
impl NativeProviderKind {
    ...
    fn spec(self) -> NativeProviderSpec {
        match self {
            Self::OpenAi => NativeProviderSpec {
                ...
                auth_env_key: None,
                login_hint: "arterm login --provider openai",
            },
            Self::Gemini => NativeProviderSpec {
                ...
                auth_env_key: None,
                login_hint: "arterm login --provider gemini",
            },
            Self::Cursor => NativeProviderSpec {
                ...
                login_hint: "arterm login --provider cursor",
            },
            Self::Copilot => NativeProviderSpec {
                ...
                auth_env_key: None,
                login_hint: "arterm login --provider copilot",
            },
            Self::Bedrock => NativeProviderSpec {
                ...
                login_hint: "arterm login --provider bedrock",
            },
            Self::Arterm => NativeProviderSpec {
                provider_id: "arterm",
                label: "Arterm Subscription",
                // The transport is OpenAI-compatible internally, but the public
                // route identity is the managed Arterm subscription. Model
                // switches use a bare model id so they stay on that runtime.
                contract: WiringContract {
                    api_method: arterm_base::subscription_catalog::ARTERM_ROUTE_API_METHOD
                        .to_string(),
                    route_provider: arterm_base::subscription_catalog::ARTERM_PROVIDER_DISPLAY_NAME
                        .to_string(),
                    expected_runtime: "arterm",
                    expected_namespace: None,
                    switch_prefix: String::new(),
                },
                auth_source: "Arterm subscription API key (ARTERM_API_KEY)",
                auth_env_key: Some("ARTERM_API_KEY"),
                login_hint: "arterm login --provider arterm",
            },
            Self::Azure => NativeProviderSpec {
                ...
                login_hint: "arterm login --provider azure",
            },
        }
    }
    ...
    fn build_runtime(self) -> anyhow::Result<std::sync::Arc<dyn arterm_base::provider::Provider>> {
        use anyhow::Context as _;
        use arterm_base::provider::Provider;
        let runtime: std::sync::Arc<dyn Provider> = match self {
            Self::OpenAi => {
                let credentials =
                    arterm_base::auth::codex::load_credentials().unwrap_or_else(|_| {
                        arterm_base::auth::codex::CodexCredentials {
                ...
                std::sync::Arc::new(arterm_provider_openai_runtime::OpenAIProvider::new(
                    credentials,
                ))
            }
            Self::Gemini => {
                std::sync::Arc::new(arterm_provider_gemini_runtime::GeminiProvider::new())
            }
            Self::Cursor => {
                std::sync::Arc::new(arterm_provider_cursor_runtime::CursorCliProvider::new())
            }
            Self::Copilot => {
                ...
                arterm_base::env::set_var("ARTERM_COPILOT_PREFETCH_STARTUP_GRACE_MS", "0");
                let runtime = match arterm_provider_copilot_runtime::CopilotApiProvider::new() {
                    Ok(runtime) => runtime,
                    Err(_) => arterm_provider_copilot_runtime::CopilotApiProvider::new_with_token(
                        String::new(),
                    ),
                };
            ...
            Self::Azure => {
                ...
                arterm_base::auth::azure::apply_runtime_env()
                    .context("apply Azure OpenAI runtime env")?;
                let runtime = arterm_provider_openrouter_runtime::OpenRouterProvider::new()
                ...
                if let Some(model) = arterm_base::auth::azure::load_model() {
    ...
    async fn resolve_credential(self) -> anyhow::Result<String> {
        ...
        match self {
            Self::OpenAi => {
                let credentials = arterm_base::auth::codex::load_credentials()
                    .context("load OpenAI credentials (run `arterm login --provider openai`)")?;
            ...
            Self::Gemini => {
                let tokens = arterm_base::auth::gemini::load_or_refresh_tokens()
                    .await
            ...
            Self::Cursor => {
                let key = arterm_base::auth::cursor::load_api_key()
                    .context("load Cursor credential (run `arterm login --provider cursor`)")?;
            ...
            Self::Copilot => {
                let token = arterm_base::auth::copilot::load_github_token()
                    .context("load GitHub Copilot token (run `arterm login --provider copilot`)")?;
            ...
            Self::Bedrock => {
                if !arterm_base::provider::bedrock::BedrockProvider::has_credentials() {
                    anyhow::bail!(
            ...
            Self::Arterm => {
                if !arterm_base::subscription_catalog::has_credentials() {
                    anyhow::bail!(
                        "no Arterm subscription credential found (set ARTERM_API_KEY or run \
                         `arterm login --provider arterm`)"
                    );
                }
                Ok("Arterm subscription credential resolved".to_string())
            }
            Self::Azure => {
                if !arterm_base::auth::azure::has_configuration() {
                    anyhow::bail!(
                        "Azure OpenAI is not fully configured (need AZURE_OPENAI_ENDPOINT plus an \
                         API key or Entra ID); run `arterm login --provider azure`"
                    );
                }
                Ok(format!(
                    "Azure OpenAI configured ({})",
                    arterm_base::auth::azure::method_detail()
                ))
            }
        }
    }
    ...
    fn cheapest_model(self, catalog: &[String]) -> Option<String> {
        ...
        let cheap_markers: &[&str] = match self {
            ...
            Self::Arterm => &["mini", "flash", "haiku", "lite", "nano"],
...
struct NativeProviderSpec {
    ...
    label: &'static str,
    /// arterm-side routing/activation contract for the wiring checkpoints.
    contract: WiringContract,
    ...
    /// `arterm login` hint surfaced when the credential cannot be resolved.
...
async fn run_generic_native_api_checks(
    provider: &dyn arterm_base::provider::Provider,
    selected: &str,
    label: &str,
    checks: &mut Vec<DoctorCheck>,
    spend: &mut DoctorSpend,
...
/// The arterm-side wiring a given compat profile is expected to activate.
///
...
/// and arterm remaps them to their native runtimes. The doctor must assert the
...
fn wiring_contract(profile: OpenAiCompatibleProfile) -> WiringContract {
    match arterm_base::auth::lifecycle::normalized_auth_provider_id(Some(profile.id)) {
...
fn run_wiring_checks_for_contract(
    ...
    let auth = AuthChanged {
        provider: arterm_base::protocol::AuthProviderId::new(auth_provider_id),
...
mod tests {
    ...
    fn native_provider_kind_maps_every_generic_id() {
        for (id, expected) in [
            ...
            ("arterm", NativeProviderKind::Arterm),
    ...
    fn native_provider_specs_are_self_consistent() {
        ...
        for kind in [
            ...
            if kind == NativeProviderKind::Arterm {
                assert!(
                    spec.contract.switch_prefix.is_empty(),
                    "Arterm switches must use bare managed model ids"
                );
            } else {
                assert!(
    ...
    /// `native_doctor_supports_provider` lives in `arterm_base::auth::doctor`
    ...
    #[test]
    fn native_arterm_contract_uses_managed_subscription_identity() {
        let contract = NativeProviderKind::Arterm.spec().contract;
        assert_eq!(
            contract.api_method,
            arterm_base::subscription_catalog::ARTERM_ROUTE_API_METHOD
        );
        assert_eq!(
            contract.route_provider,
            arterm_base::subscription_catalog::ARTERM_PROVIDER_DISPLAY_NAME
        );
        assert_eq!(contract.expected_runtime, "arterm");
        assert!(contract.expected_namespace.is_none());
