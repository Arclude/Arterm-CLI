...
use arterm_base::message::{ContentBlock, Message, Role, StreamEvent, ToolDefinition};
use arterm_base::provider::Provider;
use arterm_base::provider_catalog::{OpenAiCompatibleProfile, ResolvedOpenAiCompatibleProfile};
...
/// raise it via `ARTERM_LIVE_SMOKE_TIMEOUT_SECS` for slow reasoning models (e.g.
...
fn smoke_timeout(default_secs: u64) -> std::time::Duration {
    let secs = std::env::var("ARTERM_LIVE_SMOKE_TIMEOUT_SECS")
        .ok()
...
pub async fn fetch_live_openai_compatible_models(
    ...
    let request = arterm_base::provider::shared_http_client().get(&url);
...
/// bare id arterm uses for routing and coverage keys.
///
...
pub async fn run_live_openai_compatible_smoke(
    ...
    api_key: &str,
    model: &str,
) -> anyhow::Result<arterm_base::live_tests::LiveVerificationStage> {
    ...
    let request = arterm_base::provider::shared_http_client()
        .post(&url)
        .json(&body);
    let request = apply_provider_auth(request, &resolved, api_key);
    ...
    let mut stage = arterm_base::live_tests::LiveVerificationStage::passed(
...
mod tests {
    use super::*;
    use arterm_base::provider_catalog::resolve_openai_compatible_profile;
    use arterm_provider_metadata::{
...
pub async fn run_live_openai_compatible_tool_smoke(
    ...
    let tool_calls = parsed
        ...
        resolved.display_name,
        arterm_base::util::truncate_str(text.trim(), 1200)
    );
    ...
    let parsed_arguments = arterm_base::message::ToolCall::parse_streamed_input_to_object(arguments);
    ensure!(
...
pub async fn run_live_claude_native_smoke(
    ...
    .await?;
        ...
        arterm_base::util::truncate_str(outcome.text.trim(), 200),
        outcome.diagnostics()
    );
...
/// `tool_execution_loop`, `tool_result_followup`, and `real_arterm_tool_smoke`
...
pub async fn run_live_claude_native_tool_smoke(
    ...
    .await?;
        ...
        first.stop_reason,
        arterm_base::util::truncate_str(first.text.trim(), 200)
    );
    ...
    let parsed_arguments = arterm_base::message::ToolCall::parse_streamed_input_to_object(
    ...
    .await?;
        ...
        arterm_base::util::truncate_str(second.text.trim(), 200)
    );
...
/// arterm re-emits as text deltas, so we assert the runtime produced streamed
...
/// `real_arterm_tool_smoke` checkpoints.
...
///
/// Some native runtimes deliver a single coalesced response that arterm re-emits
...
pub async fn run_live_native_provider_reasoning_smoke(
    ...
    let mut stage = arterm_base::live_tests::LiveVerificationStage::passed(
        arterm_base::live_tests::checkpoints::REASONING_CAPABILITY,
    )
    .with_duration_ms(started.elapsed().as_millis() as u64)
    .with_evidence("model", serde_json::json!(model))
...
fn parse_tool_arguments(input_json: &str) -> serde_json::Value {
    arterm_base::message::ToolCall::parse_streamed_input_to_object(if input_json.trim().is_empty() {
        "{}"
    } else {
        input_json.trim()
    })
