...
fn first_three_launches_can_include_hotkey_notice_too() {
    ...
    // The notice should make clear the hotkey works globally, not just inside arterm.
...
fn default_resolved_hotkeys_match_legacy_three() {
    ...
    let resolved = launch_hotkeys::resolve_launch_hotkeys(
        &arterm_config_types::LaunchHotkeysConfig::default(),
        "/usr/local/bin/arterm",
        "/home/u/.arterm/hotkey/last_dir",
        "/home/u/.arterm/hotkey/last_repo",
    );
    ...
    let home = launch_hotkeys::shell_command_for(&resolved[0], "/usr/local/bin/arterm");
    assert!(home.starts_with("cd \"$HOME\"; "));
    assert!(!home.contains("self-dev"));

    let last_dir = launch_hotkeys::shell_command_for(&resolved[1], "/usr/local/bin/arterm");
    assert!(last_dir.contains("cat '/home/u/.arterm/hotkey/last_dir'"));
    assert!(last_dir.contains("cd \"$HOME\""));

    let selfdev = launch_hotkeys::shell_command_for(&resolved[2], "/usr/local/bin/arterm");
    assert!(selfdev.contains("cat '/home/u/.arterm/hotkey/last_repo'"));
    assert!(selfdev.contains("'/usr/local/bin/arterm' 'self-dev';"));
}
...
fn baked_repo_hotkey_cds_into_fixed_dir() {
    ...
    let config = arterm_config_types::LaunchHotkeysConfig {
        enabled: Some(true),
        imported: true,
        entries: vec![arterm_config_types::LaunchHotkeyEntry {
            chord: "cmd+[".to_string(),
            dir: "/Users/jeremy/arterm-github".to_string(),
            label: "arterm-github".to_string(),
            self_dev: false,
        }],
    };
    let resolved = launch_hotkeys::resolve_launch_hotkeys(
    ...
    let cmd = launch_hotkeys::shell_command_for(&resolved[0], "/usr/local/bin/arterm");
    assert!(cmd.contains("/Users/jeremy/arterm-github"));
...
fn mac_hotkey_launch_agent_plist_uses_valid_xml_quotes() {
    let plist = mac_hotkey_launch_agent_plist(
        "/Applications/Arterm.app/Contents/MacOS/arterm",
        "/tmp/arterm-hotkey.out.log",
        "/tmp/arterm-hotkey.err.log",
        "ghostty",
    );
...
#[test]
fn paused_arterm_shell_command_keeps_failures_visible() {
    let command = paused_arterm_shell_command("/tmp/arterm");
    assert!(command.contains("Press Enter to close"));
    assert!(command.contains("Arterm exited with status"));
    assert!(command.contains("arterm executable not found"));
}
...
fn macos_terminal_notice_only_fires_for_default_terminal_app() {
    ...
    assert_eq!(
        ...
        Some("Tip: Terminal.app renders arterm poorly. Try Ghostty, iTerm2, or Alacritty.")
    );
    let (title, message) = hints.display_message.expect("expected display message");
    assert_eq!(title, "Terminal");
    assert!(message.contains("Terminal.app renders arterm poorly"));
...
fn keymap_conflict_hint_full_path_debounces_and_persists_signature() {
    ...
    use arterm_config_types::KeybindingsConfig;
...
fn glyph_safe_notice_shows_once_then_debounces() {
    ...
    assert!(body.contains("ARTERM_GLYPH_SAFE_MODE=off"));
