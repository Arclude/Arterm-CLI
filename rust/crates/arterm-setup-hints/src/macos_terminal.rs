...
fn mac_terminal_pref_path() -> Result<PathBuf> {
    Ok(storage::arterm_dir()?.join("preferred_terminal.json"))
}
...
pub(super) fn effective_macos_terminal() -> MacTerminalKind {
    ...
    // > legacy `~/.arterm/preferred_terminal.json` > runtime detection.
    config_preferred_macos_terminal()
        .or_else(load_preferred_macos_terminal)
        .unwrap_or_else(detect_macos_terminal)
}

/// Read `[terminal] preferred` from `~/.arterm/config.toml`, if present and valid.
fn config_preferred_macos_terminal() -> Option<MacTerminalKind> {
    #[derive(Deserialize, Default)]
    struct Wrapper {
        #[serde(default)]
        terminal: arterm_config_types::TerminalConfig,
    }
    let dir = storage::arterm_dir().ok()?;
...
pub(super) fn paused_arterm_shell_command(exe_path: &str) -> String {
    paused_arterm_shell_command_with_args(exe_path, &[])
}

/// Like [`paused_arterm_shell_command`] but passes extra CLI args (each
/// single-quoted) to the arterm invocation, e.g. `--resume <session-id>`.
pub(super) fn paused_arterm_shell_command_with_args(exe_path: &str, args: &[String]) -> String {
...
mod tests {
    ...
    fn paused_shell_command_quotes_extra_args() {
        let cmd = super::paused_arterm_shell_command_with_args(
            "/usr/local/bin/arterm",
            &["--resume".to_string(), "session_fox_123_abc".to_string()],
        );
        assert!(cmd.contains("'/usr/local/bin/arterm' '--resume' 'session_fox_123_abc';"));
        ...
        assert_eq!(
            super::paused_arterm_shell_command_with_args("/usr/local/bin/arterm", &[]),
            super::paused_arterm_shell_command("/usr/local/bin/arterm"),
        );
    }
    ...
    fn applescript_launchers_run_under_bash_for_non_bash_login_shells() {
        ...
        let shell_command = super::paused_arterm_shell_command("/usr/local/bin/arterm");
