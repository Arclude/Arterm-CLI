//! Native SessionStart integrations for reminding users about Arterm's global
...
//! intercepting shell commands. The hook invokes a hidden, fast Arterm command;
...
pub(super) fn maybe_notify(source: &str) -> Result<()> {
    ...
    *shown = shown.saturating_add(1);
    ...
    let body = format!(
        "{} is open. Press {} anytime to launch Arterm.",
        source.label(),
        display
    );
    send_desktop_notification("Arterm shortcut", &body);
    Ok(())
}
...
fn install_hook(path: &Path, source: CliSource) -> Result<()> {
    ...
    bytes.push(b'\n');
    arterm_storage::write_bytes(path, &bytes)
...
fn hook_command(source: CliSource) -> Result<String> {
    let executable = trusted_arterm_executable()?;
    Ok(format!(
        "{} {HOOK_COMMAND_MARKER}{}",
...
/// bare `arterm` lookup in the external CLI's project-scoped PATH. Falling back
...
fn trusted_arterm_executable() -> Result<PathBuf> {
    #[cfg(windows)]
    {
    ...
    #[cfg(not(windows))]
    {
        if let Some(home) = dirs::home_dir() {
            let launcher = home.join(".local").join("bin").join("arterm");
            if launcher.is_file() {
                return Ok(launcher);
            }
        }
    ...
    std::env::current_exe().context("resolving the Arterm executable for lifecycle hooks")
}
    ...
    #[cfg(windows)]
    {
        ...
        let script = format!(
            "$title=\"{}\";$body=\"{}\";\
             ...
             [Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier('Arterm').Show([Windows.UI.Notifications.ToastNotification]::new($xml))",
            ps_quote(title),
            ps_quote(body)
        );
...
mod tests {
    ...
    fn inserts_session_start_hook_without_replacing_existing_hooks() {
        ...
        let command = "'/trusted/arterm' setup-hotkey --notify-cli-launch claude";
    ...
    fn managed_hook_update_is_idempotent() {
        let mut value = json!({});
        let command = "'/trusted/arterm' setup-hotkey --notify-cli-launch codex";
        assert!(upsert_hook(&mut value, command).unwrap());
        assert!(!upsert_hook(&mut value, command).unwrap());
            ...
            "hooks": {
                "SessionStart": [{
                    ...
                    "hooks": [
                        {
                            "type": "command",
                            "command": "arterm setup-hotkey --notify-cli-launch old",
                            "timeout": 30
                        },
    ...
    fn rejects_malformed_hook_shape_instead_of_clobbering_it() {
        ...
        assert!(
            upsert_hook(
                &mut value,
                "'/trusted/arterm' setup-hotkey --notify-cli-launch claude"
            )
            .is_err()
        );
    }
...
"#,
        ...
        assert!(!command.starts_with("arterm "));
    ...
    fn unix_hook_executable_quoting_handles_spaces_and_single_quotes() {
        assert_eq!(
            quote_hook_executable(Path::new("/tmp/Arterm's bin/arterm")),
            "'/tmp/Arterm'\\''s bin/arterm'"
        );
    }
}
