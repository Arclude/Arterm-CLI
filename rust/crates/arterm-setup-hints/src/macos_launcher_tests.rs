...
fn macos_launcher_script_shows_alerts_and_uses_terminal_launcher() {
    let script = macos_launcher_script(
        ...
        "/tmp/arterm",
        Path::new("/Users/test/Applications/Arterm.app"),
    );
    assert!(script.contains("display alert \"Arterm launch failed\""));
    assert!(script.contains("arterm setup-launcher"));
...
fn macos_launcher_refreshes_when_new_bundle_missing() {
    ...
    let app_dir = temp.path().join("Arterm.app");
    let legacy_app_dir = temp.path().join("arterm.app");
...
fn macos_notification_bundle_is_faceless_and_uses_multicall_binary() {
    ...
    assert!(plist.contains("<string>com.arterm.notifications</string>"));
    assert!(plist.contains("<string>arterm-notification-broker</string>"));
    assert!(plist.contains(arterm_build_meta::version()));
}
