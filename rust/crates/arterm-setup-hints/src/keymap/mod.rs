...
//! (macOS system shortcuts + terminal emulator bindings) so arterm can detect
//! when one of them intercepts a key arterm wants to use.
//!
...
//!
//! Conflict detection against arterm's own bindings is built on top of this in a
//! later layer.
...
pub fn snapshot_path() -> anyhow::Result<std::path::PathBuf> {
    Ok(arterm_storage::arterm_dir()?.join("keymap-snapshot.json"))
}

/// Collect a fresh snapshot and persist it to `~/.arterm/keymap-snapshot.json`.
...
pub fn refresh_and_save() -> KeymapSnapshot {
    ...
    if let Ok(path) = snapshot_path()
        ...
        && let Err(err) = arterm_storage::write_json_fast(&path, &snapshot)
    {
        arterm_logging::warn(&format!("keymap snapshot write failed: {err}"));
    }
    snapshot
}
