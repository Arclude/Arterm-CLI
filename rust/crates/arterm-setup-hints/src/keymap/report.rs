...
/// counts, and any conflicts with arterm's configured bindings.
pub fn render_report(cfg: &KeybindingsConfig, snapshot: &KeymapSnapshot) -> String {
    ...
    if term_count == 0 && sys_count == 0 && app_count == 0 {
        out.push_str(
            "\nNo machine bindings were discovered. arterm can read Ghostty bindings, macOS\n\
    ...
    if conflicts.is_empty() {
        out.push_str("No conflicts found between your arterm keybindings and the machine.\n");
    } else {
        out.push_str(&format!(
        ...
        out.push_str(
            "These keys may be captured by your terminal, macOS, or another app (window\n\
             manager, launcher) before arterm sees them.\n\
             To fix: rebind the arterm action in ~/.arterm/config.toml under [keybindings],\n\
...
fn render_conflict_block(c: &Conflict) -> String {
    ...
    format!(
        "  ⚠ {key}\n      arterm: {action} ({field} = \"{raw}\")\n      taken by {interceptor}\n",
        key = c.arterm.chord.display(),
        action = c.arterm.action,
        field = c.arterm.field,
        raw = c.arterm.raw,
