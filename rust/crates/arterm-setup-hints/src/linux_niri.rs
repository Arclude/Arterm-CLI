//! Render and install global "launch a new arterm" hotkeys on Linux/niri.
//!
...
//!     Alt+Semicolon hotkey-overlay-title="arterm: home" { spawn "sh" "-c" "..."; }
//!     // <<< arterm launch hotkeys (managed) <<<
//! ```
...
pub(crate) const NIRI_BLOCK_BEGIN: &str = "// >>> arterm launch hotkeys (managed) >>>";
/// Closing sentinel for the managed bind region inside `binds { }`.
pub(crate) const NIRI_BLOCK_END: &str = "// <<< arterm launch hotkeys (managed) <<<";
...
pub(crate) struct NiriHotkey {
    pub chord: KeyChord,
    /// Concrete directory to launch arterm in (already resolved from any
...
/// Map a arterm modifier+key chord onto niri's KDL key syntax.
///
/// niri uses `+`-joined modifiers followed by an XKB key name, e.g.
/// `Alt+Semicolon`, `Super+Shift+Apostrophe`. We translate arterm's `cmd`
...
pub(crate) fn chord_to_niri_bind(chord: &KeyChord) -> Option<String> {
    ...
    // arterm `cmd` == macOS Command == Wayland Super.
    if chord.cmd {
        parts.push("Super");
    }
    if chord.ctrl {
...
/// Translate a canonical arterm key token into the XKB key name niri expects.
...
/// Build the `sh -c` command string a bind runs to open arterm in `dir`.
///
/// We `cd` into the directory (falling back to `$HOME` if it has since been
/// removed), then launch arterm via the user's terminal. The terminal is chosen
/// by `terminal` (e.g. `kitty`); we pass it the arterm executable directly.
fn launch_shell_command(
    ...
    // cd with $HOME fallback, then exec the terminal running arterm.
    format!(
...
pub(crate) fn render_niri_bind_line(
    ...
    let title = if hotkey.self_dev {
        format!("arterm: {} (self-dev)", hotkey.label)
    } else {
        format!("arterm: {}", hotkey.label)
    };
...
mod tests {
    ...
    fn renders_bind_line_with_cd_and_terminal() {
        let line = render_niri_bind_line(
            &hk("alt+;", "/home/jeremy/arterm", "arterm", true),
            "/home/jeremy/.local/bin/arterm",
            "kitty",
            "    ",
        )
        .unwrap();
        ...
        assert!(line.contains("hotkey-overlay-title=\"arterm: arterm (self-dev)\""));
        assert!(line.contains("spawn \"sh\" \"-c\""));
        assert!(line.contains("/home/jeremy/arterm"));
    ...
    fn render_block_wraps_sentinels() {
        ...
        .unwrap();
        assert!(block.starts_with("    // >>> arterm launch hotkeys (managed) >>>"));
        assert!(
            block
                .trim_end()
                .ends_with("// <<< arterm launch hotkeys (managed) <<<")
        );
    ...
    fn spliced_config_is_accepted_by_the_real_niri_parser() {
        ...
        let dir = std::env::temp_dir().join(format!("arterm-niri-719-{}", std::process::id()));
    ...
    /// still be accepted after arterm splices its managed block in, and the block
    ...
    fn fuzz_corpus_stays_valid_after_splicing() {
        ...
        let dir = std::env::temp_dir().join(format!("arterm-niri-fuzz-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn shell_command_cds_and_self_devs() {
        let s = launch_shell_command("/bin/arterm", "kitty", "/home/u/proj", "cmd+shift+'", true);
        assert!(s.contains("cd '/home/u/proj'"));
        assert!(s.contains("exec 'kitty' '/bin/arterm' --spawn-hotkey"));
