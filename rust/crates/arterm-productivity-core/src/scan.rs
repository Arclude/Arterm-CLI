...
//! several GB. We keep a sidecar cache (`~/.arterm/cache/productivity/summaries.json`)
...
fn cache_path() -> Result<PathBuf> {
    let dir = arterm_storage::arterm_dir()?
        .join("cache")
        .join("productivity");
...
fn sessions_dir() -> Result<PathBuf> {
    Ok(arterm_storage::arterm_dir()?.join("sessions"))
}
