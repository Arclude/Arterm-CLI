//! Productivity report generation for arterm.
//!
...
/// PNG to `~/.arterm/generated-images/productivity-<timestamp>.png`.
...
fn save_png(png: &[u8]) -> Result<PathBuf> {
    let dir = arterm_storage::arterm_dir()?.join("generated-images");
