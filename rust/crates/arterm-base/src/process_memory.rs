...
/// Retention trim threshold in bytes, from `ARTERM_HEAP_RETENTION_TRIM_MB`
...
pub fn retention_trim_threshold_bytes() -> u64 {
    parse_retention_trim_threshold(
        std::env::var("ARTERM_HEAP_RETENTION_TRIM_MB")
            .ok()
            .as_deref(),
    )
}
...
fn default_heap_profile_path() -> Result<PathBuf> {
    let base = crate::storage::arterm_dir()?.join("profiles").join("heap");
    let timestamp = chrono::Utc::now().format("%Y%m%dT%H%M%SZ");
    let pid = std::process::id();
    Ok(base.join(format!("arterm-{}-{}.heap", pid, timestamp)))
}
