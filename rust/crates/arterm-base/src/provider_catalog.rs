...
pub use arterm_provider_metadata::*;
use std::collections::{HashMap, HashSet};

pub const OPENAI_COMPAT_LOCAL_ENABLED_ENV: &str = "ARTERM_OPENAI_COMPAT_LOCAL_ENABLED";
...
pub fn resolve_openai_compatible_profile_with_api_key_hint(
    ...
    if let Some(base) = env_override("ARTERM_OPENAI_COMPAT_API_BASE") {
        if let Some(normalized) = normalize_api_base(&base) {
            resolved.api_base = normalized;
        } else {
            eprintln!(
                "Warning: ignoring invalid ARTERM_OPENAI_COMPAT_API_BASE '{}'. Use https://... (or http://localhost).",
                base
            );
        }
    }

    if let Some(key_name) = env_override("ARTERM_OPENAI_COMPAT_API_KEY_NAME") {
        if is_safe_env_key_name(&key_name) {
            resolved.api_key_env = key_name;
        } else {
            eprintln!(
                "Warning: ignoring invalid ARTERM_OPENAI_COMPAT_API_KEY_NAME '{}'.",
                key_name
            );
        }
    }

    if let Some(env_file) = env_override("ARTERM_OPENAI_COMPAT_ENV_FILE") {
        if is_safe_env_file_name(&env_file) {
            resolved.env_file = env_file;
        } else {
            eprintln!(
                "Warning: ignoring invalid ARTERM_OPENAI_COMPAT_ENV_FILE '{}'.",
                env_file
            );
        }
    }

    if let Some(setup_url) = env_override("ARTERM_OPENAI_COMPAT_SETUP_URL") {
        resolved.setup_url = setup_url;
    }

    if let Some(model) = env_override("ARTERM_OPENAI_COMPAT_DEFAULT_MODEL") {
...
pub fn active_openai_compatible_display_name() -> Option<String> {
    if let Ok(profile_name) = std::env::var("ARTERM_NAMED_PROVIDER_PROFILE") {
    ...
    if let Ok(namespace) = std::env::var("ARTERM_OPENROUTER_CACHE_NAMESPACE") {
    ...
    let api_base = std::env::var("ARTERM_OPENROUTER_API_BASE")
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
        .or_else(|| env_override("ARTERM_OPENAI_COMPAT_API_BASE"));
...
pub fn runtime_provider_display_name(provider_name: &str) -> String {
    if provider_name.eq_ignore_ascii_case("openrouter") {
        if let Ok(runtime_provider) = std::env::var("ARTERM_RUNTIME_PROVIDER")
...
pub fn openai_compatible_profile_context_limit(profile_id: &str, model: &str) -> Option<usize> {
    ...
    match profile_id.as_str() {
        ...
        _ => arterm_provider_core::models::open_weight_family_context_limit(&model),
    }
}
...
fn apply_openai_compatible_profile_env_impl(
    ...
    if respect_named_profile_lock && std::env::var_os("ARTERM_PROVIDER_PROFILE_ACTIVE").is_some() {
        return;
    }
    ...
    if let Some(profile) = profile {
        let resolved = resolve_openai_compatible_profile(profile);
        crate::env::set_var("ARTERM_OPENROUTER_API_BASE", &resolved.api_base);
        crate::env::set_var("ARTERM_OPENROUTER_API_KEY_NAME", &resolved.api_key_env);
        crate::env::set_var("ARTERM_OPENROUTER_ENV_FILE", &resolved.env_file);
        crate::env::set_var("ARTERM_OPENROUTER_CACHE_NAMESPACE", &resolved.id);
        ...
        if static_models.is_empty() {
            crate::env::remove_var("ARTERM_OPENROUTER_STATIC_MODELS");
        } else {
            crate::env::set_var("ARTERM_OPENROUTER_STATIC_MODELS", static_models.join("\n"));
        }
        if resolved.requires_api_key {
            crate::env::remove_var("ARTERM_OPENROUTER_ALLOW_NO_AUTH");
            crate::env::set_var("ARTERM_OPENROUTER_TRANSPORT_STATE", "direct-api-key");
        } else {
            crate::env::set_var("ARTERM_OPENROUTER_ALLOW_NO_AUTH", "1");
            crate::env::set_var("ARTERM_OPENROUTER_TRANSPORT_STATE", "direct-no-auth");
        }
    }
}
...
fn inline_key_env_name(profile_name: &str) -> String {
    let suffix = profile_name
        ...
        .collect::<String>();
    format!("ARTERM_PROVIDER_{}_API_KEY", suffix)
}
...
pub fn apply_named_provider_profile_env_from_config(
    ...
    crate::env::remove_var("ARTERM_PROVIDER_PROFILE_ACTIVE");
    crate::env::remove_var("ARTERM_PROVIDER_PROFILE_NAME");
    crate::env::remove_var("ARTERM_NAMED_PROVIDER_PROFILE");
    apply_openai_compatible_profile_env(None);
    crate::env::set_var("ARTERM_OPENROUTER_API_BASE", &api_base);
    crate::env::set_var("ARTERM_OPENROUTER_CACHE_NAMESPACE", profile_name);
    crate::env::set_var("ARTERM_NAMED_PROVIDER_PROFILE", profile_name);
    ...
    if let Some(model) = profile
        ...
        .filter(|v| !v.is_empty())
    {
        crate::env::set_var("ARTERM_OPENROUTER_MODEL", model);
    }
    ...
    match profile.auth {
        ...
        crate::config::NamedProviderAuth::Bearer | crate::config::NamedProviderAuth::Header => {
            ...
            if let Some(key_env) = key_env {
                ...
                crate::env::set_var("ARTERM_OPENROUTER_API_KEY_NAME", &key_env);
            }
                ...
                .filter(|v| !v.is_empty())
            {
                ...
                crate::env::set_var("ARTERM_OPENROUTER_ENV_FILE", env_file);
            }
            ...
            if !requires_key {
                ...
                crate::env::set_var("ARTERM_OPENROUTER_TRANSPORT_STATE", "openrouter-api-key");
            } else {
                crate::env::set_var("ARTERM_OPENROUTER_TRANSPORT_STATE", "direct-api-key");
            }
            ...
            match profile.auth {
                crate::config::NamedProviderAuth::Bearer => {
                    crate::env::set_var("ARTERM_OPENROUTER_AUTH_HEADER", "bearer");
                }
                crate::config::NamedProviderAuth::Header => {
                    crate::env::set_var("ARTERM_OPENROUTER_AUTH_HEADER", "api-key");
                    if let Some(header) = profile
                        ...
                        .filter(|v| !v.is_empty())
                    {
                        crate::env::set_var("ARTERM_OPENROUTER_AUTH_HEADER_NAME", header);
                    }
                }
                crate::config::NamedProviderAuth::None => {}
            }
...
pub fn active_named_provider_profile_credential_source() -> Option<(String, String)> {
    ...
    let _profile_name = std::env::var("ARTERM_NAMED_PROVIDER_PROFILE")
        .ok()
        .map(|v| v.trim().to_string())
        .filter(|v| !v.is_empty())?;

    let key_env = std::env::var("ARTERM_OPENROUTER_API_KEY_NAME")
        .ok()
        .map(|v| v.trim().to_string())
        .filter(|v| !v.is_empty())
        .unwrap_or_else(|| "OPENROUTER_API_KEY".to_string());
    let env_file = std::env::var("ARTERM_OPENROUTER_ENV_FILE")
        .ok()
        .map(|v| v.trim().to_string())
        .filter(|v| !v.is_empty())
...
fn active_named_provider_profile_is_configured() -> Option<bool> {
    ...
    if std::env::var("ARTERM_OPENROUTER_ALLOW_NO_AUTH")
