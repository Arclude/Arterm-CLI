...
fn mermaid_environment_override_uses_standard_boolean_values() {
    ...
    let previous = std::env::var_os("ARTERM_ENABLE_MERMAID");
    crate::env::set_var("ARTERM_ENABLE_MERMAID", "off");

    let mut cfg = Config::default();
    cfg.apply_env_overrides();
    assert!(!cfg.features.mermaid);

    restore_env_var("ARTERM_ENABLE_MERMAID", previous);
}
...
fn auto_poke_environment_override_uses_standard_boolean_values() {
    let _guard = crate::storage::lock_test_env();
    let previous = std::env::var_os("ARTERM_AUTO_POKE");
    crate::env::set_var("ARTERM_AUTO_POKE", "off");

    let mut cfg = Config::default();
    cfg.apply_env_overrides();
    assert!(!cfg.features.auto_poke);

    restore_env_var("ARTERM_AUTO_POKE", previous);
}
...
fn latex_rendering_environment_override_accepts_aliases() {
    let _guard = crate::storage::lock_test_env();
    let previous = std::env::var_os("ARTERM_LATEX_RENDERING");
    crate::env::set_var("ARTERM_LATEX_RENDERING", "png");
    let mut cfg = Config::default();
    cfg.apply_env_overrides();
    assert_eq!(cfg.display.latex_rendering, LatexRenderingMode::Image);
    restore_env_var("ARTERM_LATEX_RENDERING", previous);
}
...
fn test_env_override_swarm_spawn_mode() {
    let _guard = crate::storage::lock_test_env();
    let prev = std::env::var_os("ARTERM_SWARM_SPAWN_MODE");
    crate::env::set_var("ARTERM_SWARM_SPAWN_MODE", "headless");
    ...
    restore_env_var("ARTERM_SWARM_SPAWN_MODE", prev);
}

#[test]
fn test_env_override_swarm_model() {
    let _guard = crate::storage::lock_test_env();
    let prev = std::env::var_os("ARTERM_SWARM_MODEL");
    crate::env::set_var("ARTERM_SWARM_MODEL", "claude-opus-4-6");
...
fn test_env_override_lifecycle_hooks() {
    let _guard = crate::storage::lock_test_env();
    let prev_turn_end = std::env::var_os("ARTERM_HOOK_TURN_END");
    let prev_timeout = std::env::var_os("ARTERM_HOOK_PRE_TOOL_TIMEOUT_MS");

    crate::env::set_var("ARTERM_HOOK_TURN_END", "my-notifier --fast");
    crate::env::set_var("ARTERM_HOOK_PRE_TOOL_TIMEOUT_MS", "250");
    let mut cfg = Config::default();
    cfg.apply_env_overrides();
    assert_eq!(
        cfg.hooks.turn_end.as_ref().and_then(HookCommands::first),
    ...
    crate::env::set_var("ARTERM_HOOK_TURN_END", " ");
    let mut cfg = Config::default();
    ...
    restore_env_var("ARTERM_HOOK_TURN_END", prev_turn_end);
    restore_env_var("ARTERM_HOOK_PRE_TOOL_TIMEOUT_MS", prev_timeout);
}

#[test]
fn test_env_override_spawn_hook() {
    let _guard = crate::storage::lock_test_env();
    let prev = std::env::var_os("ARTERM_SPAWN_HOOK");
    crate::env::set_var("ARTERM_SPAWN_HOOK", "kitty @ launch --type=tab --");
    ...
    // Empty env value disables a config-file hook.
    crate::env::set_var("ARTERM_SPAWN_HOOK", "  ");
    let mut cfg = Config::default();
    cfg.terminal.spawn_hook = Some("tmux new-window".to_string());
    cfg.apply_env_overrides();
    assert_eq!(cfg.terminal.spawn_hook, None);

    restore_env_var("ARTERM_SPAWN_HOOK", prev);
}

#[test]
fn test_env_override_focus_hook() {
    let _guard = crate::storage::lock_test_env();
    let prev = std::env::var_os("ARTERM_FOCUS_HOOK");
    crate::env::set_var("ARTERM_FOCUS_HOOK", "niri-focus-arterm");

    let mut cfg = Config::default();
    cfg.apply_env_overrides();
    assert_eq!(cfg.terminal.focus_hook.as_deref(), Some("niri-focus-arterm"));

    // Empty env value disables a config-file hook.
    crate::env::set_var("ARTERM_FOCUS_HOOK", "");
    let mut cfg = Config::default();
    cfg.terminal.focus_hook = Some("wmctrl -a".to_string());
    cfg.apply_env_overrides();
    assert_eq!(cfg.terminal.focus_hook, None);

    restore_env_var("ARTERM_FOCUS_HOOK", prev);
}
...
fn test_env_override_memory_sidecar() {
    let _guard = crate::storage::lock_test_env();
    let prev_model = std::env::var_os("ARTERM_MEMORY_MODEL");
    let prev_enabled = std::env::var_os("ARTERM_MEMORY_SIDECAR_ENABLED");
    crate::env::set_var("ARTERM_MEMORY_MODEL", "claude-haiku-4");
    crate::env::set_var("ARTERM_MEMORY_SIDECAR_ENABLED", "true");
    ...
    restore_env_var("ARTERM_MEMORY_MODEL", prev_model);
    restore_env_var("ARTERM_MEMORY_SIDECAR_ENABLED", prev_enabled);
}
...
fn test_generated_default_config_uses_low_openai_reasoning_effort() {
    ...
    crate::env::set_var("ARTERM_HOME", dir.path());
    ...
    if let Some(prev) = prev_home {
        crate::env::set_var("ARTERM_HOME", prev);
    } else {
...
fn global_config_cache_reloads_after_manual_file_edit() {
    ...
    restore_env_var("ARTERM_HOME", prev_home);
    Config::invalidate_cache();
}
...
#[test]
fn config_env_fingerprint_ignores_runtime_only_arterm_vars() {
    let _guard = crate::storage::lock_test_env();
    let prev_runtime_provider = std::env::var_os("ARTERM_RUNTIME_PROVIDER");
    let prev_active_provider = std::env::var_os("ARTERM_ACTIVE_PROVIDER");
    let prev_display_centered = std::env::var_os("ARTERM_DISPLAY_CENTERED");

    crate::env::remove_var("ARTERM_RUNTIME_PROVIDER");
    crate::env::remove_var("ARTERM_ACTIVE_PROVIDER");
    crate::env::remove_var("ARTERM_DISPLAY_CENTERED");
    let baseline = config_env_fingerprint();

    crate::env::set_var("ARTERM_RUNTIME_PROVIDER", "openai");
    crate::env::set_var("ARTERM_ACTIVE_PROVIDER", "openai");
    assert_eq!(baseline, config_env_fingerprint());

    crate::env::set_var("ARTERM_DISPLAY_CENTERED", "1");
    assert_ne!(baseline, config_env_fingerprint());

    restore_env_var("ARTERM_RUNTIME_PROVIDER", prev_runtime_provider);
    restore_env_var("ARTERM_ACTIVE_PROVIDER", prev_active_provider);
    restore_env_var("ARTERM_DISPLAY_CENTERED", prev_display_centered);
}
...
fn test_env_override_auto_server_reload() {
    let _guard = crate::storage::lock_test_env();
    let prev = std::env::var_os("ARTERM_AUTO_SERVER_RELOAD");
    crate::env::set_var("ARTERM_AUTO_SERVER_RELOAD", "false");
    ...
    if let Some(prev) = prev {
        crate::env::set_var("ARTERM_AUTO_SERVER_RELOAD", prev);
    } else {
        crate::env::remove_var("ARTERM_AUTO_SERVER_RELOAD");
    }
}
...
fn no_emoji_environment_override_disables_emoji() {
    let _guard = crate::storage::lock_test_env();
    let prev = std::env::var_os("ARTERM_NO_EMOJI");
    crate::env::set_var("ARTERM_NO_EMOJI", "1");
    let mut cfg = Config::default();
    cfg.apply_env_overrides();
    assert!(!cfg.display.emoji);

    crate::env::set_var("ARTERM_NO_EMOJI", "false");
    cfg.display.emoji = false;
    cfg.apply_env_overrides();
    assert!(cfg.display.emoji);

    restore_env_var("ARTERM_NO_EMOJI", prev);
}

#[test]
fn test_env_override_native_scrollbars() {
    let _guard = crate::storage::lock_test_env();
    let prev_chat = std::env::var_os("ARTERM_CHAT_NATIVE_SCROLLBAR");
    let prev_side = std::env::var_os("ARTERM_SIDE_PANEL_NATIVE_SCROLLBAR");
    crate::env::set_var("ARTERM_CHAT_NATIVE_SCROLLBAR", "true");
    crate::env::set_var("ARTERM_SIDE_PANEL_NATIVE_SCROLLBAR", "false");
    ...
    if let Some(prev) = prev_chat {
        crate::env::set_var("ARTERM_CHAT_NATIVE_SCROLLBAR", prev);
    } else {
        crate::env::remove_var("ARTERM_CHAT_NATIVE_SCROLLBAR");
    }
    if let Some(prev) = prev_side {
        crate::env::set_var("ARTERM_SIDE_PANEL_NATIVE_SCROLLBAR", prev);
    } else {
        crate::env::remove_var("ARTERM_SIDE_PANEL_NATIVE_SCROLLBAR");
    }
}
...
fn test_env_override_diff_mode_full_inline() {
    let _guard = crate::storage::lock_test_env();
    let prev = std::env::var_os("ARTERM_DIFF_MODE");
    crate::env::set_var("ARTERM_DIFF_MODE", "full-inline");
    ...
    if let Some(prev) = prev {
        crate::env::set_var("ARTERM_DIFF_MODE", prev);
    } else {
        crate::env::remove_var("ARTERM_DIFF_MODE");
    }
}
...
fn test_env_override_trusted_external_auth_splits_source_and_path_entries() {
    let _guard = crate::storage::lock_test_env();
    let prev = std::env::var_os("ARTERM_TRUSTED_EXTERNAL_AUTH_SOURCES");
    crate::env::set_var(
    ...
    if let Some(prev) = prev {
        crate::env::set_var("ARTERM_TRUSTED_EXTERNAL_AUTH_SOURCES", prev);
    } else {
        crate::env::remove_var("ARTERM_TRUSTED_EXTERNAL_AUTH_SOURCES");
    }
}
...
fn migrate_legacy_swarm_spawn_mode_noops_without_visible_value() {
    ...
    crate::env::set_var("ARTERM_HOME", dir2.path());
