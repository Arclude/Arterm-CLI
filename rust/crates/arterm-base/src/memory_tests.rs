    ...
    F: FnOnce(&Path) -> T,
{
    ...
    let old = std::env::var("ARTERM_HOME").ok();
    let unique = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .expect("time went backwards")
        .as_nanos();
    let dir = std::env::temp_dir().join(format!("arterm-test-{}", unique));
    fs::create_dir_all(&dir).expect("create temp dir");
    crate::env::set_var("ARTERM_HOME", &dir);
...
fn test_mode_ignores_the_project_dir_and_cannot_see_real_project_memory() {
    with_temp_home(|_home| {
        let project_dir = "/tmp/arterm-729-real-project";
...
fn project_memories_are_isolated_by_explicit_project_dir() {
    with_temp_home(|_home| {
        let manager_a = MemoryManager::new().with_project_dir("/tmp/arterm-project-a");
        let manager_b = MemoryManager::new().with_project_dir("/tmp/arterm-project-b");
...
fn manager_search_scoped_normalizes_whitespace_and_separators() {
    with_temp_home(|_home| {
        let manager = MemoryManager::new().with_project_dir("/tmp/arterm-search-normalization");
...
fn prompt_memories_scoped_keeps_only_most_recent_entries() {
    with_temp_home(|_home| {
        let manager = MemoryManager::new().with_project_dir("/tmp/arterm-prompt-topk");
...
fn goal_memory_upsert_skips_embedding_generation() {
    with_temp_home(|_home| {
        let manager = MemoryManager::new().with_project_dir("/tmp/arterm-goal-memory");
...
fn scoped_retrieval_respects_project_vs_global() {
    with_temp_home(|_home| {
        let manager = MemoryManager::new().with_project_dir("/tmp/arterm-scope-test");
...
fn retrieval_candidates_include_local_skills() {
    with_temp_home(|home| {
        ...
        fs::create_dir_all(project_dir.join(".arterm/skills/firefox-browser"))
            .expect("create skills dir");
        fs::write(
                project_dir.join(".arterm/skills/firefox-browser/SKILL.md"),
...
fn hybrid_excludes_superseded_memories() {
    with_temp_home(|_home| {
        let manager = MemoryManager::new().with_project_dir("/tmp/arterm-hybrid-supersede");
