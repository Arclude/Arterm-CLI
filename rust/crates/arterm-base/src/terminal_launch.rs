...
pub use arterm_terminal_launch::{
...
///
/// Resolution order: `ARTERM_SPAWN_HOOK` env (handled by config env overrides;
...
///
/// When a spawn hook is configured (`[terminal] spawn_hook` / `ARTERM_SPAWN_HOOK`),
/// the hook takes over the spawn: arterm runs `<hook> <program> <args...>` with
/// `ARTERM_SPAWN_*` metadata env vars so external programs (tmux, kitty remote,
/// herd, window managers) control where and how the session appears. If the
/// hook cannot be started, arterm falls back to its built-in terminal detection.
...
mod tests {
    ...
    fn spawn_via_hook_runs_hook_with_metadata_env() {
        ...
        std::fs::write(
            &hook_path,
            format!(
                "#!/bin/sh\nprintf '%s|%s|%s|%s' \"$ARTERM_SPAWN_KIND\" \"$ARTERM_SPAWN_SESSION_ID\" \"$ARTERM_SPAWN_SWARM_ID\" \"$*\" > {}\n",
        ...
        .spawn_env("ARTERM_SPAWN_SWARM_ID", "swarm-7");
        ...
        assert_eq!(
            recorded, "swarm-agent|ses_hooked|swarm-7|/usr/local/bin/arterm --resume ses_hooked",
            "hook should receive metadata env and the arterm command as argv"
        );
    }
