...
//! This lets iOS/web clients interact with arterm sessions identically
//! to TUI clients.
//!
//! Architecture:
...
pub use arterm_gateway_types::{PairedDevice, PairingCode};
...
async fn handle_pair_request(
    ...
    // Reload from disk - pairing codes are generated by `arterm pair` CLI
    *reg = DeviceRegistry::load();
    ...
    let body = serde_json::json!({
        "token": token,
        "server_name": "arterm",
        "server_version": arterm_build_meta::version(),
    });
    http_response(200, "OK", &body.to_string())
}
...
pub fn resolve_connect_host(bind_addr: &str) -> String {
    ...
    if let Some(host) = std::env::var("ARTERM_GATEWAY_HOST")
        .ok()
