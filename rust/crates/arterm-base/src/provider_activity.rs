...
//!   1. When arterm last successfully used it (for recency-sorted `/usage`).
...
//!
//! Data persists to `~/.arterm/provider_activity.json` and is shared across
...
//!   - `openrouter`, `arterm`, `copilot`, `gemini`, `cursor`, `bedrock`,
...
pub fn display_name_for_source_key(source_key: &str) -> String {
    ...
    match source_key {
        ...
        "arterm" => "Arterm subscription".to_string(),
...
/// `"NVIDIA NIM"`) plus the optional `ARTERM_RUNTIME_PROVIDER` key onto a
...
mod tests {
    ...
    #[test]
    fn record_use_and_spend_roundtrip_under_arterm_home() {
        let _env_lock = lock_env();
        clear_ledger_cache();
        let temp = tempfile::tempdir().expect("tempdir");
        let _home = EnvVarGuard::set("ARTERM_HOME", temp.path().as_os_str());
