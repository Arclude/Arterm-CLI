...
/// user takes: arterm writes the default file, the user uncomments the color
/// example, and arterm loads it back through the same cache the running process
...
fn configured_colors_survive_a_real_config_file_round_trip() {
    ...
    // The file arterm writes for a new user must document colors and parse.
    ...
    // The palette-side half of that contract is asserted in `arterm-tui-style`
...
///
/// The unit tests in `arterm-config-types` cover lenient enum parsing in
/// isolation. This covers the path the reporter actually took: hand-written
/// `~/.arterm/config.toml`, loaded through the same global cache the running
...
fn reported_display_config_survives_a_real_config_file_round_trip() {
    ...
    assert_eq!(
        ...
        arterm_config_types::ReasoningDisplayMode::Current
    );
    // "inline" is the inline-only mode: no dedicated diagram widget.
    assert_eq!(
        loaded.display.diagram_mode,
        arterm_config_types::DiagramDisplayMode::None
    );
    ...
    assert_eq!(
        loaded.display.diagram_mode,
        arterm_config_types::DiagramDisplayMode::default()
    );

    match prev_home {
        Some(prev) => crate::env::set_var("ARTERM_HOME", prev),
