...
impl McpClient {
    ...
    pub async fn connect_in_dir(
        ...
        // config. The long-lived arterm daemon contains provider credentials in
    ...
    async fn initialize(&mut self) -> Result<()> {
        let params = InitializeParams {
            ...
            client_info: ClientInfo {
                name: "arterm".to_string(),
                version: arterm_build_meta::pkg_version().to_string(),
            },
        };
...
"#;
    ...
    async fn connect_in_dir_missing_dir_falls_back_to_inherited_cwd() {
        let client = McpClient::connect_in_dir(
            ...
            &fake_server_config(),
            Some(std::path::Path::new("/nonexistent/arterm-557")),
        )
        .await
