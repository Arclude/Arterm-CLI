...
pub struct McpServerConfig {
    /// Command for stdio servers. Empty for HTTP/SSE servers, which arterm does
    ...
    /// URL for HTTP/SSE servers (Claude Code compat). Unused by arterm today.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub url: Option<String>,
    /// Headers for HTTP/SSE servers (Claude Code compat). Unused by arterm today,
...
impl McpServerConfig {
    /// arterm currently only supports stdio (command-based) MCP servers. A config
...
pub struct McpConfig {
    ...
    /// arterm's historical `servers` key.
...
impl McpConfig {
    ...
    /// servers survive in arterm's snapshot and would duplicate inline secrets.
    /// This only runs while ~/.arterm/mcp.json does not exist.
    fn import_from_codex_once() {
        let arterm_mcp = match crate::storage::arterm_dir() {
            Ok(dir) => dir.join("mcp.json"),
            Err(_) => return,
        };

        if arterm_mcp.exists() {
        ...
        let environment_value_count = imported
            ...
            .sum();
        if let Err(e) = imported.save_to_file(&arterm_mcp) {
    ...
    fn live_claude_log_message(server_count: usize, source: &str) -> String {
        format!(
            "MCP: Loaded {} server(s) live from Claude Code ({}); source values were not copied into arterm config",
            server_count, source
        )
    }
    ...
    /// order: `.arterm/mcp.json`, then `.mcp.json` (Claude Code project config),
    ...
    fn load_project_locals(project_root: &std::path::Path) -> Self {
        ...
        for relative in [".arterm/mcp.json", ".mcp.json", ".claude/mcp.json"] {
    ...
    /// Load from default locations (merges arterm global + local, local overrides),
    ...
    /// (`.arterm/mcp.json`, `.mcp.json`, `.claude/mcp.json`, and the per-project
    ...
    pub fn load_for_dir(project_dir: Option<&std::path::Path>) -> Self {
        ...
        // Load arterm's own global config (~/.arterm/mcp.json)
        if let Ok(arterm_dir) = crate::storage::arterm_dir() {
            let arterm_mcp = arterm_dir.join("mcp.json");
            if arterm_mcp.exists() {
                if let Ok(config) = Self::load_from_file(&arterm_mcp) {
        ...
        // arterm only supports stdio servers today. Drop HTTP/SSE entries (common
    ...
    /// Merge `incoming` over `existing`, except that an entry arterm cannot run
    ...
    /// working stdio server from `~/.arterm/mcp.json` and then be dropped by the
