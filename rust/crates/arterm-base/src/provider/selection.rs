...
pub(super) use arterm_provider_core::{ActiveProvider, ProviderAvailability};
...
impl MultiProvider {
    pub(super) fn auto_default_provider(availability: ProviderAvailability) -> ActiveProvider {
        arterm_provider_core::auto_default_provider(availability)
    }

    pub(super) fn parse_provider_hint(value: &str) -> Option<ActiveProvider> {
        arterm_provider_core::parse_provider_hint(value)
    }

    pub(super) fn initial_provider_from_env() -> Option<ActiveProvider> {
        let explicit = std::env::var("ARTERM_INITIAL_PROVIDER_EXPLICIT")
            .ok()
            .is_some_and(|value| {
                matches!(
    ...
    pub fn default_model_selection_from_route(
        ...
        let provider_key = match &api_method_kind {
            ModelRouteApiMethod::ArtermSubscription => Some("arterm".to_string()),
    ...
    pub(crate) fn canonical_session_provider_key(provider_key: &str) -> &str {
        ...
        if let Some(route) = arterm_provider_core::AuthRoute::parse(provider_key) {
    ...
    fn explicit_session_provider_key_for_model_request(model_request: &str) -> Option<String> {
        ...
        if let Some((prefix, rest)) = model_request.split_once(':') {
            ...
            if !prefix.is_empty() && !rest.trim().is_empty() {
                ...
                if let Some(route) = arterm_provider_core::AuthRoute::parse(prefix) {
    ...
    pub fn model_switch_request_for_session_model(
        ...
        if let Some(route) =
            arterm_provider_core::AuthRoute::parse_explicit_credential_prefix(provider_key)
        {
    ...
    pub fn model_switch_request_for_session_route(
            ...
            .filter(|api_method| !api_method.is_empty())
        {
            match ModelRouteApiMethod::parse(api_method) {
                ModelRouteApiMethod::ArtermSubscription => return model.to_string(),
    ...
    pub(super) fn resolve_config_provider_selection(
        ...
        Self::parse_provider_hint(trimmed)
            .or_else(|| {
                arterm_provider_core::AuthRoute::parse(trimmed).map(|route| route.active_provider())
            })
...
mod tests {
    ...
    fn bare_session_provider_key_restores_auto_credential_not_oauth_pin() {
        ...
        for (model, provider_key, expected_request) in [
            ...
            assert!(
                arterm_provider_core::AuthRoute::parse_explicit_credential_prefix(prefix).is_none(),
