...
fn test_openai_live_catalog_replaces_static_fallback_list() {
    ...
    assert_eq!(
        ...
        [
            ...
            arterm_provider_core::CHATGPT_WEB_MODEL.to_string()
        ]
    );
    ...
    for extra in &models[2..] {
        assert!(
            arterm_provider_core::is_openai_api_only_pro_model(extra),
...
fn test_hosted_model_guard_does_not_gate_models_by_legacy_tier() {
    ...
    crate::env::set_var("ARTERM_HOME", temp_home.path().to_string_lossy().to_string());
    crate::env::remove_var(crate::subscription_catalog::ARTERM_TIER_ENV);
    crate::subscription_catalog::clear_runtime_env();
    crate::subscription_catalog::apply_runtime_env();
    ...
    crate::env::set_var(crate::subscription_catalog::ARTERM_TIER_ENV, "ultra");
    assert!(ensure_model_allowed_for_subscription("claude-fable-5").is_ok());
...
#[test]
fn test_remote_arterm_subscription_fallback_keeps_managed_route_identity() {
    let models = vec![
        "claude-opus-4-8".to_string(),
        "claude-sonnet-4-6".to_string(),
        "gpt-5.5".to_string(),
    ...
    let routes = remote_model_routes_fallback(
        Some(crate::subscription_catalog::ARTERM_PROVIDER_DISPLAY_NAME),
        &models,
    );
...
fn test_subscription_filters_do_not_activate_from_saved_credentials_alone() {
    let _guard = crate::storage::lock_test_env();
    crate::subscription_catalog::clear_runtime_env();
    crate::env::set_var(crate::subscription_catalog::ARTERM_API_KEY_ENV, "test-key");
    ...
    crate::env::remove_var(crate::subscription_catalog::ARTERM_API_KEY_ENV);
    crate::subscription_catalog::clear_runtime_env();
}
