...
fn repeated_external_resume_reuses_the_imported_snapshot() {
    ...
    .unwrap();
    let target = arterm_session_types::ResumeTarget::ClaudeCodeSession {
        session_id: "cached".to_string(),
        session_path: transcript.to_string_lossy().to_string(),
    };

    let first = resolve_resume_target_to_arterm(&target).unwrap();
    std::fs::remove_file(&transcript).unwrap();
    let second = resolve_resume_target_to_arterm(&target).unwrap();
...
fn ordinary_resume_never_stops_a_live_claude_process() {
    ...
    let resolved = resolve_resume_target_to_arterm(&target).unwrap();
    assert!(matches!(
        resolved,
        arterm_session_types::ResumeTarget::ArtermSession { .. }
    ));
...
fn explicit_takeover_preserves_history_and_stops_only_matching_process() {
    ...
    let arterm_session_types::ResumeTarget::ArtermSession { session_id } = resolved else {
        panic!("expected Arterm session");
    };
...
#[cfg(target_os = "linux")]
#[test]
fn takeover_timeout_preserves_the_staged_arterm_session_after_sigterm() {
    use std::process::{Command, Stdio};
...
fn cached_imported_session_preserves_existing_history_verbatim() {
    ...
    legacy.append_stored_message(StoredMessage {
        id: "arterm-continuation".to_string(),
        role: Role::Assistant,
        content: vec![ContentBlock::Text {
            text: "continued inside arterm".to_string(),
            cache_control: None,
        }],
        display_role: None,
        timestamp: None,
    ...
    let resolved =
        resolve_resume_target_to_arterm(&arterm_session_types::ResumeTarget::CodexSession {
...
#[test]
fn test_import_pi_session_creates_arterm_snapshot() {
    let _guard = crate::storage::lock_test_env();
    let temp = tempfile::TempDir::new().unwrap();
    let _home = EnvVarGuard::set_path("ARTERM_HOME", temp.path());
...
#[test]
fn test_import_opencode_session_creates_arterm_snapshot() {
    let _guard = crate::storage::lock_test_env();
    let temp = tempfile::TempDir::new().unwrap();
    let _home = EnvVarGuard::set_path("ARTERM_HOME", temp.path());
...
#[test]
fn test_resolve_resume_target_to_arterm_imports_codex_session() {
    let _guard = crate::storage::lock_test_env();
    let temp = tempfile::TempDir::new().unwrap();
    let _home = EnvVarGuard::set_path("ARTERM_HOME", temp.path());
...
/// `resolve_resume_target_to_arterm`, which must import the transcript and hand
/// back a resumable `imported_cc_<id>` arterm session. This guards the full
/// detect -> import -> resume round-trip for Claude Code (previously only Codex
/// had coverage here).
#[test]
fn test_resolve_resume_target_to_arterm_imports_claude_code_session() {
    let _guard = crate::storage::lock_test_env();
    let temp = tempfile::TempDir::new().unwrap();
    let _home = EnvVarGuard::set_path("ARTERM_HOME", temp.path());
    ...
    let resolved =
        resolve_resume_target_to_arterm(&arterm_session_types::ResumeTarget::ClaudeCodeSession {
    ...
    assert_eq!(
        imported_session_id_for_target(&arterm_session_types::ResumeTarget::ClaudeCodeSession {
            session_id: "claude-resolve-test".to_string(),
            session_path: transcript_path.to_string_lossy().to_string(),
        }),
...
/// Regression for silent data loss: the picker hides the imported arterm session
...
/// user already resumed and continued that imported session inside arterm, a
/// blind re-import previously overwrote the snapshot and dropped the arterm-side
/// messages. The continuation must be preserved instead.
#[test]
fn test_reimporting_claude_session_preserves_arterm_continuation() {
    let _guard = crate::storage::lock_test_env();
    let temp = tempfile::TempDir::new().unwrap();
    let _home = EnvVarGuard::set_path("ARTERM_HOME", temp.path());
    ...
    // User resumes inside arterm and appends a arterm-only follow-up message.
    let mut session = Session::load(&imported_id).unwrap();
    session.append_stored_message(StoredMessage {
        id: "arterm-continuation".to_string(),
        role: Role::User,
        content: vec![ContentBlock::Text {
            text: "arterm-only follow up".to_string(),
            cache_control: None,
        }],
        display_role: None,
        timestamp: None,
    ...
    assert_eq!(
        after.messages.len(),
        3,
        "arterm-side continuation must not be clobbered by re-import"
    );
    let preserved = after.messages.iter().flat_map(|m| m.content.iter()).any(
        |block| matches!(block, ContentBlock::Text { text, .. } if text == "arterm-only follow up"),
    );
    assert!(
        preserved,
        "the arterm-only follow up message must be preserved"
    );
}

#[test]
fn test_import_cursor_session_creates_arterm_snapshot() {
    let _guard = crate::storage::lock_test_env();
    let temp = tempfile::TempDir::new().unwrap();
    let _home = EnvVarGuard::set_path("ARTERM_HOME", temp.path());
    ...
    // Resolving the resume target should import and remap to the arterm snapshot.
    let resumed = crate::import::resolve_resume_target_to_arterm(
        &arterm_session_types::ResumeTarget::CursorSession {
