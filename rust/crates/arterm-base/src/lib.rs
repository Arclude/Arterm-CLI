//! `arterm-base`: foundational layer of the arterm application core.
//!
//! This crate holds the downward-closed set of modules that the upper
//! server/tool/agent layer (`arterm-app-core`) depends on: provider, auth,
...
//!
//! `arterm-app-core` re-exports this crate via `pub use arterm_base::*`, so every
