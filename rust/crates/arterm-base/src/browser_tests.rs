...
async fn test_inspect_browser_status_without_binary() {
    // Hold the test-env lock: this reads ARTERM_HOME-derived paths, and other
    // tests mutate ARTERM_HOME (and write browser fixture files) under the
    // lock. Without it, the status snapshot and the exists() check below can
    // observe different ARTERM_HOME values mid-test.
    let _guard = crate::storage::lock_test_env();
...
async fn test_ensure_browser_ready_noninteractive_without_binary() {
    ...
    // that mutate ARTERM_HOME under the test-env lock.
    let _guard = crate::storage::lock_test_env();
...
//
// Bug B: once ~/.arterm/browser/.setup-complete existed, setup could never
