//! Process-wide "keep the machine awake while arterm is working" inhibitor.
//!
//! The shared `arterm serve` daemon hosts every session, so a single inhibitor
...
const DISABLE_ENV: &str = "ARTERM_DISABLE_POWER_INHIBIT";
...
/// Best-effort inhibitor that keeps the machine awake while arterm is actively
...
impl PowerInhibitor {
    ...
    /// unless the legacy `ARTERM_DISABLE_POWER_INHIBIT` env escape hatch is set.
    ///
...
impl WindowsPowerGuard {
    fn acquire() -> io::Result<Self> {
        ...
        let thread = thread::Builder::new()
            .name("arterm-power-inhibit".to_string())
...
impl Drop for WindowsPowerGuard {
    fn drop(&mut self) {
        if let Err(error) = self.stop() {
            arterm_logging::warn(&format!("failed to stop Windows power guard: {error}"));
        }
    }
}
...
fn run_windows_power_guard(
    ...
    if ready_tx.send(Ok(())).is_err() {
        if unsafe { SetThreadExecutionState(windows_clear_execution_state_flags()) } == 0 {
            arterm_logging::warn("failed to clear Windows power guard after receiver disconnect");
        }
        return;
    }
...
fn build_linux_systemd_inhibit_command(ttl: Duration) -> Command {
    ...
    command
        ...
        .arg("--who=arterm")
        .arg("--why=Arterm is streaming or processing active work")
        .arg("--mode=block")
...
mod tests {
    ...
    fn linux_inhibitor_blocks_sleep_and_lid_switch_with_bounded_ttl() {
        ...
        assert!(args.contains(&"--who=arterm".to_string()));
