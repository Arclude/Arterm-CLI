...
//! doesn't expose a usage API. Data persists to ~/.arterm/copilot_usage.json.
...
pub use arterm_usage_types::{AllTimeUsage, CopilotUsageTracker, DayUsage, MonthUsage};
...
mod tests {
    ...
    #[test]
    fn usage_path_respects_arterm_home() {
    ...
    #[test]
    fn save_and_load_roundtrip_under_arterm_home() {
        let _env_lock = lock_env();
        clear_tracker();
        let temp = tempfile::tempdir().expect("tempdir");
        let _home = EnvVarGuard::set("ARTERM_HOME", temp.path().as_os_str());
