...
impl GmailBackend {
    ...
    ///
    /// Defaults to `Direct`. Set `ARTERM_GMAIL_BACKEND=composio` (with
    /// `COMPOSIO_API_KEY` present) to broker Gmail through Composio.
    pub fn from_env() -> Self {
        let selection = std::env::var("ARTERM_GMAIL_BACKEND")
            .unwrap_or_default()
            .trim()
            .to_lowercase();
        if selection == "composio" {
            ...
            eprintln!(
                "ARTERM_GMAIL_BACKEND=composio but COMPOSIO_API_KEY is not set; falling back to direct Gmail backend"
            );
        }
        GmailBackend::Direct
    }
...
impl ComposioConnection {
    pub fn path() -> Result<std::path::PathBuf> {
        Ok(crate::storage::arterm_dir()?.join("composio_gmail.json"))
    }
...
impl GmailClient {
    ...
    pub fn not_configured_message(&self) -> &'static str {
        match &self.backend {
            GmailBackend::Direct => {
                "Gmail is not configured. Run `arterm login google` to set up Gmail access."
            }
    ...
    pub async fn connect(&self, open_browser: bool) -> Result<ComposioConnection> {
        let cfg = match &self.backend {
            ...
            GmailBackend::Direct => {
                anyhow::bail!(
                    "The Composio connect flow is only available when ARTERM_GMAIL_BACKEND=composio."
                )
            }
        };
