...
use arterm_config_types::ReasoningDisplayMode;
...
fn format_reasoning_markup(text: &str) -> String {
    ...
    for line in text.split('\n') {
        out.push_str(&arterm_render_core::reasoning_line_markup(line));
    }
        ...
        for block in &msg.content {
            match block {
                ContentBlock::Text { text: t, .. } => {
                    ...
                    // result: ...]` block is synthetic metadata arterm injects so
