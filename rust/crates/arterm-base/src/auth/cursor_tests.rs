...
#[test]
fn config_file_path_under_arterm() {
    let path = config_file_path().unwrap();
    let path_str = path.to_string_lossy();
    assert!(path_str.contains("arterm"));
...
fn save_and_load_api_key() {
    ...
    let file = dir.path().join("arterm").join("cursor.env");
...
#[test]
fn cursor_auth_file_path_respects_arterm_home() {
    // Regression: on Linux the auth.json path previously used
    // `dirs::config_dir()` directly, ignoring ARTERM_HOME. That leaked the real
    ...
    // provider correctly looked under `$ARTERM_HOME/external/...`.
    ...
    assert!(
        ...
        "cursor auth path should be under ARTERM_HOME/external, got {}",
        path.display()
    );
...
#[test]
fn cursor_auth_file_path_does_not_escape_arterm_home_on_windows() {
    let _guard = crate::storage::lock_test_env();
    let temp = tempfile::tempdir().unwrap();
    let old_home = std::env::var_os("ARTERM_HOME");
...
#[test]
fn cursor_vscdb_paths_respect_arterm_home() {
    let _guard = crate::storage::lock_test_env();
    let prev_home = std::env::var_os("ARTERM_HOME");
    let temp = TempDir::new().unwrap();
    crate::env::set_var("ARTERM_HOME", temp.path());
