...
/// Represents a named Anthropic OAuth account stored in arterm's auth.json.
...
fn relabel_accounts(auth: &mut ArtermAuthFile) -> bool {
...
pub fn arterm_path() -> Result<PathBuf> {
    Ok(crate::storage::arterm_dir()?.join("auth.json"))
}

// ---- Multi-account helpers ----

/// Read the arterm auth file, auto-migrating from legacy format if needed.
pub fn load_auth_file() -> Result<ArtermAuthFile> {
    let path = arterm_path()?;
    if !path.exists() {
        return Ok(ArtermAuthFile::default());
    }

    crate::storage::harden_secret_file_permissions(&path);

    let mut auth: ArtermAuthFile = crate::storage::read_json(&path)
        .with_context(|| format!("Could not read arterm credentials from {:?}", path))?;
...
/// Write the arterm auth file (multi-account format).
pub fn save_auth_file(auth: &ArtermAuthFile) -> Result<()> {
    let auth_path = arterm_path()?;

    let clean = ArtermAuthFile {
...
/// Falls through Claude Code -> arterm accounts -> OpenCode, preferring non-expired tokens.
pub fn load_credentials() -> Result<ClaudeCredentials> {
    ...
    // prompt, so it is read once at import time and copied into arterm's own
    ...
    if let Ok(creds) = load_arterm_credentials() {
        if creds.expires_at > now_ms {
            return Ok(creds);
        }
        expired_candidates.push(("arterm", creds));
    }
    ...
    anyhow::bail!("No Claude OAuth credentials found (checked Claude Code, arterm, OpenCode)")
}

/// Load credentials for a specific arterm account by label.
...
/// Load credentials from the active arterm account (multi-account aware).
fn load_arterm_credentials() -> Result<ClaudeCredentials> {
    let auth = load_auth_file()?;
    if auth.anthropic_accounts.is_empty() {
        anyhow::bail!("No anthropic accounts configured in arterm auth.json");
    }
    ...
    let account = auth
        ...
        .context("No anthropic accounts in arterm auth.json")?;
...
/// them into a arterm-managed Anthropic account so future loads and token
/// refreshes are served from arterm's own `auth.json`.
///
...
/// rotates into arterm's store on refresh, so the copy stays usable after the
