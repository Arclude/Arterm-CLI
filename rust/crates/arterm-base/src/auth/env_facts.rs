...
//!
//! Historically arterm discovered environment limitations *by failing*: it would
...
pub struct EnvFacts {
    ...
    /// The arterm config directory exists (or can be created) and is writable.
    ...
    pub fn precondition_message(self) -> Option<&'static str> {
        match self {
            AuthMethodChoice::BlockedConfigUnwritable => Some(
                "arterm cannot write its config directory, so a login could not be saved. Fix directory permissions (or set ARTERM_HOME) and try again.",
            ),
            AuthMethodChoice::ApiKeyNonInteractive => Some(
                "This terminal is not interactive. Provide an API key via the provider's environment variable or `arterm provider add --api-key-stdin`.",
            ),
            _ => None,
        }
    }
