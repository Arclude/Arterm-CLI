...
//
//   cargo test -p arterm-base --test header_cost_probe -- --nocapture --ignored
//
...
fn measure_auth_status_probe_cost() {
    ...
    let _ = arterm_base::auth::AuthStatus::check_fast();
...
///
/// This lives in `arterm-base` (its own test binary) rather than beside the
/// header tests because `invalidate_cached_status()` clears a process-global
/// cache that sibling tests in the `arterm-tui` binary share.
#[test]
fn invalidate_cached_status_bumps_the_auth_generation() {
    let before = arterm_base::auth::auth_status_generation();
    arterm_base::auth::AuthStatus::invalidate_cached_status();
    let after = arterm_base::auth::auth_status_generation();
