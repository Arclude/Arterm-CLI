...
//!
//! These drive the real `arterm_base::import` pipeline against synthetic
//! `.jsonl` fixtures written into a sandboxed `ARTERM_HOME`, so they never touch
...
//! environment (`ARTERM_HOME`).

use arterm_base::import::{
    import_session, import_session_from_file, list_claude_code_sessions,
    list_claude_code_sessions_lazy,
};
use arterm_base::message::ContentBlock;
use arterm_base::session::Session;
use arterm_import_core::imported_claude_code_session_id;
use std::path::{Path, PathBuf};
use std::sync::Mutex;

/// Serializes tests that mutate `ARTERM_HOME`. This test binary has its own
...
impl Drop for EnvVarGuard {
    fn drop(&mut self) {
        if let Some(prev) = self.prev.take() {
            arterm_base::env::set_var(self.key, prev);
        } else {
...
/// Create the sandboxed Claude project dir under `$ARTERM_HOME/external/...`.
...
fn import_array_tool_result_with_image_uses_placeholder_not_base64() {
    ...
    let saved = std::fs::read_to_string(arterm_session_file(temp.path(), "img-session")).unwrap();
    assert!(
        !saved.contains(&big),
...
/// Path to where arterm persists imported sessions under the sandbox.
/// With `ARTERM_HOME` set, `storage::arterm_dir()` returns it directly, and
/// sessions live at `<ARTERM_HOME>/sessions/<id>.json`.
fn arterm_session_file(home: &Path, claude_id: &str) -> PathBuf {
