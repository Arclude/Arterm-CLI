...
fn gemini_sends_a_clean_schema_for_the_hostile_tool() {
    let built = arterm_provider_gemini::build_tools(&hostile_tool()).expect("tools");
...
fn every_antigravity_route_sends_a_clean_schema_for_the_hostile_tool() {
    ...
    for model in ["gemini-3-flash", "claude-sonnet-4-5", "gpt-oss-120b"] {
        let normalized = arterm_provider_antigravity::antigravity_compatible_schema(&schema, model);
...
/// typeless property does not get a `strict` claim arterm cannot honor.
#[test]
fn openai_sends_a_clean_schema_and_does_not_overclaim_strict() {
    let built = arterm_provider_openai::request::build_tools(&hostile_tool());
    let wire = serde_json::to_value(&built).expect("serialize");
...
fn openrouter_sends_a_schema_its_strictest_upstream_accepts() {
    ...
    let normalized =
        arterm_provider_openrouter::request::sanitize_tool_parameters_schema(&combiner_schema);
    ...
    let bare =
        arterm_provider_openrouter::request::sanitize_tool_parameters_schema(&serde_json::json!({
            "type": "object"
        }));
...
fn provider_request_builders_that_reach_the_dialect_engine_are_pinned() {
    ...
    const BUILDERS: &[(&str, bool)] = &[
        ("../arterm-provider-gemini/src/lib.rs", true),
        ("../arterm-provider-antigravity/src/lib.rs", true),
        ("../arterm-provider-openrouter/src/request.rs", true),
        ("../arterm-provider-anthropic/src/lib.rs", true),
        // OpenAI's keyword subset now comes from the engine too. Strict
        // eligibility and strict normalization stay in arterm-provider-core
        ...
        ("../arterm-provider-core/src/openai_schema.rs", true),
        ("../arterm-provider-openai/src/request.rs", false),
    ];
    ...
    for (relative, should_use_engine) in BUILDERS {
        ...
        let uses_engine = source.lines().any(|line| {
            ...
            !trimmed.starts_with("//") && trimmed.contains("arterm_schema_dialect::")
        });
...
fn anthropic_sends_a_schema_without_a_top_level_combiner() {
    ...
    let built = arterm_provider_anthropic::format_tools(&combiner_tool, false, false);
    let wire = serde_json::to_value(&built).expect("serialize");
    ...
    let bare_wire =
        serde_json::to_value(arterm_provider_anthropic::format_tools(&bare, false, false))
            .expect("serialize");
...
fn a_keyword_no_deny_list_has_ever_heard_of_reaches_no_provider() {
    ...
    let gemini =
        serde_json::to_value(arterm_provider_gemini::build_tools(&novel).expect("gemini tools"))
            .expect("serialize");
    assert!(
    ...
    let openai = serde_json::to_value(arterm_provider_openai::request::build_tools(&novel))
        .expect("serialize");
    assert!(
    ...
    for model in ["gemini-3-flash", "claude-sonnet-4-5", "gpt-oss-120b"] {
        let antigravity = arterm_provider_antigravity::antigravity_compatible_schema(
