...
//!
//! The unit tests in `arterm-schema-dialect` work on schema values, and the
//! sweep in `arterm-app-core` works on arterm's own tools. Neither covers the
...
//! wire and asserts on the bytes arterm sends to the provider.

use arterm_base::mcp::McpToolDef;
...
fn playwright_schema_reaches_gemini_without_the_keyword_that_caused_issue_754() {
    ...
    let built = arterm_provider_gemini::build_tools(&defs).expect("gemini tools");
...
fn playwright_schema_reaches_openai_without_an_unsupported_construct() {
    let defs = playwright_tool_definitions();
    let built = arterm_provider_openai::request::build_tools(&defs);
...
fn the_normalized_playwright_schema_conforms_to_every_dialect() {
    ...
    for spec in arterm_schema_dialect::registry::ALL {
        let normalized = arterm_schema_dialect::dialect::apply(schema, spec);
        let errors =
        ...
        let lost = arterm_schema_dialect::must_preserve_meaning(schema, &normalized);
        assert!(
            lost.is_empty(),
...
fn recovery_triggers_on_the_error_string_the_runtime_really_builds() {
    ...
    let rejection = arterm_schema_dialect::classify(&with_context)
