...
//! consumes them. These live in `arterm-render-core` (a backend-neutral, pure
//! crate) rather than in `arterm-tui-markdown` so the foundation/streaming layer
//! can format reasoning lines without depending on any `arterm-tui-*` crate.
...
///
/// Lives here (a backend-neutral, pure crate) rather than in `arterm-tui-markdown`
/// so the foundation/streaming layer can format the summary without depending on
/// any `arterm-tui-*` crate. Re-exported from `arterm-tui-markdown` for the
/// existing `arterm_tui_markdown::reasoning_summary_line_markup` path.
