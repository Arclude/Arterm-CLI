//! Adapter: backend-neutral [`arterm_render_core::Document`] -> ratatui lines.
//!
...
fn push_code_block(
    ...
    block: &arterm_render_core::Block,
    language: Option<&str>,
) {
...
fn push_math_display(lines: &mut Vec<Line<'static>>, block: &arterm_render_core::Block) {
    let dim = Style::default().fg(md_dim_color());
...
pub fn render_markdown_via_core(text: &str) -> Vec<Line<'static>> {
    document_to_lines_with_width(&arterm_render_core::parse_markdown(text), None)
}
...
pub fn render_markdown_via_core_wrapped(text: &str, width: usize) -> Vec<Line<'static>> {
    let doc = arterm_render_core::parse_markdown(text);
