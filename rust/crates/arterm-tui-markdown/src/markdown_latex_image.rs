...
fn deferred_sender() -> &'static mpsc::Sender<DeferredLatexTask> {
    DEFERRED_TX.get_or_init(|| {
        ...
        let _ = std::thread::Builder::new()
            .name("arterm-latex-deferred".to_string())
...
impl Toolchain {
    fn from_environment() -> Self {
        Self {
            latex: std::env::var_os("ARTERM_LATEX_COMMAND")
                .map(PathBuf::from)
                .unwrap_or_else(|| PathBuf::from("latex")),
            dvipng: std::env::var_os("ARTERM_DVIPNG_COMMAND")
                .map(PathBuf::from)
                .unwrap_or_else(|| PathBuf::from("dvipng")),
            pdflatex: std::env::var_os("ARTERM_PDFLATEX_COMMAND")
                .map(PathBuf::from)
                .unwrap_or_else(|| PathBuf::from("pdflatex")),
            pdftocairo: std::env::var_os("ARTERM_PDFTOCAIRO_COMMAND")
                .map(PathBuf::from)
...
fn cache_dir() -> Result<PathBuf, String> {
    dirs::cache_dir()
        .map(|path| path.join("arterm").join("latex"))
...
fn run_command<const N: usize>(
    ...
    let output_path = working_dir.join(".arterm-command-output.log");
...
mod tests {
    ...
    fn missing_toolchain_returns_an_error_without_panicking() {
        ...
        let missing = PathBuf::from("/definitely/missing/arterm-latex-command");
