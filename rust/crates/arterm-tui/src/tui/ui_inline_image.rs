...
use arterm_tui_messages::{ImageRegion, ImageRegionRender, PreparedMessages};
...
fn prewarm_sender() -> &'static mpsc::SyncSender<PrewarmRequest> {
    PREWARM_TX.get_or_init(|| {
        ...
        if let Err(err) = std::thread::Builder::new()
            .name("arterm-inline-image-prewarm".to_string())
...
impl AnchoredInlineImages {
    ...
    pub(crate) fn unplaced_items(
        &self,
        messages: &[arterm_tui_messages::DisplayMessage],
    ) -> Vec<InlineImageItem> {
...
pub(crate) fn image_label_line(
    ...
    arterm_tui_render::truncate_line_preserving_suffix_to_width(
        &prefix,
        &Line::from(suffix),
        width as usize,
    )
...
fn image_fallback_note_line(width: u16) -> Line<'static> {
    arterm_tui_render::truncate_line_with_ellipsis_to_width(
...
pub(crate) fn build_section(
    ...
    let plain: Vec<String> = lines
        .iter()
        .map(arterm_tui_render::line_plain_text)
        .collect();
...
mod tests {
    ...
    fn build_section_emits_one_fit_region_per_image_with_label() {
        ...
        for region in &section.image_regions {
            ...
            assert!(
                arterm_tui_render::line_plain_text(first).trim().is_empty(),
        ...
        let label_line = arterm_tui_render::line_plain_text(&section.wrapped_lines[1]);
        assert!(
    ...
    fn visible_label_line_advertises_hide_badge() {
        ...
        let text = arterm_tui_render::line_plain_text(&line);
    ...
    fn generated_image_label_is_truncated_to_one_terminal_row() {
        ...
        generated.label = "/home/jeremy/arterm/.arterm/generated-images/1783707839145-ig_0c5e377de871aba5016a51388779588196944d98eff2c7aa30.png".to_string();
    ...
    fn anchored_image_lines_round_trip_through_region_scan() {
        ...
        for offset in 1..rows as usize {
            ...
            assert!(
                arterm_tui_render::line_plain_text(line).trim().is_empty(),
