...
fn copy_selection_text_with_math_targets(
    ...
    let math_targets: Vec<_> = prepared
        ...
        .filter(|target| {
            matches!(
                &target.kind,
                arterm_tui_markdown::CopyTargetKind::Math { .. }
            )
...
mod tests {
    use super::*;
    use arterm_tui_markdown::CopyTargetKind;
    use arterm_tui_messages::{CopyTarget, PreparedChatFrame, PreparedMessages, WrappedLineMap};
