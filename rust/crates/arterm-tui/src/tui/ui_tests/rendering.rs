...
fn test_render_swarm_message_centered_mode_keeps_file_activity_preview_centered_when_diff_wraps() {
    ...
    let msg = DisplayMessage::swarm(
        "File activity · rose",
        "`…/arterm/src/server/comm_sync.rs`
...
fn test_render_memory_tiles_allows_boxes_below_other_boxes() {
    let tiles = group_into_tiles(vec![
        ...
        (
            "preference".to_string(),
            "User prefers quick verification that arterm is up-to-date.".to_string(),
        ),
        ("fact".to_string(), "Jeremy".to_string()),
        (
            "entity".to_string(),
...
fn test_light_theme_adapted_frame_has_readable_contrast() {
    ...
    terminal
        .draw(|frame| {
            ...
            arterm_tui_style::theme_mode::adapt_buffer(
                frame.buffer_mut(),
                arterm_tui_style::ThemeMode::Light,
            );
        })
        .expect("draw");
...
///
/// The unit tests in `arterm-tui-style` cover the substitution in isolation, but
...
fn test_configured_palette_recolors_a_real_rendered_frame() {
    ...
    let mut palette = arterm_tui_style::Palette::default();
    palette.set(arterm_tui_style::Role::User, (250, 40, 200));
