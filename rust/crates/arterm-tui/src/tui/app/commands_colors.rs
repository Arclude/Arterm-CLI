...
//! `~/.arterm/config.toml`. This command is the interactive front end for that:
...
//! [`arterm_tui_style::harmony`] so a user gets specific, actionable feedback
//! instead of trial and error.

use super::{App, DisplayMessage};
use arterm_tui_style::palette::{ALL_ROLES, Palette, Role, parse_hex, to_hex};
...
fn active_background() -> (u8, u8, u8) {
    if arterm_tui_style::is_light_theme() {
        (255, 255, 255)
    } else {
        (18, 18, 18)
    }
...
fn show_harmony(app: &mut App) {
    ...
    let report = arterm_tui_style::analyze_harmony(&configured_palette(), background);
...
fn generate_palette(app: &mut App, seed: Option<&str>) {
    ...
    let background = active_background();
    let generated = arterm_tui_style::harmony::generate_from_seed(seed_rgb, background);
    let report = arterm_tui_style::analyze_harmony(&generated, background);
...
/// concurrent config edit by another arterm session is not clobbered.
fn persist(
