...
impl App {
    ...
    pub(super) fn maybe_begin_onboarding_flow_on_startup(&mut self) {
        ...
        // spawned by developers (e.g. the niri `arterm self-dev` hotkey) and that
    ...
    /// only counts interactive `arterm` launches (TTY-gated) and can be reset
    ...
    pub(super) fn is_new_user_install() -> bool {
        let Ok(dir) = crate::storage::arterm_dir() else {
            return true;
        };
    ...
    fn has_established_native_session_history(arterm_dir: &std::path::Path) -> bool {
        const ESTABLISHED_SESSION_THRESHOLD: usize = 10;
        let Ok(entries) = std::fs::read_dir(arterm_dir.join("sessions")) else {
            return false;
        };
    ...
    ///
    /// These are launched by developers working on arterm itself (for example the
    /// niri `arterm self-dev` hotkey). That launch path bypasses
    ...
    pub(super) fn begin_onboarding_flow_at_login(&mut self) {
        ...
        if had_imports {
            self.set_status_notice(
                "Welcome to arterm: review detected logins (arrows/hl to move, Enter to choose)",
            );
        } else {
            self.set_status_notice(
    ...
    fn handle_onboarding_import_review_key(&mut self, code: KeyCode) -> bool {
        ...
        // Set when the user wants to learn about the hosted Arterm subscription
        ...
        if open_pricing {
            self.onboarding_import_error = None;
            let url = crate::subscription_catalog::ARTERM_PRICING_URL;
            if super::helpers::open_path_or_url_detached(url).is_ok() {
                self.set_status_notice(format!("Opened Arterm pricing: {url}"));
            } else {
                self.set_status_notice(format!("Open Arterm pricing: {url}"));
            }
            return true;
        }
        if finished {
    ...
    fn onboarding_finish_import_review(&mut self) {
        ...
        // target the agent repair brief at the right `arterm auth-test --provider`.
    ...
    /// with a large `~/.arterm/sessions` directory. Doing it inline on Enter made
    ...
    pub(super) fn onboarding_prepare_recent_project_review(&mut self) -> bool {
        let Some(repository) = self.onboarding_recent_project_path() else {
            self.onboarding_show_suggestions();
            self.set_status_notice(
                "No recent Git repository found. Start arterm inside a project to review it.",
            );
            return false;
        };
        self.onboarding_finish();
    ...
    /// 1. A arterm login that exists but expired (they clearly use it).
    ...
    fn onboarding_other_provider_rows(skip: Option<&str>) -> (Vec<String>, Vec<String>) {
        ...
        let providers: [(&str, &str, AuthState); 8] = [
            ...
            ("Arterm subscription", "arterm", status.arterm),
    ...
    /// commands the agent can run (`arterm auth-test --json`, `arterm login
    /// --api-key-stdin`, `arterm provider add`), copy it to the clipboard, and
