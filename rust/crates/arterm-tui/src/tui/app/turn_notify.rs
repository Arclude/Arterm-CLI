...
///
/// Layout (macOS):
///   title:    arterm · <session> · done in <dur>
...
pub(super) fn build_turn_notification(
    ...
    let mut title = String::from("arterm");
...
mod tests {
    ...
    fn title_includes_session_and_compact_duration() {
        ...
        assert_eq!(n.title, "arterm · fox · done in 12m 34s");
    ...
    fn subtitle_holds_progress_and_body_names_the_work() {
        ...
        assert_eq!(n.title, "arterm · done in 3m 20s");
    ...
    fn empty_inputs_fall_back_to_minimal_body() {
        ...
        assert_eq!(n.title, "arterm · done in 1m 5s");
        assert_eq!(n.subtitle, None);
