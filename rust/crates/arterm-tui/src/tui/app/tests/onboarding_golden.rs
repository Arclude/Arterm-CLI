...
//   cargo test -p arterm-tui onboarding_golden -- --nocapture
...
fn onboarding_golden_walks_every_phase() {
    ...
    // import action, with a Arterm subscription alternative and secondary
    // import/telemetry controls beside it.
    {
        ...
        // The primary actions explicitly offer import or a Arterm subscription.
        assert!(text.contains("Import"), "import pill label: {text}");
        assert!(
            text.contains("Arterm subscription"),
    ...
    // 5. Suggestions (resting state).
    {
        ...
        assert!(text.contains("Welcome to arterm onboarding"), "{text}");
    }
}
...
fn onboarding_golden_telemetry_settings_page() {
    ...
    assert!(text.contains("Helps arterm the most"), "caption 1: {text}");
    assert!(
