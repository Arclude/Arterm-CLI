...
//   cargo test -p arterm-tui onboarding_eval_scorecard -- --nocapture
//
...
/// real app. `with_temp_arterm_home` must already be active.
...
fn looks_like_instruction(line: &str) -> bool {
    ...
    lower.starts_with("press ")
        ...
        // A "<command>" CTA: arterm phrases these as both "run /login" and
...
fn action_row_follows_prose() -> bool {
    ...
    let prose_idx = lines.iter().position(|l| {
        ...
        lc.contains("welcome to arterm") || lc.contains("log in")
    });
