...
fn test_recover_session_without_tools_preserves_debug_and_canary_flags() {
    ...
    app.session.working_dir = Some("/tmp/arterm-test".to_string());
    ...
    assert_eq!(app.session.working_dir.as_deref(), Some("/tmp/arterm-test"));
...
fn test_selfdev_command_spawns_session_in_test_mode() {
    ...
    let prev_test = std::env::var_os("ARTERM_TEST_SESSION");
    crate::env::set_var("ARTERM_HOME", temp_home.path());
    crate::env::set_var("ARTERM_TEST_SESSION", "1");

    let repo = create_arterm_repo_fixture();
    let mut app = create_test_app();
    ...
    let sessions_dir = crate::storage::arterm_dir().unwrap().join("sessions");
    ...
    if let Some(prev_test) = prev_test {
        crate::env::set_var("ARTERM_TEST_SESSION", prev_test);
    } else {
        crate::env::remove_var("ARTERM_TEST_SESSION");
    }
}
...
fn test_restore_reload_state_supports_legacy_input_format() {
    ...
    let arterm_dir = crate::storage::arterm_dir().unwrap();
