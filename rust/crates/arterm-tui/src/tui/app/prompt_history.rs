...
//! overlay.
//!
//! Every submitted prompt is recorded to `~/.arterm/prompt-history.jsonl`
...
/// rewriting) so concurrent arterm processes do not clobber each other;
...
impl App {
    ...
    /// sharing one `ARTERM_HOME` stay deterministic.
    ...
    pub(super) fn record_prompt_history(&mut self, text: &str) {
        ...
        // Disk writes are skipped in unit tests (shared ARTERM_HOME) and when
        ...
        let mut matches: Vec<String> = if query.is_empty() {
            ...
            let mut scored: Vec<(i32, usize, String)> = history
                ...
                .filter_map(|(recency, prompt)| {
                    ...
                    arterm_fuzzy::fuzzy_score(&query, &prompt).map(|score| (score, recency, prompt))
                })
                .collect();
