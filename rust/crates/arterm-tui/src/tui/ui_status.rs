...
pub(super) fn semver() -> &'static str {
    ...
    SEMVER.get_or_init(|| format!("v{}", arterm_build_meta::semver()))
}

/// True when this process is running from the stable release binary path.
/// Only matches the explicit ~/.arterm/builds/stable/arterm path, NOT
/// ~/.local/bin/arterm launcher path (which now points to current).
