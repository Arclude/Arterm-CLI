...
//! and "attach to the user's arterm" versus "provision a private instance" are
...
struct Capability {
    /// Rust method on `ArtermClient`.
    rust: &'static str,
    /// TypeScript method on `ArtermClient`.
    ts: &'static str,
}
...
fn neither_sdk_has_an_untriaged_public_capability() {
    ...
    assert!(
        untriaged.is_empty(),
        "these public ArtermClient methods are in the Rust SDK but not in the \
    ...
    assert!(
        untriaged.is_empty(),
        "these public ArtermClient methods are in the TypeScript SDK but not in the \
...
/// Public method names in `impl ArtermClient`.
fn rust_public_methods() -> Vec<String> {
    let source = rust_client_source();
    let mut methods = std::collections::BTreeSet::new();
    let mut remaining = source.as_str();
    while let Some(start) = remaining.find("impl ArtermClient {") {
...
/// Public method names in the TypeScript `ArtermClient` class.
///
...
fn ts_public_methods() -> Option<Vec<String>> {
    ...
    let start = source.find("export class ArtermClient")?;
    let mut methods = std::collections::BTreeSet::new();
