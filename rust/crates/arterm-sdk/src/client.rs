...
//!
//! Mirrors the TypeScript SDK's `ArtermClient` capability for capability. The
            ...
            Error::new(
                ...
                match cause.kind() {
                    std::io::ErrorKind::NotFound => format!(
                        "no harness API socket at {}: the arterm harness is not running. \
                         Start it with `arterm serve` and `arterm-harness-api-bridge`, or \
...
/// Discovery and buffering controls for [`ArtermClient::global_events`].
...
struct GlobalEventControl {
    ...
    children: Mutex<HashMap<String, ArtermClient>>,
...
pub struct ArtermClient {
    ...
    /// Server identity from the handshake, e.g. "arterm-harness-api-bridge/0.1.0".
    pub server: String,
    /// Capability strings advertised by the server.
    pub capabilities: Vec<String>,
}

impl Clone for ArtermClient {
    fn clone(&self) -> Self {
...
impl ArtermClient {
    /// Start and own a private arterm instance, then connect to it.
    ///
    /// Its state and sockets are isolated from the user's interactive arterm.
    ...
    pub fn launch(options: LaunchOptions) -> Result<Self> {
        ...
        client.instance_home = Some(instance.arterm_home.clone());
        client.instance = Some(instance);
        Ok(client)
    }

    /// Connect to the arterm running on this machine.
    ///
    /// Use this to automate the user's own arterm: a desktop app, an editor
    ...
    /// Persist an API key in arterm's owner-only provider store and hot-reload
...
/// Runtime identity and protocol metadata returned by [`ArtermClient::get_runtime_info`].
...
/// Content and truncation metadata returned by [`ArtermClient::read_file`].
...
/// Optional constraints for [`ArtermClient::search_text`].
...
/// Safe filesystem metadata returned by [`ArtermClient::file_status`].
#[derive(Debug, Clone, PartialEq, Eq)]
...
fn start_global_child(parent: &ArtermClient, control: &Arc<GlobalEventControl>, session_id: String) {
    ...
    let child = match ArtermClient::connect(ConnectOptions {
    ...
    if let Err(error) = child.attach_session(&session_id) {
        if !matches!(
            error.kind,
            ErrorKind::Harness(arterm_harness_api::ErrorCode::UnknownSession)
        ) {
            stop_global_stream(control, Some(error));
        }
        return;
