...
pub fn atomic_symlink_swap(src: &Path, dst: &Path, temp: &Path) -> std::io::Result<()> {
    ...
    #[cfg(windows)]
    {
        ...
        let old = dst.parent().unwrap_or_else(|| Path::new(".")).join(format!(
            ".arterm-launcher-old-{operation_id}{}",
            dst.extension()
...
mod tests {
    ...
    fn windows_swap_replaces_existing_launcher_via_staged_file() {
        ...
        let dst = dir.path().join("arterm.exe");
        let temp = dir.path().join(".arterm-current");
        ...
        assert!(
            !dir.path()
                .join(format!(".arterm-launcher-old-{}.exe", std::process::id()))
                .exists()
        );
    }
}
