...
//!
//! When a user asks arterm to change a setting, the agent writes
//! `~/.arterm/config.toml` and then has to guess whether the change took
...
///
/// Compares resolved paths so `~/.arterm/config.toml`, a relative path, and a
/// symlinked arterm home all resolve to the same file.
...
pub fn config_edit_notice(path: &Path, before: &str, after: &str) -> Option<String> {
    ...
    if let Err(error) = crate::config::Config::load_strict() {
        return Some(format!(
            "\n\nWARNING: {} no longer parses as TOML, so arterm is falling back to \
