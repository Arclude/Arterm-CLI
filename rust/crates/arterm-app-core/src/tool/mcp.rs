    ...
    async fn list_servers(&self) -> Result<ToolOutput> {
        ...
        if servers.is_empty() && configured.is_empty() {
            return Ok(ToolOutput::new(
                ...
                Or add servers to ~/.arterm/mcp.json or .arterm/mcp.json and use {\"action\": \"reload\"}.\n\
...
mod tests {
    ...
    fn create_test_tool() -> McpManagementTool {
        ...
        // would load the developer's real ~/.arterm/mcp.json, and list output
    ...
    impl LocalMcpConfigGuard {
        fn new(content: &str) -> std::io::Result<Self> {
            let path = PathBuf::from(".arterm/mcp.json");
            let dir = path
                .parent()
    ...
    async fn test_reload_empty_config() {
        let _guard =
            LocalMcpConfigGuard::new("{\"servers\":{}}").expect("create temporary .arterm/mcp.json");
        let tool = create_test_tool();
        let ctx = create_test_context();
