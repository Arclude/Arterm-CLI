...
impl SelfDevTool {
  ...
  case "${{1:-}}" in
    test|check|build|clippy|bench)
      if [[ "${{ARTERM_IN_DEV_CARGO:-0}}" == "1" || -z "${{ARTERM_DEV_CARGO_SCRIPT:-}}" ]]; then
        command cargo "$@"
      else
...
{command}"#
            ...
            if result.error.is_none() {
                match build::ensure_source_state_matches(&repo_dir, &expected_source) {
                    Ok(source_after_build) => {
                        ...
                        let builds_desktop2 = command.display.contains("-p arterm-desktop2");
    ...
    fn desktop_binary_name(command: &SelfDevBuildCommand) -> Option<&'static str> {
        if command.display.contains("-p arterm ") {
            return None;
        }
        if command.display.contains("-p arterm-desktop2") {
    ...
    fn validate_desktop_selfdev_binary(
        ...
        if binary_name.starts_with("arterm-desktop2") {
            let output = std::process::Command::new(&binary)
                .arg("--check-connect")
                .env("ARTERM_NON_INTERACTIVE", "1")
                .output()?;
    ...
    pub(super) async fn do_build(
        ...
        let repo_dir =
            SelfDevTool::resolve_repo_dir(ctx.working_dir.as_deref()).ok_or_else(|| {
                anyhow::anyhow!("Could not find the arterm repository directory for selfdev build")
            })?;
    ...
    pub(super) async fn do_test(
        ...
        let repo_dir =
            SelfDevTool::resolve_repo_dir(ctx.working_dir.as_deref()).ok_or_else(|| {
                anyhow::anyhow!("Could not find the arterm repository directory for selfdev test")
            })?;
        let requested_source = SelfDevTool::requested_source_state(&repo_dir)?;
...
mod desktop_binary_tests {
    ...
    fn each_desktop_build_validates_its_own_binary() {
        let desktop2 = SelfDevTool::desktop_binary_name(&command(
            "scripts/dev_cargo.sh build --profile selfdev -p arterm-desktop2 --bin arterm-desktop2 --lib",
        ));
        assert!(
            desktop2.is_some_and(|name| name.starts_with("arterm-desktop2")),
    ...
    fn tui_and_combined_builds_are_not_desktop_only() {
        for display in [
            "scripts/dev_cargo.sh build --profile selfdev -p arterm --bin arterm",
            "scripts/dev_cargo.sh build --profile selfdev -p arterm --bin arterm -p arterm-desktop2",
        ] {
            assert_eq!(
