...
fn with_temp_home<T>(f: impl FnOnce(&Path) -> T) -> T {
    ...
    let previous_home = std::env::var("ARTERM_HOME").ok();
...
#[test]
#[ignore = "local performance benchmark over the real ~/.arterm session corpus"]
fn bench_real_session_search_corpus() {
    if std::env::var("ARTERM_SESSION_SEARCH_BENCH_REAL")
        .ok()
        .as_deref()
        != Some("1")
    {
        eprintln!("set ARTERM_SESSION_SEARCH_BENCH_REAL=1 to run against the real session corpus");
        return;
    }
    ...
    for query in ["session_search", "optimization", "nonexistentneedle123"] {
        ...
        eprintln!(
            ...
            report.scanned_arterm_sessions,
            report.candidate_arterm_sessions,
            report.results.len(),
            report.truncated
        );
    }
    ...
    for query in ["session_search", "nonexistentneedle123"] {
        ...
        .expect("search succeeds");
        eprintln!(
            "BENCH_EXTERNAL query={query} elapsed_ms={} scanned_arterm={} scanned_external={} sources={:?} results={} truncated={}",
            start.elapsed().as_millis(),
            report.scanned_arterm_sessions,
...
#[test]
fn external_codex_sessions_are_searchable_without_arterm_session_dir() {
    with_temp_home(|home| {
        ...
        std::fs::remove_dir_all(home.join("sessions")).expect("remove arterm sessions dir");
        ...
        assert_eq!(report.scanned_arterm_sessions, 0);
...
#[test]
fn external_cursor_sessions_are_searchable_without_arterm_session_dir() {
    with_temp_home(|home| {
