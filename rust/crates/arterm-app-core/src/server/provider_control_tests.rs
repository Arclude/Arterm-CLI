...
impl EnvGuard {
    /// Save and clear the given env vars, and redirect `ARTERM_HOME` to a fresh
    /// empty temp dir for the lifetime of the guard.
    ///
    /// The temp home keeps these tests hermetic: provider activation reads
    /// on-disk model catalog caches (`~/.arterm/cache/<profile>_models.json`) to
    ...
    fn save(keys: &[&'static str]) -> Self {
        ...
        if !all_keys.contains(&"ARTERM_HOME") {
            all_keys.push("ARTERM_HOME");
        }
        let saved = all_keys
            .iter()
        ...
        let temp_home = tempfile::tempdir().expect("create temp ARTERM_HOME");
