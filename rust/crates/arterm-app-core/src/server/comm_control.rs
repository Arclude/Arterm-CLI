...
use arterm_agent_runtime::SoftInterruptSource;
...
fn deep_mode_assignment_content(
    ...
    if is_gate {
        ...
        let low_confidence_siblings: Vec<String> =
            arterm_plan::bridge::low_confidence_completed_ids(plan)
                .into_iter()
                .filter(|id| audited.contains(id.as_str()))
                .collect();
        arterm_swarm_core::append_deep_gate_instructions(
            content,
            item_id,
            &audited_ids,
        ...
        arterm_swarm_core::append_deep_node_instructions(content, item_id)
    }
}
...
async fn task_snapshot_for(
    ...
    let hydrated = arterm_plan::bridge::hydrate_assignment(plan, task_id, &item.content);
...
async fn handle_comm_assign_task_with_mode(
    ...
    let (selected_task_id, task_content, participant_ids, plan_item_count, blocked_reason) = {
        ...
        if let Some(found_idx) = found_idx {
            ...
            let hydrated =
                arterm_plan::bridge::hydrate_assignment(plan, &item_id, &effective_content);
            let content =
...
async fn require_plan_driver_swarm(
    ...
    let is_deep_participant = {
        ...
        plans
            .get(&swarm_id)
            .map(|plan| {
                arterm_plan::bridge::parse_mode(&plan.mode) == arterm_plan::dag::Mode::Deep
