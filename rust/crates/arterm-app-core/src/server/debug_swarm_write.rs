                ...
                Some(proposal) => {
                    if let Ok(items) = serde_json::from_str::<Vec<PlanItem>>(&proposal) {
                        let version = {
                            ...
                            if merged_count > arterm_plan::MAX_PLAN_ITEMS {
                                return Err(anyhow::anyhow!(
                                    "Plan approval would contain {} items, exceeding the per-swarm limit of {}",
                                    merged_count,
                                    arterm_plan::MAX_PLAN_ITEMS
                                ));
                            }
...
fn debug_specs(specs: Vec<DebugNodeSpec>) -> Vec<arterm_plan::dag::NodeSpec> {
    specs
        .into_iter()
        .map(|s| arterm_plan::dag::NodeSpec {
            id: Some(s.id),
            content: s.content,
            kind: arterm_plan::bridge::parse_kind(s.kind.as_deref()),
...
async fn handle_debug_graph_op(arg: &str, ctx: &DebugSwarmWriteContext<'_>) -> String {
    use arterm_plan::bridge::{apply_task_graph, to_task_graph};
    use arterm_plan::dag;
...
mod tests {
    ...
    fn isolated_runtime(dir: &tempfile::TempDir) -> EnvGuard {
        ...
        let runtime = std::env::var_os("ARTERM_RUNTIME_DIR");
        crate::env::set_var("ARTERM_RUNTIME_DIR", dir.path());
        EnvGuard {
            _lock: lock,
            runtime,
        }
