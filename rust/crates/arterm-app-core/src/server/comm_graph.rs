...
//! `arterm_plan::dag`. Each handler lifts the swarm's current `VersionedPlan` into
//! a `TaskGraph` (via `arterm_plan::bridge`), applies the engine op (which enforces
...
use arterm_plan::MAX_PLAN_ITEMS;
use arterm_plan::bridge::{apply_task_graph, parse_kind, to_task_graph};
use arterm_plan::dag::{self, HandoffArtifact, NodeSpec, NodeStatus, TaskGraph};
