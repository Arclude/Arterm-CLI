...
fn main() {
    ...
    let mut generated = String::from("pub(crate) static ARTERM_DOCS: &[(&str, &str)] = &[\n");
    for path in files {
        let relative = path
            .strip_prefix(&repo)
    ...
    let out = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR")).join("arterm_docs.rs");
    fs::write(out, generated).expect("write generated Arterm documentation corpus");
}
