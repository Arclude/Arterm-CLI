...
use arterm_provider_core::anthropic_map_tool_name_for_oauth as map_tool_name_for_oauth;
use serde::Serialize;
...
pub fn format_messages(messages: &[Message], is_oauth: bool) -> Vec<ApiMessage> {
    ...
    // arterm never intends to prefill, so a trailing assistant turn here is always
...
/// `arterm-schema-dialect` so every provider shares one implementation and one
...
fn anthropic_input_schema(schema: &Value) -> Value {
    arterm_schema_dialect::normalize(schema, &arterm_schema_dialect::registry::ANTHROPIC)
}
...
pub fn add_message_cache_breakpoint(messages: &mut [ApiMessage], cache_ttl_1h: bool) {
    ...
    if messages.len() < 3 {
        ...
        arterm_logging::info("Conversation caching: too few messages, skipping");
        return;
    }
    ...
    if assistant_indices.is_empty() {
        arterm_logging::info("Conversation caching: no assistant message found");
        return;
    }
...
mod cache_prefix_invariant_tests {
    ...
    use super::*;
    use arterm_message_types::{ContentBlock, Message, Role};
