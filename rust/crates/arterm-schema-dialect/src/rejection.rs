...
//! construct, but it cannot be complete: a provider may reject something arterm
...
//! same turn recover, and lets arterm report exactly which keyword to add.
//!
...
pub fn classify(message: &str) -> Option<SchemaRejection> {
    ...
    // reaching here means a schema arterm did not expect.
...
/// avoid a pointless retry when the rejection names something arterm did not
