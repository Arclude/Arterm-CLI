...
use arterm_schema_dialect::{RecoveryAction, quirks, registry};
...
fn each_recovering_dialect_learns_retries_once_then_refuses_to_loop() {
    for id in DIALECTS_WITH_RUNTIME_RECOVERY {
        ...
        match arterm_schema_dialect::recover_from_error(&rejection, spec) {
        ...
        assert!(
            matches!(
                arterm_schema_dialect::recover_from_error(&rejection, spec),
...
fn no_dialect_treats_an_operational_failure_as_a_schema_rejection() {
    ...
    for spec in registry::ALL {
        for message in operational {
            assert_eq!(
                arterm_schema_dialect::recover_from_error(message, spec),
...
const RUNTIME_RECOVERY_EXPECTATIONS: &[(&str, bool)] = &[
    ("../arterm-provider-gemini-runtime/src/lib.rs", true),
    ("../arterm-provider-antigravity-runtime/src/lib.rs", true),
    ...
    (
        "../arterm-provider-openai-runtime/src/openai_provider_impl.rs",
        true,
    ),
    // Still unhandled. Both forward to upstreams whose rejection texts arterm has
    ...
    ("../arterm-provider-openrouter-runtime/src/lib.rs", false),
    ("../arterm-provider-anthropic-runtime/src/lib.rs", false),
];
