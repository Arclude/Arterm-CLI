...
pub(super) fn telemetry_id_path() -> Option<PathBuf> {
    storage::arterm_dir().ok().map(|d| d.join("telemetry_id"))
}
...
pub(super) fn telemetry_state_path(name: &str) -> Option<PathBuf> {
    storage::arterm_dir().ok().map(|d| d.join(name))
}
...
pub(super) fn version() -> String {
    arterm_build_meta::pkg_version().to_string()
}
...
pub(super) fn is_arterm_repo_dir(dir: &Path) -> bool {
    ...
    std::fs::read_to_string(cargo_toml)
        .map(|content| content.contains("name = \"arterm\""))
        .unwrap_or(false)
}

fn find_arterm_repo_in_ancestors(start: &Path) -> Option<PathBuf> {
    start
        .ancestors()
        .find(|dir| is_arterm_repo_dir(dir))
        .map(Path::to_path_buf)
}

fn telemetry_arterm_repo_dir() -> Option<PathBuf> {
    if let Ok(path) = std::env::var("ARTERM_REPO_DIR") {
        let path = PathBuf::from(path);
        if is_arterm_repo_dir(&path) {
            return Some(path);
        }
    }

    let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    if let Some(repo) = find_arterm_repo_in_ancestors(&manifest_dir) {
        return Some(repo);
    }
    ...
    if let Ok(exe) = std::env::current_exe()
        && let Some(repo) = exe
            ...
            .and_then(Path::parent)
            .filter(|dir| is_arterm_repo_dir(dir))
    {
    ...
    std::env::current_dir()
        .ok()
        .and_then(|cwd| find_arterm_repo_in_ancestors(&cwd))
}

pub(super) fn build_channel() -> String {
    if std::env::var(arterm_selfdev_types::CLIENT_SELFDEV_ENV).is_ok() {
    ...
    if telemetry_arterm_repo_dir().is_some() {
...
pub(super) fn is_git_checkout() -> bool {
    telemetry_arterm_repo_dir().is_some()
}
...
pub(super) fn is_ci() -> bool {
    ...
    std::env::var("NEXTEST").is_ok() || std::env::var("ARTERM_E2E_BIN").is_ok()
}
