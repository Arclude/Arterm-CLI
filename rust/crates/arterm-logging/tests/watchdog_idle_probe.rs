...
fn idle_process_is_not_reported_as_stalled() {
    let dir = std::env::temp_dir().join(format!("arterm-wd-idle-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    unsafe {
        std::env::set_var("HOME", &dir);
        std::env::set_var("ARTERM_WATCHDOG_STALL_SECS", "3");
        std::env::set_var("ARTERM_WATCHDOG_HEARTBEAT_SECS", "0");
    }

    arterm_logging::init();
    arterm_logging::watchdog::beat("idle.probe");
    // No work guard: the process is idle, not working.
    std::thread::sleep(Duration::from_secs(15));

    let contents = arterm_logging::log_path()
