...
//!
//! Run with: cargo test -p arterm-logging --test watchdog_stall_probe -- --ignored
...
fn stalled_process_emits_watchdog_stall_event() {
    let dir = std::env::temp_dir().join(format!("arterm-wd-probe-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    unsafe {
        std::env::set_var("HOME", &dir);
        std::env::set_var("ARTERM_WATCHDOG_STALL_SECS", "5");
        std::env::set_var("ARTERM_WATCHDOG_HEARTBEAT_SECS", "5");
    }

    arterm_logging::init();
    let _work = arterm_logging::watchdog::begin_work("probe.phase");
    arterm_logging::watchdog::set_detail("probe detail");
    ...
    while Instant::now() < deadline {
        ...
        let Some(path) = arterm_logging::log_path() else {
            continue;
        };
