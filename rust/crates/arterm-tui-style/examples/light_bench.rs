...
//! analyzer. Run with:
//!   cargo run -p arterm-tui-style --example light_bench

use arterm_tui_style::palette::{ALL_ROLES, Palette, Role, parse_hex, to_hex};
use arterm_tui_style::{HarmonyReport, Oklab, analyze_harmony};
...
fn main() {
    ...
    let generated =
        arterm_tui_style::harmony::generate_from_seed(Role::User.default_rgb(), LIGHT_BG);
    let tuned = tune(&hand_tuned());

    report("derived (runtime flip of dark defaults)", &derived);
    println!();
    report("generated (generate_from_seed, arterm blue)", &generated);
    println!();
