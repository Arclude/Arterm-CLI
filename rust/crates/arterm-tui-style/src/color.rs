...
/// and Apple Terminal; see `arterm_app_core::perf` and issue #330). These
/// renderers key their rasterized-glyph cache on the full 24-bit color, so the
/// continuous color animations arterm emits (shimmer, rainbow, pulsing tool
...
/// full truecolor.
///
/// Overridable with `ARTERM_GLYPH_SAFE_MODE=on|off` (shared with the perf
...
fn fragile_glyph_cache_terminal() -> bool {
    ...
    // Mirror of `arterm_app_core::perf::detect_terminal` for the two affected
...
mod tests {
    ...
    fn test_fragile_terminal_override_off_forces_truecolor() {
        ...
        temp_env_scope(
            &[
                ("ARTERM_GLYPH_SAFE_MODE", Some("off")),
    ...
    fn test_fragile_terminal_override_on_forces_quantize() {
        temp_env_scope(&[("ARTERM_GLYPH_SAFE_MODE", Some("on"))], || {
            assert!(fragile_glyph_cache_terminal());
        });
    }
    ...
    fn test_detect_color_capability_downgrades_on_fragile_override() {
        temp_env_scope(
            &[
                ("ARTERM_GLYPH_SAFE_MODE", Some("on")),
