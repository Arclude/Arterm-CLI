#!/usr/bin/env python3
"""Benchmark and optionally regression-check arterm startup time.

This script runs isolated startup measurements under a temporary ARTERM_HOME and
ARTERM_RUNTIME_DIR so it does not interfere with the user's real server, logs, or
credentials.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("binary", nargs="?", default="./target/release/arterm")
...
def measure_server_startup(binary: str, runs: int) -> list[float]:
    times: list[float] = []
    for _ in range(runs):
        root = tempfile.mkdtemp(prefix="arterm-server-bench-")
        env = isolated_env(root)
        socket_path = env["ARTERM_SOCKET"]
        proc = None
        try:
            start = time.perf_counter()
