...
DEFAULT_SOCKET = f"/run/user/{os.getuid()}/arterm.sock"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Summarize arterm server/client process memory using smaps_rollup")
    p.add_argument("--json", action="store_true", help="Print JSON instead of a human summary")
    p.add_argument(
        "--include-aux",
        action="store_true",
        help="Include non-default-socket helper/test arterm processes in the output",
    )
    p.add_argument(
        "--socket",
        default=DEFAULT_SOCKET,
        help=f"Main arterm socket to treat as the primary instance (default: {DEFAULT_SOCKET})",
    )
    return p.parse_args()
...
def classify_process(argv: list[str], cmd: str, main_socket: str) -> tuple[str | None, bool]:
    ...
    if not argv0.startswith("arterm"):
        return None, False

    if "arterm serve" in cmd:
    ...
    if "cargo build" in cmd or "rustc --crate-name arterm" in cmd or "handle_resume_session" in cmd:
        return None, False
...
def iter_arterm_processes(main_socket: str, include_aux: bool) -> Iterable[ProcMem]:
...
def main() -> int:
    args = parse_args()
    procs = list(iter_arterm_processes(args.socket, args.include_aux))
