...
def build_specs() -> dict[str, ToolSpec]:
    arterm = shutil.which("arterm") or str(Path.home() / ".local/bin/arterm")
    ...
    specs = {
        "arterm_memory_off": ToolSpec(
            name="arterm_memory_off",
            argv=[arterm, "--no-update", "--no-selfdev"],
            version_argv=[arterm, "version"],
            env={"ARTERM_NO_TELEMETRY": "1", "ARTERM_MEMORY_ENABLED": "0"},
            arterm=True,
        ),
        "arterm_memory_on": ToolSpec(
            name="arterm_memory_on",
            argv=[arterm, "--no-update", "--no-selfdev"],
            version_argv=[arterm, "version"],
            env={"ARTERM_NO_TELEMETRY": "1", "ARTERM_MEMORY_ENABLED": "1"},
            arterm=True,
        ),
        "pi": ToolSpec(
            name="pi",
...
def run_tool(spec: ToolSpec, sessions: int, cwd: Path, timeout_s: float, settle_s: float) -> ToolRunResult:
    ...
    try:
        if spec.arterm:
            temp_root = tempfile.mkdtemp(prefix="arterm-memory-bench-")
            env = os.environ.copy()
            if spec.env:
                env.update(spec.env)
            env["ARTERM_HOME"] = os.path.join(temp_root, "home")
            env["ARTERM_RUNTIME_DIR"] = os.path.join(temp_root, "run")
            ...
            for auth_name in (
                ...
                real_auth = Path.home() / ".arterm" / auth_name
                bench_auth = Path(env["ARTERM_HOME"]) / auth_name
                if real_auth.exists() and not bench_auth.exists():
                    bench_auth.symlink_to(real_auth)
            if spec.name == "arterm_memory_on":
                real_models = Path.home() / ".arterm" / "models"
                bench_models = Path(env["ARTERM_HOME"]) / "models"
                if real_models.exists() and not bench_models.exists():
                    bench_models.symlink_to(real_models)
            socket_path = os.path.join(env["ARTERM_RUNTIME_DIR"], "bench.sock")
            ...
            if not wait_for_socket(socket_path, timeout_s):
                raise RuntimeError("arterm server did not become ready")
            if spec.name == "arterm_memory_on":
                time.sleep(max(settle_s, 5.0))
            per_session_settle = max(settle_s, 2.0) if spec.name == "arterm_memory_on" else settle_s
