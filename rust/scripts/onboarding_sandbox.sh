...
sandbox_name=${ARTERM_ONBOARDING_SANDBOX:-default}
if [[ ! "$sandbox_name" =~ ^[A-Za-z0-9][A-Za-z0-9._-]*$ || "$sandbox_name" == "." || "$sandbox_name" == ".." ]]; then
  echo "Invalid onboarding sandbox name: $sandbox_name" >&2
  exit 2
fi
scratch_root=${ARTERM_SCRATCH_DIR:-$HOME/.arterm/scratch}
sandbox_parent="$scratch_root/onboarding"
sandbox_root_default="$sandbox_parent/$sandbox_name"
sandbox_root=${ARTERM_ONBOARDING_DIR:-$sandbox_root_default}
arterm_home="$sandbox_root/home"
runtime_dir="$sandbox_root/runtime"
marker_file="$sandbox_root/.arterm-onboarding-sandbox"

ensure_dirs() {
  mkdir -p "$arterm_home" "$runtime_dir"
  chmod 700 "$sandbox_root" "$arterm_home" "$runtime_dir"
  : > "$marker_file"
...
run_in_sandbox() {
  ...
  (
    ...
    # ARTERM_CLIENT_SELFDEV_MODE; it cannot unset one inherited from a parent
    ...
    env -u ARTERM_CLIENT_SELFDEV_MODE -u ARTERM_SELFDEV -u ARTERM_CANARY \
      ARTERM_HOME="$arterm_home" \
      ARTERM_RUNTIME_DIR="$runtime_dir" \
      "$@"
  )
}
...
Commands:
  ...
  arterm [args...]        Run arterm inside the sandbox
  auth-status            Run 'arterm auth status' inside the sandbox
  fresh [args...]        Reset sandbox, then launch arterm with args
  ...
  fresh-real [--with-transcripts]
                         Reset sandbox, seed your real logins, then launch arterm
  login <provider> ...   Run 'arterm --provider <provider> login ...' in sandbox
  ...
  fixture-run <name> -- [args...]
                         Load a fixture, then run arterm with args
  help                   Show this help

Environment overrides:
  ARTERM_ONBOARDING_SANDBOX   Sandbox name (default: default)
  ARTERM_ONBOARDING_DIR       Explicit sandbox directory
  ARTERM_AUTH_FIXTURE_DIR     Fixture store (default: .tmp/auth-fixtures)
  ARTERM_ONBOARDING_KEEP_EXTERNAL=1
...
print_env() {
  ensure_dirs
  cat <<EOF
export ARTERM_HOME="$arterm_home"
export ARTERM_RUNTIME_DIR="$runtime_dir"
EOF
}
...
status() {
  ...
  echo "ARTERM_HOME:   $arterm_home"
  echo "RUNTIME_DIR:  $runtime_dir"
  echo

  if [[ -d "$arterm_home" ]]; then
    echo "Home contents:"
    find "$arterm_home" -maxdepth 3 \( -type f -o -type d \) | sed "s#^$sandbox_root#.#" | sort
  fi

  if [[ -d "$arterm_home/external" ]]; then
    echo
...
purge_external() {
  if [[ -d "$arterm_home/external" ]]; then
    rm -rf "$arterm_home/external"
    echo "Removed copied external credentials/transcripts: $arterm_home/external"
  else
...
open_shell() {
  ...
  echo "  ARTERM_HOME=$arterm_home"
  echo "  ARTERM_RUNTIME_DIR=$runtime_dir"
  env ARTERM_HOME="$arterm_home" ARTERM_RUNTIME_DIR="$runtime_dir" bash --noprofile --norc
}

run_arterm() {
  # The sandbox should behave like a real standalone install, not a self-dev
  # client. Because we launch from inside the repo, arterm would otherwise
  ...
  # spawning its own server under the sandbox's ARTERM_RUNTIME_DIR. Set
  # ARTERM_SANDBOX_SELFDEV=1 to opt back into the shared-server behavior.
  local prefix=()
  if [[ "${ARTERM_SANDBOX_SELFDEV:-0}" != "1" ]]; then
    prefix=(--no-selfdev)
  fi
  ...
  if [[ -n "${ARTERM_SANDBOX_BIN:-}" ]]; then
    if [[ -x "$ARTERM_SANDBOX_BIN" ]]; then
      run_in_sandbox "$ARTERM_SANDBOX_BIN" "${prefix[@]}" "$@"
      return
    fi
    echo "ARTERM_SANDBOX_BIN=$ARTERM_SANDBOX_BIN is not executable" >&2
    return 1
  fi
  local binary_path="$repo_root/target/debug/arterm"
  if [[ -x "$binary_path" ]]; then
    run_in_sandbox "$binary_path" "${prefix[@]}" "$@"
  else
    run_in_sandbox cargo run --bin arterm -- "${prefix[@]}" "$@"
  fi
}

run_auth_fixture() {
  ARTERM_ONBOARDING_SANDBOX="$sandbox_name" \
    ARTERM_ONBOARDING_DIR="$sandbox_root" \
    "$repo_root/scripts/auth_fixture.sh" "$@"
}

# Copy one real file from $HOME into the sandbox's external/ tree, preserving its
# relative path. arterm resolves every external credential/transcript lookup to
# $ARTERM_HOME/external/<same-relative-path-as-$HOME> when ARTERM_HOME is set, so
# seeding here makes your real logins/transcripts visible to the onboarding
# import + continue steps. Copies (never symlinks: arterm rejects symlinked auth
# files) and never touches the originals.
seed_one_file() {
  local rel=$1
  local src="$HOME/$rel"
  local dst="$arterm_home/external/$rel"
  if [[ ! -e "$src" ]]; then
    return 1
  fi
  mkdir -p "$(dirname "$dst")"
  cp -a "$src" "$dst"
  chmod -R go-rwx "$arterm_home/external" 2>/dev/null || true
  return 0
}
...
case "$command" in
  ...
  fresh-real)
    ...
    seed_real_logins "$@"
    echo
    echo "Launching sandbox arterm with your real logins available to import..."
    if run_arterm; then
      rc=0
    else
      rc=$?
    fi
    if [[ "${ARTERM_ONBOARDING_KEEP_EXTERNAL:-0}" != "1" ]]; then
      purge_external
    else
      echo "Keeping copied external credentials because ARTERM_ONBOARDING_KEEP_EXTERNAL=1."
    fi
    exit "$rc"
    ;;
  login)
    ...
    provider=$1
    shift
    run_arterm --provider "$provider" login "$@"
    ;;
  fixture-list)
