...
$originalImportOnly = $env:ARTERM_INSTALL_PS1_IMPORT_ONLY
$testRoot = Join-Path $env:TEMP ("arterm-windows-launcher-tests-{0}" -f ([guid]::NewGuid().ToString('N')))
...
function Assert-PathCount([string]$PathValue, [string]$Entry, [int]$ExpectedCount, [string]$Message) {
    $entryKey = ConvertTo-ArtermPathKey $Entry
    $count = 0
    foreach ($candidate in (Split-ArtermPathList $PathValue)) {
        if ((ConvertTo-ArtermPathKey $candidate) -eq $entryKey) { $count += 1 }
    }
...
try {
    ...
    $env:ARTERM_INSTALL_PS1_IMPORT_ONLY = '1'
    . $installScript

    $installDir = Join-Path $env:LOCALAPPDATA 'arterm\bin'
    $launcherPath = Join-Path $installDir 'arterm.exe'

    Write-Host 'test_launcher_path_localappdata'
    Assert-Equal $installDir (Get-DefaultArtermInstallDir) 'default installer path should live under LOCALAPPDATA\arterm\bin'
    Assert-Equal $launcherPath (Join-Path (Get-DefaultArtermInstallDir) 'arterm.exe') 'launcher path should be LOCALAPPDATA\arterm\bin\arterm.exe'
    ...
    $pathUpdate = Resolve-ArtermPathUpdate -InstallDir $installDir -CurrentPath $currentPath
    ...
    Assert-PathCount $pathUpdate.Path $installDir 1 'updated PATH should contain exactly one arterm launcher dir'
    Assert-Equal 2 $pathUpdate.RemovedManagedEntries 'path update should remove both stale arterm launcher entries before re-adding one'
    Assert-Equal 0 $pathUpdate.RemovedDuplicateEntries 'path update should preserve unrelated duplicate entries'
    $secondUpdate = Resolve-ArtermPathUpdate -InstallDir $installDir -CurrentPath $pathUpdate.Path
    ...
    $mockUpdate = Set-ArtermUserPath -InstallDir $installDir -CurrentPath 'C:\Tools' -SetUserPathAction $setPathAction -BroadcastAction $broadcastAction
    ...
    $noChangeUpdate = Set-ArtermUserPath -InstallDir $installDir -CurrentPath $script:appliedPath -SetUserPathAction $setPathAction -BroadcastAction $broadcastAction
    ...
    Assert-Equal 'v0.47.0' (ConvertFrom-ArtermVersionOutput 'arterm v0.47.0 (f7f5898c)') 'local artifact version parser should accept normal arterm --version output'
    Assert-Equal 'v0.47.0' (ConvertFrom-ArtermVersionOutput 'arterm collects anonymous usage statistics. arterm v0.47.0 (f7f5898c)') 'local artifact version parser should accept fresh-profile telemetry before version output'
    Assert-Equal $null (ConvertFrom-ArtermVersionOutput 'not a arterm binary') 'local artifact version parser should reject unrelated output'
    $freshProfileBinary = Join-Path $testRoot 'fresh-profile-version.cmd'
    Set-Content -LiteralPath $freshProfileBinary -Value "@echo off`r`n>&2 echo arterm collects anonymous usage statistics.`r`necho arterm v0.47.0 (f7f5898c)" -NoNewline
    Assert-Equal 'v0.47.0' (Get-ArtermVersionFromBinary $freshProfileBinary) 'binary version probe should tolerate a successful fresh-profile telemetry notice on stderr'

    Write-Host 'test_windows_architecture_detection_prefers_native_arm64'
    Assert-Equal 'arterm-windows-x86_64' (Resolve-ArtermWindowsArtifact @('X64', 'AMD64')) 'x64 Windows should select the x64 release asset'
    Assert-Equal 'arterm-windows-aarch64' (Resolve-ArtermWindowsArtifact @('Arm64')) 'native ARM64 Windows should select the ARM64 release asset'
    Assert-Equal 'arterm-windows-aarch64' (Resolve-ArtermWindowsArtifact @('X64', 'AMD64', 'ARM64')) 'emulated x64 PowerShell on Windows ARM64 should prefer the native ARM64 release asset'
    Assert-Equal $null (Resolve-ArtermWindowsArtifact @('x86', 'unknown')) 'unsupported architectures should not silently select an asset'
    ...
    $manifest = "$digest  nested/path/arterm-windows-x86_64.exe"
    $manifestBytes = [System.Text.Encoding]::UTF8.GetBytes($manifest)
    Assert-Equal $manifest (ConvertFrom-ArtermWebContent -Content $manifest) 'web response decoder should preserve string content'
    Assert-Equal $manifest (ConvertFrom-ArtermWebContent -Content $manifestBytes) 'web response decoder should decode Windows PowerShell 5.1 byte-array content as UTF-8'
    Assert-Equal $digest (Get-ArtermSha256FromManifest -ManifestText (ConvertFrom-ArtermWebContent -Content $manifestBytes) -AssetName 'arterm-windows-x86_64.exe') 'checksum parser should accept a manifest decoded from a byte-array web response'
    Assert-Equal $digest (Get-ArtermSha256FromManifest -ManifestText $manifest -AssetName 'arterm-windows-x86_64.exe') 'checksum parser should match release assets by file name'
    Assert-Equal $null (Get-ArtermSha256FromManifest -ManifestText $manifest -AssetName 'missing.exe') 'checksum parser should fail closed when the requested asset is absent'
    Assert-Equal $digest (Assert-ArtermFileChecksum -FilePath $checksumFile -ExpectedSha256 $digest -AssetName 'arterm-windows-x86_64.exe') 'checksum validation should accept the matching digest'
    $checksumThrew = $false
    try {
        Assert-ArtermFileChecksum -FilePath $checksumFile -ExpectedSha256 ('0' * 64) -AssetName 'arterm-windows-x86_64.exe' | Out-Null
    } catch {
        $checksumThrew = $true
    }
    ...
    $armManifest = "$digest  nested/path/arterm-windows-aarch64.exe"
    Assert-Equal $digest (Get-ArtermSha256FromManifest -ManifestText $armManifest -AssetName 'arterm-windows-aarch64.exe') 'checksum parser should match the Windows ARM64 release asset'
    ...
    Assert-True ($installText.Contains('"--locked", "-p", "arterm", "--bin", "arterm"')) 'source-build fallback should compile only the locked arterm binary target'

    Write-Host 'test_hotkey_shortcut_script_is_valid_powershell'
    $shortcutScript = Get-ArtermHotkeyShortcutScript -StartupShortcutPath "C:\Users\Test User\AppData\Roaming\arterm's hotkey.lnk" -ArtermExePath "C:\Program Files\arterm's bin\arterm.exe"
    ...
    Assert-True ($installText.Contains('ARTERM_WINDOWS_SETUP_SKIP_PROCESS_LIFECYCLE')) 'isolated verification should be able to create shortcut files without stopping or spawning real user listeners'

    Write-Host 'test_upgrade_replaces_launcher_no_extra_path'
    $sourceDir = Join-Path $testRoot 'sources'
    New-Item -ItemType Directory -Path $sourceDir -Force | Out-Null
    $sourceV1 = Join-Path $sourceDir 'arterm-v1.exe'
    $sourceV2 = Join-Path $sourceDir 'arterm-v2.exe'
    Set-Content -Path $sourceV1 -Value 'version-one' -NoNewline
    Set-Content -Path $sourceV2 -Value 'version-two' -NoNewline
    Install-ArtermLauncher -SourcePath $sourceV1 -LauncherPath $launcherPath | Out-Null
    Install-ArtermLauncher -SourcePath $sourceV2 -LauncherPath $launcherPath | Out-Null
    Assert-Equal 'version-two' (Get-Content -Path $launcherPath -Raw) 'upgrade should replace launcher contents with the new build'
    $tempLaunchers = @(Get-ChildItem -LiteralPath $installDir -Filter '.arterm-launcher-*.tmp.exe' -Force -ErrorAction SilentlyContinue)
    Assert-Equal 0 $tempLaunchers.Count 'launcher upgrade should clean temporary files'
    $upgradePath = Resolve-ArtermPathUpdate -InstallDir $installDir -CurrentPath $pathUpdate.Path
    ...
    $runningLauncher = Join-Path $runningDir 'arterm.exe'
    ...
    try {
        ...
        Install-ArtermLauncher -SourcePath $replacementLauncher -LauncherPath $runningLauncher | Out-Null
        ...
        $runningBackups = @(Get-ChildItem -LiteralPath $runningDir -Filter '.arterm-launcher-old-*.exe' -Force -ErrorAction SilentlyContinue)
    ...
    Remove-ArtermStaleLauncherBackups -LauncherDir $runningDir
    $runningBackups = @(Get-ChildItem -LiteralPath $runningDir -Filter '.arterm-launcher-old-*.exe' -Force -ErrorAction SilentlyContinue)
    ...
    $rollbackLauncher = Join-Path $rollbackDir 'arterm.exe'
    ...
    $rollbackThrew = $false
    try {
        Install-ArtermLauncher -SourcePath $rollbackSource -LauncherPath $rollbackLauncher | Out-Null
    } catch {
        $rollbackThrew = $true
    } finally {
    ...
    Assert-Equal 0 @(Get-ChildItem -LiteralPath $rollbackDir -Filter '.arterm-launcher-*.tmp.exe' -Force -ErrorAction SilentlyContinue).Count 'rollback should remove temporary launcher files'
    Assert-Equal 0 @(Get-ChildItem -LiteralPath $rollbackDir -Filter '.arterm-launcher-old-*.exe' -Force -ErrorAction SilentlyContinue).Count 'rollback should restore rather than retain the previous launcher backup'

    Write-Host 'test_uninstall_removes_launcher_and_only_arterm_path'
    $removeCurrentPath = "$installDir;C:\Keep;$installVariant;C:\Keep"
    $removeUpdate = Resolve-ArtermPathUpdate -InstallDir $installDir -CurrentPath $removeCurrentPath -RemoveOnly
    Assert-Equal 'C:\Keep;C:\Keep' $removeUpdate.Path 'uninstall path cleanup should remove only arterm-managed entries and preserve unrelated entries'
    Assert-Equal 2 $removeUpdate.RemovedManagedEntries 'uninstall path cleanup should remove all arterm launcher dir variants'
    Assert-PathCount $removeUpdate.Path $installDir 0 'uninstall path cleanup should leave no arterm launcher dir entries'
    ...
    if ($null -eq $originalImportOnly) { Remove-Item Env:ARTERM_INSTALL_PS1_IMPORT_ONLY -ErrorAction SilentlyContinue } else { $env:ARTERM_INSTALL_PS1_IMPORT_ONLY = $originalImportOnly }
