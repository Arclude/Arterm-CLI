...
Spawns the freshly built arterm TUI in a PTY, replies to terminal queries
...
color usage. On the white background run, arterm should detect a light theme
...
ARTERM = os.path.expanduser("~/.arterm/builds/current/arterm")
...
def run_capture(bg: str, seconds: float = 12.0) -> bytes:
    ...
    for k in ("ARTERM_THEME", "TERM_PROGRAM", "TERM_PROGRAM_VERSION", "TMUX", "STY", "SSH_TTY", "SSH_CONNECTION", "SSH_CLIENT", "KITTY_WINDOW_ID", "GHOSTTY_RESOURCES_DIR", "WEZTERM_PANE", "ZELLIJ"):
        env.pop(k, None)
...
def main() -> int:
    ...
    # arterm's dim_color() is rgb(80,80,80); its lightness flip is (175,175,175).
