...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Analyze arterm runtime memory JSONL logs for growth, spikes, attribution, and optimization hints"
    )
    parser.add_argument("paths", nargs="*", help="Specific JSONL files or directories to analyze")
    parser.add_argument(
        "--log-dir",
        help="Directory containing runtime memory JSONL logs (default: ~/.arterm/logs/memory or $ARTERM_HOME/logs/memory)",
    )
...
def default_log_dir() -> Path:
    arterm_home = os.environ.get("ARTERM_HOME")
    if arterm_home:
        return Path(arterm_home).expanduser() / "logs" / "memory"
    return Path.home() / ".arterm" / "logs" / "memory"
    ...
    if runaway_population:
        ...
        actions = [
            {
                ...
                "commands": ["arterm debug 'server:memory-incident'", "arterm debug 'swarm:list'"],
            },
            {
                "priority": 2,
        ...
        actions = [
            {
                ...
                "commands": ["arterm debug 'server:memory'"],
            }
        ]
        ...
        actions = [
            ...
            },
            {
                ...
                "commands": [
                    "arterm debug 'allocator:profile:on'",
                    "arterm debug 'allocator:profile:dump /tmp/arterm-server.heap'",
                ],
            },
        ]
