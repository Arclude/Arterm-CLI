#!/usr/bin/env python3
"""Analyze OpenAI persistent-websocket reuse and KV-cache effectiveness from arterm logs.
...
Use it before/after changing ``ARTERM_OPENAI_WS_IDLE_RECONNECT_SECS`` (or the
...
With no arguments it scans ~/.arterm/logs/arterm-*.log.
"""
...
def _log_files(argv: list[str]) -> list[str]:
    ...
    return sorted(glob.glob(os.path.join(home, ".arterm", "logs", "arterm-*.log")))
