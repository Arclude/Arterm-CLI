...
Counts production Rust occurrences of `pub use arterm_<crate>::*;` (cross-crate
...
`arterm_*` cross-crate wildcards are budgeted.
...
PATTERN = re.compile(r"^\s*pub\s+use\s+(?:::)?arterm_[a-z0-9_]+(?:::[A-Za-z0-9_]+)*::\*\s*;")
