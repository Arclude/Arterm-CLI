...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Profile a single resumed arterm spawn")
    parser.add_argument("--binary", default="./target/selfdev/arterm")
...
def latest_log_file(log_dir: Path) -> Path:
    logs = sorted(log_dir.glob("arterm-*.log"), key=lambda p: p.stat().st_mtime)
    if not logs:
...
def profile_single_spawn(binary: str, cwd: str, timeout_s: float) -> dict:
    root = Path(tempfile.mkdtemp(prefix="arterm-single-profile-"))
    home = root / "home"
    runtime_dir = root / "run"
    socket_path = runtime_dir / "arterm.sock"
    debug_socket_path = runtime_dir / "arterm-debug.sock"
    ...
    env.update(
        {
            ...
            "ARTERM_RUNTIME_DIR": str(runtime_dir),
            "ARTERM_SOCKET": str(socket_path),
            "ARTERM_DEBUG_SOCKET": str(debug_socket_path),
            "ARTERM_SWARM_ENABLED": "0",
            "ARTERM_NO_TELEMETRY": "1",
            "ARTERM_TRACE": "1",
            "ARTERM_TEMP_SERVER": "1",
            "ARTERM_SERVER_OWNER_PID": str(os.getpid()),
        }
    )
    ...
    try:
        ...
        while time.perf_counter() < deadline:
            ...
            if rlist:
                ...
                lower = buffer.lower()
                if b"loading session" in lower or b"arterm" in lower or len(buffer) > 4096:
