...
if [[ "$MODE" == "prepare-fast" ]]; then
    ...
    ARTERM_REMOTE_CARGO=0 scripts/dev_cargo.sh build --profile selfdev -p arterm --bin arterm
    source_bin="target/selfdev/arterm"
...
if [[ "$MODE" == "prepare-fast-macos" ]]; then
    ...
    ARTERM_RELEASE_BUILD=1 ARTERM_BUILD_SEMVER="$VERSION_NUM" \
        CARGO_INCREMENTAL=0 CARGO_BUILD_JOBS="${CARGO_BUILD_JOBS:-1}" \
        cargo build --release --target aarch64-apple-darwin --bin arterm
    source_bin="target/aarch64-apple-darwin/release/arterm"
...
if [[ "$MODE" == "fast-local" ]]; then
    ...
    [[ "$prepared_sha256" == "$actual_sha256" ]] || {
        echo "Error: target/selfdev/arterm changed after fast-release preparation." >&2
        exit 1
    }
    ...
    cp "$source_bin" "$DIST/arterm-linux-x86_64.bin"
    strip --strip-unneeded "$DIST/arterm-linux-x86_64.bin"
    chmod +x "$DIST/arterm-linux-x86_64.bin"
    cat > "$DIST/arterm-linux-x86_64" <<WRAPPER
#!/usr/bin/env sh
set -eu
export ARTERM_RUNTIME_RELEASE_SEMVER="$VERSION_NUM"
export ARTERM_RUNTIME_RELEASE_GIT_HASH="$(git rev-parse --short HEAD)"
export ARTERM_RUNTIME_RELEASE_GIT_DATE="$(git log -1 --format=%ci)"
export ARTERM_RUNTIME_RELEASE_GIT_TAG="$VERSION"
self_dir=\$(CDPATH= cd -- "\$(dirname -- "\$0")" && pwd)
exec "\$self_dir/arterm-linux-x86_64.bin" "\$@"
WRAPPER
    chmod +x "$DIST/arterm-linux-x86_64"
    file "$DIST/arterm-linux-x86_64.bin" | grep -q 'ELF 64-bit' || { echo "Error: bad Linux binary" >&2; exit 1; }
    version_output="$("$DIST/arterm-linux-x86_64" --version)"
    printf '%s\n' "$version_output" | grep -Fq "v$VERSION_NUM" || {
        echo "Error: fast binary reports the wrong version: $version_output" >&2
        exit 1
    }
    (cd "$DIST" && tar -cf - arterm-linux-x86_64 arterm-linux-x86_64.bin | gzip -1 > arterm-linux-x86_64.tar.gz)
    (cd "$DIST" && sha256sum arterm-linux-x86_64.tar.gz > SHA256SUMS)
    echo "  ✅ Linux artifact ready ($(( $(date +%s) - build_start ))s validation/package, $(du -h "$DIST/arterm-linux-x86_64.tar.gz" | cut -f1))"
    ...
    gh release upload "$VERSION" \
        "$DIST/arterm-linux-x86_64.tar.gz" \
        "$DIST/SHA256SUMS" \
        --clobber
...
if [[ "$MODE" == "fast-macos-local" ]]; then
    ...
    cp "$source_bin" "$DIST/arterm-macos-aarch64"
    chmod +x "$DIST/arterm-macos-aarch64"
    file "$DIST/arterm-macos-aarch64" | grep -q 'Mach-O 64-bit' || { echo "Error: bad macOS binary" >&2; exit 1; }
    (cd "$DIST" && tar czf arterm-macos-aarch64.tar.gz arterm-macos-aarch64)
    (cd "$DIST" && sha256sum arterm-macos-aarch64.tar.gz > SHA256SUMS)
    ...
    ensure_release_draft
    gh release upload "$VERSION" "$DIST/arterm-macos-aarch64.tar.gz" "$DIST/SHA256SUMS" --clobber
    gh release edit "$VERSION" --draft=false --latest
    echo ""
...
(
    ARTERM_RELEASE_BUILD=1 ARTERM_BUILD_SEMVER="$VERSION_NUM" scripts/build_linux_compat.sh "$DIST" >/dev/null
...
(
    ARTERM_RELEASE_BUILD=1 ARTERM_BUILD_SEMVER="$VERSION_NUM" \
        CARGO_INCREMENTAL=0 CARGO_BUILD_JOBS="${CARGO_BUILD_JOBS:-1}" \
        cargo build --release --target aarch64-apple-darwin --bin arterm 2>/dev/null
    cp target/aarch64-apple-darwin/release/arterm "$DIST/arterm-macos-aarch64"
    chmod +x "$DIST/arterm-macos-aarch64"
    (cd "$DIST" && tar czf arterm-macos-aarch64.tar.gz arterm-macos-aarch64)
...
ls -lh "$DIST"/*.tar.gz
file "$DIST/arterm-linux-x86_64.bin" | grep -q 'ELF 64-bit' || { echo "Error: bad Linux binary"; exit 1; }
head -1 "$DIST/arterm-linux-x86_64" | grep -q '^#!/' || { echo "Error: bad Linux wrapper"; exit 1; }
file "$DIST/arterm-macos-aarch64" | grep -q 'Mach-O 64-bit' || { echo "Error: bad macOS binary"; exit 1; }
...
gh release upload "$VERSION" \
    "$DIST/arterm-linux-x86_64.tar.gz" \
    "$DIST/arterm-macos-aarch64.tar.gz" \
    --clobber
