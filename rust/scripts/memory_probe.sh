#!/usr/bin/env bash
# memory_probe.sh - Reproducible memory probe harness for arterm TUI clients.
#
...
#   --binary <path>   Client binary (default: ~/.arterm/builds/current/arterm)
#   --arterm <path>    arterm CLI used for `arterm debug ...` (default: ~/.local/bin/arterm)
...
#
# Requirements: a running arterm server with the debug socket enabled, jq,
...
CLIENT_BIN="$HOME/.arterm/builds/current/arterm"
TESTER_CWD="$HOME"
SKIP_TRIM=0
KEEP_TESTER=0
...
[[ -x "$ARTERM_BIN" ]] || die "arterm CLI not found at $ARTERM_BIN"
[[ -x "$CLIENT_BIN" ]] || CLIENT_BIN="$(command -v arterm)" || die "client binary not found"
[[ -f "$HOME/.arterm/sessions/${SESSION_ID}.json" ]] \
    || log "WARN: $HOME/.arterm/sessions/${SESSION_ID}.json not found; resume may create a new session"

RUN_ID="probe_$(date -u +%Y%m%dT%H%M%SZ)_$$"
SESSION_FILE="$HOME/.arterm/sessions/${SESSION_ID}.json"
...
cleanup() {
    ...
    if [[ -n "$TESTER_ID" && "$KEEP_TESTER" -ne 1 ]]; then
        "$ARTERM_BIN" debug "tester:${TESTER_ID}:stop" >/dev/null 2>&1 || true
...
# ===========================================================================
WRAPPER="$(mktemp /tmp/arterm_memory_probe_wrapper.XXXXXX.sh)"
cat > "$WRAPPER" <<EOF
#!/usr/bin/env bash
...
SPAWN_OUT="$("$ARTERM_BIN" debug "tester:spawn {\"binary\":\"$WRAPPER\",\"cwd\":\"$TESTER_CWD\",\"cols\":120,\"rows\":40}")" \
...
TESTER_INFO="$("$ARTERM_BIN" debug tester:list | jq -c --arg id "$TESTER_ID" '.[] | select(.id == $id)')"
