#!/usr/bin/env python3
"""Report compile-time isolation risks in the Arterm crate graph.
...
WATCHED_CRATES = ["arterm-base", "arterm-app-core", "arterm-tui", "arterm"]
...
def target_state_violations(stats_by_name: dict[str, CrateStats]) -> list[str]:
    ...
    tui = stats_by_name.get("arterm-tui")
    if tui and "arterm-app-core" in tui.normal_workspace_deps:
        violations.append("target-state: arterm-tui still directly depends on arterm-app-core")

    app_core = stats_by_name.get("arterm-app-core")
    if app_core and "arterm-base" in app_core.normal_workspace_deps:
        violations.append("target-state: arterm-app-core still directly depends on arterm-base")

    base = stats_by_name.get("arterm-base")
    if base:
        for dep in base.normal_workspace_deps:
            if dep in {
                "arterm-azure-auth",
                "arterm-provider-gemini",
                "arterm-provider-openai",
                "arterm-provider-openrouter",
                "arterm-notify-email",
                "arterm-build-support",
            }:
                violations.append(f"target-state: arterm-base still depends on leaf/runtime crate {dep}")
        for dep in base.normal_external_deps:
            if dep.startswith("aws-") or dep in {"aws-types"}:
                violations.append(f"target-state: arterm-base still depends directly on AWS crate {dep}")

    for crate in stats_by_name.values():
        for glob in crate.glob_reexports:
            if crate.name in {"arterm", "arterm-app-core", "arterm-tui"}:
