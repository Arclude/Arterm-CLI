...
fn test_ambient_state_lifecycle() {
    use arterm::ambient::{AmbientCycleResult, AmbientState, AmbientStatus, CycleStatus};
...
fn test_ambient_scheduled_queue() {
    use arterm::ambient::{Priority, ScheduledItem, ScheduledQueue};

    let tmp = std::env::temp_dir().join("arterm-test-queue.json");
    ...
    queue.push(ScheduledItem {
        ...
        target: arterm::ambient::ScheduleTarget::Ambient,
...
fn test_adaptive_scheduler_intervals() {
    use arterm::ambient_scheduler::{AdaptiveScheduler, AmbientSchedulerConfig};
...
async fn test_ambient_end_cycle_tool() -> Result<()> {
    ...
    let result = arterm::tool::ambient::take_cycle_result();
...
async fn test_ambient_request_permission_tool() -> Result<()> {
    ...
    arterm::tool::ambient::register_ambient_session(ambient_session_id.clone());

    let response = agent.run_once_capture("Request permission").await?;
    arterm::tool::ambient::unregister_ambient_session(&ambient_session_id);
...
async fn test_ambient_runner_status() {
    use arterm::ambient_runner::AmbientRunnerHandle;
...
async fn test_ambient_runner_trigger_and_stop() {
    use arterm::ambient::AmbientStatus;
    use arterm::ambient_runner::AmbientRunnerHandle;
    use arterm::safety::SafetySystem;
...
fn test_memory_reinforcement_provenance() {
    use arterm::memory::{MemoryCategory, MemoryEntry};
...
fn test_ambient_config_defaults() {
    use arterm::config::AmbientConfig;
...
fn test_ambient_lock() {
    use arterm::ambient::AmbientLock;
    let _env = setup_test_env().expect("failed to setup isolated ARTERM_HOME");
...
async fn test_full_ambient_cycle_simulation() -> Result<()> {
    ...
    agent.set_system_prompt("You are the arterm ambient maintenance agent.");
