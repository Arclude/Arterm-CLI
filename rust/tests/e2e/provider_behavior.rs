...
async fn test_resume_restores_model_and_tool_history() -> Result<()> {
    ...
    session.add_message(
        arterm::message::Role::User,
        vec![arterm::message::ContentBlock::Text {
            text: "Run a tool".to_string(),
            cache_control: None,
        }],
    );
    session.add_message(
        arterm::message::Role::Assistant,
        vec![
            arterm::message::ContentBlock::Text {
                text: "Running...".to_string(),
                cache_control: None,
            },
            arterm::message::ContentBlock::ToolUse {
                id: "tool-1".to_string(),
                name: "bash".to_string(),
    ...
    session.add_message(
        arterm::message::Role::User,
        vec![arterm::message::ContentBlock::ToolResult {
...
async fn test_resume_session_with_local_history_uses_metadata_only_history() -> Result<()> {
    ...
    let provider_dyn: Arc<dyn arterm::provider::Provider> = provider.clone();
...
async fn test_resume_session_reports_reload_interruption_for_peer_sessions() -> Result<()> {
    let _env = setup_test_env()?;
    let runtime_dir = short_runtime_dir(format!(
        "arterm-reload-interruption-test-{}",
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
            .as_nanos()
...
/// The agent should identify as "arterm" or just a generic "coding assistant powered by Claude"
#[tokio::test]
async fn test_system_prompt_no_claude_code_identity() -> Result<()> {
    ...
    let provider_dyn: Arc<dyn arterm::provider::Provider> = provider;
    ...
    // Should identify as arterm
    assert!(
        lower_identity.contains("arterm"),
        "System prompt should identify as arterm. Found: {}",
        identity_portion
    );
