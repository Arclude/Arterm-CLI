...
impl SpawnedWindowsServer {
    fn arterm_binary() -> std::path::PathBuf {
        ...
        std::path::PathBuf::from(env!("CARGO_BIN_EXE_arterm"))
    }
    ...
    fn spawn(prefix: &str) -> Result<Self> {
        ...
        let socket_path = runtime_dir.join("arterm-windows-lifecycle.sock");
        let debug_socket_path = runtime_dir.join("arterm-windows-lifecycle-debug.sock");

        let stdout_file = std::fs::File::create(&stdout_path)?;
        let stderr_file = std::fs::File::create(&stderr_path)?;
        let mut command = Command::new(Self::arterm_binary());
        command
            .arg("--no-update")
            .arg("--socket")
            .arg(&socket_path)
            ...
            .env("ARTERM_OPENAI_COMPAT_API_BASE", "http://127.0.0.1:9/v1")
            .env("ARTERM_OPENAI_COMPAT_DEFAULT_MODEL", "windows-e2e-model")
            .env("ARTERM_OPENAI_COMPAT_LOCAL_ENABLED", "1")
    ...
    fn apply_env<'a>(&self, command: &'a mut Command) -> &'a mut Command {
        command
            .env_remove("ARTERM_TEST_SESSION")
            .env("ARTERM_HOME", &self.home_dir)
            .env("ARTERM_RUNTIME_DIR", &self.runtime_dir)
            .env("ARTERM_INSTALL_DIR", &self.install_dir)
            .env("ARTERM_NO_TELEMETRY", "1")
            .env("ARTERM_OPENAI_COMPAT_API_BASE", "http://127.0.0.1:9/v1")
            .env("ARTERM_OPENAI_COMPAT_DEFAULT_MODEL", "windows-e2e-model")
            .env("ARTERM_OPENAI_COMPAT_LOCAL_ENABLED", "1")
    ...
    fn arterm_command(&self) -> Command {
        let mut command = Command::new(Self::arterm_binary());
    ...
    fn dump_logs(&self, label: &str) {
        ...
        if let Ok(artifact_root) = std::env::var("ARTERM_E2E_ARTIFACT_DIR") {
            ...
            if let Ok(entries) = std::fs::read_dir(logs_dir) {
                let copied_logs_dir = artifact_dir.join("arterm-logs");
...
async fn windows_binary_server_accepts_clients_and_debug_cli() -> Result<()> {
    ...
    let server = SpawnedWindowsServer::spawn("arterm-windows-lifecycle-")?;
    ...
    let result = async {
        ...
        anyhow::ensure!(
            info.get("debug_control_enabled")
                ...
                == Some(true),
            "server should honor ARTERM_DEBUG_CONTROL in Windows e2e"
        );
...
async fn windows_binary_server_rebinds_named_pipe_after_exit() -> Result<()> {
    let _env = setup_test_env()?;
    let mut first = SpawnedWindowsServer::spawn("arterm-windows-rebind-")?;
