...
Uses kitty remote control to launch a real arterm TUI window, prepares the same
...
def main():
    ...
    binary = os.environ.get("ARTERM_E2E_BIN", str(REPO / "target" / "selfdev" / "arterm"))
    kitty_sock = os.environ.get("KITTY_E2E_SOCKET", find_kitty_socket())
    title = f"ARTERM_EXPAND_E2E_{int(time.time())}"
