...
This starts a real arterm TUI client inside a pseudo-terminal, prepares a real
...
Run from repo root after building arterm:
...
SOCKET_PATH = os.path.join(RUNTIME_DIR, "arterm-debug.sock")
...
def main():
    binary = os.environ.get("ARTERM_E2E_BIN")
    if not binary:
        candidates = [
            REPO / "target" / "selfdev" / "arterm",
            Path.home() / ".arterm" / "builds" / "current" / "arterm",
        ]
        binary = next((str(p) for p in candidates if p.exists()), None)
    if not binary:
        raise SystemExit("No arterm binary found. Set ARTERM_E2E_BIN or build first.")
    ...
    try:
        ...
        env.setdefault("ARTERM_CLIENT_SELFDEV_MODE", "1")
