...
impl TestEnv {
    fn new() -> Result<Self> {
        ...
        arterm::env::set_var("ARTERM_HOME", temp.path());
        arterm::config::invalidate_config_cache();
    ...
    fn config_dir(&self) -> PathBuf {
        self.temp.path().join("config").join("arterm")
    }
    ...
    fn clear_profile_keys(&self) {
        arterm::env::remove_var("OPENROUTER_API_KEY");
        for profile in openai_compatible_profiles() {
            arterm::env::remove_var(profile.api_key_env);
        }
        AuthStatus::invalidate_cache();
    }
}
...
fn write_profile_api_key_file(
    ...
    arterm::env::remove_var(&resolved.api_key_env);
    AuthStatus::invalidate_cache();
    Ok(())
}
...
fn seed_non_compatible_auto_auth(provider: LoginProviderDescriptor) -> bool {
    match provider.target {
        LoginProviderTarget::Claude => {
            arterm::env::set_var("ANTHROPIC_API_KEY", "test-anthropic-key");
            true
        }
        LoginProviderTarget::OpenAiApiKey => {
            arterm::env::set_var("OPENAI_API_KEY", "sk-test-openai-key");
            true
        }
        LoginProviderTarget::OpenRouter => {
            arterm::env::set_var("OPENROUTER_API_KEY", "sk-test-openrouter-key");
            true
        }
        LoginProviderTarget::Copilot => {
            arterm::env::set_var("COPILOT_GITHUB_TOKEN", "gho_test-copilot-token");
            true
        }
        LoginProviderTarget::Cursor => {
            arterm::env::set_var("CURSOR_API_KEY", "sk-test-cursor-key");
            true
        }
        _ => false,
    }
        ...
        for has_key in [false, true] {
            for has_default_model in [false, true] {
                for restarted in [false, true] {
                    ...
                    assert_eq!(
                        arterm::provider::openrouter::has_credentials(),
                        expected_configured,
...
fn provider_matrix_env_credentials_activate_openrouter_runtime() -> Result<()> {
    ...
    for &profile in openai_compatible_profiles() {
        ...
        let resolved = resolve_openai_compatible_profile(profile);
        arterm::env::set_var(&resolved.api_key_env, "matrix-env-secret");
        AuthStatus::invalidate_cache();
...
fn provider_matrix_custom_compat_overrides_flow_into_runtime() -> Result<()> {
    ...
    arterm::env::set_var("ARTERM_OPENAI_COMPAT_API_KEY_NAME", "GROQ_API_KEY");
    arterm::env::set_var("ARTERM_OPENAI_COMPAT_ENV_FILE", "groq.env");
    arterm::env::set_var("ARTERM_OPENAI_COMPAT_DEFAULT_MODEL", "openai/gpt-oss-120b");
    ...
    assert!(arterm::provider::openrouter::has_credentials());
    OpenRouterProvider::new()?;
    assert_eq!(
        AuthStatus::check().openrouter,
        AuthState::NotConfigured,
...
fn provider_matrix_custom_local_compat_without_api_key_activates_openrouter_runtime() -> Result<()>
{
    ...
    arterm::env::set_var("ARTERM_OPENAI_COMPAT_API_BASE", "http://localhost:11434/v1");
