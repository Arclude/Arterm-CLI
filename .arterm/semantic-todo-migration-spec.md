...
## Enums (all: snake_case string serde, Ord by declaration order, in arterm-task-types)
...
## Gate semantics (arterm-base todo.rs, app-core, tui)
...
predicates exported from arterm-base::todo, e.g.:
...
- Telemetry: TelemetryScoreSummary stays u8-based; map each enum to a
  ...
  This keeps arterm-telemetry-core, usage-types, and the worker schema untouched.
...
- Commands: `cargo test -p arterm-task-types -p arterm-base`,
  `cargo test -p arterm-app-core todo`, `cargo test -p arterm-tui todo`,
  `cargo check -p arterm-tui -p arterm-telemetry-core`.
